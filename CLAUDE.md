...
| 영역 | 주체 | 비고 |
|---|---|---|
| `write/` (학습 자료 본문) | **사용자가 직접** | Claude 는 본문 텍스트만 드림. 사용자가 직접 수정해 자료에 박음 |
| `project/` (학습 프로젝트 코드) | **Claude 가 직접** | Edit/Write/Bash 로 직접 수정. 빌드·테스트 검증까지 책임 |
| `journal/` (일지·리뷰) | **사용자가 직접** | 자기 메모라 톤이 더 자유롭다 |
| `issue/` (장애 보고서) | **사용자가 직접** | 기존 보고서 1건의 형식을 SSOT 로 따름 |
| 저장소 메타 (`CLAUDE.md`·`STUDY_INDEX.md`·`README.md`·frontmatter 정합 등) | **사용자가 명시적 지시 시 Claude 가 직접** | "너가 직접 해" 같은 명시 지시 없으면 본문 텍스트만 드림 |

이 분담은 *기본값* 이며 사용자가 *명시적으로* "너가 직접 해" / "직접 수정해" 같은 지시를 주면 Claude 가 그 자리에서 본문을 직접 Edit/Write 한다.
