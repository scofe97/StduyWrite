...
이 카테고리는 정적 분석 도구로서의 SonarQube 자체를 다룬다. CI/CD 파이프라인 그루비 작성법은 `02_Jenkins/`로, 아티팩트 저장소 운영은 `03_Nexus/`로 분담한다. SonarQube 분석 결과를 노출하는 대시보드 운영은 SonarQube 내장 UI를 기준으로 본 카테고리에서 다루지만, Prometheus/Grafana로 SonarQube 자체 메트릭을 적재하는 경우는 `06_observability/`로 넘긴다.
