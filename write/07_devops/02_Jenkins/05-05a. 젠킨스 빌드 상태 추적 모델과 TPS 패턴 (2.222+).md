...
> **본 문서는 spec(`05-05.md`)을 읽었다고 가정한 운영 해석과 TPS 매핑 패턴**이다. 상태 조회 endpoint(`/api/json`, `wfapi/describe`, `/queue/api/json`)의 호출 형식은 spec에 있다. 이 문서는 그 위에서 TPS가 DB·매핑·폴링을 어떻게 설계하는지를 정리한다.
>
> Blue Ocean/Workflow API 현대화 판단: `05-05b.md`. 인증 모델 차이가 POST에 미치는 영향: `05-02a.md`.
