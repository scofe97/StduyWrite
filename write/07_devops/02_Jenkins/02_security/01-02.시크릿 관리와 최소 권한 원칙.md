...
#### 실무에서 본 두 변종 — URL 임베디드와 -D 인자

이 패턴은 교과서 밖에서 더 교묘하게 나타납니다. 같은 파이프라인이 Harbor·MinIO·SSH 자격증명은 `withCredentials`로 잘 빼 놓고도, 두 곳에서 평문이 새는 경우를 실제로 봤습니다.

```groovy
// ❌ 변종 A: Git 저장소 URL에 계정·비밀번호를 박음
checkout([$class: 'GitSCM', branches: [[name: '*/dev']],
    userRemoteConfigs: [[url: 'http://root:cloud1234@gitlab.example.cloud/demo/app.git', credentialsId: '']]])

// ❌ 변종 B: 분석 토큰을 -D 인자에 직접
sh "./gradlew sonar -Dsonar.token='squ_f8455a...'"
```

변종 A는 `credentialsId`를 빈 문자열로 두고 URL에 `root:cloud1234`를 박았습니다. 이러면 토큰이 config.xml·빌드 로그·`git` 명령 출력 어디로든 샙니다. 변종 B는 `withSonarQubeEnv`나 `withCredentials`를 거치지 않아 토큰이 인자에 그대로 노출됩니다. **다른 시크릿을 제대로 빼 놨다는 사실이 이 두 곳의 평문을 가려 줍니다** — "우리는 크레덴셜 쓰고 있어"라는 인식이 오히려 점검을 무디게 합니다.

올바른 형태는 GitSCM은 `credentialsId`에 등록된 크레덴셜을 지정하고(URL에서 계정 제거), 토큰은 `withCredentials`로 환경변수에 주입해 `-Dsonar.token="$SONAR_TOKEN"`처럼 셸 치환으로 참조하는 것입니다. 점검 방법은 단순합니다 — config.xml과 Jenkinsfile에서 `://[^/]*:.*@`(URL 임베디드 자격증명)와 `token=['\"]?[a-z]` 같은 패턴을 grep해, 사람이 읽을 수 있는 시크릿이 박혀 있는지 봅니다.
