...
> 이 점검 문서는 보안 단계(인증·인가, 시크릿 관리)를 다 읽은 뒤 스스로를 시험하기 위한 자가 점검입니다. 먼저 §면접 질문만 보고 답을 떠올린 뒤, §정답 절에서 같은 번호로 대조하세요.
...
1. Jenkins 보안 베스트 프랙티스 5가지는 무엇입니까? *(심화: 심층 방어(Defense in Depth) 관점에서 보안 레이어 중 하나가 뚫렸을 때 나머지 레이어가 어떻게 피해를 제한합니까? 시나리오를 들어 설명할 수 있습니까?)*
2. Jenkins RBAC 구현에서 최소 권한 원칙을 어떻게 적용합니까? *(심화: 개발자가 Jenkinsfile에서 `sh 'curl http://internal-api/admin'` 같은 명령을 실행하면 RBAC만으로 방어할 수 있습니까? 추가로 필요한 보안 조치는 무엇입니까?)*
3. Jenkins Credentials Store와 HashiCorp Vault 중 무엇을 선택합니까? *(심화: Jenkins Credentials Store의 마스터 키(`$JENKINS_HOME/secrets/`)가 유출되면 어떤 일이 발생하며, 이를 방지하기 위한 조치는 무엇입니까?)*
4. Jenkins 감사 로그(Audit Trail)가 필요한 이유는 무엇입니까? *(심화: 감사 로그의 무결성(Integrity)을 보장하기 위해 어떤 조치를 취할 수 있습니까?)*
...
공식 문서는 여기에 두 가지 축을 더합니다. 첫째, Jenkins 의 [Controller Isolation](https://www.jenkins.io/doc/book/security/controller-isolation/) 은 빌드를 Controller 가 아니라 분산 Agent 에서 돌리게 해 빌드 스크립트가 `JENKINS_HOME` 에 손대지 못하게 막는 것을 *전제* 로 둡니다. 둘째, 같은 Agent 를 공유하면 *빌드끼리도* 격리가 필요합니다 — 한 팀의 빌드가 다른 팀 소스나 산출물을 건드리거나, 공개 저장소의 Pull Request 빌드가 다른 잡 데이터에 접근하는 경우입니다. Jenkins 는 ① 빌드마다 새 Agent 를 만드는 클라우드 프로바이더, ② Job Restrictions 류 플러그인으로 노드별 실행 잡 제한을 권고합니다([Securing Builds](https://www.jenkins.io/doc/book/security/securing-builds/)). PR 의 경우 fork 가 `Jenkinsfile` 자체를 바꿔 보호를 우회하지 못하도록 *fork PR 신뢰 정책*(기본 Nobody — fork 가 아닌 대상 브랜치에서 파일을 읽음)을 둡니다.
...
### 정답 3 — Credentials Store vs Vault 선택
...
> 이 점검 문서는 01장 인증·인가의 두 본편(01-01·01-02)을 다룹니다. 막힌 질문이 있으면 아래 해당 편으로 돌아가 해당 절을 다시 읽으면 동선이 이어집니다.

- [01-01. 인증과 인가 — 누가 무엇을 할 수 있는가](01-01.인증과%20인가%20%E2%80%94%20누가%20무엇을%20할%20수%20있는가.md) § "Authorization Strategy" — RBAC·API Token·Item Roles(정답 1·2 연계)
- [01-02. 시크릿 관리와 최소 권한 원칙](01-02.시크릿%20관리와%20최소%20권한%20원칙.md) § "Credentials Store" — Scope·Domain·최소 권한(정답 3 연계)
