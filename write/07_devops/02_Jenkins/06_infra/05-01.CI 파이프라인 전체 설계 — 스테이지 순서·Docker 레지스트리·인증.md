...
> [06-06. Artifactory 연동](02-03.Artifactory%20%EC%97%B0%EB%8F%99%20%E2%80%94%20%EC%95%84%ED%8B%B0%ED%8C%A9%ED%8A%B8%20%EC%A0%80%EC%9E%A5%EC%86%8C.md)은 Artifactory를 jar 아티팩트 저장소로 다뤘습니다. 같은 Artifactory가 *Docker 레지스트리*로도 동작합니다.
...
tier가 여럿이면 frontend·backend·db 각각을 build number로 태그해 push합니다. 그러면 `frontend:42`·`backend:42`처럼 같은 빌드에서 나온 이미지 세트를 명확히 식별할 수 있습니다. CI/CD 파이프라인이 push·pull하려면 Artifactory 크레덴셜(사용자명·비밀번호)과 repo 이름이 Jenkins 크레덴셜로 등록되어 있어야 합니다. 상세 설정은 [06-06. Artifactory 연동](02-03.Artifactory%20%EC%97%B0%EB%8F%99%20%E2%80%94%20%EC%95%84%ED%8B%B0%ED%8C%A9%ED%8A%B8%20%EC%A0%80%EC%9E%A5%EC%86%8C.md)에서 다룹니다.
...
K8s 동적 agent에서 Kaniko는 Docker 데몬 없이 이미지를 빌드합니다(빌드 메커니즘 상세는 [../03_agent/01-04.컨테이너%20이미지%20빌드.md](../03_agent/01-04.%EC%BB%A8%ED%85%8C%EC%9D%B4%EB%84%88%20%EC%9D%B4%EB%AF%B8%EC%A7%80%20%EB%B9%8C%EB%93%9C.md)에서 다룹니다). Kaniko Pod가 빌드한 이미지를 Artifactory에 push하려면 레지스트리 인증이 필요합니다. 이 인증 정보를 K8s **docker-registry Secret**으로 만들어 Kaniko Pod에 마운트합니다.
...
Kaniko 빌드 메커니즘(daemon-free, snapshot 방식) 자체는 [../03_agent/01-04.컨테이너 이미지 빌드](../03_agent/01-04.%EC%BB%A8%ED%85%8C%EC%9D%B4%EB%84%88%20%EC%9D%B4%EB%AF%B8%EC%A7%80%20%EB%B9%8C%EB%93%9C.md)에서 다룹니다. 이 편은 "어느 namespace에 어떤 이름으로 Secret을 만드는가"에 집중합니다.



## 5. 빌드 경로 선택 — Kaniko냐 docker build냐

> 이미지를 만들어 push한다는 목표는 같아도, 빌드 도구는 실행 환경이 가릅니다. 데몬 없는 K8s면 Kaniko, 데몬이 있는 노드면 docker build입니다.

위까지는 K8s 동적 agent에서 Kaniko로 빌드하는 경로였습니다. 그런데 모든 빌드가 K8s에서 도는 건 아닙니다. Docker 데몬이 떠 있는 VM·노드에서는 Pipeline의 `docker` DSL로 평범하게 `docker build`·`docker push`를 합니다. 같은 "이미지 빌드"라도 두 경로가 환경에 따라 갈립니다.

| 축 | Kaniko | docker build (docker DSL) |
|----|--------|---------------------------|
| 전제 환경 | K8s, Docker 데몬 없음 | Docker 데몬 있는 노드/VM |
| 빌드 방식 | 데몬리스, 레이어 snapshot | 데몬에 빌드 요청 |
| 인증 | docker-registry Secret 마운트 | `docker.withRegistry(url, credId)` |
| 빌드 환경 격리 | Pod 컨테이너 | `docker.image(img).inside()` |

docker DSL 경로의 인증·빌드 골격은 이렇습니다.

```groovy
docker.withRegistry(registryUrl, credentialsId) {   // 인증 감싸기
    docker.image(baseImg).inside('--network=host') { sh './build.sh' }  // 빌드 도구 이미지 안에서 실행
    sh 'docker build -f Dockerfile -t "$IMAGE" .'
    sh 'docker push "$IMAGE"'
}
```

`docker.withRegistry`가 Kaniko의 Secret 마운트에 대응하는 인증 처리이고, `docker.image().inside()`는 빌드 도구가 든 이미지를 띄워 그 안에서 빌드해(노드에 도구를 깔지 않아도 됩니다) Pod 격리에 대응합니다. 어느 경로든 결과(레지스트리에 push된 이미지)는 같으니, 선택 기준은 "빌드 노드에 Docker 데몬이 있는가"입니다. docker DSL 빌드의 실제 Jenkinsfile 형태는 [`06-11`](05-02.첫%20CI%20Jenkinsfile%20구현%20%E2%80%94%20완성%20코드%C2%B7Multibranch%C2%B7Blue%20Ocean.md) §5에서 이어 봅니다.
...
- [06-04. GitHub 연동 — 플러그인·PAT·웹훅](02-01.GitHub%20%EC%97%B0%EB%8F%99%20%E2%80%94%20%ED%94%8C%EB%9F%AC%EA%B7%B8%EC%9D%B8%C2%B7PAT%C2%B7%EC%9B%B9%ED%9B%85.md) § "webhook" — push·PR 이벤트로 파이프라인을 시작하는 설정
- [06-05. SonarQube 연동 — 정적분석 게이트](02-02.SonarQube%20%EC%97%B0%EB%8F%99%20%E2%80%94%20%EC%A0%95%EC%A0%81%EB%B6%84%EC%84%9D%20%EA%B2%8C%EC%9D%B4%ED%8A%B8.md) § "Quality Gate" — SCA 분석과 게이트 차단 설정 상세
- [06-06. Artifactory 연동 — 아티팩트 저장소](02-03.Artifactory%20%EC%97%B0%EB%8F%99%20%E2%80%94%20%EC%95%84%ED%8B%B0%ED%8C%A9%ED%8A%B8%20%EC%A0%80%EC%9E%A5%EC%86%8C.md) § `rtUpload` — jar·이미지 저장소 설정과 크레덴셜 등록
- [../03_agent/01-04. 컨테이너 이미지 빌드](../03_agent/01-04.%EC%BB%A8%ED%85%8C%EC%9D%B4%EB%84%88%20%EC%9D%B4%EB%AF%B8%EC%A7%80%20%EB%B9%8C%EB%93%9C.md) § "Kaniko" — daemon-free 빌드 메커니즘과 Pod template 설정
