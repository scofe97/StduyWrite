...
flowchart TB
    ...
    subgraph DaemonFree["daemon-free"]
        KN["Kaniko<br>archive(2025-06)<br>사용자 공간 빌드"]
...
| 2026 상태 | 활성 | 활성 | 활성 | archive(2025-06) |

**굵은 결론**: VM 이면 Docker, K8s 에서 속도가 우선이면 BuildKit, rootless 강제이면 Buildah — 그리고 Kaniko 는 archive 이므로 신규 도입 대상에서 제외합니다.



## 7. OCI 관점의 근본 차이 — 산출물은 같고, RUN 실행 모델이 다르다

> 본 절은 "Docker 도 Kaniko 도 결국 OCI 이미지를 만든다면 무엇이 근본적으로 다른가" 라는 질문에 답합니다. 차이는 결과물(image-spec)이 아니라 *빌드 중 실행(runtime-spec)* 계층에서 납니다.

OCI(Open Container Initiative) 는 컨테이너 표준을 세 스펙으로 나눠 둡니다(출처: opencontainers.org).

| 스펙 | 규정 범위 | 비고 |
|------|----------|------|
| image-spec | 이미지 포맷 — manifest·config·레이어 구조 | 레지스트리에 저장되는 *결과물* 의 모양 |
| runtime-spec | 언팩된 파일시스템 번들을 *실행* 하는 방법 | 참조 구현이 runc |
| distribution-spec | 레지스트리 push/pull API | `docker push` 가 말하는 프로토콜 |

산출물 차원에서는 두 도구가 구분되지 않습니다. Kaniko 가 만든 이미지의 manifest media type 은 OCI(`application/vnd.oci.image.manifest.v1+json`) 또는 Docker v2(`application/vnd.docker.distribution.manifest.v2+json`) 형식이고, `--oci-layout-path` 플래그로 OCI 이미지 레이아웃 출력도 지원합니다(출처: github.com/GoogleContainerTools/kaniko). 레지스트리와 런타임 입장에서 "Kaniko 가 구운 이미지" 라는 표식은 어디에도 없습니다.

근본 차이는 빌드 *과정* 이 runtime-spec 계층을 쓰느냐에서 납니다. Docker 의 빌더 BuildKit — Engine 23.0 부터 Linux 기본 빌더입니다(출처: docs.docker.com/engine/release-notes/23.0) — 은 buildkitd 의 기본 워커가 OCI(runc) 워커라서, Dockerfile 의 RUN 한 단계를 *runc 컨테이너를 띄워* 실행합니다(출처: github.com/moby/buildkit). 빌드 중에도 runtime-spec 위에서 도는 셈이고, 그래서 daemon 과 권한이 필요해지는 대신 RUN 단계마다 커널 수준 격리를 얻습니다. Kaniko 는 이 계층을 아예 쓰지 않습니다. executor 프로세스가 명령을 자기 컨테이너 안에서 직접 실행하고 userspace 스냅샷으로 레이어를 조립합니다(출처: github.com/GoogleContainerTools/kaniko). privileged 가 필요 없는 이유와 빌드 단계 격리를 제공하지 못하는 이유(§8)는 *같은 설계의 양면* 입니다.

![OCI 세 스펙 위의 Docker와 Kaniko 위치](_assets/01-03.oci-spec-position.svg)

runtime-spec 계층 차이에서 파생되는 부수 차이도 정리해 둡니다.

| 축 | Docker (BuildKit) | Kaniko |
|----|-------------------|--------|
| multi-arch manifest | 멀티 플랫폼 빌드 네이티브 지원 | 미지원 — 아키텍처별 빌드 후 manifest 도구로 묶음 |
| 산출 형태 | 레지스트리 push·OCI tarball·로컬 디렉토리 등 익스포터 다양 | 레지스트리 push 기본, `--no-push`/`--tar-path` 예외 |
| 실행 전제 | daemon 또는 rootless buildkitd | 공식 executor 이미지 안에서만 실행 지원 |

(출처: github.com/moby/buildkit, github.com/GoogleContainerTools/kaniko)

Jenkins 파이프라인 운영 동선(인증·캐시·푸시)에서 이 차이가 어떻게 나타나는지는 [01-04. 컨테이너 이미지 빌드](01-04.컨테이너%20이미지%20빌드.md) § "2.2 같은 Dockerfile, 다른 실행 경로" 에서 다룹니다.



## 8. Kaniko 보안 캐비엇 — "런타임 격리에 의존"
...
6. Docker(BuildKit) 와 Kaniko 는 둘 다 OCI 호환 이미지를 만드는데, "근본적 차이" 는 OCI 세 스펙 중 어느 계층에서 납니까? *(심화: 그 차이가 권한 요구·빌드 단계 격리와 어떻게 연결됩니까?)*
...
6. Docker(BuildKit) 는 RUN 단계를 OCI 런타임 `(______)` 컨테이너로 실행하지만, Kaniko 는 OCI 세 스펙 중 `(______)` 계층을 우회한 채 executor 프로세스가 직접 실행합니다.
...
### 정답 6 — 근본 차이는 runtime-spec 계층

**runtime-spec 계층** 입니다. 산출물은 둘 다 image-spec 호환 이미지라 레지스트리·런타임 입장에서 구분되지 않습니다. 갈리는 것은 빌드 *과정* — Docker(BuildKit) 는 RUN 단계를 runc(runtime-spec 의 참조 구현) 컨테이너로 띄워 실행하고, Kaniko 는 runtime-spec 계층을 아예 쓰지 않은 채 executor 프로세스가 직접 실행해 userspace 스냅샷으로 레이어를 만듭니다. (출처: github.com/moby/buildkit, github.com/GoogleContainerTools/kaniko)

### 정답 6 심화 — 권한·격리로의 연결

같은 설계의 양면입니다. 빌드 중 컨테이너를 띄우려면(BuildKit) daemon 과 그에 따른 권한이 필요하지만 RUN 단계마다 커널 수준 격리를 얻습니다. 컨테이너를 안 띄우면(Kaniko) privileged 가 필요 없어 restricted 프로파일에서 돌지만, 빌드 단계를 격리해 줄 주체도 사라져 격리가 통째로 바깥 컨테이너 런타임의 몫이 됩니다(§8 캐비엇). "privileged 불필요" 와 "격리 미제공" 은 한 결정에서 같이 나온 결과입니다.
...
6. `runc` / `runtime-spec` — BuildKit 의 기본 워커가 OCI(runc) 워커이고, Kaniko 는 runtime-spec 을 우회해 userspace 에서 직접 실행합니다 (출처: github.com/moby/buildkit, github.com/GoogleContainerTools/kaniko).



## 관련 문서

> 이 편은 "어떤 빌드 도구를 선택하는가"라는 결정 문제를 다룹니다. 선택한 도구로 실제 빌드하는 메커니즘은 01-04(컨테이너 이미지 빌드)에서, VM 소켓 보안은 01-05에서, 선택 결과를 실제로 적용하는 K8s 빌드 환경은 02-01에서 이어집니다.

  - [01-04. 컨테이너 이미지 빌드](01-04.컨테이너%20이미지%20빌드.md) — 컨테이너 빌드 메커니즘 + § "2.2 같은 Dockerfile, 다른 실행 경로" (Docker vs Kaniko 운영 차이)
  - [01-05. VM Jenkins에서의 Docker 보안 모델](01-05.VM%20Jenkins에서의%20Docker%20보안%20모델.md) — VM socket 보안
