---
title: 점검 — Kubernetes Jenkins 핵심 질문
...
# 점검 — Kubernetes Jenkins 핵심 질문

---

> 이 점검 문서는 02장(Kubernetes Agent) 을 다 읽은 뒤 스스로를 시험하기 위한 자가 점검입니다. 먼저 §면접 질문만 보고 답을 떠올린 뒤, §정답 절에서 같은 번호로 대조하세요.
...
> 7개 질문(Q1~Q7)이 왜 K8s인가·동적 Agent 흐름·Pod 구성과 영속성·보안·도구 선택의 흐름으로 어떻게 이어지는지 색상별로 묶어 보여줍니다.
...
이미지 Pull 병목은 *이미지가 노드에 도달하는 시간* 을 줄여서 해결합니다. (a) **이미지 슬림화** — Multi-stage 빌드로 Agent 이미지를 작게 만들어 pull 시간 단축. (b) **노드 로컬 캐시** — 자주 쓰는 Agent 이미지를 노드에 미리 pull(DaemonSet warmer 또는 `imagePullPolicy: IfNotPresent` + 사전 배포). (c) **내부 레지스트리 미러** — Docker Hub 대신 클러스터 내부/근접 레지스트리(Harbor 등) 에서 pull 해 네트워크 지연 감소. (d) **registry mirror + 레이어 캐시 공유**.
...
> DinD·DooD 가 호스트 소켓·privileged 를 요구하는 반면 Kaniko 는 daemon-free 라 일반 컨테이너 권한만으로 같은 결과를 냅니다. userspace 스냅샷 동작 원리는 [01-03. 빌드 도구 비교와 선택](01-03.빌드%20도구%20비교와%20선택.md) § "OCI 관점의 근본 차이" 가 정본으로 다룹니다.
...
> 이 점검 편은 K8s Jenkins 구축·운영 본편(02-01, 02-02)을 읽은 직후 자가 점검 용도입니다. 점검에서 막힌 항목은 해당 본편으로 돌아가 복습하고, Agent 기본 개념이나 빌드 도구 선택 배경이 흐릿하면 01편으로 거슬러 올라가세요.

  - [02-01. Kubernetes Jenkins 구축](02-01.Kubernetes%20Jenkins%20구축.md) — K8s Jenkins 구축 본편 (podTemplate·JNLP·StatefulSet 배포 세부)
  - [02-02. Kubernetes Jenkins 운영](02-02.Kubernetes%20Jenkins%20운영.md) — K8s Jenkins 운영 (빌드 캐시·보안 강화·트러블슈팅 세부)
  - [01-01. 실행환경으로서의 Agent](01-01.실행환경으로서의%20Agent.md) — Agent 기본 개념 (Q1·Q2 사전 지식 배경)
  - [01-03. 빌드 도구 비교와 선택](01-03.빌드%20도구%20비교와%20선택.md) — K8s 빌드 도구 (Q5 Maven·Gradle 캐시 전략 배경)
