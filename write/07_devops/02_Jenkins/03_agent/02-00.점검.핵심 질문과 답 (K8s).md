...
K8s Jenkins에서 Agent Pod 이미지 Pull이 병목이 되는 경우 어떻게 해결합니까?

## Q2: Kubernetes Plugin의 동적 Agent Pod 프로비저닝 과정을 단계별로 설명할 수 있습니까?

> 답은 §정답 절을 참조합니다.

### 심화

Kubernetes Plugin 없이 Pod을 직접 생성해 Jenkins Agent로 연결하려면 어떻게 합니까?
...
멀티 컨테이너 Pod에서 컨테이너 간 파일을 공유하려면 어떻게 설정합니까?
...
PVC의 ReclaimPolicy가 Delete일 때 Helm 릴리즈 삭제 시 데이터 손실을 방지하려면 어떻게 합니까?

## Q5: K8s Jenkins에서 빌드 캐시(Maven/Gradle)를 어떻게 관리합니까?

> 답은 §정답 절을 참조합니다.

### 심화

빌드 캐시 PVC를 ReadWriteOnce(RWO)로만 설정할 수 있는 환경에서 캐시를 공유하려면 어떻게 접근합니까?
...
JNLP 연결이 실패할 때 트러블슈팅 접근법은 무엇입니까?

## Q7: Jenkins on K8s, Jenkins X, Tekton — 어떤 상황에 어느 것을 선택합니까?

> 답은 §정답 절을 참조합니다.

### 심화

Tekton Pipeline을 직접 사용할 때 Jenkins Shared Library와 동등한 코드 재사용 메커니즘은 무엇입니까?
