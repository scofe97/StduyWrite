...
> 이 개념은 이미 아는 *패키지 매니저*(apt/npm 가 의존성 묶음을 한 명령으로 설치하듯)의 *클러스터 리소스* 측면을 일반화한 것입니다 — Helm 은 YAML 매니페스트 묶음을 차트 단위로 설치합니다.
...
```yaml
controller:
  numExecutors: 0          # 왜 0: Controller 에서 빌드 실행 금지 — secrets/ 노출 차단 (보안), 빌드는 Agent 로 격리
  installPlugins:
    - kubernetes
    - workflow-aggregator
    - git
...
- `numExecutors: 0`은 Controller에서 빌드를 실행하지 않게 합니다. 공식 문서도 빌드 격리·안정성을 위해 Controller executor 를 0 으로 두기를 권장합니다 (출처: jenkins.io/doc/book/using/using-agents). Controller/Agent 아키텍처에서 executor 1 개는 동시 빌드 1 개를 의미하므로, Controller executor 0 은 곧 Controller 에서 빌드를 한 건도 돌리지 않는다는 뜻입니다.
- `persistence`는 PVC를 생성해 `JENKINS_HOME`을 영속화합니다.
- `installPlugins`로 필수 플러그인을 자동 설치합니다.
- `resources` 의 `requests`/`limits` 는 단순 스케줄링 힌트가 아닙니다. Jenkins Kubernetes plugin 은 Agent Pod 의 JVM heap 을 *memory request 에서 유도* 하므로, request 를 너무 낮게 잡으면 limit 가 커도 heap 이 작아 OOM 이 납니다 (출처: plugins.jenkins.io/kubernetes).
...
Kaniko 를 쓰는 이미지 빌드라면 캐시 단위가 *의존성 디렉토리* 가 아니라 *Dockerfile 레이어* 로 바뀝니다. Kaniko 는 userspace 에서 Dockerfile 명령을 순차 실행하고 명령마다 파일시스템 스냅샷을 찍어 변경 레이어를 append 하며, `--cache=true` 로 RUN/COPY 레이어를 `--cache-repo`(원격 레지스트리)나 `--cache-dir`(로컬 PVC)에 캐시합니다. base 이미지 캐시는 warmer 이미지로 미리 채울 수 있고 `--cache-ttl` 로 만료를 둡니다 (출처: github.com/GoogleContainerTools/kaniko).
...
Kaniko 는 Docker 데몬 없이 Dockerfile 로 이미지를 빌드합니다. 동작 방식은 base 이미지 파일시스템을 추출한 뒤 Dockerfile 명령을 순차 실행하고, 각 명령 후 *userspace 에서 파일시스템 스냅샷*(체크섬 비교)을 찍어 변경분을 차등 레이어로 append 합니다. Docker daemon·privileged·namespacing·seccomp/AppArmor 가 모두 불필요하지만, kaniko 자체가 빌드 보안을 보장하지는 않고 컨테이너 런타임의 격리에 의존하므로 실행은 `gcr.io/kaniko-project/executor` 이미지 안에서만 이뤄집니다(debug 변종 존재). 캐시 레이어는 `--cache=true` 로 RUN/COPY 레이어를 원격 레지스트리(`--cache-repo`)에 저장하거나 PVC 로컬(`--cache-dir`)에 유지하고, base 캐시는 warmer 이미지로 미리 채우며 `--cache-ttl` 로 만료를 둡니다. 단 Windows 미지원이며 multi-arch manifest 는 생성하지 않습니다. 출처: [github.com/GoogleContainerTools/kaniko](https://github.com/GoogleContainerTools/kaniko)
