...
Kaniko는 Docker daemon 없이 *사용자 공간(user space)에서* 이미지를 빌드하므로 `--privileged`도 socket 마운트도 필요 없어, restricted 보안 프로파일에서도 동작합니다. 그래서 K8s 빌드 환경의 표준 선택지로 자리잡았습니다. daemon-free 빌드의 동작 메커니즘(userspace 스냅샷·차등 tarball)과 OCI runtime-spec 관점의 근본 차이는 [01-03. 빌드 도구 비교와 선택](01-03.빌드%20도구%20비교와%20선택.md) § "OCI 관점의 근본 차이" 가 정본으로 다룹니다 — 여기서는 *이미지 빌드 절차* 에 집중합니다.
...
### 2.2 같은 Dockerfile, 다른 실행 경로 — Docker vs Kaniko 운영 차이

> 도구를 바꿔도 Dockerfile 은 그대로입니다. 갈리는 것은 *그 Dockerfile 을 누가, 어떤 권한으로, 어떤 동선으로 실행하느냐* 입니다.

§1 의 DooD/DinD 와 §2 의 Kaniko 를 Jenkins 파이프라인 운영 관점에서 나란히 놓으면 차이가 단계마다 드러납니다. 뿌리에 있는 차이는 실행 주체 하나입니다. `docker build` 는 명령을 daemon 에 위임하고, daemon 의 빌더인 BuildKit 은 RUN 한 단계마다 runc 컨테이너를 띄워 실행합니다(출처: github.com/moby/buildkit — buildkitd 의 기본 워커가 OCI(runc) 워커). Kaniko 에는 그 위임 자체가 없습니다. executor 프로세스가 자기 컨테이너 안에서 명령을 직접 실행하고 변경분을 스냅샷합니다. 아래 표의 나머지 차이는 전부 이 한 가지에서 파생된 결과입니다.

| 운영 축 | Docker (DooD/DinD) | Kaniko |
|---------|--------------------|--------|
| RUN 실행 주체 | daemon(BuildKit)이 runc 컨테이너를 띄워 실행 | executor 프로세스가 직접 실행 + userspace 스냅샷 |
| 파이프라인 표현 | `docker.*` DSL·`withRegistry` 지원 | DSL 없음 — `sh '/kaniko/executor ...'` 한 줄 (§2 예시) |
| 레지스트리 인증 | `docker login` 세션을 daemon 이 보관 | `/kaniko/.docker/config.json` 마운트 또는 클라우드 IAM |
| 레이어 캐시 | 호스트 daemon 로컬 캐시 (DooD 는 빌드 간 자동 재사용) | Pod 휘발 → `--cache-repo` 로 레지스트리에 외부화 (§2.1) |
| 산출물 동선 | build 후 `docker push` 별도 단계 | `--destination` 빌드=푸시 일체 |
| 빌드 후 로컬 검증 | `docker images`/`docker run` 가능 | 불가 — 레지스트리에서 pull 해 확인 |
| 권한 요구 | privileged(DinD) 또는 socket(DooD) | 일반 권한 — restricted 프로파일 동작 |

두 경로를 한 장으로 보면 다음과 같습니다.

![같은 Dockerfile에서 갈라지는 Docker와 Kaniko의 실행 경로](_assets/01-04.docker-vs-kaniko-jenkins-path.svg)

표의 행 몇 개는 운영에서 실수로 이어지기 쉬워 따로 짚어 둡니다.

**인증** — `docker login` 은 daemon 이 세션을 보관해 주지만 Kaniko 에는 그 daemon 이 없습니다. 그래서 push 자격을 Secret 으로 `/kaniko/.docker/config.json` 에 마운트하거나, GKE 의 Workload Identity·EKS 의 IRSA 같은 클라우드 IAM 연계로 부여합니다. Jenkins credential 설정만 생각하면 부족하고, *그 Pod 가 어떤 자격으로 레지스트리에 push 하는가* 까지 설계 범위에 들어옵니다.

**캐시** — DooD 는 호스트 daemon 의 레이어 캐시를 설정 없이 재사용하지만, Kaniko Pod 는 빌드가 끝나면 사라지므로 캐시도 함께 증발합니다. `--cache=true` + `--cache-repo`(§2.1) 외부화를 빼먹으면 매 빌드가 base 이미지 pull 부터 `RUN npm ci` 재실행까지 처음부터 다시 합니다. 보안 이점만 보고 Kaniko 를 도입했다가 CI 가 느려지는 전형적 원인이 이 캐시 미설계입니다.

**산출물** — Kaniko 에는 로컬 이미지 스토어가 없어서 빌드 결과가 레지스트리에만 존재합니다. `docker images` 로 확인하는 습관은 통하지 않고, 검증이 필요하면 pull 해서 봅니다. push 없이 빌드만 하고 싶을 때는 `--no-push`, tarball 로 남기고 싶을 때는 `--tar-path` 가 예외 경로입니다(출처: github.com/GoogleContainerTools/kaniko).

**역할 경계** — Kaniko 가 대체하는 것은 `docker build` 와 `docker push` 뿐입니다. `docker run`·`docker exec` 같은 런타임 조작은 이미지 빌드가 아니라 컨테이너 실행이라 Kaniko 의 영역 밖이고, 그 일은 같은 Pod 의 sidecar 컨테이너와 `kubectl` 이 맡습니다. "Docker CLI 가 없으니 Kaniko 로 대신 실행한다" 는 발상은 통하지 않습니다.

두 도구가 만들어 내는 *이미지 자체* 는 모두 OCI 표준 호환이라 레지스트리·런타임 입장에서는 구분되지 않습니다. 그렇다면 무엇이 "근본적으로" 다른가 — OCI 세 스펙 계층 위에서 이 질문을 푸는 내용은 [01-03. 빌드 도구 비교와 선택](01-03.빌드%20도구%20비교와%20선택.md) § "OCI 관점의 근본 차이" 에서 다룹니다.
...
5. 같은 Dockerfile 을 DooD 와 Kaniko 로 빌드할 때, 파이프라인 운영에서 갈리는 지점을 세 가지 이상 들어 보세요. *(심화: Kaniko 빌드 결과를 `docker images` 로 확인할 수 없는 이유는 무엇입니까?)*
...
- Kaniko 는 `docker login` 세션 대신 (G)`______` 경로에 마운트된 인증 정보로 레지스트리에 push 합니다.
...
세 단계로 답합니다 — ① base 이미지 추출 → ② Dockerfile 명령 순차 실행 + 각 명령 후 userspace 파일시스템 스냅샷(체크섬) → ③ 변경분만 차등 tarball 레이어로 append. storage driver·런타임에 의존하지 않아 이식성이 확보되고 privileged 가 불필요합니다. 각 단계의 근거와 OCI runtime-spec 관점은 [01-03. 빌드 도구 비교와 선택](01-03.빌드%20도구%20비교와%20선택.md) § "OCI 관점의 근본 차이" 가 정본입니다.
...
### 정답 5 — DooD vs Kaniko 운영 차이

뿌리는 *실행 주체* 하나입니다 — DooD 는 daemon(BuildKit) 이 RUN 을 runc 컨테이너로 실행하고, Kaniko 는 executor 프로세스가 직접 실행합니다. 여기서 파생되는 운영 차이 중 세 가지를 들면 (a) **인증** — `docker login` 세션 보관 vs `/kaniko/.docker/config.json` 마운트·클라우드 IAM, (b) **캐시** — 호스트 daemon 로컬 캐시 자동 재사용 vs Pod 휘발이라 `--cache-repo` 외부화 설계 필요, (c) **푸시 동선** — build 후 별도 `docker push` vs `--destination` 빌드=푸시 일체. 그 외 권한(socket/privileged vs 일반 권한), 파이프라인 표현(DSL vs `sh` 한 줄)도 답이 됩니다.

### 정답 5 심화 — docker images 로 확인 불가한 이유

Kaniko 에는 daemon 도 로컬 이미지 스토어도 없기 때문입니다. 빌드 결과는 `--destination` 레지스트리에만 존재하므로 확인하려면 pull 해서 봅니다. push 없이 빌드만 검증하려면 `--no-push`, 파일로 남기려면 `--tar-path` 를 씁니다(출처: github.com/GoogleContainerTools/kaniko).
...
- (G) `/kaniko/.docker/config.json` — daemon 이 없어 `docker login` 세션을 못 쓰므로 이 경로의 Secret 마운트(또는 클라우드 IAM)로 push 자격을 줍니다.



## 관련 문서

> 이 편은 VM·K8s 환경에서 컨테이너 이미지를 *어떻게* 빌드하는지(DinD·DooD·Kaniko·Multi-stage)를 다룹니다. 앞뒤로 "Pipeline 안에서 Docker 명령을 호출하는 방법(01-02)", "DooD socket 의 보안 함의를 VM 맥락에서 심화한 편(01-05)", "Kaniko 외 빌드 도구를 비교해 선택하는 기준(01-03)", "K8s 클러스터에 Kaniko Agent 를 실제 구축하는 편(02-01)"으로 이어집니다.

  - [01-02. Docker with Pipeline](01-02.Docker%20with%20Pipeline.md) — Pipeline 안에서 Docker 빌드·push 명령을 호출하는 방법
  - [01-05. VM Jenkins에서의 Docker 보안 모델](01-05.VM%20Jenkins에서의%20Docker%20보안%20모델.md) — DooD socket 마운트의 보안 위협과 VM 환경 완화 전략
  - [01-03. 빌드 도구 비교와 선택](01-03.빌드%20도구%20비교와%20선택.md) — daemon-free 빌드 도구 선택 기준 + § "OCI 관점의 근본 차이" (Docker·Kaniko 가 OCI 세 스펙 중 어디서 갈리는가)
