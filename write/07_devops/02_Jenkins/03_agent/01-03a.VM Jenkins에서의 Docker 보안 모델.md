...
1. `-v /var/run/docker.sock:/var/run/docker.sock` 마운트가 *왜 호스트 root 권한 부여와 동등* 한지 한 명령어 예시로 설명할 수 있습니까?
2. 완화 전략 4종 중 *socket 마운트 위험을 원천 제거* 하는 것은 무엇이며, *피해 범위만 한정* 하는 것은 무엇입니까?
3. rootless Docker 가 *socket 유출 시에도 호스트 root 를 못 얻게* 만드는 메커니즘은 무엇입니까?
4. VM 환경에서 1단계(전용 빌드 노드 격리) 만으로도 *피해 범위가 크게 줄어드는* 이유는 무엇입니까?
