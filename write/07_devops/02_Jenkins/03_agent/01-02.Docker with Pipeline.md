...
> Docker Agent는 빌드를 깨끗한 컨테이너 안에서 실행해 "내 머신에서는 됐는데" 문제를 없앱니다. Agent에 도구를 설치하는 대신 이미지로 환경을 고정합니다.
...
> 컨테이너 안에서 Docker 이미지를 빌드하는 방법은 세 가지이며, 각각 보안과 성능 트레이드오프가 다릅니다.
...
> 빌드한 이미지를 레지스트리에 푸시하는 표준 흐름과 인증 처리 방법을 다룹니다.
...
> sidecar 패턴으로 빌드 중 의존 서비스(DB 등)를 컨테이너로 띄우는 방법을 다룹니다.
...
> latest 태그의 문제와 추적 가능한 태깅 전략을 다룹니다.
...
> Docker Agent는 "빌드 환경"을 컨테이너화하고, docker.build는 "결과물"을 컨테이너화합니다.
...
1. *Docker Agent* 와 *`docker.build`* 가 서로 다른 일이라는 사실을 한 문장으로 설명할 수 있습니까? Docker Agent 안에서 `docker build` 를 돌리려면 무엇이 추가로 필요합니까?
