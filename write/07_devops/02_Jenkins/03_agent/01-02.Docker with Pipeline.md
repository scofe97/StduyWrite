...
> 이 문서를 읽고 나면 *Docker Agent(빌드 환경 컨테이너화)* 와 *`docker.build`(결과물 컨테이너화)* 의 차이를 그림 없이 *설명* 하고, 컨테이너 안 이미지 빌드 세 방법(DooD / DinD / Kaniko)의 보안·속도를 *비교* 하며, `latest` 대신 추적 가능한 태깅 전략을 *선택* 할 수 있습니다.



## 진입 — 왜 두 가지 Docker가 헷갈리는가

> 처음 Jenkins에서 Docker를 만나면 "agent에도 docker를 쓰고 빌드에도 docker를 쓰는데, 둘이 같은 거 아닌가?" 에서 막힙니다. 이 한 컷이 이 문서 전체의 출발점입니다.

`agent { docker { image 'node:18' } }` 와 `docker.build('myapp:1.0')` 는 둘 다 `docker` 라는 단어를 쓰지만 *완전히 다른 일* 을 합니다. 전자는 *빌드가 도는 방* 을 컨테이너로 바꾸고, 후자는 *빌드의 결과물* 을 컨테이너 이미지로 굽습니다. 이 둘을 한 단어로 묶어 생각하면 "Docker Agent를 썼는데 왜 `docker build` 가 안 되지?" 라는 막다른 길에 들어섭니다. 이 문서는 그 매듭을 먼저 풀고 시작합니다.
...
> 이 개념은 새로 배우는 게 아니라, *로컬에서 `docker run -v $(pwd):/app node:18 npm ci` 로 빌드 환경을 고정하던 습관* 을 Jenkins가 stage 단위로 자동화한 것입니다.
...
Docker Agent는 *식당의 주방* 에 비유할 수 있습니다. 손님(빌드)마다 *깨끗이 비운 새 주방* 을 내주고, 식사가 끝나면 주방을 통째로 치웁니다. 다음 손님은 앞 손님이 흘린 기름때를 물려받지 않습니다. 단 이 비유는 *재료(워크스페이스의 소스코드)는 카운터에서 같이 전달된다* 는 점까지만 유효하고, "주방을 통째로 새로 짓는 비용(이미지 pull·컨테이너 생성)이 손님마다 든다" 는 점은 식당 비유로는 잘 드러나지 않습니다 — 그 비용을 줄이는 게 아래 `reuseNode`·`args` 캐시입니다.

### reuseNode와 args — 새 주방 비용 줄이기

> 본 절은 Docker Agent의 *기본 동작* 과, 매번 새 워크스페이스를 만드는 비용을 줄이는 두 공식 옵션을 다룹니다.

`agent { docker { ... } }` 의 *기본 동작* 은 빌드마다 *새 빈 워크스페이스를 만들고* 거기에 파이프라인 코드를 clone 한 뒤 컨테이너에 마운트하는 것입니다(출처: jenkins.io/doc/book/pipeline/docker). 즉 컨테이너화된 stage는 *아무 가용 agent* 위에서 임시 워크스페이스를 받아 돌 수 있습니다. 반면 `reuseNode true` 를 주면 새 워크스페이스를 만들지 않고 *현재 agent의 워크스페이스를 같은 노드에서 그대로* 마운트합니다 — stage 간 산출물을 이어받아야 할 때 필요합니다(Scripted Pipeline에서는 미지원).
        ...
        stage('Build') {
            agent {
                docker {
                    ...
                    // 왜 reuseNode true: 바깥 agent 워크스페이스를 같은 노드에서 재사용
                    //   → 앞 stage가 만든 target/ 산출물을 이 컨테이너가 그대로 이어받음
                    reuseNode true
                    // 왜 -v 캐시 마운트: ~/.m2 를 호스트와 공유해 매 빌드 의존성 재다운로드 방지
                    //   (컨테이너는 매번 삭제돼도 호스트의 .m2 캐시는 살아남음)
...
Dockerfile을 레포에 두고 *그 Dockerfile로 빌드한 이미지를 agent로* 쓰려면 `agent { dockerfile true }` 를 씁니다(출처: jenkins.io/doc/book/pipeline/docker). 빌드 환경 자체를 코드로 버전 관리하고 싶을 때의 패턴입니다.
...
> Kaniko는 2026-06부로 archive(유지보수 중단) 상태입니다. 신규 프로젝트는 `01-03.빌드 도구 비교와 선택` 의 결정 트리를 먼저 확인합니다.
...
### 빈칸 채우기 — reuseNode와 의존성 캐시

> 아래 빈칸을 채워 본 뒤 §정답 끝의 *빈칸 정답* 으로 대조하세요.

```groovy
agent {
    docker {
        image 'maven:3.9-eclipse-temurin-17'
        ______ true                          // 바깥 워크스페이스를 같은 노드에서 재사용
        args '-v $HOME/______:/root/.m2'     // 의존성 캐시를 호스트와 공유
    }
}
```
...
Kaniko 는 이 문제를 다른 방식으로 접근합니다 — daemon-free 로 동작해 privileged·socket 마운트가 모두 불필요하므로 위 두 위험을 원천적으로 비웁니다. 그 동작 메커니즘은 [01-03. 빌드 도구 비교와 선택](01-03.빌드%20도구%20비교와%20선택.md) § "OCI 관점의 근본 차이" 가 정본으로 다룹니다.
...
컨테이너가 *떴다* 는 것과 *DB 가 연결 받을 준비가 됐다* 는 것이 다르기 때문입니다. `docker.image().withRun()` 은 PostgreSQL 컨테이너를 *시작* 하지만, postgres 프로세스가 초기화·소켓 listen 까지 완료하는 데 보통 2~5초가 걸립니다(이미지·디스크 속도에 따라 더 길어질 수 있습니다). 이 대기 없이 바로 `./gradlew integrationTest` 를 돌리면 *DB 가 아직 연결을 안 받아* connection refused 로 테스트가 깨집니다. `while ! pg_isready; do sleep 1; done` 은 *DB 가 실제로 준비될 때까지* 폴링해 race condition 을 막는 게이트입니다.

### 빈칸 정답 — reuseNode와 의존성 캐시

```groovy
agent {
    docker {
        image 'maven:3.9-eclipse-temurin-17'
        reuseNode true                       // 바깥 워크스페이스를 같은 노드에서 재사용
        args '-v $HOME/.m2:/root/.m2'        // 의존성 캐시를 호스트와 공유
    }
}
```

`reuseNode true` 는 *기본 동작인 "새 빈 워크스페이스 생성"* 을 끄고 현재 agent의 워크스페이스를 같은 노드에서 그대로 마운트합니다. `args` 의 `-v $HOME/.m2:/root/.m2` 는 컨테이너가 매번 삭제돼도 *호스트의 `.m2` 캐시는 살아남게* 해, Maven 의존성을 빌드마다 다시 받지 않도록 합니다.
...
> 이 편은 "Docker Agent(환경)와 docker.build(결과물)의 구분"과 "컨테이너 안 이미지 빌드 3방법 개요"를 다룹니다. DinD·DooD의 동작 메커니즘 정본과 빌드 도구 선택 기준은 아래 편들로 이어집니다.
  ...
  - [01-04. 컨테이너 이미지 빌드](01-04.컨테이너%20이미지%20빌드.md) — DinD/DooD 메커니즘 정본과 Kaniko 빌드 흐름 상세
  - [01-05. VM Jenkins에서의 Docker 보안 모델](01-05.VM%20Jenkins에서의%20Docker%20보안%20모델.md) — docker.sock 마운트의 호스트 권한 탈취 위험과 완화 전략
  - [01-03. 빌드 도구 비교와 선택](01-03.빌드%20도구%20비교와%20선택.md) — Kaniko·Buildah·BuildKit 비교 및 환경별 선택 결정 트리
