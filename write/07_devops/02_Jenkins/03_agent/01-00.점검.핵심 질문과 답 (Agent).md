...
Kubernetes Pod Security Standards에서 DinD와 DooD가 각각 어떤 수준(privileged, baseline, restricted)에서 허용됩니까?

## Q2: Docker Agent를 사용하면 어떤 문제가 해결됩니까?

> 답은 §정답 절을 참조합니다.

### 심화

Docker Agent에서 npm 캐시나 Maven 로컬 리포지토리를 빌드 간에 공유하려면 어떻게 해야 합니까?
...
이미지 태그의 불변성(immutability)을 레지스트리 수준에서 강제하는 방법은 무엇입니까?
...
Kaniko에서 레이어 캐시 효율을 높이려면 어떤 전략을 사용해야 합니까?
...
Multi-stage 빌드에서 중간 스테이지의 결과물을 디버깅하려면 어떻게 합니까?
...
Kubernetes Plugin 환경에서 라벨 대신 Pod Template을 사용하는 방식은 어떤 장점이 있습니까?
...
상세는 `01-03.md` §2 를 참조합니다. 단, **Kaniko는 2026-06부로 archive 상태**이므로 신규 도입 시 `01-04.md` 결정 트리를 먼저 봅니다.
