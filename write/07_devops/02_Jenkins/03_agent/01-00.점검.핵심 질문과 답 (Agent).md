...
related:
  ...
  - ./01-05.VM Jenkins에서의 Docker 보안 모델.md
updated: 2026-05-29
---

# 점검 — Agent와 Docker 핵심 질문

> 이 점검 문서는 01장(Agent와 Docker) 을 다 읽은 뒤 스스로를 시험하기 위한 자가 점검입니다. 먼저 §면접 질문만 보고 답을 떠올린 뒤, §정답 절에서 같은 번호로 대조하세요.

> 다루는 문서: `01-01.실행환경으로서의 Agent` ~ `01-05.VM Jenkins에서의 Docker 보안 모델`
...
> 여섯 개 질문을 Agent 실행 모델·컨테이너 빌드 보안·이미지 위생 세 묶음으로 나눠 흐름도로 보여주고, DinD·DooD·Kaniko의 권한 차이를 한 그림에 정리합니다.
...
비교 표는 [01-04. 컨테이너 이미지 빌드](01-04.컨테이너%20이미지%20빌드.md) § "DinD vs DooD", VM 환경에서의 위험 모델은 [01-05. VM Jenkins에서의 Docker 보안 모델](01-05.VM%20Jenkins에서의%20Docker%20보안%20모델.md) 를 참조합니다.
...
상세는 [01-02. Docker with Pipeline](01-02.Docker%20with%20Pipeline.md) § "Docker Agent — 빌드 환경의 컨테이너화" 를 참조합니다.
...
태깅 전략 표는 [01-02. Docker with Pipeline](01-02.Docker%20with%20Pipeline.md) § "이미지 태깅 전략" 을 참조합니다.
...
`/var/run/docker.sock`에 접근할 수 있는 프로세스는 `docker run -v /:/host alpine chroot /host`로 호스트 root 쉘을 얻을 수 있습니다. Kubernetes Pod Security Standards `restricted` 프로파일은 호스트 path 마운트와 privileged 컨테이너를 모두 금지하므로 Docker daemon 의존이 정책 위반이 됩니다. Kaniko는 daemon 없이 사용자 공간에서 빌드해 일반 권한으로 동작하므로 이 정책을 통과합니다. 트레이드오프는 레이어 캐싱 효율이 Docker만 못해 빌드 속도가 떨어지는 것입니다.

빌드 절차는 [01-04. 컨테이너 이미지 빌드](01-04.컨테이너%20이미지%20빌드.md) § "Kaniko로 안전하게 빌드", userspace 스냅샷 동작 메커니즘과 도구 선택 결정 트리는 [01-03. 빌드 도구 비교와 선택](01-03.빌드%20도구%20비교와%20선택.md) § "OCI 관점의 근본 차이" · "빌드 도구 선택 가이드" 가 정본입니다.
...
상세는 [01-04. 컨테이너 이미지 빌드](01-04.컨테이너%20이미지%20빌드.md) § "Multi-stage 빌드" 를 참조합니다.
...
비교 표와 Pod 생성 흐름은 [01-01. 실행환경으로서의 Agent](01-01.실행환경으로서의%20Agent.md) § "정적 Agent vs 동적 Agent" 를 참조합니다.
...
> 이 점검 문서는 01-01부터 01-05까지 다섯 편을 모두 다룹니다. 점검 중 막힌 질문이 있으면 아래 해당 편으로 바로 돌아가 해당 절을 다시 읽고, 심화 답변은 각 편의 참조 절 표시를 따라가면 동선이 이어집니다.
  ...
  - [01-04. 컨테이너 이미지 빌드](01-04.컨테이너%20이미지%20빌드.md) — 컨테이너 이미지 빌드 메커니즘 § DinD·DooD·Kaniko 비교 및 Multi-stage 상세(Q1·Q4·Q5 연계)
  - [01-05. VM Jenkins에서의 Docker 보안 모델](01-05.VM%20Jenkins에서의%20Docker%20보안%20모델.md) — docker.sock 보안 § VM 환경 위험 모델(Q1 심화 연계)
  - [01-03. 빌드 도구 비교와 선택](01-03.빌드%20도구%20비교와%20선택.md) — 빌드 도구 선택 § Kaniko·Buildah·BuildKit 결정 트리(Q4 심화 연계)
