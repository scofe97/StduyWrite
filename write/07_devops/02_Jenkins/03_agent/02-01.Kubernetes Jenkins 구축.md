...
> VM 기반 Jenkins의 한계를 Kubernetes가 어떻게 해결하는지 이해하는 것이 출발점입니다.
...
> Helm 차트로 Jenkins를 설치하는 것이 표준입니다. values.yaml로 설정을 코드화합니다.
...
> Kubernetes Plugin이 동적 Agent Pod을 생성하는 핵심입니다. Cloud 설정과 Pod Template을 정의합니다.
...
> 설치 후 Agent Pod이 정상적으로 생성되고 빌드가 실행되는지 확인합니다.
...
> Kubernetes Jenkins 구축은 Helm + Kubernetes Plugin + JCasC의 조합입니다.
...
1. VM Jenkins 의 두 한계(유휴 비용·Configuration Drift) 를 K8s 가 *각각 어떤 메커니즘* 으로 해결합니까?
2. Helm `values.yaml` 의 `controller.numExecutors: 0` 이 *K8s 환경에서 특히* 중요한 이유는 무엇입니까?
3. `jenkinsTunnel` 설정이 빠지거나 잘못되면 *어떤 증상* 이 나타납니까?
4. 구축 검증에서 `kubectl get pods -n jenkins` 로 *무엇을 두 번* 확인해야 검증이 완료됩니까?
