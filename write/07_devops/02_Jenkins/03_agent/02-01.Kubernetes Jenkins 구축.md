...
- `containerCapStr`은 동시에 생성할 수 있는 최대 Pod 수입니다. 이 상한에 도달하면 초과 빌드는 Pod 공급 없이 큐에 그대로 쌓이므로, 외부 시스템이 큐 깊이로 부하를 제어할 때의 기준선이 됩니다 — 게이트 설계 관점은 [04_api/05-03](../04_api/05-03.Queue%20적재%20이후%20실행%20흐름과%20데이터%20추적.md) § "2-6 큐 깊이 게이트" 에서 다룹니다.
...
> 이 편은 K8s 위에 Jenkins 를 *처음 세우는* 구축 단계를 다룹니다. 구축 이후 일상 운영(백업·모니터링·보안)은 02-02 로, Pod Template 의 전제가 되는 Agent/Executor 개념은 01-01 로, K8s 환경에서 어떤 빌드 도구를 선택해야 하는지는 01-03 로 연결됩니다.
  ...
  - [01-03. 빌드 도구 비교와 선택](01-03.빌드%20도구%20비교와%20선택.md) — K8s 빌드 도구 선택 § 환경별 적합 도구 판단
