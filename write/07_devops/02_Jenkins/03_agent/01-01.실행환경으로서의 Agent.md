...
userspace 스냅샷 방식이기 때문에 storage driver 나 특정 컨테이너 런타임에 의존하지 않고, privileged 권한도 필요 없습니다. Pod 안에서 `container('kaniko')` 로 호출하면 일반 unprivileged 컨테이너 안에서 OCI 이미지를 만들어 레지스트리에 push 할 수 있습니다.
