          ...
          - name: "maven-builder"
            ...
            containers:
              ...
              - name: "docker"
                ...
                privileged: true           # 왜 privileged: dind는 컨테이너 안에서 Docker daemon을 띄워야 해 호스트 권한이 필요 (보안 부담은 03a편에서)
            volumes:
...
### 정답 4 심화 — Kaniko userspace 빌드 원리

Pod 안에서 `docker:24-dind` 대신 Kaniko 를 사용하는 이유는 **privileged 컨테이너 없이 이미지를 빌드**할 수 있기 때문입니다. Docker-in-Docker(DinD) 방식은 컨테이너가 호스트 커널의 Docker socket 을 직접 사용하거나 privileged 모드로 새 Docker daemon 을 띄워야 하는데, 이는 보안 정책상 허용하지 않는 클러스터가 많습니다.

Kaniko 는 다음 과정으로 이미지를 빌드합니다(출처: github.com/GoogleContainerTools/kaniko):

1. base 이미지의 파일시스템을 컨테이너 내부로 추출합니다.
2. Dockerfile 의 각 명령(RUN, COPY 등)을 순차 실행합니다.
3. 각 명령 실행 후 **userspace 에서 파일시스템 스냅샷**을 찍습니다 — storage driver 나 커널 네임스페이스가 아닌 체크섬 비교 방식으로 변경분을 감지합니다.
4. 변경된 파일만 차등 tarball 레이어로 append 하여 최종 이미지를 구성합니다.

userspace 스냅샷 방식이기 때문에 storage driver 나 특정 컨테이너 런타임에 의존하지 않고, privileged 권한도 필요 없습니다. Pod 안에서 `container('kaniko')` 로 호출하면 일반 unprivileged 컨테이너 안에서 OCI 이미지를 만들어 레지스트리에 push 할 수 있습니다. 다만 Kaniko 자체가 빌드 격리를 보장하지는 않고 *컨테이너 런타임의 격리에 의존* 하며, Windows 미지원·multi-arch manifest 미생성 같은 제약도 있습니다(출처: github.com/GoogleContainerTools/kaniko).
...
> 이 편은 Agent의 개념 구조(Node·Label·정적/동적 모델)를 다루는 출발점입니다. Docker Agent 실행 방식의 구체 구현은 다음 편에서, VM 환경에서의 보안 모델과 K8s 동적 Agent 구축은 그 이후 편에서 이어집니다.
  ...
  - [01-03a. VM Jenkins에서의 Docker 보안 모델](01-03a.VM%20Jenkins에서의%20Docker%20보안%20모델.md) — VM 보안 모델 § privileged 컨테이너의 위험
