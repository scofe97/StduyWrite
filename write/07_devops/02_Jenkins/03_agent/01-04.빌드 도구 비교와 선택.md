...
![OCI 세 스펙 위의 Docker와 Kaniko 위치](_assets/01-04.oci-spec-position.svg)
...
Jenkins 파이프라인 운영 동선(인증·캐시·푸시)에서 이 차이가 어떻게 나타나는지는 [01-03. 컨테이너 이미지 빌드](01-03.컨테이너%20이미지%20빌드.md) § "2.2 같은 Dockerfile, 다른 실행 경로" 에서 다룹니다.
...
> 이 편은 "어떤 빌드 도구를 선택하는가"라는 결정 문제를 다룹니다. 선택의 전제가 되는 컨테이너 빌드 메커니즘과 VM 소켓 보안은 01-03 묶음에서, 선택 결과를 실제로 적용하는 K8s 빌드 환경은 02-01에서 이어집니다.
  ...
  - [01-03. 컨테이너 이미지 빌드](01-03.컨테이너%20이미지%20빌드.md) — 컨테이너 빌드 메커니즘 + § "2.2 같은 Dockerfile, 다른 실행 경로" (Docker vs Kaniko 운영 차이)
