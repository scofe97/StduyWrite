...
![같은 Dockerfile에서 갈라지는 Docker와 Kaniko의 실행 경로](_assets/01-03.docker-vs-kaniko-jenkins-path.svg)
...
두 도구가 만들어 내는 *이미지 자체* 는 모두 OCI 표준 호환이라 레지스트리·런타임 입장에서는 구분되지 않습니다. 그렇다면 무엇이 "근본적으로" 다른가 — OCI 세 스펙 계층 위에서 이 질문을 푸는 내용은 [01-04. 빌드 도구 비교와 선택](01-04.빌드%20도구%20비교와%20선택.md) § "OCI 관점의 근본 차이" 에서 다룹니다.
...
> 이 편은 VM·K8s 환경에서 컨테이너 이미지를 *어떻게* 빌드하는지(DinD·DooD·Kaniko·Multi-stage)를 다룹니다. 앞뒤로 "Pipeline 안에서 Docker 명령을 호출하는 방법(01-02)", "DooD socket 의 보안 함의를 VM 맥락에서 심화한 편(01-03a)", "Kaniko 외 빌드 도구를 비교해 선택하는 기준(01-04)", "K8s 클러스터에 Kaniko Agent 를 실제 구축하는 편(02-01)"으로 이어집니다.
  ...
  - [01-04. 빌드 도구 비교와 선택](01-04.빌드%20도구%20비교와%20선택.md) — daemon-free 빌드 도구 선택 기준 + § "OCI 관점의 근본 차이" (Docker·Kaniko 가 OCI 세 스펙 중 어디서 갈리는가)
