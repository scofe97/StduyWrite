---
title: 컨테이너 이미지 빌드
tags: [jenkins, agent, docker, kubernetes, image-build, dockerfile]
status: draft
related:
  - ../README.md
updated: 2026-05-28
...
> Kaniko는 Google이 개발한 Daemonless 빌드 도구로, Docker daemon 없이 Dockerfile에서 컨테이너 이미지를 빌드한다. 핵심 아이디어는 Kaniko 컨테이너 내부에서 직접 Dockerfile을 해석하고 레이어를 조립하는 것이다 — daemon 소켓(`/var/run/docker.sock`)도 `--privileged` 권한도 필요 없으므로 Kubernetes Pod Security Standards를 준수한 상태로 이미지 빌드가 가능하다.

> ⚠️ **유지보수 상태**: Kaniko는 2026-06부로 archive 상태이므로 신규 도입 시 `04-04.md`의 결정 트리를 먼저 본다(BuildKit, kpack, Buildah, Jib/ko 비교).
...
핵심은 이 모든 과정이 **Docker daemon 없이, unprivileged 컨테이너 안에서** 완료된다는 점이다. ④ 스냅샷 단계가 Kaniko의 정체성 — userspace에서 파일시스템 변경분을 감지해 레이어를 만들기 때문에 daemon이 필요 없다.
