...
> 이 점검을 마치면 CPS와 resume의 관계, Resumable/Nonresumable step의 구분, `disableResume()`과 durability 세 레벨의 트레이드오프, Controller 장애 시 빌드 운명, Restart from Stage와 자동 resume의 차이, stash/unstash의 쓰임과 한계를 스스로 설명할 수 있는지 확인합니다.
...
Jenkins Pipeline이 CPS를 사용하는 이유가 여기에 있습니다. Controller가 재시작되어도 각 FlowNode의 상태를 복원해 이전 실행 지점부터 재개할 수 있습니다. *심화*: `@NonCPS` 메서드는 CPS 변환을 거치지 않으므로 중간 상태가 직렬화되지 않습니다. 그 안에서 `sh` 같은 step을 호출하는 것은 권장되지 않으며, resume 보장이 깨질 수 있습니다.
...
`PERFORMANCE_OPTIMIZED`는 빌드 속도가 중요하고 재실행이 가능한 CI 빌드에 적합합니다. *심화*: 파이프라인별 설정은 `options { durabilityHint('...') }` 블록으로 지정하며, 전역 기본값은 `MAX_SURVIVABILITY`로 두고 개별 파이프라인에서만 낮추는 것이 안전합니다.
