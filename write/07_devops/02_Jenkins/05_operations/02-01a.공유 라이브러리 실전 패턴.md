...
# 공유 라이브러리 실전 패턴 (Shared Libraries)
...
**왜 src/ 클래스를 분리하는가?** vars/ 함수가 복잡해지면 로직을 테스트하기 어려워집니다. 순수 로직(이미지 이름 생성, 조건 판단 등)을 src/ 클래스로 분리하면 Jenkins 환경 없이도 **단위 테스트가 가능**합니다. 다만 그 src/ 객체도 Pipeline 실행 중간에 직렬화 경계를 넘으므로 `Serializable` 구현이 필요합니다(CPS 변환·직렬화 메커니즘의 상세는 [02-03. groovy 커스텀터마이징 한계](02-03.groovy%20%EC%BB%A4%EC%8A%A4%ED%85%80%ED%84%B0%EB%A7%88%EC%9D%B4%EC%A7%95%20%ED%95%9C%EA%B3%84.md) § "CPS 변환").
