...
> 이 점검을 마치면 `vars/`와 `src/`의 경계, `@Library` 버전 고정의 이유, `call()` 진입점, CPS와 `@NonCPS`, Implicit/Explicit 로딩, JCasC와 `init.groovy.d`의 선택, Script Console의 보안 위험을 스스로 설명할 수 있는지 확인합니다.
...
- `02-01.공유 라이브러리.md` — Shared Library 구조와 로딩
- `02-01a.공유 라이브러리 실전 패턴.md` — vars/src 패턴·테스트·버전 관리
