---
title: Groovy 커스터마이징이란? — 세 실행 영역과 보안 경계
...
Jenkins의 `JENKINS_HOME` 구조와 크레덴셜 저장 개념([../02_security/01-02. 시크릿 관리와 최소 권한 원칙](../02_security/01-02.시크릿%20관리와%20최소%20권한%20원칙.md)), Groovy 기본 문법([02-04. Groovy 기본 문법](02-04.Groovy%20기본%20문법.md))을 알고 있으면 좋습니다. Jenkins가 JVM 위에서 도는 Java 애플리케이션이라는 점을 떠올릴 수 있으면 Groovy가 왜 확장 언어인지 자연스럽게 이어집니다.
...
> Pipeline Groovy가 CPS 변환으로 직렬화되며 생기는 제약이 커스텀 수단을 어디에 둘지 가르는 경계가 됨을 다룹니다.
...
> "이것을 JCasC로 할 수 있는가"를 1순위 판단 기준으로 삼아 영역별 보안 수준과 권장 대안을 정리하고, "할 수 있다"와 "해야 한다"의 구분이 운영 성숙도라는 점을 다룹니다.
