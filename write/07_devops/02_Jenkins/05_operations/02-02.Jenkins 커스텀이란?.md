...
Shared Library를 쓸 때 알아야 할 세 디렉토리가 있습니다. (출처: jenkins.io/doc/book/pipeline/shared-libraries)

- `vars/`: 파일 basename이 곧 Pipeline 전역변수 이름이 됩니다(camelCase). 각 `.groovy`의 `call()` 구현이 step처럼 호출되고, 같은 이름의 `.txt`로 문서를 답니다. 전역변수는 stateless로 두기를 권장합니다.
- `src/`: 표준 Java 소스 디렉토리 구조로, classpath에 추가됩니다. `org/foo/Bar.groovy`는 패키지 규약상 `org.foo.Bar`로 매핑됩니다.
- `resources/`: 비-Groovy 파일을 두고 `libraryResource` step으로 읽습니다.
...
Pipeline의 Groovy는 일반 Groovy와 다르게 동작합니다. Jenkins는 재기동을 대비해 Pipeline 실행 상태를 **CPS(Continuation Passing Style) 변환**으로 직렬화해 `program.dat`에 저장합니다. (출처: jenkins.io/doc/book/pipeline/cps-method-mismatches)

이 제약은 커스텀 코드를 짤 때 다음과 같이 드러납니다.

- `@NonCPS`를 붙이면 네이티브 Groovy로 실행되어 성능이 좋지만, 그 메서드 *내부에서 `node`/`sh` 같은 Pipeline step을 호출할 수 없습니다*("expected to call WorkflowScript.X" 경고가 뜹니다).
- 생성자는 CPS 변환이 되지 않습니다.
- CPS closure를 `Iterable.toSorted` 같은 non-CPS 메서드에 넘기면 예상과 다른 결과가 나옵니다.
- `"${-> var}"` 형태의 lazy GString closure도 CPS라 직렬화 맥락에서 실패할 수 있습니다.

그래서 Shared Library의 `vars/` 헬퍼를 짤 때, "무거운 순수 계산은 `@NonCPS`로, step 호출이 섞이면 일반 메서드로"라는 경계를 의식해야 합니다. 이 경계를 모르고 `@NonCPS` 안에서 `sh`를 부르면 런타임에 깨집니다.
