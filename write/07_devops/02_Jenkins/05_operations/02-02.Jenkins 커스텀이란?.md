...
> Jenkins는 Java로 작성된 애플리케이션이지만, 설정과 확장의 핵심 언어로 Groovy를 사용한다.
>
> 이 선택은 우연이 아니다. Groovy는 JVM 위에서 동작하므로 Jenkins의 모든 Java 클래스에 직접 접근할 수 있고, Java와 거의 동일한 문법을 가지면서도 스크립팅에 적합한 동적 타이핑을 지원한다.

구체적으로 Groovy가 Jenkins에 적합한 이유는 세 가지다.

1. **JVM 호환성이다**. Groovy 코드는 JVM 바이트코드로 컴파일되므로 Jenkins의 Java 클래스를 `import`하여 직접 호출할 수 있다. 별도의 브릿지나 FFI(Foreign Function Interface)가 필요 없다.
2. **문법적 간결함이다**. Java에서 10줄이 필요한 코드를 Groovy에서는 3줄로 작성할 수 있다. 세미콜론 생략, 클로저, GString 같은 편의 기능이 관리 스크립트 작성에 생산성을 높여준다.
3. **동적 타이핑이다**. 런타임에 타입을 결정하므로 탐색적 스크립팅, 즉 "이 API가 무엇을 반환하는지 실행해보면서 확인하는" 작업에 유리하다.

### Groovy가 사용되는 3가지 영역

Jenkins에서 Groovy는 명확히 구분되는 세 가지 영역에서 사용되며, 각 영역의 보안 수준과 실행 컨텍스트가 다르다.
...
**영역 1: Pipeline DSL (Jenkinsfile)**은 가장 일상적인 Groovy 사용처다.

- 개발자가 `Jenkinsfile`에 `pipeline { ... }` 블록을 작성하면, Jenkins는 이것을 Groovy 코드로 해석하고 실행한다.
- 이 영역에서는 **Script Security Plugin**이 Sandbox를 적용하여 위험한 API 호출을 차단한다.
- 개발자가 `System.exit(0)`을 Pipeline에 넣어도 실행되지 않고 관리자 승인을 요구한다.

**영역 2: Script Console (Manage Jenkins > Script Console)**은 Jenkins 관리자가 Groovy 코드를 직접 입력하여 실행하는 인터페이스다.

- 이 콘솔은 Sandbox가 적용되지 않으므로 Jenkins의 모든 Internal API에 무제한으로 접근할 수 있다.
- 크레덴셜을 평문으로 읽을 수도 있고, 모든 Job을 삭제할 수도 있다. 강력하지만 그만큼 위험한 영역이다.

**영역 3: Init Hook (JENKINS_HOME/init.groovy.d/)**은 Jenkins가 시작될 때 자동으로 실행되는 Groovy 스크립트다.

- Script Console과 동일한 권한을 가지지만, 사용자가 직접 실행하는 것이 아니라 Jenkins 프로세스가 시작 시점에 자동으로 실행한다.
- 주로 초기 설정 자동화에 사용된다.

이 세 영역의 핵심적인 차이는 **누가 실행하느냐**와 **어떤 보안 경계 안에서 실행되느냐**다. Pipeline DSL은 개발자가 작성하고 Sandbox에서 실행되며, Script Console은 관리자가 직접 실행하고 보안 경계가 없으며, Init Hook은 시스템이 자동 실행하고 보안 경계가 없다.



## 2. Init Groovy Hook (전역 초기화 스크립트)

> `JENKINS_HOME/init.groovy.d/` 디렉토리는 Jenkins의 "부팅 스크립트" 폴더다.
>
> Jenkins 프로세스가 시작되면 이 디렉토리 안의 모든 `.groovy` 파일을 알파벳 순서로 실행한다. 실행 시점은 플러그인 로딩 이후, 웹 UI가 준비되기 전이다. 이 타이밍이 중요한데, 플러그인이 로딩된 상태이므로 플러그인 API를 호출할 수 있지만, 아직 사용자 요청을 받기 전이므로 초기 설정을 안전하게 적용할 수 있기 때문이다.
...
- init.groovy.d가 플러그인 로딩 전에 실행된다면, 특정 플러그인의 API를 호출하는 스크립트가 `ClassNotFoundException`으로 실패한다.
- 반대로 UI가 준비된 후에 실행된다면, 보안 설정이 적용되기 전에 사용자가 접근할 수 있는 시간 창이 열린다.
- 플러그인 로딩 직후, UI 준비 직전이라는 타이밍은 이 두 가지 문제를 모두 피하는 최적의 시점이다.

### 파일 네이밍 컨벤션

init.groovy.d 안의 스크립트는 알파벳 순서로 실행되므로, 숫자 접두사를 붙여 실행 순서를 명시적으로 제어하는 것이 관례다.
...
Docker 기반 Jenkins를 처음 시작할 때 관리자 계정을 자동으로 설정하는 가장 일반적인 사례다. 초기 설정 마법사를 건너뛰고 바로 사용 가능한 상태로 만들 수 있다.
...
- Jenkins Docker 이미지를 CI/CD 파이프라인으로 자동 배포할 때, 초기 설정 마법사를 수동으로 진행할 수 없기 때문이다.
- 특히 Kubernetes 환경에서 Pod이 재시작될 때마다 일관된 초기 상태를 보장해야 한다.
...
- JCasC(Configuration as Code)가 크레덴셜 등록을 지원하지만, 복잡한 조건부 로직(환경변수 존재 여부에 따른 분기)이 필요할 때는 init.groovy.d가 더 유연하다.
- 다만 환경변수에 비밀값을 넣는 것 자체의 보안 고려가 필요하다. Kubernetes라면 Secret 리소스를, Docker라면 Docker Secret을 사용해야 한다.



## 3. Script Console로 할 수 있는 것들

> Jenkins Script Console (Manage Jenkins > Script Console)은 **Jenkins 프로세스 내부에서 Groovy 코드를 직접 실행하는 REPL(Read-Eval-Print Loop)이다.**
>
> Jenkins의 모든 Java 객체에 접근할 수 있다는 것이 핵심이다. Jenkins 인스턴스, 모든 Job, 모든 빌드, 모든 크레덴셜, 모든 에이전트, 플러그인 설정까지 읽고 수정할 수 있다.
...
**빌드 기록 정리**: 디스크 공간이 부족할 때 오래된 빌드 기록을 대량 삭제한다.
...
**Agent 상태 조회**: 모든 Agent의 온/오프라인 상태와 실행 중인 빌드를 확인한다.
...
**플러그인 정보 조회**: 설치된 모든 플러그인의 버전과 업데이트 가능 여부를 확인한다.
...
Script Console의 가장 심각한 보안 문제는 **크레덴셜 평문 노출**이다. 다음 코드는 Jenkins에 저장된 모든 크레덴셜의 비밀값을 평문으로 출력할 수 있다.
...
- Jenkins는 크레덴셜을 암호화하여 저장하지만, 복호화 키가 Jenkins 프로세스 메모리에 있다.
- Script Console은 그 프로세스 안에서 실행되므로 복호화 API를 직접 호출할 수 있다.
- 이것은 설계 결함이 아니라, Jenkins가 빌드 중에 크레덴셜을 사용해야 하므로 프로세스가 복호화 능력을 갖추어야 하기 때문이다.
- 문제는 그 능력이 Script Console을 통해 관리자에게 노출된다는 것이다.

따라서 Script Console 접근 권한은 **Jenkins 시스템 관리자 중에서도 극소수**에게만 부여해야 한다. Matrix-based Security에서 "Administer" 권한과 "Run Scripts" 권한을 분리하여 관리하는 것이 권장된다.
...
전역 파이프라인 Hook은 JCasC로 구현할 수 없는 대표적인 영역이다. Shared Library Wrapper(opt-in)를 기본으로 하되, Jenkinsfile 수정이 불가능한 환경에서는 `RunListener`(init.groovy.d)로 모든 빌드에 강제 적용할 수 있다. "Groovy로 할 수 있다"와 "Groovy로 해야 한다"는 전혀 다른 질문이며, 이 구분을 아는 것이 Jenkins 운영 성숙도의 지표다.
