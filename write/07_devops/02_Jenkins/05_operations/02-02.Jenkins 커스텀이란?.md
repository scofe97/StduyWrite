...
> 이 문서를 읽고 나면 Jenkins에서 Groovy가 쓰이는 세 영역(Pipeline DSL·Script Console·init.groovy.d)을 보안 수준과 실행 컨텍스트로 구분하고, init.groovy.d의 실행 타이밍과 멱등성, Script Console의 크레덴셜 평문 노출 위험을 설명하며, "JCasC로 되면 JCasC"라는 우선순위로 커스터마이징 수단을 선택할 수 있습니다.

## §사전 지식

> Jenkins의 `JENKINS_HOME` 구조와 크레덴셜 저장 개념(`04_api/01-07`), Groovy 기본 문법을 알고 있으면 좋습니다. Jenkins가 JVM 위에서 도는 Java 애플리케이션이라는 점을 떠올릴 수 있으면 Groovy가 왜 확장 언어인지 자연스럽게 이어집니다.
