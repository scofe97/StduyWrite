...
`onResumed`는 FlowExecutionListener에만 있어, Jenkins 재시작 후 중단됐던 Pipeline이 재개되는 순간을 잡는 유일한 길입니다. 그런데 이 후킹은 *재개가 실제로 일어날 때만* 발화하므로, `onResumed`의 전제는 Pipeline durability 설정과 직결됩니다 — durability 모드와 종료 방식(graceful/dirty)이 함께 재개 가부를 정합니다(3모드 정의·재개 조건 상세는 [01-01. Pipeline 내구성과 재기동](01-01.Pipeline%20%EB%82%B4%EA%B5%AC%EC%84%B1%EA%B3%BC%20%EC%9E%AC%EA%B8%B0%EB%8F%99.md) § "durability 모드"). 특히 컨테이너 환경에서 Pod가 강제 종료(dirty shutdown)되는 시나리오가 잦다면 `PERFORMANCE_OPTIMIZED` 모드는 재개 자체를 막아 `onResumed` 후킹의 전제를 깨뜨립니다.
...
> 한 가지 주의: 이 리스너 본문은 Master JVM에서 실행되는 일반 Groovy이지만, 리스너 안에서 다시 Pipeline step(`node`·`sh`)을 부르는 설계는 피해야 합니다 — Pipeline step은 CPS 변환을 거치는 `Jenkinsfile` 문맥에서만 호출 가능하고, 리스너의 일반 메서드(특히 `@NonCPS`·생성자)에서는 호출할 수 없기 때문입니다(CPS 변환·`@NonCPS`의 step 호출 제약 상세는 [02-03. groovy 커스텀터마이징 한계](02-03.groovy%20%EC%BB%A4%EC%8A%A4%ED%85%80%ED%84%B0%EB%A7%88%EC%9D%B4%EC%A7%95%20%ED%95%9C%EA%B3%84.md) § "CPS 변환의 제약").
