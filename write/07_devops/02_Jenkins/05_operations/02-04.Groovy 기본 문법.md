...
> Git 저장소 생성 → Jenkins 등록 → vars/ 함수 작성 → src/ 분리 → 테스트 → 버전 태깅으로 이어지는 단계별 셋업 가이드는 **`09-01.공유 라이브러리.md` §2 (Jenkins 디렉토리 구조와 Shared Library)**와 **`09-01a.공유 라이브러리 실전 패턴.md` §4-5 (vars/ 함수 구현 + 테스트 전략)**에 정리되어 있다. 이 문서는 Groovy 문법 자체에 집중하므로, 라이브러리 셋업 절차는 그쪽에서 본다.
>
> 셋업 후에 자주 마주치는 Groovy 함정(`call(Map config = [:])` 시그니처, CPS와 `@NonCPS`, `Serializable` 요구)은 위 §1의 GString·클로저·CPS 변환 절을 확장해 읽으면 충분하다.

<!-- 본 절은 09-01.md / 09-01a.md로 이관. 아래 잔여 단계 설명은 동일 주제 중복이므로 09-01 시리즈를 본다. -->

<details>
<summary>(이전 단계별 가이드 — 09-01 시리즈 도입 이후 보관용)</summary>
