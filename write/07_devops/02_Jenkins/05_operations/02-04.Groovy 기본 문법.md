...
> Jenkins Shared Library를 작성하려면 Groovy 문법 중 어디까지 알아야 할까요?

## §학습 목표

> 이 문서를 읽고 나면 Shared Library 작성에 필요한 Groovy 문법(`def`·GString·컬렉션·클로저·Elvis `?:`·Safe Navigation `?.`)을 구분해 쓰고, `call(Map config = [:])` 진입점 규약과 `@NonCPS`·`Serializable`이 CPS 변환과 어떻게 얽히는지 설명하며, 큰따옴표/작은따옴표가 `sh` step에서 왜 다르게 동작하는지 판단할 수 있습니다.
...
Pipeline 실행 중에 Jenkins가 재시작되어도 중단된 지점부터 재개할 수 있도록, Pipeline의 모든 상태를 디스크에 직렬화하기 때문입니다. 이것이 src/ 클래스에 `implements Serializable`이 필요한 이유입니다.
