...
Durability 레벨의 차이는 비정상 종료 상황에서 가장 명확하게 드러납니다. FlowNode를 매 step마다 기록하는 `MAX_SURVIVABILITY`와 완료 시에만 저장하는 `PERFORMANCE_OPTIMIZED`가 서로 다른 결과를 냅니다. 공식 문서는 이 차이를 정상 종료(graceful shutdown)와 비정상 종료(dirty shutdown)로 가릅니다. `/exit` 엔드포인트나 정상 서비스 종료처럼 graceful하게 내려가면 어느 모드든 재개할 수 있지만, `SIGKILL`이나 컨테이너 강제 종료 같은 dirty shutdown이 닥치면 `PERFORMANCE_OPTIMIZED`는 디스크 I/O를 최소화하느라 직전 상태를 디스크에 남기지 못해 재개에 실패하고 Freestyle 잡처럼 동작합니다. 반면 `MAX_SURVIVABILITY`는 transient data까지 자주 기록하므로 dirty shutdown 후에도 마지막 체크포인트부터 살아 돌아옵니다. 실행 중 Pipeline 상태는 `program.dat`에 직렬화됩니다. (출처: jenkins.io/doc/book/pipeline/scaling-pipeline)
