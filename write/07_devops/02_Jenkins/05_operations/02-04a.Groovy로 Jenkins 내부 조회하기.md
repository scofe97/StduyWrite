...
# Groovy로 Jenkins 내부 조회하기 (Internal API)
...
## 3. Groovy 커스터마이징 카탈로그 — 영역별 조회·조작

> §1·§2 가 *조회* 중심이라면, 본 절은 Groovy 로 *조작* 까지 가능한 영역을 카테고리별로 모은 레퍼런스입니다. 모든 예시는 Script Console 또는 init.groovy.d 에서 실행합니다(Pipeline Sandbox 에서는 대부분 차단).


### 1. Job/Pipeline 관리

> Groovy로 Job 목록 조회·활성화/비활성화·설명 변경과 Freestyle·Pipeline Job 프로그래밍 생성을 다룹니다.
...
> Groovy로 노드 상태 조회, Agent 라벨·Executor 수 변경, 오프라인 전환, SSH Agent 프로그래밍 추가를 다룹니다.
...
> Groovy로 Username/Password·Secret Text·SSH Key 크레덴셜을 등록하고 기존 크레덴셜을 삭제하는 방법을 다룹니다.
...
> Groovy로 전역 환경변수를 조회하고 추가·수정하는 방법을 다룹니다.
...
> Groovy로 LDAP 인증, Matrix 기반 인가, Markup Formatter 등 보안 영역을 설정하는 방법을 다룹니다.
...
> Groovy로 대기 중인 빌드 조회·취소, 실행 중인 빌드 조회, 특정 빌드 강제 중단을 다룹니다.
...
> Groovy로 SMTP 설정과 시스템 관리자 이메일·URL을 변경하는 방법을 다룹니다.
...
> 장기간 오프라인 상태인 정적 Agent를 임계 기간 기준으로 자동 정리하는 스크립트와 그 적용 조건을 다룹니다.
...
> 영역별로 Script Console·init.groovy.d·JCasC 대안을 비교해, JCasC로 가능하면 JCasC를 쓰고 전역 Hook만 init.groovy.d가 정당함을 정리합니다.
