...
> 이 문서를 읽고 나면 Jenkins API를 HTTPS로 호출할 때 SSL 인증서가 어느 위치(JVM·리버스 프록시·Ingress)에서 종료되는지 구분하고, 자체 서명 환경에서 `-k`와 `--cacert`·truststore의 트레이드오프를 설명하며, Agent 내부 curl의 인증서 문제를 환경별로 대응할 수 있습니다.

## §사전 지식

> HTTP/HTTPS와 TLS 핸드셰이크의 기본 개념, `01-00`의 실습 환경, 그리고 크레덴셜을 API로 주고받는다는 사실(`01-07`)을 알고 있으면 SSL이 왜 사실상 필수인지 이해하기 쉽습니다.
