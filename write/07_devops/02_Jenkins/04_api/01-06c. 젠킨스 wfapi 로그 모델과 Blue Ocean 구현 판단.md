...
> `wfapi/log`의 핵심은 범위다. 범위가 build 전체가 아니라 `execution/node/{nodeId}` 기준이라는 점을 먼저 잡아야 합니다.
...
> 로그 적재 전략은 목적에 따라 달라진다. 단순 보관이면 전체 로그 1개로 충분하지만, stage별 과거 로그 UX까지 원하면 `build + nodeId` 단위 보존이 실용적입니다.
