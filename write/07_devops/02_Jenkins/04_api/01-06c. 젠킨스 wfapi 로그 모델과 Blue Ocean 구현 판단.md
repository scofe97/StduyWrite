...
> `wfapi`는 파이프라인 구조와 node 로그를 잘 보여주지만, build 전체 로그를 구조화해서 한 번에 주는 run-level 로그 API는 제공하지 않는입니다.
...
> `nodeId`를 Jenkins agent 이름으로 이해하면 거의 반드시 모델이 꼬인입니다. `nodeId`는 파이프라인 실행 그래프 안의 FlowNode 식별자입니다.
...
반대로 아래처럼 `nodeId`만 단독 키로 쓰면 위험하입니다:
...
이 방식이 아니면 stage 수가 많은 파이프라인에서 불필요한 호출이 급격히 늘어납니입니다.



## 7. Blue Ocean를 `wfapi`로 완전히 복제할 수는 없습니다

> `wfapi`는 Blue Ocean-like 화면의 상위 70~80% 정도는 충분히 커버하지만, step 레벨 parity까지 기대하면 부족하입니다.
...
즉 UI는 Blue Ocean처럼 보이게 만들 수 있어도, 내부 데이터 모델은 `step`보입니다 `flow node` 중심으로 설계하는 편이 더 정확하입니다.



## 8. 로그 적재는 전체 로그만으로도 가능하지만, Blue Ocean-like UX까지 원하면 node 단위 보존이 유리하입니다
...
- 과거 이력 화면 성능이 Jenkins 상태에 묶인입니다.
...
- 수집과 저장 구조가 복잡해집니다.
- node 개수가 많은 파이프라인에서는 파일 수도 늘어납니입니다.
