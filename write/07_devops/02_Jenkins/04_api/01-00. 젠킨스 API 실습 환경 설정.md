...
> `01-02`부터 `01-08`까지의 문서에서 반복되는 공통 준비를 한 곳으로 모은다.
>
...
> - 각 방식의 상세 스펙과 이론은 `01-02`, `01-02a`, `01-02b`에서 다룬다.
...
`01-03` 이후 문서에서 나오는 아래 값은 이 문서에서 설정한다:
...
> `01-03` 이후 문서들은 실습용 폴더, 파이프라인, 크레덴셜, 승인 ID를 여러 번 재사용한다. 이름을 한 곳에서 정해 두면 이후 문서에서 동적 변수만 추가하면 된다.
>
...
>
> - `01-00`에서 공통값을 한 번 설정하면 이후 문서에서 바로 API 호출로 진입할 수 있다.
