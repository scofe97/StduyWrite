...
> **본 문서는 spec(`01-05.md`)을 읽었다고 가정한 운영 해석과 TPS 매핑 패턴**이다. 상태 조회 endpoint(`/api/json`, `wfapi/describe`, `/queue/api/json`)의 호출 형식은 spec에 있다. 이 문서는 그 위에서 TPS가 DB·매핑·폴링을 어떻게 설계하는지를 정리한다.
>
