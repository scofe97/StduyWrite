...
큐가 길면 기본 `/queue/api/json` 응답이 커지므로, 큐 조회에서도 `tree=`로 필요한 필드만 골라 받습니다.
...
`tree=`·`depth=`의 동작 원리와 응답 축소 효과(수십 KB→수백 바이트)는 [09-03. API 쿼리 최적화와 운영](09-03.API%20%EC%BF%BC%EB%A6%AC%20%EC%B5%9C%EC%A0%81%ED%99%94%EC%99%80%20%EC%9A%B4%EC%98%81.md)에서 다룹니다.
...
> 큐가 막히거나 비는 이유를 진단하기 위해 `/computer/api/json`(전체 노드·executor 현황)과 `/computer/{agentId}/api/json`(단일 에이전트 상세)으로 실행기 상태를 읽는 GET API를 다룹니다.
...
`totalExecutors` 값과 `computer[]._class`(VM 정적 vs K8s 동적)를 어떻게 읽어 실행기 환경을 판별하는지는 [05-03. Queue 적재 이후 실행 흐름과 데이터 추적](05-03.Queue%20%EC%A0%81%EC%9E%AC%20%EC%9D%B4%ED%9B%84%20%EC%8B%A4%ED%96%89%20%ED%9D%90%EB%A6%84%EA%B3%BC%20%EB%8D%B0%EC%9D%B4%ED%84%B0%20%EC%B6%94%EC%A0%81.md)에서 다룹니다.
