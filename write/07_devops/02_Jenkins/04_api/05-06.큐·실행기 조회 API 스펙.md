...
crumb(CSRF 방어)은 상태를 바꾸는 POST 요청 대상입니다. 이 문서의 큐·실행기 조회는 모두 읽기(GET)라 CSRF 위험이 없으므로 crumb·cookie 없이 Basic Auth만으로 호출됩니다. 상태 변경 POST는 05-01와 05-05에서 다룹니다.
...
- `05-01.빌드 실행·큐 API 스펙.md` (트리거·빌드 실행 본편)
- `05-05.빌드 중지·취소 API 스펙.md`
- `05-02.빌드 실행·큐 모델과 TPS 패턴 (2.222+).md`
- `05-03.큐-빌드 전환 흐름과 실행기 환경.md`
