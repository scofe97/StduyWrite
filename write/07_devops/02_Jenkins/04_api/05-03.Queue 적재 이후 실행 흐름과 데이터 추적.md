...
`buildNumber`를 확보하려고 `/queue/item/{queueId}/api/json` 전체를 받으면 큐 아이템 메타데이터가 수 KB 단위로 따라옵니다. `executable.number` 한 필드만 필요할 때는 `tree=executable[number]`로 응답을 수백 바이트 수준으로 줄일 수 있습니다. `tree=`는 반환 필드를 선택해 응답을 축소하고, `depth=`는 서브트리 깊이를 제어합니다 — 값이 클수록 더 깊은 중첩까지 반환합니다 (출처: jenkins.io/doc/book/using/remote-access-api). 폴링이 잦은 추적 루프에서는 이 둘로 트래픽을 한 자릿수 KB에서 수백 바이트로 떨어뜨리는 것이 비용 차이로 직결됩니다.
...
3. `tree` — `tree=executable[number]`로 반환 필드를 좁혀 응답을 수백 바이트로 축소합니다 (출처: jenkins.io/doc/book/using/remote-access-api).
