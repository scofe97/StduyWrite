...
> 같은 `/computer`·`/queue` 응답이 VM 정적 에이전트와 K8s 동적 에이전트에서 다르게 해석되는 이유를 정리하고, `totalExecutors=0`을 "실행 불가"로 오판하지 않도록 Pod 생명주기와 controller 위치·실행 위치 판별법을 다룹니다.
...
### 2-6. 큐 깊이 게이트와 K8s 프로비저닝 — 외부 제어 관점

외부 시스템이 Jenkins에 빌드를 트리거할 때 흔한 보호 장치가 *큐 깊이 게이트*입니다 — `/queue/api/json`의 적재 수가 임계값을 넘으면 백엔드가 실행 요청을 보내지 않는 방식입니다. 이 게이트는 executor 슬롯이 고정된 VM 환경을 전제로 한 설계라서, K8s 동적 Pod 환경에서는 신호의 의미가 달라집니다.

먼저 K8s에서 큐와 실행기의 관계를 정확히 잡아야 합니다. 큐 아이템이 실행기를 직접 만드는 것이 아닙니다. 아이템은 큐에서 그대로 대기하고, *큐의 부하가 수요 신호가 되어* controller의 `NodeProvisioner`가 `LoadStatistics`를 보고 Cloud(KubernetesCloud)에 Pod 프로비저닝을 요청합니다(출처: javadoc.jenkins.io `hudson.slaves.NodeProvisioner` — "Uses the LoadStatistics and determines when we need to allocate new Nodes through Cloud"). 떠오른 Pod agent는 executor를 정확히 1개 갖고(출처: jenkinsci/kubernetes-plugin `KubernetesCloud.java` — `.numExecutors(1)` 고정), agent가 연결되면 `maintain()`이 대기 중인 아이템을 그 executor에 배정합니다. 요약하면 "큐에서 실행기를 만들어 기다린다"가 아니라 *"큐에 쌓인 수요만큼 클라우드가 실행기를 공급하고, 아이템은 공급될 때까지 큐에서 기다린다"* 입니다.

이 공급 방식이 게이트 신호를 바꿉니다.

| 환경 | 큐 깊이의 의미 | 깊이 게이트 유효성 |
|------|---------------|-------------------|
| VM 정적 (executor 고정) | 슬롯 초과분이 그대로 쌓임 — 부하에 비례 | 임계값 게이트가 잘 동작 |
| K8s 동적, cap 미도달 | 아이템마다 Pod가 공급돼 큐가 금방 빔 | 깊이가 0 근처에 머물러 신호가 약함 |
| K8s 동적, `containerCap` 도달 | 초과분이 VM처럼 큐에 쌓임 | 깊이 게이트가 다시 유효 |

`containerCap`(동시 Pod 상한)에 도달하면 KubernetesCloud의 provision이 더 이상 Pod를 계획하지 않으므로 초과 아이템은 큐에 남습니다(출처: jenkinsci/kubernetes-plugin `KubernetesCloud.java` — cap 등록이 실패하면 프로비저닝 루프 중단). 그래서 K8s에서도 cap을 명시하면 큐 깊이 게이트가 VM 때와 같은 의미를 되찾습니다. 반대로 cap 없이 깊이만 보면 게이트가 거의 걸리지 않은 채 Pod가 수요만큼 계속 생성되어, 보호 대상이 Jenkins 큐가 아니라 클러스터 자원으로 바뀌어 버립니다.

백엔드 게이트를 K8s 환경에 맞게 보정하는 방법은 세 가지입니다.

1. **cap을 1차 방어선으로**: `containerCap`(Cloud 단위)·Pod 템플릿 instance cap으로 동시 실행 상한을 Jenkins 쪽에 명시하고, 게이트 임계값은 cap과 연동해 잡습니다.
2. **신호를 합산으로**: 총 부하 = 큐 적재 수 + `busyExecutors`. K8s에서 Pod agent는 executor 1개 고정이므로 `busyExecutors`가 곧 실행 중 Pod 수입니다. 깊이 단독보다 이 합이 VM/K8s 어디서나 같은 의미를 갖습니다.
3. **대기 성격을 구분**: `items[].why`의 "Waiting for next available executor"는 K8s에서 *프로비저닝 진행 중*일 수 있으므로(§2-2), 짧은 대기는 정상으로 두고 임계 판정은 대기 지속 시간과 함께 봅니다.
...
4. 큐 적재 수로 실행 요청을 막는 백엔드 게이트가 K8s 동적 Pod 환경에서 약해지는 이유와 보정 방법은?
...
5. K8s 동적 Pod 환경에서 동시 Pod 상한을 정하는 Kubernetes plugin 설정은 `______`이고, 이 상한에 도달하면 초과 아이템은 ______에 쌓입니다.
...
### 정답 4 — K8s에서의 큐 깊이 게이트

cap 미도달 상태의 K8s는 아이템마다 Pod를 공급해 큐가 금방 비므로, 깊이가 부하를 반영하지 못해 게이트가 거의 걸리지 않습니다. 보정은 ① `containerCap`으로 동시 Pod 상한을 명시해 초과분이 큐에 쌓이게 만들고, ② 게이트 신호를 "큐 적재 수 + `busyExecutors`(=실행 중 Pod 수)" 합산으로 바꾸고, ③ "Waiting for next available executor"가 프로비저닝 대기일 수 있으니 대기 지속 시간과 함께 판정하는 것입니다(§2-6).
...
5. `containerCap` / 큐 — cap 도달 시 provision이 멈춰 초과 아이템이 VM처럼 큐에 대기합니다(출처: jenkinsci/kubernetes-plugin KubernetesCloud.java).
