...
응답 크기를 다루는 손잡이는 두 개입니다. `depth=`는 서브트리를 얼마나 깊이 펼칠지를 정해, 값이 클수록 더 깊은 중첩까지 반환합니다. `tree=`는 거꾸로 **반환할 필드만 골라** 응답을 줄입니다. 둘은 방향이 반대라, "넓게 펼치되 필요한 가지만"을 원하면 `depth=`로 펼친 뒤 `tree=`로 솎아 내는 식으로 같이 씁니다. 응답 헤더 `X-Jenkins`에는 인스턴스의 버전이 실려 오므로, 클라이언트가 버전별 분기를 할 때 본문을 파싱하지 않고 헤더 한 줄로 판별할 수 있습니다(출처: jenkins.io/doc/book/using/remote-access-api).

`tree=`는 **이미 받은 응답에서 jq로 거르는 것과는 다릅니다**. jq는 큰 응답을 다 받아 온 뒤 클라이언트에서 잘라 내지만, `tree=`는 Jenkins가 직렬화 단계에서부터 필드를 생략하므로 네트워크로 나가는 바이트 자체가 줄어듭니다. 빌드 200건의 잡에서 전체 응답이 수십 KB라면, `tree=builds[number,result]`로 두 필드만 고르면 수백 바이트 수준까지 떨어집니다. 이 비유는 "주문서에 필요한 칸만 체크해서 보낸다"까지 유효하고, `tree=`가 지원하지 않는 계산·집계(예: 평균 빌드 시간)는 여전히 받아 와서 클라이언트가 처리해야 한다는 점에서 깨집니다.
...
`tree` 파라미터는 Jenkins API에서 특히 중요합니다. 기본 응답은 모든 필드를 포함하므로 빌드 이력이 쌓인 잡에서는 응답이 수십 KB에 이르는데, `tree=builds[number,result]`처럼 두 필드만 지정하면 응답이 수백 바이트 수준으로 줄어 네트워크 전송량과 파싱 시간을 함께 줄입니다(출처: jenkins.io/doc/book/using/remote-access-api). 폴링 주기가 짧은 운영 대시보드에서는 이 차이가 그대로 부하 차이로 누적됩니다.
