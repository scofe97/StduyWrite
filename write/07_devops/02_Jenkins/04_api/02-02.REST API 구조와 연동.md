...
> Jenkins REST API의 전체 구조와 고급 API, Webhook 연동, CLI, 외부 시스템 연동 패턴을 다룹니다.
> 실습 환경 설정은 02-01 참조
...
> 02-01에서 본 "UI 경로 + `/api/json`" 기본 구조를 알고 있다면, 이 문서는 그 기본 위에 stage 단위 조회·승인 자동화·시크릿 CRUD 같은 실무 API를 얹어 일반화한 것입니다.
