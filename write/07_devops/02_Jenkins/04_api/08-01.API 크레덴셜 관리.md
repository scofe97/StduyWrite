...
> 시스템 스토어의 도메인 목록을 조회하는 GET과 새 도메인을 만드는 POST를 다루며, 생성은 성공·중복 모두 302라 `Location` 헤더로 구분하는 방법까지 설명합니다.
...
> 같은 `createCredentials` 엔드포인트에서 `$class` 판별자만 바꿔 Username/Password·SSH Key·Secret Text를 생성하는 방법을 타입별로 다룹니다.
...
> 기존 크레덴셜을 `updateSubmit`으로 수정하며, 생성과 거의 같은 요청 구조에 경로로 대상 `credentialId`를 지정하는 방법을 다룹니다.
...
> 특정 크레덴셜을 `doDelete`로 삭제하며, crumb과 세션 쿠키를 동반한 POST 호출 방법을 다룹니다.
