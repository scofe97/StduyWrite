...
`tree=`로 필드를 선택하면 응답이 크게 줄어듭니다. Jenkins Remote Access API는 `tree=`로 반환 필드를 선택해 응답을 축소하도록 설계돼 있어(출처: jenkins.io/doc/book/using/remote-access-api), 도메인이 수십 개인 스토어에서 전체 객체를 받으면 응답이 수십 KB까지 부풀지만 위처럼 3개 필드만 선택하면 수백 B 수준으로 떨어집니다. 더 깊은 중첩이 필요하면 `depth=` 파라미터로 서브트리 깊이를 키워 가며(클수록 더 깊은 중첩 반환) 받습니다.
...
여기서 crumb을 붙이는 이유는 `useCrumbs: true`를 전제로 하기 때문입니다. crumb은 Default Crumb Issuer가 사용자명·웹 세션 ID·인스턴스 고유 salt를 인코딩해 발급하는 CSRF 방지 토큰이고, 클라이언트는 발급받은 crumb과 세션 쿠키를 이후 POST에 함께 보내야 합니다(출처: jenkins.io/doc/book/security/csrf-protection). 다만 API token 인증 요청은 CSRF(crumb)가 면제되므로, crumb 대신 API token으로 Basic 인증하면 crumb 발급 단계를 생략할 수 있습니다. 공식 문서도 "API tokens are preferred instead of crumbs for CSRF protection"이라고 명시합니다(출처: jenkins.io/doc/book/using/remote-access-api). 노출 시 토큰만 폐기하면 되고 비밀번호는 그대로 유지되므로, API token이 비밀번호보다 권장됩니다(출처: jenkins.io/doc/book/security/managing-security).
