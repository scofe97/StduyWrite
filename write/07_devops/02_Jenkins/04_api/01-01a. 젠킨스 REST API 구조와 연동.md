...
> Jenkins REST API의 전체 구조와 고급 API, Webhook 연동, CLI, 외부 시스템 연동 패턴을 다룹니입니다.
> 실습 환경 설정은 01-00 참조

## §학습 목표

> 이 문서를 읽고 나면 Blue Ocean·wfapi·Input·Credentials·Computer 고급 API를 용도별로 구분해 선택하고, SCM Webhook과 Generic Webhook Trigger 중 상황에 맞는 연동을 고르며, Jenkins를 외부 시스템과 Push/Pull 두 방향으로 연결하는 패턴을 설계할 수 있습니다.

## §사전 지식

> 01-01에서 본 "UI 경로 + `/api/json`" 기본 구조를 알고 있다면, 이 문서는 그 기본 위에 stage 단위 조회·승인 자동화·시크릿 CRUD 같은 실무 API를 얹어 일반화한 것입니다.
...
사용자별로 발급하는 토큰으로, Jenkins UI의 `사용자 설정 → API Token`에서 생성합니다. 비밀번호 대신 토큰을 사용하는 이유는 다음과 같습니다:
...
HTTP 요청의 Authorization 헤더에 `username:apiToken`을 Base64 인코딩하여 전달합니다. curl에서는 `-u admin:API_TOKEN` 플래그로 간편하게 사용할 수 있습니다.

**CSRF Protection (crumb)**

Jenkins는 POST 요청에 대해 CSRF(Cross-Site Request Forgery) 방지를 적용합니다. POST 요청을 보내기 전에 `/crumbIssuer/api/json`에서 crumb 값을 발급받고, 이후 요청의 헤더에 `Jenkins-Crumb: {crumb값}`을 포함해야 합니다. 이 메커니즘이 필요한 이유는 악의적인 웹 페이지가 사용자 브라우저의 Jenkins 세션을 이용하여 빌드를 트리거하는 공격을 방지하기 위해서입니다.
...
`tree` 파라미터는 Jenkins API에서 특히 중요합니다. 기본 응답은 모든 필드를 포함하여 데이터가 크기 때문에, `tree`로 필요한 필드만 지정하면 네트워크 전송량과 파싱 시간을 크게 줄일 수 있습니다.
...
Blue Ocean API와 wfapi 중 어느 것을 선택할지는 용도에 따라 다릅니입니다:
...
>
> - **SCM Webhook**: GitHub/GitLab이 코드 변경 이벤트를 Jenkins에 알립니입니다.
...
Slack slash command, AlertManager, JIRA 같은 비-SCM 시스템이 Jenkins를 트리거해야 할 때 Generic Webhook Trigger를 씁니입니다. 다만 인증이 URL의 `token` 파라미터에 의존하므로, 그 URL이 로그·프록시·브라우저 히스토리에 남으면 토큰이 노출될 수 있어 SCM Webhook의 서명 검증보다 상대적으로 취약합니다.
