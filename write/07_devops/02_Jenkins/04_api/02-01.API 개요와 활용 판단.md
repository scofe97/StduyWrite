...
`tree=`는 응답에서 가져올 필드만 선택하고, `depth=`는 서브트리를 얼마나 깊이까지 펼칠지 제어합니다. `depth` 값이 클수록 더 깊은 중첩까지 반환되므로 응답이 빠르게 커집니다(출처: jenkins.io/doc/book/using/remote-access-api). 이 두 파라미터를 모르면 불필요한 데이터를 매번 전체로 받게 되어 응답이 수 MB에 달할 수 있습니다. 상세는 09-03에서 다룹니다.
...
폴링처럼 같은 호출을 초당 반복하는 구간에서는 이 차이가 누적됩니다. 빌드 100개가 쌓인 Job을 `tree` 없이 조회하면 수십 KB~수 MB가 매 호출마다 오가지만, `tree=lastBuild[number,result]`로 좁히면 수백 바이트로 떨어집니다. 응답 크기가 수천 배 차이 나는 만큼 Controller 부하와 네트워크 비용도 그만큼 줄어듭니다.
...
crumb은 CSRF(Cross-Site Request Forgery) 방지 토큰으로, `/crumbIssuer/api`에서 발급받아 이후 POST에 헤더로 동반합니다. Default Crumb Issuer는 username, web session ID, 인스턴스 고유 salt를 인코딩해 만듭니다(출처: jenkins.io/doc/book/security/csrf-protection). 공식 문서는 CSRF 보호 측면에서 crumb보다 API token을 권장합니다 — "API tokens are preferred instead of crumbs for CSRF protection."(출처: jenkins.io/doc/book/using/remote-access-api). API token으로 인증한 요청은 crumb(CSRF) 검사가 면제되므로(출처: jenkins.io/doc/book/security/csrf-protection), 헤더 하나로 POST까지 끝납니다. token은 노출되어도 그 token만 폐기하면 되지만 비밀번호는 바꾸기 어려우므로 비밀번호 대신 token을 쓰는 편이 안전합니다(출처: jenkins.io/doc/book/security/managing-security). HTTP BASIC 인증은 `--user USER:TOKEN` 형식으로 token을 비밀번호 자리에 넣습니다(출처: jenkins.io/doc/book/using/remote-access-api).

API Token이 가능하다면 반드시 Token을 사용합니다. crumb 방식은 세션 cookie 관리가 필요하고, Jenkins 재시작 시 crumb이 무효화되는 등 운영상 번거롭습니다. 상세는 03-01에서 다룹니다.
