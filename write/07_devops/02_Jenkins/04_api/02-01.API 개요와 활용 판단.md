...
| **로그 조회** | Console 출력, Progressive 로그, Blue Ocean 로그, `wfapi` node 로그, Blue Ocean-like 구현 판단 | 07-01, 07-02, 07-03, 07-04 |
...
Folder Plugin을 사용하면 경로가 중첩됩니다. `프로젝트/프리셋/잡` 구조라면 `/job/PROJECT/job/PRESET/job/JOB`이 됩니다. 이 경로 구성 규칙을 모르면 API 호출 시 404를 만나게 됩니다. 폴더가 무엇이고 왜 이렇게 경로를 깊게 만드는지(full name 식별·namespace 격리)는 [`../06_infra/06-01.Folder 플러그인 — namespace 격리·full name 식별·폴더 스코핑.md`](../06_infra/06-01.Folder%20플러그인%20%E2%80%94%20namespace%20격리%C2%B7full%20name%20식별%C2%B7폴더%20스코핑.md)에서 다룹니다.
...
이 전환 구간은 음식점 대기표에 비유할 수 있습니다. 

- 빌드 트리거(`build`, `buildWithParameters`는 모두 POST입니다 출처: jenkins.io/doc/book/using/remote-access-api)를 보내면 곧바로 자리(Executor)가 나는 게 아니라 먼저 **대기표(queue item)** 를 받습니다. 
- 자리가 비어 안내(Executor 할당)를 받은 다음에야 **테이블 번호(build number)** 가 정해집니다. 대기표 번호로 테이블 번호를 조회하려 들면 아직 없는 값을 참조하게 됩니다. 
- 이 비유는 "대기표 → 테이블 번호로 ID가 한 번 바뀐다"는 지점까지 유효하지만, 대기표를 받자마자 취소(queue cancel)가 가능하고 같은 Job에 여러 대기표가 동시에 쌓일 수 있다는 점에서 단순 식당 대기열과 달라집니다.
...
"빌드가 성공했는가?"만 확인하려면 기본 API로 충분합니다. "어느 stage에서 실패했는가?", "현재 어느 stage가 실행 중인가?"를 알려면 wfapi를 사용해야 합니다. 상태 추적 중심 설명은 `06-01`, 엔드포인트 전체 설명은 `07-03`에서 다룹니다.
