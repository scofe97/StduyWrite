...
crumb은 CSRF 방어용 토큰입니다. Default Crumb Issuer는 사용자명, 웹 세션 ID, 인스턴스 고유 salt를 인코딩해 crumb을 만들고, 클라이언트는 이 crumb과 세션 cookie를 이후 모든 POST에 함께 보내야 합니다(출처: jenkins.io/doc/book/security/csrf-protection). crumb은 `/crumbIssuer/api`(json 또는 xml)로 발급합니다.

crumb은 콘서트 입장 시 손등에 찍어 주는 도장에 비유할 수 있습니다. 한 번 받아 두면 그날 안에서는 재입장이 자유롭지만, 손등 도장(crumb)만으로는 부족하고 입장권 반쪽(세션 cookie)을 함께 보여 줘야 통과합니다. 이 비유는 "발급 한 번 + 재사용"까지는 유효하지만, Jenkins가 재시작되면 salt와 세션이 함께 바뀌어 도장이 무효화된다는 점에서 깨집니다. 콘서트 도장은 행사장 사정으로 갑자기 무효화되지 않지만 crumb은 서버 재시작 한 번에 사라집니다.
...
API Token은 한 번 발급하면 Jenkins 재시작과 무관하게 계속 사용할 수 있습니다. crumb/cookie 관리가 필요 없으므로 자동화에 적합합니다. Jenkins 공식 문서는 CSRF 보호 측면에서 "crumb 대신 API token을 권장(API tokens are preferred instead of crumbs)"하며, **API token으로 인증한 요청은 crumb(CSRF) 검증에서 면제**됩니다(출처: jenkins.io/doc/book/using/remote-access-api, jenkins.io/doc/book/security/csrf-protection). 토큰을 비밀번호 대신 쓰면 노출 시 그 토큰만 폐기하면 되고 계정 비밀번호는 그대로 둘 수 있다는 점도 권장 이유입니다(출처: jenkins.io/doc/book/security/managing-security).
...
```bash
# GET 요청 — tree= 로 필요한 필드만 골라 응답을 줄인다
# tree 없이 /api/json 을 치면 루트 객체 전체가 수십 KB 로 내려오지만
# tree=mode,nodeDescription 처럼 두 필드만 고르면 응답이 수백 B 로 줄어든다
...
- 어떤 객체 URL에도 `/api/json`, `/api/xml`, `/api/python`을 붙여 같은 방식으로 조회할 수 있고, `tree=`로 필드를 고르거나 `depth=`로 중첩 깊이를 키워 응답 형태를 조절합니다(출처: jenkins.io/doc/book/using/remote-access-api).
