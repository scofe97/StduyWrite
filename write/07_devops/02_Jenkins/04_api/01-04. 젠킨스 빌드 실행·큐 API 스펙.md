...
응답 해석은 단순 API 조회까지만 이 문서에서 다룬다. VM/K8s 동적 판별과 Pod 생명주기 영향은 `01-04b`에서 별도로 다룬다.
...
`stop` 요청 직후 곧바로 `ABORTED`가 확정되는 것은 아니다. 최종 결과는 `01-05`의 빌드 상태 조회 API에서 확인한다.
...
> `cancelItem` 호출 시 Jenkins 내부에서 일어나는 WaitingItem/BlockedItem/BuildableItem 상태 전이, DB 기록 흐름은 `01-04c. 젠킨스 큐 내부 흐름과 실행 순서`에서 다룬다.
