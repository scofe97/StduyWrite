...
> - TPS 운영 패턴, Pre-trigger Guard, controller 실행 해석은 `01-04a`에서 별도로 다룹니다.
> - K8s/VM 실행기 환경, 큐-빌드 전환 흐름은 `01-04b`에서 별도로 다룹니다.
> - 큐 내부 메커니즘, 실행 순서, 취소 동작 흐름은 `01-04c`에서 별도로 다룹니다.

## §학습 목표

> 이 문서를 읽고 나면 `build`/`buildWithParameters`로 빌드를 트리거하고, 응답 `Location` 헤더의 `queueId`로 큐 아이템을 추적해 실제 `buildNumber`를 얻으며, `buildWithParameters`의 세 가지 파라미터 전달 방식(form 필드·json·JSON 쿼리)을 구분해 쓸 수 있습니다.

## §사전 지식

> 01-01에서 본 "트리거 직후 받는 값은 build number가 아니라 queue item ID"라는 사실과 01-02의 crumb + cookie POST 준비를 알고 있다면, 이 문서는 그 큐 전환 구간을 실제 엔드포인트와 curl 예시로 구체화한 것입니다.
...
빌드 상세 상태 추적은 이 문서 범위가 아닙니다. 그 내용은 별도 문서에서 다룬다:
...
`{queueId}`는 빌드 트리거 응답의 `Location` 헤더에서 얻습니다. `queue/item/151/`이라면 `151`이 `queueId`다.
...
현재 환경처럼 비밀번호 인증을 쓴다면 POST 전에 crumb과 cookie도 같이 준비해야 합니다. 이 문서의 POST 예시는 아래 준비가 끝났다는 전제로 작성한다:
...
`tree=` 파라미터에 `[]`가 들어가는 Jenkins API는 `curl`이 URL range로 오해할 수 있습니다. 이 경우에는 다음 둘 중 하나를 사용한다:
...
`01-04` 실습 전, `SBH` 폴더 아래에 실행 가능한 파이프라인 5개를 먼저 만듭니다. 구성은 다음과 같다:
...
성공 시 가장 먼저 볼 값은 `Location` 헤더다. 여기서 큐 아이템 URL을 얻습니다.
...
Jenkins UI가 내부적으로 쓰는 `json={parameter:[...]}` 형태다. `Content-Type`은 여전히 `application/x-www-form-urlencoded`이지만, 파라미터 값을 JSON 구조체로 묶어서 보냅니다.
