...
현재 환경처럼 비밀번호 인증을 쓴다면 POST 전에 crumb과 cookie도 같이 준비해야 합니다. crumb·CSRF 보호의 발급 절차와 API 토큰의 crumb 면제는 [03-01. 인증 API 스펙](03-01.%EC%9D%B8%EC%A6%9D%20API%20%EC%8A%A4%ED%8E%99%20%28ID-Password%20%2B%20Crumb%29.md) § "crumb 발급"에서 다룹니다. 이 문서의 POST 예시는 아래 준비가 끝났다는 전제로 작성합니다:
...
> Job의 메타데이터를 받는 `api/json`과 전체 설정을 받는 `config.xml`로 파이프라인을 조회하는 두 엔드포인트를 다룹니다.
...
위 예시는 응답 전체를 받아 `jq`로 골라 보지만, `builds` 배열처럼 무거운 이력 필드를 아예 받지 않으려면 응답 자체를 줄이는 편이 낫습니다. 응답 축소(`tree=`·`depth=`) 상세는 [09-03. API 쿼리 최적화와 운영](09-03.API%20%EC%BF%BC%EB%A6%AC%20%EC%B5%9C%EC%A0%81%ED%99%94%EC%99%80%20%EC%9A%B4%EC%98%81.md)을 참조합니다.
...
> 생성(`createItem`), 전체 교체 수정(`config.xml`), 삭제(`doDelete`)와 Jenkinsfile 검증까지 상태를 바꾸는 POST 엔드포인트들을 다룹니다.
