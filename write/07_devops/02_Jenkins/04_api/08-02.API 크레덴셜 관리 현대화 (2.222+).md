...
여기서 핵심은 `--user USER:TOKEN` 한 줄이 BASIC 인증과 CSRF 면제를 동시에 처리해 crumb 발급 절차 자체가 사라진다는 점입니다. API token이 crumb 검증에서 면제되는 원리와 `/crumbIssuer/api` 발급 절차의 풀설명은 [03-01. 인증 API 스펙](03-01.%EC%9D%B8%EC%A6%9D%20API%20%EC%8A%A4%ED%8E%99%20%28ID-Password%20%2B%20Crumb%29.md) § crumb 발급에서 다룹니다.
...
> Credentials Plugin이 읽기 작업에도 권한을 더 엄격히 요구하게 되어, `401`은 인증, `403`은 권한 문제로 갈라 의심하는 운영 해석을 다룹니다.
...
> 기본 도메인 `_` 대신 `taskCd → domainName` 으로 업무별 도메인을 나누면 경계·조회·권한 관리가 쉬워지는 운영 이점을 다룹니다.
...
도메인이 분리되지 않아 한 도메인에 크레덴셜이 수백 개 쌓이면 조회 응답이 부풀므로, 분리한 뒤 `tree=credentials[id,description]`로 id·설명만 받아 응답을 좁히면 운영 확인이 빨라집니다. `tree=`·`exclude=`로 응답을 축소하는 원리와 "수십 KB→수백 바이트" 효과의 풀설명은 [09-03. API 쿼리 최적화와 운영](09-03.API%20%EC%BF%BC%EB%A6%AC%20%EC%B5%9C%EC%A0%81%ED%99%94%EC%99%80%20%EC%9A%B4%EC%98%81.md)을 참조합니다.
...
> 수정 API인 `updateSubmit` 직접 수정과 삭제-후-재생성 upsert를 비교해, 타입 변경을 한 흐름으로 처리하려 할 때 upsert를 택하는 이유를 다룹니다.
...
> 크레덴셜 API가 깨끗한 REST 자원이 아니라 Jenkins UI 폼 액션에 가까워, 상태 코드·헤더를 먼저 보고 secret 원문을 기대하지 않는 응답 해석법을 다룹니다.
...
> Provider API라는 확장 경로가 있어도 HTTP 호출 기반 자동화의 실제 운영 스펙은 `08-01`의 REST/폼 경로라는 점을 다룹니다.
...
> 인증(`03-01`/`03-02`)·CRUD 경로(`08-01`)·운영 해석(`08-02`)을 상황별로 어느 문서에서 확인할지 구분해 안내합니다.
...
- [03-01. 인증 API 스펙](03-01.%EC%9D%B8%EC%A6%9D%20API%20%EC%8A%A4%ED%8E%99%20%28ID-Password%20%2B%20Crumb%29.md) § "crumb 발급" — crumb·CSRF 면제 발급 절차의 정본
- [09-03. API 쿼리 최적화와 운영](09-03.API%20%EC%BF%BC%EB%A6%AC%20%EC%B5%9C%EC%A0%81%ED%99%94%EC%99%80%20%EC%9A%B4%EC%98%81.md) § "응답 축소" — `tree=`·`exclude=` 응답 축소의 정본
