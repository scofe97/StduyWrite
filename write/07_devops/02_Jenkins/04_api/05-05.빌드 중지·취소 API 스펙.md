...
빌드 중지(`stop`)와 큐 취소(`cancelItem`)는 모두 상태를 바꾸는 쓰기 작업이므로 POST 메서드를 사용합니다. Jenkins 공식 문서가 `build` / `buildWithParameters`를 POST로 규정하는 것과 같은 이유로, 부작용을 일으키는 이 두 조작도 GET이 아니라 POST이며 따라서 CSRF crumb이 필요합니다 (출처: jenkins.io/doc/book/using/remote-access-api).
...
`403`은 crumb 누락일 때 자주 나오는데, API token 인증으로 호출하면 crumb 자체가 면제되므로 이 클래스의 실패를 줄일 수 있습니다. Jenkins 공식 문서는 crumb보다 API token을 CSRF 보호 측면에서 더 권장합니다 (출처: jenkins.io/doc/book/using/remote-access-api).
...
판단 기준은 `GET /queue/item/{queueId}/api/json`의 `executable` 필드입니다. 이 필드 하나만 확인하면 분기가 결정되므로, 두 API를 순서대로 시도하며 404를 받아보는 시행착오를 피할 수 있습니다. `tree=executable[number]` 같은 필드 선택을 붙이면 응답을 수십 KB 전체 큐 항목에서 수백 바이트의 필요한 필드만으로 줄일 수 있습니다 (출처: jenkins.io/doc/book/using/remote-access-api).
