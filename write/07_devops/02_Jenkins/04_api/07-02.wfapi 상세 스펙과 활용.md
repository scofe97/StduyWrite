...
>
> `wfapi` 로그 범위, `nodeId` 의미, Blue Ocean-like UI/저장 전략 판단은 `07-03.wfapi 로그 모델과 Blue Ocean 구현 판단.md`에서 별도로 정리합니다.
...
폐기 이유를 한 줄로 옮기면 "더 현대적인 대안이 등장해서"입니다. 공식 문서는 "시간이 지나면서 Blue Ocean을 대체할 더 현대적인 도구들이 생겨났다(more modern tools have cropped up to replace it)"고 직접 밝힙니다 (출처: jenkins.io/doc/book/blueocean). Blue Ocean이 처음 나오던 시절 Jenkins UI에 요구되던 디자인 기준을 채우려 만들어졌지만, 그 코드베이스가 동결(frozen)된 사이 비슷한 기능을 더 활발히 유지보수하는 플러그인들이 자리를 잡았다는 뜻입니다. 그래서 폐기의 핵심 신호는 "기능이 부족해서"가 아니라 "더 이상 *개발되지 않아서*"입니다.
...
여기서 한 가지 구분이 중요합니다. 공식 문서는 UI 대안을 명시할 뿐, "백엔드 상태 추적을 `wfapi`로 갈아타라"고 직접 말하지는 않습니다. `wfapi`를 제공하는 `Pipeline: REST API` 플러그인은 별도로 활발히 유지보수되고(HAL 표준 hyperlinked REST, 다수 컨트롤러에 설치) Blue Ocean 없이도 동작하므로, "폐기 예정인 Blue Ocean REST"와 "유지되는 `wfapi`"라는 두 사실을 잇는 것은 *실무 추론*입니다. 그래서 이 문서의 권고는 공식 명령이 아니라 "동결된 의존을 새로 묶지 말고, 유지되는 계층으로 신규 구현을 가져가자"는 운영 판단으로 읽어야 합니다.

세 계층(코어 빌드 API · `wfapi` · Blue Ocean REST)의 유지보수 상태와 대체 관계를 한눈에 보면 다음과 같습니다.

![코어 API·wfapi·Blue Ocean 세 계층의 유지보수 상태와 대체 관계](_assets/07-02.api-layers-maintenance.svg)
...
- [07-03. wfapi 로그 모델과 Blue Ocean 구현 판단](07-03.wfapi%20%EB%A1%9C%EA%B7%B8%20%EB%AA%A8%EB%8D%B8%EA%B3%BC%20Blue%20Ocean%20%EA%B5%AC%ED%98%84%20%ED%8C%90%EB%8B%A8.md) § "nodeId 의미" — 로그 범위·UI 구현·저장 전략을 FAQ처럼 정리합니다.
