...
> 06-01에서 본 기본 빌드 API와 wfapi의 관계를 알고 있다면, 이 문서는 wfapi 엔드포인트 집합의 상세 스펙과 실전 활용으로 파고든 편입니다.
...
- `06-01.빌드 상태 추적 API 스펙.md` — run 상태 추적 시나리오 안에서 `wfapi/describe`를 사용합니다.
- `06-03.상태 추적 API 현대화와 Blue Ocean 해석.md` — Blue Ocean 축소와 `wfapi` 중심 전환 배경을 설명합니다.
- `07-01.API 로그 조회와 적재.md` — `wfapi/log`와 전체 로그 적재를 함께 봅니다.
- `07-03.wfapi 로그 모델과 Blue Ocean 구현 판단.md` — `wfapi` 로그 범위, `nodeId`, UI 구현과 저장 전략을 FAQ처럼 정리합니다.
