...
> - 완료 빌드의 로그 적재 흐름과 TPS의 2-Job 분리 구조, 최근 버전 변화는 본 문서 §6~§8에서 이어서 다룹니다.
...
이 네 갈래가 목적별로 어떻게 갈리고, Blue Ocean 폐기(2026-07 예정)가 그중 어느 경로를 신규 구현에서 밀어내는지를 한 장으로 보면 다음과 같습니다.

![로그 조회 네 갈래의 목적별 선택과 Blue Ocean 폐기 영향](_assets/07-01.log-api-decision.svg)
...
> `consoleText`로 완료된 빌드의 전체 콘솔 출력을 한 번의 GET으로 받는 방법과, 실행 중 빌드 추적에는 부적합한 이유를 다룹니다.
...
> `progressiveText`로 `start` offset과 `X-Text-Size`·`X-More-Data` 헤더를 써서 실행 중 빌드의 추가된 로그만 받아 폴링하는 방법을 다룹니다.
...
> **읽기 전 주의**: Blue Ocean은 공식 문서 기준 2026년 7월 폐기 예정이라(출처: jenkins.io/doc/book/blueocean), 이 절의 `/blue/rest/` 드릴다운을 *신규* 구현의 1차 경로로 삼지 않는 편이 안전합니다. 폐기 이유와 `wfapi`로의 전환 판단은 [07-03 §6-1](07-03.wfapi%20상세%20스펙과%20활용.md)에 정리해 두었습니다. 이 절은 step 단위 로그가 꼭 필요한 *기존* 화면을 이해·유지보수하기 위한 참고로 읽습니다.
...
- GET 요청이므로 CSRF crumb은 불필요하고 Basic Auth 헤더만으로 인증이 끝납니다. crumb이 상태 변경 POST에만 필요한 이유, API token 인증 시 POST마저 면제되는 원리, 그래서 공식 문서가 crumb보다 API token을 권장하는 근거는 [03-01. 인증 API 스펙](03-01.%EC%9D%B8%EC%A6%9D%20API%20%EC%8A%A4%ED%8E%99%20%28ID-Password%20%2B%20Crumb%29.md) § "Crumb 발급"에서 다룹니다.
...
TPS는 로그 수집을 빌드 실행 중에 잡지 않습니다. 상태 추적 Job이 빌드 종료를 먼저 확정하고, 그 뒤에 별도 적재 Job이 종료·미적재 대상만 골라 완료된 빌드의 전체 로그를 한 번에 가져옵니다. 이렇게 "실시간 표시"와 "종료 후 적재"를 두 Job으로 나누면, 실행 중 로그 폴링이 적재 경로를 건드리지 않아 두 책임이 서로 간섭하지 않고 각 Job의 주기·재시도를 독립적으로 조정할 수 있습니다.
...
적재 Job이 끝나지 않은 빌드를 제외하는 이유는, 부분 로그만 박제되는 것을 막기 위해서입니다. 적재 후속의 테스트 결과 콜백은 Feign POST 호출이라, ID/Password 환경에서는 crumb이 필요하고 API Token 환경에서는 면제될 수 있습니다. 인증 모델 차이는 `03-02`를 참조합니다.
...
| Blue Ocean Plugin (2022~) | UI maintenance mode (신규 기능 중단) | `/blue/rest/`는 남았으나 신규 핵심 의존성에서는 부담 |
| 2.462 (2024) | 로그 인코딩 UTF-8 표준화 | 한글 깨짐 위험 감소 |
| Blue Ocean (2026-07 예정) | 공식 폐기(deprecated), 이후 보안·기능 업데이트 없음 | `/blue/rest/` 신규 의존 회피, step 로그는 기존 화면에 한정 |
...
Blue Ocean은 공식 문서 기준 2026년 7월 폐기 예정이라(출처: jenkins.io/doc/book/blueocean) `/blue/rest/` 로그 경로에 신규 의존을 새로 묶지 않는 편이 안전합니다. Blue Ocean 의존도 판단은 `07-04`(wfapi 로그 모델과 Blue Ocean 구현 판단)에 정리해 두었습니다.
...
- [07-03. wfapi 상세 스펙과 활용](07-03.wfapi%20%EC%83%81%EC%84%B8%20%EC%8A%A4%ED%8E%99%EA%B3%BC%20%ED%99%9C%EC%9A%A9.md) § "wfapi endpoint 집합" — 본 문서가 요약한 `wfapi/log`·`wfapi/describe` 필드 스펙의 상세 출처
- [07-04. wfapi 로그 모델과 Blue Ocean 구현 판단](07-04.wfapi%20%EB%A1%9C%EA%B7%B8%20%EB%AA%A8%EB%8D%B8%EA%B3%BC%20Blue%20Ocean%20%EA%B5%AC%ED%98%84%20%ED%8C%90%EB%8B%A8.md) § "Blue Ocean 의존도 판단" — §5 드릴다운을 신규 의존성으로 채택할지 판단하는 기준
