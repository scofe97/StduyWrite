...
>
> - 코어 빌드 상세 조회, 최근 빌드 조회, Workflow API 상태 조회, 실행 목록 조회를 다룹니입니다.
> - TPS 상태 매핑, DB 매칭, SyncJob, Blue Ocean 해석은 `01-05a`에서 별도로 다룹니입니다.
...
- 빌드 실행과 큐 확보는 이 문서 범위가 아닙니다. 그 내용은 별도 문서에서 다룹니입니다:
...
`{buildNumber}`는 `01-04`에서 확보한 큐 아이템의 `executable.number` 또는 `lastBuild.number`에서 얻는입니다.
...
모든 예시는 `01-04`까지의 준비가 끝났다는 전제입니다.
...
>
> 1. `POST /build` 또는 `POST /buildWithParameters`로 상태를 만듭니입니다.
