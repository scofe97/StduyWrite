...
> - TPS 상태 매핑, DB 매칭, SyncJob, Blue Ocean 해석은 `01-05a`에서 별도로 다룬다.
...
`{buildNumber}`는 `01-04`에서 확보한 큐 아이템의 `executable.number` 또는 `lastBuild.number`에서 얻는다.
...
- 즉 이 문서는 "조회 API를 아무 순서로나 나열"하기보다, `01-04`에서 빌드를 시작한 뒤 어떤 조회를 이어서 봐야 하는지 기준으로 목차를 구성한다.
