...
ID/Password 환경에서는 `build`/`buildWithParameters`/`stop` POST에 `crumb` + `cookie` + `Basic Auth`를 모두 실어야 합니다. API Token 환경(2.222+)에서는 `Authorization: Basic <user:token>` 한 줄로 통일되어 crumb·cookie를 제거할 수 있습니다. 다만 실제 면제 여부는 인스턴스 설정에 따라 갈리므로 TPS는 환경 메타데이터로 인증 모델을 분리하는 편이 안전합니다. 자세한 모델 비교는 `03-02.md` §1 참조.
...
- `03-01.md`, `03-02.md` — 인증 spec과 모델 변화
- `05-01.md` — 빌드 실행·큐 API 스펙
- `05-03.md` — 큐-빌드 전환 흐름, K8s/VM 실행기
- `05-04.md` — 큐 내부 메커니즘
- `06-01.md` — 빌드 상태 추적
