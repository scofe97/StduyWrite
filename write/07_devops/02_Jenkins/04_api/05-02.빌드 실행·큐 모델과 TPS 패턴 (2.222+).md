...
> 트리거 직전 `nextBuildNumber`를 미리 읽어 큐 폴링 없이 곧 시작될 빌드 번호를 추정하는 TPS 내부 최적화와, 동시 트리거 시 깨지는 한계를 다룹니다.
...
> controller가 K8s에 떠 있어도 `agent any`는 controller built-in node에서 실행될 수 있으며, 실행 위치는 배포 위치가 아니라 에이전트 프로비저닝 방식으로 판별함을 다룹니다.
...
> 전역 executor·Queue 추적의 비용을 포기하고 대상 Job에 가벼운 GET 1회로 중복 실행을 막는 최소안을 권장 전략으로 다룹니다.
...
> 같은 Job을 짧게 여러 번 트리거했을 때 `Location`이 동일하면 실패가 아니라 Jenkins Queue 병합(dedupe)으로 해석하고, 확인·완화 방법을 다룹니다.
...
> API가 직접 트리거하는 환경에서 불필요한 Quiet Period 지연을 `?delay=0sec`로 요청 단위로 건너뛰는 방법과, build 계열은 POST로 호출해야 함을 다룹니다.
...
> ID/Password 환경은 crumb·cookie·Basic Auth를 모두 실어야 하지만 API Token 환경(2.222+)은 CSRF 검사가 면제되어 인증이 한 줄로 단순해짐을 다룹니다.
