...
`redpanda-playground`는 같은 아이디어를 더 단순화해 `JenkinsJobSpec.toApiPath()`에 경로 책임을 넣었습니다. 핵심은 같다 — 경로는 호출부에서 임시로 만들지 않고 도메인 객체가 책임집니다.
...
TPS의 `createPipeline()`은 순수한 create가 아니라 upsert다 — 먼저 존재 여부를 확인하고, 이미 있으면 update로 위임하며, 반대로 update도 대상이 없으면 create로 되돌립니다.
...
Jenkins는 삭제·일부 생성 작업 뒤 `302` 리다이렉트를 반환할 수 있고, 클라이언트가 이를 따라갑니다 `403`처럼 보이는 응답을 받을 수 있습니다. TPS는 이 구간을 그대로 오류로 보지 않고 작업 특성에 맞게 성공으로 간주하는 보정을 둡니다.
...
`redpanda-playground`의 `JenkinsJobSpec`은 이 부분을 더 작게 가져간다 — Freemarker 없이 `toConfigXml()`에서 최소 `flow-definition` XML을 직접 만듭니다. 이 차이는 역할 차이에서 나옵니다. TPS는 다양한 파이프라인 유형과 파라미터 정책을 다뤄야 했고, playground는 create-only라 최소 XML만으로 충분했습니다.

## 3. 현대 Jenkins 환경이 CRUD 코드에 주는 영향

### 3-1. 인증 단순화 — endpoint는 그대로, 호출 전 코드가 얇아집니다

API Token 환경(2.222+)에서는 POST마다 crumb·cookie를 따로 관리할 필요가 줄어들어 생성·수정·삭제 POST와 Jenkinsfile 검증 POST 코드가 동시에 단순해집니다. CRUD endpoint 자체는 그대로이고, 호출 전 인증 준비가 얇아지는 것이 핵심입니다. 자세한 모델 비교는 `03-02.md` 참조.
...
Jenkinsfile 검증은 단일 API로 완결되지 않습니다. Declarative Pipeline 안에 `script {}` 블록이 들어가면 두 가지 문법 세계가 공존하기 때문입니다. Jenkins 공식 Declarative AST 문서는 `script` 블록을 "Declarative subset 검증 없이 실행되는 special step"으로 설명한다([ModelASTScriptBlock Javadoc](https://javadoc.jenkins.io/plugin/pipeline-model-api/org/jenkinsci/plugins/pipeline/modeldefinition/ast/package-summary.html)) — 즉 `pipeline-model-converter/validate`는 `script {}` 안쪽을 깊게 보지 않습니다.
...
4. Jenkinsfile 검증은 단일 API로 완결되지 않는다 — Declarative 껍질(`validate`) → script 내부 문법(`checkScriptCompile`) → 실제 run의 3계층으로 빈틈을 메웁니다.
...
- `04-01.md` — CRUD 및 검증 API 스펙
- `03-02.md` — 인증 모델 변화와 TPS 인증 패턴
