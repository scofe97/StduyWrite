...
- 장점은 단순함입니다. 
- 단점은 로그가 수 MB에 달하는 빌드에서 매 요청마다 전체를 다시 내려받는다는 것입니다. 실행 중인 빌드의 실시간 추적에는 부적합하다.
...
Jenkins에서 폴더 안에 잡을 중첩하면, 코어 API와 Blue Ocean API의 경로 구조가 달라진다. 이 차이를 모르면 404 에러를 만나게 됩니다.
...
이 API는 완료된 build가 있어야 의미가 있으므로, 실습용으로는 `API-FAIL`이나 `API-NORMAL` 같은 짧은 파이프라인이 편하다. 예를 들어 실패 build를 하나 더 만들고 10초 정도 기다린 뒤 build 번호를 다시 잡아두면 됩니다:
...
- GET 요청이므로 CSRF crumb은 불필요하다. Basic Auth 헤더만으로 인증이 완료됩니다.
