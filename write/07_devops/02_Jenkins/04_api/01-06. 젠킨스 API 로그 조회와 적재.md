...
# 젠킨스 API 로그 조회와 적재
---
> 이 문서는 Jenkins 빌드 로그 조회와, 완료된 빌드 로그를 파일시스템에 적재하는 흐름을 설명하는 스펙 문서다.
>
> - 전체 콘솔 로그, 증분 로그, stage/node 단위 로그, Blue Ocean 드릴다운, 완료 빌드의 전체 로그 조회 API를 다룬다.
> - 로그 적재 구현 패턴, 파일 저장 구조, 모듈 역할 분리, 최근 버전 변화는 `01-06a`에서 별도로 다룬다.

## 1. 이 문서의 범위

> 이 문서는 로그 조회와 적재에 직접 사용하는 아래 API와 흐름만 설명한다.
...
빌드 실행과 상태 추적은 이 문서 범위가 아니다. 그 내용은 별도 문서에서 다룬다:
...
코어 Jenkins 로그 API는 `pipelineStruct + buildNumber` 조합을 기본으로 쓴다.
...
Blue Ocean 로그 API는 코어 Jenkins와 경로 구조가 다르다.
...
모든 예시는 [01-00. 젠킨스 API 사전 준비.md](/Users/simbohyeon/Library/CloudStorage/GoogleDrive-tscofet@gmail.com/내%20드라이브/study/runners-high/write/07_devops/02_Jenkins/04_api/01-00.%20젠킨스%20API%20사전%20준비.md) 와 `01-04`, `01-05`까지의 준비가 끝났다는 전제다.
...
이 문서에서는 build 번호와 stage/node 식별자처럼 현재 실행마다 바뀌는 값만 추가로 준비한다:
...
빠른 로그 실습용 build 번호는 먼저 이렇게 잡아두면 된다:
...
GET 로그 API도 POST와 마찬가지로 HTTP 상태 코드, 헤더, 본문을 분리해서 확인할 수 있다. 기본 패턴은 다음과 같다:
...
- plain text 로그 API는 본문이 그대로 텍스트 파일에 저장된다.
- JSON 로그 API는 `body.json`으로 저장한 뒤 `jq`로 확인하면 된다.
- 정상 조회라면 보통 `HTTP_STATUS=200`이 내려온다.

### 1-3. 권장 실행 순서

이 문서는 로그 API를 개별로 설명하지만, 실제 실습은 아래 순서로 보는 편이 자연스럽다.
...
### 2-1. 상태 준비: `consoleText` 조회 전에 성공 build를 만든다

가장 단순한 `consoleText` 조회는 완료된 성공 build를 기준으로 보는 편이 읽기 쉽다.

먼저 성공한 build 하나를 만들어 두면 `consoleText` 확인이 쉽다:
...
> 가장 단순한 방식이다. 특정 빌드의 전체 콘솔 출력을 plain text로 반환한다:
...
- 장점은 단순함이다. 
- 단점은 로그가 수 MB에 달하는 빌드에서 매 요청마다 전체를 다시 내려받는다는 것이다. 실행 중인 빌드의 실시간 추적에는 부적합하다.



## 3. 증분 로그 (Progressive Text)

### 3-1. 상태 준비: 실행 중 build를 먼저 만든다

증분 로그는 실행 중 상태에서 볼 때 의미가 가장 크다.

먼저 실행 중 상태를 만들려면 `API-SLEEP10`을 다시 실행한 뒤 build 번호를 잡아두면 된다:
...
> 실시간에 가까운 로그 추적이 필요하면 `progressiveText`를 사용한다.
>
> 이미 읽은 바이트 수를 `start` 파라미터로 전달하면, Jenkins가 새로 추가된 로그만 내려준다:
...
응답 본문에는 로그 텍스트가 담기고, 핵심 정보는 응답 헤더에 있다:

| 헤더 | 설명 |
|------|------|
| `X-Text-Size` | 현재까지 출력된 총 바이트 수. 다음 요청의 `start` 값으로 사용한다 |
| `X-More-Data` | `true`면 아직 로그가 더 남아 있다. `false`이거나 헤더가 없으면 빌드가 종료된 것이다 |

### 3-3. 폴링 패턴

`start=0`으로 시작하여 응답의 `X-Text-Size`를 다음 `start`로 넘기고, `X-More-Data`가 `false`가 될 때까지 반복하는 bash 폴링 스크립트다:
...
### 4-1. 상태 준비: 실패 build와 stage ID를 먼저 확인한다

전체 로그 대신 실패 stage만 보려면, 실패 build와 `nodeId`를 먼저 잡아두는 편이 좋다.

먼저 실패 build를 만들어 두면 `wfapi/log` 예시를 재현하기 쉽다. `nodeId`가 아직 없다면 같은 build의 `wfapi/describe`에서 stage ID를 같이 확인한다:
...
> 전체 로그가 아니라 특정 스테이지의 로그만 보고 싶다면, `Pipeline: REST API` 플러그인의 node별 로그 엔드포인트를 사용한다:
...
중요한 점은 `wfapi/log`의 범위가 build 전체가 아니라 **특정 node 로그**라는 것이다.
...
즉 `wfapi`도 `stages[]` 아래로 더 내려갈 수는 있지만, 공식 문서 기준 최소 단위는 보통 `stageFlowNodes[]` 쪽에 가깝다.
...
### 5-1. 상태 준비: 실패 build와 `nodeId`, `stepId`를 먼저 잡는다

Blue Ocean 드릴다운은 실패 build를 하나 기준으로 두는 편이 자연스럽다.

`nodeId`, `stepId`를 아직 모르면 먼저 nodes와 steps를 조회해 잡는다:
...
> Blue Ocean REST API는 `/blue/rest/` 경로 아래에 위치한다. 주요 엔드포인트는 다음과 같다:
...
노드 목록 조회 응답은 각 스테이지의 상세 정보를 담고 있다:
...
Jenkins에서 폴더 안에 잡을 중첩하면, 코어 API와 Blue Ocean API의 경로 구조가 달라진다. 이 차이를 모르면 404 에러를 만나게 된다.
...
- 코어 API는 폴더마다 `/job/`을 반복하지만, Blue Ocean은 `/pipelines/`를 반복한다. 

### 5-5. 로그 조회 전체 흐름

전체 로그를 수집하는 경로(A)와 스테이지별로 드릴다운하는 경로(B), 두 가지 접근이 가능하다:
...
- 경로 A는 단순하지만 전체 로그에서 실패 원인을 직접 찾아야 한다. 
- 경로 B는 API 호출이 많지만 실패 지점의 로그만 정확히 추출할 수 있다. 



## 6. 빌드 완료 후 로그 적재

> 로그 적재의 핵심은 단 하나의 Jenkins API 호출이다

```
GET /{pipelineStruct}/{buildNumber}/logText/progressiveText?start=0
```

- `start=0`으로 호출하면 빌드의 전체 콘솔 로그를 한 번에 가져온다. 
- `### 3-2`에서 설명한 증분 로그 API와 동일한 엔드포인트이지만, 빌드가 이미 종료된 상태이므로 한 번의 호출로 전체 로그를 수집한다.

이 API는 완료된 build가 있어야 의미가 있으므로, 실습용으로는 `API-FAIL`이나 `API-NORMAL` 같은 짧은 파이프라인이 편하다. 예를 들어 실패 build를 하나 더 만들고 10초 정도 기다린 뒤 build 번호를 다시 잡아두면 된다:
...
- GET 요청이므로 CSRF crumb은 불필요하다. Basic Auth 헤더만으로 인증이 완료된다.
