...
>
> - 전체 콘솔 로그, 증분 로그, stage/node 단위 로그, Blue Ocean 드릴다운, 완료 빌드의 전체 로그 조회 API를 다룹니입니다.
> - 로그 적재 구현 패턴, 파일 저장 구조, 모듈 역할 분리, 최근 버전 변화는 `01-06a`에서 별도로 다룹니입니다.
...
인증과 crumb/cookie 준비는 별도 문서에서 다룹니입니다:

- `01-02. 젠킨스 인증 API 스펙 (ID-Password + Crumb).md`
- `01-02a. 젠킨스 인증 모델과 TPS 패턴 (2.222+).md`

빌드 실행과 상태 추적은 이 문서 범위가 아닙니다. 그 내용은 별도 문서에서 다룹니입니다:
...
코어 Jenkins 로그 API는 `pipelineStruct + buildNumber` 조합을 기본으로 씁니입니다.
...
즉 이 문서에서는 아래 공통값을 다시 설명하지 않는입니다:
...
### 2-1. 상태 준비: `consoleText` 조회 전에 성공 build를 만듭니입니다
...
- 장점은 단순함입니다. 
- 단점은 로그가 수 MB에 달하는 빌드에서 매 요청마다 전체를 다시 내려받는다는 것입니다. 실행 중인 빌드의 실시간 추적에는 부적합하입니다.



## 3. 증분 로그 (Progressive Text)

### 3-1. 상태 준비: 실행 중 build를 먼저 만듭니입니다

증분 로그는 실행 중 상태에서 볼 때 의미가 가장 큽니입니다.
...
### 5-1. 상태 준비: 실패 build와 `nodeId`, `stepId`를 먼저 잡는입니다

Blue Ocean 드릴다운은 실패 build를 하나 기준으로 두는 편이 자연스럽습니다.

`nodeId`, `stepId`를 아직 모르면 먼저 nodes와 steps를 조회해 잡는입니다:
...
- 코어 API는 폴더마입니다 `/job/`을 반복하지만, Blue Ocean은 `/pipelines/`를 반복합니다. 
...
이 API는 완료된 build가 있어야 의미가 있으므로, 실습용으로는 `API-FAIL`이나 `API-NORMAL` 같은 짧은 파이프라인이 편하입니다. 예를 들어 실패 build를 하나 더 만들고 10초 정도 기다린 뒤 build 번호를 다시 잡아두면 됩니다:
