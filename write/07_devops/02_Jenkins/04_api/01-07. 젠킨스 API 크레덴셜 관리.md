...
>
> - 도메인 목록 조회, 도메인 생성, 크레덴셜 목록 조회, 단건 조회, 생성, 수정, 삭제 API를 다룹니입니다.
> - TPS의 도메인 분리, upsert 패턴, 권한 모델 해석, 최근 버전 변화는 `01-07a`에서 별도로 다룹니입니다.
...
인증 헤더와 crumb/cookie 준비는 별도 문서에서 다룹니입니다:
...
Credentials Plugin API는 `system store` 아래에서 `domainName`과 `credentialId`를 조합해 경로를 만듭니입니다.
