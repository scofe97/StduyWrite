...
이 선택은 폐기 일정으로도 뒷받침됩니다. Jenkins 공식 문서 기준 Blue Ocean은 2026년 7월 폐기 예정이며, 그 이후로는 보안 수정도 기능 업데이트도 받지 않습니다 (출처: jenkins.io/doc/book/blueocean). 폐기 이유는 기능 부족이 아니라 코드베이스가 동결된 사이 더 현대적인 대안이 등장했다는 데 있습니다(상세 근거는 [07-02 §6-1](07-02.wfapi%20상세%20스펙과%20활용.md)). step parity를 얻겠다고 폐기 예정인 Blue Ocean REST에 *신규* 의존을 새로 묶으면, 폐기 시점에 그 경로를 걷어내는 비용이 예약됩니다. 그래서 step 레벨이 정말 필요한 *기존* 화면에만 Blue Ocean을 남기고, 신규 구현은 `flow node` 중심으로 가는 편이 안전합니다.
...
![wfapi의 Blue Ocean 커버리지와 폐기 타임라인 기반 구현 판단](_assets/07-03.wfapi-coverage-decision.svg)
