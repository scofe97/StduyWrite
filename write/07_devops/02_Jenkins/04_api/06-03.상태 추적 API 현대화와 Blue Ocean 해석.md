...
```bash
# 빌드 객체 상태를 JSON 으로 — tree= 로 필요한 필드만 골라 응답을 줄임
# (필드를 안 고르면 전체 트리가 내려와 응답이 수십 KB 까지 커질 수 있음)
curl -s --user "USER:TOKEN" \
  "http://JENKINS/job/pipeline/42/api/json?tree=building,result,duration" \
  | head -c 400

# depth= 로 서브트리 깊이를 키우면 중첩 객체까지 펼쳐서 내려옴
# (기본은 얕게 반환 — 큐/실행기처럼 중첩이 깊으면 depth 를 올려야 보임)
curl -s --user "USER:TOKEN" \
...
상태 추적은 보통 TPS 폴링 잡이 주기적으로 호출합니다. 이때 인증 방식 선택이 운영 안정성에 영향을 줍니다. Jenkins 공식 문서는 CSRF 보호 목적에서 crumb보다 API token을 권장합니다 ("API tokens are preferred instead of crumbs for CSRF protection.", 출처: jenkins.io/doc/book/using/remote-access-api). API token으로 인증하는 요청은 CSRF(crumb) 면제 대상이므로, 폴링 잡이 매번 `/crumbIssuer/api`로 crumb을 먼저 받아오는 단계를 생략할 수 있습니다 (출처: jenkins.io/doc/book/security/csrf-protection).

```bash
# HTTP BASIC 으로 API token 인증 — 이 경우 crumb 발급/동반이 불필요
# (token 인증은 CSRF 면제라, 폴링 잡이 crumb 왕복을 한 번 줄임)
curl -s --user "USER:11aabbccddeeff..." \
  "http://JENKINS/job/pipeline/42/wfapi/describe"
```

API token은 비밀번호 대신 권장됩니다. 노출되더라도 해당 토큰만 폐기하면 되고 비밀번호 자체는 바꿀 필요가 없기 때문입니다 (출처: jenkins.io/doc/book/security/managing-security). 폴링 잡처럼 자격 증명이 장기 상주하는 환경에서는 이 폐기 가능성이 운영상 큰 차이를 만듭니다.
...
- [07-02. wfapi 상세 스펙과 활용](07-02.wfapi%20상세%20스펙과%20활용.md) § "stages 트리" — `pauseDurationMillis`·`stages[]` 구조의 상세 스펙
- [07-03. wfapi 로그 모델과 Blue Ocean 구현 판단](07-03.wfapi%20로그%20모델과%20Blue%20Ocean%20구현%20판단.md) § "Blue Ocean 구현 판단" — Blue Ocean 의존성 판단의 로그 관점 연장
