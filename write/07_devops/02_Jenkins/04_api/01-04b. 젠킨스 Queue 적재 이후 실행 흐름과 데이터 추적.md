...
TPS나 외부 실행 관리 서비스가 빌드 추적을 위해 최소한으로 저장해야 하는 값은 네 개다.
...
에이전트 유형에 따라 executor 해석 방식이 달라집니다. VM 정적 에이전트는 숫자가 비교적 고정이지만, K8s 동적 에이전트는 Pod 생명주기 때문에 응답이 시점마다 달라집니다. 큰 그림은 다음과 같다.

- **VM 정적**: `/computer/api/json`의 노드와 executor 수가 평소에도 계속 보인다.
- **K8s 동적**: 평소에는 안 보이다가, 빌드 요청 시 Pod가 뜨면서 노드와 executor가 잠시 나타난다. `totalExecutors` 하나만 보면 오판하기 쉽습니다.
...
판단에 쓰는 API와 K8s에서의 해석 주의점은 다음과 같다.
