...
---
> 이 문서는 Jenkins Pipeline의 `input` 승인 API와 Jenkins 운영 상태를 확인하는 조회 API 자체를 설명하는 스펙 문서다.
>
> - 승인 대기 조회, 승인, 거절, 시스템 정보 조회, 노드 조회, Metrics Plugin 기반 헬스체크 API를 다룬다.
> - `PAUSED_PENDING_INPUT` 해석, 승인자 제어, readiness 정책, TPS 운영 패턴은 `01-08a`에서 별도로 다룬다.

## 1. 이 문서의 범위

> 이 문서는 배포 승인과 운영 상태 확인에 직접 사용하는 아래 API만 설명한다.
...
승인 대기 상태를 만드는 빌드 실행과 상태 추적은 이 문서 범위가 아니다. 그 내용은 별도 문서에서 다룬다:
...
승인 API는 `pipelineStruct + buildNumber + inputId` 조합을 쓴다.
...
운영 조회 API는 Jenkins 루트 또는 `computer` 아래 경로를 쓴다.
...
모든 예시는 [01-00. 젠킨스 API 사전 준비.md](/Users/simbohyeon/Library/CloudStorage/GoogleDrive-tscofet@gmail.com/내%20드라이브/study/runners-high/write/07_devops/02_Jenkins/04_api/01-00.%20젠킨스%20API%20사전%20준비.md) 와 `01-02`까지의 준비가 끝났다는 전제다.

이 문서에서는 아래 공통값을 다시 설명하지 않는다:
...
이 문서에서는 현재 승인 대기 build 번호처럼 실행마다 달라지는 값만 추가로 준비한다:
...
현재 환경처럼 비밀번호 인증을 쓴다면 POST 전에 crumb과 cookie도 같이 준비해야 한다. 이 문서의 POST 예시는 `01-02`에서 발급한 crumb과 session cookie를 그대로 사용한다.

응답 확인 원칙은 다음과 같다:

- 승인 API 같은 POST는 본문보다 `HTTP_STATUS`와 `headers.txt`를 먼저 본다.
- JSON 조회 API는 `jq`로 필요한 필드만 정리한다.
- Metrics Plugin 경로는 플러그인 미설치 시 `404`가 날 수 있다.

### 1-3. 권장 실행 순서

이 문서는 API를 개별 스펙으로 설명하지만, 실제 실습은 아래 순서로 보는 편이 자연스럽다.
...
> 배포 승인 API는 build가 실제로 `input` 단계에서 멈춰 있어야 의미가 있다.
>
> - `inputId`는 Jenkinsfile의 `input id: '...'` 값을 그대로 사용한다.
> - POST 계열은 crumb과 session cookie를 함께 전송해야 한다.

### 2-1. 사전 준비: 승인 대기 build와 `inputId`를 확보한다

승인 대기 build 번호와 현재 상태를 확인하는 예시는 다음과 같다:
...
`status` 필드에 올 수 있는 주요 값은 다음과 같다:
...
`status`가 `PAUSED_PENDING_INPUT`이 아니면 승인 API를 호출해도 `404` 또는 의미 없는 응답이 돌아온다.
...
이 경로는 Jenkins UI 성격이 강해서 JSON보다 HTML 또는 폼 응답으로 보이는 경우가 많다. 응답별 동작은 다음과 같다:
...
`200`이 돌아왔더라도 본문은 JSON이 아닌 HTML일 수 있으므로, 승인 대기 여부를 프로그래밍 방식으로 판단하려면 `wfapi/describe`의 `status` 필드를 쓰는 편이 안정적이다.
...
`302` 응답 시 `-o /dev/null`로 본문을 버리고 있으므로, 성공 여부는 `HTTP_STATUS=302`와 `headers.txt`의 `Location` 헤더로 확인한다. `headers.txt` 예시는 다음과 같다:
...
응답 패턴은 `proceed`와 동일하다. 성공 시 `302`를 반환하며, `Location` 헤더가 빌드 페이지를 가리킨다. `proceed`와 차이점은 전송할 파라미터가 없다는 것뿐이므로, 에러 응답도 파라미터 관련 `400`이 발생하지 않는다.
...
응답 패턴은 `proceedEmpty`와 동일하게 성공 시 `302`다. 차이는 거절 후 파이프라인이 `ABORTED` 상태로 종료된다는 점이다.

에러 케이스는 다음과 같다:

| 상태 코드 | 의미 | 대응 |
|-----------|------|------|
| `302` | 거절 성공 | 빌드가 `ABORTED`로 종료된다. `wfapi/describe`로 확인 |
| `403` | 권한 없음 | crumb 값 재발급 또는 `submitter` 제한 확인 |
| `404` | 대상 없음 | `inputId`, `BUILD_NUMBER_APPROVAL` 확인. 이미 처리됐을 수 있다 |

거절 후 `wfapi/describe`를 다시 호출하면 다음과 같이 확인할 수 있다:
...
> Jenkins 시스템 전체의 기본 정보를 가져오는 가장 간단한 API다.
>
> - 응답에는 `mode`, `nodeName`, `useCrumbs`, `jobs` 배열 등이 포함된다.
> - 인프라 레벨에서 Jenkins 자체가 살아 있는지 확인하는 용도로 자주 쓴다.

### 3-1. `GET /api/json`

> Jenkins 시스템 기본 정보를 조회한다.
...
| `useCrumbs` | Boolean | `true`면 POST 요청에 crumb이 필수다. `false`인 환경은 crumb 없이 POST가 가능하지만 보안상 권장하지 않는다 |
...
> 전체 노드와 executor 현황을 확인하는 API다.
>
> - `busyExecutors`와 `totalExecutors`로 클러스터 전체 부하를 파악한다.
> - 특정 노드가 `offline` 상태인지, 유휴(`idle`) 상태인지도 함께 확인할 수 있다.

### 4-1. `GET /computer/api/json`

> 전체 노드와 executor 상태를 조회한다.
...
> Jenkins 코어가 기본 제공하지 않는 운영 친화적인 헬스체크 경로를 Metrics Plugin이 제공한다.
>
> - 플러그인이 설치되지 않으면 두 경로 모두 `404`를 반환한다.
> - `/ping`은 생존 확인용, `/healthcheck`는 항목별 상세 상태 확인용으로 용도가 다르다.

### 5-1. `GET /metrics/currentUser/ping`

> Metrics Plugin이 설치되어 있으면 간단한 ping 체크를 할 수 있다.
...
정상 응답(`200`)은 단순 텍스트 한 줄이다:

```text
pong
```

`Content-Type`은 `text/plain`이다. JSON이 아니므로 `jq`로 파싱하지 않는다. 이 경로는 로드밸런서나 Kubernetes liveness probe처럼 "응답이 오는가"만 확인하는 용도로 설계됐기 때문에, 본문보다 HTTP 상태 코드가 중요하다.
...
> Metrics Plugin이 설치되어 있으면 상세 헬스체크 JSON을 조회할 수 있다.
...
정상 응답(`200`)은 항목별 헬스체크 결과를 담은 JSON이다:
...
- `01-02a. 젠킨스 인증 모델과 TPS 패턴 (2.222+).md`
- `01-05. 젠킨스 빌드 상태 추적 API 스펙.md`
- `01-08a. 젠킨스 API 배포 승인과 운영 관리 현대화.md`
