...
> - `PAUSED_PENDING_INPUT` 해석, 승인자 제어, readiness 정책, TPS 운영 패턴은 `01-08a`에서 별도로 다룹니다.
...
> `01-02`까지의 인증(Basic + crumb/cookie), `01-04`·`01-05`의 빌드 실행·상태 추적을 알고 있어야 합니다. 승인 API는 빌드가 실제로 `input`에서 멈춰 `PAUSED_PENDING_INPUT` 상태일 때만 의미가 있다는 전제도 함께 기억해 두세요.
...
현재 환경처럼 비밀번호 인증을 쓴다면 POST 전에 crumb과 cookie도 같이 준비해야 합니다. 이 문서의 POST 예시는 `01-02`에서 발급한 crumb과 session cookie를 그대로 사용합니다.

응답 확인 원칙은 다음과 같습니다:

- 승인 API 같은 POST는 본문보입니다 `HTTP_STATUS`와 `headers.txt`를 먼저 봅니다.
...
응답 패턴은 `proceed`와 동일하입니다. 성공 시 `302`를 반환하며, `Location` 헤더가 빌드 페이지를 가리킨입니다. `proceed`와 차이점은 전송할 파라미터가 없다는 것뿐이므로, 에러 응답도 파라미터 관련 `400`이 발생하지 않는입니다.
...
`Content-Type`은 `text/plain`입니다. JSON이 아니므로 `jq`로 파싱하지 않는입니다. 이 경로는 로드밸런서나 Kubernetes liveness probe처럼 "응답이 오는가"만 확인하는 용도로 설계됐기 때문에, 본문보다 HTTP 상태 코드가 중요합니다.
