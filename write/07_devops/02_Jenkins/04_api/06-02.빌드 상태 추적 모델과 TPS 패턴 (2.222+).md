...
즉 Jenkins 상태를 해석할 때는 "어느 API의 어떤 필드인가"를 먼저 구분해야 합니다. 응답이 무거우면 `tree=`로 필요한 필드만 골라 받을 수 있습니다. 예를 들어 코어 build API에서 `building`과 `result`만 필요하면 `?tree=building,result`를 붙여 수십 KB 응답을 수백 바이트로 줄입니다(출처: jenkins.io/doc/book/using/remote-access-api). 폴링처럼 같은 객체를 반복 조회할 때 이 절약이 누적되어 의미가 큽니다.
