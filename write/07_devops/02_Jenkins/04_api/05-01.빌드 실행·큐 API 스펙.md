...
>
> - 빌드 트리거(`build`/`buildWithParameters`) API와 파라미터 전달 방식·한도를 다룹니다.
...
> - 빌드 중지·취소 API는 `05-05`에서, 큐 아이템·전체 큐·실행기 조회 API는 `05-06`에서 별도로 다룹니다.
...
트리거부터 빌드 번호 확보까지의 두 단계 전환과, 파라미터가 어느 한도 아래에서 전달되는지를 한 장으로 보면 다음과 같습니다.

![빌드 트리거 두 단계 전환과 파라미터 전달 한도](_assets/05-01.build-trigger-queue-flow.svg)
...
트리거 이후 단계인 큐 아이템·전체 큐·실행기 조회 API는 `05-06.큐·실행기 조회 API 스펙.md`에서, 빌드 중지·취소 API는 `05-05.빌드 중지·취소 API 스펙.md`에서 다룹니다.
...
5. 실행 중인 빌드를 멈추고 싶을 때만 `POST /{pipelineStruct}/{buildNumber}/stop`을 호출합니다. (스펙은 `05-05` 참조)
...
> 파라미터 없는 `build`와 파라미터 빌드 `buildWithParameters` 두 POST 엔드포인트로 빌드를 트리거하고, 파라미터 전달 세 가지 방식을 구분하는 법을 다룹니다.
...
> 한도는 Jenkins 코어가 아니라 그 아래 서블릿 컨테이너(Winstone-Jetty) 계층이 겁니다. 그래서 Remote Access API 문서만 봐서는 한도가 보이지 않습니다.

Jenkins 코어는 파라미터 key-value의 개수나 길이를 따로 제한하지 않습니다. 실제 한도는 내장 컨테이너인 Winstone-Jetty가 form 파싱 단계에서 적용하며, 기본값 기준으로 세 가지만 기억하면 됩니다. (출처: github.com/jenkinsci/winstone `Option.java`, jetty.org "Configuring Form Size")

| 한도 | 기본값 | 적용 대상 | 조정 옵션 |
|------|--------|----------|----------|
| form 파라미터 개수 | `10000`개 | 방식 1·2 (form body) | `--maxParamCount` (Jetty `maxFormKeys`) |
| form body 전체 크기 | `Integer.MAX_VALUE` (사실상 무제한) | 방식 1·2 (form body) | `--requestFormContentSize` (Jetty `maxFormContentSize`) |
| 요청 라인 + 헤더 크기 | `8192` bytes | 방식 3 (`build?json=` 쿼리) | `--requestHeaderSize` |

개수 한도 10000은 같은 hash 버킷으로 쏠리는 키를 대량 전송해 CPU를 태우는 hash-collision DoS(oCERT #2011-003)를 막기 위한 방어값이라, 정상적인 파라미터 빌드에서 닿을 일은 거의 없습니다. 넘기면 Jetty가 form 파싱 자체를 거부해 HTTP `400`으로 끝납니다.

길이는 반대로 form body 기준 사실상 무제한입니다. 개별 value의 길이 제한이 따로 있는 것이 아니라 body 전체 크기 하나로만 관리하는데, Jetty 자체 기본값은 200,000 bytes지만 Winstone이 이를 무제한으로 풀어 둡니다. 실제로 Jenkins 2.204.3~2.204.4에서 이 Jetty 기본값이 새어 들어와 큰 form 제출 시 `Form is larger than max length 200000` 400 에러가 났고, 2.204.5/2.222.1에서 무제한으로 복원된 사고가 있습니다(JENKINS-60409). 단 방식 3(쿼리 전달)만은 JSON이 URL에 실려 요청 라인으로 들어가므로 8KB 한도를 그대로 받습니다. 방식 3 절에서 "파라미터가 많거나 값이 길면 방식 1이나 2가 안전"하다고 한 근거가 이 값입니다.

운영에서 한도보다 더 자주 만나는 제약은 *정의 여부*입니다:

- Job에 정의되지 않은 이름의 파라미터는 에러 없이 조용히 버려집니다(SECURITY-170). 호출은 `201`로 성공하는데 파이프라인 안에서 값이 비어 있어 원인 추적이 어렵습니다. 꼭 풀어야 하면 `-Dhudson.model.ParametersAction.keepUndefinedParameters=true` 또는 `safeParameters` 허용 목록을 씁니다. (출처: jenkins.io/doc/book/managing/system-properties)
- Jenkins 앞에 리버스 프록시가 있으면 그쪽 body 한도가 먼저 걸립니다. 예를 들어 nginx `client_max_body_size` 기본값은 1MB입니다.
- Winstone 대신 Tomcat 같은 외부 컨테이너로 배포했다면 위 표가 아니라 그 컨테이너의 form 한도가 적용됩니다.
...
- Winstone 커맨드라인 옵션과 기본값 (github.com/jenkinsci/winstone)
- Jenkins 시스템 프로퍼티 — keepUndefinedParameters/safeParameters (jenkins.io/doc/book/managing/system-properties)
