...
`build`/`buildWithParameters`/`stop`은 모두 POST이므로 CSRF 보호 대상입니다. 비밀번호 인증으로 호출할 때는 crumb 토큰과 세션 쿠키를 함께 보내야 합니다. 반면 API token으로 인증하면 crumb이 면제되어 이 준비 단계를 통째로 건너뛸 수 있습니다. 그래서 공식 문서는 "API tokens are preferred instead of crumbs for CSRF protection" 라고 안내합니다. 자동화에서는 `--user USER:TOKEN` 한 줄로 끝나는 API token 방식이 crumb 발급·갱신 부담을 없애 줍니다. (출처: jenkins.io/doc/book/security/csrf-protection — API token 인증 요청은 crumb 면제 / jenkins.io/doc/book/using/remote-access-api — API tokens 권장)
...
`tree=`는 응답 필드를 골라 받는 파라미터입니다. 폴더 하나에 Job이 수십 개면 `/api/json` 기본 응답이 수십 KB까지 커지는데, `tree=jobs[name,url,color]`로 세 필드만 선택하면 같은 응답이 수백 바이트 수준으로 줄어 파싱과 전송이 빨라집니다. 반대로 `depth=` 는 서브트리를 *얼마나 깊게* 펼칠지 정하는 파라미터로, 값이 클수록 더 깊은 중첩 객체까지 한 응답에 담깁니다. 필드를 *좁히는* `tree=` 와 깊이를 *넓히는* `depth=` 를 같이 쓰면 필요한 깊이의 필요한 필드만 정확히 받을 수 있습니다. (출처: jenkins.io/doc/book/using/remote-access-api — `tree=` 반환 필드 선택, `depth=` 서브트리 깊이 제어)
