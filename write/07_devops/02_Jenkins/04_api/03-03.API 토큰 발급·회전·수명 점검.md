...
> `generateNewToken`으로 새 API Token을 발급하고, 발급 직후 응답에서만 노출되는 plain token 값을 파일로 박제해 실제 호출로 확인하는 흐름을 다룹니다.
...
- 위 요청이 crumb·cookie 없이 `Authorization + Content-Type`만으로 도는 이유는 API 토큰 인증이 CSRF crumb 면제 대상이기 때문입니다. crumb 발급(`/crumbIssuer/api`)과 세션 쿠키 동반은 비밀번호 인증 POST에만 필요하고 토큰 POST에서는 통째로 사라집니다 — 면제 원리와 발급 절차는 [03-01. 인증 API 스펙](03-01.%EC%9D%B8%EC%A6%9D%20API%20%EC%8A%A4%ED%8E%99%20%28ID-Password%20%2B%20Crumb%29.md) § "GET /crumbIssuer/api/json"에서 다룹니다.
...
> `rename` endpoint로 `tokenUuid`와 `newName`을 보내 기존 토큰의 표시 이름을 바꾸는 방법을 다룹니다.
...
> `revoke`로 특정 토큰 하나를, `revokeAll`로 현재 사용자의 토큰 전부를 폐기하는 두 endpoint를 다룹니다.
...
> 2.462.3에는 자동 만료 시각 인터페이스가 없어, 생성·마지막 사용 시점과 6/12개월 회전 권장 경계를 기준으로 수명을 점검하는 방법을 다룹니다.
