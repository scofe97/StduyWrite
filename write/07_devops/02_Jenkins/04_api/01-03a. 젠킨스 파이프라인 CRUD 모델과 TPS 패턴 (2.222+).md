...
API Token 환경(2.222+)에서는 POST마다 crumb·cookie를 따로 관리할 필요가 줄어들어 생성·수정·삭제 POST와 Jenkinsfile 검증 POST 코드가 동시에 단순해집니다. CRUD endpoint 자체는 그대로이고, 호출 전 인증 준비가 얇아지는 것이 핵심입니다. 자세한 모델 비교는 `01-02a.md` 참조.
...
> Spec(`01-03` §3-4)에서 `checkScriptCompile`과 `pipeline-model-converter/validate`의 요청·응답 형식을 다뤘습니다. 이 절은 **두 API가 실제로 무엇을 잡고 무엇을 놓치는지**, 그리고 어떻게 조합해야 하는지를 다룹니다.
