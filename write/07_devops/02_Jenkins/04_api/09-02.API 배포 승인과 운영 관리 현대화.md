...
승인/거절 POST 요청 자체는 API Token으로 인증하면 crumb 없이 단순화됩니다. Default Crumb Issuer가 발급하는 crumb는 CSRF 방지 토큰이지만, API token 인증 요청은 CSRF(crumb)에서 면제되기 때문입니다(출처: jenkins.io/doc/book/security/csrf-protection). 그래서 backend가 `proceedEmpty`/`abort`를 호출할 때는 `--user USER:TOKEN` BASIC 인증만으로 충분하고, crumb 발급(`/crumbIssuer/api`) 단계를 건너뛸 수 있습니다(출처: jenkins.io/doc/book/using/remote-access-api).
...
readiness probe를 만들 때는 응답 크기도 함께 줄이는 것이 좋습니다. `/api/json` 전체는 잡·빌드 목록까지 끌고 와 응답이 수십 KB에 달하지만, `tree=` 파라미터로 필요한 필드만 선택하면 수백 바이트 수준으로 줄어듭니다(출처: jenkins.io/doc/book/using/remote-access-api). 예를 들어 `/computer/api/json?tree=busyExecutors,totalExecutors` 처럼 두 필드만 뽑으면, probe가 매 초 폴링해도 부담이 작습니다.
...
`/computer/api/json` 응답을 더 깊이 들여다봐야 할 때는 `depth=` 파라미터로 서브트리 깊이를 키울 수 있습니다. `depth=0`(기본)은 얕은 요약만 주지만, `depth=1`은 각 `computer` 항목 안의 중첩 객체까지 펼쳐 노드별 상세를 한 번에 가져옵니다(출처: jenkins.io/doc/book/using/remote-access-api). 다만 depth를 키울수록 응답이 커지므로, 운영 폴링에서는 `tree=`로 필드를 좁히는 쪽을 우선합니다.
...
API Token이 crumb보다 권장되는 이유는 두 가지입니다. 첫째, API token 인증 요청은 CSRF 면제라 승인/거절 POST 흐름이 단순해집니다(출처: jenkins.io/doc/book/using/remote-access-api). 둘째, 토큰이 노출되면 그 토큰만 폐기하면 되고 비밀번호는 그대로 유지되므로 운영 위험이 낮습니다(출처: jenkins.io/doc/book/security/managing-security).
