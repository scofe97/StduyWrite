...
이때 인증 방식도 같이 짚어 둘 만합니다. HTTP BASIC 인증은 `--user USER:TOKEN` 형태로 보내며, CSRF 보호용 crumb 대신 API 토큰을 쓰는 편이 권장됩니다(출처: jenkins.io/doc/book/using/remote-access-api). API 토큰 인증 요청은 CSRF(crumb) 면제 대상이라(출처: jenkins.io/doc/book/security/csrf-protection), 자동화 스크립트에서 crumb 발급 왕복을 줄일 수 있습니다. 토큰은 노출돼도 그 토큰만 폐기하면 되고 비밀번호는 그대로 유지되므로, 비밀번호 대신 API 토큰을 쓰는 편이 권장됩니다(출처: jenkins.io/doc/book/security/managing-security).
...
- 또한 크레덴셜 생성/수정은 상태를 바꾸는 요청이므로 POST로 보내며, POST 요청은 crumb(CSRF 토큰)을 동반해야 합니다. crumb은 `/crumbIssuer/api/json`으로 발급받고, 기본 Crumb Issuer는 username + 웹 세션 ID + 인스턴스 고유 salt를 인코딩합니다(출처: jenkins.io/doc/book/security/csrf-protection). 다만 API 토큰으로 인증하면 이 crumb 요구가 면제됩니다.
