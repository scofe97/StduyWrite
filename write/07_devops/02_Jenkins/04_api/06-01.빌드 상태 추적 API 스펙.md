...
다만 위 방식은 Jenkins가 전체 메타데이터를 먼저 직렬화해 보낸 뒤 클라이언트의 `jq`가 버리는 구조라, 큰 빌드에서는 응답이 수십 KB에 달합니다. 필드 선택을 *서버 쪽*에서 끝내려면 `tree=` 파라미터로 반환 필드를 직접 지정합니다. 전송량이 수십 KB에서 수백 바이트 수준으로 줄어듭니다.

```bash
# tree=로 필드를 서버에서 선택 → 응답 자체가 number/building/result 세 필드만 담아 작아집니다
# (출처: jenkins.io) "tree= 로 반환 필드를 선택해 응답을 축소"
curl -k -sS -u "${JENKINS_USER}:${JENKINS_PASS}" \
...
# (출처: jenkins.io) "어떤 객체 URL에도 /api/ 를 붙여 접근(/api/json, /api/xml, /api/python)"
...
같은 객체를 XML로 받아 특정 노드만 뽑고 싶다면 `/api/xml`에 `xpath=`를 붙여 노드를 선택하고, 불필요한 노드는 `exclude=`로 제거할 수 있습니다(`exclude=`는 반복 지정이 가능합니다). Python 리터럴이 필요하면 `/api/python`도 같은 객체에서 그대로 동작합니다. (출처: jenkins.io)
...
stage 안의 더 깊은 세부(개별 step 등)까지 한 번에 펼치려면 표준 Remote Access API에서는 `depth=` 파라미터로 서브트리 깊이를 키울 수 있습니다. `depth=0`이면 최상위 필드만, 값을 키울수록 더 깊은 중첩이 응답에 포함됩니다. 다만 깊이를 키운 만큼 응답이 커지므로, 상태 판정에 필요한 깊이만 지정하는 편이 좋습니다. (출처: jenkins.io)
...
폴링 한 차례의 전송량도 간격 못지않게 중요합니다. 매 호출마다 전체 메타데이터(수십 KB)를 받으면 controller와 네트워크 양쪽에 부담이 쌓이므로, `?tree=number,building,result`로 필요한 세 필드만 받아 수백 바이트로 줄이는 편이 좋습니다. 폴링 횟수가 수십~수백 회로 누적될수록 이 차이가 그대로 비례해 커집니다.
