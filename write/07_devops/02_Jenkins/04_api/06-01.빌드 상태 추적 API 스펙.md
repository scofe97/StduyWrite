...
> `lastBuild/api/json`과 `lastBuild/buildNumber`로 최근 빌드의 메타데이터 또는 번호만 확보해 이후 조회의 입력으로 넘기는 흐름을 다룹니다.
...
> `{buildNumber}/api/json`으로 특정 빌드의 `building`·`result`·소요 시간 필드를 조회해 실행 중 여부와 최종 결과를 판정하는 방법을 다룹니다.
...
> `wfapi/describe`로 파이프라인 전체와 stage 단위 상태를 읽되, 빌드 번호 있는 경로를 기준으로 잡고 Job 기준 경로는 신뢰하지 않는 이유를 다룹니다.
...
> `wfapi/runs`로 한 파이프라인의 최근 실행 여러 건을 배열로 받아 상태 흐름을 비교하는 방법을 다룹니다.
...
> 빌드 완료를 기다리는 polling 루프에서 폴링 간격과 타임아웃을 정하고 `tree=`로 호출당 전송량까지 줄이는 설계를 다룹니다.
