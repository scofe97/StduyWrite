...
> 이 문서를 읽고 나면 `input` 단계에서 멈춘 빌드를 `proceed`·`proceedEmpty`·`abort`로 **제어**하고, `wfapi/describe`의 `status`로 승인 대기 여부를 **판별**하며, `/api/json`·`/computer/api/json`과 Metrics Plugin의 `/ping`·`/healthcheck` 응답을 비교해 운영 상태를 **점검**할 수 있습니다.
...
## 진입 — 왜 배포 승인을 API로 다루는가

> 사람이 Jenkins UI 버튼을 눌러 배포를 승인하면, 그 승인은 외부 시스템(예: TPS 결재 화면)에서 추적할 수 없습니다. 승인 게이트를 API로 열고 닫을 수 있어야 비로소 외부 결재·배포 파이프라인과 한 흐름으로 묶입니다.

Jenkins Pipeline의 `input` 스텝은 빌드를 일시정지시키고 사람의 승인을 기다립니다. 이 정지 상태를 외부 시스템이 *관측*하고(승인 대기인가) *제어*하려면(승인·거절) HTTP 엔드포인트가 필요합니다. 동시에, 승인을 보내기 전에 "Jenkins가 살아 있고 빌드를 받을 executor가 있는가"를 확인하는 운영 조회 API도 같은 맥락에서 묶입니다. 이 문서는 승인 제어 API와 운영 점검 API를 한 묶음으로 다룹니다.
...
> 이 개념은 이미 아는 `05-01` 빌드 실행 API의 "정지·재개" 측면입니다. 빌드를 *시작*하는 것이 `05-01`이라면, 빌드를 중간에 *멈춰 세우고 사람의 결정을 받아 다시 흐르게* 하는 것이 이 문서의 승인 API입니다.
...
승인 대기 상태를 만드는 빌드 실행과 상태 추적은 이 문서 범위가 아닙니다. 그 내용은 별도 문서에서 다룹니다:
...
`input` 스텝은 자동개찰구 앞에서 멈춰 선 사람에 비유할 수 있습니다. 게이트(개찰구)가 닫혀 있으면(`PAUSED_PENDING_INPUT`) 카드를 대는 동작(`proceed`/`proceedEmpty`)으로 통과시키거나 되돌려 보내는(`abort`) 두 갈래만 남습니다. 이 비유는 "정지 후 사람의 단일 결정으로 진행/중단이 갈린다"까지 유효하고, 게이트가 이미 열렸거나 사람이 떠난 뒤(이미 승인·거절 처리됨)에는 깨집니다. 그 시점에는 카드를 대도 아무 일이 없고 Jenkins는 `404`를 돌려줍니다. 승인을 적용하는 호출(`proceed`/`proceedEmpty`/`abort`)이 모두 `POST`인 이유도 같습니다. 상태를 바꾸는 요청은 GET이 아니라 POST여야 하고, 비밀번호 인증 POST는 crumb(CSRF 토큰)을 함께 보내야 통과합니다. crumb·CSRF 보호의 원리와 발급 절차는 [03-01. 인증 API 스펙](03-01.%EC%9D%B8%EC%A6%9D%20API%20%EC%8A%A4%ED%8E%99%20%28ID-Password%20%2B%20Crumb%29.md) § "Crumb 발급"에서 다룹니다.
...
# -X POST: 상태를 바꾸는 승인이므로 GET이 아닌 POST. 조회는 GET, 적용은 POST로 구분
# -b cookies.txt + -H CRUMB: 비밀번호 인증 POST라 세션쿠키와 crumb을 함께 보내야 CSRF 검사를 통과한다 (API token 인증이면 crumb 면제)
# --data-urlencode json=...: input 스텝이 요구하는 파라미터를 Jenkins가 기대하는 {"parameter":[...]} 형태로 전달. name은 Jenkinsfile의 parameters 정의와 일치해야 한다
# -o /dev/null: 성공은 302 리다이렉트라 본문이 비어 있으므로 버리고 헤더(headers.txt)만 본다
...
어떤 객체 URL에도 `/api/json`을 붙여 접근할 수 있고, `/api/xml`·`/api/python`도 같은 방식입니다(출처: jenkins.io/doc/book/using/remote-access-api). 헬스체크처럼 자주 폴링하는 경로는 `mode` 하나만 필요한데 `jobs` 배열까지 전부 받으면 응답이 부풀므로, `?tree=mode,useCrumbs,numExecutors`처럼 필요한 필드만 골라 받습니다. 이 응답 축소(`tree=`·`depth=`·`exclude=`)의 동작 원리와 비용 비교는 [09-03. API 쿼리 최적화와 운영](09-03.API%20%EC%BF%BC%EB%A6%AC%20%EC%B5%9C%EC%A0%81%ED%99%94%EC%99%80%20%EC%9A%B4%EC%98%81.md)에서 다룹니다. 응답 헤더의 `X-Jenkins` 값이 곧 Jenkins 버전이라, 버전 분기 로직은 이 헤더로 판단합니다(출처: jenkins.io/doc/book/using/remote-access-api).
...
`useCrumbs`가 `true`인 환경에서 POST(승인 API 포함)를 자동화할 때는, crumb을 매번 발급해 붙이는 대신 API token으로 HTTP BASIC 인증(`--user USER:TOKEN`)을 쓰는 편이 단순합니다. API token 인증 요청이 CSRF(crumb)를 면제받는 이유와 토큰 발급·회전 절차는 [03-01. 인증 API 스펙](03-01.%EC%9D%B8%EC%A6%9D%20API%20%EC%8A%A4%ED%8E%99%20%28ID-Password%20%2B%20Crumb%29.md)과 [03-03. API 토큰 발급·회전·수명 점검](03-03.API%20%ED%86%A0%ED%81%B0%20%EB%B0%9C%EA%B8%89%C2%B7%ED%9A%8C%EC%A0%84%C2%B7%EC%88%98%EB%AA%85%20%EC%A0%90%EA%B2%80.md)에서 다룹니다.
...
> 빈칸에 들어갈 값을 떠올린 뒤 문서 끝 §빈칸 정답에서 대조하세요.

1. 승인을 적용하는 호출(`proceed`/`proceedEmpty`/`abort`)은 상태를 바꾸므로 HTTP 메서드 ( ㄱ )를 쓰고, 비밀번호 인증일 때는 CSRF 방지 토큰인 ( ㄴ )을 함께 보내야 합니다.
2. `/api/json` 응답에서 필요한 필드만 골라 응답 크기를 줄이려면 ( ㄷ )= 파라미터를, 서브트리를 더 깊게 펼치려면 ( ㄹ )= 파라미터를 씁니다.
3. `/computer/{nodeName}/api/json`에서 `offline`이 `true`인데 ( ㅁ )가 `false`이면, 관리자가 수동으로 내린 게 아니라 연결 자체가 끊긴 상태라 인프라 점검이 필요합니다.
4. crumb 대신 ( ㅂ )으로 인증하면 CSRF 검사가 면제되며, 응답 헤더 ( ㅅ ) 값으로 Jenkins 버전을 알 수 있습니다.
...
1. (ㄱ) `POST` / (ㄴ) crumb(CSRF 토큰)
2. (ㄷ) `tree` / (ㄹ) `depth`
3. (ㅁ) `temporarilyOffline`
4. (ㅂ) API token / (ㅅ) `X-Jenkins`
...
> 이 문서가 다룬 승인·운영 조회 API의 인증 준비, 승인 대기 상태를 만드는 빌드 실행, 같은 장의 모델·현대화 해석은 아래 문서로 이어집니다.

- [03-01. 인증 API 스펙 (ID-Password + Crumb)](03-01.인증%20API%20스펙%20%28ID-Password%20%2B%20Crumb%29.md) § "Crumb 발급" — 승인 POST에 필요한 crumb·cookie를 발급하는 절차와 CSRF 보호 원리
- [03-03. API 토큰 발급·회전·수명 점검](03-03.API%20토큰%20발급·회전·수명%20점검.md) — crumb 대신 쓰는 API token의 발급·회전과 CSRF 면제 근거
- [05-01. 빌드 실행·큐 API 스펙](05-01.빌드%20실행·큐%20API%20스펙.md) § "빌드 트리거" — 승인 대기 상태를 만드는 input 단계까지 빌드를 진행시키는 실행 API
- [06-01. 빌드 상태 추적 API 스펙](06-01.빌드%20상태%20추적%20API%20스펙.md) § "wfapi/describe" — 승인 대기 판별에 쓰는 `status` 필드의 원 출처
- [09-02. API 배포 승인과 운영 관리 현대화](09-02.API%20배포%20승인과%20운영%20관리%20현대화.md) § "PAUSED_PENDING_INPUT 해석" — 승인자 제어·readiness 정책과 TPS 운영 패턴
- [09-03. API 쿼리 최적화와 운영](09-03.API%20쿼리%20최적화와%20운영.md) § "tree·depth 최적화" — 운영 조회 폴링 비용을 줄이는 응답 축소 전략
