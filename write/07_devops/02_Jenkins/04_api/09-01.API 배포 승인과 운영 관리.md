...
`input` 스텝은 자동개찰구 앞에서 멈춰 선 사람에 비유할 수 있습니다. 게이트(개찰구)가 닫혀 있으면(`PAUSED_PENDING_INPUT`) 카드를 대는 동작(`proceed`/`proceedEmpty`)으로 통과시키거나 되돌려 보내는(`abort`) 두 갈래만 남습니다. 이 비유는 "정지 후 사람의 단일 결정으로 진행/중단이 갈린다"까지 유효하고, 게이트가 이미 열렸거나 사람이 떠난 뒤(이미 승인·거절 처리됨)에는 깨집니다. 그 시점에는 카드를 대도 아무 일이 없고 Jenkins는 `404`를 돌려줍니다. 승인을 적용하는 호출(`proceed`/`proceedEmpty`/`abort`)이 모두 `POST`인 이유도 같습니다. 상태를 바꾸는 요청은 GET이 아니라 POST여야 하고, Jenkins는 이런 상태 변경 POST에 crumb(CSRF 토큰)을 요구합니다(출처: jenkins.io/doc/book/security/csrf-protection).
...
어떤 객체 URL에도 `/api/json`을 붙여 접근할 수 있고, `/api/xml`(`xpath=`로 노드 선택, `exclude=`로 노드 제거)·`/api/python`도 같은 방식입니다(출처: jenkins.io/doc/book/using/remote-access-api). `mode` 하나만 필요한데 `jobs` 배열까지 전부 받으면 응답이 수십 KB로 부풀 수 있으므로, `?tree=mode,useCrumbs,numExecutors`처럼 `tree=` 파라미터로 반환 필드를 골라 응답을 수백 바이트로 줄입니다. 더 깊은 중첩이 필요할 때는 `depth=` 값을 키울수록 서브트리를 더 깊게 펼쳐 반환하므로(출처: jenkins.io/doc/book/using/remote-access-api), 헬스체크처럼 자주 폴링하는 경로일수록 `tree=`로 최소 필드만 받는 편이 네트워크·파싱 비용 면에서 유리합니다. 응답 헤더의 `X-Jenkins` 값이 곧 Jenkins 버전이라, 버전 분기 로직은 이 헤더로 판단합니다(출처: jenkins.io/doc/book/using/remote-access-api).
...
`useCrumbs`가 `true`인 환경에서 POST(승인 API 포함)를 자동화할 때는, crumb을 매번 발급해 붙이는 대신 API token으로 HTTP BASIC 인증(`--user USER:TOKEN`)을 쓰는 편이 단순합니다. API token 인증 요청은 CSRF(crumb)가 면제되고, Jenkins 공식 문서도 "API tokens are preferred instead of crumbs for CSRF protection"이라고 권장합니다(출처: jenkins.io/doc/book/using/remote-access-api). 토큰은 비밀번호 대신 권장되며, 노출되면 토큰만 폐기하면 되고 비밀번호는 그대로 둘 수 있습니다(출처: jenkins.io/doc/book/security/managing-security).
...
- [03-01. 인증 API 스펙 (ID-Password + Crumb)](03-01.인증%20API%20스펙%20%28ID-Password%20%2B%20Crumb%29.md) § "Crumb 발급" — 승인 POST에 필요한 crumb·cookie를 발급하는 절차
