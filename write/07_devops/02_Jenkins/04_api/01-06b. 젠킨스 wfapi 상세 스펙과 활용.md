...
# 젠킨스 wfapi 상세 스펙과 활용
---
> 이 문서는 Jenkins의 `wfapi`를 별도 API 제품처럼 이해할 수 있도록 엔드포인트 집합, 응답 구조, Blue Ocean 대체 범위를 한 번에 정리한 스펙 문서다.
>
> - `01-05`와 `01-06`에서 부분적으로 사용한 `wfapi`를 한 문서로 묶어 설명한다.
> - job 단위, run 단위, node 단위 조회와 `pendingInputActions`, `artifacts`, `wfapi/log`까지 다룬다.
>
> `wfapi` 로그 범위, `nodeId` 의미, Blue Ocean-like UI/저장 전략 판단은 `01-06c. 젠킨스 wfapi 로그 모델과 Blue Ocean 구현 판단.md`에서 별도로 정리한다.

## 1. 이 문서의 범위

> 이 문서는 Jenkins Pipeline REST API Plugin이 제공하는 `wfapi` 엔드포인트만 별도로 정리한다.
...
- `01-05. 젠킨스 빌드 상태 추적 API 스펙.md`
  - `wfapi/describe`를 상태 추적 시나리오 안에서 설명한다.
- `01-06. 젠킨스 API 로그 조회와 적재.md`
  - `wfapi/log`를 로그 수집 흐름 안에서 설명한다.
- `01-06a. 젠킨스 API 로그 조회 현대화.md`
  - Blue Ocean 축소와 `wfapi` 전환 방향을 설명한다.

즉 이 문서는 "언제 `wfapi`를 쓰는가"보다 "`wfapi` 자체가 어떤 API 묶음인가"를 설명하는 데 집중한다.

### 1-1. 공통 경로 규칙

`wfapi`는 일반 Jenkins 코어 API의 `/api/json`과 별도 경로 계층이다.
...
`{pipelineStruct}`와 `{buildNumber}` 확보 방식은 이 시리즈 앞 문서를 그대로 따른다.

### 1-2. 공통 요청 규칙

모든 예시는 `01-05`, `01-06`까지의 준비가 끝났다는 전제다.

즉 이 문서에서는 다음 값을 다시 자세히 설명하지 않는다:
...
`curl` 예시는 모두 GET 조회이므로 crumb 없이 Basic Auth만 사용한다:
...
처음부터 모든 endpoint를 다 외울 필요는 없다. 보통은 아래 순서로 이해하면 된다:

1. `/{pipelineStruct}/wfapi`로 job 단위 루트와 `_links`를 확인한다.
2. `/{pipelineStruct}/wfapi/runs`로 최근 실행 목록과 stage 요약을 본다.
3. `/{pipelineStruct}/{buildNumber}/wfapi/describe`로 특정 run의 전체 상태를 본다.
4. `stages[].id`를 뽑아 `execution/node/{nodeId}/wfapi/describe`로 stage 내부를 본다.
5. 마지막으로 `execution/node/{nodeId}/wfapi/log`로 node 로그를 읽는다.



## 2. `wfapi`가 무엇인가

> `wfapi`는 Blue Ocean API도 아니고, Jenkins 코어 `/api/json`도 아니다. Pipeline용 보조 REST 계층으로 보는 편이 정확하다.

2026년 4월 24일 기준 Jenkins 공식 플러그인 문서에서 `wfapi`는 `Pipeline: REST API` Plugin이 제공하는 endpoint 집합으로 소개된다. 이 플러그인은 당시 버전 `2.41`, 최소 Jenkins `2.479.1`을 요구한다.
...
- `wfapi`는 Blue Ocean이 없어도 별도로 동작할 수 있다.
- Blue Ocean을 줄이더라도 `wfapi`까지 같이 없애는 것은 아니다.
- stage 상태 추적과 node 로그 조회는 `wfapi`로 상당 부분 대체 가능하다.

### 2-1. 플러그인 존재 여부 확인

`wfapi`는 Jenkins 코어 API가 아니라 플러그인 제공 기능이므로, 실제 컨트롤러에 설치됐는지 먼저 확인하는 편이 안전하다.
...
즉 `blueocean`이 없어도 `pipeline-rest-api`만 있으면 `wfapi`는 계속 쓸 수 있다.

### 2-2. HAL 스타일 `_links`

`wfapi` 응답의 특징은 `_links`다. 공식 문서는 이 API가 HAL 스타일 hyperlink 구조를 따른다고 설명한다.

즉 클라이언트가 모든 하위 URL을 문자열 조합으로 만들기보다, 응답의 `_links`를 따라가며 다음 리소스로 이동할 수 있다.
...
실무에서는 문자열 조합도 자주 하지만, 응답에 있는 링크를 신뢰하면 플러그인 구현 차이에 덜 민감해진다.



## 3. Job 단위 `wfapi`

> job 단위 endpoint는 build 번호를 모를 때 진입점 역할을 한다.

### 3-1. `GET /{pipelineStruct}/wfapi`

이 endpoint는 특정 Pipeline job 자체를 설명한다.
...
실무적으로 중요한 점은 run 배열의 각 원소가 이미 stage 요약을 포함한다는 것이다. 즉 "최근 실행 여러 건의 stage 상태를 훑어보기"에는 `runs`가 꽤 효율적이다.
...
또한 run 원소의 `_links`는 추가 drill-down 힌트를 준다:

- `_links.self.href`
  - 해당 run의 `wfapi/describe`
- `_links.pendingInputActions.href`
  - 승인 대기 run일 때 존재할 수 있다
- `_links.artifacts.href`
  - archived artifact가 있을 때 존재할 수 있다
...
이 endpoint는 특정 실행 1건의 전체 상태를 반환한다.
...
이 endpoint 하나로 보통 다음 질문에 답할 수 있다:
...
`wfapi/describe`의 `stages[]`가 끝이 아니다. 각 stage는 다시 node 상세 조회로 내려갈 수 있다.
...
즉 `wfapi`는 보통 다음 두 단계까지 본다고 이해하면 된다:
...
실제 공식 예시에서도 stage 하나 아래에 `Git`, `Shell Script` 같은 하위 node가 들어간다.

다만 공식 `wfapi` 문서 기준으로는 여기서 한계가 있다:

- `stageFlowNodes[]` 아래를 `stepId` 같은 별도 REST 자원으로 더 쪼개는 endpoint는 문서화되어 있지 않다.
- 즉 Blue Ocean의 `nodes/{id}/steps/{stepId}/log`와 같은 step 전용 drill-down은 `wfapi`의 표준 endpoint 집합에 없다.

그래서 판단은 다음처럼 하는 편이 정확하다:
...
실습과 기존 시리즈 문서 기준으로 자주 보는 상태는 다음과 같다:
...
중요한 점은 코어 API의 `result`와 `wfapi`의 `status`가 완전히 같은 층위가 아니라는 것이다.

- 코어 `result`
  - build 전체 최종 결과에 가깝다.
- `wfapi status`
  - run 진행 상태와 stage 상태를 더 세밀하게 보여준다.

### 4-4. `GET /{pipelineStruct}/{buildNumber}/wfapi/pendingInputActions`

이 endpoint는 승인 대기 중인 `input` 액션을 반환한다.
...
이 endpoint는 아무 때나 무조건 쓰는 것이 아니라, 보통 `wfapi/describe.status == "PAUSED_PENDING_INPUT"`일 때만 이어서 호출한다.

즉 권장 패턴은 다음과 같다:

1. 먼저 `wfapi/describe`를 조회한다.
2. `status`가 `PAUSED_PENDING_INPUT`인지 확인한다.
3. 맞으면 `pendingInputActions`를 호출해 `proceedUrl`과 `abortUrl`을 얻는다.

### 4-5. `GET /{pipelineStruct}/{buildNumber}/wfapi/artifacts`

이 endpoint는 archived artifacts를 반환한다.
...
이 endpoint는 모든 run에서 항상 의미가 있는 것은 아니다.

- artifact를 아카이브하지 않은 run
  - 링크 자체가 없거나 빈 결과일 수 있다.
- artifact를 저장한 run
  - `_links.artifacts`를 따라가면 된다.



## 5. Node 단위 `wfapi`

> run 단위 `stages[]`가 stage 지도라면, node 단위 endpoint는 그 stage 안을 더 파고드는 상세 지도다.

### 5-1. `GET /{pipelineStruct}/{buildNumber}/execution/node/{nodeId}/wfapi/describe`

이 endpoint는 특정 stage/node의 상세 구조를 보여준다.

먼저 run-level `stages[]`에서 `id`를 하나 뽑는다:
...
`stageFlowNodes[]`에서 중요하게 보는 값은 다음과 같다:
...
즉 stage 하나가 실패했을 때는 보통 다음 순서로 본다:

1. run-level `stages[]`에서 실패 stage의 `id`를 찾는다.
2. node-level `wfapi/describe`로 들어간다.
3. `stageFlowNodes[]`에서 실제 실패한 하위 node와 `error.message`를 찾는다.

### 5-2. `GET /{pipelineStruct}/{buildNumber}/execution/node/{nodeId}/wfapi/log`

이 endpoint는 특정 node의 로그를 JSON으로 반환한다.
...
이 endpoint가 무엇을 주고 무엇을 주지 않는지는 범위로 이해하는 편이 쉽다:
...
즉 `wfapi/log`는 build 전체 로그를 대체하지 않는다. 특정 stage 또는 flow node에 매달린 로그를 잘라서 보는 API다.

또한 `wfapi/log`는 Blue Ocean step 로그와도 다르다:
...
따라서 실패 지점을 좁히는 용도에는 `wfapi/log`가 더 적합하다.

### 5-3. `hasMore`와 로그 수집 판단

`wfapi/log` 응답의 `hasMore`는 이 node 로그가 아직 더 이어질 수 있는지를 보여준다.
...
다만 build 전체 실시간 tail은 여전히 `progressiveText`가 더 자연스럽다. `wfapi/log`는 "어느 node가 문제였는지"를 좁히는 데 더 적합하다.

### 5-4. `stageFlowNodes[]`별 로그로 더 쪼개 보기

`wfapi`에서 `stages[]`보다 더 세부적으로 로그를 보고 싶다면, stage 자체의 `nodeId`만 보지 말고 `stageFlowNodes[]`의 각 `id`를 다시 로그 대상으로 삼으면 된다.

흐름은 다음과 같다:

1. `/{build}/wfapi/describe`에서 실패 stage의 `id`를 찾는다.
2. `execution/node/{stageId}/wfapi/describe`로 들어간다.
3. `stageFlowNodes[]`에서 하위 `id`와 `_links.log.href`를 확인한다.
4. 그 하위 `id`로 `execution/node/{flowNodeId}/wfapi/log`를 호출한다.

즉 `wfapi`도 stage보다 더 세부적으로 내려갈 수는 있다. 다만 그 최소 단위는 보통 `flow node` 수준이며, 공식 문서 기준으로 Blue Ocean처럼 step 자원을 별도 계층으로 노출하지는 않는다.



## 6. Blue Ocean과의 대응 관계

> `wfapi`는 Blue Ocean을 많이 대체하지만, step 단위까지 완전히 치환하지는 못한다.

2026년 4월 24일 기준 Jenkins 공식 Blue Ocean 문서는 Blue Ocean이 2026년 7월 deprecated 예정이라고 안내한다. 플러그인 페이지도 기능 추가 없이 선택적 보안/결함 수정만 제공한다고 설명한다.

그래서 현재 기준의 대응 관계를 정리하면 다음과 같다:
...
> 실제 운영에서는 `wfapi` 하나만 쓰기보다 코어 API와 조합하는 편이 가장 안정적이다.

가장 실용적인 조합은 다음과 같다:

1. 빌드 실행 직후에는 `queue/api/json`으로 `buildNumber`를 확보한다.
2. 실행 여부와 최종 결과는 `/{buildNumber}/api/json`으로 본다.
3. stage 상태와 승인 대기는 `/{buildNumber}/wfapi/describe`로 본다.
4. 실패한 stage의 `id`를 얻어 `execution/node/{nodeId}/wfapi/describe`로 들어간다.
5. 해당 node 로그는 `execution/node/{nodeId}/wfapi/log`로 읽는다.
6. step 수준까지 더 쪼개야 하면 그때만 Blue Ocean API를 호출한다.
...
즉 `wfapi`는 Blue Ocean의 완전한 대체물이 아니라, "대부분의 백엔드 상태 추적과 로그 축소 조회를 담당하는 중간 계층"으로 이해하면 가장 실무적이다.



## 8. 관련 문서

- `01-05. 젠킨스 빌드 상태 추적 API 스펙.md` — run 상태 추적 시나리오 안에서 `wfapi/describe`를 사용한다.
- `01-05b. 젠킨스 상태 추적 API 현대화와 Blue Ocean 해석.md` — Blue Ocean 축소와 `wfapi` 중심 전환 배경을 설명한다.
- `01-06. 젠킨스 API 로그 조회와 적재.md` — `wfapi/log`와 전체 로그 적재를 함께 본다.
- `01-06a. 젠킨스 API 로그 조회 현대화.md` — Blue Ocean 로그 API를 줄이고 `wfapi`로 넘어가는 현재 판단을 정리한다.
- `01-06c. 젠킨스 wfapi 로그 모델과 Blue Ocean 구현 판단.md` — `wfapi` 로그 범위, `nodeId`, UI 구현과 저장 전략을 FAQ처럼 정리한다.
