...
> `wfapi`는 Blue Ocean API도 아니고, Jenkins 코어 `/api/json`도 아닙니다. Pipeline용 보조 REST 계층으로 보는 편이 정확하다.
...
즉 `wfapi`도 stage보다 더 세부적으로 내려갈 수는 있습니다. 다만 그 최소 단위는 보통 `flow node` 수준이며, 공식 문서 기준으로 Blue Ocean처럼 step 자원을 별도 계층으로 노출하지는 않는다.
