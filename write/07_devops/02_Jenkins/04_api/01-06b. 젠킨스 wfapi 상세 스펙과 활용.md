...
> - job 단위, run 단위, node 단위 조회와 `pendingInputActions`, `artifacts`, `wfapi/log`까지 다룹니입니다.
>
...
`stages[].id`는 FlowNode의 ID이며, stage 로그 조회 경로의 `nodeId`로 그대로 씁니입니다. 즉 describe로 구조를 받고, 그 id로 해당 stage 로그에 진입합니다.
