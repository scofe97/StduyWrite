...
>
> - `06-01`와 `07-01`에서 부분적으로 사용한 `wfapi`를 한 문서로 묶어 설명합니다.
> - job 단위, run 단위, node 단위 조회와 `pendingInputActions`, `artifacts`, `wfapi/log`까지 다룹니다.
>
> `wfapi` 로그 범위, `nodeId` 의미, Blue Ocean-like UI/저장 전략 판단은 `07-04.wfapi 로그 모델과 Blue Ocean 구현 판단.md`에서 별도로 정리합니다.

> 이 문서를 읽고 나면 `wfapi/describe`로 stage 구조·상태를 조회하고, `wfapi/runs`로 재실행 기록을 비교하며, stage 로그 경로로 실패 지점을 디버깅하고, 진행률을 예측·계산하며, 코어 API와 wfapi 중 무엇을 호출할지 선택하고, wfapi의 한계까지 설명할 수 있습니다.



## 사전 지식

> 06-01에서 본 기본 빌드 API와 wfapi의 관계를 짚어 둔다면, 이 문서는 wfapi 엔드포인트 집합의 상세 스펙과 실전 활용으로 파고든 편입니다.



## 진입 — 왜 코어 `/api/json`만으로는 stage를 못 보는가

> 코어 빌드 API는 빌드를 "성공/실패/진행 중" 한 덩어리로만 보여줍니다. 그런데 운영자가 정말 궁금한 것은 "어느 stage에서 멈췄는가", "승인 대기는 몇 번째 단계인가", "병목이 어느 stage인가"입니다. 이 stage 단위 해상도를 채워 주는 보조 REST 계층이 `wfapi`입니다. 코어 API가 건물 전체의 점멸 표시등이라면, wfapi는 층별 평면도에 해당합니다. 이 문서는 그 평면도를 어떻게 읽고 어디까지 내려갈 수 있는지를 엔드포인트 단위로 정리합니다.
...
> 이 개념은 이미 아는 REST 리소스 트리(URL에 `/api/json`을 붙여 어떤 객체든 조회하던 패턴)의 "Pipeline 전용 확장" 측면입니다. 다만 경로 계층이 `/api/`가 아니라 `/wfapi/`로 갈라진다는 점만 새로 익히면 됩니다.
...
모든 예시는 `06-01`, `07-01`까지의 준비가 끝났다는 전제입니다.

즉 이 문서에서는 다음 값을 다시 자세히 설명하지 않습니다:
...
`curl` 예시는 모두 GET 조회이므로 crumb 없이 Basic Auth만 사용합니다. crumb은 상태를 바꾸는 POST(예: `build`, `proceedUrl`)의 CSRF 방지 토큰이라, 읽기 전용 GET에는 동반할 필요가 없습니다(출처: jenkins.io/doc/book/security/csrf-protection).

```bash
curl -k -sS -u "${JENKINS_USER}:${JENKINS_PASS}" \
  "<wfapi URL>"
# HTTP BASIC 인증: --user USER:TOKEN 형태. 비밀번호 대신 API 토큰을 쓰면
#   노출 시 그 토큰만 폐기하면 되어 운영 위험이 작다 (비밀번호는 바꾸기 어렵다)
```

운영 자동화에서는 `JENKINS_PASS` 자리에 비밀번호 대신 **API 토큰**을 넣는 편이 권장됩니다. 공식 문서는 "API tokens are preferred instead of crumbs for CSRF protection"이라고 명시하며, API 토큰 인증 요청은 crumb(CSRF) 검증 자체가 면제됩니다(출처: jenkins.io/doc/book/using/remote-access-api · jenkins.io/doc/book/security/csrf-protection). 즉 토큰을 쓰면 GET·POST 모두에서 crumb 발급 단계를 건너뛸 수 있어 스크립트가 단순해집니다.
...
> `wfapi`는 Blue Ocean API도 아니고, Jenkins 코어 `/api/json`도 아닙니다. Pipeline용 보조 REST 계층으로 보는 편이 정확합니다.
...
# depth=1: 플러그인 배열 한 단계 안쪽까지 펼쳐 각 플러그인 속성을 받기 위함
#          (depth 미지정 시 plugins 배열 자체가 비어 나올 수 있음)
# tree=plugins[...]: 필요한 5개 필드만 골라 응답을 축소 — 수백 개 플러그인 전체 메타데이터를
#                    다 받으면 수백 KB라, 필드 선택으로 수 KB 수준까지 줄임
```

`depth=`는 반환할 서브트리의 깊이를 정하고, `tree=`는 반환할 필드를 골라 응답을 줄입니다. 둘은 모든 Jenkins 객체 URL의 `/api/`에 공통으로 통합니다(출처: jenkins.io/doc/book/using/remote-access-api). 예를 들어 위 호출처럼 `tree=plugins[shortName,...]`로 5개 필드만 남기면 전체 플러그인 메타데이터(수백 KB)가 수 KB로 줄어듭니다. 응답이 무거운 자동화 폴링에서는 이 두 파라미터로 트래픽을 한 자릿수 비율까지 낮추는 것이 정석입니다.
...
`wfapi` 응답의 특징은 `_links`입니다. 공식 문서는 이 API가 HAL 스타일 hyperlink 구조를 따른다고 설명합니다.
...
실무에서는 문자열 조합도 자주 하지만, 응답에 있는 링크를 신뢰하면 플러그인 구현 차이에 덜 민감해집니다.
...
# .[0:5]: runs 배열은 누적 실행 전체라 길어질 수 있어 최근 5건만 잘라 본다
# stages를 {id,name,status}로만 추림: run 1건당 stage가 여러 개라 전체를 펼치면
#   응답이 화면을 넘겨 — 비교에 필요한 식별·상태 3개 필드만 남겨 한눈에 훑기 위함
```
...
> 실무에서 가장 많이 쓰는 핵심 endpoint는 결국 `/{buildNumber}/wfapi/describe`입니다.
...
stage 트리는 행정 구역에 빗대면 이해하기 쉽습니다. run이 "시"라면 `stages[]`는 "구"이고, 그 아래 `stageFlowNodes[]`는 "동" 단위입니다. 상위에서 하위로 좁혀 가며 문제가 난 구역을 찾아 들어갑니다. 이 비유는 "계층을 따라 내려가며 범위를 좁힌다"는 점까지 유효하고, "동 아래 번지(step) 단위로는 wfapi가 더 쪼개 주지 않는다"는 점에서 깨집니다. 번지 단위가 필요하면 Blue Ocean의 step 자원으로 넘어가야 합니다.
...
- `stageFlowNodes[]` 아래를 `stepId` 같은 별도 REST 자원으로 더 쪼개는 endpoint는 문서화되어 있지 않습니다.
- 즉 Blue Ocean의 `nodes/{id}/steps/{stepId}/log`와 같은 step 전용 drill-down은 `wfapi`의 표준 endpoint 집합에 없습니다.

그래서 판단은 다음처럼 하는 편이 정확합니다:
...
3. 맞으면 `pendingInputActions`를 호출해 `proceedUrl`과 `abortUrl`을 얻습니다.
...
먼저 run-level `stages[]`에서 `id`를 하나 뽑습니다:

```bash
export NODE_ID_FAILED=$(curl -k -sS -u "${JENKINS_USER}:${JENKINS_PASS}" \
  "${JENKINS_URL}${PIPELINE_FAIL_STRUCT}/${BUILD_NUMBER_FAIL}/wfapi/describe" \
  | jq -r '.stages[] | select(.status == "FAILED") | .id' | head -n 1)
# select(.status == "FAILED"): 여러 stage 중 실패한 것만 골라 node drill-down 진입점을 잡는다
# head -n 1: 실패 stage가 둘 이상일 수 있어 — 가장 먼저 깨진 지점부터 보려고 첫 id만 취함
...
1. run-level `stages[]`에서 실패 stage의 `id`를 찾습니다.
2. node-level `wfapi/describe`로 들어갑니다.
3. `stageFlowNodes[]`에서 실제 실패한 하위 node와 `error.message`를 찾습니다.
...
즉 `wfapi/log`는 build 전체 로그를 대체하지 않습니다. 특정 stage 또는 flow node에 매달린 로그를 잘라서 보는 API입니다.
...
따라서 실패 지점을 좁히는 용도에는 `wfapi/log`가 더 적합합니다.
...
다만 build 전체 실시간 tail은 여전히 `progressiveText`가 더 자연스럽습니다. `wfapi/log`는 "어느 node가 문제였는지"를 좁히는 데 더 적합합니다.
...
1. `/{build}/wfapi/describe`에서 실패 stage의 `id`를 찾습니다.
...
즉 `wfapi`도 stage보다 더 세부적으로 내려갈 수는 있습니다. 다만 그 최소 단위는 보통 `flow node` 수준이며, 공식 문서 기준으로 Blue Ocean처럼 step 자원을 별도 계층으로 노출하지는 않습니다.
...
Jenkins 공식 문서는 Blue Ocean이 2026년 7월 폐기(deprecated) 예정이며, 그 이후로는 보안 수정도 기능 업데이트도 받지 않는다고 안내합니다 (출처: jenkins.io/doc/book/blueocean). 폐기 전까지도 플러그인 페이지 기준 기능 추가 없이 중대한 보안·결함만 선택적으로 수정합니다.

### 6-1. 왜 폐기되고, 왜 `wfapi`로 가는가

폐기 이유를 한 줄로 옮기면 "더 현대적인 대안이 등장해서"입니다. 공식 문서는 "시간이 지나면서 Blue Ocean을 대체할 더 현대적인 도구들이 생겨났다(more modern tools have cropped up to replace it)"고 직접 밝힙니다 (출처: jenkins.io/doc/book/blueocean). 

Blue Ocean이 처음 나오던 시절 Jenkins UI에 요구되던 디자인 기준을 채우려 만들어졌지만, 그 코드베이스가 동결(frozen)된 사이 비슷한 기능을 더 활발히 유지보수하는 플러그인들이 자리를 잡았다는 뜻입니다. 그래서 폐기의 핵심 신호는 "기능이 부족해서"가 아니라 "더 이상 *개발되지 않아서*"입니다.

공식이 짚는 UI 대안은 Pipeline Graph View와 Pipeline: Stage View 두 플러그인이며, 선호 이유로 "actively maintained(활발히 유지보수됨)"를 듭니다. 즉 선택 기준이 기능 우위가 아니라 *유지보수 지속성*이라는 점이 폐기 맥락과 같은 결입니다.

여기서 한 가지를 정확히 구분해야 합니다. 공식 문서는 UI 대안을 명시할 뿐, "백엔드 상태 추적을 `wfapi`로 갈아타라"고 *명령*하지는 않습니다.

- 그렇지만 공식 UI 대안 중 하나인 Pipeline: Stage View가 stage 데이터를 바로 `wfapi`(=`Pipeline: REST API` 플러그인)에서 받습니다. Stage View 문서는 stage view를 구동하는 REST 속성(`RunExt`·`StageNodeExt`·`FlowNodeExt`)을 설명하며 `pipeline-rest-api`를 직접 가리킵니다 (출처: plugins.jenkins.io/pipeline-stage-view · github.com/jenkinsci/pipeline-stage-view-plugin). 즉 "stage 조회는 `wfapi`"는 두 사실을 잇는 막연한 추론이 아니라, 공식 UI 대안이 서 있는 토대를 그대로 따라간 *구조적 사실*입니다.
- 그래도 "공식이 `wfapi`를 쓰라고 명령했다"와 "공식 대안이 `wfapi` 위에 서 있다"는 다릅니다. 후자까지만 말하고 전자로 넘어가지 않습니다. 그래서 이 문서의 권고는 공식 명령이 아니라 "동결된 의존을 새로 묶지 말고, 유지되는 계층으로 신규 구현을 가져가자"는 운영 판단으로 읽되, 그 판단이 공식 대안의 토대와 일치한다는 점에서 근거가 단단합니다.

세 계층(코어 빌드 API · `wfapi` · Blue Ocean REST)의 유지보수 상태와 대체 관계를 한눈에 보면 다음과 같습니다.

![코어 API·wfapi·Blue Ocean 세 계층의 유지보수 상태와 대체 관계](_assets/07-03.api-layers-maintenance.svg)
...
### 빈칸 채우기 — wfapi 경로와 파라미터

> 빈칸을 먼저 채워 본 뒤 문서 끝 §빈칸 정답에서 대조하세요.

1. wfapi 응답은 클라이언트가 URL을 문자열 조합하지 않고 다음 리소스로 이동하도록 `______` 스타일 hyperlink 구조를 따릅니다.
2. run이 승인 대기일 때 `wfapi/describe.status` 값은 `______`이고, 이때만 이어서 `wfapi/______`를 호출해 `proceedUrl`을 얻습니다.
3. 응답을 줄이려면 반환 필드를 고르는 `______=` 파라미터를, 서브트리 깊이를 정하려면 `______=` 파라미터를 `/api/`에 붙입니다.
4. stage 내부의 실제 실행 단위는 `stageFlowNodes[]`이며, 그 최소 단위는 `______` 수준입니다. step 자원까지는 `______` API가 더 정밀합니다.
...
### 빈칸 정답 — wfapi 경로와 파라미터

1. `_links`(HAL 스타일).
2. `PAUSED_PENDING_INPUT` / `pendingInputActions`.
3. `tree=` / `depth=`.
4. `flow node` / Blue Ocean.



## 관련 문서

> wfapi를 상태 추적·로그 시나리오 안에서 다시 보거나, 인증·승인 같은 인접 API로 넓히려면 아래 문서로 이어 가세요.

- [06-01. 빌드 상태 추적 API 스펙](06-01.%EB%B9%8C%EB%93%9C%20%EC%83%81%ED%83%9C%20%EC%B6%94%EC%A0%81%20API%20%EC%8A%A4%ED%8E%99.md) § "wfapi/describe" — run 상태 추적 시나리오 안에서 같은 엔드포인트를 쓰는 맥락을 봅니다.
- [06-03. 상태 추적 API 현대화와 Blue Ocean 해석](06-03.%EC%83%81%ED%83%9C%20%EC%B6%94%EC%A0%81%20API%20%ED%98%84%EB%8C%80%ED%99%94%EC%99%80%20Blue%20Ocean%20%ED%95%B4%EC%84%9D.md) § "Blue Ocean 축소" — wfapi 중심 전환 배경을 보강합니다.
- [07-01. API 로그 조회와 적재](07-01.API%20%EB%A1%9C%EA%B7%B8%20%EC%A1%B0%ED%9A%8C%EC%99%80%20%EC%A0%81%EC%9E%AC.md) § "wfapi/log" — node 로그와 전체 로그 적재를 함께 봅니다.
- [07-04. wfapi 로그 모델과 Blue Ocean 구현 판단](07-04.wfapi%20%EB%A1%9C%EA%B7%B8%20%EB%AA%A8%EB%8D%B8%EA%B3%BC%20Blue%20Ocean%20%EA%B5%AC%ED%98%84%20%ED%8C%90%EB%8B%A8.md) § "nodeId 의미" — 로그 범위·UI 구현·저장 전략을 FAQ처럼 정리합니다.
- [09-01. API 배포 승인과 운영 관리](09-01.API%20%EB%B0%B0%ED%8F%AC%20%EC%8A%B9%EC%9D%B8%EA%B3%BC%20%EC%9A%B4%EC%98%81%20%EA%B4%80%EB%A6%AC.md) § "pendingInputActions" — `proceedUrl`로 승인을 진행하는 운영 흐름으로 이어집니다.
