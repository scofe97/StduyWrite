...
> 이 점검 문서는 2단계(Pipeline 정의·패턴·실패 대응)를 다 읽은 뒤 스스로를 시험하기 위한 자가 점검입니다. 먼저 §면접 질문만 보고 답을 떠올린 뒤, §정답 절에서 같은 번호로 대조하세요.
...
Q3 의 핵심은 *후처리가 어떤 순서로 도는가* 입니다. 결과별 조건(`success`/`failure`/`changed`)이 먼저 평가되고 `always` 가 마지막에 돕니다. 이 순서를 알면 "왜 알림이 두 번 가는가" 같은 의문이 풀립니다.

```mermaid
flowchart TB
    S["stages 실행 종료"]
    S --> R{"빌드 결과 판정"}
    R -->|성공| SU["success<br>알림 · CD 트리거"]
    R -->|실패| FA["failure<br>@here 멘션"]
    R -->|상태 전환| CH["changed<br>전환 알림"]
    SU --> AL["always<br>워크스페이스 정리<br>테스트 결과 수집"]
    FA --> AL
    CH --> AL
    AL --> END["빌드 종료"]

    style S fill:#eceff1,stroke:#455a64,color:#000
    style R fill:#e3f2fd,stroke:#1565c0,color:#000
    style SU fill:#e8f5e9,stroke:#2e7d32,color:#000
    style FA fill:#ffebee,stroke:#c62828,color:#000
    style CH fill:#fff3e0,stroke:#ef6c00,color:#000
    style AL fill:#f3e5f5,stroke:#7b1fa2,color:#000
    style END fill:#eceff1,stroke:#455a64,color:#000
```

> 결과별 조건이 먼저, `always` 가 마지막입니다. `post` 는 pipeline 레벨과 stage 레벨 둘 다에서 정의 가능하지만, 같은 일을 두 레벨에 동시에 두면 알림이 두 번 가거나 정리가 두 번 도므로 *한 책임은 한 레벨에만* 둡니다.
...
1. Pipeline as Code가 Freestyle Job보다 나은 이유는 무엇입니까? *(심화: Jenkinsfile을 팀에서 공유할 때 Shared Library와의 관계는 무엇입니까? 공통 로직을 어떻게 중앙화합니까?)*
2. Declarative와 Scripted Pipeline의 차이는 무엇입니까? *(심화: Declarative 안에서 `script {}` 블록을 과도하게 쓰면 사실상 Scripted와 같아집니다. 이 경계를 어떻게 관리하겠습니까?)*
3. post actions 종류와 실무 활용 패턴은 무엇입니까? *(심화: stage 레벨 `post`와 pipeline 레벨 `post`를 어떤 기준으로 구분해서 씁니까?)*
4. when {} 조건을 Git Flow에 매핑하는 방법은 무엇입니까? *(심화: `when { changeset }` 사용 시 첫 빌드에서는 이전 커밋이 없어 변경 파일을 감지할 수 없습니다. 어떻게 해결합니까?)*
5. 병렬 실행의 이점과 주의사항은 무엇입니까? *(심화: 병렬 테스트 결과를 하나의 리포트로 합치려면 어떤 접근이 필요합니까?)*
6. 수동 승인 게이트 구현 시 에이전트 점유 문제는 무엇입니까? *(심화: 승인 게이트 없이 완전 자동화된 프로덕션 배포(Continuous Deployment)로 전환하려면 어떤 전제 조건이 필요합니까?)*
7. Multibranch Pipeline이 일반 Pipeline Job과 다른 점은 무엇입니까? *(심화: 모노레포에서 Multibranch Pipeline을 사용할 때, 변경된 모듈만 빌드하려면 어떻게 설계해야 합니까?)*
...
비교 표는 [02-01. 코드로 파이프라인 정의하기](02-01.코드로%20파이프라인%20정의하기.md) § "Declarative vs Scripted Pipeline" 을 참조합니다.

### 정답 2 심화 — script 블록 경계 관리
...
조건별 표는 [02-02. Declarative Pipeline 핵심 구조](02-02.Declarative%20Pipeline%20핵심%20구조.md) § "Post Actions" 를 참조합니다.

### 정답 3 심화 — stage·pipeline 레벨 post 구분
...
브랜치별 동작 표는 [02-03. Pipeline 패턴](02-03.Pipeline%20패턴.md) § "브랜치별 조건부 실행 (when)" 을 참조합니다.

### 정답 4 심화 — 첫 빌드 changeset 처리
...
상세는 [02-04. 실패 대응과 파이프라인 원칙](02-04.실패%20대응과%20파이프라인%20원칙.md) § "retry, timeout, input" 을 참조합니다.
...
### 정답 7 — Multibranch vs 일반 Pipeline
...
기능 비교 표는 [02-03. Pipeline 패턴](02-03.Pipeline%20패턴.md) § "Multibranch Pipeline" 을 참조합니다.

### 정답 7 심화 — 모노레포 변경 모듈만 빌드
...
> 이 점검 문서는 02장 파이프라인의 다섯 본편(02-01~02-05)을 다룹니다. 막힌 질문이 있으면 아래 해당 편으로 돌아가 해당 절을 다시 읽으면 동선이 이어집니다.

- [02-01. 코드로 파이프라인 정의하기](02-01.코드로%20파이프라인%20정의하기.md) § "Scripted vs Declarative" — Pipeline as Code 이점·script 위험 신호(정답 1·2 연계)
- [02-02. Declarative Pipeline 핵심 구조](02-02.Declarative%20Pipeline%20핵심%20구조.md) § "post 블록" — 4계층 블록·post 처리(정답 3 연계)
- [02-03. Pipeline 패턴](02-03.Pipeline%20패턴.md) § "병렬·Multibranch" — 병렬·changeset·Multibranch(정답 5·7 연계)
- [02-04. 실패 대응과 파이프라인 원칙](02-04.실패%20대응과%20파이프라인%20원칙.md) § "input 승인 게이트" — input 게이트 Executor 점유(정답 6 연계)
- [02-05. sh step 셸 실행 위생](02-05.sh%20step%20셸%20실행%20위생.md) § "set -euo pipefail" — 셸 실패 전파·크레덴셜 위생
