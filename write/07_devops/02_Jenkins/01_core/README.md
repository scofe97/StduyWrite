...
| 02 파이프라인 | 02-05 | [sh step 셸 실행 위생](02-05.sh%20step%20셸%20실행%20위생.md) | set -euo pipefail, exit code 전파, returnStatus·returnStdout, rm -rf 빈 변수·cd 위치 이탈 함정 |

처음 보는 학습자는 01-00 자가 점검부터 풀고 막힌 만큼 01-01·01-02 로 보강합니다. UI 로 Job 을 만들어 본 사람은 02-01 부터 진입해도 흐름이 끊기지 않습니다. 이미 Declarative 를 쓰고 있다면 02-03·02-04 에서 평소 자기 파이프라인의 약한 지점을 점검하고, sh 안쪽 셸 위생까지 조이려면 02-05 로 마무리합니다.
