...
임시 에이전트(Ephemeral Agent)가 상시 에이전트보다 보안상 유리한 이유는 무엇입니까?

## Q2: Controller Executor를 0으로 설정하면 어떻게 됩니까?

> 답은 §정답 절을 참조합니다.

### 심화

Pipeline은 시작됐는데 Queue에서 빌드가 빠지지 않을 때, Flyweight와 Heavyweight 경계 중 어디서 멈춰 있는지를 어떻게 식별합니까?

## Q3: Queue에 있는 빌드와 실행 중인 빌드를 어떻게 구분합니까?

> 답은 §정답 절을 참조합니다.

### 심화

Queue에서 빌드가 빠지지 않는 원인 3가지를 말할 수 있습니까? (힌트: Label, Executor, Agent 상태)
...
`agent none` Pipeline에서 모든 stage가 `agent { label 'gpu' }`를 쓴다면, GPU 노드가 모두 사용 중일 때 어떤 일이 벌어집니까?
...
`for` 루프로 1000번 반복하는 코드를 Pipeline Groovy에 직접 작성하면 어떤 문제가 생깁니까?

## Q6: JENKINS_HOME은 어디에 존재하고, 무엇을 백업해야 합니까?

> 답은 §정답 절을 참조합니다.

### 심화

Jenkinsfile을 SCM에 저장하는 것과 `jobs/config.xml`에 인라인으로 저장하는 것의 차이는 무엇이며, 백업 전략에 어떤 영향을 줍니까?
...
상세 흐름(queueId vs buildNumber)은 `01-02.md` §3 을 참조합니다.
...
분류 표와 실습 코드는 `01-02.md` §5 를 참조합니다.
...
디렉토리 구조와 항목별 설명은 `01-01.md` §4 를 참조합니다.
