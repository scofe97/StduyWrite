...
`agent { docker { image 'node:18' } }`를 선언하면 Jenkins는 **Agent 노드에서** `docker pull` → `docker run`을 실행합니다. 여기서 핵심적인 전제가 있습니다:

- Agent(또는 Controller)에 **Docker Engine이 설치되어 있어야 합니다**. VM Agent라면 해당 VM에 Docker가 설치되어 있는지 확인하고, 없으면 `sh 'docker ...'` 시점에서 "docker: command not found"로 실패합니다.
- 이미지가 Agent 로컬에 없으면 **Docker Hub(또는 설정된 Registry)에서 자동으로 pull**합니다. 이미지가 존재하지 않는 이미지명이면 pull 단계에서 실패하고 빌드가 중단됩니다.
- Private Registry의 이미지라면 `docker { image '...'; registryUrl '...'; registryCredentialsId '...' }` 형태로 인증 정보를 함께 지정해야 합니다.
...
pipeline {
    agent {
        docker {
            ...
            // 왜 -v 캐시 마운트: 매 빌드마다 Maven 의존성을 새로 받지 않기 위함
...
`-v` 옵션으로 캐시를 마운트하면 매 빌드마다 의존성을 다시 내려받지 않아 빌드 시간을 크게 단축할 수 있습니다.

### kubernetes agent의 동작 원리

`agent { kubernetes { ... } }`를 선언하면 Jenkins **Kubernetes Plugin**이 빌드용 Pod를 동적으로 생성합니다. 빌드가 끝나면 Pod는 자동 삭제됩니다.

- Controller가 K8s 클러스터 안에 있을 필요는 없습니다. Controller가 외부에 있어도 `kubeconfig`로 클러스터에 접근할 수 있으면 Pod를 생성할 수 있습니다.
- Pod 안에 **여러 컨테이너**를 정의할 수 있습니다. `container('name')` 블록으로 특정 컨테이너에서 명령을 실행합니다.
- JNLP 컨테이너가 자동으로 추가되어 Controller와 통신합니다.
...
pipeline {
    ...
    stages {
        stage('Build') {
            steps {
                // 왜 container('maven'): Pod 안 여러 컨테이너 중 maven 환경에서만 명령 실행
                container('maven') {
...
docker agent와 kubernetes agent의 핵심 차이는 다음과 같습니다:
...
### docker vs kubernetes agent 격리 모델

> 같은 "컨테이너 기반 격리" 라도 *경계의 위치* 가 다릅니다. docker 는 Agent 안에서, kubernetes 는 Pod 자체가 단위입니다.

```mermaid
flowchart LR
    subgraph DockerWay["docker agent"]
        DH["Agent 호스트 OS<br>(VM/물리)"]
        DD["Docker Engine"]
        DC["빌드 컨테이너<br>(node:18 등)"]
        DH --> DD --> DC
    end

    subgraph K8sWay["kubernetes agent"]
        KCT["Controller<br>(kubeconfig 보유)"]
        KAPI["K8s API Server"]
        subgraph POD["Pod (동적 생성/삭제)"]
            C1["container: maven"]
            C2["container: docker"]
            C3["container: jnlp<br>(자동 추가)"]
        end
        KCT --> KAPI --> POD
    end

    style DH fill:#fff3e0,stroke:#ef6c00,color:#000
    style DD fill:#fff3e0,stroke:#ef6c00,color:#000
    style DC fill:#e8f5e9,stroke:#2e7d32,color:#000
    style KCT fill:#e3f2fd,stroke:#1565c0,color:#000
    style KAPI fill:#e3f2fd,stroke:#1565c0,color:#000
...
> 주황색은 *Agent 호스트와 커널을 공유* 하는 자리, 초록색은 *빌드 명령이 도는 컨테이너*, 파란색은 *K8s 컨트롤 플레인* 입니다. docker 는 Agent 호스트 OS 가 단일 실패 지점이 되고, kubernetes 는 Pod 자체가 단위라 여러 컨테이너를 sidecar 로 묶는 게 자연스럽습니다.
