...
> 실패 대응, 라이브러리 분리, 파이프라인 작성 원칙을 다룹니다. 본편은 `retry`·`timeout`·`input` 세 제어 블록을 *본문*으로 깊게 다루고, Shared Library 와 Docker 는 "무엇을 위해 있고 어디서 깊게 보는가"만 짚는 *포인터*입니다 — 전자는 05_operations, 후자는 03_agent 가 SSOT 입니다.
...
- Shared Library를 도입하면 개별 Jenkinsfile이 비즈니스 로직(무엇을 빌드하고 어디에 배포하는가)에만 집중할 수 있습니다. 공통 인프라 코드(어떻게 빌드하고 알리는가)는 라이브러리에서 한 번만 관리합니다. 상세 구현과 vars/ 작성법은 [../05_operations/02-01. 공유 라이브러리](../05_operations/02-01.공유%20라이브러리.md) 와 [../05_operations/02-01a. 공유 라이브러리 실전 패턴](../05_operations/02-01a.공유%20라이브러리%20실전%20패턴.md) 에서 다룹니다.
...
- Docker in Docker 구성이나 멀티 컨테이너 Agent 패턴은 [../03_agent/01-02. Docker with Pipeline](../03_agent/01-02.Docker%20with%20Pipeline.md) § "2 호스트 Docker vs DinD vs Kaniko" 에서 상세히 다룹니다.
