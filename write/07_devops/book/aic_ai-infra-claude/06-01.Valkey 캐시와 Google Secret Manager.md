...
| 도구 | 특징 | 단점 | Notiflex 적합도 |
|------|------|------|----------------|
| Valkey | Redis 포크, BSD 라이선스 | 커뮤니티 아직 성장 중 | ★★★ |
| Redis | 인메모리 DB 표준 | SSPL 라이선스(상용 제한) | ★★ |
| Memcached | 단순 키-값 캐시 | 영속성 없음, INCR 원자성 제한 | ★ |
| DragonflyDB | Redis 호환, 멀티 스레드 | 아직 성숙하지 않음, Helm Chart 미성숙 | ★ |
...
`provider`와 `driver` 이름을 GKE managed용으로 정확히 써야 합니다. GKE managed는 provider `gke` / driver `secrets-store-gke.csi.k8s.io`이고, 오픈소스 Secrets Store CSI는 provider `gcp` / driver `secrets-store.csi.k8s.io`입니다. 이름이 비슷해 섞어 쓰면 Pod가 시작되지 않습니다.
...
단 클러스터와 노드풀 양쪽 모두에서 Workload Identity가 활성화돼야 합니다. 한쪽만 켜면 `metadata server not available` 에러가 납니다. GKE는 `--workload-pool`과 Secret Manager 애드온을 켜야 이 조합이 동작합니다.
...
- **드라이버 이름을 정확히**: GKE managed는 `secrets-store-gke.csi.k8s.io`(provider `gke`)입니다. 오픈소스 Secrets Store CSI(`secrets-store.csi.k8s.io`, provider `gcp`)와 이름이 비슷해 혼동하면 Pod가 시작되지 않으니 매니페스트에서 확인하세요.
...
- [06-02. 점진적 배포 Canary와 claude-context](./06-02.%EC%A0%90%EC%A7%84%EC%A0%81%20%EB%B0%B0%ED%8F%AC%20Canary%EC%99%80%20claude-context.md) — 같은 6장의 배포 전략 축(Canary)과 AI 협업 문서 3층 구조
