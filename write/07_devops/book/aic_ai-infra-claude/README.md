...
"Claude로 만든다"는 *주제*가 아니라 *방법*입니다. 각 장은 인프라를 한 조각 쌓을 때마다 그 작업을 Claude Code와 어떻게 협업했는지를 마무리 절에 정리합니다(`CLAUDE.md` 행동규칙 → 메모리 컨텍스트 → 아키텍처 결정 기록 → `claude-context/` → `settings.local.json` 권한 분리 → `command-guardrails/`). 모델 자체의 특성·LLM 일반론은 `10_AI/` <!-- 링크 끊김(2026-08): ../../../../10_AI/README.md -->, 순수 K8s 네트워킹·오브젝트 이론은 `08_cloud/kubernetes/` <!-- 링크 끊김(2026-08): ../../../../08_cloud/kubernetes/README.md -->가 맡습니다. 이 책은 그 둘을 *배포 인프라를 만드는 워크플로우*로 엮는 자리입니다.
