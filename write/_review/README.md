---
title: write 학습 문서 복습 시스템
tags: [moc, review, spaced-repetition, sm-2]
status: final
related:
  - ./_template/review-template.md
  - ./_template/review-routine.md
  - ../README.md
updated: 2026-09-09
---

# write 학습 문서 복습 시스템

---

> write/ 의 모든 학습 문서를 *간격 반복(spaced repetition) + 능동 회상(active recall)* 으로 장기 기억에 안착시키는 시스템. Ebbinghaus 망각곡선 + SM-2 알고리즘 + Karpicke testing effect 의 학술 근거 기반.

## 빠른 시작

> **단일 통합 프롬프트 1개** → [_template/coworker-prompts.md](_template/coworker-prompts.md)
>
> 이 프롬프트 하나만 Claude routine 에 박거나 직접 복붙하면 *상황을 알아서 판단* 해 회차 1·2·3·종합 평가 중 필요한 동작을 자동 실행합니다. 시나리오마다 다른 프롬프트 고를 필요 없음.

**실행 주기**: 매주 월·수·금 오전 9시 (`cron: 0 9 * * 1,3,5`) — 학술 근거 기반 ([routine §5](_template/review-routine.md#5-pomodoro-결합-권장-패턴))

**자동 등록 (`/schedule` 활용)**:

```
/schedule 등록:
이름: weekly-review
cron: 0 9 * * 1,3,5
프롬프트: write/_review/_template/coworker-prompts.md 의 "통합 프롬프트" 섹션 그대로 실행
```

운영 가이드·졸업 기준·간격 표 같은 상세 맥락은 [review-routine.md](_template/review-routine.md) 참조.

## 구조

```
write/_review/
├── README.md                        ← 진입점 (현재 파일, 회차 통계 표)
├── _template/
│   ├── review-template.md           ← 복습 파일 표준 템플릿 (5문제 + SM-2 + 3축)
│   └── review-routine.md            ← 운영 가이드 (간격·트리거·졸업·코워크 프롬프트)
└── YYYY-MM-DD/                      ← 회차별 날짜 폴더
    └── {카테고리}_{번호}.review.md   ← 문서별 1파일
```

`_review/` 와 `_template/` 의 언더스코어 prefix 는 `second-brain-harness.md §4.1` <!-- 링크 끊김(2026-08): ../../../claude/.claude/skills/content/writing/references/second-brain-harness.md --> 의 "일반 최종본이 아님" 규약.

## 핵심 원칙 3가지

1. **Active recall (Karpicke & Roediger 2006)** — 답을 *떠올린 뒤* 정답과 비교. 답을 먼저 보면 학습 효과 80% 감소
2. **Spaced repetition (Ebbinghaus 1885)** — 1·3·7·14·30일 간격으로 회차 반복. 매번 quality 점수로 다음 간격 조정 (SM-2 변형)
3. **Interleaving (회차 4 이상)** — 회차 1~3 은 같은 카테고리 blocked, 회차 4+ 는 카테고리 무관 섞기 (transfer 효과)

## 회차 통계 표

> 각 회차 종합 평가 후 자동 갱신. 평균 quality 가 회차마다 올라가는지가 학습 효과의 직접 지표.

| 회차 | 날짜 | 대상 편수 | 평균 quality | 3축 평균 | 졸업 수 |
|------|------|---------|------------|---------|--------|
| 1 | 2026-05-26 (예정) | 18 (querydsl 16 + JDBC 2) | _ | _ | _ |
| 1 | 2026-07-13 | 1 (Kubernetes in Action 08-01) | 3 | 미측정 | 0 |
| 1 | 2026-07-14 | 1 (Kubernetes in Action 08-02) | 3 | 미측정 | 0 |
| 1 | 2026-08-26 | 2 (Networking and Kubernetes 00-03·01-03) | _ | _ | _ |
| 1 | 2026-08-31 | 7 (Networking and Kubernetes 01-01·01-02 + 02-01~02-05) | _ | _ | _ |
| 1 | 2026-09-06 | 2 (Networking and Kubernetes 01-01·01-02 — 08-31 폴더 미완분) | 2.0 | 2.17 | 0 |
| 2 | 2026-09-06 | 2 (Kubernetes in Action 08-01·08-02) | 1.0 | 1.33 | 0 |
| 1 | 2026-09-08 | 3 (Learning Modern Linux 07-01·07-02·07-03) | 2.67 | 2.89 | 0 |
| 1 | 2026-09-09 | 5 (Computer Networking Top-Down `03-01`~`03-05`, 3장 전체) | _ | _ | _ |
| 2 | 2026-09-09 | 1 (Learning Modern Linux `07-03` — 회차 1 이 quality 2 라 하루 뒤 재시도) | 2.0 | 2.67 | 0 |
| 3 | 2026-09-10 | 1 (Learning Modern Linux `07-03` — 막힌 3축만 좁혀서) | 3.0 | 3.0 | 0 |
| 1 | 2026-09-14 | 4 (Computer Networking Top-Down `05-01`~`05-04`, 5장 전체 — 즉석 모드) | 2.0 | 2.33 | 0 |
| 2 | 2026-09-15 | 4 (Computer Networking Top-Down `05-01`~`05-04`, 5장 전체 — 막힌 자리 재출제 + 드릴 공백. 2연속 q2 로 재학습 등록) | 2.0 | 2.33 | 0 |
| 1 | 2026-09-16 | 5 (Computer Networking Top-Down `03-01`~`03-05`, 3장 전체 — 큐의 `대기` 를 연 첫 회차. 원문은 채점 직전에 해당 절만 읽는 방식으로 진행) | 2.57 | 2.67 | 0 |
| 1 | 2026-09-17 | 5 (Networking and Kubernetes `02-01`~`02-05`, 2장 전체 — 챕터 단위 시트 `nak_ch02.review.md` 한 장) | 2.86 | 2.33 | 0 |
| 2 | 2026-09-20 | 5 (Networking and Kubernetes `02-01`~`02-05`, 2장 전체 — 회차 1 에서 막힌 다섯 자리만 좁혀 재출제. 쉬운 둘을 뺐으므로 **같은 다섯 기준 2.4 → 2.6**) | 2.6 | 2.67 | 0 |

회차 5 에서 *졸업 수 / 대상 편수 ≥ 80%* 면 본 시스템이 잘 작동하는 것.

### 미완 회차

> 파일은 만들어졌고 자답·quality 가 비어 있는 회차. `learning-status.mjs` 가 `quality 미입력 — 회차 미완` 으로 잡는다.

| 폴더 | 편수 | 대상 |
|------|-----|------|
| `2026-08-26/` | 2 | Networking and Kubernetes `00-03`·`01-03` |
| ~~`2026-08-31/`~~ | ~~5~~ | ~~Networking and Kubernetes `02-01`~`02-05`~~ → **해소**. 편 단위 시트 5장은 커밋 `74cbd440a`(2026-09-12, 「편 단위 복습 시트를 걷어내고 챕터 단위 큐로 명부를 옮긴다」)에서 걷혔고, 2026-09-17 에 챕터 단위 `nak_ch02.review.md` 한 장으로 다시 서서 q3 로 완료됐다. `01-01`·`01-02` 는 2026-09-06 완료 |
| `2026-09-09/` | 5 | Computer Networking Top-Down `03-01`~`03-05` (3장 Transport 전체) |
| `2026-09-09/` | 8 | **Kubernetes in Action `11-01`~`13-03`** — Service 3편 · Ingress 2편 · Gateway API 3편 |
| `2026-09-10/` | 8 | **Kubernetes in Action `14-01`~`16-03`** — ReplicaSet 2편 · Deployment 3편 · StatefulSet 3편 |
| `2026-09-09/` | 5 | Computer Networking Top-Down `03-01`~`03-05` (3장 전체). 같은 날 **3장 전체 Phase 4(5축)를 먼저 돌렸고** 그 결과를 각 시트 머리말에 박아 뒀습니다 — 넷 통과·함정 축 미통과(처리량과 타이밍을 「속도」로 뭉침, 두 번째 반복). `03-04` 는 Phase 4 미출제라 미검증 구간으로 표시 |

밀린 회차를 한 번에 몰아 돌리지 않는다 — 오늘 몫만 하고 나머지는 다음 회차로 잇는다. 한 세션 표준은 5편이다.

2026-09-06 세션에서 `01-02`·`01-01` 두 편을 완료했고 **둘 다 quality 2** 로 재학습이 등록됐다. 다음 우선순위는 `00-03`(네트워크 선수 개념) — 두 편의 실패가 모두 선행 개념 층에서 났고 ZPD 판정도 `내림 (선행 개념으로)` 이다.

2026-09-10 에 **Learning Modern Linux `07-03` 이 회차 3 에서 처음으로 올랐다**(quality 2 → 3). 회차 2 에서 0 점이던 `curl -I` 함정이 4 점, 2 점이던 종료 코드가 3 점이다. 반복 인출이 먹은 첫 사례라 기록해 둔다. **다만 회차 3 에서 문항 설계 오류가 하나 드러났다** — Q2 가 물은 `ss` 의 `Recv-Q`/`Send-Q` 는 원문 어디에도 없는 내용이었고, 회차 2 정답이 그 표에 「원본 §3」 을 단 것도 잘못된 출처 표기였다. 학습자 축이 아니므로 노트 보강 뒤 재출제한다. 기준 답안은 원문에서만 인용한다는 규약이 지켜지지 않으면 이렇게 **학습자가 답할 수 없는 문항을 내고 감점하게 된다.**

2026-09-10 에 **Kubernetes in Action 14~16장 8편**을 이어서 신설했다. 이제 이 책의 회차 파일은 18편(`08-01`·`08-02` + 11~16장 16편)이고 회차 파일 없는 노트가 31편이다. 11~13장과 같은 방식으로 만들되 **정답은 원본 §정답을 축자로 옮겼다** — 회차 파일을 쓰며 요약하면 출처가 어긋난다(2026-09-10 `07-03` 회차 2 정답 4 가 실제로 그렇게 어긋났다). 원본 문항이 5 개를 넘는 편(15-01 은 8, 15-02·16-02·16-03 은 9)은 다섯을 골라 5 축에 배치하고 **남긴 문항을 파일 머리말에 명시**해 회차 2 가 그 자리에서 뽑도록 했다. 학습 목표가 앞 편 상호 참조로 시작하는 넷은 상대 경로를 `_review/` 기준으로 다시 겨눴다 — 링크 13 개 전부 실재 확인했다.

2026-09-09 에 **Kubernetes in Action 11~13장 8편**의 회차 파일을 신설했다. 이 책은 정독 노트 49편인데 회차 파일이 `08-01`·`08-02` 둘뿐이어서, 학습 속도를 복습이 전혀 따라가지 못하고 있었다. 문항은 각 편의 §면접에서 받을 만한 질문을 5축(정의·동기·메커니즘·적용·함정)에 배치하고 정답은 §정답을 따랐다. 나머지 39편은 아직 회차 파일이 없다 — `08-01`·`08-02` 가 회차 2 에서 quality 1 로 퇴행하고 재학습이 등록된 상태라 대량 생성은 그 뒤로 미룬다.

## 졸업한 문서 목록

> 회차 4~5 에서 졸업 기준 (quality ≥ 4 + 메타인지 평균 ≥ 3.6 + 미해결 _mistakes 0개) 충족한 문서.

*(아직 졸업 문서 없음 — 첫 회차 30일 후 첫 졸업 후보 등장 예정)*

## 운영 중인 오답 노트

| 카테고리 | _mistakes.md 경로 | 누적 패턴 수 |
|---------|------------------|------------|
| querydsl | `../05_data/03_persistence/querydsl/_mistakes.md` | 0 |
| JDBC wrap | `../05_data/_mistakes.md` | 0 |
| Kubernetes in Action | `../08_cloud/book/kubernetes-in-action/_mistakes.md` | 6 |

복습 회차에서 *quality ≤ 3 인 질문* 이 발생할 때마다 해당 카테고리 `_mistakes.md` 에 append. 같은 패턴 *3회 이상* 반복 시 원본 챕터 본문 보강 트리거.

## 학술 근거

본 시스템의 모든 설계 결정은 다음 학술 근거에 기반:

- [Forgetting curve (Ebbinghaus 1885, Wikipedia)](https://en.wikipedia.org/wiki/Forgetting_curve) — 1h 안 50%, 24h 안 70% 망각
- [Karpicke & Roediger (2006)](https://cognitivetrain.com/active-recall/) — 1주일 후 retrieval practice 그룹이 80% 더 많이 기억
- [Rowland (2014) 메타분석](https://www.sciencedirect.com/science/article/abs/pii/S0959475217301810) — 159 비교/61 연구, retrieval practice 효과 medium-large
- [SuperMemo SM-2 (Wozniak 1987)](https://en.wikipedia.org/wiki/SuperMemo) — quality 점수 + easiness factor 기반 간격 자동 조정
- [Interleaving research (PMC)](https://pmc.ncbi.nlm.nih.gov/articles/PMC8476370/) — 초보 blocked, 숙련 interleaved

상세는 [_template/review-routine.md](_template/review-routine.md) §1·§4 참조.

## 관련 문서

- [_template/review-template.md](_template/review-template.md) — 복습 파일 표준 템플릿
- [_template/review-routine.md](_template/review-routine.md) — 운영 가이드 + 코워크 프롬프트 §7
- `../../../claude/.claude/skills/content/writing/references/02-learned-docs.md` <!-- 링크 끊김(2026-08): ../../../claude/.claude/skills/content/writing/references/02-learned-docs.md --> §12~§15 — 학습 문서 하네스의 Stage 2 워크플로우 규약
