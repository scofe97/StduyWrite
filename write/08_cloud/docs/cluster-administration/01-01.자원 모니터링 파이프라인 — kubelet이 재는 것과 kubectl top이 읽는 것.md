...
여기서 나온 값을 실제로 소비하는 쪽은 오토스케일러입니다. HPA 가 사용률을 계산할 때 분모로 쓰는 것이 requests 이므로, 이 파이프라인의 값과 requests 설정이 함께 움직입니다. 그 동작은 [오토스케일링](../../kubernetes/05_scheduling/05-03.%EC%98%A4%ED%86%A0%EC%8A%A4%EC%BC%80%EC%9D%BC%EB%A7%81.md)에 있습니다.
