...
여기서 `node.kubernetes.io/instance-type` 과 `topology.kubernetes.io/zone` 을 찾습니다. 공식 문서는 두 라벨 모두 클라우드 프로바이더를 쓰는 경우에만 설정된다고 못 박습니다.[^k8s-labels] 앞의 것은 kubelet 이 클라우드 프로바이더가 정의한 인스턴스 타입으로 채웁니다. 뒤의 것은 kubelet 또는 외부 cloud-controller-manager 가 클라우드 프로바이더에서 받은 정보로 채웁니다.

이 라벨이 붙어 있으면 노드는 클라우드가 만들어 준 서버입니다. 읽기 1 쪽입니다.

한 걸음 더 들어가려면 cloud-controller-manager 가 도는지 봅니다.

```bash
kubectl get pods -n kube-system | grep cloud-controller
```

공식 문서는 이 컴포넌트의 노드 컨트롤러를 이렇게 설명합니다. 클라우드 인프라에 새 서버가 만들어지면 오브젝트를 갱신하고, 클라우드 프로바이더 API 에서 얻은 그 서버의 고유 식별자로 Node 오브젝트를 갱신한다는 것입니다.[^k8s-ccm] 이것이 돌고 있다면 Node 하나가 클라우드의 서버 하나에 대응한다는 뜻입니다.

### 라벨이 하나도 없을 때

클라우드 프로바이더 연동 없이 세운 클러스터도 있습니다. 그때는 노드 안을 봅니다. SSH 가 없어도 됩니다.

```bash
kubectl debug node/<노드이름> -it --image=ubuntu
```

공식 문서는 이 명령을 SSH 로 노드에 접근할 수 없는 상황에 유용하다고 소개하며, 만들어진 파드가 노드의 루트 파일시스템을 `/host` 에 마운트한다고 적습니다.[^k8s-node-debug] 그 안에서 펌웨어가 알려 주는 제조사 문자열을 읽습니다.

```bash
cat /host/sys/class/dmi/id/sys_vendor
cat /host/sys/class/dmi/id/product_name
```

베어메탈이면 서버 제조사 이름이 나오고, 가상 머신이면 하이퍼바이저나 클라우드 플랫폼 이름이 나옵니다. 이 경로는 Linux 커널이 펌웨어의 DMI 정보를 노출하는 자리이며 k8s.io 밖입니다.
...
컨트롤 플레인 노드가 목록에 없고 `kube-apiserver` 파드도 안 보이는데 클러스터가 멀쩡히 동작한다면, 컨트롤 플레인이 이 클러스터 밖에서 돌고 있다는 뜻입니다.

### 여기서 클러스터의 답이 끝납니다

노드가 가상 머신이라는 것까지는 클러스터가 알려 줍니다. 그러나 그 노드 VM 이 어느 물리 호스트에 얹혀 있는지, 그 호스트를 다른 클러스터와 나눠 쓰는지는 클러스터가 답하지 못합니다.

§6 에서 본 이유 그대로입니다. 노드는 자기에게 보이는 것만 보고하고, 스케줄러의 계산은 kubelet 통제 밖의 프로세스를 제외합니다. 아래층의 동거는 그 바깥보다도 더 바깥입니다.

그 답은 OpenStack 쪽에서 인스턴스가 어느 컴퓨트 호스트에 배치됐는지 조회해야 나오고, 그 조회는 보통 관리자 권한을 요구합니다. 결국 이 한 가지는 인프라 담당자에게 물어야 합니다. 다만 그때 물어볼 문장이 달라집니다. "클러스터끼리 노드를 공유하나요"가 아니라 "이 클러스터들의 노드 VM 이 같은 컴퓨트 호스트에 배치되나요"라고 물으면 됩니다.
...
물어볼 사람이 마땅치 않으면 §7 의 절차로 앞의 두 가지는 직접 가릴 수 있습니다. 세 번째, 곧 물리 호스트를 실제로 나눠 쓰는지는 클러스터 안에서 답이 나오지 않습니다.
...
[^k8s-labels]: Kubernetes — Well-Known Labels, Annotations and Taints. 두 라벨의 설정 조건. <https://kubernetes.io/docs/reference/labels-annotations-taints/>

[^k8s-ccm]: Kubernetes — Cloud Controller Manager. 노드 컨트롤러가 클라우드의 서버와 Node 오브젝트를 잇는 방식. <https://kubernetes.io/docs/concepts/architecture/cloud-controller/>

[^k8s-node-debug]: Kubernetes — Debugging Kubernetes Nodes With Kubectl. `kubectl debug node` 와 `/host` 마운트. <https://kubernetes.io/docs/tasks/debug/debug-cluster/kubectl-node-debug/>
