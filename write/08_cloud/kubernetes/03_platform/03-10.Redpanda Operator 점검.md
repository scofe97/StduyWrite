...
Strimzi는 Apache Kafka(JVM)를 Kubernetes에서 운영하기 위한 Operator다. 기존 Kafka 클러스터를 이관하거나 Kafka 에코시스템(Kafka Streams, KSQL)을 그대로 유지해야 할 때 적합합니다. `KafkaTopic`, `KafkaUser` 같은 세분화된 CRD로 세밀한 제어가 가능합니다.
