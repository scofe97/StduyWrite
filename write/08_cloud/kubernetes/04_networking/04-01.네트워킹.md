...
8. 외부 요청만 실패하면 먼저 클러스터 내부에서 같은 Service 호출이 성공하는지 확인합니다. 내부 호출이 성공하면 backend 선택과 기본 dataplane은 정상이라는 근거가 되므로 Ingress·Gateway controller, Route, LoadBalancer·NodePort로 범위를 좁힙니다. 내부 호출도 실패하면 외부 진입 규칙보다 Service 하위 계층을 먼저 검사합니다.
...
따라서 Service로도 연결하지 못하면 Service Mesh를 보기 전에 CNI, DNS, Service, EndpointSlice, NetworkPolicy를 확인합니다. 기본 연결은 성공하지만 서비스 간 보안·재시도·트래픽 분할이 문제라면 `서비스 메시 기초` <!-- 링크 끊김(2026-08): ../../service-mesh/01_foundation/01-01.서비스 메시 기초.md -->로 넘어갑니다.



## 7. 점검과 다음 단계
> 개요를 자신의 말로 재구성한 뒤, 막힌 계층의 개별 정본으로 내려갑니다.

YAML 필드, 기능 상태, 운영 제약을 깊게 확인해야 하면 README의 학습 순서에 따라 04-04~04-10 정본으로 이동합니다.
