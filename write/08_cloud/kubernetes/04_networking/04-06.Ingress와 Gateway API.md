...
진입 주소가 할당됐다면 HTTP 응답으로 다음 점검 계층을 좁힙니다. 404는 host·path 규칙과 default backend를 먼저 확인하고, 502나 503은 Service port와 EndpointSlice의 Ready endpoint를 확인합니다. Gateway API는 Gateway와 Route의 `Accepted`, `ResolvedRefs` 조건을 먼저 확인한 뒤 dataplane 로그로 넘어가야 선언 오류와 backend 장애를 빠르게 분리할 수 있습니다.
...
- `Gateway API와 트래픽` <!-- 링크 끊김(2026-08): ../../service-mesh/01_foundation/03-01.Gateway API와 트래픽.md --> — Service Mesh 관점에서 다시 보는 후속 문서
