...
TLS Passthrough는 L7 기능(경로 기반 라우팅, 헤더 조작)을 사용할 수 없습니다. Controller가 TLS 내용을 복호화하지 않으므로 SNI 기반 호스트 라우팅만 가능합니다.
...
TCPRoute와 UDPRoute는 아직 실험적(Experimental) 상태입니다. 사용 전에 설치한 Controller의 Conformance Test 통과 현황을 확인해야 합니다.
...
심화 질문. 같은 호스트+경로 충돌 시 어떤 Route가 우선하는가? Gateway API는 결정론적 우선순위 규칙을 따릅니다. 가장 긴 경로 → Exact가 Prefix보다 우선 → 생성 시각이 빠른 것 → 알파벳 순서(namespace/name). `status.parents[].conditions`에 "패배한" Route의 충돌 원인이 기록됩니다.
