...
선택 기준은 단순합니다. VPC 안에서 가능하다면 네이티브, 그 외는 VXLAN. Cilium은 두 모드를 노드별로 섞을 수 있고, 단계적 전환이 가능합니다.
...
`Layer 2` 모드는 ARP/NDP 기반입니다. 한 노드만 LB IP를 ARP 응답으로 광고하고, 모든 트래픽이 그 노드로 들어온 뒤 kube-proxy로 분산됩니다. 설정이 단순하지만 단일 진입 노드가 SPOF입니다. 노드 장애 시 다른 노드가 ARP를 가져오는 데 약간의 지연이 있습니다.
...
기능 측면에서는 두 솔루션 모두 ToR 스위치와의 peering, ECMP 분산, BFD 같은 표준을 지원합니다. Cilium은 K8s API와의 통합이 더 가까워 BGP peering을 CR(`CiliumBGPPeeringPolicy`)로 표현하고, ServiceLB를 BGP로 광고하는 흐름이 자연스럽습니다. Calico는 BGP가 더 오래된 만큼 복잡한 멀티 AS 시나리오에서 검증된 케이스가 많습니다.
