...
IPv4/IPv6 이중 스택은 Kubernetes v1.23부터 stable이며 v1.21부터 기본 활성화되어 있습니다. 이 기능은 한 Pod에 IPv4 주소 하나와 IPv6 주소 하나를 할당하며 Service가 IPv4·IPv6 또는 둘 다를 사용하게 합니다. Pod는 두 인터페이스 계열을 통해 클러스터 밖으로 나갈 수 있습니다.
...
기존 Service의 primary family를 반대 family로 바꾸어야 한다면 필드 수정만으로는 처리할 수 없습니다. 원하는 primary 순서로 새 Service를 만들고 클라이언트를 전환해야 합니다. 이때 DNS 캐시, Service 이름, 외부 LoadBalancer 주소처럼 주소 안정성에 의존하는 항목을 함께 점검해야 합니다.
...
현재 문서는 dual-stack control plane과 CNI가 준비된 환경을 전제하지 않으므로 실행 결과를 제시하지 않습니다. 검증 환경에서는 `kubectl get nodes -o wide`로 노드 주소, `kubectl get pod -o wide`로 Pod 주소, `kubectl get service -o yaml`로 `ipFamilyPolicy`, `ipFamilies`, `clusterIPs`를 확인합니다. 이어서 EndpointSlice에 각 family의 Ready endpoint가 존재하는지를 대조해야 Service 주소 할당과 실제 전달 경로를 분리해 판단할 수 있습니다.
...
> 이중 스택의 기반 경로와 Service 주소 필드를 연결합니다.
