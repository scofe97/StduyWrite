...
> Kubernetes 보안의 출발점은 API 서버와의 통신이 어떻게 보호되고 인증되는지 이해하는 것입니다. kubeconfig·인증서·CA 는 각각 다른 층위의 개념입니다. 본 장은 컨트롤 플레인 PKI(API 서버·etcd·kubelet 인증서)에 집중하고, 외부 트래픽 인증서 자동화(cert-manager·Issuer·Certificate)는 [04-06 Ingress와 Gateway API §3](../04_networking/04-06.Ingress%EC%99%80%20Gateway%20API.md#3-tls-관리)에서 다룹니다.
...
갱신은 두 경로가 있습니다. 컨트롤 플레인 **업그레이드 시 kubeadm 이 자동 갱신**하고([05-01](06-01.%ED%81%B4%EB%9F%AC%EC%8A%A4%ED%84%B0%20%EC%97%85%EA%B7%B8%EB%A0%88%EC%9D%B4%EB%93%9C%EC%99%80%20ETCD%20%EB%B0%B1%EC%97%85%C2%B7%EB%B3%B5%EA%B5%AC.md) 참조), 필요하면 수동으로 갱신합니다.
...
## 5. 점검 질문
> RBAC 전에 인증과 신뢰 체인을 먼저 점검합니다.

### Q1. kubeconfig를 왜 신뢰된 출처에서만 받아야 하는가?

공식 문서에도 경고가 있습니다. kubeconfig는 단순 텍스트 설정처럼 보여도 인증 실행 경로를 포함할 수 있어서, 신뢰할 수 없는 파일을 그대로 쓰면 악성 동작이나 정보 노출로 이어질 수 있습니다. 셸 스크립트처럼 조심해서 다루는 편이 맞습니다.

### Q2. TLS와 RBAC은 무엇이 다른가?

TLS는 누구를 신뢰하고 누구로 인증할지를 다루고, RBAC은 인증이 끝난 뒤 무엇을 할 수 있는지를 다룹니다. 접속 실패와 권한 실패를 같은 문제로 보면 진단이 오래 걸립니다.

### Q3. 인증서 만료는 언제 드러나는가?

평소에는 잘 안 보이지만, API 접속 실패, kubelet 인증 실패, kubeadm 업그레이드 시점에 갑자기 드러납니다. 그래서 인증서 관리는 보안 장보다 운영 장과 더 가까운 주제이기도 합니다.

### Q4. 서버 인증서와 클라이언트 인증서는 어떻게 다른가?

서버 인증서는 *"이 호스트가 진짜 그 호스트인가"*를 증명합니다. API 서버, etcd, kubelet이 각자 서버 인증서를 들고 있으며 SAN(Subject Alternative Name)에 호스트명·IP가 박혀 있습니다. 클라이언트가 접속할 때 이 인증서의 CN/SAN을 확인하고 신뢰 CA로 서명된 것인지 검사합니다.

클라이언트 인증서는 *"내가 누구인가"*를 증명합니다. kubeconfig에 들어 있는 `client-certificate`/`client-key`가 그것이며, CN이 사용자명, O(Organization)가 그룹이 됩니다. API 서버는 이 CN/O를 RBAC 결정의 입력으로 씁니다. 같은 PKI 안에서 발급되더라도 *역할이 정반대*다 — 한쪽은 "내 신원을 상대에게 보여 주고", 한쪽은 "상대가 진짜 그 서버인지 검증한다". 운영 사고는 둘을 같은 의미로 본 자리에서 자주 납니다.
...
- [Ingress와 Gateway API](../04_networking/04-06.Ingress%EC%99%80%20Gateway%20API.md) — cert-manager 로 외부 트래픽 인증서 자동화
- [RBAC과 보안](../07_security/07-01.RBAC%EA%B3%BC%20%EB%B3%B4%EC%95%88.md) — Admission·ServiceAccount 토큰
- [클러스터 업그레이드와 ETCD 백업·복구](06-01.%ED%81%B4%EB%9F%AC%EC%8A%A4%ED%84%B0%20%EC%97%85%EA%B7%B8%EB%A0%88%EC%9D%B4%EB%93%9C%EC%99%80%20ETCD%20%EB%B0%B1%EC%97%85%C2%B7%EB%B3%B5%EA%B5%AC.md)
