...
1. **Troubleshooting(30%)** — `kubectl get/describe/logs/exec`, 노드·Pod·서비스 장애 진단([09-01](09-01.%EB%AA%A8%EB%8B%88%ED%84%B0%EB%A7%81%EA%B3%BC%20%ED%8A%B8%EB%9F%AC%EB%B8%94%EC%8A%88%ED%8C%85.md))
2. **Cluster Architecture(25%)** — kubeadm 업그레이드·etcd 백업([06-01](../06_architecture/06-01.%ED%81%B4%EB%9F%AC%EC%8A%A4%ED%84%B0%20%EC%97%85%EA%B7%B8%EB%A0%88%EC%9D%B4%EB%93%9C%EC%99%80%20ETCD%20%EB%B0%B1%EC%97%85%C2%B7%EB%B3%B5%EA%B5%AC.md)), RBAC([07-01](../07_security/07-01.RBAC%EA%B3%BC%20%EB%B3%B4%EC%95%88.md))
3. **Services & Networking(20%)** — Service·Ingress·NetworkPolicy([04_networking](../04_networking/README.md))
4. **Workloads & Scheduling(15%)** — Deployment·rollout, 스케줄링([05-01](../05_scheduling/05-01.%EC%8A%A4%EC%BC%80%EC%A4%84%EB%A7%81%EA%B3%BC%20%EB%85%B8%EB%93%9C%20%EC%84%A0%ED%83%9D.md))
5. **Storage(10%)** — PVC·StorageClass·StatefulSet([03-01](../03_storage/03-01.%EC%8A%A4%ED%86%A0%EB%A6%AC%EC%A7%80%EC%99%80%20%EC%83%81%ED%83%9C.md))

여기에 JSONPath([09-03](09-03.JSONPath%EC%99%80%20kubectl%20%EA%B3%A0%EA%B8%89%20%EC%A1%B0%ED%9A%8C.md))·`kubectl explain`·`drain`·`rollout undo` 같은 반복 명령이 붙습니다.
...
## 5. 점검 질문
> 시험 준비를 별도 암기집이 아니라 운영 루틴 압축으로 다시 점검합니다.

### Q1. CKA에서 가장 먼저 빨라져야 하는 것은 무엇인가?

`kubectl get`, `describe`, `logs`, `exec`, `apply`, `edit` 같은 기본 명령입니다. 문제를 많이 아는 것보다 기본 명령을 빠르게 쓰는 쪽이 점수에 더 직접적으로 연결됩니다.

### Q2. JSONPath를 시험 준비에 왜 넣는가?

반복 조회를 줄이고 필요한 필드만 빠르게 뽑을 수 있기 때문입니다. 다만 JSONPath 자체를 암기 과목으로 보기보다, 실무에서 자주 쓰는 짧은 스니펫 몇 개를 손에 익히는 편이 더 효율적입니다.

### Q3. 이 문서 세트로 어떻게 복습하는가?

핵심 워크로드, 네트워킹, 스토리지, 보안, 운영 심화 순으로 한 번 돌리고, 각 deepdive에서 명령과 체크리스트만 다시 보는 방식이 가장 자연스럽습니다.
...
- [JSONPath와 kubectl 고급 조회](09-03.JSONPath%EC%99%80%20kubectl%20%EA%B3%A0%EA%B8%89%20%EC%A1%B0%ED%9A%8C.md) — 조회 속도
- [모니터링과 트러블슈팅](09-01.%EB%AA%A8%EB%8B%88%ED%84%B0%EB%A7%81%EA%B3%BC%20%ED%8A%B8%EB%9F%AC%EB%B8%94%EC%8A%88%ED%8C%85.md) — 배점 1위 영역
