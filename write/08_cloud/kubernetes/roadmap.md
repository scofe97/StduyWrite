...
- **etcd**: 클러스터 상태 저장소 · 모든 리소스의 원천 데이터 · 백업/복구에서 가장 중요 · Raft 합의로 강한 일관성(Leader 선출·Log Replication·Quorum·Split-brain 방지, 홀수 대 구성) → [05-01 §2](06_architecture/06-01.%ED%81%B4%EB%9F%AC%EC%8A%A4%ED%84%B0%20%EC%97%85%EA%B7%B8%EB%A0%88%EC%9D%B4%EB%93%9C%EC%99%80%20ETCD%20%EB%B0%B1%EC%97%85%C2%B7%EB%B3%B5%EA%B5%AC.md)
...
> TLS truststore 의 *전체 계층 이동*(JVM/Envoy/OS/VM)은 [05_JVM roadmap §26](../../01_language/book/Inside%20the%20Java%20Virtual%20Machine%20JVM%20Advanced%20Features%20and%20Best%20Practices/roadmap.md) 에 메인으로 정리돼 있다. 여기서는 K8s 측 관점 — *인증서 발급·배포* 만 다룬다.
...
- ⚠️ **핵심: Secret 에 CA 인증서가 있다 ≠ JVM 이 그 CA 를 신뢰한다.** trust-manager → ConfigMap 에 CA bundle 배포 → Pod mount → JVM truststore 로 변환 또는 JVM 옵션 연결, 까지 이어져야 실제 통신이 성공한다. JVM 측 변환(keytool·JAVA_TOOL_OPTIONS·initContainer)은 [05_JVM roadmap §26.5](../../01_language/book/Inside%20the%20Java%20Virtual%20Machine%20JVM%20Advanced%20Features%20and%20Best%20Practices/roadmap.md) 참조.
