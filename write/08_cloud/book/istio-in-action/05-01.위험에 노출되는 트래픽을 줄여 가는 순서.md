...
2. DestinationRule이 라벨을 subset으로 이름 붙이고 VirtualService가 그 subset으로 경로를 정합니다 — 이것을 근거와 함께 설명해 보십시오.
3. 가중치는 합이 100이어야 하고 모든 버전이 subset으로 선언돼야 합니다 — 이것을 근거와 함께 설명해 보십시오.
4. 미러링은 복제본의 응답을 프록시가 무시하므로 실사용자에게 영향이 없습니다 — 이것을 근거와 함께 설명해 보십시오.
