...
learning:
  ...
  blocked_count:
  next_lesson: "본편 14장 + 부록 A~E 전부 완주. 다음은 Phase 4 자답 회차 — 열여덟 편의 면접 질문에 실제로 답해 보고 `last_verified` 를 채운다"
updated: 2026-09-04
---
...
| B·E | [부록 — 사이드카를 이루는 넷과 VM 이 받는 다섯 파일](a0-02.%EB%B6%80%EB%A1%9D%20%E2%80%94%20%EC%82%AC%EC%9D%B4%EB%93%9C%EC%B9%B4%EB%A5%BC%20%EC%9D%B4%EB%A3%A8%EB%8A%94%20%EB%84%B7%EA%B3%BC%20VM%20%EC%9D%B4%20%EB%B0%9B%EB%8A%94%20%EB%8B%A4%EC%84%AF%20%ED%8C%8C%EC%9D%BC.md) | 프록시가 설정을 받는 경로는 파드에서 주입이고 VM 에서 파일 다섯인데, 사이드카 자신은 그 차이를 모른다 | 작성 완료 |
| C | [부록 — 신원을 문서로 만드는 네 규격](a0-03.%EB%B6%80%EB%A1%9D%20%E2%80%94%20%EC%8B%A0%EC%9B%90%EC%9D%84%20%EB%AC%B8%EC%84%9C%EB%A1%9C%20%EB%A7%8C%EB%93%9C%EB%8A%94%20%EB%84%A4%20%EA%B7%9C%EA%B2%A9.md) | 워크로드는 자기 신원을 정의하는 비밀을 가질 수 없고, 그 제약 하나가 규격의 나머지 구조를 결정한다 | 작성 완료 |
| D | [부록 — 포트 열넷과 디버그 엔드포인트 지도](a0-04.%EB%B6%80%EB%A1%9D%20%E2%80%94%20%ED%8F%AC%ED%8A%B8%20%EC%97%B4%EB%84%B7%EA%B3%BC%20%EB%94%94%EB%B2%84%EA%B7%B8%20%EC%97%94%EB%93%9C%ED%8F%AC%EC%9D%B8%ED%8A%B8%20%EC%A7%80%EB%8F%84.md) | 포트는 다른 서비스를 향한 것과 사람이 들여다보려는 것으로 갈리고, 뒤쪽은 운영에서 끄라고 저자가 적는다 | 작성 완료 |
...
| 막힌 지점 | 없음 |
| 다음 레슨 후보 | **본편 14장 + 부록 A~E 전부 완주.** 이 책에서 남은 것은 읽기가 아니라 되새김이 됐다. 다음은 Phase 4 자답 회차 — 열여덟 편이 각각 남긴 면접 질문에 실제로 답해 보고 막힌 자리를 `blocked_count` 에, 회차 날짜를 `last_verified` 에 남겨 둔다. 부록 D 를 쓰면서 포트 수를 다시 셌다 — 앞서 이 칸에 적었던 '열일곱' 은 세지 않고 쓴 값이었고 실제는 에이전트 여덟에 Pilot 여섯을 더한 열넷. A·D 를 2장·10장에 흡수하려던 앞의 계획은 원문을 읽고 접었다 — A 는 프로파일 여덟과 리소스 분리 절차를, D 는 포트 열넷과 엔드포인트 목록을 담아 본문에 녹이면 둘 다 흐려진다 |
| 최근 검증 결과 | Phase 4 를 열려고 질문을 펼치다가 둘을 고쳤다 (2026-09-04). 열여덟 편의 면접 질문 139 개 중 41 개가 답을 문장째 적어 두고 '이것을 근거와 함께 설명해 보십시오' 만 붙인 자문자답이었다. 본편 열네 편이 정확히 셋씩 갖고 있었고 부록 넷은 0 이었다. 그 41 개의 「정답」 본문은 질문을 그대로 복사한 것이라 회상할 것이 남지 않았다. 전부 실제 질문으로 되돌리고 정답 제목을 새 질문에 맞췄다. 또 부록 넷이 정답을 `<details>` 로 감쌌는데 Typora 는 이것을 접지 않아 가림 효과가 0 이었다. 헤딩 방식으로 바꾸고, 형제 책 폴더에는 있고 이 폴더에만 없던 자답 자제 안내를 §정답 절 머리에 넣었다. 부록 D §1 검사·벽 단락 0곳 통과, 도식 7장(타입 7종 — `deployment`·`tree`·`sequence`·`architecture`·`swimlane`·`dependency`·`process`) 본문 폭 800px 렌더로 전수 확인, 원문 식별자 42개 대조 일치 (2026-09-04). 눈 확인에서 셋을 고쳤다 — `agent-endpoints` 가 여섯 형제 사이에 없는 계층을 만들던 것(줄 사이 화살표를 걷고 왼쪽 레일로 둘째 버스를 따로 먹였다), `agent-ports` 의 헬스 체크 화살표가 15020→15021 로 거꾸로였던 것, `tool-mapping` 의 긴 도구 이름이 팬인 배지를 파고들던 것. 부록 C §1 검사·벽 단락 0곳 통과, 도식 8장(타입 8종 — `layers`·`uml-class`·`nested`·`flowchart`·`swimlane`·`sequence`·`data-flow`·`process`) 본문 폭 800px 렌더로 전수 확인, 원문 식별자 42개 대조 일치 (2026-09-04). 링크 하나를 고쳤다 — 『Container Security』 11장의 파일명을 지어냈고 디스크의 실제 이름으로 바꿨다. 눈 확인에서는 `filter-metadata` 의 연결선 세로 구간이 라벨을 관통하던 것을 직선으로 폈다. 부록 B+E §1 검사·벽 단락 0곳 통과, 도식 8장(타입 8종 — `deployment`·`swimlane`·`sequence`·`architecture`·`tree`·`er`·`loop`·`process`) 본문 폭 800px 렌더로 전수 확인, 원문 식별자 44개 대조 일치 (2026-09-04). 눈 확인에서 셋을 고쳤다 — `cluster-env` 의 대응선이 임의의 y 에서 나가 어느 줄이 어느 식별자인지가 틀리게 그려진 것(줄 좌표에서 뽑도록 고치고 상자를 줄 순서대로 재배치), `troubleshoot-loop` 의 진입 화살표가 두 행 사이 빈칸을 가리키던 것, `injection-two-ways` 의 칸 넷이 1044 까지 나가던 것. 부록 A §1 검사·벽 단락 0곳 통과, 도식 7장(타입 7종 — `dependency`·`layers`·`tree`·`flowchart`·`state`·`quadrant`·`process`) 본문 폭 800px 렌더로 전수 확인, 원문 식별자 33개 대조 일치 (2026-09-04). 눈 확인에서 넷을 고쳤다 — `profile-map` 의 버스가 계보 밖 루트 셋 밑을 지나 그 셋도 default 계열로 보이던 것, `operator-name` 의 전이 라벨 마스크가 상태 상자 변을 파먹던 것, `install-paths` 의 팬인 배지가 그려진 화살표 수와 어긋나던 것, `split-install` 의 산문이 끝 타원에 붙던 것. 14장 §1 검사·벽 단락 0곳 통과, 도식 9장(타입 9종 — `pyramid`·`uml-class` 첫 사용) 렌더 확인, 절 서사 골격 검출 0곳, 원문 식별자 108개 대조 일치 (2026-09-02). 자체 적대적 점검에서 셋을 고쳤다 — `WasmPlugin` 도식이 모듈을 당겨 오는 주체를 컨트롤 플레인으로 그린 것(원문은 프록시가 직접 내려받는다), `EnvoyFilter` 를 13장 리소스라 적은 것, 그리고 저자가 하지 않은 "라스트 마일은 대개 첫째·둘째 문에서 끝난다"는 분류. 13장도 같은 방식으로 셋을 고쳤고 10~12장은 서브에이전트 적대적 검증으로 각각 6·6·11건을 고쳤다. 1~9장까지 같은 게이트를 통과했다. Phase 4 자답은 아직 없어 `learning.last_verified` 비움 |
| 복습 회차 | 0 |
