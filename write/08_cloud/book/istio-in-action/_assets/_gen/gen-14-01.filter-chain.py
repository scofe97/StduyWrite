...
ring(32, 132, 936, 396, "LISTENER · 포트를 열고 바이트를 읽는다", "Envoy 는 L3/L4 프록시다", f"{INK}30", f"{INK}04")
ring(76, 196, 848, 296, "NETWORK FILTER CHAIN", "바이트 스트림을 인코딩 · 디코딩한다 — MongoDB · Redis · Thrift · Kafka", MUTED, f"{INK}07")
ring(120, 260, 760, 196, "HTTP CONNECTION MANAGER", "바이트를 HTTP 헤더 · 본문 · 트레일러로 바꾼다", f"{INK}55", f"{INK}09")
ring(164, 324, 672, 108, "HTTP FILTER CHAIN", "요청 위에서 도는 필터들 — 이 장의 확장이 꽂히는 자리", ACC, f"{ACC}0E", focal=True)
...
for i, (lab, c) in enumerate(labels):
    ...
    if i < len(labels) - 1:
        d.path(f"M {x + BW - 24} {BY + BH / 2} H {x + 124}", MUTED, 1.0, m="ar")
