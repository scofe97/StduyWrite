...
for i, (tag, name, mid, right, focal) in enumerate(rows):
    ...
    d.t(LX + LW - 20, y + 44, right, 11, MUTED, KR, "end")

d.path(f"M {LX - 44} {Y0 + 8} L {LX - 44} {Y0 + 3 * LH - 8}", MUTED, 1.2, m="ar")
d.t(LX - 60, Y0 + 24, "난이도", 11, SOFT, KR, "end")
d.t(LX - 60, Y0 + 3 * LH - 24, "높아진다", 11, SOFT, KR, "end")

d.t(28, 412, "메시의 가치는 안에서 도는 서비스 수에 비례하므로 넓힐수록 두 번째 층의 위험도 함께 커진다", 11, SOFT, KR, "start")
d.t(28, 436, "저자는 서비스 메시가 유일한 해법도 아니고 단점이 없지도 않다고 못 박는다", 11, MUTED, KR, "start")
