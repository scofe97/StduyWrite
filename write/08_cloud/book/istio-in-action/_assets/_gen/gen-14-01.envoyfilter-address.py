...
NX, NW, NH, Y0, STEP = 220, 620, 60, 128, 84
nodes = [
...
for i, (key, val, note) in enumerate(nodes):
    ...
    d.t(NX + NW + 24, y + 36, note, 10.5, MUTED, KR, "start")
    if i < len(nodes) - 1:
...
d.t(NX + NW + 24, PY_ + 36, "라우터 바로 앞에 끼운다", 10.5, ACC, KR, "start")

d.t(32, 728, "저자의 경고 셋 — 하위 호환을 가정하지 말 것 · 다른 Istio 리소스가 전부 번역된 뒤에 적용됨 · 잘못 쓰면 데이터 플레인 전체가 내려감", 11, SOFT, KR, "start")
