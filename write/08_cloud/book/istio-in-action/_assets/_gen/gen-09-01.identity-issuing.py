...
zone(60, 128, 280, 116, "ISTIO-SYSTEM")
zone(412, 128, 528, 180, "ISTIOINACTION")

node(88, 156, 228, 68, "istiod", "certificate authority")
node(440, 156, 220, 68, "webapp", "sa/webapp")
node(692, 156, 220, 68, "catalog", "sa/catalog")

# 발급 경로 — 컨트롤 플레인에서 두 워크로드로 내려가는 버스
d.path("M 200 224 L 200 280 L 552 280 L 552 226", INFO, 1.2, m="ar")
d.path("M 552 280 L 804 280 L 804 226", INFO, 1.2, m="ar")
d.t(232, 260, "SVID 발급 · 순환", 11, INFO, KR, "start", 600)

# 상호 인증 — 두 사이드카 사이
d.path("M 660 190 L 688 190", ACC, 1.6, m="acc")
d.t(676, 172, "mTLS", 10, ACC, MONO, "middle", 600)

# SVID 의 내용물 — 존 밖으로 내려 둔다
d.box(412, 348, 500, 112, PAPER2, RULE, 1.0, 6)
d.t(432, 374, "catalog 의 X.509 안에 든 것", 11, ACC, KR, "start", 600)
...
d.t(28, 520, "검증은 /var/run/secrets/istio/root-cert.pem 으로 하고, 서명이 맞으면 한 줄짜리 확인이 나온다", 11, MUTED, KR, "start")
