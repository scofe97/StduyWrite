...
def node(x, y, w, h, tag, name, sub, focal=False, stack=False):
    ...
    if stack:
        ...
        d.t(x + w / 2, y + 84, sub, 9, MUTED, KR, "middle")
    else:
        d.t(x + 62, y + 24, name, 13, ACC if focal else INK, KR, "start", 600)
        d.t(x + 62, y + 42, sub, 9, MUTED, MONO, "start")

zone(48, 140, 640, 356, "WEST-NETWORK · KUBERNETES")
zone(788, 140, 404, 356, "VM-NETWORK")

node(72, 168, 400, 60, "CP", "istiod", "meshID usmesh · west-cluster")
node(72, 240, 400, 60, "SVC", "webapp · catalog", "istioinaction 네임스페이스")
node(72, 312, 400, 60, "GW", "인그레스 게이트웨이", "바깥에서 들어오는 트래픽")
node(496, 168, 168, 204, "EWG", "동서 게이트웨이", "두 망을 잇는 문", focal=True, stack=True)
node(812, 168, 356, 60, "APP", "forum", "8080 을 듣는다")
node(812, 240, 356, 60, "AGT", "istio-agent", "Envoy · DNS 프록시")
node(812, 336, 356, 60, "GRP", "WorkloadGroup 의 network", "vm-network")

d.path("M 810 252 H 668", INFO, 1.3, m="info")
d.path("M 810 288 H 668", ACC, 1.5, m="acc")
d.t(739, 240, "15012", 9, INFO, MONO, "middle", 600)
d.t(739, 308, "15443", 9, ACC, MONO, "middle", 600)
d.path("M 494 198 H 476", INFO, 1.3, m="info")
d.path("M 494 270 H 476", ACC, 1.5, m="acc")

d.t(48, 544, "여는 것 둘 — 15443 은 VM 에서 온 요청을 메시 안의 서비스로 역방향 프록시하고,", 11, SOFT, KR, "start")
d.t(48, 566, "Gateway 와 VirtualService 는 istiod 로 가는 트래픽을 받아들인다", 11, SOFT, KR, "start")
d.t(48, 594, "망 이름을 붙이는 자리 — 클러스터는 istio-system 에 topology.istio.io/network=west-network 라벨을 건다", 11, MUTED, KR, "start")
d.t(48, 622, "같은 망이었다면 이 문이 필요 없다 — 컨트롤 플레인이 IP 로 곧장 보낸다", 11, SOFT, KR, "start")
