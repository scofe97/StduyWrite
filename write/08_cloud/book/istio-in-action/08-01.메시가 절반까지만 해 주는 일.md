...
>
> 화면 셋은 재료와 그리는 대상이 다릅니다. Grafana는 집계된 시계열을 수치로 그리고, Kiali는 같은 시계열을 재료로 방향 그래프를 그리며, Jaeger는 개별 요청 하나가 지나간 홉을 그립니다. 저자가 Kiali를 소개하며 Grafana와의 차이부터 못 박는 것이 이 구분입니다.
>
...
2. 세 화면은 재료와 그리는 대상이 다릅니다 — 이것을 근거와 함께 설명해 보십시오.
3. 추적에서 프록시의 몫은 스팬 생성과 헤더 부착까지입니다 — 이것을 근거와 함께 설명해 보십시오.
4. 추적 설정은 메시 전역 `MeshConfig`와 워크로드 애노테이션 두 자리에 모입니다 — 이것을 근거와 함께 설명해 보십시오.
