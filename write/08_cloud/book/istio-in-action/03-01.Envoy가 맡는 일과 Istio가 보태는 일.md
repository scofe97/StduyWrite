...
2. Envoy 가 L3/L4 프록시와 갈리는 지점은 무엇이고, 그 차이가 무엇을 가능하게 합니까?
3. 요청 하나가 Envoy 를 지나며 거치는 구성요소를 순서대로 짚고, 들어오는 쪽과 나가는 쪽을 무엇이라 부르는지 말해 보십시오.
4. xDS 리소스를 종류별로 따로 받으면 어떤 문제가 생기고, Istio 는 그것을 어떻게 피합니까?
