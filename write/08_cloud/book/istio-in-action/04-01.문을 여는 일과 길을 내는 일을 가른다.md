...
도식은 세 모드를 다섯 질문으로 가른 행렬입니다. 게이트웨이가 인증서를 갖는가, 클라이언트 인증서를 검증하는가, TLS가 어디서 끝나는가, VirtualService는 무엇으로 매칭하는가, Gateway의 포트 프로토콜은 무엇인가. 색이 붙은 칸이 PASSTHROUGH의 종료 지점입니다. 게이트웨이가 TLS를 열지 않고 SNI만 읽는 유일한 모드입니다.
...
2. Istio 는 쿠버네티스 `Ingress` v1 을 그대로 쓰지 않고 `Gateway`·`VirtualService` 를 새로 만들었습니다. 그 이유를 드십시오.
3. 게이트웨이의 TLS 세 모드는 무엇을 기준으로 갈리며, 각각 어디에서 TLS 가 끝납니까?
4. 프로덕션 `default` 프로필에서 접근 로그는 어떤 상태이며, 그것을 워크로드 하나에만 켜려면 어떻게 합니까?
...
### 정답 4 — default 프로필의 접근 로그는 어떤 상태이고 어떻게 켭니까
