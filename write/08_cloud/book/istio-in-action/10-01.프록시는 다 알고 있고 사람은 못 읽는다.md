...
## 핵심 요약
> 이 장 전체가 매달린 하나의 생각은 **프록시가 이미 답을 갖고 있고 문제는 그 답이 13,934줄이라는 것**입니다. 저자가 도구를 여럿 늘어놓는 이유도 기능이 달라서가 아니라 의심 범위를 좁히는 단계가 달라서입니다.

그 생각은 두 사실에서 나옵니다. 요청 하나에 관여하는 것이 넷이라 전부 뒤지면 시간이 걸리고, 데이터 플레인 설정은 설계상 나중에 일관해집니다. 그래서 "아직 안 왔다"와 "틀렸다"가 겉으로 같아 보입니다. 저자가 배제를 먼저 시키는 이유가 그것입니다.

본문은 그 좁히기를 순서로 풉니다. `proxy-status`로 아직 안 온 것을 걷어낸 뒤 자동 분석기로 대부분을 끝냅니다. 그래도 안 잡히면 리스너에서 엔드포인트까지 체인을 손으로 질의하고, 마지막에 로그를 켭니다. 마지막에 함정이 하나 남습니다. 같은 실패를 클라이언트 쪽 프록시와 서버 쪽 프록시가 다르게 기록해서, 어느 쪽 대시보드를 보느냐로 성공률이 갈립니다.
...
- 계층 순서대로 네트워크를 진단하는 도구 선택: [Linux 네트워크 진단 도구](../networking-and-kubernetes/02-04.Linux%20%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC%20%EC%A7%84%EB%8B%A8%20%EB%8F%84%EA%B5%AC%20%E2%80%94%20%EA%B3%84%EC%B8%B5%20%EC%88%9C%EC%84%9C%EB%8C%80%EB%A1%9C%20%EC%88%98%EC%82%AC%ED%95%98%EA%B8%B0.md)
...
2. 데이터 플레인을 파고들기 전에 무엇을 먼저 배제해야 합니까? 그 순서를 지켜야 하는 이유는 무엇입니까?
3. `istioctl analyze`·Kiali·`istioctl describe` 는 각각 어느 시점 어느 범위에서 봅니까?
4. Envoy 클러스터 이름 한 줄에서 무엇을 읽어 낼 수 있으며, 그것이 `istioctl proxy-config` 질의와 어떻게 이어집니까?
