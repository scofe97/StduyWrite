...
source:
  ...
  - https://coredns.io/plugins/route53/
...
> CoreDNS 는 존 데이터를 파일뿐 아니라 디렉터리 스캔, 호스트 테이블, AWS Route 53 에서도 읽습니다. 어디에 두느냐가 그 데이터를 누가 어떻게 고치는지를 정합니다.

## 핵심 요약

> 이 장 전체가 매달린 하나의 생각은 존 데이터의 출처를 바꾸면 관리 방식이 통째로 바뀐다는 것입니다.
...
5. `route53` 이 존을 지정하는 형식과 자격증명을 주는 경로를 말하고, 원서 문법 가운데 지금 버전에서 달라진 자리를 가를 수 있습니다.
...
무엇을 고를지는 기능이 아니라 **관리 방식**이 정합니다. 넷 다 이름을 주소로 바꿔 주지만, 누가 어떻게 고치고 되돌리는지가 다릅니다.

노트는 그 차이를 세 가지 물음으로 가릅니다. 저자가 이렇게 세지는 않지만 각 절이 흩어 적은 사실을 모으면 이 셋이 됩니다. 첫째는 존 전체를 담느냐, 곧 `SOA` 가 있느냐입니다. 둘째는 다른 서버로 전송할 수 있느냐입니다. 셋째는 Corefile 을 고치지 않고 새 존이 들어오느냐입니다. 도식이 이 세 물음을 칸으로 그렸고, 셋째에 "된다"가 붙는 것은 `auto` 하나뿐입니다.
...
> **원문 정오**: 원서가 이 설정과 짝지어 보여 주는 존 파일 Example 4-3 에는 `www IN AAAA 2001:db8:42:1:1` 이 들어 있습니다. 그룹이 다섯뿐이라 유효한 IPv6 주소가 아니며, 같은 파일의 `ns1`·`ns2` 는 `2001:db8:42:1::53` 처럼 `::` 를 제대로 씁니다. 2장 Example 2-6·2-7 에서 나온 것과 같은 부류이므로 옮겨 쓸 때 `::` 를 넣어 `2001:db8:42:1::1` 로 적어야 합니다.
...
> **노트의 읽기**: 위 두 줄은 보조 서버 둘을 적은 것이 아닙니다. 앞줄은 그 주소에 NOTIFY 를 보내면서 전송도 허용하라는 뜻이고, 뒷줄은 전송 자체는 어디서 요청하든 받아 주라는 뜻입니다. 하나는 알림 대상이고 하나는 허용 범위입니다.

> **버전 차이**: 이 문법 블록은 2019년 원서의 모습입니다. `transfer to` 는 CoreDNS 1.8.0 에서 `file`·`auto`·`secondary` 에서 빠지고 별도 `transfer` 플러그인으로 옮겨 갔습니다.[^n180] 지금 버전에 원서대로 `file { transfer to ... }` 를 적으면 알 수 없는 하위 지시어라는 오류로 CoreDNS 가 뜨지 않습니다. `upstream` 은 공식 문서의 문법에서는 빠졌지만 파서가 아직 받아서 버리므로(`case "upstream": // remove soon`) 적어도 오류는 나지 않습니다.[^filesetup] 현재 `file` 문법은 `reload`·`reload_by_mtime`·`fallthrough` 셋이고, `reload_by_mtime` 은 `SOA` 시리얼 대신 파일 수정 시각으로 다시 읽을지 판단하게 합니다.[^fileplugin] 시리얼 올리는 것을 자주 잊는 운영에서 쓸모가 있습니다.

`reload` 는 존 데이터 파일의 `SOA` 시리얼 번호가 바뀌었는지 확인하는 주기입니다. 새 시리얼을 발견하면 파일을 다시 읽고, `transfer` 에 지정된 보조 서버들에 NOTIFY 를 보냅니다. **기본값은 1분**이고 `0` 으로 두면 주기 확인이 꺼집니다. 이 기본값은 지금 문서에도 그대로입니다.[^fileplugin] 값은 숫자 뒤에 `s`·`m`·`h` 를 붙입니다.
...
여기가 이 절의 요점입니다. 그 디렉터리에 이름만 제대로 붙인 파일을 새로 만들면 존이 하나 더 생깁니다. Corefile 은 손대지 않습니다. 다만 저자가 괄호로 다는 단서가 있습니다. 새로 생기는 존은 `ZONES` 로 적어 둔 도메인, 위 예제라면 `example` 아래여야 합니다. 현재 문서는 이것을 더 정확히 적습니다. 범위 밖 존도 읽기는 읽지만 결코 질의되지 않습니다.[^autoplugin] 파일은 읽히는데 답이 안 나가는 증상이라 헷갈리기 쉬운 자리입니다.
...
정규식은 사실상 Perl 캡처 그룹을 담을 것으로 기대됩니다. `.*` 를 감싼 괄호가 origin 이 들어 있는 파일 이름의 구간을 가리키고, 여기서는 `.zone` 앞의 전부입니다. `{1}` 은 그 구간에 대한 역참조입니다. 두 번째 캡처 그룹이 필요하면 `{2}` 처럼 다른 역참조도 쓸 수 있습니다. 현재 문서는 이 정규식이 앞뒤를 고정하지 않는다고 경고합니다(`"REGEXP is unanchored"`). `foo.example.zone.bak` 같은 백업 파일까지 걸리지 않게 하려면 `^(.*)\.zone$` 처럼 앵커를 붙입니다.[^autoplugin]

원서는 `file` 의 `transfer`·`reload`·`upstream` 을 그대로 쓸 수 있다고 적습니다. 어느 서버가 이 존들을 전송받고 NOTIFY 를 받을지, 디렉터리를 얼마나 자주 스캔할지, 외부 이름을 풀 때 어느 서버를 쓸지를 정합니다. 지금 버전의 `auto` 문법에 남은 것은 `directory` 와 `reload` 둘입니다.[^autoplugin] 전송은 2절처럼 `transfer` 플러그인으로 옮겨 갔습니다. `reload` 는 디렉터리를 다시 훑는 주기이기도 해서 기본값 1분 동안은 새 파일을 놓아도 존이 바로 생기지 않습니다.
...
> **원문 정오**: 경로도 한 글자 어긋납니다. 3절 Example 4-8 의 `auto` 는 `/etc/coredns/zones` 를 보는데 Example 4-10 은 `/etc/coredns/zone` 으로 동기화합니다. 두 예제를 그대로 합치면 `auto` 가 빈 디렉터리를 보게 되므로 둘 중 하나로 맞춰야 합니다.

이 명령은 저장소의 파일을 `/etc/coredns/zone` 으로 동기화하고 30초마다 변경을 확인합니다. 지금의 git-sync 는 확인 주기를 `--period` 로 적고, 동기화한 결과를 `--link` 로 지정한 심볼릭 링크로 노출합니다.[^gitsync] 그래서 `auto` 의 `directory` 는 그 링크 경로를 가리켜야 합니다.
...
이렇게 읽은 존은 온전한 존이 아닙니다. `SOA` 레코드가 없어서 다른 DNS 서버로 전송할 수 없습니다. 그래서 `hosts` 는 보통 존 전체를 관리하는 데 쓰지 않고 낱개 도메인 이름을 싣는 데 씁니다. 저자가 드는 용례가 광고에 쓰이는 도메인 이름을 담은 호스트 테이블을 읽어 그것들을 `0.0.0.0` 으로 보내는 것입니다.
...
많은 조직이 AWS 에서 권한 DNS 서비스를 받으려고 Route 53 을 씁니다. 그 존을 CoreDNS 가 답하게 하려면 데이터를 어떻게든 가져와야 합니다.
...
마지막 줄의 `default` 는 자격증명 파일 안의 특정 프로파일을 가리킵니다. `hosts` 처럼 `route53` 도 `fallthrough` 를 지원하고, 원서 시점에는 `file` 처럼 외부 이름을 풀 상류 서버를 지정하는 `upstream` 도 지원했습니다.

> **버전 차이**: 지금 `route53` 문법은 원서와 꽤 다릅니다. `upstream` 이 문서에서 빠졌고(파서는 `file` 처럼 아직 받아서 버립니다), `aws_endpoint` 와 가져오는 주기를 정하는 `refresh` 가 새로 붙었습니다. 무엇보다 플러그인 안에 키를 직접 적는 `aws_access_key` 는 `"Deprecated, uses other authentication methods instead"` 로 표시돼 있습니다.[^route53] 그래서 지금 자격증명을 주는 길은 환경 변수나 공유 자격증명 파일, `credentials` 로 고른 프로파일, EC2 인스턴스 역할 쪽입니다. 원서의 첫 예제 형태는 피하는 편이 맞습니다.



## 결정 치트시트

> 네 갈래 중 무엇을 고를지를 상황으로 정리했습니다. 근거는 모두 본문입니다.
...
| 원서 문법대로 `file { transfer to ... }` 를 썼더니 CoreDNS 가 안 뜬다 | 별도 `transfer` 플러그인으로 옮긴다 | 1.8.0 부터 `file`·`auto` 에 `transfer` 계열이 없다 |
...
**한 가지 실행**: `auto` 로 디렉터리 하나를 걸어 두고, CoreDNS 를 띄운 채로 `db.` 로 시작하는 파일을 하나 더 만들어 봅니다. Corefile 을 손대지 않고 존이 늘어나는지 확인하면 3절이 몸에 붙습니다. 존 파일에는 `SOA` 가 있어야 존으로 읽히므로 가장 짧은 파일도 `SOA` 한 줄은 담습니다.

```bash
mkdir -p zones
cat > zones/db.foo.example <<'ZONE'
$TTL 60
@   IN SOA ns1.foo.example. admin.foo.example. 1 1h 15m 7d 60
    IN NS  ns1.foo.example.
www IN A   10.0.0.1
ZONE
printf '. {\n    auto example {\n        directory ./zones\n        reload 10s\n    }\n    log\n}\n' > Corefile
coredns -dns.port 1053
# 다른 터미널에서 — 첫 존이 답하는지 본 뒤, 둘째 파일을 놓고 다시 묻는다
dig @127.0.0.1 -p 1053 www.foo.example
sed 's/foo/bar/g' zones/db.foo.example > zones/db.bar.example
sleep 12 && dig @127.0.0.1 -p 1053 www.bar.example
```

`reload 10s` 를 준 것은 기본값 1분을 기다리지 않으려는 것입니다. 파일을 놓기 전에 물으면 맡은 존이 없어 `REFUSED` 가 옵니다. 둘째 `dig` 가 그 대신 주소를 돌려주면 Corefile 을 손대지 않고 존이 하나 늘어난 것입니다.
...
7. 원서 3장·4장의 Corefile 을 그대로 복사해 지금 CoreDNS 로 띄우면 어디서 막히고, 어떻게 고치겠습니까?
8. `auto example` 로 걸어 둔 디렉터리에 `db.foo.test` 를 놓았습니다. 파일은 읽히는데 `www.foo.test` 질의에 답이 없습니다. 왜 그렇습니까?
...
### 정답 7 — 원서 Corefile 을 지금 띄우면

`file` 이나 `auto`, `secondary` 안의 `transfer to` 에서 막힙니다. 1.8.0 부터 이 하위 지시어가 없어져 알 수 없는 하위 지시어 오류로 CoreDNS 가 뜨지 않습니다. 나가는 전송이 필요하면 같은 서버 블록에 `transfer { to ... }` 를 따로 두고, 필요 없으면 줄을 지웁니다. `upstream` 은 받아서 버리므로 막히지는 않지만 지워 두는 편이 깔끔합니다.

### 정답 8 — 읽히는데 답이 없는 존

`foo.test` 가 `auto` 의 `ZONES` 인 `example` 아래가 아니기 때문입니다. `auto` 는 범위 밖 존도 읽기는 하지만 그 존으로 질의를 받지 않습니다. `ZONES` 에 `test` 를 더하거나 인자를 비워 서버 블록의 존을 따르게 합니다.



## 핵심 개념 체크리스트

- [ ] 네 갈래를 존 전체·전송·자동 편입 세 물음으로 갈라 표로 그릴 수 있는가?
- [ ] 파일 하나로 여러 존을 기술하는 조건과, 그러면 모든 존의 내용이 같아진다는 대가를 말할 수 있는가?
- [ ] `auto` 의 기본 파일 패턴과 정규식·역참조로 origin 을 뽑는 규칙을 댈 수 있는가?
- [ ] `hosts` 가 만드는 레코드 셋과 별칭이 `CNAME` 이 되지 않는 점을 짚을 수 있는가?
- [ ] `route53` 의 존 지정 형식을 쓰고, 지금 쓸 자격증명 경로와 원서 문법에서 달라진 자리를 댈 수 있는가?
...
[^autoplugin]: CoreDNS `auto` 플러그인 공식 문서 — "**REGEXP** is unanchored; use `^` and `$` to require matching the full", `reload` 는 "how often CoreDNS should scan the directory to watch for file removal and addition. Default is one minute." 문법 블록은 `auto [ZONES...] { directory DIR [REGEXP ORIGIN_TEMPLATE] / reload DURATION }` 이며 `upstream` 과 `transfer` 계열은 나오지 않습니다. `"Will happily pick up a zone for example.COM, except it will never be queried, because the auto directive only is authoritative for example.ORG."` <https://coredns.io/plugins/auto/> (2026-09-23 조회)

[^fileplugin]: CoreDNS `file` 플러그인 공식 문서 — 문법 블록은 `file DBFILE [ZONES... ] { reload DURATION / reload_by_mtime / fallthrough [ZONES...] }` 이며 `upstream` 과 `transfer` 계열은 나오지 않습니다. `reload` 는 "interval to perform a reload of the zone if the SOA version changes. Default is one minute", `reload_by_mtime` 은 "decision to reload the zone will be based on the zone file modification time, instead of change in the SOA serial". <https://coredns.io/plugins/file/> (2026-09-23 조회)

[^filesetup]: CoreDNS `plugin/file/setup.go` — `case "upstream":` 아래 `// remove soon` 과 `c.RemainingArgs()`. <https://raw.githubusercontent.com/coredns/coredns/master/plugin/file/setup.go> (2026-09-23 조회)

[^n180]: CoreDNS 1.8.0 릴리스 노트 — "plugins wanting to perform outgoing zone transfers now use this plugin: *file*, *auto*, *secondary* and *kubernetes* are converted." <https://raw.githubusercontent.com/coredns/coredns/master/notes/coredns-1.8.0.md>

[^route53]: CoreDNS `route53` 문서 — 문법 블록 `aws_access_key [AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY] # Deprecated, uses other authentication methods instead.`, `aws_endpoint ENDPOINT`, `credentials PROFILE [FILENAME]`, `fallthrough [ZONES...]`, `refresh DURATION`. <https://coredns.io/plugins/route53/> (2026-09-23 조회)

[^gitsync]: kubernetes/git-sync README — "One of the things in that directory is a symlink (see the `--link` flag)", "git-sync looks for changes in the remote repo periodically (see the `--period` flag)". <https://github.com/kubernetes/git-sync> (2026-09-23 조회)
