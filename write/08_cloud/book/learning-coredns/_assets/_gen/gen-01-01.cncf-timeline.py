...
# 2026-09-23 보강: 공식 발표문 날짜로 포크를 2016년 3월로 옮기고, incubating(2018-02)과
#   쿠버네티스 1.11(2018-06-27, CoreDNS GA · kubeadm 기본값)을 더했다. 1.13 에서 GA 가 된 것은
#   CoreDNS 자체가 아니라 "기본 DNS" 라는 지위다(1.13 발표 "CoreDNS as the default DNS").
#   출처: cncf.io/announcements/2019/01/24/coredns-graduation, kubernetes.io 1.11·1.13 발표.
...
# (시점, 방향, 층, 이름, 설명, 색, 반지름) — 층 2 는 이웃 라벨과 가로로 겹치는 사건을 한 단 더 띄운다
events = [
    (2015.55, "up",   1, "CNCF 설립",              "쿠버네티스 관리 목적",        INFO, 4),
    (2016.2,  "down", 1, "Caddy 를 포크",          "2016년 3월 · Miek Gieben",    INFO, 4),
    (2017.5,  "up",   1, "CNCF sandbox",           "합류 월 미상",                INFO, 4),
    (2018.1,  "up",   2, "incubating",             "2018년 2월",                  INFO, 4),
    (2018.49, "down", 1, "쿠버네티스 1.11",        "CoreDNS GA · kubeadm 기본",   INFO, 4),
    (2018.923, "up",  1, "쿠버네티스 1.13",        "기본 DNS 로 GA",              ACC,  6),
    (2019.063, "down", 2, "CNCF graduated",        "2019년 1월 24일",             INFO, 4),
]
for t, side, lv, name, sub, color, r in events:
    x = px(t)
    ink = ACC if color is ACC else INK
    off = 0 if lv == 1 else 64
    if side == "up":
        d.line(x, BASE - r, x, BASE - 56 - off, RULE, 1.0)
        d.t(x, BASE - 90 - off, name, 14, ink, KR, "middle", 600)
        d.t(x, BASE - 70 - off, sub, 13, MUTED)
    else:
        d.line(x, BASE + r, x, BASE + 64 + off, RULE, 1.0)
        d.t(x, BASE + 82 + off, name, 14, ink, KR, "middle", 600)
        d.t(x, BASE + 102 + off, sub, 13, MUTED)
