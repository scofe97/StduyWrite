...
> 이 편이 매달린 하나의 생각은 체인의 순서가 Corefile 이 아니라 `plugin.cfg` 에서 정해진다는 것입니다.
...
2. Docker 로 빌드하는 길과 워크스테이션에 Go 를 두고 빌드하는 길을 반복 횟수와 설치 부담으로 비교해 고를 수 있습니다.
3. Corefile 과 `plugin.cfg` 가 각각 무엇을 정하는지 가르고, `plugin.cfg` 한 줄의 위치가 체인의 어디에 대응하는지 말할 수 있습니다.
4. `main` 을 갈아 끼우는 길이 언제 필요한지와 그때 무엇을 얻는지 설명할 수 있습니다.
5. 자기 `main` 에서 새 `FlagSet` 과 Corefile 문자열을 쓰는 이유를 라이브러리 관점에서 설명할 수 있습니다.
...
남는 것은 바이너리에 무엇이 어떤 순서로 들어가는가입니다.
...
> **노트의 읽기**: 원서는 Go 1.12 이상을 요구합니다. 최신 릴리스 v1.14.7 의 `go.mod` 는 `go 1.25.0` 이고 master 는 이미 `go 1.26.0` 입니다.[^gomod] 일곱 해 사이에 요구 버전이 열세 단계 넘게 올라갔으니, 원서를 따라 하려면 그 시절 태그를 받아야 하고 최신을 빌드하려면 최신 Go 가 있어야 합니다.
...
빌드도 같은 이미지로 합니다. 이번에는 먼저 받아 둔 `coredns` 디렉터리로 들어가서 그곳을 마운트합니다.
...
만들어지는 것은 리눅스 바이너리입니다. 맥에서 빌드해도 그렇습니다. 빌드를 돌리는 컨테이너가 리눅스이기 때문이라고 저자들이 각주로 짚습니다. 워크스테이션 빌드와 나오는 바이너리의 구성은 같지만, 맥에서 돌릴 바이너리가 필요하다면 이 길로는 얻지 못합니다.
...
저자들이 Docker 빌드의 단점을 하나로 짚습니다. 반복 개발을 할 때 정말 느리다는 것입니다. 매번 새 컨테이너에서 시작해 패키지를 전부 다시 내려받기 때문입니다. 중간 이미지를 저장해 두는 방법도 있지만 길게 보면 그럴 만한 가치가 없다고 저자들이 각주로 덧붙입니다.
...
파일 머리의 주석이 다시 빌드하는 법부터 적습니다. 목록을 고친 뒤 `go gen && go build` 를 돌리라는 것입니다. 그런데 `go` 명령에는 `gen` 이라는 하위 명령이 없어 그대로 치면 실패합니다. 뜻한 것은 `go generate` 이고, 지금 파일은 그렇게 고쳐져 있습니다. 이 단계가 `plugin.cfg` 를 읽어 지시자 순서를 담은 Go 소스를 새로 만듭니다. 그다음 `go build` 가 그 소스로 바이너리를 짓습니다. `Makefile` 도 빌드 전에 `go generate coredns.go` 를 부릅니다.[^plugincfg]

그다음 주석이 스스로 경고를 답니다. 지시자는 실행돼야 할 순서로 등록되고, 순서가 아주 중요하다는 것입니다. 요청이 지나갈 때 모든 플러그인은 자기 아래의 모든 플러그인에게 영향을 받지만 자기 위의 플러그인이 무엇을 하는지는 신경 쓰지 않아야 한다고 적습니다.
...
> **노트의 읽기**: 지금 master 의 `plugin.cfg` 는 84줄이고 그중 플러그인 엔트리가 62개입니다.[^plugincfg] 81번째 줄이 `on:github.com/coredns/caddy/onevent` 인데, 원서가 실은 같은 줄은 `on:github.com/mholt/caddy/onevent` 였습니다. CoreDNS 가 Caddy 를 자기 조직으로 포크해 간 것이고, 이 변화는 9장 후반부의 코드 예제 전체에 걸립니다.
...
`any:github.com/coredns/any` 를 `cancel:cancel` 바로 다음 줄에 넣고 다시 빌드한 뒤 `./coredns -plugins` 를 실행하면 목록 맨 위에 `dns.any` 가 보입니다. 알파벳 순서로 나오기 때문입니다. 그래서 이 목록으로는 들어갔는지만 확인할 수 있고, 체인에서 몇 번째인지는 알 수 없습니다.
...
이 Corefile 로 서버를 띄우고 원서 Example 9-8 처럼 ANY 질의를 보냅니다.

```bash
dig -p 5300 -t ANY example.com @localhost
```

RFC 를 가리키는 `HINFO` 레코드 하나가 돌아옵니다.
...
이 실험은 운영에서 진단하는 법도 바꿉니다. Corefile 만 보고 "이 플러그인이 저것보다 먼저 도니까" 라고 추론하면 틀립니다. 실제 순서는 그 바이너리를 만든 `plugin.cfg` 에 있는데, 이미 만들어진 바이너리에서는 그 파일이 손에 없고 `-plugins` 목록은 알파벳 순서입니다. 남의 빌드를 쓴다면 그 빌드의 `plugin.cfg` 를 함께 받아 두어야 합니다.

> **노트의 읽기**: 저자들은 각주에 `any` 가 v1.5.1 에서 이미 인트리로 들어갔고, 그래서 이 절차는 v1.5.0 에 맞춘 것이라고 적습니다. 지금 `plugin.cfg` 에서는 50번째 줄이 `any:any` 이고 `forward:forward` 는 77번째 줄입니다.[^plugincfg] 원서가 넣은 `cancel:cancel` 다음보다는 한참 아래지만 `forward` 보다는 위라서, 기본 빌드에서도 `any` 가 ANY 질의를 먼저 받습니다. 이 실습을 그대로 재현하려면 원서처럼 v1.5.0 태그를 받아야 합니다. 실습의 요점인 줄 위치를 옮겨 보는 절차는 지금 파일에서도 그대로 됩니다. 현재 `-plugins` 출력에는 `dns.` 접두사가 붙지 않아 `any` 로만 보입니다.[^caddyplugins]
...
실제 사례가 하나 있습니다. 쿠버네티스의 Node-Local DNS Cache 가 노드마다 작은 전용 DNS 서버를 두는데, 그 서버가 CoreDNS 이면서 이 용례에 필요한 고루틴을 띄우는 자기 `main` 루틴으로 돕니다. 원서는 이 기능을 곧 나올 기능이라 부르고 각주에 1.13 에서 알파, 1.15 에서 베타였다고 적습니다. 지금은 1.18 에서 안정 단계가 된 기능입니다.[^nodelocal]

`plugin.cfg` 를 줄이는 길과 `main` 을 갈아 끼우는 길은 목적이 다릅니다. 앞엣것은 바이너리를 줄이는 일이고 표준 `main` 을 그대로 씁니다. 뒤엣것은 초기화에 내 코드를 끼워 넣는 일입니다. 바이너리 크기만 목적이라면 `plugin.cfg` 로 충분하고, 표준 초기화 밖의 일이 필요할 때만 한 층 더 갑니다.
...
`main.go` 는 서른여덟 줄이 전부입니다. `main` 의 흐름은 넷입니다.

1. 플래그를 파싱해 구조체를 만듭니다.
2. 그 구조체로 Corefile 을 메모리에 만듭니다.
3. 그 Corefile 로 서버를 띄웁니다.
4. 서버가 끝나기를 기다립니다. 특별한 시그널 처리를 넣지 않는 한 이 끝은 영영 오지 않습니다.
...
> **노트의 읽기**: 원서 예제는 `github.com/mholt/caddy` 를 import 하고, 저자들은 각주에 이후 버전의 Caddy 가 `github.com/caddyserver/caddy` 로 옮겼다고 적습니다. CoreDNS 가 실제로 간 곳은 그쪽이 아닙니다. 현재 `go.mod` 는 `github.com/coredns/caddy` 를 씁니다.[^gomod] Caddy 본류를 따라가는 대신 자기 조직으로 포크한 것이라, 원서의 import 경로를 그대로 쓰면 지금은 빌드되지 않습니다. 포크를 자기들이 관리하므로 필요하면 릴리스 태그 없이 커밋을 직접 물릴 수도 있습니다. v1.14.7 의 `go.mod` 가 `v1.1.4-0.20250930002214-...` 같은 의사 버전을 물렸고, 지금 master 는 정식 태그 `v1.1.4` 로 돌아와 있습니다.[^gomod]
...
낯선 쪽은 재빌드입니다. Spring 에서 기능을 켜는 일은 대개 의존성을 하나 넣고 프로퍼티를 적는 일입니다. 자동 설정이 클래스패스를 보고 붙습니다. CoreDNS 에는 그 층이 없습니다. 클래스패스에 해당하는 것이 `plugin.cfg` 이고, 그것을 바꾸면 반드시 다시 빌드해야 합니다. 저자들이 첫 문단에서 적은 대로 플러그인이 동적으로 적재되지 않기 때문입니다.
...
5. 사내 빌드에 넣은 외부 플러그인이 Corefile 에서는 `forward` 보다 위에 적혀 있는데 질의를 한 번도 받지 못합니다. 무엇부터 확인하겠습니까? `./coredns -plugins` 출력은 도움이 됩니까?
...
### 정답 5 — 그 빌드의 plugin.cfg

Corefile 의 순서는 체인 순서가 아니므로, 그 바이너리를 만든 `plugin.cfg` 에서 그 플러그인 줄이 `forward` 보다 아래에 있는지부터 봅니다. 아래에 있다면 `forward` 가 먼저 질의를 가져가 답하므로 그 플러그인은 요청을 보지 못합니다. `-plugins` 출력은 바이너리에 들어갔는지만 알려 줍니다. 목록이 알파벳 순서라 체인에서의 자리는 알 수 없습니다.
...
- [ ] 플러그인이 동적으로 적재되지 않는다는 사실에서 재빌드와 고정된 순서가 함께 나온다는 것을 설명할 수 있는가?
- [ ] Docker 빌드와 워크스테이션 빌드를 반복 횟수와 설치 부담으로 비교하고, Docker 빌드에서 `-u` 를 붙이는 자리와 떼는 자리를 말할 수 있는가?
- [ ] Corefile 과 `plugin.cfg` 가 각각 정하는 것을 가르고, `any` 실험에서 줄 하나를 옮기면 왜 응답이 바뀌는지 말할 수 있는가?
- [ ] `main` 을 갈아 끼우는 길이 `plugin.cfg` 를 줄이는 길과 목적이 어떻게 다른지 설명할 수 있는가?
- [ ] 자기 `main` 에서 새 `flag.FlagSet` 을 만들고 Corefile 을 문자열로 짓는 이유, 그리고 `ServerTypeName` 이 `"dns"` 여야 하는 이유를 말할 수 있는가?
...
[^gomod]: CoreDNS `go.mod` — v1.14.7 태그는 `go 1.25.0` 과 `github.com/coredns/caddy v1.1.4-0.20250930002214-15135a999495`, master 는 `go 1.26.0` 과 `github.com/coredns/caddy v1.1.4`(2026-09-26 조회). <https://github.com/coredns/coredns/blob/master/go.mod>
[^plugincfg]: CoreDNS `plugin.cfg` master(2026-09-26 조회) — 파일 84줄에 플러그인 엔트리 62개. `any:any` 는 50번째 줄, `forward:forward` 는 77번째 줄, `on:github.com/coredns/caddy/onevent` 는 81번째 줄. 머리 주석 ``"run `go generate && go build`"``, `Makefile` 의 `go generate coredns.go`. <https://github.com/coredns/coredns/blob/master/plugin.cfg>
...
[^caddyplugins]: coredns/caddy `plugins.go` — `str += name[4:] + "\n" // drop dns. prefix caddy adds`. <https://github.com/coredns/caddy/blob/master/plugins.go>
[^nodelocal]: Kubernetes 문서 「Using NodeLocal DNSCache in Kubernetes Clusters」 — `feature-state for_k8s_version="v1.18" state="stable"`. <https://kubernetes.io/docs/tasks/administer-cluster/nodelocaldns/>
