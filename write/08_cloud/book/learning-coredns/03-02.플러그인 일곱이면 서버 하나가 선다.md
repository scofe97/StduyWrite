...
- **순서를 정하는 것은 등록 순서이고, 방향이 아래쪽입니다.** `plugin.cfg` 의 머리말은 지시어가 실행될 순서대로 등록되며 그 순서가 매우 중요하다고 적습니다. 이어서 모든 플러그인이 요청 처리 중에 자기 **아래** 플러그인들의 영향을 받되, 자기 위의 플러그인이 무엇을 하는지는 신경 쓰지 않아도 된다고 못 박습니다.[^plugincfg] 방향성까지 적은 문장인데 원서 본문은 순서가 고정이라는 사실까지만 말합니다.
