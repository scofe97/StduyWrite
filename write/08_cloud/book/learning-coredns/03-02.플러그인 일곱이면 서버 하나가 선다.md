...
`except` 는 `FROM` 에 지정한 도메인의 서브도메인 가운데 넘기지 **않을** 것을 지정합니다. `max_fails` 는 죽었다고 보기까지 필요한 연속 실패 횟수로 기본값이 2이고, `0` 으로 두면 절대 죽었다고 표시하지 않습니다. `health_check` 는 확인 주기이며 기본값이 0.5초입니다. `expire` 는 재사용하려고 들고 있던 TCP·TLS 연결을 언제 버릴지 정하며 기본값이 10초입니다.
