...
표의 낯선 이름 셋만 짧게 풀어 둡니다. 동적 업데이트는 존 파일을 손으로 고치지 않고 DNS 메시지로 레코드를 더하고 지우는 기능입니다. DNSSEC은 응답에 서명을 붙여 위조를 가려내게 하는 확장입니다. DoT는 DNS 질의를 TLS 연결 안에 실어 엿보기를 막는 전송 방식입니다.
