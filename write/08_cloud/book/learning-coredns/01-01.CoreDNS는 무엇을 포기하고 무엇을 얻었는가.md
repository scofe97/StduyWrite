...
연표에는 원서에 없는 점도 둘 넣었습니다. 2018년 2월의 incubating 승격과 2018년 6월의 쿠버네티스 1.11입니다. CoreDNS 자체가 GA가 되고 kubeadm의 기본값이 된 것은 1.11이고, 1.13은 그것을 기본 DNS로 올린 릴리스입니다(심화 학습). 연표에서 눈여겨볼 것은 마지막 두 점의 간격입니다. 쿠버네티스 1.13 릴리스와 CNCF 졸업 발표 사이는 52일입니다. 도식의 그 좁은 간격은 축소한 것이 아니라 실제 간격입니다.
...
- 기본 빌드에는 완전 재귀가 지금도 없습니다. CoreDNS 공식 플러그인 목록에 루트부터 시작하는 재귀 해석기는 없고, `forward` 플러그인은 상류 리졸버로 DNS 메시지를 프록시한다고만 적혀 있습니다.[^plugins] 예외는 외부 플러그인입니다. CoreDNS가 직접 관리하는 `unbound` 플러그인은 `"perform recursive queries using libunbound"` 라고 적으며, 이것을 넣어 다시 빌드하면 재귀가 됩니다.[^unbound] 표 1-1은 기본 빌드 기준으로 읽는 것이 맞습니다.
