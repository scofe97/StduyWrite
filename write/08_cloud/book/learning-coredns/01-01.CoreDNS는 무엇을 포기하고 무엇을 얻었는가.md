...
source:
  ...
  - https://bind9.readthedocs.io/en/v9.18.0/notes.html
related:
  - ./README.md
...
> CoreDNS는 전통 DNS 서버의 기능을 다 갖추는 대신 컨테이너 오케스트레이터와 말이 통하는 쪽을 골랐습니다. 이 노트는 원서 1장을 그 교환의 대차대조표로 읽습니다.

## 핵심 요약

> 이 노트가 1장에서 읽어 낸 하나의 생각은 CoreDNS가 DNS 서버를 더 잘 만든 결과가 아니라, 컨테이너 환경의 서비스 디스커버리를 풀려고 기능을 덜어낸 서버라는 것입니다.

이 노트는 그 덜어냄을 세 걸음으로 다시 세웠습니다. 원서는 서버의 계보부터 열지만, 여기서는 그 서버가 풀려는 문제부터 놓습니다. 컨테이너가 부하에 따라 뜨고 지는 환경에서는 "지금 떠 있는 목록"을 호출자가 미리 들고 있을 수 없습니다. 그다음 Miek Gieben이 Caddy를 포크해 얻은 세 가지가 그 문제에 어떻게 맞아떨어지는지 봅니다. 마지막으로 완전 재귀를 포기했다는 사실을 저자가 BIND(Berkeley Internet Name Domain, 가장 오래 쓰여 온 DNS 서버 구현)와의 대조표로 못 박습니다.

값을 매기면 이렇습니다. 얻은 것은 etcd나 쿠버네티스 같은 오케스트레이터와 직접 통신하는 능력입니다. 내준 것은 완전 재귀와 동적 업데이트입니다. 원서는 이것을 설계상의 거래라고 부르지 않고 집필 시점의 한계로 적습니다. "교환"이라는 틀은 이 노트의 읽기입니다. 쿠버네티스 1.13이 CoreDNS를 기본 DNS로 삼았고, 저자는 그 뒤로 설치 기반이 폭발하리라 내다봅니다.
...
- DNS 프로토콜 기초와 재귀의 상세 → 원서 2장, 그리고 [네트워크 로드맵 1단계 DNS resolver](../../../roadmap/network-roadmap.md#%ED%95%99%EC%8A%B5-%EC%88%9C%EC%84%9C)
...
- 질의를 보내는 쪽의 동작, 곧 `resolv.conf`·ndots·search domain → [명세 밖으로 나가면 이식성을 내준다](./06-04.%EB%AA%85%EC%84%B8%20%EB%B0%96%EC%9C%BC%EB%A1%9C%20%EB%82%98%EA%B0%80%EB%A9%B4%20%EC%9D%B4%EC%8B%9D%EC%84%B1%EC%9D%84%20%EB%82%B4%EC%A4%80%EB%8B%A4.md)

남는 것은 저자가 왜 이런 서버를 만들었다고 말하는가입니다. 기능 목록이 아니라 기능을 덜어낸 판단과 그 대가가 이 노트의 몫입니다.
...
CoreDNS가 빛나는 자리가 바로 여기라고 저자는 못 박습니다. 유연하고 안전한 DNS 서버라는 성질에 더해, 쿠버네티스를 포함한 여러 컨테이너 오케스트레이션 시스템과 직접 통합됩니다. 저자가 아마도 가장 큰 이점일 것이라며 꼽는 것도 etcd나 쿠버네티스 같은 컨테이너 기반 시설 및 오케스트레이션 시스템과 통신하는 능력입니다.
...
원서는 "더 빠르고 더 안전하다"고만 적고 이유를 풀지 않습니다. 노트는 이것을 두 갈래로 읽습니다. 실행되지 않는 코드는 질의마다 거치는 단계를 늘리지 않으니 속도가 붙습니다. 실행되지 않는 코드는 공격 표면, 곧 바깥 입력이 닿아 악용될 수 있는 코드 경로가 되지 않으니 안전해집니다. 기능을 켜는 만큼만 위험을 지는 구조입니다.
...
완전 재귀는 세 걸음으로 이뤄집니다. DNS 이름 공간의 루트에서 시작해 루트 DNS 서버에 질의합니다. 돌아온 참조를 따라 다음 서버로 내려갑니다. 권한 있는 DNS 서버 가운데 하나가 답을 줄 때까지 그 내려감을 반복합니다. CoreDNS는 이 일을 스스로 하지 못합니다. 대신 그 일을 대신해 줄 다른 DNS 서버에 의존하며 이 서버를 보통 포워더라 부릅니다. 재귀와 포워더의 자세한 이야기는 원서 2장, 이 노트로는 [위임이 그은 경계가 질의 경로를 정한다 §6](./02-01.%EC%9C%84%EC%9E%84%EC%9D%B4%20%EA%B7%B8%EC%9D%80%20%EA%B2%BD%EA%B3%84%EA%B0%80%20%EC%A7%88%EC%9D%98%20%EA%B2%BD%EB%A1%9C%EB%A5%BC%20%EC%A0%95%ED%95%9C%EB%8B%A4.md)이 맡습니다.

CoreDNS가 자기 몫으로 남기는 것은 질의를 받고 넘기고 되돌려 주는 일입니다. 도식의 붉은 칩이 CoreDNS가 스스로 못 하는 일을 가리키고, 그 일은 포워더 레인에서 오른쪽으로 뻗는 화살표들이 대신합니다.
...
표를 어느 쪽이 앞서느냐로 갈라 읽으면 성격이 드러납니다. BIND가 앞서는 줄은 셋입니다. 완전 재귀, 동적 업데이트, DNSSEC처럼 전통 DNS 서버가 오래 갖춰 온 기능입니다. CoreDNS가 앞서는 줄도 셋이며 쿠버네티스·Route 53 연동과 DoT, 곧 컨테이너와 클라우드 쪽 기능입니다. 노트가 이 장을 교환으로 읽는 근거가 바로 이 표입니다.

표의 낯선 이름 셋만 짧게 풀어 둡니다. 동적 업데이트는 존 파일을 손으로 고치지 않고 DNS 메시지로 레코드를 더하고 지우는 기능입니다. DNSSEC(Domain Name System Security Extensions)은 응답에 서명을 붙여 위조를 가려내게 하는 확장입니다. DoT는 DNS 질의를 TLS 연결 안에 실어 엿보기를 막는 전송 방식입니다.

나머지 낯선 용어도 걱정하지 말라고 저자는 덧붙입니다. 이후 장에서 모두 다룬다는 예고입니다.

표 1-1 은 2019년의 표라서 지금 공식 자료와 대조하면 세 줄이 달리 읽힙니다.

기본 빌드에는 완전 재귀가 지금도 없습니다. CoreDNS 공식 플러그인 목록에 루트부터 시작하는 재귀 해석기는 없습니다. `forward` 플러그인은 상류 리졸버로 DNS 메시지를 프록시한다고만 적혀 있습니다.[^plugins] 예외는 외부 플러그인입니다. CoreDNS가 직접 관리하는 `unbound` 플러그인은 `"perform recursive queries using libunbound"` 라고 적으며, 이것을 넣어 다시 빌드하면 재귀가 됩니다.[^unbound] 표 1-1은 기본 빌드 기준으로 읽는 것이 맞습니다.

암호화 전송은 양쪽 모두 넓어졌습니다. CoreDNS에는 DoT 말고도 `https`(DoH)와 `https3`(DoH3), `quic` 플러그인이 생겼습니다.[^plugins] BIND도 2022년의 9.18부터 `"Support for DNS over TLS (DoT) has been added to named"` 이고 DoH도 함께 지원합니다.[^bind918] 그래서 DoT 줄의 "BIND 없음"은 지금은 맞지 않습니다.

DNSSEC이 제한적인 이유는 기본 빌드에 서명 쪽만 있고 검증 쪽이 없다는 데 있습니다. `dnssec` 플러그인은 서빙하는 데이터에 즉석 서명을 붙이고, 뒤에 생긴 `sign` 플러그인은 `"sign adds DNSSEC records to zone files"` 처럼 존 파일에 미리 서명합니다.[^plugins] 둘 다 서명기이지 검증기가 아닙니다. 검증은 앞의 외부 `unbound` 플러그인이 `"Unbound uses DNSSEC by default when resolving"` 이라고 적는 쪽에서 얻을 수 있습니다.[^unbound]
...
CoreDNS를 쓰기로 정했다 해도 한 가지가 남습니다. 클러스터의 이름 해석을 통째로 맡겨도 되는지를 무엇을 보고 판단할까요? 저자는 이 절을 CNCF와의 공식 관계로 소개하고, CoreDNS가 쿠버네티스에 얼마나 결정적인지 보여 주는 증거로 기본값 채택을 듭니다. 노트는 그것을 성능 수치가 아니라 소속과 기본값이라는 판단 근거로 읽습니다.
...
CNCF는 클라우드 기반 애플리케이션을 짓는 데 중요한 여러 기술의 집이 됐습니다. 메트릭 수집과 알림을 맡는 프로메테우스, 서비스 프록시인 Envoy가 그런 예입니다. 저자가 이 둘을 든 것은 CNCF가 어떤 성격의 집인지 보이려는 것이라, 이 장의 논지와는 별개로 읽어도 됩니다. CNCF가 관리하는 프로젝트는 성숙도 단계를 밟습니다. 초기 단계 프로젝트를 위한 sandbox, 수용이 늘고 있는 프로젝트를 위한 incubating, 넓게 채택할 만큼 성숙한 프로젝트를 위한 graduated 순서입니다.
...
연표에는 원서에 없는 점도 둘 넣었습니다. 2018년 2월의 incubating 승격과 2018년 6월의 쿠버네티스 1.11입니다. CoreDNS 자체가 GA가 되고 kubeadm의 기본값이 된 것은 1.11이고, 1.13은 그것을 기본 DNS로 올린 릴리스입니다. 연표에서 눈여겨볼 것은 마지막 두 점의 간격입니다. 쿠버네티스 1.13 릴리스와 CNCF 졸업 발표 사이는 52일입니다. 도식의 그 좁은 간격은 축소한 것이 아니라 실제 간격입니다.

쿠버네티스 쪽은 두 단계였습니다. 2018년 6월 1.11 발표문에서 `"CoreDNS Promoted to General Availability"` 가 먼저 나오고, 이때 CoreDNS는 kubeadm의 기본값이 됐습니다.[^k8s111] 1.13에서 GA로 졸업한 것은 CoreDNS 자체가 아니라 `"CoreDNS as the default DNS"`, 곧 기본 DNS라는 지위입니다.[^k8s113]
...
> 1장만으로도 가를 수 있는 판단을 모았습니다. 근거는 표 1-1과 본문이며, DNSSEC 행만 5절에서 대조한 플러그인 문서를 함께 씁니다.
...
| DNSSEC 검증까지 필요하다 | CoreDNS 단독으로는 부족 | 기본 빌드에는 서명 플러그인만 있고 검증기가 없음 |
| 설정을 짧게 유지하고 필요한 기능만 켜고 싶다 | CoreDNS | 안 켠 플러그인은 코드가 실행되지 않음 |
| 인터넷에 열리는 서버에서 메모리 접근 오류가 걱정된다 | 그 부류만 놓고 보면 CoreDNS | 언어가 그 부류를 막아 줌. 동적 업데이트·DNSSEC 요구가 있으면 위 행이 먼저 |
...
Spring 애플리케이션 쪽에서 보면 이 장은 이름 해석의 서버 편을 설명합니다. 클러스터 안의 Spring 앱이 다른 서비스를 호출할 때 그 이름을 IP로 바꾸는 주체가 CoreDNS입니다. 이 장은 그 주체가 왜 정적 목록이 아니라 오케스트레이터와 붙어 있는 서버인지를 말합니다. 반대편인 클라이언트 동작, 곧 `resolv.conf`와 ndots와 JVM DNS 캐시는 [네트워크 로드맵 1단계](../../../roadmap/network-roadmap.md#%ED%95%99%EC%8A%B5-%EC%88%9C%EC%84%9C)가 맡습니다.
...
CoreDNS에서 필요 없는 플러그인은 설정하지 않습니다. 설정하지 않은 플러그인의 코드는 실행되지 않습니다. 원서는 그래서 더 빠르고 더 안전하다고만 적습니다. 풀어 읽으면 실행되지 않는 코드는 질의 처리 단계를 늘리지 않고, 바깥 입력이 닿는 공격 표면도 되지 않습니다.

### 정답 4 — 완전 재귀 미지원

루트 DNS 서버부터 시작해 권한 서버에 닿을 때까지 참조를 따라가는 일을 CoreDNS가 스스로 하지 못한다는 뜻입니다. 그래서 그 일을 대신할 포워더를 따로 두어야 합니다. 쿠버네티스 안이라면 클러스터 바깥 이름이 이 경로를 탑니다(§5 노트의 읽기).
...
무엇을 못 해도 되는지로 가릅니다. 완전 재귀·동적 업데이트·DNSSEC 검증이 필요하면 BIND 쪽이고, 쿠버네티스나 Route 53 연동이 필요하면 CoreDNS입니다. 원서 표는 DoT도 CoreDNS 쪽에 두었지만 BIND 9.18부터는 양쪽 모두 지원하므로 지금은 가르는 기준이 되지 못합니다.
...
- [ ] 플러그인을 켜지 않는 것이 왜 속도와 보안 양쪽에 걸리는지 설명할 수 있는가?
- [ ] 완전 재귀의 정의를 루트와 참조와 권한 서버라는 말로, 포워더가 끼는 질의 경로로 설명할 수 있는가?
- [ ] 표 1-1의 여섯 항목을 BIND가 앞서는 셋과 CoreDNS가 앞서는 셋으로 갈라, 그것을 근거로 둘 중 하나를 고를 수 있는가?
...
- [질문과 답이 어긋나면 클라이언트가 버린다 §9](./07-01.%EC%A7%88%EB%AC%B8%EA%B3%BC%20%EB%8B%B5%EC%9D%B4%20%EC%96%B4%EA%B8%8B%EB%82%98%EB%A9%B4%20%ED%81%B4%EB%9D%BC%EC%9D%B4%EC%96%B8%ED%8A%B8%EA%B0%80%20%EB%B2%84%EB%A6%B0%EB%8B%A4.md) — 필터링 DNS 로 이름 해석을 막는 가정용 사례(원서 밖)
- [OS 네트워크 딥다이브 로드맵](../../../roadmap/network-roadmap.md) — resolver 클라이언트 측 범위의 SSOT
...
[^plugins]: CoreDNS 공식 플러그인 목록 — `forward`는 상류 리졸버 프록시, `dnssec`은 즉석 서명, `sign`은 존 파일 서명("sign adds DNSSEC records to zone files", <https://coredns.io/plugins/sign/>), `https`·`https3`·`quic`은 암호화 전송 서버 옵션. <https://coredns.io/plugins/> (2026-09-02 조회, `sign` 은 2026-09-23 조회)
...
[^unbound]: CoreDNS 외부 플러그인 `unbound` — "unbound - perform recursive queries using libunbound", "Maintained by CoreDNS", "Unbound uses DNSSEC by default when resolving". <https://coredns.io/explugins/unbound/> (2026-09-23 조회)

[^bind918]: BIND 9.18.0 릴리스 노트 — "Support for DNS over TLS (DoT) has been added to named", "TLS is used by both DNS over TLS (DoT) and DNS over HTTPS (DoH)." <https://bind9.readthedocs.io/en/v9.18.0/notes.html> (2026-09-23 조회)

[^k8s111]: Kubernetes 1.11 릴리스 발표 — "CoreDNS Promoted to General Availability", "CoreDNS is now available as a cluster DNS add-on option, and is the default when using kubeadm." <https://kubernetes.io/blog/2018/06/27/kubernetes-1.11-release-announcement/> (2026-09-23 조회)
