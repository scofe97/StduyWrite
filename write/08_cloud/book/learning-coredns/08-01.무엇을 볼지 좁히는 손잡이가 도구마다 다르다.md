...
> 이 편이 매달린 하나의 생각은 관측 도구를 고르는 일이 곧 그 값을 깎을 손잡이를 고르는 일이라는 것입니다.
...
`prometheus` 의 손잡이는 CoreDNS 밖, 곧 Prometheus 의 수집 규칙에 있습니다. `debug` 는 안에도 밖에도 조절할 자리가 없고 켜고 끄는 것이 전부입니다. 원서가 이 장에서 유일하게 상자를 쳐서 운영에 쓰지 말라고 경고하는 것도 이 플러그인인데, 그 이유는 손잡이가 없어서가 아니라 크래시 복구를 끄기 때문입니다.

일곱 해가 지나 가장 크게 달라진 곳은 `prometheus` 입니다. 원서 표 8-1 의 CoreDNS 지표 열 중 넷이 1.7.0 에서 이름을 바꿨고 하나는 다른 지표의 라벨로 흡수돼 없어졌습니다. 그 다섯이 하필 카운터 계열 전부입니다. 그 표를 보고 요청 수나 응답 코드 대시보드를 만들면 아무 값도 그려지지 않습니다.
...
1. 지표와 로그가 같은 질의를 각각 무엇으로 바꾸는지 설명하고, 원서 표 8-1 의 지표 이름을 지금 쓰면 무슨 일이 생기는지 말할 수 있습니다.
2. 기본 로그 한 줄에서 요청과 응답을 가르고, `log` 를 좁히는 손잡이 셋을 목적에 맞게 고를 수 있습니다.
3. `log` 대신 `dnstap` 을 쓸 이유를 형식과 값으로 설명할 수 있습니다.
4. `errors` 의 `consolidate` 정규식을 얼마나 넓게 잡을지 진단 가능성과 맞바꿔 판단할 수 있습니다.
...
> 이 편은 원서 8장 전체입니다. 추출 분량이 3,353단어로, 1장(1,389)과 4장(2,492) 다음으로 짧습니다.
...
남는 것은 각 도구가 무엇을 보여 주고 그 값을 어디서 깎는가입니다.
...
`ADDRESS` 는 listen 할 IP 와 포트이고, 생략하면 `localhost:9153` 입니다. 플러그인은 `/metrics` 핸들러를 가진 HTTP 서버를 띄웁니다. 다른 경로로 오면 404 를 돌려줍니다.

지표가 값을 아끼는 방식은 집계입니다. 질의 한 건이 로그에서는 줄 하나가 되지만 지표에서는 이미 있던 카운터 하나가 1 올라갈 뿐입니다. 저장되는 것은 사건이 아니라 사건의 수입니다.

다만 공짜는 아닙니다. 카운터가 라벨 조합마다 따로 생기기 때문입니다. `coredns_dns_requests_total` 하나가 `server`·`zone`·`view`·`proto`·`family`·`type` 여섯 라벨을 달고 있습니다. 존이 많아지면 시계열 수가 그 수만큼 곱해집니다. `type` 은 그나마 상한이 있습니다. 흔한 유형 열여덟 가지만 제 이름으로 두고 나머지는 `other` 하나로 묶기 때문입니다.[^metrics]

이 값을 깎는 손잡이는 CoreDNS 쪽에 없습니다. `prometheus` 플러그인이 받는 것은 주소와 `runtime_metrics` 뿐이고, 뒤엣것은 값을 더하는 쪽입니다. 라벨을 줄이려면 CoreDNS 밖에서, 곧 Prometheus 의 수집 규칙에서 걸러야 합니다.

### 원서의 이름표를 그대로 쓰면 안 됩니다

표 8-1 은 `process_`·`go_` 로 시작하는 프로세스와 Go 런타임 지표를 서른다섯 줄 싣고, 그다음 CoreDNS 고유 지표 열 줄을 싣습니다. 그 열 줄 중 다섯이 원서의 이름으로는 더 이상 없습니다. 넷은 1.7.0 에서 개명됐고 하나는 라벨로 흡수됐습니다.[^n170] 하필 대시보드와 경보가 실제로 세는 카운터 계열이 전부 그 다섯에 들어 있습니다.

| 원서 표 8-1 | 현재 | 무슨 일이 있었나 |
|------------|------|----------------|
| `coredns_dns_request_count_total` | `coredns_dns_requests_total` | 개명 · `view`·`type` 라벨 추가 |
| `coredns_dns_request_type_count_total` | 없어짐 | `type` 이 위 지표의 라벨로 흡수 |
| `coredns_dns_request_do_count_total` | `coredns_dns_do_requests_total` | 개명 · `view` 라벨 추가 |
| `coredns_dns_response_rcode_count_total` | `coredns_dns_responses_total` | 개명 · `view`·`plugin` 라벨 추가 |
| `coredns_panic_count_total` | `coredns_panics_total` | 개명 |
| `coredns_dns_request_duration_seconds` | 그대로 | `view` 라벨 추가 · 문서는 `type` 도 적지만 코드에는 없음 |
...
> **노트의 읽기**: 표 8-1 에서 살아남은 지표에는 원서에 없던 `view` 라벨이 붙었고, `coredns_dns_https_responses_total` 과 `coredns_dns_quic_responses_total` 이 새로 생겼습니다.[^metrics] 문서와 코드가 한 자리에서 갈립니다. README 는 `request_duration_seconds` 의 라벨을 `server, zone, view, type` 으로 적지만, `plugin/metrics/vars/vars.go` 가 선언하는 라벨은 `server`·`zone`·`view` 셋입니다.[^vars] 대시보드에서 이 히스토그램을 유형별로 가르려 하면 빈 결과를 받습니다. 1장에서 본 DoH·DoQ 지원이 지표에도 자리를 잡은 것입니다. 문법에도 블록이 생겨 `runtime_metrics` 를 켜면 Go 런타임 지표 전체가 더 붙습니다. 스칼라가 대략 백 개, 히스토그램이 여덟 개 늘고, 한 서버 블록에서 켜면 프로세스 전체에 걸려 재시작 전까지 리로드해도 유지된다고 문서가 적습니다.[^metrics]

지표 개명은 대시보드만 깨지 않습니다. `coredns_dns_response_rcode_count_total` 로 `SERVFAIL` 비율을 재던 경보 규칙은 지표가 사라지면 값을 못 구하고, 값을 못 구하는 규칙은 대개 조용히 발화하지 않습니다. 무언가 시끄럽게 깨지는 대신 감시가 조용히 없어지는 종류의 변화입니다.

흡수된 `type` 은 질의어도 바꿉니다. 원서 시절에는 유형별 요청 수를 `coredns_dns_request_type_count_total` 이라는 별도 지표에서 읽었고, 지금은 `coredns_dns_requests_total` 에 붙은 `type` 라벨로 거릅니다. 같은 질문을 묻는 일이 지표 이름을 고르는 일에서 라벨을 거르는 일로 옮겨 갔습니다.
...
읽는 법이 절반씩 갈립니다. 큰따옴표 안이 요청이고 그 뒤가 응답입니다.
...
출발지부터 처리 시간까지 표의 뒤쪽 열 행은 순서가 임의가 아니라 Common Log Format 이라는 기본 형식이 정한 것입니다. 형식 문자열로는 자리표시자 열넷이고, 표는 `{remote}:{port}` 와 `{type} {class} {name}` 처럼 붙어 찍히는 것을 한 행으로 묶었습니다. 앞의 두 칸은 형식 문자열에 없습니다.

```bash
{remote}:{port} - {>id} "{type} {class} {name} {proto} {size} {>do} {>bufsize}" {rcode} {>rflags} {rsize} {duration}
```

> **노트의 읽기**: 앞의 두 칸 중 시각은 지금 기본 출력에 없습니다. 원서 시절 CoreDNS 의 로그 패키지는 줄마다 RFC 3339 시각을 직접 찍었지만, 지금은 표준 로거의 플래그를 0 으로 두어 `[INFO]` 같은 심각도만 붙고 시각은 붙지 않습니다.[^pkglog] 현재 문서의 예시 줄도 `[INFO] [::1]:50759 - ...` 로 시작합니다.[^log] 시각은 컨테이너 런타임이나 로그 수집기가 붙이는 몫이 됐습니다.
...
둘째는 어떤 결과를 볼 것인가입니다. 응답의 종류로 거를 수 있고, 종류는 넷입니다.
...
셋째는 한 줄에 무엇을 적을 것인가입니다. 형식 문자열을 직접 줄 수 있고, 쓸 수 있는 칸은 열여덟입니다.
...
> **노트의 읽기**: 이 플러그인은 원서 이후 크게 자랐습니다.[^dnstap] `identity`·`version`·`extra`·`skipverify` 를 블록으로 받고, `tls://` 스킴이 생겼으며, `dnstap listen SOCKET` 으로 받는 쪽이 될 수도 있습니다. `extra` 는 7장에서 본 metadata 를 넣는 자리라고 문서가 직접 적습니다. 값을 깎는 쪽 사실도 하나 적혀 있습니다. 플러그인의 버퍼는 메시지 1만 개이고 그 위로 넘치는 메시지는 버리며, 버렸다는 사실은 로그에 남긴다는 것입니다. 새 문법의 `queue` 인자가 그 1만에 곱할 배수를 받습니다.[^dnstap]

`log` 와 `dnstap` 은 둘 중 하나를 고르는 관계가 아닙니다. 원서가 차례로 놓아 대안처럼 읽히지만 목적이 다르면 함께 켤 수 있습니다. 사람이 눈으로 읽을 것은 좁게 건 `log` 로, 나중에 분석할 것은 `dnstap` 으로 보내는 조합이고, 그만큼 값도 두 번 냅니다.
...
> `debug` 는 조절할 자리가 없습니다. 크래시에서 복구하는 동작을 막는 것이 이 플러그인의 전부이고, 그 복구가 없어지면 서버가 죽기 때문에 원서가 상자를 쳐서 경고합니다.

![debug 가 복구를 끄는 자리는 서버 본체 한 곳이다](./_assets/08-01.debug-recover.svg)

보통 CoreDNS 는 요청을 처리하다 panic 이 나도 죽지 않습니다. 서버 본체가 요청마다 `recover()` 를 걸어 panic 을 잡고, panic 지표를 하나 올린 뒤 그 질의에 `SERVFAIL` 을 돌려주고 계속 돕니다.[^src] 그런데 크래시를 일으킨 문제를 디버깅하려면 이 동작이 방해가 됩니다. panic 이 조용히 삼켜져 스택 트레이스가 손에 남지 않기 때문입니다.
...
원서가 이 장에서 유일하게 상자를 쳐 경고합니다. `debug` 플러그인이 CoreDNS 의 자동 재시작을 막기 때문에 운영에서는 절대 쓰면 안 된다는 것입니다. 이 노트는 원서가 재시작이라 부른 것을 위의 같은 프로세스 안 복구로 읽습니다. 그것을 끄면 panic 하나가 프로세스 전체를 끝내고 그 서버가 맡던 모든 질의가 함께 멈춥니다.

쿠버네티스라면 kubelet 이 컨테이너를 다시 띄웁니다. 그래도 그 사이는 비고, 같은 질의가 다시 오면 또 죽습니다.

> **노트의 읽기**: 스택 트레이스만 원한다면 지금은 `debug` 없이 얻는 길이 있습니다. 5절 끝에서 본 `errors` 의 `stacktrace` 옵션을 켜면, 서버가 panic 을 복구하면서 그 스택을 오류 로그에 함께 남깁니다.[^errors] 복구를 끄지 않으니 운영에서도 켤 수 있는 쪽입니다.
...
이 사례가 자료의 등급을 가릅니다. 공식 문서가 1차 자료인 것은 문법과 기본값까지이고, 플러그인 사이의 상호작용처럼 여러 파일에 걸친 동작은 코드를 봐야 확정됩니다.
...
|-------------|------|----------------|------|
| 몇 건인지·얼마나 걸리는지 | `prometheus` | CoreDNS 쪽엔 없음 · Prometheus 수집 규칙에서 라벨을 거릅니다 | 시계열이 라벨 조합만큼 늘어납니다 |
...
| 크래시의 스택 트레이스 | `debug` | 없음 | 운영 금지 · panic 이 프로세스를 끝냅니다 · 문서와 달리 `errors` 가 있어도 무력화되지 않습니다 |
...
여기서 주의할 것은 원서의 지표 이름을 그대로 쓰면 둘 다 안 된다는 점입니다. 대시보드를 만들기 전에 실제로 도는 CoreDNS 의 `/metrics` 를 한 번 긁어 이름을 확인하는 편이 빠릅니다.
...
7. 한 서버 블록에 `errors` 와 `debug` 를 함께 적었습니다. 공식 문서 문장대로라면 `debug` 가 무력화된다는데, 실제로 그런지 무엇으로 확정하겠습니까? 그리고 이 설정을 운영에 두면 무엇이 위험합니까?
8. 원서 표 8-1 의 카운터 계열 지표 이름으로 만든 대시보드와 경보를 지금 CoreDNS 에 붙이면 무슨 일이 일어나고, 왜 늦게 발견됩니까?
9. 원서 Example 8-21 의 정규식 `".* network is unreachable$"` 를 그대로 쓰면 접힌 줄에는 무엇이 찍힙니까? `".* unreachable$"` 로 넓히면 무엇을 잃습니까?
...
공식 문서 문장만으로는 확정할 수 없습니다. `plugin/errors/errors.go` 에 `recover` 가 있는지, 복구가 어디서 걸리는지를 소스에서 확인해야 합니다. 지금 코드에서는 복구가 서버 본체 한 곳에만 있고 `debug` 가 그것을 끄므로, `errors` 가 같은 블록에 있어도 `debug` 는 동작합니다. 그래서 운영에 두면 panic 하나가 프로세스를 끝내고 그 서버의 모든 질의가 멈춥니다. 원서가 운영 금지를 상자로 친 이유가 그것입니다.

### 정답 8 — 빈 그래프와 울리지 않는 경보

다섯 이름이 지금은 없습니다. 넷은 1.7.0 에서 개명됐고 `type` 지표는 라벨로 흡수됐습니다. 없는 지표를 묻는 질의는 오류가 아니라 빈 결과를 돌려주므로 대시보드는 빈 그래프가 되고 경보 규칙은 값을 못 구해 발화하지 않습니다. 깨졌다는 신호가 없으니, 장애가 났는데 경보가 오지 않았을 때에야 발견됩니다.

### 정답 9 — 설정한 정규식 그대로

접힌 줄은 설정한 정규식을 그대로 되뇌므로 `'.* network is unreachable$'` 가 찍힙니다. 원서 출력의 `^` 는 설정에 없던 것이라 원서 쪽 둘 중 하나가 틀렸습니다. `".* unreachable$"` 로 넓히면 더 많이 접히지만, 접힌 줄에 남는 것이 정규식과 건수와 기간뿐이라 원래 오류가 어느 주소, 어느 상류에 대한 것이었는지 알 수 없게 됩니다.



## 핵심 개념 체크리스트

- [ ] 지표가 값을 아끼는 방식이 집계라는 것과, 원서 표 8-1 의 카운터 이름을 지금 쓰면 대시보드와 경보가 조용히 비는 이유를 말할 수 있는가?
- [ ] 기본 로그 한 줄에서 요청과 응답의 경계를 짚고, `NAMES`·`class`·형식 문자열 중 무엇을 쓸지 목적에 맞게 고를 수 있는가?
- [ ] `dnstap` 이 `log` 의 두 문제를 어떻게 겨냥하는지와 `full` 이 더하는 것을 설명할 수 있는가?
- [ ] `consolidate` 정규식을 넓힐수록 무엇을 잃는지 예를 들어 판단할 수 있는가?
- [ ] `trace` 의 두 주의점(표본율·시계)과 `debug` 를 운영에 켜면 panic 이 프로세스를 끝낸다는 점을 말할 수 있는가?
...
[^dnstap]: CoreDNS — dnstap 플러그인의 현재 문법과 `listen` 모드, `extra` 의 metadata 연결, `"buffer of 10000 messages, above that number dnstap messages will be dropped (this is logged)"`. <https://coredns.io/plugins/dnstap/>
[^errors]: CoreDNS — errors 플러그인의 `consolidate` 인자와 `stacktrace` 옵션(`"Option stacktrace will log a stacktrace during panic recovery."`). <https://coredns.io/plugins/errors/>
...
[^src]: CoreDNS 소스 — `plugin/errors/errors.go` 에 `recover` 없음, `core/dnsserver/server.go` 의 `if !s.debug { defer func() { ... recover() ... vars.Panic.Inc(); errorAndMetricsFunc(s.Addr, w, r, dns.RcodeServerFailure) } }` 와 구조체 필드 `debug bool // disable recover()`, 그리고 `for _, site := range group { if site.Debug { s.debug = true; log.D.Set() } }`. <https://github.com/coredns/coredns/blob/master/core/dnsserver/server.go>
...
[^n170]: CoreDNS 1.7.0 릴리스 노트 — `coredns_dns_request_count_total -> coredns_dns_requests_total` 등 개명과 `"coredns_dns_request_type_count_total is now part of coredns_dns_requests_total"`. <https://coredns.io/2020/06/15/coredns-1.7.0-release/>
[^vars]: CoreDNS 소스 `plugin/metrics/vars/vars.go` — `"request_duration_seconds"` 의 라벨 `[]string{"server", "zone", "view"}`, `report.go` 의 `RequestDuration.WithLabelValues(server, zone, view)`. <https://github.com/coredns/coredns/blob/master/plugin/metrics/vars/vars.go>
[^pkglog]: CoreDNS 소스 — v1.5.0 `plugin/pkg/log/log.go` 의 `time.Now().Format("2006-01-02T15:04:05.000Z07:00")`, 현재 `coremain/run.go` 의 `// LogFlags are initially set to 0 for no extra output`. <https://github.com/coredns/coredns/blob/master/coremain/run.go>
