...
namespace의 기원은 **Plan 9 운영체제**로 거슬러 올라갑니다. 당시 대부분의 운영체제는 파일에 대한 단일한 "name space"를 가졌습니다. Unix 계열은 파일시스템 마운트를 허용했지만 모든 마운트가 시스템 전체에서 공유되는 하나의 파일명 뷰로 들어갔습니다. Plan 9에서는 각 프로세스가 자기만의 "name space" 추상을 가진 프로세스 그룹에 속했고, 그 그룹이 볼 수 있는 파일의 계층이 따로 있었습니다. 각 프로세스 그룹은 **서로를 보지 않고** 자기 파일시스템 집합을 마운트할 수 있었습니다.
...
**프로세스는 종류마다 정확히 하나의 namespace에 속합니다.** Linux 시스템을 시작하면 종류마다 namespace가 하나씩 있고, 거기에 추가 namespace를 만들어 프로세스를 배정할 수 있습니다. 현재 namespace 목록은 `lsns` 명령으로 봅니다.
...
`unshare`의 man 페이지는 이 명령이 "부모로부터 일부 namespace를 unshare한 채로 프로그램을 실행"하게 해 준다고 설명합니다. 이 설명을 풀어 보면 이렇습니다. 프로그램을 실행하면 커널이 새 프로세스를 만들어 그 안에서 프로그램을 실행합니다. 이 일은 실행 중인 프로세스(부모)의 맥락에서 이뤄지고 새 프로세스는 자식이라 불립니다. **"unshare"는 자식이 부모의 namespace를 공유하는 대신 자기 것을 갖게 된다는 뜻입니다.**
...
새 UTS namespace를 가진 프로세스에서 `sh` 셸이 돌고 그 셸 안에서 실행하는 프로그램은 그 namespace를 물려받습니다. `hostname experiment`로 호스트명을 바꾼 뒤 확인하면 `experiment`가 나옵니다. 그런데 **`exit`으로 나와서 확인하면 호스트는 여전히 원래 이름입니다.** 반대 방향도 마찬가지입니다. 호스트의 호스트명을 바꿔도 namespace 안의 프로세스가 인식하는 호스트명에는 영향이 없습니다.
...
첫 `whoami`는 동작하지만 **그다음 명령부터 전부 `Cannot fork` 오류로 실패합니다.** 성공적이지 않아 보이지만 출력에 흥미로운 단서가 있습니다. 오류는 `<명령>: <프로세스 ID>: <메시지>` 형태로 나오는데, **프로세스 ID가 매번 증가합니다.** 이 순서를 보면 첫 `whoami`가 프로세스 ID 1로 돌았다고 짐작할 수 있습니다. **프로세스 ID 번호가 다시 시작됐다는 것 자체가 PID namespace가 어떤 식으로든 작동하고 있다는 단서입니다.** 다만 프로세스를 하나밖에 못 돌린다면 쓸모가 없습니다.
...
여기서 이 절의 결론이 나옵니다. **어느 PID namespace에서 돌든 `ps`는 `/proc`을 들여다봅니다.** 그러므로 `ps`가 새 namespace 안의 프로세스 정보만 반환하게 하려면, 커널이 namespace화된 프로세스 정보를 쓸 수 있는 **별도의 `/proc` 디렉토리 복사본**이 필요합니다. 그리고 `/proc`은 루트 바로 아래 디렉토리이므로, 이것은 곧 **루트 디렉토리를 바꾸는 일**을 뜻합니다.
...
여기에 미묘한 점이 있습니다. **실행 파일이 새 프로세스의 경로(PATH) 안에 있어야 합니다.** 이 프로세스는 부모의 환경을 물려받으므로 `PATH` 환경변수도 함께 받습니다. `alpine` 안의 `bin` 디렉토리가 새 프로세스에게는 `/bin`이 되므로, 일반적인 경로에 `/bin`이 포함돼 있다면 경로를 명시하지 않고도 `ls`를 찾아 씁니다.

**새 루트 디렉토리를 받는 것은 자식 프로세스뿐입니다.** 그 프로세스가 끝나면 제어가 부모로 돌아옵니다. 자식으로 셸을 돌리면 즉시 끝나지 않으므로 효과를 관찰하기 좋습니다. Alpine 안에서는 `bash`가 동작하지 않는데, Alpine 배포판에 `bash`가 없어 새 루트 안에 존재하지 않기 때문입니다. `bash`를 포함하는 Ubuntu 같은 배포판의 파일시스템으로 같은 실습을 하면 동작합니다.
...
`chroot` 외에 `pivot_root`라는 시스템 콜이 있습니다. 이 장의 목적에서 둘 중 무엇을 쓰는지는 구현 세부이고, **핵심은 컨테이너가 자기만의 루트 디렉토리를 가져야 한다는 점**입니다. 그러나 보안 관점에서는 차이가 있습니다.

**`pivot_root`가 `chroot`보다 보안상 유리합니다.** 주된 차이는 `pivot_root`가 mount namespace를 활용한다는 것입니다. 옛 루트가 더 이상 마운트되지 않으므로 **그 mount namespace 안에서 접근할 수 없게 됩니다.** 반면 `chroot` 시스템 콜은 이 방식을 취하지 않아 **옛 루트가 마운트 지점을 통해 여전히 접근 가능한 채로 남습니다.**
...
저자의 정리가 이 절의 핵심입니다. 호스트명을 격리하는 것보다 훨씬 복잡했지만, **PID namespace 생성 · 루트 디렉토리 변경 · 프로세스 정보를 다루는 의사 파일시스템 마운트라는 세 가지의 조합**으로 컨테이너가 자기 프로세스만 보도록 제한할 수 있습니다.
...
여기서 3절과 똑같은 문제가 반복됩니다. mount namespace 안에서 마운트 목록을 전부 뽑으면 호스트의 마운트가 잔뜩 보입니다. **커널이 프로세스별 마운트 지점 정보를 `/proc/<PID>/mounts`로 알려 주기 때문**입니다. 자기 mount namespace를 가진 프로세스라도 호스트의 `/proc`을 쓰고 있으면 그 파일에 기존 호스트 마운트가 전부 들어 있습니다.

그래서 **컨테이너화된 프로세스에 완전히 격리된 마운트 집합을 주려면, 새 mount namespace 생성 · 새 루트 파일시스템 · 새 `proc` 마운트를 결합**해야 합니다. PID에서 배운 것과 같은 구조입니다.

```bash
sudo unshare --mount chroot alpine sh
```

`docker run -v <호스트 디렉토리>:<컨테이너 디렉토리>`로 호스트 디렉토리를 컨테이너에 마운트하는 방식도 이 원리입니다. 컨테이너의 루트 파일시스템이 자리를 잡은 뒤 대상 컨테이너 디렉토리를 만들고, 원본 호스트 디렉토리를 그 대상에 bind 마운트합니다. **컨테이너마다 자기 mount namespace를 갖기 때문에 이렇게 마운트된 호스트 디렉토리는 다른 컨테이너에서 보이지 않습니다.**
...
**namespace 종류를 확인할 때 주의할 점이 하나 있습니다.** `namespaces(7)` 문서에는 각 namespace 종류별로 `/proc/[pid]/ns/*` 파일이 언제부터 제공됐는지가 함께 적혀 있는데, 이 버전은 **namespace 자체의 도입 시점과 다릅니다.** 예를 들어 mount namespace의 `/proc` 핸들 파일은 Linux 3.8부터지만 mount namespace 자체는 2.4.19부터입니다. 버전을 인용할 때 둘을 섞지 않아야 합니다.
