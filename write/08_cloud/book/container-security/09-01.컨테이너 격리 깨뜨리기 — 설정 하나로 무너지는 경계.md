...
출발점은 컨테이너 세계에서 가장 기본값이 위험한 동작입니다. **컨테이너는 기본으로 root로 돕니다.** 그리고 user namespace를 쓰지 않는 한 그 root는 **호스트의 root와 같습니다**.
...
이미지가 비root 사용자를 지정하지 않았고 실행할 때도 사용자를 지정하지 않으면, 컨테이너는 **기본적으로 root로 돕니다**. 그리고 user namespace를 쓰지 않는 한 이것이 컨테이너 안의 root일 뿐 아니라 **호스트의 root이기도 하다**는 것을 쉽게 확인할 수 있습니다.
...
> `docker` 대신 `runc`를 쓰면 이 시연이 덜 극적입니다. (곧 다룰 rootless 컨테이너를 빼면) 애초에 컨테이너를 돌리려면 호스트에서 root여야 하기 때문입니다. 일반적으로 **namespace를 만들 충분한 capability를 가진 것이 root뿐**이라서입니다. Docker에서는 root로 도는 **Docker 데몬**이 대신 컨테이너를 만들어 줍니다.

Docker에서 비root 사용자가 시작해도 컨테이너가 root로 돈다는 사실은 **권한 상승의 한 형태**입니다. 컨테이너가 root로 도는 것 자체가 반드시 문제는 아니지만 보안을 생각하면 경보가 울립니다.

공격자가 root로 도는 컨테이너를 탈출하면 **호스트에 대한 완전한 root 접근**을 얻습니다. 머신의 모든 것에 자유롭게 접근한다는 뜻입니다. 저자는 이렇게 묻습니다 — 공격자가 호스트를 장악하기까지 **방어선이 단 하나뿐인 상태**로 있고 싶은가.
...
서버에서 직접 돌도록 설계됐던 소프트웨어를 감싼 이미지가 많습니다. Nginx가 그 예입니다. Docker가 유행하기 훨씬 전부터 있었고 지금은 Docker Hub의 이미지로 제공되는데, **집필 시점의 표준 Nginx 이미지는 기본이 root**였습니다. `docker top`으로 보면 master 프로세스가 root로 돕니다.

이유가 있습니다. 서버에서 돌 때 nginx는 기본으로 전통적인 웹 포트 80에서 요청을 받습니다. 2장에서 본 대로 **1024 미만의 낮은 번호 포트를 여는 데는 `CAP_NET_BIND_SERVICE`가 필요**합니다. 이를 보장하는 가장 간단한 방법이 root로 돌리는 것이었습니다.

그런데 **컨테이너에서는 이 요구가 훨씬 덜 합리적입니다.** 포트 매핑이 있으니 nginx 코드는 아무 포트에서나 듣고, 필요하면 그것을 호스트의 80번에 매핑하면 됩니다.

root 실행이 문제임을 인식한 여러 벤더가 지금은 일반 비특권 사용자로 도는 이미지를 제공합니다. Nginx의 경우 `nginx-unprivileged` 저장소가 있습니다. 다른 애플리케이션은 더 까다로워서 Dockerfile 몇 줄 수정을 넘어 코드 변경이 필요할 수 있는데, **Bitnami가 여러 인기 애플리케이션의 비root 이미지를 만들어 유지**하고 있습니다.
...
- 스캔 문제와 이어지지만 **더 나쁜 것** — 실행 중인 인스턴스마다 정확히 어떤 버전이 설치됐는지 알기 어렵습니다. 취약점을 알게 돼도 **어느 컨테이너를 죽이고 다시 배포해야 할지 모릅니다.**
...
4장의 예제를 따라 해 봤다면 컨테이너를 만드는 동작 일부에 **root 권한이 필요**하다는 것을 압니다. 이는 여러 사용자가 같은 머신에 로그인하는 전통적 공유 환경에서는 받아들이기 어렵습니다. 대학 시스템처럼 학생과 교직원이 공유 머신에 계정을 갖는 경우, 시스템 관리자가 컨테이너를 만들라고 root 권한을 주는 것을 거부하는 것은 **당연합니다**. 그 권한이면 다른 사용자의 코드나 데이터에 무슨 짓이든 할 수 있기 때문입니다.
...
rootless 컨테이너는 [4장에서 본 user namespace](./04-02.%EB%82%98%EB%A8%B8%EC%A7%80%20namespace%EC%99%80%20%ED%98%B8%EC%8A%A4%ED%8A%B8%EC%97%90%EC%84%9C%20%EB%B3%B8%20%EC%BB%A8%ED%85%8C%EC%9D%B4%EB%84%88.md) 기능을 씁니다. **호스트의 평범한 비root 사용자 ID가 컨테이너 안에서 root로 매핑**됩니다. 어떤 식으로든 컨테이너 탈출이 일어나도 공격자가 자동으로 root 권한을 갖지 않으므로 **유의미한 보안 강화**입니다.
...
Docker를 비롯한 런타임은 컨테이너 실행 시 `--privileged` 옵션을 지정할 수 있게 합니다. Andrew Martin은 이를 **"컴퓨팅 역사상 가장 위험한 플래그"** 라 불렀는데, 이유가 있습니다. 대단히 강력한데 **널리 오해받고 있습니다**.
...
답은 이렇습니다. Docker에서 프로세스가 기본적으로 root 사용자 ID로 도는 것은 맞지만, **root의 일반적인 Linux capability 중 상당수는 당연히 주어지지 않습니다**. `capsh` 유틸리티로 확인하면 차이가 보입니다.
...
플래그 없이 주어지는 정확한 집합은 **구현에 따라 다릅니다**. OCI가 기본 집합을 정의하고 `runc`가 그것을 부여합니다.

> 주목할 점이 있습니다. 그 **기본 집합에 이미 `CAP_SYS_ADMIN`이 포함**됩니다. 이 하나가 **namespace 조작과 파일시스템 마운트를 비롯한 방대한 특권 활동**을 엽니다. Eric Chiang은 `/dev`의 장치를 컨테이너 파일시스템에 마운트해 호스트 파일시스템으로 탈출하는 예를 보였습니다.

Docker가 `--privileged`를 도입한 것은 **"Docker in Docker"** 를 가능하게 하기 위해서였습니다. 컨테이너로 도는 빌드 도구와 CI/CD 시스템이 이미지를 빌드하려고 Docker 데몬에 접근해야 해서 널리 쓰입니다. 다만 저자는 Docker in Docker와 이 플래그 전반을 **신중하게** 쓰라고 조언합니다.

### 더 미묘한 위험 — 역도 참이라는 오해

저자가 짚는 더 미묘한 위험이 있습니다. 사람들이 이 플래그가 컨테이너에 root 권한을 주는 데 필요하다고 생각하기 때문에, **그 역도 참이라고 믿는다**는 것입니다. 즉 **이 플래그 없이 도는 컨테이너는 root 프로세스가 아니라고 오해합니다.** 앞 절이 보여 준 대로 사실이 아닙니다.
...
파일시스템 전체를 마운트하는 것은 극단적인 예입니다. 저자는 미묘함의 정도가 다른 사례들을 나열합니다.
...
마지막 것이 특히 교묘합니다. 컨테이너 로그 파일이 `/var/log`에서 파일시스템의 다른 곳을 가리키는 **심볼릭 링크**인데, **컨테이너가 그 링크를 다른 파일로 돌리는 것을 막을 방법이 없기** 때문입니다.
...
소켓 마운트의 아주 흔한 용도가 Jenkins 같은 CI 도구입니다. 파이프라인의 일부로 이미지 빌드를 돌리라고 Docker에 지시하려면 소켓이 필요합니다. **정당한 용도이지만 공격자가 찌를 수 있는 약한 배를 만듭니다.** Jenkinsfile을 수정할 수 있는 사용자는 Docker가 명령을 실행하게 만들 수 있고, 여기에는 **기저 클러스터에 대한 root 접근을 얻는 명령**이 포함될 수 있습니다. 그래서 저자는 **운영 클러스터에서 Docker 소켓을 마운트하는 CI/CD 파이프라인을 돌리는 것은 대단히 나쁜 관행**이라고 못 박습니다.
...
컨테이너화된 프로세스는 모두 호스트에서 보이므로, 프로세스 namespace를 컨테이너에 공유하면 **그 컨테이너가 다른 컨테이너의 프로세스까지 보게 됩니다**. 원문의 시연은 이렇습니다.
...
두 번째 컨테이너 안에서 `ps | grep sleep`을 하면 **첫 번째 컨테이너의 `sleep` 프로세스가 보입니다**. 저자의 표현으로 더 흥미로운 것은 그다음입니다. 두 번째 컨테이너에서 `kill -9 <pid>`를 실행하면 **첫 번째 컨테이너의 프로세스를 죽일 수 있습니다.**
...
멀티테넌트 환경이라면 더 주의해야 합니다. 저자의 마지막 경고가 날카롭습니다. **`--privileged` 컨테이너는 같은 호스트의 다른 모든 컨테이너에 완전히 접근할 수 있습니다.** 같은 Kubernetes namespace에 있는지 같은 **상대적으로 피상적인 통제와 무관하게** 그렇습니다.
...
이 장의 "소켓 접근 = 호스트 root" 경고는 지금 더 중요해졌습니다. 원문 이후 **소켓 마운트를 우회하는 대안이 자리 잡았기** 때문입니다. CI에서 이미지를 빌드하려고 소켓을 내주는 대신, 데몬 없이 빌드하는 도구(6장 심화에서 정리한 **BuildKit rootless·Kaniko·Buildah** 계열)를 쓰면 이 위험을 통째로 제거할 수 있습니다. 저자가 "정당하지만 약한 배"라 표현한 그 절충이 **지금은 굳이 감수하지 않아도 되는 절충**이 됐습니다.
...
- [ ] 기본 집합에 이미 `CAP_SYS_ADMIN`이 있다는 사실을 안다
