...
이 경계는 계속 서버 쪽으로 움직여 왔습니다. Deployment의 롤아웃은 예전에 클라이언트에 있던 기능입니다. `kubectl apply`도 서버 사이드 apply로 옮겨 갔습니다. 그래도 클라이언트에 남은 기능이 여전히 많습니다. 그 기능은 언어마다 다시 구현해야 합니다. 저자는 Java 클라이언트가 `kubectl` 기능을 가장 두껍게 흉내 낸다고 적습니다.
...
|---|---|---|---|
| `GET /api/v1/namespaces/{ns}/pods` | `CoreV1Api.list_namespaced_pod(ns)` | `CoreV1Api.listNamespacedPod(ns)` | `client.CoreV1.ListNamespacedPod(ns)` |
| `GET /api/v1/namespaces` | `CoreV1Api.list_namespace()` | `CoreV1Api.listNamespace()` | `client.CoreV1.ListNamespace()` |
| `POST /api/v1/namespaces/{ns}/pods` | `CoreV1Api.create_namespaced_pod(ns, pod)` | `CoreV1Api.createNamespacedPod(...)` | `client.CoreV1.CreateNamespacedPod(...)` |
| `GET /apis/apps/v1/namespaces/{ns}/deployments` | `AppsV1Api.list_namespaced_deployment(ns)` | `AppsV1Api.listNamespacedDeployment(ns)` | `client.AppsV1.ListNamespacedDeployment(ns)` |
...
> **버전 차이**: 원서 .NET port-forward 예제는 서브프로토콜을 `"v4.channel.k8s.io"`로 박아 넣습니다. 이 클러스터가 협상한 값은 exec에서 `v5.channel.k8s.io`였습니다. 서브프로토콜을 상수로 넘기는 코드는 클러스터 버전이 올라가면 손봐야 합니다.



## 원서 코드의 오류 목록

> 이 장에서 발견한 것을 한자리에 모읍니다. 앞에서 교훈이 있는 것은 본문에 병기했고, 이 표가 전수 목록입니다.
...
| watch · .NET | `using (watch.Watch<V1Pod, V1PodList>((type, item) => {…}` | 괄호 미종결 | 원문 |
...
| port-forward · .NET | 서브프로토콜 `"v4.channel.k8s.io"` | 실측 클러스터는 `v5` 협상 | 실행 |
| watch 절 도입 | "the Kuberentes API also provides watch" | 철자 오류 | 원문 |
| 도입부 | "there are a high-quality clients" | 문법 오류 | 원문 |
| 클라이언트 설치 절 | 패키지 관리자 예시로 `npm` | 나열한 셋(Python·Java·.NET) 중 npm을 쓰는 것은 없음 | 원문 |
