...
> 이 노트가 남긴 델타만 묻습니다. `completions`×`parallelism` 조합, `backoffLimit`, Indexed 모드, `suspend`, 자동 삭제, CronJob 운영 옵션은 위임 표의 문서들이 각자 질문 세트를 갖고 있습니다.

1. 일반 Pod로 데이터베이스 마이그레이션을 돌리면 무엇이 잘못되나요? Job은 그것을 어떻게 막나요?
2. Job이 만든 Pod에는 Job이 스스로 고른 라벨이 붙습니다. 이 동작이 왜 필요한가요? 그것을 끄고 라벨을 직접 지정하는 경우로 책이 드는 예는 무엇인가요?
3. 원서 Table 12-1의 Behavior 칸만 보면 work queue 유형과 One shot이 구분되지 않습니다. 차이는 어디에 있나요?
4. 같은 실패인데 `restartPolicy`가 `OnFailure`일 때와 `Never`일 때 클러스터에 남는 흔적이 어떻게 달라지나요? 책은 어느 쪽을 권하나요?
5. `OnFailure`로 두었을 때 재시작에 Job은 어디까지 관여하나요?
6. work queue 랩에서 큐 데몬을 ReplicaSet으로 감싼 이유와, 그 앞에 Service를 붙인 이유는 각각 무엇인가요?
7. 컨슈머 Job에서 `completions`를 비워 둔 것이 이 랩의 핵심입니다. 그러면 Job이 어떤 모드로 들어가고, 워커들에게 어떤 제약이 생기나요?
8. 실습 정리가 `kubectl delete rs,svc,job -l chapter=jobs` 한 줄로 끝나는 이유는 무엇인가요? 이 방식이 놓치는 것은 무엇인가요?

### 정답

1. 일반 Pod는 종료 코드와 무관하게 계속 재시작합니다. 마이그레이션을 그렇게 돌리면 끝난 작업을 무한히 반복해 DB를 계속 다시 채웁니다. Job은 Pod가 **성공적으로 끝나는 것**을 목표로 삼는 오브젝트라, 도달하면 그대로 끝냅니다.
2. 라벨로 Pod 집합을 가리는 컨트롤러에서 Pod가 여러 오브젝트에 걸쳐 재사용되면 예상치 못한 동작이 생깁니다. Job은 시작과 끝이 있어 사용자가 많이 만들게 되고, 그만큼 고유한 라벨을 고르기가 어렵고 또 중요해집니다. 그래서 Job이 자동으로 고유 라벨을 붙입니다. 책이 드는 예는 **실행 중인 Job을 그것이 관리하는 Pod를 죽이지 않고 교체**하는 경우입니다.
3. 표의 Behavior만 보면 둘 다 "한 번 돌고 성공 종료"로 읽힙니다. 차이는 본문에 있습니다. **워커 Job은 큐가 빌 때까지 각 일감을 처리합니다.** Pod 하나하나가 한 번 도는 것은 맞지만, 그 한 번 안에서 큐를 계속 비우다가 더 꺼낼 것이 없을 때 끝납니다.
4. `OnFailure`면 kubelet이 같은 Pod를 그 자리에서 다시 시작합니다. 계속 실패하면 `CrashLoopBackOff`가 되고 재시작 전에 잠시 기다립니다. Pod 하나가 재시작 횟수를 쌓아 갑니다. `Never`면 kubelet이 재시작하지 않고 그 Pod를 실패로 선언하며, 그러면 Job이 **대체 Pod를 새로 만들어** 실패한 Pod가 하나씩 쌓입니다. 책은 `OnFailure`를 권합니다. 다만 흔적이 사라지는 것은 아니어서 `RESTARTS` 횟수는 그대로 쌓입니다.
5. 관여하지 않습니다. 재시작과 백오프가 **전부 노드의 kubelet 안에서** 끝납니다.
6. ReplicaSet은 **머신 장애가 나도 새 Pod가 만들어지게** 하려는 것입니다. Service는 생산자와 소비자가 **DNS로 큐를 찾게** 하려는 것입니다. 목적이 서로 다릅니다.
7. `completions`를 비우면 Job이 **worker-pool 모드**로 들어갑니다. 첫 Pod가 종료 코드 0으로 빠져나가는 순간 Job은 정리 단계로 접어들고 새 Pod를 더 만들지 않습니다. 뒤집어 말하면 **일이 다 끝나기 전에는 어떤 워커도 먼저 나가면 안 됩니다.** 모두가 큐가 빈 것을 확인하고 함께 마무리해야 합니다.
8. 이 랩의 매니페스트 넷이 모두 `chapter: jobs` 라벨을 달고 있기 때문입니다. 놓치는 것은 **그 라벨을 넣지 않은 리소스**입니다. 앞의 one-shot 예제와 뒤의 CronJob 예제에는 이 라벨이 없어 저 명령으로 지워지지 않습니다.
