...
# 타입 스펙: type-flowchart.md — 질문 하나("코드가 어디서 도는가")에서 두 갈래만 나는 라우팅이다.
#           본문이 "인증 정보를 얻는 자리는 둘뿐" 이라 못 박으므로 나열이 아니라 갈림길이어야 한다.
#           독자의 동작은 "나는 어느 쪽인가" 를 정하고 그 패널만 읽는 것이다.
#           어긋나는 지점: 갈림이 마름모가 아니라 칩이고, 가지 끝이 단계 상자가 아니라 같은 세 칸
#           (읽는 것 · 클라이언트 호출 · 걸리는 자리)을 가진 패널이다 — 대조가 형태로 보이게 하려고
#           양쪽 칸 높이를 맞췄다.
