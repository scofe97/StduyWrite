...
# 타입 스펙: type-dp-security-matrix.md — 열이 두 API(Ingress · 메시), 행이 같은 두 기준
#           (호환해야 하는 폭 · 그래서 가능한 능력)인 대조 격자다. 같은 기준을 같은 높이에
#           놓는 것이 논증이고, 연결선이 없어 정본의 금지를 지킨다.
#           어긋나는 지점: 칸이 권한값이 아니라 구간 막대와 목록이다. 아래 노드 띠는
#           architecture 어휘로 두 API 가 각각 어디서 작동하는지를 덧붙인 것이다.
#           type-bar 는 기각 — 막대 길이가 수사적 비례이지 측정값이 아니고, 정본이 요구하는
#           축·눈금·값 라벨이 없다. 없는 수치를 있는 척하지 않는다.
