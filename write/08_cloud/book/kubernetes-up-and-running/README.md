...
| [12-01](./12-01.Jobs%20%E2%80%94%20%EB%81%9D%EB%82%98%EB%8A%94%20%EC%9D%BC%EA%B3%BC%20work%20queue%20%EB%9E%A9.md) | Jobs — 끝나는 일과 work queue 랩 (Ch12) | 완료 · 랩 |
...
| [19-01](./19-01.Securing%20Applications%20%E2%80%94%20%EC%A3%BD%EC%9D%80%20%EC%8B%A4%EC%8A%B5%20%EC%9D%B4%EB%AF%B8%EC%A7%80%EC%99%80%20proc%20%EB%A1%9C%20%EB%8B%A4%EC%8B%9C%20%EC%84%B8%EC%9A%B4%20%EB%9E%A9.md) | Securing Applications — 죽은 실습 이미지와 /proc 로 다시 세운 랩 (Ch19) | 완료 · 델타 · 랩 |
| [20-01](./20-01.Policy%20and%20Governance%20%E2%80%94%20Gatekeeper%20%EB%A1%9C%20%EB%A7%8C%EB%93%A4%EA%B8%B0%20%EC%A0%84%EC%97%90%20%EB%A7%89%EA%B3%A0%20%EB%A7%8C%EB%93%A0%20%EB%92%A4%EC%97%90%20%EC%84%B8%EB%8A%94%20%EB%B2%95.md) | Policy and Governance — Gatekeeper 로 만들기 전에 막고 만든 뒤에 세는 법 (Ch20) | 완료 · 랩 |
| [21-01](./21-01.Multicluster%20%E2%80%94%20%EB%8A%98%EB%A6%AC%EA%B8%B0%20%EC%A0%84%EC%97%90%20%EA%B0%96%EC%B6%9C%20%EA%B2%83%EA%B3%BC%20%EB%8A%98%EB%A6%B0%20%EB%92%A4%20%EA%B0%88%EB%A6%AC%EB%8A%94%20%EC%84%B8%20%EB%AA%A8%EB%8D%B8.md) | Multicluster — 늘리기 전에 갖출 것과 늘린 뒤 갈리는 세 모델 (Ch21) | 완료 · 델타 |
...
| 최근 검증 결과 | 21장 — 코드·랩이 없는 판단 장이라 실측 대신 원문 대조로 검증. 일관성은 DDIA 정독본에 위임. 적대적 검증이 **도식 2장의 날조**를 잡음 — 본문은 대부분 통과했는데 도식이 원서에 없는 처방(클러스터별 버전 분리)과 동작(anycast 광고 철회)을 "원서 21장" 서명 아래 실었음. 도식을 별도 검증 대상으로 삼아야 한다는 교훈 · 20장 — 전항 통과, 정오·버전 차이 6건 병기(실행 4 · 원문 2). Gatekeeper v3.23.0 을 kind 에 실제 설치해 판정하고 제거. 원서가 "이 설치로는 안 켜진다"고 한 변형이 지금은 기본 포함됨. 적대적 검증이 **생략을 부재로 오독한 것 1건**(원서가 웹훅 출력을 `...`로 생략한 것을 "원서에 없는 웹훅"으로 단정)과 발췌를 "그대로"라 적은 것 2건(initContainers 규칙·excludedNamespaces 누락)을 잡아 정정. 원서의 준수 파드는 정책은 통과하고 ImagePullBackOff 로 죽음 · 19장 — 전항 통과, 정오·버전 차이 12건 병기(실행 6 · 원문 5 · 공식 대조 1). 적대적 검증이 상호참조 오류 1건(원서는 20장을 가리키는데 17장으로 적음)과 인과 오귀속 1건, 빈 위임 링크 1건을 잡아 정정. **책의 실습 이미지 둘 다 사용 불가** — `gcr.io/kuar-demo/*` 는 익명 pull 거부(401), `r.j3ss.co/amicontained` 는 도메인 소멸. 랩을 `/proc/self/status` 로 다시 세워 실측 · 18장 — 전항 통과, 원서 코드 오류 19건 병기(실행 7 · 원문 10 · 공식 대조 2). Python 클라이언트 36.0.3 을 격리 venv 에 두고 kind 클러스터로 실제 실행해 판정. 적대적 검증이 역방향 오류 1건을 잡아냄 — port-forward 의 `v4.channel.k8s.io` 는 원서가 맞고 `v5` 는 exec 전용, 핸드셰이크 재실측으로 확정 · 17장 — 전항 통과, 버전 차이 5건 병기(v1beta1 3종 소멸·`v1` CRD 스키마 필수화·`signerName`·웹훅 필수 필드 2종·VAP 대체 경로). kind 클러스터 서버 dry-run 으로 랩 전 단계 실증 · 16장 6건 · 15장 3건 · 12장 2건 · 7장 4건 · 14장 5건, 모두 전항 통과 |
| 복습 회차 (_review) | 미개시 |
...
흐름·관계·상태 전이 도식은 이 폴더의 `_assets/`에 SVG로 두고 본문에서 `![설명](_assets/파일명.svg)`로 참조합니다. 원서에 도식이 없는 장이 많아 대부분 새로 그리게 됩니다. SVG의 SSOT는 `_assets/_gen/gen-<이름>.py` 생성기이므로 SVG를 손으로 고치지 않고 생성기를 고쳐 다시 돌립니다. 그리기 전에 본문부터 읽습니다 — 본문이 도식의 규격을 직접 적어 둔 자리가 있어(예: 14장의 "위 도식의 점선"), 임의로 그리면 본문이 틀려집니다.
