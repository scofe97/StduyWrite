...
09-01편에서는 질문을 담은 이미지를 만들고, 컨테이너 시작 시 그 파일을 새 위치로 복사한 뒤, 다른 컨테이너가 볼 수 있게 그 위치에 볼륨을 마운트했습니다. 단순한 일치고는 손이 많이 갑니다. 이제 더 간단한 방법이 있습니다 — image 볼륨입니다. 다만 이 글을 쓰는 시점에 이 기능은 기본 활성화가 아니라 `ImageVolume` 피처 게이트로 켜야 합니다.

> **피처 게이트란**: 쿠버네티스에서 기능이 일반 사용 가능(GA)이 되기 전에는 보통 피처 게이트 뒤에 숨어 있습니다. 클러스터 관리자가 명시적으로 켜야 하고, 켜지 않으면 API에 필드가 보여 설정할 수는 있어도 동작하지 않습니다.
...
> 이 글을 쓰는 시점에는 image 아티팩트만 지원됩니다. 다만 결국 모든 OCI 아티팩트를 지원할 계획이라, 나중에는 레지스트리에 아티팩트를 push하기만 하면 되고 Dockerfile을 만들 필요조차 없어집니다.
...
hostPath 볼륨은 데이터베이스 데이터를 두기에 좋은 곳이 *아닙니다*. 데이터베이스 파드가 항상 같은 노드에서 돈다는 보장이 없다면, 다른 노드로 재스케줄됐을 때 데이터에 접근할 수 없기 때문입니다. hostPath는 보통 노드에서 도는 프로세스가 읽거나 만드는 파일 — 시스템 수준 로그 같은 것 — 을 파드가 읽고 써야 할 때 씁니다.
...
만약 공격자가 쿠버네티스 API에 접근해 프로덕션 클러스터에 이런 파드를 배포할 수 있다면 노드 전체가 장악됩니다. 이 글을 쓰는 시점에 쿠버네티스는 일반 사용자가 hostPath 볼륨을 쓰는 것을 막지 않아, 그 자체로는 전혀 안전하지 않습니다.
...
init 컨테이너 방식은 파일을 담은 이미지를 만들고, 컨테이너가 그 파일을 볼륨으로 복사하고, 다른 컨테이너가 그 볼륨을 마운트하는 여러 단계를 거칩니다. image 볼륨은 그 이미지를 *볼륨째로* 마운트해 복사 단계와 init 컨테이너 자체를 없앱니다. 제약은 두 가지입니다 — 이 글을 쓰는 시점에 `ImageVolume` 피처 게이트를 켜야 동작하고(기본 비활성화), 아직 image 아티팩트만 지원됩니다.
...
`Directory`는 경로에 디렉터리가 *이미 있어야* 하고, 없으면 파드를 못 돌게 합니다. `DirectoryOrCreate`는 같되, 아무것도 없으면 빈 디렉터리를 *만들어* 줍니다. type을 지정하는 이유는, 그 경로가 컨테이너 프로세스가 기대하는 종류(파일·디렉터리·블록 장치·소켓 등)인지 마운트 전에 검사해, 어긋나면 조용히 잘못 동작하는 대신 파드 이벤트로 실패 원인을 드러내기 위해서입니다.
