...
막는 장치는 따로 켜야 합니다. Pod Security Standards[^pss]의 **Baseline** 프로파일이 `hostPath` 볼륨을 금지합니다. 네임스페이스에 `pod-security.kubernetes.io/enforce: baseline` 라벨을 붙이면 그 네임스페이스에서 hostPath 파드가 거부됩니다. 클러스터를 남과 공유한다면 "위험하다"로 끝내지 말고 이 라벨이 붙어 있는지 확인하는 편이 낫습니다.
