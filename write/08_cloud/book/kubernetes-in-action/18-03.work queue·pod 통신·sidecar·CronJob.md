...
아래는 이 편 전체를 한 장으로 이은 지도입니다. 위쪽 세 줄기는 Job 하나를 넓히는 이야기이고 서로 독립입니다. 점선 아래의 CronJob은 층위가 다릅니다 — Job을 고치는 게 아니라, 그 Job을 시간표에 올려 반복시키는 바깥 껍질입니다.
...
색이 붙은 곳은 두 군데입니다. 붉은 띠는 사이드카를 일반 컨테이너에 두면 Job이 영영 끝나지 않는다는 뜻이고, 앰버 띠는 놓친 스케줄이 100개를 넘으면 컨트롤러가 Job을 아예 만들지 않는다는 뜻입니다.
...
![CronJob 동작: schedule에 맞춰 Job이 생성되고, 각 Job이 Pod를 만들어 완료까지 실행되는 반복 타임라인](_assets/18-03-cronjob-operation.svg)
...
컨트롤러가 30초 안에 Job을 만들지 못하면 만들지 않고 `MissSchedule` 이벤트를 냅니다[^cj-events]. 이 필드를 설정하지 않고 컨트롤러가 오래 오프라인이었다가 복귀하면, 그동안 놓친 Job을 한꺼번에 모두 만드는 바람직하지 않은 동작이 생길 수 있습니다. 놓친 스케줄이 100개를 넘으면 컨트롤러는 Job을 만들지 않고 `too many missed start times…`로 시작하는 오류를 로그에 남깁니다[^starting-deadline]. 시작 마감을 설정하면 이 사태를 막을 수 있습니다.
...
![concurrencyPolicy 세 정책 비교: Allow는 겹쳐 실행, Forbid는 새 Job을 건너뜀, Replace는 이전 Job을 지우고 새 Job으로 교체하는 타임라인](_assets/18-03-concurrency-policy.svg)
...
[^starting-deadline]: 공식 문서 [CronJob — Deadline for delayed Job start](https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/#starting-deadline). "If you don't specify `startingDeadlineSeconds` for a CronJob, the Job occurrences have no deadline." 놓친 스케줄에 대해서는 "If there are more than 100 missed schedules, then it does not start the Job and logs the error."라고 적고, 짧은 값에 대한 caution 은 "If `startingDeadlineSeconds` is set to a value less than 10 seconds, the CronJob may not be scheduled. This is because the CronJob controller checks things every 10 seconds."입니다.
[^concurrency]: 공식 문서 [CronJob — Concurrency policy](https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/#concurrency-policy). 적용 범위는 "Note that concurrency policy only applies to the Jobs created by the same CronJob. If there are multiple CronJobs, their respective Jobs are always allowed to run concurrently."입니다. `Forbid` 설명에는 "Also note that when the previous Job run finishes, `.spec.startingDeadlineSeconds` is still taken into account and may result in a new Job run."이라는 단서가 붙습니다.
[^cj-events]: `MissSchedule`·`JobAlreadyActive`라는 이벤트 이름은 원서 §18.2 서술을 따랐습니다. 공식 문서에는 두 문자열이 나오지 않고, 해당 동작을 "the CronJob skips that instance of the Job"·"the CronJob skips the new Job run"처럼 건너뛴다고만 기술합니다. 이름을 확정하려면 실제 클러스터의 `kubectl get events`로 확인해야 합니다.
[^cron-macro]: 매크로 표는 공식 문서 [CronJob — Schedule syntax](https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/#schedule-syntax)의 표를 옮긴 것입니다(`@yearly (or @annually)`·`@daily (or @midnight)` 별칭과 등가 crontab 식 포함). 다만 일·요일 필드가 OR로 묶이는 규칙은 이 공식 문서에 없습니다. 같은 절이 문법 정본으로 위임하는 [robfig/cron](https://pkg.go.dev/github.com/robfig/cron/v3) 문서와 전통적인 Vixie cron 동작이 근거이며, "둘 다 `*`가 아닐 때만 OR"라는 전제가 붙습니다.
[^cj-suspend]: 공식 문서 [CronJob — Schedule suspension](https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/#schedule-suspension). "This setting does _not_ affect Jobs that the CronJob has already started." 중단 중 동작은 "all subsequent executions are suspended (they remain scheduled, but the CronJob controller does not start the Jobs to run the tasks) until you unsuspend the CronJob."입니다.
