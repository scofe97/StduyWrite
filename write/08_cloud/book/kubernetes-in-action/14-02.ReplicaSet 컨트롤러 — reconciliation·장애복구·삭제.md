...
source:
  ...
  - https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md  # level-based vs edge-based·2→5→3 예시 (2026-08-10 조회)
  - https://github.com/kubernetes/community/blob/master/contributors/devel/sig-api-machinery/controllers.md  # Level driven, not edge driven (2026-08-10 조회)
related:
...
### 이 성질에는 이름이 있습니다 — level-triggered

지금 상태(level)를 보고 판단하는 방식을 **level-triggered**, 변화의 순간(edge)에 반응하는 방식을 **edge-triggered**라 부릅니다. 쿠버네티스는 전자를 택했고, 이것이 API 설계 문서에 명시된 원칙입니다.[^level-driven]

| | 무엇에 반응하는가 | 알림을 놓치면 |
|---|---|---|
| edge-triggered | 상태가 **바뀌는 순간** | 그 변화는 영영 처리되지 않음. 진행 상황을 따로 저장해야 함 |
| level-triggered | **지금의 상태** | 다음 관찰에서 자연히 복구됨 |

공식 문서는 이렇게 설명합니다. `replicas`를 한 번의 PUT으로 2에서 5로 올렸다가 다음 PUT으로 3까지 내리면, 시스템은 중간의 **5를 거쳐 갈 의무가 없습니다.** 최종적으로 적힌 3만 보고 그리로 갑니다.[^level-driven] 컨트롤러 입장에서 "5였던 적이 있다"는 사실은 존재하지 않는 것과 같습니다.

컨트롤러 작성 가이드는 이 점을 더 직접적으로 못 박습니다 — 어떤 필드가 `true`로 보인다고 해서 그것이 `false`에서 `true`로 **바뀌는 것을 봤다고 가정하면 안 되고**, 지금 `true`라는 사실만 관찰한 것입니다.[^level-driven] watch 를 쓰더라도 이 제약은 그대로입니다.

두 가지 실용적 결과가 따라옵니다. 컨트롤러는 **진행률을 저장할 필요가 없고**, 그래서 언제 죽었다 살아나도 안전합니다. 재시작한 컨트롤러는 "어디까지 했더라"를 복원하지 않고 그저 지금 상태를 다시 볼 뿐입니다. 앞의 resync 가 성립하는 이유도 같습니다 — 전체를 다시 훑어도 같은 결론이 나오니 몇 번이든 반복해도 안전합니다.
...
[^level-driven]: Kubernetes 공식 [API Conventions](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md) (2026-08-10 조회). 원문은 "the system's behavior is ***level-based* rather than *edge-based***. This enables robust behavior in the presence of missed intermediate state changes." 이고, 2→5→3 예시는 "if a value is changed from 2 to 5 in one PUT and then back down to 3 in another PUT the system is **not required to 'touch base' at 5**" 입니다. 같은 문서의 conditions 절에도 "The system is level-based rather than edge-triggered" 가 반복됩니다. 컨트롤러 작성 지침은 [Writing Controllers](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-api-machinery/controllers.md) 의 "**Level driven, not edge driven.**" 항목으로, "If an API object appears with a marker value of `true`, you **can't count on having seen it turn from `false` to `true`**, only that you now observe it being `true`. Even an API watch suffers from this problem" 라고 적습니다.
