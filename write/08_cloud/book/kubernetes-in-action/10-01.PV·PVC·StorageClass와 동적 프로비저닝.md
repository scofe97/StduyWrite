...
위 그림이 네 모드를 제한 단위와 동시 사용 축으로 한눈에 놓은 것입니다. Once 계열은 "하나만"의 하나가 파드냐 노드냐로 갈리고, Many 계열은 read-write를 허용하느냐로 갈립니다.
...
이 절 머리의 전이도가 PV가 거치는 상태를 정리한 것입니다. PVC를 지웠다고 PV가 곧바로 사라지는 게 아니라, 먼저 Released가 되고 그다음 reclaim policy가 운명을 정합니다.
...
`Retain`은 PVC를 지워 PV가 해제돼도 볼륨을 보존하고 운영자가 수동 회수합니다 — 수동 생성 PV의 기본값입니다. `Delete`는 해제 시 PV 오브젝트와 밑바탕 스토리지를 자동 삭제합니다 — 동적 프로비저닝 PV의 기본값입니다. 그래서 동적 프로비저닝에서 PVC를 지우면 보통 데이터까지 사라지고, 이를 막으려면 삭제 전 policy를 Retain으로 바꿔야 합니다.
