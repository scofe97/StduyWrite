...
GKE `standard-rwo`와 필드 구조가 같고 provisioner만 다릅니다. GKE는 클라우드 디스크(pd)를 만들고, kind는 노드의 로컬 디렉터리를 씁니다. parameters가 아예 없는 것도 눈에 띄는데, local-path 프로비저너는 넘길 옵션이 없어서입니다. 그런데도 `storageClassName`을 비운 PVC 매니페스트는 두 클러스터에서 똑같이 동작합니다 — 기본 StorageClass가 다리를 놓아 주기 때문입니다.
