...
이 개념은 "감시 대상이 스스로 죽으면 다시 띄우고, 살아 있어도 주기적으로 안부를 물어 이상하면 갈아 끼운다"는 익숙한 헬스체크 발상입니다.

다만 쿠버네티스는 그 안부 묻기(probe)의 방식·주기·실패 판정 기준을 컨테이너 단위로 선언하게 하고, 재시작 자체도 "같은 컨테이너 재가동"이 아니라 "새 컨테이너 생성"이라는 점이 다릅니다.
...
첫 종료 때는 즉시 재시작되지만, 다음번에는 쿠버네티스가 10초를 기다린 뒤 재시작합니다.

이 지연은 이후 종료마다 20초·40초·80초·160초로 두 배씩 늘어나고, 그 뒤로는 5분으로 유지됩니다. 시도 사이에 두 배씩 늘어나는 이 지연을 **지수 백오프(exponential back-off)** 라 부릅니다.
...
liveness가 항상 실패하는 Pod를 띄워 재시작을 지켜보면 교과서의 `10초 → 20초 → 40초`가 곧바로 보이지 않습니다. `livenessProbe.exec: cat /tmp/never-exists`에 `failureThreshold: 1`을 걸고 `kubectl get pod -w`로 관찰한 결과입니다. 실측 재시작 시각(AGE에서 `X ago`를 빼 계산)은 이렇습니다.
...
프로세스가 종료되면 쿠버네티스가 재시작해 주지만, 애플리케이션은 종료되지 않은 채 응답 불능이 될 수도 있습니다. 예를 들어 메모리 누수가 있는 Java 애플리케이션은 `OutOfMemoryError`를 뱉기 시작하지만 JVM 프로세스는 계속 돕니다.

애플리케이션이 스스로 오류를 잡아 종료할 수도 있지만, 무한 루프나 데드락에 빠져 스스로 감지할 수 없다면 어떻게 할까요? 이런 경우에도 재시작되게 하려면 애플리케이션의 상태를 밖에서 확인해야 합니다.

**liveness probe**를 정의하면 쿠버네티스가 애플리케이션이 살아 있는지 확인합니다. Pod의 각 컨테이너마다 지정할 수 있고 쿠버네티스가 주기적으로 probe를 실행합니다.

애플리케이션이 응답하지 않거나, 오류가 나거나, 응답이 부정적이면 컨테이너는 비정상으로 간주돼 종료되고, restartPolicy가 허용하면 재시작됩니다.
...
### 실제 확인 — startup이 도는 동안 liveness는 침묵한다

"startup이 성공하기 전에는 liveness가 실행되지 않는다"는 규약을 직접 확인했습니다. 시작에 20초가 걸리는 앱을 흉내 내어(20초 뒤에야 `/tmp/ready` 파일을 만드는 busybox), startup과 liveness에 **똑같은** `cat /tmp/ready` 명령을 걸었습니다 — 파일이 없으면 둘 다 실패할 명령입니다.

```yaml
startupProbe:  { exec: {command: ["cat","/tmp/ready"]}, periodSeconds: 3, failureThreshold: 10 }
livenessProbe: { exec: {command: ["cat","/tmp/ready"]}, periodSeconds: 5, failureThreshold: 3 }
```

기동 20초 동안 파일이 없으니 두 probe 다 실패해야 할 것 같지만, 이벤트에는 한쪽만 남았습니다.

```bash
kubectl get events --field-selector involvedObject.name=startup-demo
```

```pseudocode
Warning  Unhealthy  Startup probe failed: cat: can't open '/tmp/ready': No such file or directory   (x7)
# Liveness probe failed 이벤트는 0건
```

같은 명령인데 **startup은 7번 실패 기록이 남고, liveness는 단 한 번도 실패하지 않았습니다** — liveness가 그 20초 동안 **아예 실행되지 않았기** 때문입니다. 그리고 startup이 7번 실패하는 사이에도 `RESTARTS`는 `0`이었습니다. 실패가 `failureThreshold`(10) 예산 안이라 "아직 뜨는 중"으로 취급돼 컨테이너를 죽이지 않은 것입니다. 20초 뒤 파일이 생겨 startup이 성공하자 비로소 liveness로 전환되고 `READY`가 `1/1`이 됐습니다.

![같은 cat /tmp/ready 명령을 걸어도 startup은 기동 20초 동안 7번 실패 기록이 남고 liveness는 그 사이 비활성이라 0건이며, startup 성공 후에야 liveness로 인계되는 시간 축 비교](_assets/06-02-startup-liveness-timeline.svg)

이것이 startup probe가 따로 있는 이유를 실물로 보여 줍니다. **기동 구간의 실패는 startup이 관대하게 흡수해 죽이지 않고, 뜨고 난 뒤의 빠른 장애 감지는 liveness가 맡는** 분업입니다.

하나의 probe로는 느린 기동 허용과 빠른 재시작이라는 두 요구를 동시에 만족할 수 없습니다. 매니페스트는 실습 저장소의 `06-status/pod-startup-probe.yaml`입니다.
...
**무엇을 확인할 것인가.** kiada 컨테이너의 probe는 루트 URI에 대한 단순 HTTP 응답만 확인하지만, 이런 단순한 probe도 서버가 HTTP 요청에 응답하지 않게 됐을 때(그것이 서버의 본업이므로) 재시작을 일으키니 없는 것보다 훨씬 낫습니다.

더 나은 확인을 위해 웹 애플리케이션은 보통 `/healthz` 같은 전용 헬스체크 엔드포인트를 노출하고, 호출되면 내부 주요 컴포넌트의 상태를 점검합니다.
...
Spring Boot 2.3+의 Actuator는 이 편의 원칙을 그대로 담은 전용 엔드포인트를 제공합니다. `management.endpoint.health.probes.enabled=true`를 켜면 `/actuator/health/liveness`와 `/actuator/health/readiness`가 열립니다. 쿠버네티스 환경에서는 자동으로 활성화됩니다.

중요한 것은 liveness 그룹에 기본적으로 DB·외부 서비스 인디케이터가 **포함되지 않는다**는 점입니다. §9의 "외부 요인을 liveness에서 확인하지 말라"는 원칙을 프레임워크가 구현한 것으로, DB가 잠깐 죽었다고 앱을 재시작해 봐야 연쇄 재시작만 일어나기 때문입니다.

DB 연결 상태는 readiness 그룹에 넣어 트래픽만 차단하고, liveness는 애플리케이션 컨텍스트의 내부 상태(`LivenessState.CORRECT`)만 봅니다.

**startup probe는 JVM 앱에서 특히 실용적입니다.** Spring Boot 앱은 클래스 로딩·빈 초기화·커넥션 풀 워밍업으로 시작에 수십 초가 걸리는 일이 흔한데, 이 시간을 liveness의 `initialDelaySeconds`로 때우면 정상 운영 중 장애 감지도 그만큼 느려집니다. §8의 패턴대로 `periodSeconds: 10` × `failureThreshold: 12` 짜리 startup probe로 시작 구간을 분리하면 됩니다. 그러면 무거운 배치성 초기화가 있는 앱도 재시작 루프에 갇히지 않으면서 운영 중에는 5~10초 안에 비정상을 잡습니다.

exec probe로 `java -jar healthcheck.jar` 같은 것을 돌리는 안티패턴은 §9의 경고 그대로입니다 — probe마다 JVM을 새로 띄우면 그 비용이 컨테이너 자원 쿼터를 갉아먹습니다.

**`CrashLoopBackOff`와 exit code 137의 조합은 Spring 운영에서 가장 흔한 장애 신호입니다.** `kubectl describe`의 `Last State`에서 exit code 137 에 reason 이 OOMKilled면 메모리 한도 문제입니다 — 02-01의 cgroup·힙 설정을 참고합니다. exit code 1이면 시작 실패한 애플리케이션 예외이므로 `kubectl logs -p`로 이전 컨테이너의 스택트레이스를 봐야 합니다.

지수 백오프 때문에 "5분째 안 뜨는" 것처럼 보여도 실제로는 대기 중일 수 있으니, `message` 필드의 back-off 시간을 먼저 확인하는 습관이 진단 시간을 줄여 줍니다.
