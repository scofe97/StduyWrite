...
이것이 startup probe가 따로 있는 이유를 실물로 보여 줍니다 — **기동 구간의 실패는 startup이 관대하게 흡수(죽이지 않음)하고, 뜨고 난 뒤의 빠른 장애 감지는 liveness가 맡는** 분업입니다. 하나의 probe로는 이 두 요구(느린 기동 허용 + 빠른 재시작)를 동시에 만족할 수 없습니다. (매니페스트: 실습 저장소 `06-status/pod-startup-probe.yaml`)
