...
# 타입 스펙: 같은 30회가 경로에 따라 어떻게 갈리는지가 요점이라, 두 경로를 위아래로 두고
...
ddx.band(d, 104, 612, "한 요청을 셋에 복제하는 것이 아니라 연결 하나가 하나로 간다")
...
y2 = panel(400, "② 클러스터 안에서 ClusterIP 로 — 노드 데이터패스가 고른다",
           "새 연결마다 endpoint 하나를 골라 DNAT 한다. kube-proxy 가 미리 규칙을 깔아 둔다.",
           [7, 10, 13], OK)
d.chip(700, 400 + 8, "세 Pod 로 분산", OK, 11)

d.line(36, 372, 964, 372, RULE, 0.8)

d.t(36, 552, "표본이 30회라 13/10/7 정도의 편차는 난다 — iptables 규칙이 확률적으로 고르기 때문이다.",
     12, MUTED, KR, "start")
d.t(36, 576, "그중 하나는 다른 노드(worker2)였다 — 분산은 노드 경계를 넘는다.", 12, MUTED, KR, "start")
d.legend(628, [("분산이 발동하지 않는 경로", BAD), ("분산이 발동하는 경로", OK)])
