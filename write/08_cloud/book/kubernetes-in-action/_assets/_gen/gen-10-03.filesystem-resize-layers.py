...
# 타입 스펙: type-layers.md — 본문과 이 도식의 대체텍스트가 다 "2 층 구조"라 부른다 — 아래가 블록 디바이스,
#           위가 파일시스템이고 위아래 순서가 곧 논지다. 층마다 그것을 넓히는 주체와 시점이 다르다는 것이
#           이 그림이 더하는 것이다.
#           정본은 같은 x·같은 폭의 띠 4~6 겹이 계약인데 여기는 두 겹이다 — 본문이 두 층이라 못박으므로
#           겹을 늘리지 않는다. 02-01.copy-on-write 도 같은 두 겹 형태다.
#           2026-08-29 정정: 처음에 process 로 적었으나, 본문이 "층" 이라 부르는 것을 단계 프레임워크로
#           라벨하면 본문과 도식이 다른 말을 하게 된다.
