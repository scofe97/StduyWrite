...
# 타입 스펙: type-architecture.md — 노드 둘이 경계 상자이고 붉은 점선이 그 경계를 건너간다. 앞 도식과 반대로 경계를 넘는 것이
#           실패의 증거다.
...
# y=424 는 노드 상자(200~430) 안이라 상자 아래 변이 글자를 관통했다. 건너가는 점선은 상자를
# 넘어야 하지만 그 설명 글까지 넘을 이유는 없으므로 두 상자 아래로 내린다.
d.t(590, 456, "Service 는 전역 엔드포인트에서 고른다", 11, BAD, KR)
