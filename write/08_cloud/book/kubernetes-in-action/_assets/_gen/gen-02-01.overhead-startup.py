...
# 타입 스펙: type-bar.md — 본문에 비율 수치가 없으므로 막대 차트를 쓰면 없는 정밀도를 지어내게 된다.
#           대신 같은 크기의 자리를 세는 칸 그림으로 둔다 — 자리 개수는 본문이 말하는
#           사실이고("VM 은 앱 2개가 한계"), 눈으로 셀 수 있다.
#           같은 서버에 들어가는 앱 수를 세는 그림이라 이산 수량 비교다. 다만 본문에 비율 수치가
#           없어 값 축과 눈금 대신 같은 크기의 칸을 세게 두었다 — 정본의 막대 문법과 어긋나는 자리다.
#           아래 단계 막대는 같은 출발선을 공유해 gantt 처럼 읽히지만, 이 그림의 값은 위쪽 칸 개수다.
