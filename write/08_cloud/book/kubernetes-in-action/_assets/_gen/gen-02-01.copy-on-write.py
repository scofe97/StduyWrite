...
# 타입 스펙: type-layers.md — 위아래 층이 요점이라 층 그림. 공유 베이스가 두 컨테이너 밑에 하나로 깔리는
#           구조 자체가 답이므로, 베이스를 두 열에 걸치는 한 칸으로 그린다.
#           읽기 전용 베이스 위에 쓰기 레이어가 얹히는 위아래 순서가 곧 논지다.
#           정본은 같은 x·같은 폭의 띠 4~6 겹이 계약인데 여기는 두 겹이고 위층만 컨테이너별로
#           갈린다 — 공유되는 층이 하나라는 사실을 폭으로 보이려고 일부러 어긋낸 자리다.
