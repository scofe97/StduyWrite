...
# 세 산출물로 비스듬히 뻗는 대신 줄기 하나를 내려 가로 버스에서 갈라낸다. 버스(y=380)는
# 위 캡션(baseline 366)과 아래 상자(top 400) 사이를 지나고, 줄기는 캡션 글자 폭(x 443~757)을 비켜
# x=830 으로 내려온다.
d.path("M 830 274 L 830 380", OK, 1.3)
d.path("M 320 380 L 880 380", OK, 1.3)
for i, (nm, s) in enumerate((("StatefulSet", "파드와 PVC"), ("Service", "headless · 신원"),
                             ("Secret", "자격 증명"))):
    ddx.node(d, 320 + i * 280, 434, nm, s, 240, 68, INFO)
    d.path(f"M {320 + i*280} 380 L {320 + i*280} 396", OK, 1.3, m="ok")

d.path("M 40 178 L 40 168 L 640 168 L 640 178", ACC, 1.2)
d.t(340, 156, "사용자 손이 닿는 곳", 11, ACC, KR)
