...
> **실습에서 확인한 것 (kind k8s-lab, v1.35).** 지원하지 않는 필드로 필터를 걸면 kubectl이 조용히 빈 목록을 주는 게 아니라 `field label not supported: spec.containers[0].image`처럼 어느 필드가 안 되는지 짚어 에러로 알려줍니다. 어떤 필드가 되는지 외우지 않고 일단 써 보는 방식이 여기서 유효합니다. 손으로 돌려 본 STEP별 출력은 실습 저장소 [k8s_in_action/07-organizing-objects/fields](../../../../../k8s_in_action/07-organizing-objects/fields/README.md)에 박제해 두었습니다.
