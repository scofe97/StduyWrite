...
`kind-k8s-lab`(v1.35)에서 `app.conf`·`db.conf` 두 엔트리를 담은 볼륨의 실제 구조를 `ls -lA`로 보면, 바깥의 `app.conf`·`db.conf`·`..data`는 모두 `l`로 시작하는 심링크이고 진짜 파일은 타임스탬프 디렉터리(`..2026_07_15_05_29_12`) *안*에 `-rw-r--r--`로 들어 있습니다.

- ConfigMap을 `after`로 수정하자 새 디렉터리 `..2026_07_15_05_30_25`가 생기고 `..data` 심링크가 그 새 디렉터리를 가리키도록 바뀌었습니다.
- 반면 `app.conf`·`db.conf` 심링크의 mtime은 처음 그대로 `05:29`였습니다. 그러면서 두 파일 내용이 `before`에서 `after`로 *함께* 바뀌었습니다. 개별 파일 심링크는 손대지 않고 `..data` 하나만 새 디렉터리로 튼다는 물증입니다.
