...
이 경계는 실무에서 특히 헷갈리므로 한 번 더 못박습니다. emptyDir 디렉터리의 이름에는 `pod_UID`가 들어갑니다. 그래서 **같은 파드가 유지되는 한** — 컨테이너가 죽어 재시작해도 — `pod_UID`가 그대로라 같은 디렉터리를 다시 마운트합니다. 그 덕에 데이터가 살아남습니다. 반면 파드 자체가 교체되면 이야기가 달라집니다.

파드는 세 경우에 교체됩니다. `kubectl delete pod`로 직접 지울 때, Deployment가 롤아웃(이미지 변경·설정 변경)으로 파드를 갈아 끼울 때, 그리고 노드 장애로 파드가 다른 노드로 옮겨질 때입니다. 이 셋 다 이전 파드는 사라지고 **새 `pod_UID`를 가진 새 파드**가 생깁니다. 새 `pod_UID`는 곧 새 디렉터리 경로를 뜻합니다. 그래서 새 파드는 언제나 빈 emptyDir에서 시작합니다.

여기서 실전 함정이 나옵니다. MongoDB 같은 상태 저장 앱을 `Deployment` + `emptyDir`로 굴리면, 롤아웃이 일어날 때마다 데이터가 소멸합니다 — 이미지 태그 하나만 바꿔도 파드가 교체되어 새 빈 디렉터리에서 다시 시작하기 때문입니다. emptyDir는 "파드 생애 동안의 임시 작업 공간"이지 영속 저장소가 아니라는 뜻입니다. 이 문제를 제대로 푸는 것이 [10장의 퍼시스턴트 볼륨](./10-01.PV%C2%B7PVC%C2%B7StorageClass%EC%99%80%20%EB%8F%99%EC%A0%81%20%ED%94%84%EB%A1%9C%EB%B9%84%EC%A0%80%EB%8B%9D.md)(파드 교체와 무관하게 살아남는 저장소)과 16장의 StatefulSet(파드마다 고정된 자기 볼륨을 갖는 워크로드)입니다.
