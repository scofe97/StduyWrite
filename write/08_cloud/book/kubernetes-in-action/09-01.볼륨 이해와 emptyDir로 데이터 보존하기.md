...
이 개념은 [05-03편의 init 컨테이너·사이드카](./05-03.%EB%A9%80%ED%8B%B0%20%EC%BB%A8%ED%85%8C%EC%9D%B4%EB%84%88%C2%B7init%C2%B7%EB%84%A4%EC%9D%B4%ED%8B%B0%EB%B8%8C%20%EC%82%AC%EC%9D%B4%EB%93%9C%EC%B9%B4%EC%99%80%20%EC%82%AD%EC%A0%9C.md)에서 다룬 "파드 = 컨테이너의 논리적 묶음"을 파일시스템 축으로 이어받은 것입니다. 컨테이너들은 CPU·RAM·네트워크 인터페이스를 공유합니다. 다만 일반 컴퓨터의 프로세스와 달리 *파일시스템은 각자 격리된 것*을 씁니다. 각 컨테이너의 파일시스템은 이미지가 제공합니다.

파드는 하나의 애플리케이션을 돌리는 작은 논리적 컴퓨터에 가깝습니다. 그 애플리케이션은 여러 컨테이너로 이뤄질 수 있습니다. 각 컨테이너는 자기 이미지에서 온 파일로 파일시스템을 시작합니다. 프로세스가 그 파일을 고치거나 새로 만들 수는 있습니다. 다만 [06장에서 본 파드 생애주기](./06-01.Pod%20%EC%83%81%ED%83%9C%20%E2%80%94%20phase%C2%B7conditions%C2%B7%EC%BB%A8%ED%85%8C%EC%9D%B4%EB%84%88%20%EC%83%81%ED%83%9C.md)대로, 컨테이너가 종료되고 재시작하면 그 변경은 전부 사라집니다.

왜 사라질까요? 컨테이너는 *진짜로 재시작되는 게 아니라 새 인스턴스로 교체*되기 때문입니다. 새 컨테이너의 파일시스템은 다시 이미지 상태에서 출발합니다. 그래서 컨테이너화된 애플리케이션은 재시작하면 멈췄던 지점에서 이어갈 수 없습니다. 어떤 애플리케이션은 이래도 괜찮습니다. 반면 파일시스템의 전체나 일부를 재시작 너머로 보존해야 하는 애플리케이션도 있습니다. 이때 파드에 볼륨을 붙여 컨테이너에 마운트합니다.
...
이 장에서는 Kiada 애플리케이션에 새 **Quiz 서비스**를 붙입니다. Kiada 웹 앱이 표시할 객관식 문제를 제공하고 사용자의 응답도 저장하는 서비스입니다. 지속 저장이 필요하므로, 볼륨이 왜 없으면 안 되는지를 직접 보여 주는 예제가 됩니다.
...
주의할 점은, quiz 파드는 *여전히 같은 파드*라는 것입니다. `quiz-api` 컨테이너는 이 시간 내내 멀쩡히 돌았습니다. `mongo` 컨테이너만 재시작됐는데, 더 정확히는 *재생성*됐습니다. 이번엔 셧다운으로 유발했지만 어떤 이유로든 일어날 수 있습니다.

이런 데이터 손실은 받아들일 수 없습니다. 데이터가 컨테이너 재시작을 넘어 살아남으려면 컨테이너 *바깥*, 즉 볼륨에 저장돼야 합니다.
...
볼륨은 파드나 노드 같은 최상위 리소스가 아니라 *파드 안의 구성 요소*입니다. 파드의 생애주기를 공유합니다. 볼륨은 파드 수준에서 정의되고 컨테이너의 원하는 위치에 마운트됩니다. 볼륨의 생애주기는 컨테이너가 아니라 파드 전체에 묶이므로, 볼륨은 컨테이너 재시작을 넘어 데이터를 보존하는 데 쓸 수 있습니다.

파드의 모든 볼륨은 파드가 세팅될 때 만들어집니다. 어느 컨테이너보다도 *먼저*이며, 파드가 종료될 때 함께 허물어집니다. 컨테이너가 (재)시작될 때마다 그 컨테이너가 쓰도록 설정된 볼륨이 컨테이너 파일시스템에 마운트됩니다. 볼륨과 마운트가 쓰기 가능으로 설정돼 있으면 컨테이너 안 애플리케이션이 볼륨을 읽고 쓸 수 있습니다.
...
볼륨은 파드 생애주기에 묶여 파드가 사는 동안만 존재합니다. 다만 볼륨 *타입*에 따라서는 파드와 볼륨이 사라진 뒤에도 파일이 온전히 남아 새 볼륨에 다시 마운트될 수 있습니다. 파드 볼륨이 파드 바깥의 지속 저장소, 보통 네트워크 연결 스토리지(NAS)에 매핑되는 경우입니다. 이때 데이터는 지속적이므로 파드가 다른 워커 노드의 새 파드로 교체돼도 애플리케이션이 이전 데이터를 쓸 수 있습니다.
...
특정 클라우드 제품에 묶인 in-tree 타입은 폐기됐습니다. `gcePersistentDisk`·`awsElasticBlockStore`·`azureFile`/`azureDisk`가 여기 해당합니다. 공식 문서의 해당 절 제목에 `(deprecated)`가 붙어 있습니다. 이들의 동작은 대응하는 CSI 드라이버로 넘어갔습니다. 파드에는 보통 `persistentVolumeClaim` 볼륨을 대신 씁니다.

`nfs`는 이 목록에 넣지 않습니다. 특정 벤더 제품이 아니라 표준 프로토콜이라 폐기 대상이 아니고, 공식 문서에도 deprecation 표시가 없습니다. 다만 파드 매니페스트에 NFS 서버 주소를 직접 박는 방식은 이식성이 떨어지므로, 실무에서는 같은 NFS를 PV·PVC 뒤에 두는 편이 낫습니다.
...
같은 quiz 파드를 세 형태로 돌려 보면 이 두 줄의 무게가 드러납니다. kind(k8s-lab, v1.35)에서 실제로 돌려 봤습니다. 볼륨 없는 파드는 mongo 재시작 후 질문 수가 0으로 증발했습니다. emptyDir를 붙인 파드는 1로 보존됐습니다. init 컨테이너까지 붙이면 파드가 뜰 때 질문 6개가 자동으로 채워졌습니다.

![볼륨 유무에 따른 데이터 생존 — 세 단계 실습](_assets/09-01-volume-three-stages.svg)
...
이 경로는 kubelet의 구현 세부이지 보장된 API가 아닙니다. 실습에서 눈으로 확인하는 용도로는 유용하지만, 스크립트가 이 경로를 전제로 동작하게 짜지는 않습니다.
...
실제로 kind의 워커 노드(`k8s-lab-worker`)에서 이 경로를 열어 보면, 노드 디렉터리의 `ls` 결과가 컨테이너 안 `/data/db`의 `ls`와 완전히 같았습니다. MongoDB가 만든 `WiredTiger`·`_mdb_catalog.wt`는 물론 `collection-5b3e9bdf-….wt` 같은 UUID 파일명까지 한 글자도 다르지 않았습니다. 마운트가 별도의 복사본을 만드는 게 아니라, 노드의 한 디렉터리라는 **같은 실체**를 두 곳에서 보여주기 때문입니다.

![emptyDir 볼륨의 노드 저장 위치와 마운트](_assets/09-01-emptydir-node-path.svg)

다만 데이터가 컨테이너 재시작은 넘어도 *파드 삭제는 넘지 못합니다*. 파드를 지우면 이 디렉터리도 지워집니다. 데이터를 제대로 지속하려면 다음 장의 퍼시스턴트 볼륨이 필요합니다.

### 컨테이너 재시작은 넘고 파드 교체는 못 넘는 경계

이 경계는 실무에서 특히 헷갈리므로 한 번 더 못박습니다. emptyDir 디렉터리의 이름에는 `pod_UID`가 들어갑니다. 그래서 **같은 파드가 유지되는 한** `pod_UID`가 그대로이고, 컨테이너가 죽어 재시작해도 같은 디렉터리를 다시 마운트합니다. 그 덕에 데이터가 살아남습니다. 반면 파드 자체가 교체되면 이야기가 달라집니다.

파드가 교체되는 경로는 여럿입니다. 흔한 것만 꼽아도 이렇습니다.

- `kubectl delete pod`로 직접 지울 때
- Deployment가 롤아웃(이미지 변경·설정 변경)으로 파드를 갈아 끼울 때
- 노드 장애로 파드가 다른 노드로 옮겨질 때
- 노드의 메모리·디스크 압박으로 kubelet이 파드를 축출할 때
- 우선순위가 높은 파드에 자리를 내주고 선점당할 때
- `kubectl drain`이나 taint 기반 축출로 노드에서 밀려날 때

공식 문서가 조건을 열거하지 않고 "파드가 **어떤 이유로든** 노드에서 제거되면 emptyDir 데이터는 영구히 삭제된다"고 적는 이유가 여기 있습니다. 목록을 외우는 것보다 경계를 기억하는 편이 낫습니다 — 위 어느 경로든 이전 파드는 사라지고 **새 `pod_UID`를 가진 새 파드**가 생깁니다. 새 `pod_UID`는 곧 새 디렉터리 경로를 뜻하므로, 새 파드는 언제나 빈 emptyDir에서 시작합니다.

여기서 실전 함정이 나옵니다. MongoDB 같은 상태 저장 앱을 `Deployment` + `emptyDir`로 굴리면 롤아웃이 일어날 때마다 데이터가 소멸합니다. 이미지 태그 하나만 바꿔도 파드가 교체되어 새 빈 디렉터리에서 다시 시작하기 때문입니다.

emptyDir는 "파드 생애 동안의 임시 작업 공간"이지 영속 저장소가 아니라는 뜻입니다. 이 문제를 제대로 푸는 것이 [10장의 퍼시스턴트 볼륨](./10-01.PV%C2%B7PVC%C2%B7StorageClass%EC%99%80%20%EB%8F%99%EC%A0%81%20%ED%94%84%EB%A1%9C%EB%B9%84%EC%A0%80%EB%8B%9D.md)(파드 교체와 무관하게 살아남는 저장소)과 16장의 StatefulSet(파드마다 고정된 자기 볼륨을 갖는 워크로드)입니다.
...
메모리에 만드는 것은 민감 데이터를 다룰 때도 좋습니다. 디스크에 쓰이지 않아, 데이터가 원하지 않게 오래 남거나 노출될 확률이 줄기 때문입니다. 쿠버네티스가 Secret 데이터를 컨테이너에 노출할 때 쓰는 것과 같은 접근입니다.

#### tmpfs는 공짜가 아니다

빠르고 안전해 보이지만 대가가 있습니다. **tmpfs에 쓴 파일은 그 파일을 쓴 컨테이너의 메모리 한도에 계상됩니다.**[^tmpfs-mem] 디스크에 쓸 때와 달리 파일 크기가 곧 메모리 사용량이므로, 한도에 걸리면 애플리케이션이 메모리를 더 쓴 것이 없는데도 OOM으로 죽습니다. JVM이라면 힙과 tmpfs 사용량을 **합쳐서** `limits.memory`를 잡아야 한다는 뜻입니다.

볼륨 크기는 `sizeLimit`으로 제한합니다.
...
volumes:
  - name: content
    emptyDir:
      medium: Memory
      sizeLimit: 64Mi    # 이 볼륨이 먹을 수 있는 상한
```

`sizeLimit`이 가장 중요해지는 쪽은 직관과 반대입니다. 파드에 메모리 한도가 **걸려 있을 때**가 아니라 **안 걸려 있을 때**입니다. 한도가 있으면 tmpfs가 커져도 그 한도에서 멈추지만, 한도가 없으면 상한이 사라져 노드의 가용 메모리를 전부 먹을 수 있습니다. 공식 문서가 이 상황을 서비스 거부(DoS)와 OS의 OOM 처리로 이어진다고 경고합니다.[^emptydir-size] `sizeLimit`을 아예 안 적으면 메모리 기반 볼륨의 크기는 **노드 allocatable 메모리**가 됩니다.

한 가지 더 있습니다. `sizeLimit`은 기본 매체(노드 디스크)에서도 상한을 정하지만, 그 저장 공간은 노드의 ephemeral storage에서 나옵니다. 로그 파일이나 이미지 레이어가 그 공간을 먼저 채우면 **`sizeLimit`에 닿기 전에** 볼륨이 먼저 고갈될 수 있습니다.
...
파일을 볼륨에 넣는 한 방법이 [init 컨테이너](./05-03.%EB%A9%80%ED%8B%B0%20%EC%BB%A8%ED%85%8C%EC%9D%B4%EB%84%88%C2%B7init%C2%B7%EB%84%A4%EC%9D%B4%ED%8B%B0%EB%B8%8C%20%EC%82%AC%EC%9D%B4%EB%93%9C%EC%B9%B4%EC%99%80%20%EC%82%AD%EC%A0%9C.md)입니다. init 컨테이너는 파일을 어디서든 가져올 수 있습니다. Git 저장소를 clone할 수도 있고 자기 이미지에 파일을 담아 둘 수도 있습니다.

여기서는 질문을 JSON 파일로 담은 이미지를 만듭니다. 그 파일을 공유 볼륨에 복사해 MongoDB가 시작 시 읽게 합니다.

![init 컨테이너로 emptyDir 볼륨을 초기화하는 흐름](_assets/09-01-init-container-volume-init.svg)

파드가 시작되면 볼륨이 먼저 만들어지고, 그다음 init 컨테이너가 시작됩니다. 이는 매니페스트에서 볼륨을 init 컨테이너보다 앞에 정의하든 뒤에 정의하든 마찬가지입니다.

init 컨테이너 시작 전 `initdb` 볼륨이 마운트되고, 컨테이너 이미지 안의 `insert-questions.js` 파일이 볼륨으로 복사됩니다. 복사가 끝나면 init 컨테이너가 종료되고 본 컨테이너들이 시작됩니다. `initdb` 볼륨은 MongoDB가 초기화 스크립트를 찾는 위치에 마운트돼, 첫 시작 시 스크립트가 실행되어 질문이 삽입됩니다.
...
컨테이너는 재시작될 때 *진짜로 재시작되는 게 아니라 새 인스턴스로 교체*됩니다. 새 컨테이너의 파일시스템은 이미지에서 다시 만들어지므로 이전 컨테이너가 쓴 파일은 모두 사라집니다. 그래서 재시작을 넘어 살아남아야 하는 데이터는 컨테이너 파일시스템이 아니라 *볼륨*에 둬야 합니다. 볼륨은 생애주기가 컨테이너가 아닌 파드에 묶인 저장 공간입니다. 볼륨은 컨테이너가 (재)시작될 때마다 다시 마운트되므로 새 컨테이너가 이전 데이터에 이어서 접근합니다.
...
[^tmpfs-mem]: Kubernetes 공식 문서 [Volumes / emptyDir](https://kubernetes.io/docs/concepts/storage/volumes/#emptydir): "While tmpfs is very fast, be aware that, unlike disks, files you write count against the memory limit of the container that wrote them."

[^emptydir-size]: 같은 절에 크기 기본값과 고갈 조건이 함께 적혀 있습니다 — "If no size is specified, memory-backed volumes are sized to node allocatable memory." / "The storage is allocated from node ephemeral storage. If that is filled up from another source, the `emptyDir` may run out of capacity before this limit."

    한도가 없을 때의 위험은 [Resource Management for Pods and Containers](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#memory-backed-emptydir)가 경고합니다 — "If you do not set a memory limit, the pod has no upper bound on memory consumption, and can consume all available memory on the node. This can result in a denial of service."
