...
이 문서를 읽고 나면 Ingress·Ingress 컨트롤러·리버스 프록시 세 요소의 역할을 구분할 수 있습니다. 호스트·경로 규칙으로 여러 서비스를 한 IP에 노출하는 법도, pathType 세 종류의 매칭 규칙과 default backend도 근거와 함께 설명할 수 있습니다.
...
Ingress는 클러스터 애드온인 Ingress 컨트롤러가 제공합니다. GKE는 GLBC, AWS는 AWS Load Balancer Controller, Azure는 AGIC를 제공합니다. 없으면 직접 설치합니다. 원서는 쿠버네티스 메인테이너가 만든 Nginx Ingress 컨트롤러를 씁니다. Nginx 저작자판과는 별개이고, 입문자는 전자로 시작하는 편이 낫습니다.
...
```pseudocode
1. 컨트롤러 설치  →  2. IngressClass 자동 생성  →  3. Ingress가 그 이름을 지목
```

앞의 둘이 먼저 서 있어야 셋째가 의미를 갖습니다. 다음 절에서 "ADDRESS가 안 뜨면 `ingressClassName`을 확인하라"고 하는 증상이 이 순서를 어겼을 때 나옵니다 — 없는 이름을 부르면 아무 컨트롤러도 자기 일이라 여기지 않고, 그래서 에러 대신 침묵이 옵니다.
...
ADDRESS가 클라이언트가 요청을 보낼 L7 프록시의 IP입니다. 같은 값이 `status.loadBalancer.ingress`에도 있습니다. 주소가 몇 분 뒤에도 안 뜨면 어떤 컨트롤러도 이 Ingress를 처리하지 않은 것이니, 컨트롤러 실행 여부와 `ingressClassName` 지정 여부를 확인합니다.
...
그래서 `example.com` 자체까지 받으려면 규칙을 따로 써야 합니다. 와일드카드 문법은 [§6](#6-규칙-여럿을-한-ingress로-통합)에서 다시 다룹니다.
...
이 판정이 §5의 경로 매칭과 **같은 요소 단위 원리**입니다. 요소 분해를 나란히 놓은 표는 [§5](#5-경로-기반-라우팅과-pathtype)에 있습니다. host 생략은 와일드카드보다도 넓은 catch-all이라, 다음 절의 defaultBackend와 짝을 이룹니다.
...
세 요소는 설정을 담는 Ingress API 오브젝트, 트래픽을 백엔드로 넘기는 L7 로드밸런서(리버스 프록시), 그리고 API를 감시하며 프록시를 배치·설정하는 Ingress 컨트롤러입니다. "Ingress 컨트롤러"라는 말은 흔히 프록시와 컨트롤러를 한 덩어리로 가리킵니다. 그러나 둘은 별개 컴포넌트입니다. 컨트롤러는 감시·설정 주체이고, 프록시는 실제 HTTP 트래픽을 처리하는 주체입니다.
...
- ⭐ **두 편을 한 장으로** — 12-02 §「12장 전체 — 요청 하나가 지나는 길」에 브라우저부터 파드까지의 전 구간을 그린 도식이 있습니다. 이 편에서 다룬 것들(컨트롤러↔프록시 분리·`pathType`·파드 IP 직접·default backend)이 각각 어디서 작동하는지도 그 그림에 함께 표시돼 있습니다.
...
- [04-06. Ingress와 Gateway API](../../kubernetes/04_networking/04-06.Ingress%EC%99%80%20Gateway%20API.md) — Ingress 통합 개념본(책 비종속)
- [04-04. Service와 EndpointSlice](../../kubernetes/04_networking/04-04.Service%EC%99%80%20EndpointSlice.md) — Ingress가 참조하는 Service·엔드포인트
