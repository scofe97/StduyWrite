...
ReplicaSet의 개념 모델·컨트롤러·reconciliation은 [핵심 워크로드 통합 개념본](../../kubernetes/01_workloads/01-01.%ED%95%B5%EC%8B%AC%20%EC%9B%8C%ED%81%AC%EB%A1%9C%EB%93%9C.md)이 크게 다룹니다. 이 편은 책의 실습(Kiada 서비스·kubectl 실측·소유권) 관점에서 차이분을 정리합니다.
...
> **공식 문서와 대조.** `replicas`를 생략하면 1이 된다는 서술은 API 레퍼런스가 그대로 확인해 줍니다 — "This is a pointer to distinguish between explicit zero and unspecified. Defaults to 1."[^api-ref] 다만 **필수 필드 표시는 책과 갈립니다**. 레퍼런스에서 필수(`*`) 표시가 붙은 것은 `selector` 하나뿐이고 `template`에는 없습니다.[^api-ref] 실질적으로는 template 없이 만든 ReplicaSet이 파드를 만들 수 없어 쓸모가 없으니, 책의 "둘 다 필수"는 *실용적 서술*로 읽는 편이 맞습니다. 스키마상 강제되는 것은 selector 뿐입니다.
>
...
>
> `fullyLabeledReplicas`를 단순히 "현재 수"로 읽으면 어긋납니다 — 이 필드는 *템플릿 label 을 온전히 갖춘* 파드만 셉니다. 그래서 adopt 된 파드의 label 이 템플릿과 다르면 `replicas` 와 `fullyLabeledReplicas` 가 벌어질 수 있고, 그 차이 자체가 "이 세트에 템플릿 밖에서 흘러든 파드가 있다"는 신호가 됩니다. `readyReplicas` 와 `availableReplicas` 의 차이는 `minReadySeconds` 대기분이라, 롤아웃 중 둘이 다른 것은 정상입니다.
...
> **공식 문서로 보강.** 어노테이션을 안 붙인 파드의 암묵값은 **0**이고 음수도 허용해, 범위는 `[-2147483648, 2147483647]`입니다. 특정 파드를 마지막까지 남기려면 양수를, 먼저 지우려면 음수를 줍니다. 이 기능은 v1.22부터 **beta(기본 활성)**이며 `PodDeletionCost` 피처 게이트로 끌 수 있습니다.[^deletion-cost]
>
