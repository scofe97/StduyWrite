...
PV를 리사이즈하려면 PVC를 쓰는 파드를 지우고 다시 만들어야 합니다. 그러면 청구와 볼륨이 새 크기(10Gi)로 표시됩니다. 리사이즈는 새 크기를 지정하고 파드를 재시작하면 끝나는 간단한 작업입니다.
...
PVC·PV가 만들어지면 파드가 스케줄되고 PV가 `/mnt/ephemeral`에 마운트됩니다. ephemeral PVC의 생애주기는 파드에 묶여, 파드를 지우면 PVC가 지워지고, PV의 retain policy가 Delete라 PV도 지워집니다.
...
- [08_cloud/kubernetes — 스토리지와 상태](../../kubernetes/01_foundation/01-03.%EC%8A%A4%ED%86%A0%EB%A6%AC%EC%A7%80%EC%99%80%20%EC%83%81%ED%83%9C.md) — 스토리지 전반의 통합 개념본
