...
---
> **PVC는 크기만 나중에 키울 수 있습니다.** 확장은 블록 디바이스와 파일시스템 두 층에서 따로 일어나며, 온라인 확장을 지원하면 파드를 재시작하지 않고 끝납니다. VolumeSnapshot으로 볼륨을 백업·복제하고, ephemeral 볼륨으로 파드 전용 PV를 자동 생성·삭제할 수 있습니다.



> 이 문서를 읽고 나면 PVC 리사이즈가 두 층으로 나뉘는 이유와 파드 재시작이 필요한 경우를 구분해 진단하고, VolumeSnapshot·VolumeSnapshotClass·VolumeSnapshotContent 세 오브젝트의 관계를 PVC·PV에 빗대어 설명하며, ephemeral 볼륨이 emptyDir보다 나은 점을 근거와 함께 말할 수 있습니다.
...
[10-01](./10-01.PV%C2%B7PVC%C2%B7StorageClass%EC%99%80%20%EB%8F%99%EC%A0%81%20%ED%94%84%EB%A1%9C%EB%B9%84%EC%A0%80%EB%8B%9D.md)·[10-02](./10-02.%EC%A0%95%EC%A0%81%20%ED%94%84%EB%A1%9C%EB%B9%84%EC%A0%80%EB%8B%9D%EA%B3%BC%20node-local%20%EB%B3%BC%EB%A5%A8.md)에서 두 가지를 배웠습니다. 스토리지를 정적·동적으로 프로비저닝해 파드에 붙이는 법, 그리고 기존 PVC 를 `dataSourceRef` 로 참조해 볼륨을 복제하는 법입니다.

이 편은 복제 없이 기존 PV 를 다룹니다. 세 가지 작업입니다.

1. 이미 바인딩된 볼륨의 크기를 키웁니다.
2. 그 볼륨의 스냅샷을 만들어 백업합니다.
3. 만든 스냅샷에서 새 PV 를 복원합니다.



## 1. PVC 리사이즈

> PVC의 spec은 대부분 생성 후 불변이지만, 크기(resources.requests)는 바인딩 후에도 키울 수 있습니다. 확장은 블록 디바이스와 파일시스템 두 층을 거치며, 온라인 확장이 안 되는 경우에만 파드 재시작이 필요합니다.

사용자는 PVC 와 StorageClass 로 볼륨을 self-provision 합니다. 다만 나중에 다른 StorageClass 로 바꾸려면 새 PVC 를 만들어야 합니다. 쿠버네티스는 PVC 의 storageClassName 수정을 지원하지 않아, 시도하면 이런 에러가 납니다.
...
이 조건이 떴다면 파드를 지우고 다시 만들어 재마운트를 유발합니다. 그러면 청구와 볼륨이 새 크기(10Gi)로 표시됩니다. 다만 **재시작이 언제나 필요한 것은 아닙니다.** 왜 그런지는 두 층을 갈라 봐야 보입니다.

### 왜 CAPACITY가 바로 안 바뀌는가 — 2층 확장

리사이즈가 한 번에 끝나지 않는 이유는 스토리지가 두 층으로 나뉘어 있기 때문입니다. 두 층의 확장 시점이 서로 다릅니다. 아래 층인 **블록 디바이스**는 CSI 컨트롤러가 파드와 무관하게 즉시 키웁니다. 반면 그 위 **파일시스템**은 자기 메타데이터에 옛 크기를 기록 중이라 디바이스가 커진 것을 스스로 모릅니다. `resize2fs`(ext4)·`xfs_growfs`(xfs)로 "너 커졌다"를 알려 줘야 합니다.

이 두 번째 단계가 언제 일어나는지가 갈림길입니다. 공식 문서는 **파드가 시작될 때** 또는 **파드가 도는 중에** 일어난다고 적습니다 — 후자는 밑바탕 파일시스템이 온라인 확장을 지원할 때입니다.[^online-resize]

| 경로 | 조건 | 파드 재시작 |
|---|---|---|
| 온라인 확장 | 밑바탕 파일시스템·드라이버가 지원 | 필요 없음. 확장이 끝나는 즉시 파드가 넓어진 볼륨을 쓴다 |
| 오프라인 확장 | 온라인 확장을 지원하지 않음 | 필요. 재마운트 시점에 파일시스템이 확장된다 |

사용 중인 PVC를 확장하는 기능은 **v1.24에서 stable**이 됐고, 공식 문서는 이 경우 "파드나 Deployment를 지우고 다시 만들 필요가 없다"고 못박습니다. 그러니 `FileSystemResizePending`은 리사이즈의 정상 단계가 아니라 **오프라인 경로에서만 관찰되는 조건**으로 읽어야 합니다. 이 조건이 뜬 채로 멈춰 있다면 파드를 재시작해 재마운트를 유발하면 풀립니다.

파일시스템 종류에도 제약이 있습니다. 파일시스템이 든 볼륨은 **XFS·Ext3·Ext4일 때만** 확장할 수 있습니다.[^online-resize]
...
리사이즈는 새 크기를 지정하면 시작되고, 온라인 확장이 되면 그대로 끝납니다. 안 되면 파드를 재시작해 재마운트 시점에 마무리합니다.

kind에서 따라 하려면 한 가지를 먼저 확인해야 합니다. kind의 기본 StorageClass인 `standard`(`rancher.io/local-path`)는 **확장 자체를 지원하지 않습니다.** `allowVolumeExpansion` 필드가 없어 PVC 크기를 늘리려는 시도가 거부됩니다.
...
즉 이 절의 실습은 확장을 지원하는 StorageClass가 있는 클러스터에서만 재현됩니다. local-path에서는 오프라인 흐름을 밟는 것이 아니라 **첫 단계에서 막힙니다.** 확장 가부는 언제나 StorageClass의 `allowVolumeExpansion`이 정합니다.
...
백업·복제를 위해, CSI 드라이버가 지원하면 PV의 스냅샷을 만들 수 있습니다. VolumeSnapshot 오브젝트를 만들어 소스로 쓸 PV의 PVC를 가리킵니다.

> **먼저 확인할 것.** `VolumeSnapshot`·`VolumeSnapshotContent`·`VolumeSnapshotClass` 세 타입은 **코어 API가 아니라 CRD**입니다. snapshot controller와 함께 클러스터에 따로 설치돼야 하고, 설치는 쿠버네티스 배포판의 몫입니다.[^snapshot-crd] kind처럼 아무것도 깔지 않은 클러스터에서 아래 매니페스트를 그대로 적용하면 `no matches for kind "VolumeSnapshot"`으로 막힙니다. 실습 전에 `kubectl get crd | grep snapshot`으로 있는지부터 봅니다.

스냅샷 클래스를 지정하려면 VolumeSnapshotClass가 있어야 합니다. 지정하지 않으면 기본 클래스가 있을 때 그것이 쓰입니다.
...
deletionPolicy: Delete            # VSC와 함께 밑바탕 스토리지 스냅샷까지 삭제
```

`deletionPolicy: Delete`는 VolumeSnapshotContent 오브젝트만 지우는 것이 아니라 **밑바탕 스토리지의 실제 스냅샷까지 지웁니다.** 백업으로 오래 남길 스냅샷이라면 `Retain`을 씁니다.
...
- 운영자는 `kubectl get vsc`로 VolumeSnapshotContent를 나열합니다. vsc 는 volumesnapshotcontent 의 약어입니다. VolumeSnapshot 하나는 VolumeSnapshotContent 하나와 연결되고, 스냅샷을 또 만들려면 새 VolumeSnapshot을 만들어 새 VolumeSnapshotContent를 얻습니다.
...
> ephemeral 볼륨은 emptyDir처럼 파드 전용 임시 저장이지만, 만들어지는 게 진짜 PV라 **스냅샷·data source·용량 강제 같은 PV의 모든 기능을 씁니다.**
...
ephemeral 볼륨으로 만들어지는 PV는 로컬이든 네트워크 연결이든 *정상 PV*라, 표준 PV·PVC의 모든 기능을 제공합니다. 볼륨에서 스냅샷을 만들 수 있고, 다른 data source로 초기화할 수 있습니다. 크기 상한도 다릅니다. emptyDir에도 `sizeLimit` 필드가 있지만 그 저장 공간은 노드의 ephemeral storage에서 나오므로, 로그나 이미지 레이어가 먼저 채우면 상한에 닿기 전에 볼륨이 고갈될 수 있습니다. ephemeral 볼륨은 자기 PV의 용량으로 막히므로 상한이 볼륨 자체에 붙습니다. 물론 ephemeral 볼륨은 나중에 리사이즈할 수도 있습니다.

핵심은 아래 표의 능력들이 개별 기능이 아니라 하나의 사실에서 파생된다는 점입니다. emptyDir는 노드의 빈 디렉터리일 뿐이라 아무 오브젝트도 없지만, ephemeral 볼륨은 `volumeClaimTemplate`으로 진짜 PVC·PV를 만듭니다. 만들어진 게 정상 PV이기 때문에 스냅샷·data source·용량 강제·리사이즈가 전부 *상속*됩니다.
...
| 크기 상한 | `sizeLimit` 은 있으나 노드 ephemeral storage 를 공유해 한도 전 고갈 가능 | PV 용량으로 강제 |
| 리사이즈 | 불가 | 가능 |
...
1. **큰 임시 데이터에 크기 상한을 걸어야 할 때**. 로그 파싱·비디오 트랜스코딩 파드가 수십 GB scratch 공간을 씁니다. emptyDir 에도 `sizeLimit` 을 걸 수는 있지만 그 공간이 노드 ephemeral storage 라, 같은 노드의 다른 소비자와 물려 있어 격리가 약합니다. ephemeral 은 전용 PV 를 잡으므로 "1GiB까지만"이 볼륨 자체에 강제됩니다.
...
ephemeral 볼륨으로 만들어지는 것은 *정상 PV*라, emptyDir 가 못 하는 스냅샷·data source 초기화·리사이즈를 지원하고 용량도 PV 로 강제합니다. emptyDir 에도 `sizeLimit` 은 있지만 노드 ephemeral storage 를 공유하며, 그 밖의 PV 기능은 없습니다. ephemeral PVC의 이름은 *파드 이름 + 볼륨 이름*으로 자동 생성됩니다 — 파드 `demo-ephemeral`, 볼륨 `my-volume`이면 `demo-ephemeral-my-volume`입니다.



[^online-resize]: [Expanding Persistent Volumes Claims](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#expanding-persistent-volumes-claims): "File system expansion is either done when a Pod is starting up or when a Pod is running and the underlying file system supports online expansion." / "You can only resize volumes containing a file system if the file system is XFS, Ext3, or Ext4." in-use 확장은 같은 문서에서 `v1.24 stable` 입니다.

[^snapshot-crd]: Kubernetes 공식 문서 [Volume Snapshots](https://kubernetes.io/docs/concepts/storage/volume-snapshots/): "The CRDs and snapshot controller installations are the responsibility of the Kubernetes distribution."
