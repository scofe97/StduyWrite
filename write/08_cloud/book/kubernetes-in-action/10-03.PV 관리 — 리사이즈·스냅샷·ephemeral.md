...
여기서 중요한 오해 하나를 짚고 갑니다. A층과 B층은 *둘 중 하나를 고르는 게* 아니라 **항상 위아래로 겹쳐 함께 쓰입니다**. 아래 층(블록 디바이스)이 *땅*이라면, 위 층(파일시스템)은 그 땅 위에 그린 *지도*입니다. 땅이 없으면 지도가 그릴 대상이 없고, 지도가 없으면 파일·디렉터리 개념이 없어 바이트만 있는 빈 공간입니다.
