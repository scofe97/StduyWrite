...
위 지도가 이 편 전체를 한 장으로 이은 것입니다. Job 하나의 생애주기를 세 층으로 나눠 놓았습니다. 위층은 Job 오브젝트의 상태가 생성에서 완료까지 가로로 흐르고, 가운데는 그 아래에서 실제로 도는 파드, 아래층은 끝난 뒤의 정리입니다.
...
색은 범례 넷을 그대로 따릅니다. 파란 상자는 Job 오브젝트의 상태, 초록 태그는 파드가 실제로 도는 구간, 앰버는 파드를 새로 만들지 않는 구간입니다. 코럴은 정리에 걸리는 것이라, 완료 뒤 남은 파드와 아래층의 정리 수단 셋이 같은 색입니다.

눈여겨볼 곳은 두 군데입니다. 가운데 층에서 `일시 정지` 아래만 앰버인데, 그동안은 파드를 새로 만들지 않는다는 뜻입니다(§5). 아래층 오른쪽의 `§7 TTL 컨트롤러`는 자동 삭제를 켜면 실패 원인을 볼 로그까지 함께 사라진다는 뜻입니다.
