...
위 지도가 이 편 전체를 한 장으로 이은 것입니다. 세 전략을 나란히 놓는 대신 **통제권이 누구에게 있는가**를 축으로 세웠습니다. 아래로 갈수록 자동화가 줄고 통제가 늘어납니다.
...
ControllerRevision은 파드를 만들지도 소유하지도 않습니다. Deployment의 ReplicaSet이 "파드 관리"와 "템플릿 보관"을 겸했다면, StatefulSet은 앞의 일을 직접 하고 뒤의 일만 이 오브젝트에 맡깁니다. 즉 중간 계층이 아니라 **기록 전용 오브젝트**입니다.

그래서 연결은 label로 이뤄집니다. 위 출력의 `controller-revision-hash` 열이 그것인데, 값이 곧 ControllerRevision 이름입니다.

```pseudocode
파드 quiz-0  --[controller-revision-hash: quiz-6c48bdd8df]-->  ControllerRevision (revision 1)
파드 quiz-2  --[controller-revision-hash: quiz-6945968d9 ]-->  ControllerRevision (revision 2)
```

16-01에서 StatefulSet 컨트롤러가 파드에 붙이는 label 두 개를 봤는데, 그중 하나가 이것이었습니다. 당시에는 "ReplicaSet의 `pod-template-hash`와 같은 역할"이라고만 했지만, 실제로 가리키는 대상이 ControllerRevision 오브젝트입니다. 반대 방향으로는 ControllerRevision의 `ownerReferences`가 StatefulSet을 가리키므로, StatefulSet을 지우면 revision도 함께 정리됩니다.

이 구조가 다음 절의 partition 재생성을 떠받칩니다. 컨트롤러가 ordinal을 보고 어느 revision을 쓸지 정한 뒤, 해당 ControllerRevision에서 템플릿을 꺼내 파드를 만들기 때문입니다.
...
OnDelete는 클러스터 중요 파드를 더 통제된 방식으로 업데이트합니다. 노력은 더 들지만 그만큼 진행을 붙잡아 둘 수 있습니다. RollingUpdate는 첫 파드 업데이트가 그 노드의 다른 파드를 망가뜨려도 minReadySeconds만 지나면 다음으로 진행해 연쇄 장애를 낼 수 있습니다. 반면 OnDelete는 각 파드 업데이트 후 다른 파드의 정상 동작을 확인하고 다음을 지울 수 있습니다.

### 왜 RollingUpdate가 연쇄 장애를 못 막는가

원인은 readiness probe가 **자기 자신에게만** 묻는다는 데 있습니다. quiz-2의 probe는 quiz-2의 엔드포인트를 찌를 뿐, "내가 옆 파드를 망가뜨리고 있는가"는 묻지 않습니다. 그래서 다음 결함들이 probe를 그대로 통과합니다.

- 새 버전이 공유 볼륨이나 DB 스키마를 옛 버전이 읽지 못하는 형태로 바꿔 놓은 경우
- 새 버전이 노드 자원을 과하게 써서 같은 노드의 다른 파드가 OOM으로 죽는 경우
- 새 버전이 리플리카 셋에 잘못된 형식으로 참여해 다른 replica의 복제를 깨뜨리는 경우

셋 다 교체된 파드 자신은 멀쩡하므로 컨트롤러는 성공으로 판정하고 다음 파드로 넘어갑니다. 결함이 전체로 퍼진 뒤에야 사람이 알아챕니다. `minReadySeconds`를 늘려도 근본 해결은 아닙니다. 관측 시간이 길어질 뿐, 여전히 파드 자신의 상태만 보기 때문입니다.

OnDelete가 주는 것은 속도가 아니라 **판단 지점**입니다. 한 파드를 갈고 나서 다른 replica의 로그와 복제 상태를 눈으로 확인한 뒤 다음을 지울 수 있습니다. 컨트롤러가 볼 수 없는 범위를 사람이 대신 보는 구조입니다.
...
Kubernetes Operator는 앱 전용 컨트롤러로, 쿠버네티스 위에서 도는 앱의 배포·관리를 자동화합니다. 보통 앱을 만든 조직이 개발합니다. 그 앱의 관리법을 가장 잘 알기 때문입니다. 쿠버네티스에 기본 포함되지 않아 별도 설치해야 합니다.

> **개념 위임.** 이 절은 StatefulSet 맥락에서 Operator가 왜 필요한지와 MongoDB 사례만 다룹니다. CRD 스키마 작성, reconciliation loop 구현, 성숙도 모델, 개발 프레임워크 선택은 [Operator 패턴](../../kubernetes/08_extending/08-01.Operator%20%ED%8C%A8%ED%84%B4.md)이 정본입니다. 책 정독본은 [Operator — CRD로 도메인 지식을 자동화하기](../kubernetes-patterns/28-01.Operator%20%E2%80%94%20CRD%EB%A1%9C%20%EB%8F%84%EB%A9%94%EC%9D%B8%20%EC%A7%80%EC%8B%9D%EC%9D%84%20%EC%9E%90%EB%8F%99%ED%99%94%ED%95%98%EA%B8%B0.md)에 있고, 앱별 Operator는 `08_extending/08-02`~`08-06`이 MySQL·PostgreSQL·Redis·Kafka·Redpanda를 각각 다룹니다.
...
이 커스텀 오브젝트는 코어 API와 구조가 같습니다. apiVersion·kind·metadata·spec을 그대로 씁니다. 적용하면 Operator의 reconciliation loop가 처리해 **StatefulSet·Service 2개·Secret 몇 개**를 만들고, 모두 example-mongodb 오브젝트가 소유합니다. 이 하위 오브젝트를 직접 바꾸면 Operator가 즉시 되돌립니다. StatefulSet을 손으로 스케일해도 원래 값으로 돌아온다는 뜻입니다. Operator는 MongoDB 리플리카 셋 initiate까지 자동으로 해, 추가 수동 설정이 필요 없습니다.
...
### Operator는 StatefulSet을 대체하지 않는다

여기서 오해하기 쉬운 지점이 있습니다. Operator를 쓰면 StatefulSet이 필요 없어지는 것처럼 보이지만, 실제로는 **Operator가 StatefulSet을 만들어 씁니다**. 위 출력의 파드 이름이 `example-mongodb-0`으로 ordinal을 달고 있는 것이 그 증거입니다. 신원·PVC·역순 업데이트가 여전히 필요하므로 그 일을 하는 기계를 없앨 이유가 없습니다.

둘은 대체 관계가 아니라 계층 관계입니다.

| | StatefulSet | Operator |
|---|---|---|
| 아는 것 | replica 수·ordinal·PVC 같은 범용 규칙 | quorum·replSet 재구성 같은 앱 전용 규칙 |
| 하는 일 | 신원을 유지합니다 | 언제 스케일하고 무엇을 재구성할지 판단합니다 |
| 없으면 | 신원이 사라집니다 | 그 판단을 사람이 합니다 |

StatefulSet이 신원을 유지하는 기계라면 Operator는 그 기계를 앱 지식에 따라 조작하는 운전자입니다. 사용자가 다루는 대상만 바뀝니다. `kubectl scale sts` 대신 `members` 값을 고치면, Operator가 그것을 StatefulSet 조작으로 번역합니다.

[16-02](./16-02.StatefulSet%20%EB%8F%99%EC%9E%91%20%E2%80%94%20%EB%AF%B8%EC%8B%B1%20%ED%8C%8C%EB%93%9C%C2%B7%EB%85%B8%EB%93%9C%20%EC%9E%A5%EC%95%A0%C2%B7%EC%8A%A4%EC%BC%80%EC%9D%BC%C2%B7retention.md)에서 컨트롤러가 판단할 정보가 없어 멈추던 자리들을 정리했는데, Operator가 메우는 것이 바로 그 자리입니다. 다만 전부는 아닙니다. 노드가 정말 죽었는지 같은 클러스터 밖 사실은 Operator도 알지 못합니다. 앱 지식이 있으면 판단 가능한 부분만 메울 뿐, 불확실성 자체를 없애지는 않습니다.
...
6. StatefulSet의 RollingUpdate는 왜 ordinal 역순인가요? 생성은 오름차순인데 교체는 역순인 이유가 무엇인가요?
7. ControllerRevision과 파드는 어떻게 연결되나요? ControllerRevision이 중간 계층이 아니라는 것은 무슨 뜻인가요?
8. RollingUpdate가 연쇄 장애를 막지 못하는 근본 원인은 무엇인가요? minReadySeconds를 늘리면 해결되나요?
9. Operator는 StatefulSet을 대체하나요? 둘의 관계를 설명해 보세요.
...
OnDelete는 롤아웃을 완전히 통제하려 할 때 씁니다. 파드를 수동 삭제해야 컨트롤러가 새 템플릿으로 교체하므로, 어느 파드를 언제 업데이트할지 직접 정합니다.

RollingUpdate는 첫 파드 업데이트가 그 노드의 다른 파드를 망가뜨려도 minReadySeconds만 지나면 다음으로 진행해 연쇄 장애를 낼 수 있습니다. readiness probe가 파드 자신만 검사하기 때문입니다. OnDelete는 각 파드 업데이트 후 다른 파드의 정상 동작을 확인하고 다음을 지울 수 있어 클러스터 중요 파드에 더 안전합니다.
...
### 정답 7 — ControllerRevision과 파드의 연결

`controller-revision-hash` label로 연결됩니다. 파드에 붙은 이 label의 값이 곧 ControllerRevision 오브젝트의 이름이라, 어느 파드가 어느 revision 소속인지 이것으로 판별합니다. 반대 방향으로는 ControllerRevision의 `ownerReferences`가 StatefulSet을 가리킵니다.

"중간 계층이 아니다"라는 것은 ControllerRevision이 파드를 만들지도 소유하지도 않는다는 뜻입니다. Deployment의 ReplicaSet은 파드 관리와 템플릿 보관을 겸했지만, StatefulSet은 파드 관리를 직접 하고 템플릿 보관만 이 오브젝트에 맡깁니다. 즉 `StatefulSet → ControllerRevision → Pod` 같은 소유 사슬이 아니라, StatefulSet이 파드를 직접 소유하고 ControllerRevision은 옆에서 기록만 하는 구조입니다.
...
### 정답 9 — Operator와 StatefulSet의 관계

대체하지 않습니다. Operator가 **StatefulSet을 만들어 씁니다**. MongoDB Operator가 만든 파드 이름이 `example-mongodb-0`으로 ordinal을 달고 있는 것이 그 증거입니다. 신원·PVC·역순 업데이트가 여전히 필요하므로 그 일을 하는 기계를 없앨 이유가 없습니다.

둘은 계층이 다릅니다. StatefulSet은 replica·ordinal·PVC 같은 범용 규칙으로 신원을 유지하는 기계입니다. Operator는 quorum이나 replSet 재구성 같은 앱 전용 규칙으로 그 기계를 조작하는 운전자입니다. 사용자가 다루는 대상만 `kubectl scale sts`에서 `members` 값으로 바뀌고, Operator가 그것을 StatefulSet 조작으로 번역합니다.
