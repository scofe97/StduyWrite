...
§2의 label selector 개념을 kind에서 직접 실증한 기록입니다. [03-02. 첫 애플리케이션 배포와 스케일링](./03-02.%EC%B2%AB%20%EC%95%A0%ED%94%8C%EB%A6%AC%EC%BC%80%EC%9D%B4%EC%85%98%20%EB%B0%B0%ED%8F%AC%EC%99%80%20%EC%8A%A4%EC%BC%80%EC%9D%BC%EB%A7%81.md)에서 배포한 kiada Deployment로 확인했습니다. `kubectl expose`로 Service를 만들면 고정된 ClusterIP가 할당되고, Service는 `app=kiada` 라벨을 가진 Pod를 스스로 찾아 **Endpoints**에 등록합니다.
...
핵심은 두 가지입니다. 첫째, **ClusterIP(`10.96.154.45`)는 Pod가 죽고 새로 떠서 IP가 바뀌어도 변하지 않습니다** — 그래서 클라이언트는 이 IP 하나만 알면 됩니다. 둘째, Service는 라벨 셀렉터로 대상 Pod를 **자동으로** 찾습니다. Pod를 3개로 스케일하면, 내가 Service를 건드리지 않았는데도 새 Pod 두 개가 `app=kiada` 라벨을 갖자마자 Endpoints에 추가되어 목록이 세 개(`10.244.1.8`, `10.244.1.9`, `10.244.2.5`)로 늘어납니다. Endpoint 하나(`10.244.2.5`)는 다른 노드(worker2)의 Pod였는데도 자동 등록됐습니다. "Pod 집합이 바뀌어도 진입점은 고정"이라는 Service의 존재 이유가 여기서 실증됩니다.
...
| **kube-proxy** | 그 명단으로 각 노드에 ClusterIP→파드 DNAT iptables 규칙 관리 | IP로 *길을 놓는다* |

한 줄로 외우면 이렇습니다 — **스케줄러=노드, CNI=IP 발급, EndpointSlice 컨트롤러=명단, kube-proxy=길**. IP를 실제로 만드는 건 CNI 하나뿐입니다. 컨트롤러는 그 IP를 명단에 옮겨 적을 뿐입니다. kube-proxy는 그 명단으로 §5에서 본 DNAT 길을 놓습니다.

> **Endpoints는 deprecated, 지금은 EndpointSlice.** 위에서 쓴 `kubectl get endpoints`의 `Endpoints`(코어 v1) 오브젝트는 v1.33부터 deprecated이고, 표준은 `discovery.k8s.io/v1`의 **EndpointSlice**입니다. v1.33 이상 클러스터에서 `kubectl get endpoints`를 쓰면 deprecation 경고가 뜹니다. 최신 명령은 `kubectl get endpointslices -l kubernetes.io/service-name=kiada`입니다. *왜* 바뀌었는지(대규모 Service 확장성)와 조각 상한 등 상세는 [11-03 § "Endpoints와 EndpointSlice"](./11-03.%EC%97%94%EB%93%9C%ED%8F%AC%EC%9D%B8%ED%8A%B8%C2%B7DNS%C2%B7%ED%86%A0%ED%8F%B4%EB%A1%9C%EC%A7%80%C2%B7readiness.md)가 정본입니다.
...
이 "계층"을 편지 부치기에 빗대면 왜 쿠키가 안 되는지가 분명해집니다. 세 계층은 이렇게 대응합니다.

- **L7 애플리케이션** = 편지 *내용*. 쿠키·경로·헤더가 여기 있습니다.
- **L4 전송** = *봉투*. 출발/목적 포트가 여기 있고, Service가 보는 계층입니다.
- **L3 네트워크** = 봉투 겉면의 *주소*. 클라이언트 IP가 여기 있습니다.

Service는 L4에서 봉투와 주소만 읽지, 봉투 안 편지를 열지 않습니다. 그래서 편지 내용인 쿠키는 보이지 않습니다.

![세션 어피니티가 ClientIP만 되는 이유 — Service는 L4에서 동작](_assets/11-01-osi-l4-l7-affinity.svg)

그래서 어피니티를 걸 수 있는 유일한 기준이 패킷에서 바로 읽히는 클라이언트 IP입니다. 쿠키 기반 어피니티가 꼭 필요하면 L7을 이해하는 계층을 앞에 둬야 합니다.
...
ClientIP 어피니티에는 한계도 있습니다. 회사 NAT 뒤의 여러 사용자가 같은 출발 IP로 보이면, 그 사용자들의 연결이 전부 한 파드로 몰려 부하 분산이 깨집니다. 쿠키라면 사용자별로 구분되겠지만, L4에서는 그 정보가 보이지 않습니다. 쿠키·경로 기반 라우팅은 [11-02](./11-02.%EC%99%B8%EB%B6%80%20%EB%85%B8%EC%B6%9C%20%E2%80%94%20NodePort%C2%B7LoadBalancer%C2%B7%ED%8A%B8%EB%9E%98%ED%94%BD%20%EC%A0%95%EC%B1%85.md) 이후 Ingress에서 다룹니다.
...
파드는 같은 네임스페이스의 Service를 이름만으로 resolve하고, 다른 네임스페이스면 `http://quiz.kiada/`처럼 네임스페이스를 붙입니다. Service는 다음 DNS 이름들로 resolve됩니다 — `<service>`(같은 네임스페이스), `<service>.<namespace>`, `<service>.<namespace>.svc`, `<service>.<namespace>.svc.cluster.local`. FQDN을 다 안 적어도 되는 이유는 파드의 `/etc/resolv.conf`에 있는 `search` 줄이 도메인 접미사를 차례로 붙여 매칭될 때까지 시도하기 때문입니다.

`quiz` 한 단어가 ClusterIP까지 풀리는 전체 흐름은 이렇습니다. kubelet이 파드를 만들 때 `/etc/resolv.conf`에 `search`(네임스페이스 접미사들)·`nameserver`(CoreDNS IP)·`options ndots:5`를 자동으로 채웁니다.

![Service 이름이 ClusterIP로 풀리는 DNS resolve 흐름](_assets/11-01-dns-resolve-flow.svg)

`ndots:5`인 이유는 쿠버네티스의 가장 긴 표준 FQDN(`_port._proto.svc.ns.svc.cluster.local` 꼴)이 점 5개라, 그보다 점이 적은 이름은 상대 이름으로 보고 `search` 접미사를 붙이기 때문입니다. 파드의 기본 `dnsPolicy`는 `ClusterFirst`라, 클러스터 도메인에 맞는 질의는 CoreDNS가 답하고 `www.kubernetes.io` 같은 외부 이름은 업스트림 DNS로 넘깁니다. 이때 돌아오는 A 레코드는 *파드 IP가 아니라* Service의 ClusterIP라는 점이 핵심입니다 — 이름 하나가 앞서 본 그 가상 IP로 풀려, 이후 §5의 DNAT 경로를 탑니다.

이 편은 resolve *흐름*만 다룹니다. DNS 레코드 유형인 A·SRV·CNAME과 headless, CoreDNS 내부 동작은 다음 두 정본이 자세히 다룹니다.
...
- [02-05. DNS와 CoreDNS](../../kubernetes/04_networking/04-05.DNS%EC%99%80%20CoreDNS.md) — CoreDNS 동작 통합 개념본
...
kube-proxy가 심는 규칙은 리눅스 커널의 **netfilter**(방화벽·NAT를 담당하는 서브시스템)의 `nat` 테이블에 들어가는 iptables 규칙입니다. 이 DNAT가 *실제 네트워크 구조의 어디에서* 일어나는지를 보면 ClusterIP의 정체가 분명해집니다. 아래 그림은 클라이언트 파드가 보낸 패킷이 노드·veth·eth0을 지나 대상 파드에 닿기까지, 목적지 IP가 어느 지점에서 바뀌는지 보여 줍니다.

![실제 네트워크 구조 위의 DNAT 흐름 — 클라이언트 파드에서 대상 파드까지](_assets/11-01-network-topology-dnat.svg)

핵심은 ClusterIP(`10.96.74.151`)가 노드 1·2의 어느 `eth0`·`veth`에도 붙어 있지 않은 가상 IP라는 점입니다. 출발 노드의 커널이 DNAT로 실제 파드 IP(`10.244.2.9`)로 바꿔야 비로소 패킷이 물리망을 타고 목적지에 닿습니다. 이 흐름이 §5의 두 관찰을 한꺼번에 설명합니다.

- **ping이 안 되는 이유**: DNAT 규칙은 "ClusterIP:포트" 조합에만 걸립니다. ping은 포트 없는 ICMP라 매칭되는 규칙이 없어 패킷이 그대로 버려집니다.
- **분산이 완벽한 균등이 아닌 이유**: 위 "확률적으로 하나 선택"이 iptables의 random 모듈입니다. 그래서 30번 요청이 10/10/10이 아니라 13/10/7처럼 확률적으로 나뉩니다.

#### 리눅스 커널 레벨 — 패킷이 실제로 지나는 길

위 흐름을 커널 안으로 한 단계 더 내려가 보면, kube-proxy의 정체가 분명해집니다. kube-proxy는 트래픽을 직접 나르는 프록시가 아니라 *커널에 규칙만 심는 컨트롤러*입니다. EndpointSlice 명단을 감시해 각 노드의 netfilter에 규칙을 심고 갱신할 뿐, 실제 패킷 변환은 커널이 합니다.

ClusterIP로 온 패킷은 netfilter의 `PREROUTING` 훅에서 `nat` 테이블로 들어가 다음 체인들을 차례로 탑니다.

- `KUBE-SERVICES`: 모든 ClusterIP 매칭의 진입 체인
- `KUBE-SVC-xxxx`: 엔드포인트 중 하나를 확률로 선택
- `KUBE-SEP-yyyy`: 목적지를 실제 파드 IP로 바꾸는 DNAT 실행

여기서 **conntrack**(연결 추적)이 핵심 역할을 합니다. DNAT로 바꾼 ClusterIP↔파드 IP 매핑을 연결 테이블에 저장해 둡니다. 응답 패킷이 파드 IP를 출발지로 돌아올 때, conntrack이 그 출발지를 원래 ClusterIP로 되돌립니다. 이 역변환이 없으면 클라이언트는 자기가 보낸 적 없는 낯선 파드 IP에서 온 응답이라 여겨 버립니다.

아래 그림이 이 왕복을 한눈에 보여 줍니다. 위쪽 컨트롤 플레인에서는 kube-proxy가 규칙을 심습니다. 아래쪽 데이터패스에서 요청은 체인을 타며 DNAT를 받고, 응답은 conntrack이 역변환합니다.

![kube-proxy가 심은 규칙 위로 패킷이 왕복하는 커널 데이터패스 — 요청 DNAT와 응답 conntrack 역변환](_assets/11-01-kube-proxy-kernel-path.svg)

규칙을 *조회하는 방식*은 kube-proxy 모드에 따라 다릅니다. 기본인 **iptables 모드**는 체인을 위에서 아래로 선형 탐색하므로, Service가 수만 개면 규칙도 수만 개가 되어 조회와 갱신이 O(n)으로 느려집니다. **IPVS 모드**는 리눅스 커널의 LVS 모듈을 써서 해시 테이블로 O(1)에 엔드포인트를 찾으므로, 서비스 수와 무관하게 조회 시간이 일정합니다. 그래서 대규모 클러스터에서는 IPVS가 유리합니다. 소규모·중규모에서는 iptables로 충분합니다.

![kube-proxy 규칙 조회 방식 — iptables 선형 스캔과 IPVS 해시 조회](_assets/11-01-iptables-vs-ipvs.svg)
...
- [02-05. DNS와 CoreDNS](../../kubernetes/04_networking/04-05.DNS%EC%99%80%20CoreDNS.md) — 클러스터 DNS resolve 통합 개념본
