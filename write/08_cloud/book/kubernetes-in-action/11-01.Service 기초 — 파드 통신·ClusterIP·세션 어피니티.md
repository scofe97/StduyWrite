...
```mermaid
flowchart TB
    A["파드가 quiz 조회"] --> B{"이름의 점 개수 < ndots(5)?"}
    B -->|"점 0개 → 상대 이름"| C["search 접미사 차례로 붙임<br>quiz.default.svc.cluster.local"]
    B -->|"점 5개 이상 → 절대 이름"| G["접미사 없이 그대로 조회"]
    C --> D["CoreDNS 질의"]
    D --> E["A 레코드 응답<br>= Service의 ClusterIP"]
    E --> F["10.96.136.190 반환"]
```
...
kube-proxy가 심는 규칙은 리눅스 커널의 **netfilter**(방화벽·NAT를 담당하는 서브시스템)의 `nat` 테이블에 들어가는 iptables 규칙입니다. ClusterIP로 보낸 패킷은 다음 흐름을 탑니다.

```mermaid
flowchart LR
    A["클라이언트 파드<br>curl 10.96.74.151:80"] --> B["netfilter nat 테이블<br>PREROUTING / OUTPUT 체인"]
    B --> C{"DNAT 규칙<br>ClusterIP:80 매칭?"}
    C -->|"매칭 (포트 있음)"| D["Endpoints 중<br>확률적으로 하나 선택"]
    C -->|"ping (포트 없는 ICMP)"| X["매칭 규칙 없음<br>→ 패킷 버려짐"]
    D --> E["목적지를 파드 IP로 교체<br>10.244.2.9:80"]
    E --> F["파드로 전달"]
```

이 흐름이 §5의 두 관찰을 한꺼번에 설명합니다.
