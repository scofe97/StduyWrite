...
Deployment로 배포한 stateless 워크로드는 소와 같습니다 — 파드가 교체돼도 알아채지 못합니다. stateful 워크로드는 애완동물과 같습니다 — 잃어버린 애완동물은 새것으로 그냥 대체할 수 없고, 같은 이름을 줘도 원래처럼 행동하지 않습니다. 하지만 소프트웨어 세계에서는 교체 인스턴스에 같은 네트워크 신원과 상태를 줄 수 있으면 가능합니다. StatefulSet으로 배포하면 정확히 이 일이 일어납니다.
...
StatefulSet의 파드는 서로 완전한 복제본이 아닙니다 — 각 파드가 다른 PersistentVolumeClaim을 가리키기 때문입니다. 이름도 랜덤이 아니라 각 파드에 고유한 **ordinal 번호**가 붙고, 각 PVC도 마찬가지입니다. 파드가 삭제·재생성되면 교체한 파드와 같은 이름을 받고, 특정 ordinal의 파드는 항상 같은 번호의 PVC와 짝지어집니다. 즉 특정 replica의 상태는 파드가 몇 번 재생성돼도 항상 같습니다.
...
StatefulSet과 결합하면 파드마다 추가 DNS 레코드가 생겨 파드 이름으로 IP를 조회할 수 있습니다. 이 레코드는 headless Service가 StatefulSet과 연결되지 않으면 존재하지 않습니다. 개별 파드를 resolve하도록 `quiz-pods`라는 Service를 새로 만듭니다.
...
각 파드 이름은 StatefulSet 이름 + ordinal 인덱스입니다. ordinal은 기본 0부터 시작하며, `spec.ordinals.start`로 커스텀 시작값을 줄 수 있습니다.
...
Deployment는 파드를 ReplicaSet이 소유하고 그 ReplicaSet을 Deployment가 소유하지만, **StatefulSet은 파드를 직접 소유**합니다(ownerReferences가 StatefulSet). 복제와 업데이트를 StatefulSet이 직접 맡습니다. 파드의 volumes를 보면 템플릿의 `claimName: db-data`가 파드에서는 `db-data-quiz-0`으로 바뀌어 있습니다 — 각 파드가 자기 PVC를 받고, 이름은 claimName + 파드 이름으로 만들어지기 때문입니다.
...
이전에는 init 컨테이너로 데이터를 임포트했지만, 데이터가 복제되는 지금은 여러 번 임포트될 수 있어 무효합니다. 대신 전용 파드로 옮깁니다. `restartPolicy: OnFailure`로 한 번만 실행하고, 실패 시 성공할 때까지 재시작합니다.
...
Cattle(소)은 균질해 이름이 없고 차이가 무의미해 교체가 자명합니다 — Deployment의 stateless 파드가 그렇습니다(교체돼도 알아채지 못함). Pets(애완동물)는 이름·개성이 있어 잃으면 그냥 대체할 수 없습니다 — StatefulSet의 stateful 파드가 그렇습니다. 다만 소프트웨어는 교체 인스턴스에 같은 네트워크 신원과 상태를 줄 수 있어, StatefulSet이 정확히 이 일을 합니다(같은 이름·PVC·주소로 재생성).

### 정답 3 — PVC 템플릿과 ordinal

StatefulSet은 Deployment와 달리 **PersistentVolumeClaim 템플릿**(volumeClaimTemplates)을 지정할 수 있습니다. 그 결과 컨트롤러가 replica마다 Pod뿐 아니라 PVC도 함께 만들어, 파드가 서로 완전한 복제본이 아니라 각자 다른 PVC를 가리킵니다. 이름도 랜덤이 아니라 고유 ordinal 번호(quiz-0·quiz-1)가 붙고, 특정 ordinal 파드는 항상 같은 번호 PVC와 짝지어져, 재생성돼도 상태가 동일합니다. 또 기본적으로 파드가 동시에 아니라 하나씩(ready 대기) 만들어집니다.
...
Deployment는 파드를 ReplicaSet이, 그 ReplicaSet을 Deployment가 소유하는 2단계 구조지만, StatefulSet은 복제와 업데이트를 직접 맡아 **파드를 직접 소유**합니다(ownerReferences.kind가 StatefulSet). PVC 이름은 volumeClaimTemplates의 name(claimName)과 파드 이름을 합쳐 만들어집니다 — 템플릿의 `db-data`가 quiz-0 파드에서는 `db-data-quiz-0`이 됩니다. 각 파드가 자기 PVC를 받기 때문입니다.
...
- [스토리지와 상태](../../kubernetes/01_foundation/01-03.%EC%8A%A4%ED%86%A0%EB%A6%AC%EC%A7%80%EC%99%80%20%EC%83%81%ED%83%9C.md) — StatefulSet 3보장·headless·volumeClaimTemplates 통합 개념본(개념 위임)
