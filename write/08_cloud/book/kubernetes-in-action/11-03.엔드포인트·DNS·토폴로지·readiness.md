...
차이를 클라이언트 쪽에서 보면 더 분명해집니다. 일반 Service에서 앱이 손에 쥔 주소는 cluster IP 하나뿐이고, 그 뒤 넷 중 어디로 갈지는 커널이 확률로 고릅니다. 앱은 그 선택에 관여하지도, 결과를 통보받지도 못합니다 — 어느 파드에 닿았는지 모르니 "넷 다 보냈다"를 확인할 방법이 없습니다. headless에서는 파드 IP 네 개를 앱이 그대로 받으므로, 코드에서 직접 돌면서 몇 개에 보냈는지 세고 어느 파드가 실패했는지도 알 수 있습니다.
...
그래서 headless를 고르는 기준은 부하 분산이 아니라 **목적지를 앱이 알아야 하는가**입니다. 캐시 무효화를 모든 파드에 뿌려야 하거나, StatefulSet에서 `mysql-0`을 primary로 지목해야 하거나, Kafka·Cassandra처럼 멤버 목록을 직접 관리해야 하는 경우가 그렇습니다. 부하만 나누면 되는 상황이라면 일반 Service가 낫습니다 — 커널이 대신 골라 주므로 앱이 아무것도 안 해도 됩니다.
