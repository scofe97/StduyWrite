...
> **Spring 관점.** 스프링에서 이 차이가 갈리는 곳은 **주소를 누가 확정하는가** 한 지점입니다. 평소 쓰는 `restClient.post().uri("http://quote/cache/invalidate")` 한 줄에서는 호스트명 `quote`가 그대로 넘어가고, 커넥션을 열 때 JDK가 DNS를 조회하면서 IP 하나로 좁혀집니다. 일반 Service면 그 답이 cluster IP 하나이고, 파드 선택은 커널의 DNAT에서 끝납니다. `RestClient`는 자기가 어느 파드와 말했는지 모르고 알 방법도 없어서, 같은 호출을 네 번 돌려도 넷 다 닿았다는 보장이 없습니다 — 매번 확률이라 같은 파드에 겹칠 수 있습니다.
>
