...
> **주의 — "Pod 템플릿만"은 조금 좁은 표현입니다.** `minReadySeconds` 는 Deployment 뿐 아니라 **ReplicaSet 에도 있는 필드**라 revision 에 함께 스냅샷됩니다.[^undo-template-only] 즉 undo 대상에 포함됩니다. 공식 문서가 "only the Deployment's Pod template part is rolled back" 이라고 적는 것은 *새 revision 을 만드는 트리거가 Pod 템플릿 변경뿐* 이라는 맥락에서의 서술이고, ReplicaSet 이 실제로 들고 있는 필드는 그보다 조금 넓습니다. §2 에서 `minReadySeconds` 로 결함 버전을 막아 두었다면, undo 후 그 값이 그대로 남아 있으리라 가정하지 않는 편이 안전합니다.
