...
Pod가 시작될 때 상태 전이는 `Init:0/2` → `Init:1/2` → `PodInitializing` → `Running` 순으로 나타납니다.

- `Init:0/2` — 첫 init 컨테이너가 도는 중입니다.
- `Init:1/2` — 첫 init 이 끝나고 둘째가 도는 중입니다.
- `PodInitializing` — 모든 init 이 끝나고 주 컨테이너 이미지를 받는 중입니다.
- `Running` — 주 컨테이너가 돕니다.
...
실측 시각으로는 sidecar 가 :17 에 먼저 뜨고 → init-work 가 :18 에 그 뒤로 돌고 → main·helper 가 :25~26 에 병렬로 시작했습니다. 사이드카가 init보다 먼저 떠서, init이 도는 동안에도 살아있다는 점이 핵심입니다.
