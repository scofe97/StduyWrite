...
이렇게 되는 근본 이유는 **TCP 연결이 "바로 붙은 두 끝점" 사이의 것**이기 때문입니다. 각 중계점은 다음 상대에게 "내가 클라이언트다"라고 자기를 소개하며 **새 연결을 엽니다.** 그래서 kiada가 아는 상대는 "바로 앞에서 자기에게 TCP를 연 그것" 하나뿐이고, 그 앞의 앞(원래 요청자)은 연결 정보에 남지 않습니다. 아래 방법 ①~④를 하나씩 실습하면서, 각 방법의 홉 경로를 실제 쿠버네티스 아키텍처 위에 그린 그림을 함께 봅니다 — 굵은 앰버 화살표가 각 방법에서 "kiada에게 마지막으로 연결한 것"이고, 그 IP가 곧 Client IP입니다.
...
![방법 ① 워커 노드에서 접근 — Control Plane을 거치지 않고 노드가 kiada에 직접 연결, Client IP는 노드 브리지 IP](_assets/05-02-flow-1-worker-node.svg)
...
![방법 ② 임시 클라이언트 Pod — Control Plane을 거치지 않고 Pod가 CNI로 kiada에 직접 연결, Client IP는 그 임시 Pod의 IP](_assets/05-02-flow-2-client-pod.svg)
...
![방법 ③ port-forward — 네 방법 중 유일하게 모든 컴포넌트를 거쳐 kubelet이 Pod의 loopback으로 붙으므로 Client IP는 127.0.0.1](_assets/05-02-flow-3-port-forward.svg)
...
![방법 ④ API 서버 프록시 — API 서버가 마지막으로 kiada에 연결하므로 Client IP는 API 서버 IP, kubelet은 건너뜀](_assets/05-02-flow-4-api-proxy.svg)
