...
>
> 다만 막는 방식이 두 갈래입니다. **직접 0을 적는 것**은 API 검증이 거부하지만, 퍼센트가 **내림 때문에 0이 되는 경우**는 거부할 수 없습니다(예: replicas 3에 `maxUnavailable: 10%` → 0.3 → 0). 이때는 컨트롤러가 두 값이 모두 0으로 계산된 것을 보고 `maxUnavailable`을 1로 되돌려 롤아웃이 멈추지 않게 합니다.[^fenceposts]
...
컨트롤러가 "진행 중인 작업"을 들고 있지 않기 때문입니다. 매 조정마다 현재 템플릿 해시와 맞는 ReplicaSet을 새 것으로 지목하고 나머지를 줄일 뿐이라, 중간 버전을 거쳐 갈 의무가 없습니다(level-triggered). 같은 이유로 롤아웃 중 컨트롤러가 죽어도 파드는 무사하고, 돌아오면 진행률 복원 없이 이어갑니다.
