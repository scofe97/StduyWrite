...
![스키마에 있는 필드라 apply는 성공하지만 구현이 그 필터를 모르면 조용히 무시되고, 그래서 채널과 레벨 두 축이 필요합니다](_assets/13-01-schema-shape-not-behavior.svg)
...
그래서 두 축이 각각 다른 것을 보장합니다.

- **release channel** — 내가 쓴 YAML이 다음 버전에서도 유효한가
- **support level** — 구현을 바꿔도 같게 동작하는가
...
![Gateway status 세 층 중 kubectl get gtw는 1·2층만 보여 주고, 문제가 있는 3층 attachedRoutes는 따로 조회해야 합니다](_assets/13-01-status-layers-hide-problem.svg)
