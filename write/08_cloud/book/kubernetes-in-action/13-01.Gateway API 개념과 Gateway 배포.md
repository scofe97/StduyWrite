...
먼저 channel입니다. 앞서 `-k`로 설치한 CRD를 열어 보면 metadata에 두 줄이 붙어 있습니다.
...
`experimental` 채널로 설치한 CRD는 같은 자리의 값만 다릅니다. TCPRoute가 그렇습니다.

```yaml
# kubectl get crd tcproutes.gateway.networking.k8s.io -o yaml
metadata:
  annotations:
