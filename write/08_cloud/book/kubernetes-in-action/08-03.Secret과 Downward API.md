...
필드 차이 외에 쿠버네티스의 **취급**도 다릅니다. Secret 데이터는 그 Secret이 필요한 Pod가 도는 **노드에만 배포**됩니다. secret 볼륨으로 마운트하면 노드에서 tmpfs 같은 **메모리 저장소**에 놓여 디스크에 기록되지 않습니다.

이 보장은 볼륨 경로에 대한 것입니다. 환경변수로 주입한 값이 컨테이너 런타임 메타데이터에 어떻게 남는지까지는 보장 범위 밖입니다. §4에서 환경변수 주입을 권하지 않는 이유 중 하나입니다. 민감 데이터가 새어 나갈 가능성을 줄이는 조치들이며, 그래서 민감 데이터는 ConfigMap이 아니라 Secret에만 담아야 합니다.
...
`--cert`가 인증서 파일 경로, `--key`가 개인 키 파일 경로입니다. 주석을 코드 밖에 둔 이유는 셸 문법 때문입니다 — 줄 끝의 `\`는 **바로 뒤에 오는 개행만** 이스케이프하므로, `\` 다음에 공백과 `#`이 붙으면 줄 이음이 깨지고 명령이 거기서 끊깁니다. 위 블록에 행말 주석을 달면 `--key` 없이 실행돼 실패합니다.
...
두 형태 모두 Secret 이름을 위치 인자로 받습니다. 이름을 빼면 `error: exactly one NAME is required, got 0`으로 막힙니다. 파일에서 만들 때는 키 이름도 `.dockerconfigjson`으로 지정해야 합니다 — `kubernetes.io/dockerconfigjson` 타입이 요구하는 키가 그것이기 때문입니다.
...
지금까지 전달한 설정은 전부 정적이었습니다. Pod를 배포하기 전에 알던 값이고, 같은 Pod의 복제본 여럿이 모두 같은 값을 씁니다.

그런데 그렇게 전달할 수 없는 값이 두 부류 있습니다. 하나는 Pod가 생성·스케줄링되기 전에는 알 수 없는 데이터입니다. Pod의 IP, 노드 이름, Pod 이름 자신이 여기 해당합니다. 다른 하나는 매니페스트의 다른 곳에 이미 적힌 데이터, 즉 컨테이너의 CPU·메모리 할당량입니다. 이쪽은 알 수는 있지만 같은 값을 매니페스트에 두 번 적게 됩니다.
...
환경변수로 받으려면 `valueFrom` 아래에 둘 중 하나를 씁니다. ConfigMap이 `configMapKeyRef`, Secret이 `secretKeyRef`를 두던 바로 그 자리입니다.

- `fieldRef`: Pod의 일반 메타데이터
- `resourceFieldRef`: 컨테이너의 컴퓨팅 리소스 제약

파일로 받으려면 downwardAPI 볼륨을 씁니다(다음 장).
...
resourceFieldRef로는 여덟 종류를 환경변수·볼륨 어느 쪽으로든 주입할 수 있습니다. `requests.cpu`·`requests.memory`·`requests.ephemeral-storage`·`requests.hugepages-*` 네 가지와, 각각에 대응하는 `limits.*` 네 가지입니다.
...
- divisor를 생략하면 1입니다. 환경변수는 컨테이너 정의 안에 있으므로 기본적으로 **그 컨테이너의** 리소스 제약이 쓰이는데, 같은 Pod 안 다른 컨테이너의 한도가 필요하면 resourceFieldRef의 `containerName` 필드로 지정합니다. downwardAPI 볼륨으로 리소스 값을 받을 때는 `containerName`이 선택이 아니라 **필수**입니다 — 볼륨은 파드 레벨이라 어느 컨테이너의 한도인지 스스로 알 수 없기 때문입니다.

여기에 CPU 한정으로 걸리는 함정이 하나 있습니다. **주입된 값은 divisor로 나눈 뒤 정수로 올림됩니다.** 위 예시처럼 `limits.cpu`를 divisor 없이 받으면 `500m`(0.5 코어) 한도가 `MAX_CPU_CORES=1`로 들어옵니다.[^cpu-round] 0.5 코어짜리 컨테이너가 자신을 1코어로 인식하는 셈이라, 이 값으로 스레드 풀 크기를 잡으면 실제 할당량의 두 배를 잡습니다. 밀리코어 단위 한도를 쓴다면 `divisor: 1m`으로 받아 밀리코어 정수로 다뤄야 합니다.

```yaml
    - name: MAX_CPU_MILLICORES
      valueFrom:
        resourceFieldRef:
          resource: limits.cpu
          divisor: 1m               # 500m → 500. 올림에 먹히지 않는다
```

한도를 아예 지정하지 않은 컨테이너에서 `limits.*`를 주입하면 값이 비는 것이 아니라 **노드의 allocatable 값**이 들어옵니다. 한도를 안 걸었을 뿐인데 앱은 노드 전체를 자기 몫으로 읽게 되므로, 이 값을 튜닝 근거로 쓸 거라면 한도를 명시하는 편이 안전합니다.
...
`resourceFieldRef`의 리소스 한도 주입은 JVM에서 쓰임이 시대에 따라 갈립니다. JDK 10 이전에는 컨테이너 한도를 JVM이 몰라, 이 주입으로 받은 값에서 `-Xmx`를 직접 계산했습니다. 지금은 02-01에서 본 `UseContainerSupport`가 cgroup 한도를 직접 읽으므로 힙 설정 목적으로는 불필요합니다. 대신 스레드 풀 크기 계산 같은 애플리케이션 수준 튜닝에 쓰는 것이 맞습니다. 다만 그 용도라면 §6에서 짚은 CPU 올림을 반드시 함께 봐야 합니다 — `divisor` 없이 받은 `limits.cpu`는 `500m`도 `1`로 올라오므로, 그 숫자로 풀 크기를 잡으면 할당량의 두 배를 잡게 됩니다.



[^cpu-round]: 한도 미지정 시의 동작은 [Downward API](https://kubernetes.io/docs/concepts/workloads/pods/downward-api/)에 있습니다 — "the kubelet defaults to exposing the maximum allocatable value for CPU and memory based on the node allocatable calculation." CPU 올림과 볼륨의 `containerName` 필수는 공식 서술을 찾지 못해 kind v1.35 실측으로 확인했습니다.
