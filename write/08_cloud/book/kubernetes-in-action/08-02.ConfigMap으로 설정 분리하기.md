...
08-01에서는 설정을 Pod 매니페스트에 직접 넣었습니다. 이미지에 하드코딩하는 것보다는 낫습니다. 다만 배포 환경마다 매니페스트를 따로 둬야 할 수 있어 여전히 이상적이지 않습니다. 개발·스테이징·프로덕션 클러스터가 각각 다른 값을 쓰기 때문입니다.

같은 Pod 정의를 여러 환경에서 재사용하려면 설정을 매니페스트에서 분리해야 합니다. 그 방법 중 하나가 설정을 **ConfigMap** 오브젝트로 옮기고 Pod에서 이름으로 참조하는 것입니다.
...
## 3. 환경변수로 주입 — configMapKeyRef와 envFrom (컨테이너 적용)
...
| ConfigMap에 새 키가 추가될 때 | 매니페스트를 고치기 전에는 들어오지 않음 | 매니페스트를 안 고쳐도 들어오지만, **다음에 만들어지는 파드부터** |
| 적합한 상황 | 필요한 설정과 이름을 명시적으로 관리 | 같은 목적의 설정 묶음을 통째로 사용 |

표의 마지막 줄에 조건이 붙는 이유는 환경변수가 **컨테이너 생성 시점에 한 번 복사되는 값**이기 때문입니다. ConfigMap에 키를 더해도 이미 돌고 있는 컨테이너의 환경에는 나타나지 않고, 파드가 새로 만들어질 때 반영됩니다.[^env-restart] 이 성질은 §4에서 다시 다룹니다.
...
spec:
  ...
  - name: kiada
    envFrom:
    - configMapRef:
        name: kiada-env       # 키 지정 없이 이름만
        optional: true
```

단점은 키를 환경변수 이름으로 변환할 수 없다는 것입니다 — 키가 이미 올바른 형태여야 하며, 가능한 유일한 변환은 각 키 앞에 **prefix를 붙이는 것**뿐입니다.

여기서 참조 대상을 `kiada-config`가 아니라 `kiada-env`로 바꾼 데는 이유가 있습니다. 앞서 만든 `kiada-config`의 유일한 키는 `status-message`인데, **대시가 들어간 이름은 유효한 환경변수 이름이 아닙니다.** 그 ConfigMap을 `envFrom`으로 주입하면 변수가 하나도 만들어지지 않습니다. 그러니 `envFrom`용 묶음은 처음부터 환경변수 이름 규칙에 맞춰 따로 만듭니다.

```bash
kubectl create configmap kiada-env \
  --from-literal INITIAL_STATUS_MESSAGE="This comes from kiada-env"
```

더 곤란한 것은 **실패가 조용하다**는 점입니다. 유효하지 않은 키는 오류 없이 건너뛰어지고 파드는 정상적으로 뜹니다.[^invalid-env] "분명히 ConfigMap에 넣었는데 컨테이너에 변수가 없다"는 상황이 되는데 로그에는 아무것도 남지 않습니다. 원인은 파드 이벤트의 `InvalidVariableNames`에서 확인합니다.

```bash
kubectl describe pod <pod-name> | grep -A2 InvalidVariableNames
```

이 제약이 두 필드가 나뉜 이유를 오히려 선명하게 합니다. `configMapKeyRef`는 `env[].name`으로 이름을 새로 지으므로 `status-message` 같은 키도 `INITIAL_STATUS_MESSAGE`로 받을 수 있습니다. `envFrom`은 키 이름을 그대로 쓰니 ConfigMap을 만들 때부터 이름을 맞춰 둬야 합니다.
...
여기서 주입 위치의 레벨을 짚어 둡니다. `configMapKeyRef`와 `envFrom`은 `spec.containers[]` 안에 두는 **컨테이너 레벨** 필드입니다. 

- 환경변수는 프로세스마다 주입되므로 컨테이너별로 다른 값을 가질 수 있어야 하기 때문입니다. 반면 configMap 볼륨은 `spec.volumes[]`에 선언하는 **파드 레벨**입니다. 
- 각 컨테이너는 `volumeMounts`로 그 볼륨을 자기 파일시스템 어디에 붙일지 고릅니다. 그래서 같은 볼륨을 여러 컨테이너가 서로 다른 경로에 마운트할 수 있습니다. 볼륨의 선언·투영 방식은 [09-03](./09-03.ConfigMap%C2%B7Secret%C2%B7Downward%20API%C2%B7projected%20%EB%B3%BC%EB%A5%A8.md)에서 다룹니다.
...
**이 파일 갱신은 파드를 교체하거나 컨테이너를 재시작하지 않고 일어나므로 무중단입니다.** 다만 즉시가 아니라 kubelet 동기화 주기와 캐시 전파 지연만큼 늦습니다.

실제로 `kind-k8s-lab`(v1.35)에서 ConfigMap을 고친 뒤 마운트 파일이 바뀌기까지 약 56초가 걸렸습니다. 그동안 파드 UID와 `restartCount`는 그대로였습니다. 지연의 크기는 kubelet의 `configMapAndSecretChangeDetectionStrategy` 설정을 따릅니다 — watch(기본)·ttl 기반·API 서버 직접 조회 세 가지입니다. 볼륨의 배치·투영 원리는 [09-03](./09-03.ConfigMap%C2%B7Secret%C2%B7Downward%20API%C2%B7projected%20%EB%B3%BC%EB%A5%A8.md)에서 다룹니다.
...
파일 방식이라도 값의 성격에 따라 결국 파드를 교체해야 하는 경우가 있습니다. `server.port`가 그 예입니다. 톰캣은 부팅하면서 그 포트로 소켓을 bind합니다. 파일이 무중단으로 `9090`으로 바뀌어도 이미 열린 리스너를 런타임에 옮길 수는 없습니다.

갈리는 기준은 **그 값을 애플리케이션이 언제 읽는가**입니다. 세 갈래로 나뉩니다.

| 값이 읽히는 시점 | 예 | 파일 갱신만으로 반영되나 |
|---|---|---|
| 프로세스 시작 시 한 번 | `server.port`, JVM 힙 크기, 커넥션 풀 초기 크기 | 안 됨 — 재기동 필요 |
| 명시적 리프레시 신호를 받을 때 | `@RefreshScope` 빈, actuator `/refresh`, `spring.cloud.kubernetes.reload` | 앱이 그 장치를 구현했을 때만 |
| 요청마다 다시 | 로그 레벨, 기능 플래그 | 됨 — 무중단 반영 |

그래서 "파일 갱신은 무중단"과 "그 값을 앱이 런타임에 받아들인다"는 별개의 문제입니다. 앞의 것은 쿠버네티스가 보장하고, 뒤의 것은 애플리케이션이 그 값을 언제 읽도록 짜였는가에 달렸습니다.
...
[^env-restart]: Kubernetes 공식 문서 [ConfigMaps](https://kubernetes.io/docs/concepts/configuration/configmap/) — "ConfigMaps consumed as environment variables are not updated automatically and require a pod restart."

[^invalid-env]: Kubernetes 공식 문서 [Configure a Pod to Use a ConfigMap](https://kubernetes.io/docs/tasks/configure-pod-container/configure-pod-configmap/) — "keys that are considered invalid environment variable names are skipped. The pod is allowed to start. … An event will be generated with the reason `InvalidVariableNames`."
