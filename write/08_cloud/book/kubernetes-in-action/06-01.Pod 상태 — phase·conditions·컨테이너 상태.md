...
> ⚠️ **라이브 클러스터에서는 condition이 5개로 보입니다.** 위 표는 책 기준 4개지만, 실측(v1.35)에서는 맨 위에 `PodReadyToStartContainers`가 하나 더 나옵니다. 이는 1.29에서 추가된 것으로, "샌드박스(Pod 네트워크 등)가 준비돼 컨테이너를 시작할 수 있다"는 뜻입니다. 쿠버네티스가 새 관측 축이 필요할 때 **기존 것을 바꾸지 않고 condition을 리스트에 추가**하는 그 확장 방식(4장에서 본 open-closed)의 실례입니다.
