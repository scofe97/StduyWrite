...
컨테이너가 네 번 재시작했는데도 phase는 `Running` 그대로입니다. 여기서 두 경우를 구분해야 합니다. **같은 Pod 안의 컨테이너가 죽은** 경우는 kubelet이 그 자리에서 컨테이너만 다시 띄우므로 Pod는 그대로 살아 있고 phase도 `Running`에 머뭅니다. 반면 **Pod 자체가 사라진**(노드 장애로 축출되거나 삭제된) 경우는 그 Pod의 "복구"라는 게 따로 없습니다 — phase를 되돌리는 게 아니라, Deployment가 **새 Pod**를 만들어 `Pending`부터 다시 시작시키는 것입니다(05-03 §5). 즉 쿠버네티스는 Pod를 고쳐 쓰지 않고 새로 만들어 교체합니다.
...
t=4가 핵심입니다 — **네이티브 사이드카는 `running`인데 phase는 여전히 `Pending`**이고, 주 컨테이너 `main`은 아직 `PodInitializing`(시작 전)입니다. 사이드카가 `initContainers`에 있어 phase 판정에 들어가지 않기 때문입니다. Running 정의(§2)는 노드 바인딩·모든 컨테이너 생성까지 요구하지만, 이 실험처럼 다른 조건이 다 충족된 상황에서 마지막 관문은 "주 컨테이너가 떴는가"입니다. 즉 컨테이너별 state와 Pod 전체 phase는 별개로 움직입니다 — 개별 컨테이너가 `running`이라고 phase가 `Running`이 되지는 않습니다.
...
![컨테이너 state 3개의 전이와 각 state의 대표 reason — Waiting→Running→Terminated로 전이하며 재시작 시 Terminated에서 Waiting으로 되돌아간다(이때도 Pod phase는 Running 유지). CrashLoopBackOff·OOMKilled 같은 이름은 state가 아니라 Waiting·Terminated state의 reason이다](_assets/06-01-state-reason-map.svg)
...
3. **계속 재시작된다면 `lastState`로 직전 죽음을 본다.** `lastState`는 재시작 원인 추적에 특히 유용합니다. Spring 앱이 `OutOfMemoryError`나 OOMKilled로 죽었다면 `lastState.terminated.exitCode`(137이면 KILL 시그널)와 `reason`(OOMKilled)이 남아 있어, 현재 컨테이너가 멀쩡해 보여도 직전 사인을 확인할 수 있습니다.
