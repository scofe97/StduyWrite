...
정밀 도식이 필요하면 이 폴더의 `_assets/`에 SVG를 두고 본문에서 `![설명](_assets/파일명.svg)`로 참조합니다. 흐름·관계·상태 전이는 Mermaid로 본문에 직접 그립니다.

### 전체 요약 도식의 위치 — 이 폴더의 앵커

`writing-method` 규칙 4는 문서 전체를 요약하는 도식 한 장을 `## 학습 목표` 뒤·본문 첫 절 앞에 두라고 정합니다. 그런데 이 폴더의 편들은 `## 학습 목표` 헤딩을 쓰지 않고, 대신 H1 아래에 `> 이 문서를 읽고 나면 …` 인용문을 두고 곧바로 `## 진입`으로 갑니다. 규칙을 문자 그대로 적용할 자리가 없으므로 이 폴더에서는 아래로 고정합니다.

> **`## 진입` 절의 산문 뒤, 첫 번호 절(`## 1.`) 앞.**

진입 절이 "앞 편에서 무엇을 했고 이 편은 무엇을 하는가"를 말하는 자리이므로, 그 직후가 규칙 4가 의도한 "독자가 산문을 읽기 전에 어디로 가는지 잡는" 지점과 같습니다. 파일명은 `_assets/{편번호}.chapter-overview.svg`로 씁니다.

도식 바로 앞에는 산문 두 단락을 둡니다. 첫 단락은 뼈대(파이프라인인지 진단표인지)와 각 줄기가 어느 절인지를, 둘째 단락은 색이 붙은 곳이 무엇을 뜻하는지를 밝힙니다. 도식 안에는 범례를 넣지 않으므로 이 문장이 범례를 대신합니다.

적용 현황은 13-03과 16-01~16-03입니다. 나머지 편은 아직 이 도식이 없으며, 규칙 4가 조건부(주요 절이 흐름·인과·계층·분기로 연결될 때만)이므로 일괄 소급하지 않고 편을 손볼 때 함께 판단합니다.
...
예를 들어 서비스·디스커버리는 개념 노트 [02-04.Service와 EndpointSlice](../../kubernetes/04_networking/04-04.Service%EC%99%80%20EndpointSlice.md)에 이미 정리돼 있습니다. 책 편을 쓸 때 해당 개념이 나오면 그 편의 `## 관련 문서`에서 이 노트들로 링크하고, 반대로 개념 노트의 `related`에도 이 책 편을 걸어 양방향을 유지합니다. 통째 병합은 하지 않고 링크로만 잇습니다 — 나중에 이 둘을 한 개념으로 합칠지 판단할 때 연결이 끊기지 않게 하기 위해서입니다.
