...
> ⚠️ **키는 5개인데 섹션은 4개입니다.** `apiVersion`과 `kind` 두 키가 묶여 ① Type metadata 한 섹션을 이룹니다. 특히 헷갈리기 쉬운 지점 — **Type metadata에는 `metadata` 키가 들어가지 않습니다.** `metadata` 키는 ② Object metadata 전용입니다. 이름이 둘 다 "metadata"라 착각하기 쉽지만, "이게 무슨 타입인가"는 `apiVersion`+`kind`가, "이 인스턴스가 누구인가"는 `metadata`가 담습니다.

아래 그림은 이 네 섹션을 위에서 아래로 쌓아 보여주면서, 사용자가 spec에 원하는 상태를 쓰고 status에서 실제 상태를 읽는 흐름, 그리고 그 반대편에서 컨트롤러가 spec을 읽어 클러스터를 조정하고 status를 다시 쓰는 흐름(§5에서 설명)까지 한 장에 담았습니다.
