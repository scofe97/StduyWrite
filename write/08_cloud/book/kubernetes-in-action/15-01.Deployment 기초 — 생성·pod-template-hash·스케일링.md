...
![15-01 전체 지도 — 간접 구조가 무엇을 가능하게 하는가](_assets/15-01.chapter-overview.svg)
...
위 지도가 이 편 전체를 한 장으로 이은 것입니다. 네 칸이 세로로 이어지며, 첫 칸의 간접 구조가 나머지 전부를 설명하는 열쇠입니다. 각 상자 오른쪽의 절 번호가 본문에서 그 부분을 다루는 곳입니다.

- §1 관계 — Deployment는 파드를 직접 만들지 않고 ReplicaSet을 사이에 둡니다
- §2·§3 생성과 조회 — spec은 ReplicaSet에 strategy 하나가 더해진 모양입니다
- §4 pod-template-hash — 그 간접 구조가 세대를 어떻게 구분하는지 보입니다
- §5·§6 스케일과 삭제 — 두 작업 모두 같은 계층을 따라 아래로 전달됩니다

색이 붙은 곳은 두 군데뿐입니다. 나머지 상자는 전부 같은 회색이라 색을 해석할 일이 없습니다. 앰버 띠는 해시가 이름과 label 양쪽에 붙어야 새 파드가 옛 ReplicaSet에 잡히지 않는다는 뜻이고, 붉은 띠는 `--cascade=orphan`이 바로 아래 한 단계만 끊어 파드는 그대로 남는다는 뜻입니다.
...
`describe`가 conditions를 Type·Status·Reason 세 열로만 찍는다는 점은 알아 둘 만합니다. 같은 condition의 `message` 필드에는 더 긴 설명이 담깁니다. RS까지 내려가기 전에 `kubectl get deploy where-are-the-pods -o yaml`로 `status.conditions[].message`를 먼저 확인하면 한 단계 아낄 수 있습니다.
...
밑의 ReplicaSet을 봐야 합니다. Deployment의 conditions에서 `ReplicaFailure=True`·reason `FailedCreate`를 볼 수 있지만 뭉뚱그린 표현입니다. 진짜 원인은 밑의 ReplicaSet의 events 에서 봅니다. Deployment YAML status 의 message 필드도 같은 것을 담습니다. 원인은 대개 사용자 권한 관련(예: service account 누락)입니다. 결론은 파드가 안 뜨면 Deployment가 아니라 밑의 ReplicaSet에서 원인을 찾는다는 것입니다.
