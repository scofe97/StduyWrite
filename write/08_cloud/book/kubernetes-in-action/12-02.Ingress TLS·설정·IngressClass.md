...
그러면 왜 그런 필드를 따로 두었을까요. **닭과 달걀** 때문입니다. 서버는 암호화를 시작하려면 먼저 인증서를 내밀어야 하는데, 한 IP에 도메인이 여럿이면 **어느 도메인의 인증서를 내밀지**부터 정해야 합니다. 그 답이 적힌 `Host` 헤더는 암호화된 본문 안에 있어 이 시점에는 읽을 수 없습니다. 순서가 어긋나는 것입니다.

![인증서를 고르려면 목적지를 알아야 하는데 그 정보가 암호 뒤에 있어, 서로가 서로를 기다립니다](_assets/12-02-sni-chicken-and-egg.svg)

그래서 핸드셰이크의 맨 앞, 아직 아무것도 암호화되지 않은 자리에 목적지 이름을 **평문으로** 실어 보내는 필드를 따로 만들었습니다. 그것이 SNI(Server Name Indication)입니다. [12-01](./12-01.Ingress%EB%A1%9C%20%EC%97%AC%EB%9F%AC%20%EC%84%9C%EB%B9%84%EC%8A%A4%EB%A5%BC%20%ED%95%9C%20IP%EC%97%90%20%EB%85%B8%EC%B6%9C%ED%95%98%EA%B8%B0.md) §1의 봉투 비유로 옮기면 **SNI는 봉투 겉면의 수취인 이름**입니다 — 봉투를 열지 않아도 읽히는 유일한 정보이고, 그래서 passthrough에서도 호스트만은 가를 수 있습니다.

#### 실제로 어디에 실리나

클라이언트가 TLS를 시작하며 보내는 **첫 메시지**가 `ClientHello`이고, SNI는 그 안의 확장 필드입니다. 이름이 `server_name`이라 값도 호스트명 하나뿐입니다.

```text
ClientHello                       ← 평문. 아직 암호가 협상되기 전입니다
  ├─ 지원하는 암호 목록
  ├─ 랜덤값
  └─ extensions
       └─ server_name: api.example.com   ← SNI

(이후 협상이 끝나야 암호화 시작 — Host 헤더·경로·쿠키는 그 뒤)
```
