...
앞의 셋(`scheme`·`subnets`·`inboundCIDRs`)은 **네트워크 경계**를, `group.name`은 **비용 구조**를, `namespaceSelector`는 **누가 이 클래스를 쓸 수 있는가**를 정합니다. 앱 개발자가 `ingressClassName: alb-internal` 한 줄을 적을 때 실제로 고르는 것이 이것들입니다.

**중요한 성질 하나 — 클래스 파라미터가 Ingress annotation을 이깁니다.** 같은 항목이 양쪽에 있으면 `IngressClassParams` 쪽이 이기고 annotation은 무시됩니다. 이 우선순위가 있어야 "관심사 분리"가 실제로 성립합니다. 앱 쪽에서 annotation으로 `scheme: internet-facing`을 적어도 클래스가 `internal`로 못 박아 두었으면 공인 IP가 열리지 않습니다 — **인프라팀의 결정이 앱 쪽 실수나 임의 설정에 뚫리지 않는다**는 뜻입니다.
