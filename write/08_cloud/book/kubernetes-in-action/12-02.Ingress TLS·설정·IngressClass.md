...
![요청 하나가 브라우저에서 파드까지 가는 전 구간과, 12장이 다룬 것들이 각각 작동하는 자리](_assets/12-full-request-journey.svg)
...
프록시 자리에 12장의 대부분이 몰려 있습니다. TLS를 풀고(§1), 호스트를 가르고(§2), 경로를 가르고(12-01 §5), 헤더와 쿠키를 손대고(§3), 어디에도 안 맞으면 default backend로 보냅니다(12-01 §7). **이 넷이 전부 "암호가 프록시에서 풀린다"는 전제 위에 서 있어서**, passthrough를 고르면 ①이 사라지고 ②의 SNI만 남습니다.
