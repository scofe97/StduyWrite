...
**왜 또 한 단계를 두나.** `IngressClass`는 쿠버네티스 표준 타입이라 **스키마가 고정**돼 있는데, `scheme`이나 `ipAddressType`은 AWS에만 있는 개념입니다. 표준 타입에 특정 벤더 필드를 넣을 수 없으니, 벤더가 자기 CRD를 만들고 표준 쪽은 그것을 가리키는 포인터만 갖습니다. §2에서 본 "모든 구현의 모든 설정을 스키마에 담을 수 없다"와 같은 사정이고, 해법도 같은 결입니다.
