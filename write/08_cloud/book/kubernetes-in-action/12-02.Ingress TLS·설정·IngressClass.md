...
`server_name: api.example.com` 한 줄이 실제로는 길이가 앞에 붙는 중첩 구조로 실립니다.

![server_name 확장의 중첩 구조와 실제 바이트 배열](_assets/12-02-sni-wire-format.svg)
...
명세가 못 박은 제약 하나가 실무에서 걸립니다. **`HostName`에 IP 주소를 넣을 수 없습니다.**[^rfc6066] DNS 호스트명만 허용되므로, IP로 직접 접속하면 SNI 자리가 아예 비고 서버는 어느 인증서를 내밀지 모릅니다. 아래 함정이 여기서 나옵니다.
