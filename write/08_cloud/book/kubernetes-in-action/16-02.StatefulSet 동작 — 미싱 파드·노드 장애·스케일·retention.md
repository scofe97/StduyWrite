...
아래는 이 편 전체를 한 장으로 이은 지도입니다. 앞 편과 달리 단계가 순서대로 이어지지 않습니다. 네 상황은 각각 독립해서 갈라지고, 갈리는 기준은 하나입니다 — **무엇이 사라졌는가.**
...
다만 이 제약이 걸리는 곳은 **API 오브젝트**입니다. 실제로 도는 **컨테이너**까지 묶지는 못합니다. 오브젝트가 사라진 뒤에도 노드에서 컨테이너가 계속 돌고 있으면 보장이 깨질 수 있습니다. 이 편의 §2가 다루는 것이 정확히 그 상황이고, 강제 삭제가 위험한 이유도 여기에 있습니다.
...
정책은 **StatefulSet이 삭제되거나 스케일다운되어 파드가 제거될 때만** 적용됩니다. 노드 장애로 파드가 죽고 control plane이 교체 파드를 만드는 경우에는 정책과 무관하게 기존 PVC가 유지되며, 볼륨은 새 파드가 뜰 노드에 다시 붙습니다. 그래서 장애 노드의 PVC를 정리하려면 `kubectl delete pvc`로 직접 지워야 합니다(§2). 또 이 필드를 쓰려면 feature gate `StatefulSetAutoDeletePVC`가 필요합니다. API 서버와 controller manager 양쪽에 켜져 있어야 합니다.
