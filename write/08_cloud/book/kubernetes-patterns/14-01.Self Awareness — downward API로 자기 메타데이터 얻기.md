...
`fieldRef`로 Pod 수준 메타데이터에 접근합니다.

**여기서 원서를 그대로 따르면 안 되는 지점이 있습니다.** 원서는 이 키들을 환경변수와 downwardAPI 볼륨 양쪽에서 모두 쓸 수 있는 것처럼 적지만, 공식 문서는 **셋으로 나눠** 설명합니다. 키마다 쓸 수 있는 자리가 다릅니다.

| 키 | 설명 | 환경변수 | 볼륨 |
|----|------|:-------:|:----:|
| `metadata.name` | Pod 이름 | O | O |
| `metadata.namespace` | Pod가 실행되는 네임스페이스 | O | O |
| `metadata.uid` | Pod의 고유 ID | O | O |
| `metadata.labels['key']` | 해당 라벨 키의 값 | O | O |
| `metadata.annotations['key']` | 해당 어노테이션 키의 값 | O | O |
| `spec.nodeName` | Pod를 호스팅하는 노드의 이름 | O | **X** |
| `spec.serviceAccountName` | Pod가 사용하는 ServiceAccount | O | **X** |
| `status.hostIP` | 노드의 주 IP 주소 | O | **X** |
| `status.hostIPs` | 위의 듀얼스택 버전 | O | **X** |
| `status.podIP` | Pod의 주 IP 주소 | O | **X** |
| `status.podIPs` | 위의 듀얼스택 버전 | O | **X** |
| `metadata.labels` | **모든** 라벨 | **X** | O |
| `metadata.annotations` | **모든** 어노테이션 | **X** | O |

규칙으로 줄이면 셋입니다. `metadata`의 **개별 키**는 양쪽 다 됩니다. `spec`·`status` 계열은 **환경변수 전용**입니다. 전체 라벨·어노테이션은 **볼륨 전용**입니다.

마지막 줄이 특히 중요합니다. 아래에서 볼 "라벨 전체를 파일로 받는" 용법이 애초에 환경변수로는 불가능하기 때문입니다.

> **limits를 안 걸면 노드 전체 값이 나옵니다.** 원서에는 없는 동작입니다. 컨테이너에 CPU·메모리 limit이 지정되지 않았는데 downward API로 그 값을 노출하려 하면, kubelet은 실패하거나 빈 값을 주는 대신 **노드 allocatable의 최대값**을 내놓습니다. 아래 심화 학습에서 스레드풀을 `limits.cpu`에 비례해 잡자고 하는데, limit이 비어 있으면 그 계산이 노드 전체 코어 수를 기준으로 돌아가니 주의해야 합니다.
...
spec:
  ...
  - name: pod-info
    downwardAPI:
      items:
      - path: labels              # 라벨마다 한 줄씩 key="value" 형식으로 담깁니다
...
**6.** 원서가 드는 것은 아홉 개입니다. 다만 **키마다 쓸 수 있는 자리가 다릅니다.**

- **환경변수·볼륨 양쪽** — `metadata.name`, `metadata.namespace`, `metadata.uid`, `metadata.labels['key']`, `metadata.annotations['key']`
- **환경변수 전용** — `spec.nodeName`, `spec.serviceAccountName`, `status.hostIP`, `status.podIP` (여기에 듀얼스택용 `status.hostIPs`·`status.podIPs` 가 더 있습니다)
- **볼륨 전용** — `metadata.labels`, `metadata.annotations` (키 없이 **전체**를 받는 형태)

규칙으로 줄이면 `metadata` 의 개별 키는 양쪽 다 되고, `spec`·`status` 계열은 환경변수 전용, 전체 라벨·어노테이션은 볼륨 전용입니다. 원서는 이 구분 없이 "양쪽에서 쓸 수 있다"고 적으니 그대로 외우면 틀립니다.
...
**9.** `metadata.labels`와 `metadata.annotations` 참조로 **모든** 라벨과 어노테이션을 파일에 담을 수 있습니다(이 형태는 **볼륨 전용**이라 환경변수로는 불가능합니다). 형식은 줄마다 `key="escaped-value"` 로, **값이 따옴표로 감싸집니다.** 원서는 `name=value` 로 적지만 실제로는 따옴표가 붙으니, 파싱할 때 이 점을 고려해야 합니다. 라벨이나 어노테이션이 바뀌면 그 파일이 갱신됩니다.
...
환경변수는 Pod 시작 시점에 값이 확정되므로 실행 중 라벨이나 어노테이션이 바뀌어도 반영하지 못합니다. 새 값을 보려면 재시작해야 합니다. downwardAPI 볼륨은 파일로 마운트되어 변경이 파일에 반영되며 `metadata.labels` 참조로 모든 라벨을 `key="escaped-value"` 형식으로 담을 수도 있습니다(이 전체 형태는 볼륨에서만 됩니다). 다만 그 파일을 감지해 다시 읽는 것은 앱의 책임입니다. 그 로직이 없으면 볼륨이라도 재시작이 필요합니다.
