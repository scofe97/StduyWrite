...
Pod 안 컨테이너는 하나의 Pod IP·포트 범위와 `localhost`를 공유합니다.[^k8s-pods] 반대로 환경변수와 루트 파일시스템은 컨테이너별로 독립적이며, 파일을 공유하려면 같은 Volume을 명시적으로 마운트해야 합니다.
...
Service는 자신의 DNS 이름과 가상 IP를 통해 안정적인 접속점을 제공합니다.[^k8s-services] Pod가 스케일되거나 교체돼도 소비자는 같은 Service를 호출하고, Endpoint 집합만 바뀝니다.
...
**ResourceQuota**는 Namespace 전체가 쓸 수 있는 CPU·메모리·오브젝트 수의 상한을 정합니다.[^k8s-quota] 반면 **NetworkPolicy**는 Label selector를 사용해 어떤 Pod의 ingress·egress를 허용할지 정합니다.[^k8s-netpol]
...
스케줄러가 컨테이너마다 다른 노드를 고르지 않고, Pod 안 모든 컨테이너의 자원 요구를 합쳐 한 노드에 배치한다는 뜻입니다.

따라서 Pod 안 컨테이너는 함께 배포·스케일됩니다. 독립 배포가 필요한 기능을 같은 Pod에 넣으면 안 됩니다.
...
- 개념 노트: [Service와 EndpointSlice](../../kubernetes/04_networking/04-04.Service%EC%99%80%20EndpointSlice.md)
...
[^k8s-pods]: Kubernetes 공식 문서 — Pod는 하나 이상의 컨테이너가 스토리지·네트워크를 공유하는 최소 배포 단위.
<https://kubernetes.io/docs/concepts/workloads/pods/>

[^k8s-services]: Kubernetes 공식 문서 — Service는 Pod 집합에 안정적인 이름과 IP로 접근하는 추상화.
<https://kubernetes.io/docs/concepts/services-networking/service/>

[^k8s-quota]: Kubernetes 공식 문서 — ResourceQuota는 Namespace 단위 자원·오브젝트 수 상한을 제한.
<https://kubernetes.io/docs/concepts/policy/resource-quotas/>

[^k8s-netpol]: Kubernetes 공식 문서 — NetworkPolicy는 Label selector 기반으로 Pod 간 ingress/egress를 허용·차단.
