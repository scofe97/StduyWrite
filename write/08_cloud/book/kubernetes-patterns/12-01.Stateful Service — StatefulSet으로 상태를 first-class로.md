...
- `serviceName` 필드로 Service에 연결하는 것은 **필수**입니다.
...
**5.** 스케일 업은 새 Pod와 PVC를 함께 만들지만, 스케일 다운은 Pod만 지우고 PVC와 PV는 남깁니다. 상태 앱의 스토리지가 중요하며 실수로 인한 스케일 다운이 데이터 손실을 일으켜서는 안 된다는 전제에서 나온 설계상의 의도입니다. 의도적으로 줄였고 데이터를 옮겼다고 확신하면 PVC를 수동으로 지워 PV 회수를 허용합니다.
