...
훅의 호출 메커니즘은 4장의 헬스 프로브와 비슷하며 두 핸들러 타입을 지원합니다.

| 핸들러 | 동작 |
|--------|------|
| `exec` | 컨테이너에서 명령을 직접 실행합니다 |
| `httpGet` | Pod 컨테이너가 연 포트로 HTTP GET 요청을 보냅니다 |
...
여기서 함정이 생깁니다. 이 값이 Kubernetes의 `terminationGracePeriodSeconds`보다 크면 앱이 요청을 다 마치기 전에 SIGKILL로 강제 종료돼 graceful shutdown이 무의미해집니다. 둘 다 기본값이 30초라 그냥 두면 아슬아슬하게 겹칩니다.
