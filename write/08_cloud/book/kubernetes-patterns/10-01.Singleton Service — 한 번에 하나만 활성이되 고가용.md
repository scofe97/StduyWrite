...
두 방식이 주는 보장은 같지 않습니다. **ReplicaSet은 가용성을 일관성보다 우선**해 at-least-once 의미를 적용하므로 노드 분단 같은 상황에서 가끔 두 개가 동시에 돕니다. **StatefulSet은 일관성을 우선**해 at-most-one을 보장하는 대신 가끔 0개가 됩니다.
...
2. ReplicaSet의 at-least-once와 StatefulSet의 at-most-one이 가용성과 일관성 중 무엇을 택한 결과인지 설명할 수 있습니다.
...
StatefulSet이 at-most-one을 지킬 수 있는 것은 **ordinal 인덱스와 순차 보장** 덕입니다. `pod-0`을 새로 만들기 전에 기존 `pod-0`이 확실히 종료됐음을 요구하거나 수동 개입을 요구해, 같은 정체성의 Pod가 둘이 되는 것을 막습니다. 그 대가가 가끔 0개입니다.
...
Kubernetes 위에서라면 책의 권장대로 ZooKeeper 대신 **Kubernetes Lease API**를 백엔드로 쓰는 편이 깔끔합니다. Spring Cloud Kubernetes의 leader election이 이를 제공하며, 락만을 위한 별도 인프라 없이 클러스터의 etcd를 재활용합니다.
...
- [ ] ReplicaSet의 at-least-once와 StatefulSet의 at-most-one이 가용성과 일관성 중 무엇을 택한 결과인지 설명할 수 있습니까?
