...
이름 하나가 실제 Pod에 닿기까지를 두 단계로 나눠 봅니다. 앞은 이름이 IP가 되는 **이름 해석**이고, 뒤는 그 IP가 실제 Pod 주소로 바뀌는 **주소 변환**입니다.

![이름 해석 — 짧은 이름이 clusterIP가 되기까지](./_assets/13-01.packet-path.svg)

소비자가 짧은 이름을 부르면 `/etc/resolv.conf`에 적힌 DNS 서버에 질의합니다. 이 파일에는 Service 목록이 아니라 **DNS 서버의 위치와 `search` 접미사 목록만** 들어 있어, Service를 아무리 만들어도 파일 내용은 바뀌지 않습니다. `ndots:5` 때문에 점이 다섯 개 미만인 짧은 이름은 `search` 접미사를 차례로 붙여 시도하고, 같은 namespace라면 첫 시도에서 응답을 받습니다. 이름 목록을 들고 있는 쪽은 CoreDNS이고 Pod는 부를 때마다 물어볼 뿐이라, 앞서 본 환경변수의 시간 의존이 여기에는 없습니다.

![주소 변환 — 목적지만 바뀌고 소스는 그대로](./_assets/13-01.packet-nat.svg)

clusterIP를 목적지로 만들어진 패킷은 같은 노드 커널의 nat 테이블에서 kube-proxy가 넣어둔 규칙 체인에 걸립니다. `KUBE-SERVICES`에서 서비스별 `KUBE-SVC`로, 다시 백엔드마다 하나씩 있는 `KUBE-SEP`로 이어지며, **확률 기반**으로 Pod 하나를 고릅니다. 백엔드가 셋이면 첫 규칙이 1/3, 다음이 남은 것 중 1/2, 마지막이 무조건 매칭되어 결과적으로 균등 분포가 됩니다.

여기서 짚을 것은 **바뀌지 않는 쪽**입니다. 목적지만 clusterIP에서 선택된 Pod의 실제 IP·targetPort로 바뀌고 **소스 주소는 소비자 Pod의 IP 그대로입니다.** DNAT만 일어나고 SNAT는 일어나지 않습니다. 그래서 제공자 Pod의 애플리케이션 로그에는 노드 주소가 아니라 **실제 호출자의 IP가 남습니다.** 뒤에 볼 NodePort가 소스 NAT로 클라이언트 IP를 가리는 것과 정확히 갈리는 지점입니다.

변환 내역은 conntrack에 기록됩니다. 응답 패킷은 소스가 제공자 Pod IP인 상태로 출발하는데, 소비자 노드의 conntrack이 그 기록을 보고 소스를 다시 clusterIP로 되돌립니다. 이 역변환이 없으면 소비자 커널은 **자기가 요청한 적 없는 주소에서 온 응답**이라 판단해 연결에 매칭하지 못하고 패킷을 버립니다. 제공자 Pod 쪽은 반대로 clusterIP를 한 번도 보지 못하고 자기 IP로 온 평범한 요청으로 받습니다.
