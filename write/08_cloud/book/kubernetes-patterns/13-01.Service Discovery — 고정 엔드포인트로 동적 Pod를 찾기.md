...
related:
  ...
  - ../networking-and-kubernetes/02-02.iptables·IPVS·eBPF — kube-proxy를 이해하는 세 기술.md
...
4번 패킷은 소스가 소비자 Pod IP, 목적지가 clusterIP입니다. 커널에서 목적지가 실제 Pod IP로 바뀔 때 **소스는 보존됩니다.** DNAT만 일어나고 SNAT는 일어나지 않으므로 제공자 앱 로그에 **실제 호출자의 IP가 남습니다** — 뒤에 볼 NodePort가 소스 NAT로 클라이언트 IP를 가리는 것과 갈리는 지점입니다. netfilter 체인 구성·conntrack 역변환·kube-proxy 모드 차이는 [N&K 02-02. iptables·IPVS·eBPF](../networking-and-kubernetes/02-02.iptables%C2%B7IPVS%C2%B7eBPF%20%E2%80%94%20kube-proxy%EB%A5%BC%20%EC%9D%B4%ED%95%B4%ED%95%98%EB%8A%94%20%EC%84%B8%20%EA%B8%B0%EC%88%A0.md)이 정본입니다.
