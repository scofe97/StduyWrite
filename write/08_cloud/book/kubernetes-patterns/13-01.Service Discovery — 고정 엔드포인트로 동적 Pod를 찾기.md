...
이 예제에서 `random-generator`가 두 번 나오는데 **둘은 서로 다른 값이고 일치할 이유가 없습니다.** `metadata.name`은 우리가 짓는 Service 이름이고 그대로 DNS 이름이 됩니다. `selector`의 `app: random-generator`는 Pod에 붙은 **라벨 값**과 맞물리는 별개 값으로, Pod의 라벨이 `app: rng`라면 여기도 `rng`를 써야 합니다. 예제가 우연히 같은 이름을 썼을 뿐입니다. 아래 YAML들도 같은 관례를 따르므로, 앞으로 나오는 `random-generator`·`external-service` 같은 이름은 전부 임의로 지은 예시 값으로 읽으시면 됩니다.
...
같은 두 작업을 순서만 바꿔 실행한 결과입니다. A에서는 Pod가 시작할 때 kubelet이 그 시점까지 존재하는 Service를 훑어 변수를 채우지만, B에서는 시작 시점에 Service가 없어 변수가 만들어지지 않습니다. 뒤늦게 Service를 만들어도 소급되지 않는 까닭은 Kubernetes의 제약이 아니라 **운영체제의 성질**입니다. 리눅스에서 환경변수는 `execve` 시점에 커널이 프로세스 메모리로 복사하고, 그 뒤로는 밖에서 바꿔 넣을 방법이 없습니다. 그래서 Pod를 재시작해 `execve`를 다시 하게 만드는 것이 유일한 해결입니다.

B에서도 clusterIP·iptables 규칙·DNS 등록은 전부 정상입니다. 그 Pod 안에서 `curl http://random-generator`는 그대로 통합니다. **네트워크가 끊긴 것이 아니라 주소를 알아내는 경로 하나만 막힌 것입니다.**
...
| 마디 | 채워지는 값 | 예시 |
|---|---|---|
| `{service-name}` | Service 이름 — 우리가 짓습니다 | `random-generator` |
| `{namespace}` | Service가 속한 namespace | `default` |
| `svc` | **고정** — Service 리소스임을 나타냅니다 | `svc` |
| `{cluster-domain}` | 클러스터 고유 접미사 — 설치 시 정해집니다 | `cluster.local` |

앞 두 마디만 상황에 따라 바뀌고 `svc`와 클러스터 접미사는 사실상 고정입니다.

원한다면 클러스터 접미사를 생략할 수 있습니다. 같은 namespace에서 접근한다면 namespace도 생략할 수 있어 `random-generator` 한 마디로 줄어듭니다.
...
그림의 1~4가 이름이 주소로 바뀌는 순서입니다. 소비자가 짧은 이름을 부르면 `/etc/resolv.conf`에 적힌 DNS 서버에 질의합니다. 이 파일에는 Service 목록이 아니라 **질의할 서버 주소와 붙여 볼 접미사만** 들어 있어, Service를 아무리 만들어도 파일 내용은 바뀌지 않습니다. `ndots:5`는 점이 다섯 개 미만인 짧은 이름에 `search` 접미사를 차례로 붙여 시도하게 하는 설정으로, 같은 namespace라면 첫 접미사에서 응답을 받습니다. 이름 목록을 들고 있는 쪽은 CoreDNS이고 Pod는 부를 때마다 물어볼 뿐이라, 앞서 본 환경변수의 시간 의존이 여기에는 없습니다. headless Service라면 3번에서 갈려 clusterIP 대신 백킹 Pod들의 IP가 그대로 응답됩니다(12장).

4번에서 만들어진 패킷은 소스가 소비자 Pod IP이고 목적지가 clusterIP입니다. 그 뒤 커널에서 목적지가 실제 Pod IP로 바뀌는데, 이때 **목적지만 바뀌고 소스는 보존됩니다.** DNAT만 일어나고 SNAT는 일어나지 않으므로 제공자 Pod의 애플리케이션 로그에는 노드 주소가 아니라 **실제 호출자의 IP가 남습니다.** 뒤에 볼 NodePort가 소스 NAT로 클라이언트 IP를 가리는 것과 갈리는 지점입니다. netfilter 규칙 체인의 구성, conntrack의 역변환, kube-proxy 모드별 차이는 [K8s 패킷 여정 — netfilter·conntrack·라우팅](../../../02_os/networking/01-02.K8s%20%ED%8C%A8%ED%82%B7%20%EC%97%AC%EC%A0%95%20%E2%80%94%20netfilter%C2%B7conntrack%C2%B7%EB%9D%BC%EC%9A%B0%ED%8C%85.md)이 정본입니다.
