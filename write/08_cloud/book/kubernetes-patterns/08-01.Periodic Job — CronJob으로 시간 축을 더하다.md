...
> 주기 작업은 시스템 유지보수(백업·정리·아카이빙)나 비즈니스 작업(파일 전송·DB 폴링·뉴스레터 발송)에 흔합니다. 
>
> 전통적으로 cron이나 전용 스케줄러를 썼지만 단일 서버 cron은 **단일 장애점**이고 앱 안에 스케줄러(Quartz·Spring Batch·`ScheduledThreadPoolExecutor`)를 넣으면 스케줄러를 HA로 만들기 위해 **앱 전체를 HA로** 만들어야 하고 리더 선출 같은 분산 문제가 따라옵니다. 
>
> **CronJob**은 cron 포맷으로 Job을 스케줄해, 개발자가 스케줄링이 아니라 **수행할 작업만** 구현하게 합니다.
...
**놓친 스케줄이 100회를 넘으면 그 밀린 실행을 따라잡지 않습니다.** 컨트롤러는 마지막 스케줄 시각부터 지금까지 몇 번을 놓쳤는지 세는데, 그 수가 100을 넘으면 Job을 시작하지 않고 다음 오류를 남깁니다.

```
too many missed start times. Set or decrease .spec.startingDeadlineSeconds or check clock skew
```

여기서 오해하기 쉬운 지점이 있습니다. 공식 문서는 이 동작이 **밀린 것을 따라잡는(catch-up) 경우에만 해당하고 CronJob이 아주 멈추는 것은 아니라고** 못박습니다. 컨트롤러가 한동안 죽어 있었거나 클럭 스큐가 생겼을 때 그 공백을 메우지 않을 뿐, 이후 정상 스케줄은 계속 돕니다.

`startingDeadlineSeconds`는 여기서 두 번째 역할을 합니다. 이 값이 설정돼 있으면 컨트롤러는 **마지막 스케줄 시각이 아니라 그 초 범위 안에서만** 놓친 횟수를 셉니다.

공식 문서의 예시가 이 차이를 잘 보입니다. 1분마다 도는 CronJob에서 컨트롤러가 `08:29`부터 `10:21`까지 죽어 있었다고 합시다.

| `startingDeadlineSeconds` | 세는 구간 | 놓친 횟수 | 결과 |
|--------------------------|----------|----------|------|
| 설정 안 함 | 마지막 스케줄 시각부터 지금까지 | 100 초과 | 시작하지 않습니다 |
| `200` | 최근 200초만 | **3회** | `10:22`에 정상 시작합니다 |

200초 창에서는 1분 간격이 세 번밖에 안 들어가니 3회입니다. 이 필드를 "늦으면 건너뛰기" 용도로만 알고 있으면 이 쓸모를 놓칩니다.
...
**17.** 그 **밀린 실행을 따라잡지 않고** `too many missed start times` 오류를 로그에 남깁니다. 컨트롤러가 한동안 죽어 있었거나 클럭 스큐가 생겼을 때 벌어집니다.

주의할 것은 이것이 **catch-up 상황에만 해당한다**는 점입니다. 공식 문서가 "does not mean the CronJob will stop running" 이라고 명시하듯, CronJob 자체가 죽는 것은 아니고 이후 정상 스케줄은 계속 돕니다.

**18.** 집계 **기준 구간**을 바꿉니다. 이 값이 없으면 마지막 스케줄 시각부터 지금까지를 세지만, 값이 설정돼 있으면 **그 초 범위 안에서만** 셉니다.

공식 문서 예시로는, 1분마다 도는 CronJob 에서 컨트롤러가 `08:29`~`10:21` 동안 죽어 있었을 때 `startingDeadlineSeconds` 가 없으면 놓친 횟수가 100 을 넘어 시작하지 않습니다. 반면 `200` 으로 두면 최근 200 초만 보므로 놓친 횟수가 **3회**로 집계되어 `10:22` 에 정상 시작합니다.
