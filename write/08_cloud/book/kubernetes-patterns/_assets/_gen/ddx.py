"""kubernetes-in-action 11~13 장 공용 헬퍼 — dd.py 프리미티브 위에 이 장이 반복해 쓰는 형태만 얹는다.
...
def msg(d, a, b, label, y, c=None, mk="ar", dash=None, sub=None):
    """Seq.msg 의 한글 안전판 — 라벨 글꼴을 내용으로 고른다.
    한글을 mono 로 찍으면 자간이 벌어져 읽기 나빠진다(스타일 계약 타이포그래피)."""
    c = c or MUTED
    x1, x2 = d.LX[a], d.LX[b]; s = 1 if x2 > x1 else -1
    d.path(f"M {x1+10*s} {y} L {x2-12*s} {y}", c, 1.5, m=mk, dash=dash)
    mx = (x1 + x2) / 2
    lat = all(ord(ch) < 128 for ch in label)
    d.t(mx, y - 9, label, 11, c, MONO if lat else KR, "middle", 600)
    if sub:
        slat = all(ord(ch) < 128 for ch in sub)
        d.t(mx, y + 16, sub, 9 if slat else 10, MUTED, MONO if slat else KR)


def selfmsg(d, a, label, y, c=None, sub=None, w=58):
    c = c or MUTED
    x = d.LX[a]
    d.path(f"M {x+10} {y-10} L {x+w} {y-10} L {x+w} {y+10} L {x+13} {y+10}", c, 1.4, m="ar")
    lat = all(ord(ch) < 128 for ch in label)
    d.t(x + w + 10, y - 4, label, 11, c, MONO if lat else KR, "start", 600)
    if sub: d.t(x + w + 10, y + 13, sub, 10, MUTED, KR, "start")


def state(d, a, txt, y, c, dx=0):
    """레인 위 상태 칩. Seq.state 는 폭을 len()*7 로 잡아 한글이 넘친다."""
    x = d.LX[a] + dx
    w = textw_tight(txt, 11) + 26
