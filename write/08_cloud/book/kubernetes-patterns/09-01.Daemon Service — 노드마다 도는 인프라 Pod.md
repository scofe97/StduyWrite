...
DaemonSet에는 관리 방식에서 오는 고유한 특성이 여럿 있습니다. Pod에 `nodeName`이 이미 지정돼 **스케줄러 없이도** 뜰 수 있습니다. 그래서 다른 Pod가 노드에 놓이기 전에 실행될 수 있습니다. 스케줄러를 쓰지 않으니 노드의 `unschedulable` 필드도 존중하지 않습니다. `restartPolicy`는 `Always`만 가능하고, 여러 컨트롤러가 이 Pod를 더 높은 우선순위로 취급합니다.
...
| `unschedulable` | 스케줄러를 쓰지 않으므로 노드의 이 필드를 존중하지 않습니다 |
...
DaemonSet의 주 use case는 클러스터의 특정 노드에서 **시스템에 결정적인 Pod**를 돌리는 것입니다. DaemonSet 컨트롤러는 Pod 명세의 `nodeName` 필드를 설정해 Pod를 노드에 직접 할당하는 방식으로, 자격 있는 모든 노드가 Pod 복사본을 돌리도록 보장합니다. 이 덕분에 DaemonSet Pod는 기본 스케줄러가 시작되기도 전에 스케줄될 수 있고, 사용자가 구성한 스케줄러 커스터마이징의 영향도 받지 않습니다.
...
그래서 **Kubernetes v1.17 이후** DaemonSet은 Pod에 `nodeName` 대신 `nodeAffinity` 필드를 설정해 기본 스케줄러로 스케줄합니다. 이 변경으로 기본 스케줄러가 DaemonSet 실행의 필수 의존이 되지만, 동시에 taint·toleration·Pod 우선순위·preemption을 DaemonSet에 가져옵니다. 자원 기근 상황에서도 원하는 노드에 Pod를 돌리는 전반적 경험이 개선됩니다.
...
**4.** DaemonSet 컨트롤러가 Pod 명세의 `nodeName` 필드를 직접 설정해 노드에 할당하기 때문입니다. 여기서 네 가지 특성이 따라옵니다. 기본 스케줄러가 시작되기 전에도 스케줄될 수 있습니다. 다른 어떤 Pod가 노드에 놓이기 전에 실행됩니다. 사용자의 스케줄러 커스터마이징에도 영향받지 않습니다. 그리고 스케줄러를 쓰지 않으므로 노드의 `unschedulable` 필드를 존중하지 않습니다.
