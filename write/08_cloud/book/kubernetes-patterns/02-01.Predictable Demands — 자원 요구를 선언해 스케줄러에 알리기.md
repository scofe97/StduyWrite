...
related:
  ...
  - ../../docs/scheduling-eviction/01-01.노드 압박 축출과 디스크 관리 — DiskPressure는 무엇을 보고 켜지는가.md
...
> 『Kubernetes Patterns』 2장 정독. 핵심 한 줄은 "앱이 필요로 하는 자원과 런타임 의존성을 **미리 선언**해야 Kubernetes가 그 앱을 놓을 올바른 자리를 찾는다"입니다.

**Predictable Demands**는 이 책의 첫 실전 패턴입니다. 선언하지 않으면 스케줄러는 컨테이너를 속이 안 보이는 상자로 취급해, 클러스터가 꽉 차면 그냥 버립니다.
...
위 갈래인 런타임 의존성은 다시 둘로 나뉩니다. 이 구분이 이 장에서 가장 헷갈리는 지점입니다. 볼륨과 hostPort는 **스케줄 자체를 막습니다**. ConfigMap과 Secret은 스케줄은 통과시킨 뒤 **기동을 막습니다**.

아래 갈래인 자원 프로파일에서는 `requests`가 스케줄러의 노드 선택에 쓰입니다. 그리고 `requests`와 `limits`의 조합이 QoS 등급을 정해 kubelet이 죽이는 순서를 결정합니다.

가로줄 아래 두 상자는 이 흐름에 겹쳐 걸리는 별개 축입니다. Pod Priority는 QoS와 직교합니다. Quota와 LimitRange는 개별 Pod 바깥에서, 네임스페이스 단위로 총량과 기본값을 강제합니다.



## 핵심 요약

> 성공적인 배포·관리·공존의 토대는 앱의 **자원 요구와 런타임 의존성을 식별해 선언하는 것**입니다. 선언이 있어야 스케줄러가 어느 노드에 놓을지 지능적으로 판단하고 운영자가 용량 계획을 세울 수 있습니다.

자원은 `requests`(최소 보장)와 `limits`(상한)로 나눠 선언합니다. 이 둘의 조합이 Pod의 QoS 등급을 정하고, QoS 등급은 자원이 부족할 때 죽는 순서의 추정치가 됩니다.
...
비슷한 의존성이 **hostPort**에서도 생깁니다. hostPort로 컨테이너 포트를 호스트의 특정 포트에 노출하면, 노드에 대한 또 하나의 런타임 의존성이 생기고 Pod가 놓일 수 있는 자리가 제한됩니다. hostPort는 배치된 노드에서 그 포트를 점유하므로, **같은 `hostIP`·프로토콜·포트 조합을 쓰는 Pod는 노드당 하나**로 제한됩니다. 포트 충돌 때문에 그 워크로드는 클러스터의 노드 수만큼만 스케일할 수 있습니다. 포트가 다른 별개의 Pod까지 막지는 않으므로, 제한을 거는 것은 노드 자체가 아니라 그 포트 조합이라고 읽어야 정확합니다.
...
| **노드 전체의 가용 메모리** | kubelet (축출) | Pod가 **축출**돼 상태가 `Failed`가 되고, 컨트롤러가 관리하는 Pod라면 대체 Pod가 새로 생성됩니다 |
...
다만 `OOMKilled`가 곧 "내 limit을 넘겼다"는 확증은 아닙니다. kubelet이 메모리를 회수하기 전에 노드가 먼저 OOM에 빠지면 커널 `oom_killer`가 대신 반응합니다. 이때는 QoS로 정해지는 `oom_score_adj`(Guaranteed −997, BestEffort 1000)가 얹혀, **자기 request 대비 메모리를 많이 쓰는 낮은 QoS 컨테이너가 먼저 죽습니다.**[^node-oom] 그래서 `OOMKilled`를 봤을 때는 컨테이너 limit과 노드 전체 메모리를 함께 봐야 합니다.
...
![선언 조합별 QoS 등급과 축출에서의 의미](./_assets/02-01.qos-and-resources.svg)

왼쪽 열이 무엇을 적었는가이고 오른쪽 세 열이 그 결과입니다. 주황으로 강조한 둘째 행이 이 책이 권하는 조합입니다. `requests`만 적고 `limits`를 생략한 이 흔한 경우가 Burstable이라는 점, 그리고 등급 이름이 축출 순서를 확정하지 않는다는 점이 이 그림에서 읽어야 할 두 가지입니다.
...
두 가지 단서를 붙여 둡니다. 첫째, 판정에 쓰이는 자원은 **CPU와 메모리뿐**입니다. `ephemeral-storage`나 hugepages만 선언한 Pod는 등급상 여전히 BestEffort입니다. 둘째, 위 세 줄은 **컨테이너 수준 선언** 기준입니다. Pod-level resources(`spec.resources`)를 쓰는 최신 클러스터에서는 Pod 수준의 CPU·메모리 request와 limit이 같아도 Guaranteed가 됩니다.
...
메모리를 `requests = limits`로 고정하면 그 컨테이너의 메모리 몫은 등급과 무관하게 예약량과 상한이 같아집니다. 그래서 노드가 메모리로 압박받아도 이 컨테이너는 자기 request를 넘긴 상태로 오래 남지 않습니다. limit 집행은 사후 반응이라 순간 초과 자체는 있을 수 있지만, 그 즉시 limit에 걸려 되돌려집니다. 뒤에서 보겠지만 축출 판정에서 실제로 중요한 것이 바로 그 "request를 넘겼는가"입니다.
...
**QoS와 Pod Priority는 직교하는 두 기능으로, 서로 연결돼 있지 않고 겹침도 거의 없습니다.** QoS는 주로 **kubelet**이 컴퓨트 자원이 낮을 때 **노드 안정성**을 지키는 맥락에서 쓰입니다. 다만 앞서 봤듯 kubelet이 축출 대상을 고를 때 실제로 보는 것은 등급이 아니라 request 초과 여부이고, Pod Priority는 그다음 기준으로 들어옵니다. 반대로 **스케줄러**의 preemption 로직은 대상을 고를 때 **QoS를 완전히 무시**합니다. 대기 중인 높은 우선순위 Pod의 요구를 만족하는 가장 낮은 우선순위 Pod 집합을 고를 뿐입니다.
...
spec:
  limits:
  - min:            { memory: 250Mi, cpu: 250m }   # requests·limits 최소 (defaultRequest 이하여야 함)
...
원서 예제는 `min.cpu`를 `500m`으로 적지만 위에서는 `250m`으로 낮췄습니다. API 서버가 `min`이 `defaultRequest`보다 크면 LimitRange 생성을 거부하기 때문입니다 — `min value 500m is greater than default request value 250m`으로 떨어집니다.[^limitrange-validation] 원서 값을 그대로 옮기면 적용되지 않는 매니페스트가 됩니다.
...
환경마다 컨테이너 자원 프로파일과 인스턴스 수가 달라, 다목적 환경의 용량 계획은 단순하지 않습니다. 비프로덕션 클러스터는 하드웨어 활용을 위해 주로 Best-Effort·Burstable 컨테이너를 둡니다. 죽어도 치명적이지 않기 때문입니다. 프로덕션 클러스터는 안정·예측을 위해 주로 Guaranteed와 일부 Burstable을 둡니다. 여기서 컨테이너가 죽으면 용량을 늘려야 한다는 신호로 읽습니다.
...
첫째, 힙을 컨테이너 limit의 일부(대략 75% 안팎)로만 잡습니다. 최신 JVM은 `-XX:MaxRAMPercentage`로 컨테이너 인지 자동 계산을 지원해, 고정 `-Xmx` 대신 이 플래그를 쓰면 limit이 바뀌어도 비율이 유지됩니다. 다만 **JVM 기본값이 25%라 `-XX:MaxRAMPercentage=75` 처럼 값을 반드시 붙여 써야 합니다.** 이 플래그는 값을 요구하므로 `=75` 없이 넘기면 옵션 오류로 기동이 실패하고, 아예 지정하지 않으면 기본값 25%가 적용돼 힙이 limit의 4분의 1로 좁아집니다.
...
> 자원 관리의 개념 메커니즘(QoS 결정 규칙·CPU throttling·LimitRange/ResourceQuota 상세)은 공식 문서 [Resource Management for Pods and Containers](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/)가 정본입니다. 이 편은 그것을 다시 설명하지 않고 **패턴의 의도와 Spring 렌즈**만 남깁니다.
...
그럼에도 메모리를 같게 두는 것이 최선인 이유는 등급 이름이 아니라 실질에 있습니다. 예약량과 상한이 같으면 그 컨테이너는 **자기 memory request 를 넘긴 채로 남을 수 없고**(넘기는 즉시 limit 에 걸립니다), 축출 판정에서 실제로 중요한 것이 바로 그 "request 를 넘겼는가" 이기 때문입니다. 덤으로 상한이 예측 가능해져 용량 계획이 쉬워집니다.
...
**17.** 서로 연결돼 있지 않고 겹침도 거의 없다는 뜻입니다. **kubelet** 은 컴퓨트 자원이 낮을 때 노드 안정성을 지키는 쪽에서 움직입니다. 축출 대상은 request 초과 여부를 먼저 보고 그다음 Pod Priority 와 초과량으로 정합니다. 반대로 **스케줄러**의 preemption 로직은 대상을 고를 때 **QoS 를 완전히 무시**합니다. 대기 중인 높은 우선순위 Pod 의 요구를 만족하는 가장 낮은 우선순위 Pod 집합을 고를 뿐입니다.
...
> 이 패턴을 실제 배포에 옮길 때 지킬 것들입니다. 값을 추측하지 않고 실측하는 것이 출발점이고, 나머지는 그 값을 어떻게 거는가의 문제입니다.
...
주의할 것이 둘 있습니다. 첫째, 축출 순서를 등급이 결정하지 않습니다. 공식 문서는 kubelet이 QoS 등급으로 순서를 정하지 않는다고 명시하고, 실제로는 request 초과 여부와 Pod Priority와 초과량을 봅니다. 등급은 추정치입니다. 둘째, 흔히 권장되는 "메모리는 requests = limits, CPU는 requests만"을 따르면 CPU limit이 없어 등급은 Burstable입니다. 그래도 이 설정이 옳은 이유는 등급 이름이 아니라, 메모리 예약량과 상한이 같아지면 그 컨테이너가 자기 request를 넘긴 채로 남을 수 없기 때문입니다. 축출 판정의 1번 기준이 바로 그것입니다.
...
- 개념 노트: [노드 압박 축출과 디스크 관리](../../docs/scheduling-eviction/01-01.%EB%85%B8%EB%93%9C%20%EC%95%95%EB%B0%95%20%EC%B6%95%EC%B6%9C%EA%B3%BC%20%EB%94%94%EC%8A%A4%ED%81%AC%20%EA%B4%80%EB%A6%AC%20%E2%80%94%20DiskPressure%EB%8A%94%20%EB%AC%B4%EC%97%87%EC%9D%84%20%EB%B3%B4%EA%B3%A0%20%EC%BC%9C%EC%A7%80%EB%8A%94%EA%B0%80.md) — kubelet 이 실제로 축출을 고르는 기준
...
[^node-oom]: Kubernetes — Node-pressure Eviction 의 "Node out of memory behavior". kubelet 이 회수하기 전에 노드가 OOM 에 빠지면 커널 oom_killer 가 QoS 기반 oom_score_adj 를 더해 대상을 고릅니다.
<https://kubernetes.io/docs/concepts/scheduling-eviction/node-pressure-eviction/#node-out-of-memory-behavior>

[^limitrange-validation]: Kubernetes 소스 `pkg/apis/core/validation/validation.go` 의 `ValidateLimitRange` — `min` 이 `defaultRequest` 보다 크면 생성을 거부합니다.
