...
### 핵심 포인트 3가지
1. Endpoints의 ready/notReady 두 목록이 readiness probe의 실체적 결과입니다 — 서비스가 트래픽을 보낼 주소는 ready 목록뿐입니다.
2. 병목의 구조는 "모든 노드의 kube-proxy가 모든 Endpoints를 watch + 변경 시 객체 전체 재전송"입니다 — Pod가 많을수록 객체가 크고 변경이 잦아 곱으로 악화됩니다.
3. slice는 기본 100개(최대 1,000) 주소 단위로 쪼개지고, 기존 slice를 먼저 채우되 재균형은 없으며, 트랜잭션 API 부재로 일시적 중복이 가능합니다.

### 자주 묻는 질문
Q: StatefulSet에는 왜 headless 서비스를 권하나요?
A: StatefulSet의 가치가 Pod별 고정 정체성인데, 일반 ClusterIP는 어느 Pod로 갈지 가려 버리기 때문입니다. headless 서비스는 Pod별 DNS 레코드(postgres-0.postgres...)를 만들어 클라이언트가 특정 멤버를 직접 지목할 수 있게 합니다.

Q: Pod는 정상인데 서비스에서 트래픽이 안 옵니다. 어디부터 보나요?
A: `kubectl describe endpoints <서비스>`로 그 Pod IP가 Addresses에 있는지, NotReadyAddresses로 밀려나 있는지 봅니다. 후자라면 readiness probe 실패이고, 아예 없다면 라벨 셀렉터 불일치를 의심합니다.

Q: EndpointSlice 도입으로 kube-proxy 설계가 바뀌었나요?
A: 아닙니다. "어떤 Pod든 어떤 서비스로 즉시 라우팅 가능"이라는 전제(전 노드 watch)는 유지하고, watch당 전송되는 데이터 단위만 전체 목록에서 부분집합으로 줄였습니다.
