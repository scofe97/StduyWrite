...
review:
  ...
  # 2026-09-05 도식 보강 — 콘텐츠 절 3개 중 §3 에만 도식이 있어 §1·§2 에 한 장씩 신설.
  # §1 stateful-identity(type-tree) — 이름이 headless Service 에서 Pod 별 레코드와 서비스 레코드로 갈리는 구조.
  #   본문의 '두 레코드가 공존한다 · 클라이언트가 고른다'와 'serviceName 필수 · headless 여야 위 갈래가 생긴다'를 형태로 옮겼다.
  # §2 endpoints-two-lists(type-flowchart) — readiness 통과·실패가 .addresses/.notReadyAddresses 로 갈리고,
  #   아래 띠에 라벨 제거 시 두 컨트롤러(endpoints · ReplicaSet)가 각각 하는 일을 붙였다.
  # 타입은 이 편의 기존 둘(process · dp-security-matrix)과 겹치지 않게 골랐다. 사실 변경 없음 — 본문 서술을 그대로 그렸다.
updated: 2026-09-05
---
...
서비스를 배우기 전에 그 재료부터 보는 편입니다. StatefulSet은 Pod에 안정된 네트워크 정체성을 주는 워크로드입니다. Endpoints는 서비스가 라우팅할 준비된 주소 목록이고, EndpointSlice는 그 목록이 대형 클러스터에서 병목이 되는 문제를 부분집합 분할로 푼 진화형입니다.
...
![이름은 headless Service 에서 두 갈래로 갈린다](_assets/05-01.stateful-identity.svg)

왼쪽이 ordinal 을 주는 워크로드이고, 가운데가 그 이름을 맡는 headless Service 입니다. 오른쪽 두 갈래가 거기서 갈려 나오는 레코드 둘인데, 위가 멤버를 지목하는 이름이고 아래가 서비스 전체를 부르는 이름입니다.
...
기본 설정은 `updateStrategy.type: RollingUpdate` + `podManagementPolicy: OrderedReady`입니다. 여기서 업데이트한 Pod가 끝내 ready가 되지 못하면 수동 개입이 필요합니다. StatefulSet은 "고정된 영속 머신 집단"을 본뜬 범용 해법이라 답답할 때가 있고 서드파티 대안도 많으며 일상적 앱 배포용이 아닙니다.
...
![readiness 의 결과가 두 목록으로 갈린다](_assets/05-01.endpoints-two-lists.svg)

왼쪽 검사 하나가 가운데 두 목록으로 갈리고, 오른쪽이 그 목록에 오른 주소가 받는 대우입니다. 아래 띠는 라벨을 뗐을 때 두 컨트롤러가 각각 무엇을 하는지입니다.
...
문제는 규모에서 옵니다. kube-proxy는 모든 노드에서 돌며 서비스 라우팅을 위해 클러스터의 모든 Endpoints를 watch합니다. 노드 수천 대·Pod 수만 개면 주소 하나만 바뀌어도(롤링 업데이트·스케일·축출·헬스체크 실패) Endpoints 객체 전체가 kube-proxy 수천 개로 갑니다. Pod가 많을수록 객체는 커지고 변경은 잦아지니 etcd·API 서버·네트워크 전체가 짓눌립니다. 저자들은 "많은 사용자가 endpoint watch를 클러스터 규모의 궁극적 병목으로 꼽는다"고 전합니다.
...
세부 규칙도 실무에 닿습니다. slice당 최대 주소 수는 kube-controller-manager의 `--max-endpoints-per-slice`로 정하며 기본 100, 최대 1,000입니다. 컨트롤러는 새 slice를 만들기 전에 기존 slice를 채우려 하지만 재균형(rebalance)은 하지 않습니다. 트랜잭션 API가 없어 같은 주소가 잠시 여러 slice에 겹칠 수 있으므로 slice 소비 코드(kube-proxy 포함)는 이를 감안해야 합니다. 특정 서비스의 slice들은 `kubernetes.io/service-name` 라벨로 조회합니다.

**EndpointSliceMirroring 컨트롤러**가 Endpoints 를 slice 로 미러링합니다. 기존 시스템이 Endpoints 를 계속 쓰는 동안에도 slice 를 진실의 원천으로 삼을 수 있습니다. 앞의 EndpointSlice 컨트롤러와는 다른 컨트롤러입니다.
...
- StatefulSet 상세(ordinal·Pets vs Cattle·headless 결합)는 형제 정독본 [『Kubernetes in Action』 16-01](../kubernetes-in-action/16-01.StatefulSet%20%EA%B8%B0%EC%B4%88%20%E2%80%94%20Pets%20vs%20Cattle%C2%B7ordinal%C2%B7headless%20Service.md)이 정본입니다. 이 편은 네트워킹(DNS 정체성)만 다룹니다.
...
7. (빈칸) slice당 주소는 기본 ___개·최대 ___개입니다. 컨트롤러는 기존 slice를 먼저 채우되 ___은 하지 않습니다. 트랜잭션 API가 없어 같은 주소가 ___에 일시 등장할 수 있습니다.
...
"EndpointSlice란 서비스 뒤 Pod 주소 목록을 쪼갠 리소스로, 바뀐 slice만 보내 대형 클러스터의 Endpoints watch 병목을 줄입니다."
