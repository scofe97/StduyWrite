...
![짧은 이름 하나가 다섯 번의 질의가 된다](_assets/04-03.dns-search-path.svg)

윗줄이 차례로 시도되는 이름이고 그 아래가 결과입니다. 맨 아래 두 상자가 횟수를 줄이는 두 처방입니다.
...
질의 비용의 근원이 검색 경로입니다. 리졸버는 **질의명을 각 검색 도메인 뒤에 붙여** 후보 이름을 만듭니다. `web` 한 마디를 물으면 `web.default.svc.cluster.local` → `web.svc.cluster.local` → `web.cluster.local` → (호스트 검색 도메인) → 마지막에 `web` 자체 순으로 시도됩니다. 뒤쪽 두 개가 그 자체로 질의되는 게 아니라 **매번 앞에 `web.` 이 붙는다**는 점이 요점입니다.

시도 횟수는 검색 도메인 수(클러스터 기본 3개)에 딸린 값이고, 각 시도가 A·AAAA 두 질의를 내보내므로 실제 패킷은 그 두 배입니다. 짧은 이름 하나에 왕복이 여러 번 쌓여 지연을 키웁니다. `ndots:5` 의 5는 **질의 횟수가 아니라** "점이 5개 미만이면 검색 경로를 먼저 시도한다"는 임계값입니다 — 둘을 섞어 "다섯 번 질의한다"로 읽으면 안 됩니다.
