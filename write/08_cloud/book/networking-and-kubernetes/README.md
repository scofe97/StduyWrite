...
| 난이도 레벨 | **같은 레벨 유지** — 01-02 Phase 1 에서 막힌 축이 셋이었지만 전부 되묻기 한두 번에 자력 도달했다. 내릴 이유는 없고, 올리기엔 "어느 계층이 무슨 책임을 지는가"가 아직 흔들린다. 01-03 이 마침 그 축(IP·라우팅의 책임 범위)이라 표적으로 맞는다 |
| 막힌 지점 | 01-02 (1) 공개키 방향 반전 — "클라이언트가 자물쇠를 주고 서버가 열쇠로 푼다"로 이해. 실제는 서버가 `Certificate` 로 자물쇠를 주고, 잠그는 쪽이 클라이언트 (2) 첫 메시지가 암호화돼 있다는 전제 — `ClientHello` 시점에는 잠글 도구가 손에 없다 (3) 누락 감지를 L7 책임으로 배치 — 누락·순서는 L4 가 끝내고 올려 보내며 L7 은 **경계**(`Content-Length`)를 맡는다. 셋 다 "몰랐다"가 아니라 "안다고 여겼다"에 속한다. 01-01 잔여 3건은 이번 회차에서 재확인 완료 |
| 다음 레슨 후보 | **01-03 Phase 1** — 계층별 책임 배치를 표적으로 삼는다. 1장 실습(Phase 3)은 01-04 가 01-02·01-03 개념에 의존하므로 01-04 까지 읽은 뒤 한 세션으로 몰아서 진행 |
| 최근 검증 결과 | 2026-08-16~17 01-02 learning-session — Phase 1 통과(문서 §1~§6 전체 커버). 잘못 알던 인과 3건 교정(위 막힌 지점). 예측 대조: TLS 핸드셰이크 화살표 수를 3 으로 예측했으나 실제 9 — 3-way 는 신뢰 관계가 이미 선 상태에서 번호만 맞추면 되지만 TLS 는 연장 합의·신원 확인·비밀 전달·조작 여부 되짚기 네 가지를 해야 한다는 차이를 못 본 데서 어긋났다. 이전 회차는 2026-08-15 01-01 learning-session(Phase 1 통과, Phase 4 5축 자답, 막힘 3) |
| 보강 산출물 | 전체 요약 도식 13장 신설(`*.chapter-overview.svg`, svg-check 전부 PASS) · H2 요약 인용 전 절 삽입 · 원문자 본문·SVG 전수 제거 · 기존 SVG 15장 가독성 정합화(대비 경고 100→8) · 2026-08-15 `01-01.encapsulation-roundtrip` 신설(Archify architecture, 정적 QA 31개 통과) + 01-01 §3·§4 설명 빈틈 3곳 보강 · 2026-08-16~17 01-02 `tls-handshake.sequence`(archify sequence, 메시지 10·구간 3·활성 5) 신설 + §3·§5 에 비유 절 2개 신설(전화 통화 / 은행 창구) |
| 복습 회차 | 0 (간격 반복 `_review` 기준). 별도로 learning-session 진행 2편 — 01-01(Phase 1·4), 01-02(Phase 1) |
