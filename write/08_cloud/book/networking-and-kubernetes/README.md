...
| [00-03](./00-03.%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC%20%EC%84%A0%EC%88%98%20%EA%B0%9C%EB%85%90.md) | 네트워크 선수 개념 — 주소·인터페이스·장비 (본문이 설명 없이 쓰는 낱말) | 초안 |
...
| 난이도 레벨 | **같은 레벨 유지** — 01-02 Phase 1 에서 막힌 축이 여섯이었지만 전부 되묻기 한두 번에 자력 도달했다. 특히 제시된 비유가 개념과 어긋나는 지점을 세 번 정확히 지적해(우편함이 암호화를 못 담음 · 암구호가 잠그는 도구가 아님 · 자물쇠 비유로는 서명 방향이 설명되지 않음) 매번 비유를 갈아야 했다. 내릴 이유는 없고, 올리기엔 "어느 계층이 무슨 책임을 지는가"가 아직 흔들린다. 01-03 이 마침 그 축(IP·라우팅의 책임 범위)이라 표적으로 맞는다 |
| 막힌 지점 | 01-02 여섯 건, 전부 "몰랐다"가 아니라 **"안다고 여겼다"** 계열이다. (1) 공개키 방향 반전 — 실제는 서버가 `Certificate` 로 자물쇠를 주고 잠그는 쪽이 클라이언트 (2) 첫 메시지가 암호화돼 있다는 전제 — `ClientHello` 시점에는 잠글 도구가 손에 없다 (3) 누락 감지를 L7 책임으로 배치 — 누락·순서는 L4 가 끝내고 올려 보내며 L7 은 **경계**(`Content-Length`)를 맡는다 (4) 시퀀스 번호를 **패킷** 단위로 이해 — 실제는 바이트이고, 방향마다 따로 센다 (5) TIME-WAIT 주체를 서버로 봄 — 마지막 ACK 를 **보낸** 쪽(먼저 닫은 쪽)이 2MSL 을 기다린다 (6) 4-way 의 "마지막 ACK" 를 첫 FIN 으로 혼동. (3)·(6) 이 같은 계열이라 다음 편 표적. 01-01 잔여 3건은 이번 회차에서 재확인 완료 |
| 다음 레슨 후보 | **01-03 Phase 1** — 계층별 책임 배치를 표적으로 삼는다. Phase 1 은 **절 커버리지 표를 먼저 세우고** 시작한다(01-01·01-02 에서 일부 절만 다루고 게이트를 닫은 일이 두 번 있었다). 1장 실습(Phase 3)은 01-04 가 01-02·01-03 개념에 의존하므로 01-04 까지 읽은 뒤 한 세션으로 몰아서 진행 |
| 최근 검증 결과 | 2026-08-16~17 01-02 learning-session — Phase 1 통과, 문서 **§1~§6 전체** 커버. 1차 판정은 §5·§1 만 다루고 "전체 커버"로 닫았던 것을 철회하고 §2·§3·§4·§6 을 추가로 다뤘다. 예측 대조: TLS 핸드셰이크 화살표 수를 3 으로 예측했으나 실제 9 — 3-way 는 신뢰 관계가 이미 선 상태에서 번호만 맞추면 되지만 TLS 는 연장 합의·신원 확인·비밀 전달·조작 여부 되짚기 네 가지를 해야 한다는 차이를 못 본 데서 어긋났다. 이전 회차는 2026-08-15 01-01 learning-session(Phase 1 통과, Phase 4 5축 자답, 막힘 3) |
| 보강 산출물 | 전체 요약 도식 13장 신설(`*.chapter-overview.svg`, svg-check 전부 PASS) · H2 요약 인용 전 절 삽입 · 원문자 본문·SVG 전수 제거 · 기존 SVG 15장 가독성 정합화(대비 경고 100→8) · 2026-08-15 `01-01.encapsulation-roundtrip` 신설(Archify architecture, 정적 QA 31개 통과) + 01-01 §3·§4 설명 빈틈 3곳 보강 · 2026-08-16~17 01-02 archify 도식 6장 신설(`tcp-12packets`·`tcp-open-3way`·`tcp-close-paths`·`handshake-why-3-and-4`·`tcp-window`·`tcp-vs-udp`) + `tls-handshake.sequence` 신설, **mermaid 3 → 0**, §3·§5 중복 산문 정리, §2 플래그 원말·시퀀스 방향 독립 보강, §4 캡처 줄별 주석, §3·§5 비유 절 2개(전화 통화 / 은행 창구) |
| 복습 회차 | 0 (간격 반복 `_review` 기준). 별도로 learning-session 진행 2편 — 01-01(Phase 1·4), 01-02(Phase 1, 전체 절 커버) |
