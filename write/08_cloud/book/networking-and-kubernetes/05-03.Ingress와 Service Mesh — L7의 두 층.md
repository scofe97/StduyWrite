...
review:
  ...
  #   ① Istio 유래 — Envoy가 Lyft산이고 Istio는 Google·IBM·Lyft(Envoy팀) 공동 (책이 "Lyft가 Istio를"로 오기)
  #   ② Linkerd 자기모순 — 책이 "트래픽 시프팅·트레이싱 없음"이라 했으나 같은 장 install 출력에 SMI
...
![L7의 두 층 — Ingress(밖→안)와 Service Mesh(안↔안)](_assets/05-03.ingress-vs-mesh-two-layers.svg)
...
3. **앱을 메시에 넣기** — `cat web.yaml | linkerd inject - | kubectl apply -f -`가 Deployment에 `linkerd.io/inject: enabled` 어노테이션을 심고, proxy-injector(mutating webhook)가 그 어노테이션을 보고 사이드카를 주입합니다.
