...
**밖에서 클러스터로 들어오는 구간**은 대체로 주소를 바꿉니다. NodePort 패킷은 DNAT 으로 목적지가 컨트롤러 Pod 가 되고 `externalTrafficPolicy` 가 기본값 `Cluster` 면 출발지도 노드 IP 가 됩니다. 클라우드 LB 는 구현마다 달라서, 스스로 연결을 끊고 새로 여는 것도 있고 Pod 를 직접 겨누는 것도 있습니다. 04-01 §1 이 "출발지까지 바뀌는 경우" 셋 중 마지막으로 적어 둔 자리가 이것입니다.
...
단서가 하나 붙습니다. 컨트롤러가 백엔드를 서비스의 ClusterIP 로 부르면 그 한 걸음에는 DNAT 이 붙습니다. 커뮤니티 NGINX Ingress 는 기본적으로 EndpointSlice 의 Pod IP 로 붙어 DNAT 을 건너뜁니다. `service-upstream` 이나 SSL passthrough 를 쓰면 ClusterIP 를 거칩니다. 컨트롤러와 설정에 달린 이야기입니다.

### 리액트 앱은 클러스터에 있는데 왜 밖에서 들어옵니까

![정적 파일은 클러스터에, 실행은 브라우저에](_assets/05-03.react-runs-in-browser.svg)

위 그림이 이 물음의 답입니다. 클러스터 안에는 정적 파일을 서빙하는 Pod 가 있고, 그 파일을 받아 실행하는 자리는 사용자 브라우저입니다. 프런트엔드를 클러스터에 올려 두고 나면 "내 앱도 클러스터 안에 있으니 API 호출은 내부 통신 아닌가"라는 생각이 자연스럽게 따라옵니다.

정적 SPA 를 전제로 보면 답이 분명합니다. **클러스터에 있는 것은 그 앱의 정적 파일이지 실행이 아닙니다.** 브라우저가 `index.html` 과 번들 JS 를 받아 간 순간, 그 코드는 사용자의 브라우저에서 돕니다. 그래서 리액트가 부르는 API 는 클러스터 안에서 나가는 요청이 아니라, 앞 절 두 구간 그림의 맨 윗줄로 **다시 들어오는** 요청입니다. 파일이 어디서 왔는지는 그다음 요청이 어디서 출발하는지와 상관이 없습니다.
...
TLS는 `.spec.tls[*].secretName`으로 `kubernetes.io/tls` Secret(`tls.crt`·`tls.key`, base64)을 참조해 기본 지원됩니다. 인증서를 손으로 관리하기 싫다면 cert-manager로 자동 발급·갱신할 수 있습니다.
...
실습은 커뮤니티 NGINX 컨트롤러를 KIND에 배포하는 것으로 시작합니다(extraPortMappings로 로컬 80/443 연결). `/host`는 기존 서비스로, `/data`는 두 번째 Deployment·서비스로 보내는 규칙을 적용합니다. `curl localhost/host`·`curl localhost/data`는 서로 다른 백엔드에 닿습니다. 배포 후 `kubectl wait --for=condition=ready pod --selector=app.kubernetes.io/component=controller`로 컨트롤러 준비를 기다린 뒤 규칙을 적용합니다.
...
화살표 위에 적힌 것이 자리를 옮긴 기능입니다. 아래 띠가 그 이동이 만들어 낸 성질입니다. 옮겼다고 공짜는 아닙니다. Pod 마다 프록시가 하나씩 늘어 그만큼 자원과 지연이 붙습니다.
...
구조는 셋입니다. 클러스터 출입 트래픽은 게이트웨이가, 서비스 간 트래픽은 Pod 안 사이드카 프록시가 mTLS로 처리하고 컨트롤 플레인이 이를 설정합니다.

이 분리가 운영 안정성의 근거이기도 합니다. 원문은 "컨트롤 플레인이 다운돼 갱신이 불가능해도 서비스·앱 트래픽은 영향받지 않는다"까지입니다. 사이드카가 이미 받은 설정으로 계속 돈다는 이유는 풀이입니다.
...
저자들이 꼽는 메시 최고 사용처는 서비스 간 mTLS, 그다음이 개발자에겐 서킷브레이커·장애 테스트, 네트워크 관리자에겐 고급 라우팅 정책입니다.
...
2. **설치** — `linkerd install | kubectl apply -f -`로 깔고 `linkerd check`로 검증합니다. identity·proxy-injector·sp-validator 등 RBAC·웹훅이 함께 깔립니다.
3. **앱을 메시에 넣기** — `cat web.yaml | linkerd inject - | kubectl apply -f -`가 Deployment에 `linkerd.io/inject: enabled` 어노테이션을 심습니다. proxy-injector(mutating webhook)가 이를 보고 사이드카를 주입합니다.
...
> 💬 **비유**: Ingress와 메시는 건물의 두 통신 설비입니다. Ingress는 1층 안내 데스크로, 방문객(외부 HTTP)의 용건(host·path)대로 부서를 안내하고 데스크 하나로 여러 부서를 감당해 경비(LB 비용)를 아낍니다. 서비스 메시는 부서 간 전화망 교체 공사로, 책상마다 비서(사이드카)가 통화를 암호화(mTLS)하고 불통이면 재다이얼(재시도)하며 품질을 기록(관측)합니다. 직원(앱 코드)은 아무것도 바꾸지 않았는데 통신 품질이 바뀝니다.
...
- Ingress는 기능 동결됐고 후계는 Gateway API(GatewayClass·Gateway·HTTPRoute, 2023년 1.0 GA)입니다 — [Gateway API 공식](https://gateway-api.sigs.k8s.io/). 역할 분리와 표현력이 Ingress의 어노테이션 남발 문제를 대체합니다. 역할은 인프라·클러스터 운영·앱 개발로 나뉘고, 표현력에는 헤더 매칭과 가중치 분배가 들어갑니다. 형제 정독본 [13-01 Gateway API](../kubernetes-in-action/13-01.Gateway%20API%20%EA%B0%9C%EB%85%90%EA%B3%BC%20Gateway%20%EB%B0%B0%ED%8F%AC.md)가 실습 정본입니다.
...
### 1. Ingress 한 줄 정의
"Ingress란 호스트·경로로 HTTP를 서비스에 라우팅하는 L7 설정 스펙입니다. LB 하나로 다수 서비스를 노출해 비용을 통합하고 동작은 따로 배포한 컨트롤러가 맡습니다."

### 2. 스펙 + 컨트롤러
Ingress는 스펙(설정 명세)일 뿐이라, 규칙만 만들고 컨트롤러가 없으면 아무 일도 일어나지 않습니다. Kubernetes는 기본 컨트롤러를 시작해 주지 않으므로 직접 배포해야 합니다. 컨트롤러가 여럿이면 각 Ingress가 IngressClass(`.spec.ingressClassName`)로 고릅니다.
...
LoadBalancer는 L4라 프로토콜을 안 가리지만 워크로드와 1:1입니다. Ingress는 L7(HTTP 전용)이라 host·path로 LB 하나 뒤 여러 서비스에 나눕니다. 비HTTP는 LoadBalancer, HTTP 다수 서비스는 Ingress가 기본 선입니다.
...
### 8. Ingress 와 NAT
구간을 갈라야 답이 됩니다. 밖에서 들어오는 구간은 대체로 DNAT 으로 목적지를 컨트롤러 Pod 로 바꾸고 `externalTrafficPolicy` 가 `Cluster` 면 출발지도 노드 IP 로 바꿉니다. 반면 컨트롤러에서 백엔드로 가는 구간은 NAT 이 아니라 프록시입니다 — 연결을 받아 끝내고 새로 하나를 엽니다. "NAT 없음" 약속은 Pod 사이 통신과 노드-Pod 통신에 걸린 것이라, 경계를 넘어 들어오는 첫 구간과는 애초에 무관합니다.
