...
아래는 이 편이 다루는 두 층을 한 장에 놓은 지도입니다. 둘을 가르는 기준은 트래픽의 방향입니다. 클러스터 경계를 넘어 밖에서 안으로 들어오는 HTTP 를 맡는 것이 위쪽의 Ingress 입니다(§1·§2). 클러스터 안에서 서비스끼리 오가는 통신을 맡는 것이 아래쪽의 메시입니다(§3·§4). 메시 쪽 서비스 상자마다 붙은 사이드카가 앱 코드 대신 mTLS 와 재시도를 처리하는 프록시입니다. 컨트롤 플레인은 그 사이드카들에 설정을 배포합니다.
