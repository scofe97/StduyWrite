...
두 모드의 갈림을 한 장으로 접으면 이렇습니다. 위 줄이 routes-based, 아래 줄이 VPC-native 이고, 오른쪽 칸이 각 모드에서 트래픽이 Pod 에 닿는 방식입니다.
...
표의 앞부분을 도식으로 접으면 차이가 두 층으로 보입니다. 위 줄이 가상망의 범위와 서브넷 경계이고, 아래 줄이 기본 라우트와 라우팅 범위, 그 위에 얹히는 관리형 Kubernetes 입니다. GCP 만 라우팅 범위가 VPC 이고 나머지 둘은 서브넷이라는 점이 위쪽의 글로벌·리전 차이와 짝을 이룹니다.
