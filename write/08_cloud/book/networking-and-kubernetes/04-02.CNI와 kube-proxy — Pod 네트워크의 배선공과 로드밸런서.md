...
아래는 이 편이 다루는 두 컴포넌트를 나란히 놓은 지도입니다. 왼쪽 줄기가 Pod 에 주소와 길을 주는 CNI 이고, 오른쪽 줄기가 서비스 IP 를 실제 Pod 로 번역하는 kube-proxy 입니다. 둘은 일하는 시점이 다릅니다. CNI 는 Pod 가 만들어질 때, kube-proxy 는 트래픽이 들어올 때 움직입니다. 각 상자 오른쪽의 절 번호가 본문에서 그것을 다루는 곳입니다. 색이 붙은 곳은 한 군데뿐이고 경고입니다. iptables 모드가 진짜 로드밸런싱이 아니라는 사실이 실무 오해 1순위라 표시했습니다.
