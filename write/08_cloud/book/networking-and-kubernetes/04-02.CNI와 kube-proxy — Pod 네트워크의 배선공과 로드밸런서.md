...
source:
  ...
  - "강의 트랜스크립트: Ch-06 Network 2 — Linux Network (플레이빌더 카이)"  # 9스텝 점검 순서 (원서 밖 보강, 02_os/networking 이관, 미실측)
related:
...
allow:
  ...
  - "10.0.0.0/8"        # §5 FIB 트리 예제 — RFC 1918 대역 자체 (02_os/networking 이관)
  - "10.244.0.0/16"     # Flannel 기본 Pod CIDR 예제
  - "10.244.175.0/26"   # §5 노드별 Pod 블록 예제
  - "10.244.49.64/26"   # 같은 사유
  - "10.244.242.64/26"  # 같은 사유
  - "10.244.242.66"     # §5 9스텝의 예제 Pod 주소
review:
  ...
  #     학습자가 Phase 1 에서 브리지·대역(=stdin 설정)만 답하고 "어느 Pod 인지" 축을 못 댄 자리다. CNI_NETNS 를 02-05 실습의 `ip link set … netns` 와 이었다.
  ...
  #     서비스 5천 × Pod 10 = 엔드포인트 줄 5만이라는 계산과, 한 줄 갱신에 테이블 전체 재작성이라는 축. 메커니즘 수치는 02-03 로 링크.
  ...
  #   §4 체인 절을 `### 서비스 하나가 체인 셋으로 번역된다`로 재작성 — 학습자가 02-03 와 중복임을 지적했고 실제로 확률 계산·규칙 평가가 겹쳤다.
  #     이 편의 관심은 "서비스 오브젝트가 무엇으로 번역되는가"라 3층 표만 남기고 규칙 평가와 확률 산식은 02-03 §3 으로 넘겼다.
...
CNI 명세는 놀랄 만큼 단순합니다 — 바이너리 하나가 ADD·DEL·CHECK·VERSION 네 연산을 JSON stdin/stdout으로 처리하면 끝입니다. 그 뒤에서 Cilium·Flannel·Calico는 전혀 다른 방식으로 경쟁합니다. kube-proxy는 2장의 iptables 확률 fan-out과 IPVS로 서비스를 로드밸런싱합니다.
...
도식의 2번이 순서상 중요합니다. 네임스페이스가 먼저 생겨야 플러그인에게 넘길 `CNI_NETNS` 값이 존재합니다. 그래서 배선은 컨테이너 생성 뒤에 옵니다. 6번의 결과가 돌아오면 Pod 의 컨테이너들이 그 네임스페이스를 함께 쓰며 뜹니다.
...
디렉토리에 파일이 여럿이면 런타임은 이름을 사전순으로 정렬해 **첫 번째 유효한 것 하나만** 씁니다. 나머지는 읽고 버리는 것이 아니라 아예 보지 않습니다. 숫자 접두사를 붙이는 관례가 여기서 나옵니다. 옛 파일을 안 지워도 앞자리의 새 CNI 가 이기므로 Cilium 은 `05-cilium.conflist`, Calico 는 `10-calico.conflist` 입니다.
...
이 분리를 명세가 허용할 뿐 강제하지는 않습니다. Cilium 이 실제로 노드에 써 두는 conflist 에는 `ipam` 칸이 아예 없고 `plugins` 에도 `cilium-cni` 하나뿐입니다. 주소 배분을 자기 안에서 처리하기 때문입니다. 그래서 노드 `/opt/cni/bin/` 의 `host-local` 은 그 클러스터에서 한 번도 실행되지 않는, 노드 이미지 기본 표준 플러그인일 뿐입니다.
...
그래서 nftables 모드는 이제 실험이 아니라 프로덕션에서 골라 써도 되는 선택지입니다([용어집](./00-01.%EC%9A%A9%EC%96%B4%EC%A7%91.md)). IPVS 의 스케줄러 6종은 rr·lc·dh·sh·sed·nq 이고 기본이 rr 입니다. 세 기술의 내부 차이는 [02-03](./02-03.iptables%C2%B7IPVS%C2%B7eBPF%20%E2%80%94%20kube-proxy%EB%A5%BC%20%EC%9D%B4%ED%95%B4%ED%95%98%EB%8A%94%20%EC%84%B8%20%EA%B8%B0%EC%88%A0.md)가 정본입니다.
...
이 절의 관심은 규칙이 어떻게 평가되는가가 아니라 **서비스라는 오브젝트가 무엇으로 번역되는가**입니다. 규칙 평가와 확률 계산은 [02-03](./02-03.iptables%C2%B7IPVS%C2%B7eBPF%20%E2%80%94%20kube-proxy%EB%A5%BC%20%EC%9D%B4%ED%95%B4%ED%95%98%EB%8A%94%20%EC%84%B8%20%EA%B8%B0%EC%88%A0.md) §3 이 정본이라 여기서 되풀이하지 않습니다.
...
그래서 대안이 나왔습니다. IPVS 는 커널 해시 테이블을 써서 조회 비용이 백엔드 수에 덜 끌려가고 갱신도 항목 단위로 합니다. nftables 는 같은 문제를 iptables 의 후계 문법으로 풉니다. Cilium 은 eBPF 맵으로 처리하며 kube-proxy 자체를 대체합니다. 세 기술의 내부 차이와 수치는 [02-03](./02-03.iptables%C2%B7IPVS%C2%B7eBPF%20%E2%80%94%20kube-proxy%EB%A5%BC%20%EC%9D%B4%ED%95%B4%ED%95%98%EB%8A%94%20%EC%84%B8%20%EA%B8%B0%EC%88%A0.md)가 정본이라 여기서 되풀이하지 않습니다.
...
>
> kube-proxy는 "고객센터(ClusterIP)로 전화 주세요"라고 알리고 전화를 근무 중인 상담원(ready Pod)에게 배분하는 대표번호 교환원입니다. 교환원의 배분 방식이 절차 규정집(iptables 확률 규칙)이냐 전담 교환기(IPVS)냐가 모드 선택입니다.



## 5. 예측한 경로가 실제 그대로인지 아홉 번 확인하기
> 원서 밖 보강입니다. 강의 트랜스크립트(Ch-06 Network 2 — Linux Network)를 근거로 `02_os/networking` 에서 이관했습니다(2026-09-21).

**전제를 먼저 못 박습니다. 아래 순서는 Calico CNI 와 kube-proxy `iptables` 모드를 전제하고, 출처가 강의 트랜스크립트라 이 저장소의 실습 환경에서 재현한 기록이 아닙니다.** 같은 장의 [04-04 실습편](./04-04.Kubernetes%20%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC%20%EC%8B%A4%EC%8A%B5%20%E2%80%94%20CNI%20%EB%B6%80%EC%9E%AC%EB%B6%80%ED%84%B0%20%EC%A0%95%EC%B1%85%C2%B7DNS%EA%B9%8C%EC%A7%80.md)은 Cilium 클러스터에서 값을 직접 재어 `확인된 것`으로 적습니다. 그쪽과 달리 이 절은 **점검 순서의 뼈대**로 읽습니다.

§1~4 가 CNI 와 kube-proxy 가 무엇을 하는지를 따로 봤다면, 여기서는 그 둘이 만든 결과물을 Pod 에서 출발해 한 줄로 이어 확인합니다. 1~2번은 배선, 3~5번은 규칙, 6번은 상태, 7~9번은 경로를 봅니다. 어디서 끊기는지가 곧 어느 컴포넌트의 문제인지입니다.
...
8번에서 Calico 의 VXLAN 항목은 일반 ARP 와 달리 `PERMANENT` 로 찍힙니다. 왜 그렇게 심는지와 NUD 일곱 상태는 [02-01. 커널이 패킷을 다루는 법](./02-01.%EC%BB%A4%EB%84%90%EC%9D%B4%20%ED%8C%A8%ED%82%B7%EC%9D%84%20%EB%8B%A4%EB%A3%A8%EB%8A%94%20%EB%B2%95%20%E2%80%94%20%EC%86%8C%EC%BC%93%C2%B7Netfilter%C2%B7Conntrack%C2%B7%EB%9D%BC%EC%9A%B0%ED%8C%85.md) §6 이 다룹니다. 핑 로스 장애에서 1순위로 살펴볼 값입니다.

### Calico 노드의 라우팅 테이블 읽기
> 7번 스텝 `ip route get` 이 무엇을 돌려주는지 미리 읽어 두는 자리입니다. 전제는 이 절 머리와 같습니다.
...
#### 스케일인으로 사라진 Pod 주소는 블랙홀이 받습니다

직전에 스케일인으로 사라져 `calixxx` 인터페이스가 없는 IP로 패킷이 들어오면 어떻게 될까요? 그 IP는 `10.244.242.64/26` 같은 자기 노드 Pod 대역에 속합니다. 가장 구체적인 개별 규칙을 못 찾았으니 윗단의 `/26` 블랙홀 규칙으로 매칭되어 즉시 폐기됩니다. 이 안전망이 없다면 패킷이 default 게이트웨이로 빠져 옆 노드로 나갑니다. 옆 노드 라우팅 테이블도 같은 대역이라 다시 원래 노드로 돌려보내는 라우팅 루프가 발생합니다. 클러스터 전체 네트워크 대역폭을 마비시킬 수 있는 시나리오이므로 블랙홀 한 줄이 운영 안정성의 핵심입니다.

#### 수천 줄이어도 느려지지 않는 이유 — FIB 트리
...
### Calico 환경에서 먼저 보는 두 값 — `proxy_arp` 와 `rp_filter`
> 원서 밖 보강입니다. 위 1번과 8번 스텝이 왜 그 값을 보는지가 여기에 있습니다.

`rp_filter` 의 개념 자체는 [02-01](./02-01.%EC%BB%A4%EB%84%90%EC%9D%B4%20%ED%8C%A8%ED%82%B7%EC%9D%84%20%EB%8B%A4%EB%A3%A8%EB%8A%94%20%EB%B2%95%20%E2%80%94%20%EC%86%8C%EC%BC%93%C2%B7Netfilter%C2%B7Conntrack%C2%B7%EB%9D%BC%EC%9A%B0%ED%8C%85.md) §4 심화 1 이 다루고, 여기서는 증상과 진단만 봅니다.


Pod 안에서 `ip route`를 보면 기본 게이트웨이가 `169.254.1.1`이라는 고정 가상 주소로 잡혀 있습니다. Pod가 이 게이트웨이의 MAC을 ARP로 묻는 순간 호스트의 `calixxx` 인터페이스가 자기 MAC으로 대신 응답합니다. 이 동작이 proxy ARP입니다. 인터페이스의 `proxy_arp` 값이 1이어야 하고, 0이면 Pod 트래픽이 호스트 라우팅 테이블로 진입하지 못해 통신이 끊깁니다. Calico 환경 통신 장애에서 가장 먼저 점검할 한 값입니다.
...
- kube-proxy 최신 지형은 [02-03 심화 학습](./02-03.iptables%C2%B7IPVS%C2%B7eBPF%20%E2%80%94%20kube-proxy%EB%A5%BC%20%EC%9D%B4%ED%95%B4%ED%95%98%EB%8A%94%20%EC%84%B8%20%EA%B8%B0%EC%88%A0.md)에 있습니다. nftables 모드가 1.33에서 GA가 되어 이 편의 "4모드"는 이제 5모드입니다.



## 결정 치트시트
> kube-proxy 모드 선택 기준입니다. CNI 선택은 03-02·02-03 치트시트와 겹치므로 그쪽에 위임합니다.
...
### 1. kube-proxy 한 줄 정의
"kube-proxy란 노드마다 돌며 ClusterIP 트래픽을 준비된 Pod IP(EndpointSlice, 구 Endpoints)로 번역(DNAT)하는 데몬입니다. iptables 확률 체인이나 IPVS로 fan-out을 구현합니다."
...
"IP 공간을 크게 쓸 것인가, 캡슐화 비용과 직접 연결성 포기를 감수할 것인가"의 거래입니다. flat은 클러스터 IP를 그대로 써 IP가 많이 듭니다. overlay는 언더레이 위에 가상망을 만들어 언더레이 IP가 노드에만 필요한 대신 호스트가 Pod에 직접 닿지 못합니다.
