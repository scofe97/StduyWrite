...
| Calico | 지원 | etcd 또는 Kubernetes API | BGP 기반 L3 이 기본. 노드 간 L2 인접이 안 되는 환경용 VXLAN·IP-in-IP 캡슐화 모드와 eBPF 데이터플레인도 제공 |
| Weave Net | 지원 | 외부 저장소 불필요 | 메시 오버레이 — **Weaveworks 2024-02 폐업으로 아카이브·비유지보수. 신규 채택 대상이 아닙니다** |
...
직접 만들어 보면 그 장면이 나옵니다. 기본 CNI 를 끄고 클러스터를 세우면 노드가 NotReady 에서 멈춰 기다리고, 그 자리가 CNI 가 채워야 할 빈칸입니다.
...
책은 모드를 넷으로 들고 "전부 어느 정도 iptables 에 의존한다"고 정리하지만 둘 다 지금은 맞지 않습니다. **모드는 다섯**입니다 — nftables 가 1.33 GA 로 합류했습니다. 그리고 **`kernelspace` 는 iptables 를 전혀 쓰지 않습니다.** Windows 의 HNS·VFP 를 쓰므로 Linux 기술인 iptables 와 무관합니다. `nftables` 모드도 iptables 가 아닌 nft 를 씁니다. "전부 iptables 에 의존"이 성립하는 것은 userspace·iptables·ipvs 세 모드까지입니다.

- userspace — 최초의 모드. iptables로 서비스 IP를 자체 웹 서버로 몰아 프록시. 책은 "피하라"는 권고로 적지만 **1.23 에서 deprecated, 1.26 에서 아예 제거**됐습니다. 지원되는 클러스터에서는 선택할 수 없는 모드입니다
- iptables — 기본값이자 최다 사용. 진짜 로드밸런싱이 아니라 연결 fan-out
- ipvs — IPVS로 연결 로드밸런싱. 스케줄러 6종(rr·lc·dh·sh·sed·nq, 기본 rr)
- kernelspace — Windows 전용 모드. HNS·VFP 기반이라 iptables·IPVS(Linux 전용)를 쓰지 않습니다
- nftables — 1.29 alpha → 1.31 beta → **1.33 GA**. iptables 모드의 후계로, 규칙 갱신과 조회가 선형 순회에서 벗어납니다(상세는 [02-02](./02-02.iptables%C2%B7IPVS%C2%B7eBPF%20%E2%80%94%20kube-proxy%EB%A5%BC%20%EC%9D%B4%ED%95%B4%ED%95%98%EB%8A%94%20%EC%84%B8%20%EA%B8%B0%EC%88%A0.md))
