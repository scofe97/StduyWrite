...
review:
  ...
  method: >
    ...
    2026-08-30 800줄 규정을 넘겨 §6~§8(막기·진단·나누기)을 02-06 로 분리하고 세팅 절을 신설.
...
네트워크 네임스페이스 둘을 만들고 veth 쌍으로 브리지에 물리면 컨테이너 네트워킹의 뼈대가 그대로 나옵니다. 거기에 포워딩과 MASQUERADE 를 얹으면 Pod 가 밖으로 나가는 경로가 됩니다. 그 과정에서 세 번 실패했고, 실패 메시지가 각각 다른 계층을 가리켰습니다. 02-04 의 진단 사다리를 직접 만든 환경에 적용해 본 기록이기도 합니다.
...
`via` 가 없습니다. `.238` 은 `default` 에도 걸리고 `192.168.139.0/24` 에도 걸리는데 커널이 더 좁은 쪽을 골랐습니다. 02-02 §3 의 구체성 우선 규칙이 여기서 실물로 확인됩니다.
...
![목적지가 바뀌면 MAC 이 바뀌고 IP 는 그대로다](_assets/02-05.two-tables.svg)
...
![실습으로 지은 배선 — 네임스페이스 둘을 브리지 하나로 잇는다](_assets/02-05.netns-lab-topology.svg)
...
![명령 하나가 목록을 어떻게 바꾸는가](_assets/02-05.build-steps.svg)
...
**바깥쪽 끝 둘만** `master br0` 로 나옵니다. 안쪽 끝은 나오지 않습니다. 브리지에 물리는 것은 한쪽 끝뿐이라는 02-01 §4 의 서술이 그대로입니다. `state forwarding` 은 스위치로 동작 중이라는 뜻입니다.
...
![네임스페이스를 밖으로 내보내려면 세 곳에 하나씩 필요하다](_assets/02-05.three-requirements.svg)
...
이 스위치는 **목적지가 내가 아닌 패킷을 남에게 전달할 것인가**를 정합니다. `ns1` 이 보낸 패킷은 목적지가 `.238` 이라 `ubuntu` 것이 아닙니다. 02-02 §1 의 네 갈래 중 외부에서 외부로 가는 갈래이고 `FORWARD` 훅을 지납니다. 이 값이 `0` 이면 그 훅에 닿기도 전에 버려집니다.
...
![장치마다 바뀌는 필드가 다르고, MASQUERADE 는 한 칸만 더 바꾼다](_assets/02-05.what-changes-where.svg)
...
![커널이 문구를 던지고 내가 고치기를 세 번 반복했다](_assets/02-05.repair-journey.svg)

### 풀려는 문제

위 절차를 처음부터 매끄럽게 밟은 것이 아닙니다. 세 번 막혔고 원인이 모두 달랐습니다. 그런데 **증상 문구가 매번 다음에 볼 곳을 정확히 가리켰습니다.** 02-04 의 진단 사다리를 직접 만든 환경에 적용한 사례라, 이 절이 이 편에서 가장 다시 읽을 값이 있는 부분입니다.
...
버그가 아니라 설계입니다. 02-02 §2 에 이미 적혀 있습니다.
...
![실패 문구가 어느 단에서 멈췄는지 말해 준다](_assets/02-05.diagnostic-ladder.svg)

**같은 "안 된다"인데 문구가 다르면 파야 할 곳이 다릅니다.** 02-04 이 도구 목록이 아니라 순서에 관한 편인 이유가 여기 있습니다.
...
![패킷 하나가 지나는 자리에 이 절의 네 주제가 놓인다](_assets/02-05.section5-path.svg)

### 풀려는 문제

02-02 §2 에서 conntrack 엔트리에 튜플이 두 줄 실린다고 읽었습니다. 보통은 뒤집힌 형태지만 NAT 뒤에서는 다를 수 있다고도 했습니다. 그 "다를 수 있다"를 실물로 확인하는 것이 이 절입니다.

실험은 규칙을 하나씩 얹으며 세 번 관측했습니다.

![규칙을 하나씩 얹을 때마다 conntrack 테이블이 달라진다](_assets/02-05.tracking-states.svg)
...
플래그가 여기서 처음 몰려 나오니 한 줄을 토막 내 두겠습니다. 02-03 에서 세운 네 칸이 그대로 플래그가 됩니다.
...
그리고 이것이 이 한 줄이 **추적을 켜는** 이유이기도 합니다. `-m conntrack` 을 불러오는 순간 커널은 누군가 연결 상태를 볼 작정임을 알게 됩니다. 나머지 문법과 규칙이 읽히는 순서는 [02-03 §1·§2](./02-03.iptables%C2%B7IPVS%C2%B7eBPF%20%E2%80%94%20kube-proxy%EB%A5%BC%20%EC%9D%B4%ED%95%B4%ED%95%98%EB%8A%94%20%EC%84%B8%20%EA%B8%B0%EC%88%A0.md)가 다룹니다.
...
이 세 줄이 **안에서 나가는 것은 되고 밖에서 새로 들어오는 것은 안 되는** 방화벽입니다. 02-02 §2 의 "방화벽의 방향성은 Conntrack 위에 선다"가 이 모양입니다.

같은 세 줄인데 패킷에 따라 도착지가 갈립니다.

![같은 세 줄인데 패킷에 따라 도착지가 다르다](_assets/02-05.firewall-two-traces.svg)
...
![한 줄에 튜플이 둘 — 어디까지가 원본이고 어디부터가 응답인가](_assets/02-05.entry-anatomy.svg)
...
바뀌는 것은 응답 방향입니다. `ubuntu2` 가 `.208` 로 답장할 것이므로, 커널이 기다려야 할 응답의 `dst` 도 그 값이어야 합니다. 02-02 §2 의 "응답 방향 튜플은 원본을 뒤집은 값이 아니라 변환된 주소를 담는다"가 이 한 필드입니다.
...
**conntrack 이 엔트리를 만든 뒤로는 둘째 패킷이 nat 규칙을 거치지 않습니다.** 이미 아는 연결이니 규칙을 다시 고르지 않고 저장된 튜플로 바로 변환합니다. 02-02 §2 의 "NAT 는 아예 Conntrack 위에서 동작한다"가 이 숫자 하나로 증명됩니다.

![둘째 패킷은 nat 규칙을 아예 거치지 않는다](_assets/02-05.first-packet-only.svg)
