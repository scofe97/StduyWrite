...
두 통제의 차이를 한 장으로 접으면 이렇습니다. 위 줄이 각각이 걸리는 층이고, 아래 줄이 그 층에서 나오는 성격 차이입니다. 아래 줄의 stateful·stateless 가 연결 장애 때 무엇을 확인해야 하는지를 가릅니다.
...
인스턴스 타입 하나가 어떻게 Pod 상한이 되는지를 네 칸으로 끊으면 이렇습니다. 세 번째 칸의 29 가 공식이 내놓는 값이고, 마지막 칸의 27 이 시스템 Pod 를 빼고 실제로 배포할 수 있는 수입니다.
