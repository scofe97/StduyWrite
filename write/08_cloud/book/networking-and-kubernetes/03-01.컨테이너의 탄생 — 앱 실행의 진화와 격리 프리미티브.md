...
아래는 이 편 전체를 "컨테이너를 분해했다가 다시 손으로 조립하는" 순서로 이은 지도입니다. 각 단계 오른쪽의 절 번호가 본문에서 그것을 다루는 곳입니다. 중간에 길이 둘로 갈라지는 곳이 컨테이너를 이루는 두 프리미티브이고, 그 둘이 다시 만나는 자리가 맨손 배선 실습입니다. 색이 붙은 곳은 그 마지막 한 군데뿐이고 경고입니다. 이 일을 사람이 직접 하면 위험하다는 사실이 CNI 가 존재하는 이유라 표시했습니다.
