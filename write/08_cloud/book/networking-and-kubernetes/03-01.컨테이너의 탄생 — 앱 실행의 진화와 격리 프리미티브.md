...
아래 도식은 그 뒤에 일어나는 일을 시간 순으로 봅니다. 세로선 하나가 부품 하나의 생애이고, 굵은 막대가 그 부품이 살아 있는 구간입니다. 가운데 runC의 막대만 도중에 끊기고 shim의 막대는 끝까지 이어지는데, 그 차이가 이 절의 요점입니다.
...
아래 도식이 그 분업을 한 장에 담은 것입니다. 왼쪽 끝의 runC가 컨테이너를 만들 때 두 가지를 함께 준비하고, 가운데 점선 상자 안의 둘이 오른쪽 컨테이너 프로세스에 각각 다른 제약을 겁니다. 컨테이너 프로세스가 호스트 커널을 그대로 공유한다는 점이 VM과 갈리는 지점입니다.
...
아래 도식이 그 배선을 네 단계로 묶은 것입니다. 위쪽 한 줄이 순서대로 밟는 길이고, 상자 가운데 줄이 그 단계에서 실제로 치는 명령입니다. 맨 아래 줄은 그 단계에서 함께 기억할 것이며 칸마다 성격이 다릅니다. 차례로 먼저 켜 둬야 할 전제, 놓치기 쉬운 지점, 걸렸을 때 나타나는 증상, 성공했을 때 보이는 신호입니다. 아래로 갈라지는 점선이 마지막 함정입니다. 배선을 다 끝내도 기본 경로를 빼먹으면 그쪽으로 떨어집니다.
