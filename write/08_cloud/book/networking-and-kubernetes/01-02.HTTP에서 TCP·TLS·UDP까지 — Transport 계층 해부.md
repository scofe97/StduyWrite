...
아래는 이 편 전체를 신뢰성의 비용이라는 하나의 축으로 이은 지도입니다. 먼저 비용을 세고 그다음 그 값을 낼지 고르는 순서입니다. 각 단계 오른쪽의 절 번호가 본문에서 그것을 다루는 곳입니다. 4단계에서 비용이 드러나면 거기서 길이 갈라집니다. 더 내는 쪽이 TLS이고 안 내는 쪽이 UDP입니다. 색이 붙은 곳은 그 4단계 한 군데뿐이고 경고입니다. 12패킷 중 데이터가 2개뿐이라는 사실이 이 편의 결론이라 표시했습니다.
