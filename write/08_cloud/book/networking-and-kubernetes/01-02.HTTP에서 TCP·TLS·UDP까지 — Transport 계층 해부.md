...
표의 11개가 모두 이 그림 위에 있습니다. 서버만 거치는 것이 `LISTEN`·`SYN-RECEIVED`·`CLOSE-WAIT`·`LAST-ACK` 넷입니다. 클라이언트만 거치는 것은 `SYN-SENT`·`FIN-WAIT-1`·`FIN-WAIT-2`·`TIME-WAIT` 넷입니다. `CLOSED`와 `ESTABLISHED`는 양쪽에 다 나타납니다. 마지막 `CLOSING`은 드문 경우라 아래 주석으로 뺐습니다.
...
지금까지 본 그림은 읽기 좋게 접은 것입니다. 접지 않고 펴면 메시지는 열 개가 됩니다. 그 열 개가 노출 상태에 따라 세 구간으로 갈립니다. 화살표마다 이름 옆에 그 메시지가 어떤 상태로 망을 건너는지 붙였습니다. 실선은 내용을 실어 나르는 메시지입니다. 점선은 아무것도 나르지 않는 선언입니다. 굵은 초록 실선 하나가 이 그림에서 유일하게 비밀을 나르는 화살표입니다. 세로선에 걸친 막대는 그 쪽이 계산하고 있는 구간입니다.
