...
위 그림이 체인과 테이블의 순서를 보여 준다면, 정작 규칙 한 줄이 어떻게 생겼는지는 아직 안 봤습니다. kind 클러스터에서 뽑은 규칙 하나를 토큰 단위로 끊어 놓고, 각 토큰이 패킷에 무엇을 하는지 옆에 나란히 적으면 이렇습니다.

![규칙 한 줄을 토큰까지 쪼개면 — 조건 넷은 아무것도 안 바꾸고 타깃 하나만 바꾼다](_assets/02-02.iptables-chain-traverse.svg)

왼쪽은 `iptables -t nat -S KUBE-SEP-2MJG2J3URJK2NCRL` 이 그대로 뱉은 문자열을 조각낸 것입니다. `-p tcp`와 `-m tcp`는 패킷을 읽어 판정만 하고, `-m comment`는 커널이 아예 무시하는 사람용 표식입니다. 셋 다 헤더를 건드리지 않습니다.
