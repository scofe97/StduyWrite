...
체인은 규칙의 목록입니다. 최상위 체인 5개는 PREROUTING·INPUT·FORWARD·OUTPUT·POSTROUTING이며, [전편][prev]의 Netfilter 훅 5개에 그대로 대응됩니다. 어느 체인에 규칙을 넣느냐가 곧 그 규칙이 언제 평가되는지를 정합니다.

[prev]: ./02-01.%EC%BB%A4%EB%84%90%EC%9D%B4%20%ED%8C%A8%ED%82%B7%EC%9D%84%20%EB%8B%A4%EB%A3%A8%EB%8A%94%20%EB%B2%95%20%E2%80%94%20%EC%86%8C%EC%BC%93%C2%B7Netfilter%C2%B7Conntrack%C2%B7%EB%9D%BC%EC%9A%B0%ED%8C%85.md
...
한 가지 전환 경고도 있습니다. 대부분의 배포판이 iptables를 더 성능 좋은 nftables로 교체하는 중이고 일부는 이미 nftables 기반 iptables를 탑재했는데, 저자들은 Kubernetes와 iptables/nftables 전환 사이에 알려진 문제가 많아 nftables 기반 iptables 사용을 당분간 피하라고 강하게 권합니다(2021년 기준 — 이후 변화는 심화 학습 참조).
...
- 프로토콜 `-p`
- 입력·출력 인터페이스 `-i`, `-o`
- Conntrack 상태 `-m state --state NEW,ESTABLISHED`
...
> 리스트 순회의 한계가 수치로 드러나는 절입니다. 규칙 16만 개에 5시간이 걸리는 지점에서 해시 기반 조회와 6가지 밸런싱 모드가 대안이 됩니다.
...
패킷 포워딩 방식은 NAT(주소 재작성), DR, IP 터널링 세 가지를 지원합니다. 세션 어피니티도 지원하는데 Kubernetes에서는 `Service.spec.sessionAffinity`로 노출됩니다 — 어피니티 시간 창 안의 반복 연결은 같은 호스트로 갑니다.
...
```pseudocode
# ipvsadm -A -t 1.1.1.1:80 -s lc          # least-connection 가상 서비스
...
eBPF는 샌드박스된 특수 프로그램을 커널 안에서 실행하는 시스템입니다. Netfilter·iptables처럼 커널과 유저스페이스를 오가지 않는다는 점이 구조적 차이입니다. 전신인 BPF(Berkeley Packet Filter)는 패킷 필터링용이었고 tcpdump가 대표 사용처입니다 — tcpdump에 필터를 지정하면 BPF 프로그램으로 컴파일되어 커널이 그 필터로 패킷을 거르고, 결과는 맵(BPF 프로그램이 데이터를 교환하는 키-값 자료구조)으로 tcpdump에 전달됩니다.
...
세 세대를 한 장으로 정리하면 다음과 같습니다.

![kube-proxy 서비스 구현의 3세대 — iptables·IPVS·eBPF](_assets/02-02.kube-proxy-three-generations.svg)
...
- 실무 함정 하나. `iptables`와 `ip6tables`는 완전히 분리된 도구라, IPv4 규칙은 IPv6 트래픽에 적용되지 않습니다. 듀얼 스택 환경에서 방화벽 규칙을 IPv4에만 넣고 IPv6를 빠뜨리면, IPv6 경로로 우회하는 트래픽이 무방비로 열립니다. 규칙을 넣을 때는 양쪽(`iptables`·`ip6tables`)을 함께 점검합니다.
...
| 서비스 수천 개 이상, 규칙 갱신이 느려짐 | IPVS 모드 | 해시 기반 조회 + 6가지 밸런싱 모드, kube-proxy 내장 |
| L7 정책·관측·최고 성능까지 필요 | eBPF (Cilium) | kube-proxy 대체, 커널 내 직접 라우팅, L7 로드밸런싱 |
| 장수 연결(gRPC)의 부하 쏠림 | 어느 모드로도 해결 안 됨 | 클라이언트 측 LB·주기적 재연결·서비스 메시로 접근 |
