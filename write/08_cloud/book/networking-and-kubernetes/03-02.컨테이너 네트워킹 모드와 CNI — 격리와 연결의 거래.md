...
review:
  ...
  # 사실오류 2 (CNM 3부품 오역): (1) sandbox 를 "호스트의 모든 컨테이너에 대한 netns 관리"로 적었으나 실제는 **컨테이너 하나**의 네트워크 스택(netns·인터페이스·라우팅·DNS) 단위.
  # (2) endpoint 를 "네트워크 위의 호스트"로 적었으나 실제는 sandbox 를 network 에 잇는 **가상 인터페이스**(veth 한쪽).
  # 내부 불일치 1: 심화 학습이 "Docker Desktop 4.34 부터 host networking 지원"이라 단정했는데, 실제는 Linux 컨테이너 한정·설정에서 켜야 하는 베타.
  # frontmatter 리뷰 노트는 이 제약을 알고 있었으나 본문에 반영되지 않았다. "낡았다"에서 "조건부로 풀렸다"로 교정.
  # 2026-08-28 보강 러너 — 절 서사 골격 정합. §1(모드 선택)·§2(Docker 실측)·§3(CNM)에 "### 풀려는 문제" 도입 소절 신설.
  # §4(호스트 넘기)·§5(CNM 의 문제)는 이미 문제로 여는 절이라 그대로 뒀다.
  # 심화 학습의 Docker Desktop host networking 문장 150자 초과분 1건 분리. 사실 변경 없음.
updated: 2026-08-28
---
...
![일곱 모드는 격리를 얼마나 포기하는가의 눈금 위에 있다](_assets/03-02.isolation-spectrum.svg)

가운데 다섯은 격리 정도가 아니라 연결 방식이 갈리는 구간이라, 눈금 위 순서를 매기지 않고 한데 두었습니다.

### 풀려는 문제

`docker run` 에 붙일 네트워크 모드가 일곱 개입니다. 이름만 봐서는 무엇을 고를지 정할 근거가 없어 대개 기본값을 그냥 씁니다. 그러다 성능이 아쉬워 Host 로 바꾸면 앞 편에서 봤던 포트 충돌이 그대로 돌아오고, Macvlan 을 골라 두면 클라우드에서 아예 안 뜹니다.

일곱을 따로 외우는 대신 하나의 축 위에 세우면 선택이 쉬워집니다. 모드가 다른 것은 결국 격리를 얼마나 내주고 무엇을 얻느냐의 차이이기 때문입니다.
...
전편에서는 컨테이너 네트워크를 만드는 데 명령을 열 줄 넘게 손으로 쳤습니다. Docker 는 그것을 `docker run` 한 번으로 끝냅니다. 그러면 그 배선은 어디로 갔을까요 — 자동으로 됐다고 해서 없어진 것은 아닙니다.

확인할 수 있어야 고칠 수 있습니다. 컨테이너 네트워크가 안 될 때 브리지를 볼지 veth 를 볼지 경로를 볼지 가르려면, 정상일 때 그 셋이 호스트 어디에 어떤 모양으로 남는지를 먼저 봐 둬야 합니다.
...
앞 두 절이 Docker 가 만들어 놓은 결과였다면, 이제 그것을 만드는 쪽의 설계를 볼 차례입니다. 여기에 답이 필요한 질문이 하나 딸려 옵니다. Docker 가 컨테이너 네트워킹의 사실상 표준이던 시기에, Kubernetes 는 왜 이 모델을 쓰지 않고 다른 길을 갔을까요.

이유는 세 부품 자체가 아니라 그 모델이 *제공하지 않은* 것에 있습니다. 그래서 이 절은 부품을 나열하는 데서 끝나지 않고 빠진 자리를 함께 짚습니다. §5 의 갈림길이 거기서 시작됩니다.

Libnetwork는 Docker의 컨테이너 네트워킹 구현이고, 그 설계 철학이 CNM(Container Networking Model)입니다. 세 부품으로 움직입니다.

- **sandbox**: 컨테이너 **하나**의 네트워크 스택 격리 단위 — 그 컨테이너의 netns·인터페이스·라우팅 테이블·DNS 설정을 담습니다. 호스트 전체를 관장하는 것이 아니라 컨테이너마다 하나씩 생깁니다
- **endpoint**: sandbox 를 network 에 잇는 **가상 인터페이스**(veth 쌍의 한쪽). "네트워크 위의 호스트"가 아니라 연결점입니다
...
위 도식이 그 구조입니다. 점선 상자 안이 libnetwork가 제공하는 세 부품이고, 왼쪽의 네트워크 컨트롤러가 Docker engine의 API로 이들을 지휘합니다. 오른쪽 바깥으로 나가는 점선 화살표가 §5에서 갈라서는 지점입니다. global 드라이버가 노드 간 조율을 하려면 CNM이 제공하지 않는 저장소를 밖에서 가져와야 합니다.
...
그 왕복을 네 단계로 끊은 것이 위 도식입니다. 두 번째와 네 번째 칸이 양 끝 VTEP가 하는 일입니다. 그 사이에 낀 세 번째 칸이 물리망이고, 물리망은 바깥 헤더만 보고 나를 뿐 안에 무엇이 들었는지 모릅니다.
...
- "Docker Desktop은 host 모드 미지원"은 책 시점(2021)의 사실입니다. Docker Desktop 4.34(2024)부터 Mac·Windows 에서도 host networking 을 쓸 수 있습니다. 다만 **전면 지원이 아닙니다.** Linux 컨테이너 한정이고, 설정에서 따로 켜야 하며(베타·opt-in) 문서에 명시된 제약이 남아 있습니다. [Docker 문서 — host network driver](https://docs.docker.com/engine/network/drivers/host/) 참조. 본문 표의 경고는 "낡았다"기보다 **"조건부로 풀렸다"** 로 읽는 편이 정확합니다.
