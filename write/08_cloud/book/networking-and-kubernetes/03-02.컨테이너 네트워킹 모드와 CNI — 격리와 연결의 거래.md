...
아래는 이 편 전체를 "한 호스트 안의 선택지에서 출발해 호스트를 넘고, 마지막에 표준이 갈리는" 순서로 이은 지도입니다. 각 단계 오른쪽의 절 번호가 본문에서 그것을 다루는 곳입니다. 색이 붙은 곳은 마지막 한 군데뿐이고 경고입니다. Docker 가 만든 CNM 이 아니라 CNI 가 Kubernetes 의 표준이 됐다는 사실이 이 편의 결론이자 다음 장 전체의 전제라 표시했습니다.
