...
아래는 이 편이 다루는 다섯 유형을 층으로 쌓아 본 지도입니다. 아래층이 위층의 토대가 되므로 LoadBalancer 는 NodePort 를, NodePort 는 ClusterIP 를 안에 품고 있습니다. 왼쪽 상자 옆에 적힌 호출자가 그 층을 부르는 주체이고, 맨 아래 Endpoints 와 Pod 가 전편에서 본 재료입니다. 오른쪽 "스택 밖의 두 갈래"가 로드밸런싱을 하지 않는 headless 와 ExternalName 입니다. 본문은 §2 ClusterIP 부터 아래에서 위로 올라갑니다.
