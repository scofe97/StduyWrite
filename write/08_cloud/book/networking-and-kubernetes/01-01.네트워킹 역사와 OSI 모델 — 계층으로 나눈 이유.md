...
review:
  ...
  method: >
    ...
    2026-08-15 보강 러너 1·2축 — H1 뒤 불필요한 `---` 제거(형제 편과 정합).
    책 PDF 재대조로 TCP/IP 이기종 통신의 근거(개방 프로토콜이 양 끝 구현에 조건을 걸지 않음) 보충.
    연표 RFC 760 과 본문 RFC 791 의 관계를 1차 자료(RFC 791 표지 `Replaces: RFC 760`)로 확인해
    §1 에 설명 문단 + `[^rfc760]` 각주 신설 — 서로 다른 문서이며 노트 내부 모순이 아님.
    §5 에 Go 서버 기동 실습 블록 추가. 사실 변경 없음.
    2026-08-15 §5 적대적 검증(별개 에이전트, 기본값 "문제 있음") — 신규 11항 전부 일치, 기존 사실 변경 0건.
    검증 중 원서 연표 오류 1건 확인(RFC 760 발행은 1980-01 인데 원서는 1981 로 표기). 원서 표기는
    표에 그대로 두고 본문에 1차 자료 기준을 병기(Class A 2^24 각주와 같은 방식).
    2026-08-15 chapter-overview 를 Archify dataflow 로 전환 — 4단계 arc(실패→개념 틀→표준 경쟁→남은 것)로
    재설계하고 절 번호는 stage 머리글에 담음. 손그림 6상자를 1:1로 옮기지 않은 이유는 dataflow 가
    stage 5개 상한이고, 대각 분기 edge 는 라벨 필수인데 라벨을 놓을 자리가 없어서다(설계 시스템 §Route rules).
    형제 3편이 같은 형식·수치를 공유. showcase validate + 정적 QA 통과, receipt 동봉.
    2026-08-15 osi-tcpip-mapping 손그림 SVG 를 Archify architecture 로 전환 — 계층별 PDU·책임을 함께 싣고
    상위 3계층이 Application 하나로 접히는 관계를 직교 경로로 표현. showcase validate + 정적 QA 통과.
    2026-08-15 §2 에 encapsulation-roundtrip(Archify architecture) 신설 — 송신·수신 두 호스트를 boundary 로
    나눠 내려가며 붙이고 올라가며 벗기는 대칭을 한 장에 담음. §4 mapping 도식과 주제가 갈린다(저쪽은 7↔4 대응,
    이쪽은 왕복). 게이트 31개 전부 통과, PNG 눈 검사로 겹침·잘림·화살촉 확인. 사실 변경 없음.
    2026-08-15 learning-session Phase 4 에서 드러난 설명 빈틈 3곳 보강(기존 사실 변경 0건).
    §3 에 NCP 몰락과 OSI 패배를 가르는 문단 신설 — 학습자가 둘을 한 사건으로 묶어 "OSI 는 안 쪼갠 쪽"
    으로 오해했다. CLNP 예시는 책 밖 보강이라 `[^osi-clnp]` 각주로 출처 분리(IEEE Spectrum 원문 대조,
    TP0~4·X.400 등 그 글이 명시하지 않는 이름은 넣지 않음). §4 에 TCP/IP 계층이 번호가 아닌 이름으로
    불린다는 명확화 한 문단, 그리고 라우터가 Internet 계층까지만 구현하는 이유(end-to-end)를 기존
    "신뢰성은 양 끝" 문장과 잇는 한 문단 추가.
    2026-08-28 결정 치트시트 정합 — 표에서 `ip link` 만 현대 명령이고 나머지가 net-tools 였다.
    `arp -a`·`netstat -r`·`netstat -ap TCP` 에 `ip neigh`·`ip route`·`ss -tanp` 를 이 편의 기존 `ifconfig` / `ip addr` 관용대로 병기했다.
    기존 명령은 지우지 않았다. 대응표 정본은 02-03 심화 학습.
source:
...
아래는 이 장 전체를 "단일 프로토콜의 실패 → 그것을 설명하는 언어 → 표준 경쟁 → 남은 것"의 네 박자로 이은 지도입니다. 각 칸 위의 머리글에 그 대목을 다루는 절 번호가 붙어 있습니다. 붉은색이 칠해진 3단계가 이 장에서 가장 자주 오해되는 자리입니다. OSI는 표준 경쟁에서 졌는데도 용어만 살아남았다는 사실이 그것입니다. 마지막 칸이 초록인 이유는 그 둘(4계층 모델과 Go 서버)이 이 장을 넘어 다음 편들로 이어지기 때문입니다.
...
![하나가 다 하던 프로토콜이 둘로 갈라진 자리](_assets/01-01.ncp-split.svg)

왼쪽부터 오른쪽으로, 한 몸이던 책임이 어느 시점에 어떻게 갈라졌는지의 순서입니다. 갈라진 뒤에 무엇이 좋아졌는지가 이 절의 요점입니다.

1969년 ARPANET은 UCLA·스탠퍼드 연구소·UC 산타바바라·유타대 4개 노드로 출발했습니다. 노드 간 통신은 1970년 NCP(Network Control Protocol)로 열렸고 Telnet·FTP 같은 최초의 컴퓨터 간 프로토콜도 NCP 위에서 나왔습니다.

저자들은 NCP의 몰락 원인을 성공 그 자체에서 찾습니다. ARPANET이 커지면서 연결되는 네트워크의 종류가 다양해졌는데, NCP는 그 수요와 다양성을 감당하지 못했습니다. 1974년 Vint Cerf·Yogen Dalal·Carl Sunshine이 RFC 675로 초안을 쓴 TCP는 이종 네트워크 사이에서도 패킷을 교환할 수 있었습니다.
...
연표의 RFC 760 과 본문의 RFC 791 은 서로 다른 문서입니다. 

- IP 를 규정한 첫 문서가 RFC 760(*DoD standard Internet Protocol*)이고 RFC 791 이 그것을 대체해 현재의 IPv4 명세가 됐습니다. 
- RFC 791 표지에 `Replaces: RFC 760` 으로 적혀 있습니다.[^rfc760]

다만 연도는 원서 표기와 1차 자료가 다릅니다. 위 표는 원서를 따라 1981 로 두었습니다. RFC 760 의 실제 발행은 1980년 1월이고, 1981년 9월은 RFC 791 의 발행 시점입니다.

> 💬 **비유**: NCP에서 TCP/IP로의 전환은 만능 직원 한 명이 하던 일을 부서로 나눈 조직 개편과 비슷합니다. 
>
> 회사(네트워크)가 작을 때는 한 명이 주문 접수부터 배송까지 다 해도 되지만, 지점이 늘고 거래처마다 방식이 다르면 그 사람이 병목이 됩니다. TCP/IP는 "배송 주소 관리 부서(IP)"와 "배송 확인 부서(TCP)"를 분리해 각자 잘하는 일만 하게 만든 셈입니다.
...
![송신 측은 감싸며 내려가고 수신 측은 벗기며 올라간다](_assets/01-01.encapsulation-roundtrip.svg)

위 그림은 그 감싸기가 양쪽 호스트에서 어떻게 짝을 이루는지 한 장으로 옮긴 것입니다. 왼쪽은 계층을 내려가며 헤더를 하나씩 붙이고, 오른쪽은 같은 순서를 거꾸로 밟으며 그 헤더를 벗깁니다. 가운데 물리 매체는 자기가 나르는 비트가 어느 계층의 것인지 알지 못합니다.

프로토콜 하나가 주소도 신뢰성도 표현 형식도 맡으면, 한 군데만 바꾸려 해도 전체를 다시 만들 수밖에 없습니다. §1 의 NCP 는 그 문제를 견디지 못했습니다. 책임을 층으로 갈라 두면 바뀌는 층만 갈아 끼울 수 있습니다. OSI 는 그 경계를 일곱 개로 그어 이름을 붙인 틀입니다.
...
![무너진 이유가 서로 다른 두 사건](_assets/01-01.two-defeats.svg)

두 행이 서로 다른 사건입니다. 진 이유도 남은 것도 다르다는 점을 먼저 두고 아래를 읽습니다.
...
여기서 갈라 둘 것이 있습니다. §1의 NCP가 무너진 것과 OSI가 진 것은 **다른 사건**입니다. NCP는 하나의 프로토콜이 모든 일을 떠맡은 설계였고 그 방식 자체가 한계였습니다. 반면 OSI는 TCP/IP와 마찬가지로 **책임을 쪼갠 쪽**이었습니다.

두 진영의 차이는 쪼갰느냐가 아니라 몇 조각으로 어떻게 쪼갰느냐, 그리고 그 조각을 채울 프로토콜을 누가 먼저 돌아가게 만들었느냐였습니다. 그래서 OSI에서 사라진 것은 계층 분리라는 발상이 아니라 그 발상을 구현한 프로토콜들입니다. 네트워크 계층을 맡기로 했던 ISO의 ConnectionLess Network Protocol이 그런 예입니다.[^osi-clnp]
...
TCP/IP는 운영체제와 아키텍처 차이에 독립적인 개방 프로토콜로 이기종 네트워크를 만듭니다. 호스트가 Windows든 Linux든, 웹 서버가 Apache든 Nginx든 통신할 수 있는 이유는 OSI 모델과 유사한 책임 분리 덕분입니다. 좀 더 정확히 말하면, 프로토콜이 공개돼 있고 그 명세가 양 끝단의 구현에 아무 조건도 걸지 않기 때문입니다. 규약은 "무엇을 주고받을지"만 정하고 "그것을 어떤 OS의 어떤 소프트웨어가 만들었는지"는 묻지 않습니다. 그래서 서로를 모르는 구현끼리도 같은 규약만 지키면 통신이 성립합니다.

TCP/IP 계층에는 번호가 없습니다. Application·Transport·Internet·Link라는 이름으로만 부릅니다. 실무에서 말하는 L3·L4·L7이 OSI 번호인 이유가 여기 있습니다. 구현은 이쪽 모델을 따르면서 부르는 말은 저쪽에서 빌려 오는 셈입니다.
...
Internet 항목의 마지막 문장은 장비의 생김새로도 드러납니다. 경로 중간의 라우터는 목적지 주소를 보고 다음 홉을 정하는 데까지만 관여하므로 Internet 계층까지 구현하면 충분합니다. 다음 셋은 전부 양 끝 호스트의 Transport 계층 몫입니다.

- 유실된 조각을 다시 보내는 일
- 도착한 조각을 순서대로 맞추는 일
- 포트로 어느 프로세스에 넘길지 고르는 일

중간 장비가 재전송까지 맡으려면 지나간 데이터를 전부 보관해야 하는데, 애초에 자기에게 도달하지 못한 조각은 세는 것조차 불가능합니다. 결손을 알 수 있는 자리는 몇 개를 보냈는지 아는 쪽과 몇 개를 받아야 하는지 아는 쪽뿐입니다.
...
다음 편부터 이 서버가 계속 필요하므로 지금 한 번 띄워 두면 좋습니다. 위 코드를 `main.go` 로 저장하고 아래를 실행합니다.

```bash
# 서버 기동 — 별도 의존성 없이 표준 라이브러리만 씁니다
go run main.go &

# 응답 확인 — "Hello" 가 나오면 준비 완료입니다
curl localhost:8080
...
`curl` 이 `Hello` 를 돌려주면 이 책의 나머지 실습이 전부 이 서버 위에서 돌아갑니다. 포트 `8080` 이 이미 쓰이고 있으면 코드의 `0.0.0.0:8080` 을 다른 번호로 바꾸고, 이후 실습의 포트도 같이 바꿔 읽습니다.
...
- OSI 대 TCP/IP 표준 전쟁의 전말은 IEEE Spectrum의 [OSI: The Internet That Wasn't](https://spectrum.ieee.org/osi-the-internet-that-wasnt)가 자세합니다. 정부·ISO 주도의 완벽주의 설계가 이미 돌아가는 코드(TCP/IP)에 진 사례이자 IETF 문화 "rough consensus and running code"의 배경입니다.
...
(1)~(5)는 차례로 Application·Transport·Network·Data Link·Physical 계층의 단위입니다.
...
[^rfc760]: RFC 791 표지의 `Replaces: RFC 760` 표기와 두 문서의 발행일을 확인할 수 있는 1차 자료입니다.
<https://www.rfc-editor.org/rfc/rfc791.txt>

[^rfc2026]: IETF — The Internet Standards Process, Revision 3. 표준 트랙 성숙도 단계는 §4.1 Standards Track Maturity Levels 아래에 있습니다. Proposed Standard 는 §4.1.1, Internet Standard 는 §4.1.3 입니다.
<https://datatracker.ietf.org/doc/html/rfc2026>

[^osi-clnp]: IEEE Spectrum — *OSI: The Internet That Wasn't*. OSI 진영이 네트워크 계층용으로 만든 ISO ConnectionLess Network Protocol 을 다룹니다. 연결지향 대 비연결 노선 대립이 표준화를 어떻게 지연시켰는지도 함께 짚습니다. 책 밖 보강입니다.
