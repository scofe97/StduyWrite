...
여기서 갈라 둘 것이 있습니다. §1의 NCP가 무너진 것과 OSI가 진 것은 **다른 사건**입니다. NCP는 하나의 프로토콜이 모든 일을 떠맡은 설계였고 그 방식 자체가 한계였습니다. 반면 OSI는 TCP/IP와 마찬가지로 **책임을 쪼갠 쪽**이었습니다. 두 진영의 차이는 쪼갰느냐가 아니라 몇 조각으로 어떻게 쪼갰느냐, 그리고 그 조각을 채울 프로토콜을 누가 먼저 돌아가게 만들었느냐였습니다. 그래서 OSI에서 사라진 것은 계층 분리라는 발상이 아니라 그 발상을 구현한 프로토콜들입니다. 네트워크 계층을 맡기로 했던 ISO의 ConnectionLess Network Protocol이 그런 예입니다.[^osi-clnp]
...
Internet 항목의 마지막 문장은 장비의 생김새로도 드러납니다. 경로 중간의 라우터는 목적지 주소를 보고 다음 홉을 정하는 데까지만 관여하므로 Internet 계층까지 구현하면 충분합니다. 유실된 조각을 다시 보내고, 도착한 조각을 순서대로 맞추고, 포트로 어느 프로세스에 넘길지 고르는 일은 전부 양 끝 호스트의 Transport 계층 몫입니다. 중간 장비가 재전송까지 맡으려면 지나간 데이터를 전부 보관해야 하는데, 애초에 자기에게 도달하지 못한 조각은 세는 것조차 불가능합니다. 결손을 알 수 있는 자리는 몇 개를 보냈는지 아는 쪽과 몇 개를 받아야 하는지 아는 쪽뿐입니다.
...
[^osi-clnp]: IEEE Spectrum — *OSI: The Internet That Wasn't*. OSI 진영이 네트워크 계층용으로 만든 ISO ConnectionLess Network Protocol 과, 연결지향 대 비연결 노선 대립이 표준화를 어떻게 지연시켰는지를 다룹니다. 책 밖 보강입니다.
