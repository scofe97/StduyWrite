...
ddx.band(d, 104, 516, "같은 장비 · 배선 아닌 태그로 분리")
...
for x, name, tag, note, c in [(D10_X, "VLAN 10", "태그 10", "브로드캐스트가 이 안에서만", INFO),
                              (D20_X, "VLAN 20", "태그 20", "옆 태그 방송 차단", WARN)]:
    d.o.append(f'<rect x="{x}" y="{DOM_Y}" width="{DOM_W}" height="{DOM_H}" rx="8" '
               f'fill="{c}08" stroke="{c}" stroke-width="1.2"/>')
    # 링 라벨 마스크는 테두리와 겹친다 — 도메인 안쪽 위(호스트 줄 위 빈 띠)에 쓴다
    d.t(x + 16, DOM_Y + 24, f"{name} · {tag}", 12, c, KR, "start", 600)
    cx = x + DOM_W // 2
...
ddx.focal_tag(d, MIDX, DOM_Y + DOM_H + 34, "다른 태그 사이 방송 차단", 264)

# 하단 해설(혼잡과 노출이 줄어든다)은 도식 뒤 본문 문단이 말한다 — 뺐다
d.legend(536, [("VLAN 10", INFO), ("VLAN 20", WARN), ("갈라지는 자리", ACC)])
