...
W, H = 938, 530
# 2026-09-15: 라벨 열을 216 으로 줄여 홈통을 48 로 넓혔다 — 병합 칩이 양옆 상자를 덮지 않게 한다
LX, LW = 12, 216            # 확인해야 할 것
...
for x, c, head, sub in ((OX, OK, "열 때 — 3-way", "세 패킷"),
                        (CX_, WARN, "닫을 때 — 4-way", "네 패킷 그대로")):
...
d.t(OX + CW / 2, MCY + 12, "2·3번을 한 패킷에", 11, ACC)
open_cell(3)

# 왼쪽 대괄호가 어느 두 행이 합쳐졌는지를 물어 준다 — 홈통(228~276) 한가운데에 세운다
R2, R3 = ROW_Y + STRIDE + ROW_H / 2, ROW_Y + STRIDE * 2 + ROW_H / 2
BKX = LX + LW + 24
d.path(f"M {OX-6} {R2} L {BKX} {R2} L {BKX} {R3} L {OX-6} {R3}", ACC, 1.4)
d.chip(BKX, MCY, "병합", ACC, 11)
