...
d.t(DENY[0], DENY[1] + 8, "매칭 없음", 12, MUTED, KR)
d.t(DENY[0], DENY[1] + 28, "선택된 순간의 기본값", 12, BAD, KR)
# 드롭 x 가 박스 가로 범위 안에 있으므로 수직으로 내린다 — 옆으로 꺾으면 화살표가 거꾸로 읽힌다
d.path(f"M {CX[2]} {300+58+6} L {CX[2]} {DENY[1]-46-10}", BAD, 1.4, m="bad", dash="6 5")
d.t(CX[2] + 14, 300 + 58 + 30, "매칭 없음", 12, BAD, KR, "start")

d.legend(568, [("첫 관문", ACC), ("예외로 열림", OK), ("선택된 순간의 기본값", BAD)])
