...
W, H = 1120, 740
LANE_W, SEQ_RIGHT = 176, 892      # 오른쪽 228px 은 수신 버퍼 게이지가 쓴다
LX, RX = 24 + LANE_W / 2, SEQ_RIGHT - LANE_W / 2
MID = (LX + RX) / 2
Y0, STRIDE = 200, 54
GX, GW, GMAX = 922, 104, 4000     # 게이지 만점은 첫 광고값 4000
...
for cx, name, sub in ((LX, "보내는 쪽", "sender"), (RX, "받는 쪽", "receiver")):
    ...
    d.t(cx, 141, sub, 9, MUTED, MONO)
for cx in (LX, RX):
    d.line(cx, 154, cx, 670, RULE, 1.0, "3 6")

d.t(GX + GW / 2, 182, "수신 버퍼 여유", 9, SOFT, MONO)
...
for slot in range(9):
    ...
    if slot == STOP_SLOT:
        ...
        d.t(LX + 120, y + 5, "망은 한가한데 데이터가 흐르지 않는다", 9, MUTED, KR, "start")
        continue
    ...
    d.t(GX + GW + 8, gy + 14, str(win), 10, c, MONO, "start", 600)
