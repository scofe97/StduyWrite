...
ddx.band(d, 104, 536, "양 끝은 Application 까지 · 라우터는 L3 에서 멈춤")
...
# ── 출발 호스트 — 내려가며 감싼다 ──────────────────────────
for cy, (t, s, g) in zip(CY, [("Application", "HTTP GET 생성", "DATA"),
...
# ── 목적 호스트 — 올라가며 벗긴다 ──────────────────────────
for cy, (t, s, g) in zip(CY, [("Application", "cURL 수신", "IP 헤더 이미 제거"),
                              ("Transport", "포트로 소켓 판별", "SEGMENT 해제"),
                              ("Network", "내 IP 인지 확인", "PACKET 해제"),
                              ("Link", "MAC 제거", "FRAME 해제")]):
...
d.t(M, CY[0] + 24, "열리지 않는 칸", 13, ACC, KR, "middle", 600)
d.t(M, CY[1] - 24, "Application · Transport 칸 없음", 12, ACC, KR)
d.t(M, CY[1] + 0, "포트 · HTTP 미확인", 12, MUTED, KR)

cell(M, CY[2], "Network", "IP 로 다음 홉", "L3 · 여기가 천장", INFO)
cell(M - 70, CY[3], "Link 수신", "MAC 제거", "L2", w=132)
cell(M + 70, CY[3], "Link 송신", "새 MAC 기록", "L2", w=132)
...
# 하단 해설(라우터는 IP 헤더까지 · MAC 은 매 홉 새로)은 도식 뒤 본문 문단이 말한다 — 뺐다
d.legend(556, [("L3 가 천장", INFO), ("도착", OK), ("열리지 않는 칸", ACC)])
