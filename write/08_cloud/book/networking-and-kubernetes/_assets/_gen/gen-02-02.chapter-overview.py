# 02-02.chapter-overview — 세로 체인 + 이 편의 경계
# 2026-09-21 옛 02-01 을 둘로 나누며 새로 만들었다. 02-01 지도와 같은 좌표에 이 편(훅·연결 추적·
#            라우팅) 두 칸만 점선으로 감싼다. 위 두 칸은 02-01 의 번호를 달고 흐리게 둔다.
# 2026-09-21 2차 — 02-01 §3 이 네임스페이스(§3)·veth·브리지(§4)로 갈려 둘째 칸 이름과 번호를 맞췄다.
...
d = D(W, H, "02-02 · CHAPTER MAP",
      "커널이 패킷을 판정하는 층 개념 지도 — 이 편은 점선 안을 다룬다",
      "위는 유저 공간, 아래는 물리 네트워크. 커널이 맡는 네 갈래 중 아래 두 칸이 이 편이고 위 두 칸은 02-01 이다. 시간순이 아니라 주제 배치이며, 시간순은 3절 끝의 관통 도식이 맡는다.",
      lead="주제 배치도 · 패킷이 지나는 시간순이 아니다")

BX, BW, BH = 180, 640, 64
RING = (156, 456, 688, 244)                                    # 링 라벨 마스크가 첫 행에 닿지 않게 위로
APP_CY, NIC_CY = 180, 756
STEPS = [(296, "소켓 · fd", "앱과 커널의 분업", "02-01 §1 · §2", False),   # stride 112 — 통로 48px
         (408, "네임스페이스 · veth · 브리지", "격리와 배선", "02-01 §3 · §4", False),
         (520, "훅 · 연결 추적", "버릴지 고칠지", "§1 · §2", True),
         (632, "라우팅", "어디로 넘길지", "§3", True)]
...
ddx.ring_label(d, rx, ry, "이 편 — 훅 · 연결 추적 · 라우팅", 11, ACC)
...
d.legend(840, [("커널 밖", INFO), ("이 편의 범위", ACC), ("02-01 이 다룸", MUTED)])
