...
d = D(W, H, "ONE ADDRESS CHANGES · HOW MUCH TRAVELS",
      ...
      "바뀐 것은 주소 하나인데, 오가는 것이 객체 전체냐 그 주소가 든 slice 뿐이냐로 갈린다. 노드 수천 대를 곱하면 그 차이가 병목이 된다.",
      lead="바뀐 건 주소 하나인데 오가는 게 전체냐 일부냐로 갈린다")
ddx.band(d, 104, 480, "× 노드 수천 대 = 규모의 한계")
ddx.matrix(d, 44,
  [(304, "무엇을 쥐고 있나"), (284, "주소 하나가 바뀌면"), (292, "노드마다 받는 양")],
  [([("Endpoints 1 개", "Pod 주소 전부 한 객체"), ("객체 전체 재전송", "주소 1 개만 바뀌어도"),
     ("전량", "kube-proxy × 노드 수천")], BAD),
   ([("EndpointSlice N 개", "slice 마다 부분집합"), ("바뀐 slice 만 갱신", "나머지 slice 그대로"),
     ("갱신 범위 축소", "kube-proxy × 노드 수천")], OK)],
  hdr_y=224, row_h=96, gap=16, focal_col=2, sizes=(13, 12))
d.legend(496, [("전량 재전송", BAD), ("바뀐 slice 만", OK)])
