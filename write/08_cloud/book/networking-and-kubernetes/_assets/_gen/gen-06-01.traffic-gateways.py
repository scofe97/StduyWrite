...
TY, TH = 152, 96
for x, w, nm, sub, arrow, c in ((32, 452, "NAT 디바이스", "사설 인스턴스의 인터넷 출구", "밖으로만", INFO),
    ...
    d.t(x + w // 2, TY + 82, arrow, 12, SOFT, KR)
...
d.t(36, EY - 14, "ELB 넷 · 서로 다른 계층", 12, SOFT, KR, "start")
for i, (nm, layer, note, c) in enumerate(ELB):
    ...
    d.t(x + EW // 2, EY + 62, ddx.fit(layer, 12, EW - 20, layer), 12, MUTED, KR)
    d.t(x + EW // 2, EY + 90, ddx.fit(note, 12, EW - 20, note), 12, SOFT, KR)

d.legend(EY + EH + 24, [("경로 부품", INFO), ("피하거나 안 쓰는 것", WARN), ("EKS 가 쓰는 것", ACC)])
