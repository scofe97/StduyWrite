...
d = D(W, H, "IPTABLES · CHAIN FIRST, TABLE INSIDE",
      ...
      lead="레인 = 체인 · 레인 안 칸 = 그 체인이 실제로 가진 테이블 (평가 순서 Raw → Mangle → NAT → Filter)")


def lane(y, h, name, sub, mono_name=True):
...
lane(148, 124, "PREROUTING", "체인 · 라우팅 판단 전")
lane(296, 76, "라우팅 판단", "체인을 가르는 유일한 분기", mono_name=False)
lane(396, 124, "INPUT", "체인 · 라우팅 판단 후")
