...
d = D(W, H, "KNOCK FROM ANOTHER HOST · RESULT MATRIX",
      ...
      "두드린 곳이 무엇이냐에 따라 실패의 종류가 다르다. 거부와 경로 없음은 원인이 전혀 다른 실패다.",
      lead="두드린 곳에 따라 실패의 종류가 다르다 — 거부와 경로 없음은 다른 실패다")
...
ROWS = [(("호스트IP:80", "192.168.1.20:80"), ("성공", OK), ("매핑이 받아 넘긴다", "80 → 8080")),
        ...
        (("컨테이너IP", "172.17.0.2:any"), ("경로 없음", BAD), ("No route to host", "라우팅 테이블에 없다"))]
...
d.t(36, 524, "거부는 그 주소까지 닿았으나 아무도 안 듣는다는 뜻이고, 경로 없음은 그 주소에 "
             "닿을 길 자체가 없다는 뜻이다 — 사설 대역이라 밖에서 길이 없다", 12, MUTED, KR, "start")
d.legend(564, [("성공", OK), ("거부 — 닿았으나 안 들음", WARN), ("경로 없음 — 못 닿음", BAD)])
