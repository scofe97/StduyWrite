...
ROWS = [(("호스트IP:80", "192.168.1.20:80"), ("성공", OK), ("매핑이 받아 넘김", "80 → 8080")),
        (("호스트IP:8080", "192.168.1.20:8080"), ("거부", WARN), ("Connection refused", "TCP RST — 아무도 안 들음")),
        (("컨테이너IP", "172.17.0.2:any"), ("닿지 않음", BAD), ("메시지는 셋으로 갈림", "경로 항목 없음"))]

ddx.band(d, 104, 520, "실패는 두 종류 · 거부와 닿지 않음")
x = X0
XS = []
for w, name in COLS:
    XS.append((x, w))
...
# 거부와 닿지 않음의 뜻은 본문 §3 도입이 맡는다
d.legend(536, [("성공", OK), ("거부 — 닿았으나 안 들음", WARN), ("닿지 않음 — 경로가 없다", BAD)])
