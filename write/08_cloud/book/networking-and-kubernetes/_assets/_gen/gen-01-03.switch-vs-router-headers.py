...
ddx.band(d, 104, 464, "여는 깊이 = 장비가 할 수 있는 일")
d.t(48, 156, "프레임 한 장의 생김새 — 겉봉 안에 속, 속 안에 데이터", 12, SOFT, KR, "start", 600)
...
for x, y, w, h, lab, c in RINGS:
    ...
    # 링 라벨 마스크는 테두리와 겹친다 — 각 겹 테두리 바로 위(바깥 겹의 안쪽 여백)에 쓴다
    d.t(x + 14, y - 10, lab, 12, c, KR, "start", 600)
d.t(240, 328, "아무도 열지 않는 겹", 12, ACC, KR, "middle", 600)
...
row(248, "스위치 · L2 — 겉봉만", INFO,
    [("겉봉 읽기", "목적지 MAC"), ("MAC 표 조회", "나갈 포트 하나"), ("그대로 전달", "겉봉 그대로")])
row(396, "라우터 · L3 — 속의 IP 까지", WARN,
    [("속 읽기", "목적지 IP"), ("라우팅 표 조회", "다음 홉"), ("겉봉 교체", "속 IP 는 그대로")])

# 하단 해설(같은 프레임에서 여는 깊이 하나로 갈린다)은 도식 뒤 본문 두 문단이 말한다 — 뺐다
d.legend(484, [("겉봉 · 스위치", INFO), ("속 · 라우터", WARN), ("아무도 안 여는 겹", ACC)])
