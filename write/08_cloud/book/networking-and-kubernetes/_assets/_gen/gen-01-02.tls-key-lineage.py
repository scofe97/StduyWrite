...
#           본문이 틀려진다.
# 타입 스펙: type-dependency.md — master secret 한 노드에 부모가 셋이다 — type-dependency 정본이 말하는 (a) 팬인이 바로 이것이고,
#           트리로는 부모 여럿을 담지 못한다. 아래 두 칸에서 나가는 선이 하나도 없는 것이
#           "키는 만들어진 자리를 떠나지 않는다"는 주장이다.
