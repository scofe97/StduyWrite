...
def box(cx, cy, w, h, t, s, tag, c=None, focal=False):
    ...
    d.t(cx, cy + 4, ddx.fit(s, 12, w - 16, s), 12, MUTED, KR)
    d.t(cx, cy + 26, ddx.fit(tag, 12, w - 14, tag), 12, SOFT, KR)

ddx.band(d, 104, 504, "다리 수 = 관리 경로의 여유")
...
# 방향 라벨은 VPC 점선 경계 위에 앉아 뺐다 — 두 화살표의 뜻은 범례가 나른다
...
d.legend(520, [("AWS 쪽", INFO), ("내 쪽", WARN), ("다리 · 관리 → 등록 방향", ACC)])
