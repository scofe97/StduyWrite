# 02-03.three-levels-nested — 한 훅을 실행 축으로 열어 테이블과 규칙까지 세 겹으로 보인다
# 2026-09-20 축 정정: 본문이 구성(테이블 → 체인 → 규칙)과 실행(훅 → 등록된 테이블 → 규칙)을
#            두 축으로 갈랐다. 이 도식은 실행 축이므로 바깥 링을 "훅"으로 명시한다.
#            이전 판은 바깥을 "체인"이라 부르면서 본문 구성 서술("테이블이 체인을 담고")과
#            방향이 반대로 읽혀 혼동을 만들었다. Security 도 "INPUT 에 없음"으로 적었으나
#            존재 표대로 INPUT 에 있고 SELinux 미사용 시 비어 있을 뿐이라 문구를 고쳤다.
# 본문 요구: "구성은 테이블이 체인을 담고 체인이 규칙을 담습니다. 실행은 패킷이 도달한 훅에
#            등록된 처리들이 우선순위 순으로 도는 방식입니다."
...
d = D(W, H, "IPTABLES · OPENING ONE HOOK",
      "훅을 열면 테이블이, 테이블을 열면 규칙이 나온다",
      "실행 축으로 한 겹씩 연 것이다. 바깥 링이 훅(패킷이 어느 자리에 도달했는가), 가운데 링이 그 훅에 "
      "등록돼 우선순위 순으로 도는 테이블, 안쪽 링이 그 테이블의 규칙이다. INPUT 훅의 filter 를 끝까지 열었다.",
      lead="구성은 테이블이 체인을 담지만, 실행은 훅이 바깥이다 — 같은 사실을 보는 두 축")
...
ddx.ring_label(d, R1[0], R1[1], "1 훅 · 언제", 13, MUTED)
ddx.ring_label(d, R2[0], R2[1], "2 테이블 · 순서", 13, MUTED)
...
# 해설 문장 셋을 칩형 라벨로 줄였다 — 02-03 본문은 2026-09-16 사실 정정 중이라 산문을 옮기지 않았다
d.t(R2[0] + 24, 584, "Raw 는 INPUT 훅에 없음(PREROUTING·OUTPUT 뿐) · Security 는 있으나 SELinux 전용이라 보통 빈 체인",
    12, MUTED, KR, "start")
d.t(R1[0] + 24, 620, "나머지 네 훅도 같은 세 겹 · 차이는 등록된 테이블 · 칸마다 별개 체인", 12, MUTED, KR, "start")

d.t(24, 656, "위에서 아래로 · 멈추는 범위는 타깃이 정함(LOG 는 계속) · 안 걸리면 기본 정책", 12, MUTED, KR, "start")
d.legend(680, [("이 테이블 순회 끝", OK), ("종결 아님 · 계속", INFO), ("차단", BAD),
               ("여는 항목", INK), ("지금 연 겹", ACC)])
d.save("02-03.three-levels-nested.svg")
