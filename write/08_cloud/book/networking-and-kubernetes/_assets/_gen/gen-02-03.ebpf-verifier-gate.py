# 02-03.ebpf-verifier-gate — eBPF 가 커널 안으로 들어가는 경로와 그 관문
# 본문 요구: "eBPF는 샌드박스된 특수 프로그램을 커널 안에서 실행하는 시스템입니다."
#            + §5 제어/데이터 구분 표 — "양쪽 모두 제어는 유저스페이스에서 하고
#              데이터 처리는 커널 안에서 합니다. 다른 것은 내려보내는 것이 규칙이냐 프로그램이냐,
#              그리고 그 사이에 검사 관문이 있느냐입니다."
...
# 2026-09-20 사실 정정: lead 가 "Netfilter·iptables 는 패킷마다 경계를 오간다" 였다. 틀렸다.
#            iptables 의 규칙 평가도 커널 안에서 끝나고, 유저스페이스로 올라가는 것은
#            NFQUEUE 를 명시적으로 걸었을 때뿐이다(iptables-extensions(8) NFQUEUE 절).
#            도식이 본문의 오류를 그대로 실어 나르고 있었으므로 제어/데이터 축으로 바꾼다.
...
d = D(W, H, "EBPF · THE VERIFIER IS THE GATE",
      ...
      "위 레인이 유저스페이스이고 아래가 커널이다. 로드할 때 제어가 내려가고 그 뒤로는 맵을 통해 데이터가 올라온다. "
      "붙은 프로그램은 이벤트마다 커널 안에서 돈다. iptables 도 같은 제어·데이터 구분을 가지며, "
      "다른 것은 내려보내는 것이 규칙이냐 프로그램이냐와 그 사이의 검사 관문이다.",
      lead="유저스페이스는 제어(설치·갱신)에서 개입 · 데이터 처리는 커널 안 — iptables 도 마찬가지")
...
lane(U_Y, "유저스페이스", "제어 — 로드 · 결과 읽기")
lane(K_Y, "커널", "데이터 — 검사 · 실행")
...
d.t(40, 500, "verifier 가 로드 시점에 확인하는 넷 — 안전성 검사이지 행위 감사가 아니다", 12, SOFT, KR, "start", 600)
...
d.save("02-03.ebpf-verifier-gate.svg")
