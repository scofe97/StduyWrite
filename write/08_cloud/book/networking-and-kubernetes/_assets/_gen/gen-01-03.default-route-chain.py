...
ddx.band(d, 104, 736, "모르면 위로 · 꼭대기에는 떠넘길 위 없음")

# ── 떠넘기기 사슬 ─────────────────────────────────────────
box(*NB, BW, BH, "내 노트북", "10.0.0.5", "출발")
box(*HOME, BW, BH, "집 공유기", "표는 두세 줄뿐", "모르면 위로")
box(*ISP, BW, BH, "지역 ISP", "고객 대역만 앎", "모르면 위로")
rx, ry, rw, rh = RING
d.o.append(f'<rect x="{rx}" y="{ry}" width="{rw}" height="{rh}" rx="8" '
           f'fill="{INFO}06" stroke="{INFO}" stroke-width="1.2" stroke-dasharray="7 6"/>')
# 링 라벨 마스크는 테두리와 겹친다 — 링 바로 위 빈 자리에 쓴다
d.t(rx + 260, ry - 10, "꼭대기 — 떠넘길 위가 없는 자리", 12, INFO, KR, "start", 600)
box(*TOPR, "상위 사업자", "기본 경로 없음", "전체 표 보유", INFO)
...
box(*LAN, "집 안 기기", "10.0.0.x 일 때", "여기서 끝", dash=True)
# 10.0.0.0/24 줄에서 곧장 내려오면 아래 0.0.0.0/0 줄을 관통한다 — 표 오른쪽 바깥으로 돌아 내려온다
r1y = ty + 46 + 24
d.path(f"M {tx+tw-12+6} {r1y} L {tx+tw+16} {r1y} L {tx+tw+16} {LAN[1]} L {LAN[0]+LAN[2]//2+10} {LAN[1]}",
       MUTED, 1.3, m="ar", dash="5 5")
# 0.0.0.0/0 에서 사슬이 시작된다
d.path(f"M {tx+40} {ty+154} L {tx+40} {ISP[1]} L {CHX+BW//2+10} {ISP[1]}", ACC, 1.8, m="acc")
d.t(tx + 56, ISP[1] - 14, "여기서 밖으로", 12, ACC, KR, "start")

box(*GNET, "구글 쪽 망", "8.8.8.0/24 광고", "대역의 주인", OK)
...
# 하단 해설(꼭대기는 기본 경로 없이 전체 표)은 본문 §3 L418 목록이 말한다 — 뺐다
d.legend(756, [("전체 표 보유", INFO), ("도착", OK), ("걸리는 줄", ACC)])
