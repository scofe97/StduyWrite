...
ddx.band(d, 104, 404, "껍데기만 붙었다 벗겨짐 · 속 프레임은 불변")
for cx, t in zip(CX, ["(1) 노드 A 안에서", "(2) 감싸기", "(3) IP 망 통과", "(4) 노드 B 안에서"]):
...
shell(CX[2], CY, "겉만 읽힘");                          frame(CX[2], CY + 16, SOFT)
frame(CX[3], CY)

for cx, note, c in zip(CX, ["경계 통과 불가", "봉투 부착", "라우터에겐 평범한 IP", "원래 프레임 그대로"],
                       [MUTED, ACC, MUTED, OK]):
    d.t(cx, CY + SH // 2 + 26, note, 12, c, KR)

for j, lab in enumerate(["넣기", "봉함된 채", "꺼내기"]):
...
# 하단 해설(가두는 VLAN · 잇는 VXLAN)은 도식 뒤 본문 문단이 말한다 — 뺐다
d.legend(424, [("겉봉 · 라우터가 읽는 부분", INFO), ("감싸는 순간", ACC)])
