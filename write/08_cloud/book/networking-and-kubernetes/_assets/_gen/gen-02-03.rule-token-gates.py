# 02-03.rule-token-gates — 규칙 한 줄을 토큰으로 쪼개면 패킷을 바꾸는 것은 하나뿐
...
ddx.band(d, 104, 600, "iptables -t nat -S KUBE-SEP-2MJG2J3URJK2NCRL 출력 한 줄 원본")
...
NODES = [("-A KUBE-SEP-2MJG…", "이 규칙이 놓일 체인", "조건도 동작도 아님", False),
         ("-p tcp", "통과 여부만 결정", "이 줄의 유일한 조건", False),
         ("-m comment · -m tcp", "판정 영향 없음", "사람용 메모와 빈 토큰", False),
         ("-j DNAT", "목적지 교체", "dst → 10.244.1.66:8080", True)]
...
d.t(CX[1], SKIP_CY - 14, "규칙 전체 건너뜀", 13, MUTED, KR, "middle", 600)
d.t(CX[1], SKIP_CY + 8, "뒤 토큰 미평가", 12, MUTED, KR)
...
# '패킷을 바꾸는 것은 마지막 -j 하나뿐'과 판정 무관 토큰 설명은 본문 산문이 맡는다
d.legend(616, [("패킷을 바꾸는 자리", ACC), ("유일한 조건", INFO), ("판정 무관", SOFT)])
d.save("02-03.rule-token-gates.svg")
