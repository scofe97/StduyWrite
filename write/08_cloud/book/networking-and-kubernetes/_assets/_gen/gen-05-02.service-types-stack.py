...
def box(cx, cy, w, h, t, s, tag, c=None, focal=False, dash=False):
    ...
    d.t(cx, cy + 8, ddx.fit(s, 12, w - 16, s), 12, MUTED,
        MONO if all(ord(ch) < 128 or ch in ':~' for ch in s) else KR)
    if tag: d.t(cx, cy + 30, ddx.fit(tag, 12, w - 14, tag), 12, SOFT, KR)

ddx.band(d, 104, 584, "호출자 위치 = 들어오는 겹")
...
box(HL[0] + HL[2] // 2, HL[1] + HL[3] // 2, HL[2], HL[3],
    "Headless", "clusterIP: None", "DNS 가 명단 반환", OK, dash=True)
d.path(ddx.elbow(CIP[0]+CIP[2]+6, CIP[1]+CIP[3]//2, HL[0]-10, HL[1]+HL[3]//2), OK, 1.4, m="ok", dash="6 5")
d.t(HL[0] + HL[2] // 2, HL[1] - 14, "LB 를 뺀 변형", 12, OK, KR)
...
d.legend(600, [("호출자 · 바깥 층", INFO), ("노드 포트 층", WARN), ("가장 안쪽", ACC), ("변형", OK)])
