# 02-03.nat-table-traverse — 한 요청이 nat 을 지나는 자리는 훅 둘이고, 그 안은 점프로 겹겹이다
...
#           2026-08-29 대조: 레인이 팀이 아니라 훅이라는 점은 chain-then-table 과 같다. 레인을 넘는
#           인계가 있어 swimlane 을 유지한다.
...
d.legend(776, [("지나는 체인", INFO), ("바뀌는 자리 · 레인을 갈아타는 자리", ACC),
               ("노드에서 온 요청일 때만", WARN)])
d.save("02-03.nat-table-traverse.svg")
