...
for i, (title, sub, done, o1, o2) in enumerate(STATES):
    ...
    d.t(BX + 18, cy + 17, ddx.fit(sub, 12, BW - 36, f"sub {sub}"), 12, MUTED, KR, "start")

    oc = INK if done else SOFT
    d.line(OX, y, OX, y + BH, RULE, 0.8)
    lat = all(ord(c) < 128 for c in o1)
    d.t(OX + 18, cy - 4, ddx.fit(o1, 12, OW - 36, f"obs {o1}"), 12, oc,
        MONO if lat else KR, "start")
    d.t(OX + 18, cy + 17, ddx.fit(o2, 12, OW - 36, f"obs2 {o2}"), 12, MUTED, KR, "start")
...
d.t(CXX + 20, 584, "묶음 6~8 · 이름 · L7 · 노드 밖 · 모두 완료", 12, SOFT, KR, "start")

d.legend(612, [("실행해서 값을 본 상태", INFO), ("지금 도달한 자리", ACC), ("아직 실행 전", SOFT)])
