...
ddx.band(d, 104, 548, "번호는 조직마다 하나 · 라우터 대수와 무관")
# 링 라벨 마스크는 테두리와 겹친다 — 조직 이름을 경계 안쪽 빈 모서리(경계 라우터 위)에 쓴다
for (rx, ry, rw, rh), lab, anchor in [(R1, "AS 100 — 사업자 한 곳", "end"), (R2, "AS 200 — 다른 사업자", "start")]:
    d.o.append(f'<rect x="{rx}" y="{ry}" width="{rw}" height="{rh}" rx="8" '
               f'fill="{INFO}06" stroke="{INFO}" stroke-width="1.2" stroke-dasharray="7 6"/>')
    d.t(rx + rw - 16 if anchor == "end" else rx + 16, ry + 24, lab, 12, INFO, KR, anchor, 600)

rt(*E1, "경계 라우터", "eBGP 담당", "AS 100")
rt(*E2, "경계 라우터", "eBGP 담당", "AS 200")
for cx, cy in I1: rt(cx, cy, "내부 라우터", "iBGP 만", "AS 100")
for cx, cy in I2: rt(cx, cy, "내부 라우터", "iBGP 만", "AS 200")

# 내부 → 경계 연결은 직각으로 꺾는다 — 두 내부 라우터가 한 줄기(TRUNK)로 모인다
for (ix, iy), (ex, ey) in [(I1[0], E1), (I1[1], E1), (I2[0], E2), (I2[1], E2)]:
    ...
    trunk = (sx + tx) // 2
    d.path(f"M {sx} {iy} L {trunk} {iy} L {trunk} {ey} L {tx} {ey}", MUTED, 1.3, dash="5 5")
...
# 두 조직 경계 사이 홈통은 60px 뿐이다 — 라벨이 점선을 가로지르지 않게 네 글자로 둔다
d.t(500, 356, "번호 추가", 12, ACC, KR)

# 하단 해설(목록에 숫자 두 개만)은 도식 뒤 본문 목록이 말한다 — 뺐다. 목록 칩은 그 결과라 남긴다
d.chip(500, 480, "AS_PATH  [100, 200]", INFO, 12)
