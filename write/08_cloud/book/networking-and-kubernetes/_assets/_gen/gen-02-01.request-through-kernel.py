...
BW, BH, GAP = 130, 104, 12   # 11px 라벨 수용
CX = [12 + BW // 2 + i * (BW + GAP) for i in range(7)]           # 80 220 360 500 640 780 920
CY = 300
...
for cx, (l, s, t, c) in zip(CX, NODES):
    ...
    d.t(cx, CY - 2, ddx.fit(s, 11, BW - 12, s), 11, MUTED,
        MONO if all(ord(ch) < 128 or ch in '·' for ch in s) else KR)
    d.t(cx, CY + 26, ddx.fit(t, 11, BW - 10, t), 11, SOFT, KR)
...
d.t(rx + 8, CY + BH // 2 + 30, "경계 진입", 11, ACC, KR, "start")
d.t(rx + rw - 8, CY + BH // 2 + 30, "경계 이탈", 11, ACC, KR, "end")
