# 02-03.chain-then-table — 체인이 바깥이고 테이블이 그 안이라는 것을 레인으로 보인다
...
d = D(W, H, "IPTABLES · WHAT IS REGISTERED AT EACH HOOK",
      "훅마다 등록된 것이 다르다 — 라우팅 판단에서 갈리는 두 갈래",
      "레인 하나가 훅 하나다. 레인 안의 칸이 그 훅에 등록돼 우선순위 순으로 도는 테이블이고, "
      "점선 칸은 그 훅에 등록되지 않아 차례가 없는 테이블이다. 칸 하나가 곧 그 테이블이 이 훅에 가진 체인이다.",
      lead="레인 = 훅(위는 체인 이름, 파란 글자는 커널의 훅 이름) · 열 = 테이블 · 칸마다 별개 체인")

# 열 머리 — 그 테이블이 어느 체인에 놓일 수 있는지
for cx, (nm, note) in zip(CX, [("Raw", "PREROUTING·OUTPUT 뿐"), ("Mangle", "다섯 체인 전부"),
                               ("NAT", "두 번 켜짐"), ("Filter", "INPUT·FORWARD·OUTPUT")]):
...
lane(688, "POSTROUTING", "떠나기 직전", "NF_IP_POST_ROUTING", "노드 밖으로")
...
d.t(CX[1], 330, "바뀐 목적지로 경로 조회 · DNAT 이 먼저인 이유", 13, MUTED, KR)
...
d.t(CX[1], 526, "목적지가 남이면 아래 레인", 13, SOFT, KR)
...
# 열 정렬의 뜻과 NAT 열이 두 번 켜지는 이유는 본문 산문이 맡는다
d.legend(828, [("그 체인이 가진 테이블", INFO), ("없는 테이블", SOFT),
               ("체인을 갈아타는 자리", ACC)])
d.save("02-03.chain-then-table.svg")
