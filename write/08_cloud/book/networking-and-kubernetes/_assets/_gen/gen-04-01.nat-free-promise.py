...
def box(cx, title, lines, c, focal=False):
    ...
    for i, ln in enumerate(lines):
        d.t(cx, BY + 66 + i * 24, ddx.fit(ln, 12, BW - 16, ln), 12, MUTED, KR)

box(CX[0], "포트 제약", ["주소·포트·프로토콜", "조합에 프로그램 하나", "두 웹 서버의", "80 포트 충돌"], WARN)
box(CX[1], "IP-per-Pod", ["Pod 마다 고유 IP", "포트 플래그·DNAT·", "리버스 프록시 없이", "각자 사용"], INFO)
box(CX[2], "NAT 없음 3가지 약속", ["컨테이너끼리", "노드 ↔ 컨테이너", "안에서 본 자기 IP =", "밖에서 본 그 IP"], INFO)
box(CX[3], "떠안은 복잡성", ["Pod 마다 IP 할당", "그 IP 의 라우팅", "클러스터 몫"], ACC, focal=True)
...
d.legend(352, [("없애려던 제약", WARN), ("선택과 약속", INFO), ("지불한 값", ACC)])
