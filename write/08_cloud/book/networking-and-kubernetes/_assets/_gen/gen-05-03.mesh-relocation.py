...
for i, ln in enumerate(["게이트웨이 · 클러스터 출입 트래픽", "사이드카 · 서비스 간 mTLS 트래픽", "컨트롤 플레인 · 둘의 설정"]):
...
for i, ln in enumerate(["디스커버리 · 로드밸런싱", "복원력 · 보안 · 관측", "라우팅 제어 · API"]):
    ...
    d.t((MX + RX) // 2, BY + 44 + i * 22, ddx.fit(ln, 12, RX - MX - 24, ln), 12, MUTED, KR)

FY = BY + BH + 32
d.tone(BX, FY, RX + RW - BX, 60, ACC, 6, "12", 1.4)
d.t(500, FY + 38, "컨트롤 플레인 장애에도 받은 설정으로 동작하는 사이드카 · 운영 안정성의 근거", 13, ACC, KR)

d.legend(FY + 84, [("있던 자리", WARN), ("옮겨 간 자리", INFO), ("자리 이동이 만든 성질", ACC)])
