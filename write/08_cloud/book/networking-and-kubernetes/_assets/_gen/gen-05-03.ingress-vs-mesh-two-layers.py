...
def box(cx, cy, t, s, tag, c=None, w=BW, dash=False):
    ...
    d.t(cx, cy + 0, ddx.fit(s, 12, w - 16, s), 12, MUTED, KR)
    d.t(cx, cy + 26, ddx.fit(tag, 12, w - 14, tag), 12, SOFT, KR)

ddx.band(d, 104, 540, "방향 둘 · 밖 → 안 · 안 ↔ 안")
rx, ry, rw, rh = RING
d.o.append(f'<rect x="{rx}" y="{ry}" width="{rw}" height="{rh}" rx="8" '
           f'fill="{INFO}06" stroke="{INFO}" stroke-width="1.2" stroke-dasharray="7 6"/>')
ddx.ring_label(d, rx, ry, "Kubernetes 클러스터 · 안쪽 통신은 사이드카 담당", 11, INFO, off=16)

box(*EXT, "외부 클라이언트", "HTTP(S) 요청", "클러스터 밖", INFO, w=160)
box(*ING, "Ingress", "host·path fan-out", "밖 → 안 L7 라우터", INFO)
box(*SVA, "서비스 A", "app + 사이드카", "프록시가 mTLS 적용", OK)
box(*SVB, "서비스 B", "app + 사이드카", "재시도·관측·라우팅 대행", OK)
box(*CPL, "메시 컨트롤 플레인", "사이드카에 설정 배포", "장애 시에도 데이터 플레인 동작", MUTED, w=280, dash=True)

d.path(f"M {EXT[0]+80+6} {EXT[1]} L {ING[0]-BW//2-10} {ING[1]}", INFO, 1.6, m="info")
# 방향 라벨은 점선 경계·상자 변에 걸려 뺐다 — 방향은 색과 범례가 나른다
d.path(f"M {ING[0]+BW//2+6} {ING[1]} L {SVA[0]-BW//2-10} {SVA[1]}", INFO, 1.6, m="info")
d.path(f"M {SVA[0]+BW//2+6} {SVA[1]} L {SVB[0]-BW//2-10} {SVB[1]}", OK, 1.8, m="ok")
...
d.t(CPL[0], CPL[1] - 74, "설정 배포", 12, MUTED, KR)

d.legend(556, [("밖 → 안", INFO), ("안 ↔ 안", OK)])
