...
review:
  ...
  # 2026-09-09 구조 교정 + 2차 모델(Codex) 검증 반영. 관찰 표와 §실습 상태는 이번에도 건드리지 않았다.
  #   §5 앞 빈 줄이 둘뿐이던 것을 셋으로 교정(1부 규칙 2). 어제 회차에서 범위 밖으로 두고 보고했던 자리다.
  #   사실오류 1 — 어제 넣은 §8 문단이 "줄이 없으면 VM 은 어디로도 보내지 않는다"고 적었는데,
  #     같은 문서 관찰 8-3 은 "기본 경로로 내보내고 밖에서는 아무도 답하지 않는다"이다. 관찰과 맞게 고쳤다.
  #   §8 둘째 문단의 "/16 한 줄이면 끝난다"를 조건절로 낮췄다 — 그 노드가 나머지로 넘겨 주어야 성립하고 실행하지 않았다.
  #   **보고(고치지 않음)**: 관찰 2-1 의 "등록된 순서로 잘려 나가므로"는 배정 결과만으로 입증되지 않는 인과다.
  #     관찰 기록이라 손대지 않았고, 대신 §2 「확인된 것」이 그 사실을 드러내도록 적었다. 판단은 학습자 몫으로 남긴다.
updated: 2026-09-09
---
...
CNI 가 없는 클러스터를 일부러 만들어 노드가 멈춰 서는 장면부터 시작합니다. 이어서 다섯을 확인합니다.

- 주소가 잘려 내려오는 곳
- 서비스 하나의 규칙 줄 수
- readiness 를 잃은 Pod 의 줄이 실제로 사라지는지
- 정책을 걸면 함께 끊기는 것
- 이름 하나의 질의 횟수
...
그래서 Pod 가 뜨느냐 마느냐를 가르는 기준은 **자기 IP 가 필요한가** 하나입니다. `hostNetwork` 인 `etcd`·`kube-apiserver`·`kube-controller-manager`·`kube-scheduler`·`kube-proxy` 는 CNI 없이도 뜹니다. `CoreDNS` 만 Pod IP 를 받아야 해서 걸립니다.
...
다만 **조각 번호는 노드 이름 순서가 아닙니다.** `worker` 가 `10.244.2.0/24`, `worker2` 가 `10.244.1.0/24` 를 받았습니다. 관측 칸은 이것을 "등록된 순서로 잘려 나간다"로 적었는데, 그 순서까지는 이 실습이 직접 재지 않았습니다. 확실한 것은 이름으로 대역을 추측하면 안 된다는 것입니다.
...
막은 쪽이 누구인지는 코드와 본문으로 갈립니다. `/health` 는 `403` 에 `Access denied` 한 줄(`content-length: 15`)이었습니다. 정책에 더한 `/nope` 는 `404` 에 `nginx/1.31.5` HTML 이었습니다. 둘 다 서버에 없는 경로인데 답한 쪽이 다릅니다.
...
한 노드만 경유하게 하면 되지 않느냐는 물음이 여기서 나옵니다. **라우팅은 보내는 쪽이 정하므로** 그 경유도 VM 이 정해 줘야 합니다. VM 의 테이블에 `10.244.1.0/24` 로 가는 줄이 없으면 VM 은 그 대역을 기본 경로로 내보내고 그쪽에서는 아무도 답하지 않습니다. 8-3 에서 본 `000 · 4.00s` 가 그 상태입니다. 경유할 노드를 정해 주는 것도 결국 VM 의 테이블에 줄을 넣는 일이라, 줄 없이 경유만 시킬 방법이 없습니다.

큰 대역을 한 노드에 통째로 걸면 경유 자체는 성립합니다. 이 실습에서 해 보지는 않았습니다. `10.244.0.0/16` 을 노드 하나로 보내고 그 노드가 나머지로 넘겨 주기만 하면 줄은 하나로 끝납니다. 대신 다른 노드로 갈 트래픽이 전부 그 노드를 지납니다. 그 노드가 병목이 되고 그 노드가 죽으면 클러스터 전체가 밖에서 안 보입니다. 줄 하나를 아끼고 단일 장애점을 얻는 거래입니다. **노드마다 한 줄씩 넣는 쪽이 나은 이유가 이것입니다.**
