...
아래는 이 편 전체를 모델에서 컴포넌트로 내려가는 순서로 이은 지도입니다. 위 두 단계가 "주소를 어떻게 줄 것인가"라는 모델의 결정이고, 아래 세 단계가 그 결정을 실행하는 컴포넌트입니다. 각 단계 오른쪽의 절 번호가 본문에서 그것을 다루는 곳입니다. 색이 붙은 곳은 마지막 한 군데뿐이고 경고입니다. liveness 에 외부 의존을 넣는 실수가 이 편에서 가장 값비싼 오용이라 표시했습니다.
