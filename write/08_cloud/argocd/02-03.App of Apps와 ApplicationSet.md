...
> `--context` 에 넘기는 값은 로컬 kubeconfig 의 컨텍스트 이름이다. 위 예시의 `tps-v305p` 는 2026-09-10 현재 해당 클러스터 API 가 응답하지 않아 로컬에서 제거된 상태이므로, 그대로 실행하면 컨텍스트를 찾지 못한다. 자기 환경의 컨텍스트 이름(`kubectl config get-contexts`)으로 바꿔 읽는다.
