---
title: 04-01.message-lib config 5개 클래스 종합
...
2. Kafka Producer/Consumer의 직렬화·역직렬화 구조와 Avro의 wire format (`../01-03.Avro`, `../02-01.Avro 직렬화 예외처리 전략`).
3. Kafka의 idempotent producer와 단일 파티션 EOS의 의미 (`../03-04.Exactly-once 의미론과 Consumer Idempotency`).
...
`05-03`이 *예외 처리*만 통합했다면, 본 글은 *config 디렉토리 전체*를 통합한다. `KafkaErrorConfig` 라인별 정독은 [01-13](../05_ConsistencyPattern/05-04.KafkaErrorConfig%20DLT%20헤더%20폭증%20사고.md)에서 풀로 풀려 있고, 본 글 §5는 한 단락 요약만 둔다.
...
세 가지 키가 결정적이다. `acks=all` + `retries>0` + `enable.idempotence=true` 셋이 단일 파티션 단위 Exactly-once 발행의 조건이다. `03-04 §2`에서 다룬 EOS 트리오가 정확히 여기서 박힌다. 셋 중 하나라도 빠지면 브로커 재시도 시 같은 메시지가 두 번 들어가거나 ack 손실로 누락이 생긴다.
...
> 본 클래스의 *라인 단위* 정독, `excludeHeader(EX_STACKTRACE)`/`setStripPreviousExceptionHeaders(true)`의 도입 배경, 실제 운영 사고 회고는 [01-13](../05_ConsistencyPattern/05-04.KafkaErrorConfig%20DLT%20헤더%20폭증%20사고.md)에서 풀로 다룬다. 여기서는 다섯 클래스 안에서의 *위치*만 짚는다.
...
1. **[04-02.message-lib config 학습 검증](04-02.message-lib%20config%20학습%20검증.md)** — 면접 Q&A 10개와 빈 Spring Boot 프로젝트에서 직접 돌리는 실습 과제 3개. 본 문서의 학습 목표 다섯 가지(§0-3)에 답할 수 있는지 점검한다.
2. **[04-03.message-lib config 운영 이식 가이드](04-03.message-lib%20config%20운영%20이식%20가이드.md)** — 다른 프로젝트에 패턴을 옮길 때의 결정 체크리스트(브로커 종류·Schema Registry 유무·직렬화 형식·idempotence), PR 머지 전 코드 체크리스트, 운영 모니터링 항목.
...
Redpanda v25.3에서 본 다섯 클래스는 모두 *그대로* 동작한다. 두 가지만 운영 환경마다 별도 검증이 필요하다 — Schema Registry URL의 yaml 명시 여부, idempotence 옵션이 의존 모듈에서 비활성화되지 않았는지. 두 항목의 점검 방법은 [01-14 §3](04-03.message-lib%20config%20운영%20이식%20가이드.md)에서 다룬다.
...
- [05-01.Spring Kafka DLT와 Producer Config](../05_ConsistencyPattern/05-01.Spring%20Kafka%20DLT와%20Producer%20Config.md) — Producer 설정과 DLT 라우팅의 기본 모델
- [05-03.Kafka 예외 처리 통합](../05_ConsistencyPattern/05-03.Kafka%20예외%20처리%20통합.md) — 본 글이 *config 디렉토리 전체*라면, 03-03은 *예외 처리 측면만*
- [05-04.KafkaErrorConfig DLT 헤더 폭증 사고](../05_ConsistencyPattern/05-04.KafkaErrorConfig%20DLT%20헤더%20폭증%20사고.md) — KafkaErrorConfig 한 클래스의 라인 단위 정독과 사고 회고
- [03-02.Spring Kafka 운영 고급](03-02.Spring%20Kafka%20운영%20고급.md) — 운영 환경에서 추가로 챙기는 설정 모음
- [../02-01.Avro 직렬화 예외처리 전략](../02_MessageContract/02-01.Avro%20직렬화%20예외처리%20전략.md) — `ErrorHandlingDeserializer` 위임 패턴의 직렬화 측면
- [../03-04.Exactly-once 의미론과 Consumer Idempotency](../05_ConsistencyPattern/03-04.Exactly-once%20의미론과%20Consumer%20Idempotency.md) — EOS 트리오의 의미와 단일 파티션 EOS
- [04-02.message-lib config 학습 검증](04-02.message-lib%20config%20학습%20검증.md) — 본 문서를 다 읽은 뒤 Q&A·실습으로 이해를 검증하는 다음 단계
- [04-03.message-lib config 운영 이식 가이드](04-03.message-lib%20config%20운영%20이식%20가이드.md) — 검증을 마친 패턴을 다른 프로젝트로 옮길 때의 체크리스트
...
- 본 저장소 내부: `../02-01.Avro 직렬화 예외처리 전략`, `../03-04.Exactly-once 의미론과 Consumer Idempotency`, `05-03.Kafka 예외 처리 통합`, `05-04.KafkaErrorConfig DLT 헤더 폭증 사고`.
...
> - **상세 참조**: [05-04.KafkaErrorConfig DLT 헤더 폭증 사고](../05_ConsistencyPattern/05-04.KafkaErrorConfig%20DLT%20헤더%20폭증%20사고.md) (단일 클래스 회고), [05-03.Kafka 예외 처리 통합](../05_ConsistencyPattern/05-03.Kafka%20예외%20처리%20통합.md) (예외 처리 통합 가이드).
