...
리소스 패턴 타입(`literal` vs `prefixed`)도 운영 비용에 직결된다. 토픽 이름을 도메인 prefix(`tps.executor.`, `tps.operator.`)로 끊는 우리 명명 규칙([03-01.토픽 디자인](./../03_TopicDesign/01-01.토픽%20디자인.md))과 prefix ACL을 같이 쓰면, 새 토픽이 생겨도 ACL을 다시 부여하지 않아도 된다. 반면 `literal`로만 쓰면 토픽이 추가될 때마다 ACL PR이 따라간다. 우리 환경에서는 prefix를 기본으로 두고, 민감한 토픽 한두 개만 literal로 좁히는 방식이 운영 부담이 가장 낮다.
