---
title: 02-02.Redpanda Console 인증
tags: []
status: draft
related: []
updated: 2026-05-09
...
> 배포 차트의 Console 인증 슬롯(jwtSigningKey·oidc.clientSecret·license)이 `values-dev/ppp`에서 비어 있어, Console이 "익명도 다 보여주는" 모드로 떠 있다. 무료 라이선스에서 그 게이트를 세우는 방법을 정리한다.
...
> dev는 nginx Ingress, ppp는 Istio Gateway로 Console에 닿는다. 인증 키 슬롯은 세 군데(values.yaml의 secret, values-dev/ppp의 console.config, 브로커 `auth.sasl.enabled`)에 흩어져 있고 모두 비어 있다.
...
> 차트가 OIDC·RBAC 슬롯을 만들어 두었어도, license 키가 없으면 Console 바이너리가 라이선스 검증에 실패해 로그인 화면을 그리는 코드 경로에 진입조차 못 한다. 무료 사용자가 포기하는 것은 네이티브 로그인 화면과 사용자별 권한 분리 둘이다.
...
> nginx ingress의 `auth-basic` 어노테이션으로 Console 앞에 HTTP Basic 게이트를 세운다. Console은 손대지 않아 가장 단순하지만, SSO가 안 되고 ingress 전제라 Istio를 쓰는 ppp에는 못 쓴다.
...
> nginx의 `auth-url` 어노테이션이 요청마다 oauth2-proxy에 부속 요청을 보내 OIDC 세션을 검사한다. 사내 SSO·그룹 단위 제한을 얻지만 별도 워크로드 운영 부담이 있고, 이 역시 nginx ingress 전제라 ppp에는 못 옮긴다.
...
> oauth2-proxy를 별도 Deployment 대신 Console과 같은 Pod의 사이드카로 넣어 Pod 진입점에서 인증을 끝낸다. ingress·Istio 무엇을 쓰든 무관해 dev·ppp 양쪽에 동일하게 적용된다.
...
> ppp는 Istio 메시 안에 있으므로 `RequestAuthentication`으로 JWT를 검증하고 `AuthorizationPolicy`로 통과 규칙을 건다. 사이드카가 인증을 처리해 Console·oauth2-proxy 추가 배포가 없지만, Istio 메시 밖 환경에는 적용되지 않는다.
...
> dev는 빠른 옵션 A 또는 SSO가 필요하면 B, 환경 무관하게 가려면 C, ppp 메시 안이면 D로 갈린다. 의사결정 트리와 표로 환경·요구에 맞는 옵션을 고른다.
...
> 네 옵션 모두 Console UI 앞단만 가린다. 브로커 9093 포트로 직접 붙는 클라이언트 평면과 사용자별 권한 분리는 어느 옵션으로도 해결되지 않으며, 이는 SASL([02-03](02-03.Kafka·Redpanda%20SASL%20인증.md))의 몫이다.
...
마지막으로 **Kafka 브로커 SASL/TLS** 도입을 별도 과제로 분리한다. 이건 Console 인증과 무관한 더 큰 작업이지만, 본 문서의 모든 옵션은 어디까지나 UI 게이트라는 사실을 잊지 않기 위해 함께 적어 둔다. 자세한 절차는 [02-03.Kafka·Redpanda SASL 인증](02-03.Kafka·Redpanda%20SASL%20인증.md)에서 다룬다.
