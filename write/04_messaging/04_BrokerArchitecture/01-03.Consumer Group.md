...
- 브로커/파티션/복제의 아키텍처 상세는 [01-01.메시지 큐 아키텍처](01-01.메시지%20큐%20아키텍처.md)를 참조한다.
...
`subscribe()`에는 토픽 목록 대신 정규식을 넘길 수도 있다. 정규식이 여러 토픽 이름에 매칭되고, 매칭되는 이름의 새 토픽이 생기면 거의 즉시 리밸런스가 일어나 Consumer가 그 토픽 소비를 시작한다. Kafka와 다른 시스템 사이를 복제하거나 스트림을 처리하는 애플리케이션에서 흔히 쓴다.

```java
// test로 시작하는 모든 토픽을 구독 — 새 매칭 토픽 생성 시 자동 합류
consumer.subscribe(Pattern.compile("test.*"));
```

다만 클러스터에 파티션이 3만 개 이상으로 아주 많으면 주의해야 한다. 정규식 구독의 토픽 필터링은 *client 측*에서 일어나므로, Consumer가 전체 토픽·파티션 목록을 정기적으로 broker에 요청해 새 토픽을 감지한다. 목록이 크고 Consumer가 많으면 이 메타데이터가 broker·client·네트워크에 상당한 부하를 주고, 메타데이터 대역폭이 실제 데이터 대역폭보다 커지는 경우도 있다. 또 정규식으로 구독하려면 client가 클러스터 전체 토픽을 describe할 권한, 즉 클러스터 전역 describe 권한이 필요하다.

반대로 그룹과 리밸런스가 전혀 필요 없는 경우도 있다. 단일 Consumer가 토픽의 모든 파티션 또는 특정 파티션을 항상 읽는다면, 그룹 대신 파티션을 직접 할당하면 된다. 이때는 `subscribe()` 대신 `assign()`을 쓴다. Consumer는 토픽을 구독해 그룹의 일원이 되거나, 스스로 파티션을 할당하거나 둘 중 하나만 할 수 있고 동시에는 불가능하다(단, offset 커밋에는 여전히 `group.id`가 필요하다).

```java
// 특정 토픽의 모든 파티션을 직접 할당 — 그룹·리밸런스 없음
...
수동 할당의 대가는 새 파티션 알림이 없다는 점이다. 누군가 토픽에 파티션을 추가해도 Consumer는 통지받지 못하므로, `partitionsFor()`를 주기적으로 확인하거나 파티션 추가 시 애플리케이션을 재시작해 대응해야 한다.

Consumer를 종료할 때는 `close()`를 호출한다. `close()`는 두 가지를 한다. 필요하면 offset을 커밋하고, Group Coordinator에 그룹을 떠난다고 알린다. 후자 덕분에 Coordinator가 즉시 리밸런스를 트리거해, 종료하는 Consumer의 파티션이 다른 Consumer에 배정되기까지 `session.timeout.ms`를 기다릴 필요가 없다. 종료 절차의 상세는 [01-06.Consumer poll 루프와 종료](01-06.Consumer%20poll%20루프와%20종료.md)에서 다룬다.
...
> Consumer Group은 파티션마다 마지막 처리 위치를 오프셋으로 추적해 `__consumer_offsets`에 커밋한다. 이 위치가 재시작·리밸런스 후 어디서 재개할지를 결정한다.
...
- [01-01.메시지 큐 아키텍처](01-01.메시지%20큐%20아키텍처.md) — 파티션·복제 추상 위에서 Consumer Group이 동작
- [01-04.리밸런스 프로토콜](01-04.리밸런스%20프로토콜.md) — JoinGroup/SyncGroup의 더 깊은 흐름
- [03-04.Exactly-once 의미론과 Consumer Idempotency](../05_ConsistencyPattern/03-04.Exactly-once%20의미론과%20Consumer%20Idempotency.md) — 오프셋 커밋과 처리 사이의 비대칭
- [03-01.Inbox](../05_ConsistencyPattern/03-01.Inbox.md) — Consumer 측 멱등성을 강제하는 짝 패턴
...
- 리밸런스 프로토콜 심화: [01-04.리밸런스 프로토콜](01-04.리밸런스%20프로토콜.md)
...
> - **상세**: [`02-04.Avro Consumer 수신 패턴`](../02_MessageContract/02-04.Avro%20Consumer%20수신%20패턴.md) — JSON과 Avro consumer를 토픽별로 분리하는 이유.
>
...
>
> - 같은 메시지를 두 번 consume해도 결과가 한 번만 반영되도록 `TB_TRB_OX_002` (`(MSG_ID, CONSUMER_GROUP)` PK) 도입. 상세 의미론은 [`03-04.Exactly-once 의미론과 Consumer Idempotency`](../05_ConsistencyPattern/03-04.Exactly-once%20의미론과%20Consumer%20Idempotency.md), TPS 코드 측 계획은 `message-lib/docs/inbox-idempotency-plan.md`.
