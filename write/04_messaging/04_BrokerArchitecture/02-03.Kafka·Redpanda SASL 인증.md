...
> UI 게이트는 브라우저 접속만 막을 뿐 9092 포트로 직접 붙는 프로듀서·컨슈머 평면은 보호하지 못한다. SASL이 꺼져 있으면 모든 클라이언트가 익명이고, 브로커는 그 익명조차 인지하지 못한다.
...
> TLS는 전송 도청을, SASL은 클라이언트 신원을 책임진다. 둘은 직교하므로 운영 기본 조합은 `SASL_SSL` 하나로 수렴하며, SASL은 `SaslHandshake`/`SaslAuthenticate` 두 프레임으로 와이어 프로토콜에 끼어든다.
...
> 다섯 메커니즘(PLAIN·SCRAM-256·SCRAM-512·OAUTHBEARER·GSSAPI)과 참고용 mTLS를 비교하면, 운영계 표준은 동적 사용자 관리·salted challenge-response·Kafka/Redpanda 양쪽 1급 지원을 갖춘 SCRAM-SHA-512 + SASL_SSL로 수렴한다.
...
> 클라이언트는 `security.protocol`·`sasl.mechanism`·`sasl.jaas.config` 세 줄로 SASL을 켠다. 한 JVM이 여러 클러스터에 다른 신원으로 붙는 경우가 흔해, JVM 전역 파일 대신 클라이언트 단위 인라인 JAAS가 표준이다.
...
> SCRAM-SHA-512는 4-step challenge-response로, 서버가 비밀번호를 받지 않고도 클라이언트가 비밀번호를 안다는 사실을 검증한다. 각 단계에 nonce가 들어가 직전 핸드셰이크 재생 공격이 통하지 않는다.
...
> SASL이 *누구인지*를, ACL이 *무엇을 할 수 있는지*를 책임진다. 두 층을 같이 켜지 않으면 "인증된 익명"이 된다. Redpanda는 사용자 생성과 권한 부여가 분리돼 superuser가 ACL을 별도로 줘야 한다.
...
> SCRAM이라도 ClientProof 캡처 후 오프라인 brute-force가 가능하므로 TLS 채널 안에 SASL을 묶는 SASL_SSL이 운영 기본이다. internal/external 리스너 분리·truststore 핫리로드·호스트네임 검증·cipher suite까지 함께 결정해야 한다.
...
> Redpanda는 단일 바이너리·KRaft 대체 구조 때문에 Kafka와 네 가지가 다르다. ZooKeeper JAAS 파일이 없고, `redpanda.yaml` 키 이름이 다르며, OAUTHBEARER·Kerberos가 라이선스 의존이고, Helm 차트 인터페이스가 별도다.
...
> 진입점은 차트(`redpanda` helm values의 `auth.sasl`·`tls`)와 라이브러리(`KafkaDefaultsEnvironmentPostProcessor`에 프로파일별 SASL 키 주입) 두 곳이다. Schema Registry·Connect도 같은 인증을 통과해야 하며, 모듈별로 Write/Read 권한을 짝지어 부여한다.
...
> 운영 중인 클러스터는 기존 PLAINTEXT 리스너를 둔 채 SASL_SSL 리스너를 추가하는 이중 리스너로 한 서비스씩 전환한다. PLAINTEXT를 닫기 전까지는 롤백이 가능하며, inter-broker 전환은 모든 브로커가 새 리스너를 들은 다음에 한다.
...
> SASL을 켜도 빈 토픽만 보이는 ACL 누락, SCRAM iterations 과소, 리밸런스 중 토큰 만료, mTLS·SASL Principal 충돌, PLAIN+SCRAM 혼용, Schema Registry 인증 누락 — 인증을 켰다는 사실이 안전을 보장하지 않는 함정들이 남는다.
