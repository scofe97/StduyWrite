---
title: 09-02.Saga 엔진 비교
tags: [advanced, saga, orchestration, temporal, cadence, camunda, conductor]
status: draft
related:
  - ../../04_ConsistencyPattern/04-01.Choreography Saga.md
...
> 상위 `05-02.Orchestration Saga`가 Saga의 일반 구조를 다뤘다면, 여기서는 그 Saga 로직을 어디에 두느냐의 선택지를 비교한다. 직접 코드로 구현하는 손코딩 오케스트레이션과 외부 워크플로우 엔진(Temporal, Cadence, Camunda 8, Conductor, AWS Step Functions, Axon Saga, eventuate-tram-sagas) 사이의 트레이드오프를 정리하고, 도입 시그널·마이그레이션 비용을 사례 중심으로 본다.
...
본 학습 트리의 정상 흐름에서 `05-02.Orchestration Saga`를 마쳤다면, 운영 워크플로우의 단계 수와 수명을 분기마다 측정해 본 문서를 다시 읽는다. 도입 검토 시점에는 [09-04.스키마 거버넌스](09-04.스키마%20거버넌스.md)와 함께 — 새 엔진 도입은 곧 새 스키마 진화 정책 도입이다.
