...
- outbox 테이블 자체의 설계는 `[05-03.Outbox](../04_ConsistencyPattern/04-03.Outbox.md)`
- Debezium 설정과 EventRouter는 `[05-06.CDC](../04_ConsistencyPattern/04-06.CDC.md)`
- 컨슈머 측 멱등성과 Inbox 패턴은 `[05-05.Inbox](../04_ConsistencyPattern/04-05.Inbox.md)`
...
같은 토픽에 여러 종류 이벤트가 섞이는 경우 컨슈머가 `event_type`을 보고 분기한다 (§3-1의 switch 패턴). `[05-06.CDC §1-3](../04_ConsistencyPattern/04-06.CDC.md)`의 Envelope 스키마 전략과 결합하면 이 분기가 자연스럽다.

`[05-02.Orchestration Saga](../04_ConsistencyPattern/04-02.Orchestration%20Saga.md)`의 *명령-응답* 모델과는 결이 다르다. Saga 오케스트레이터는 매 단계마다 *명령*을 보내고 *응답 이벤트*를 기다리지만, Temporal에서는 Workflow가 직접 Activity를 호출하고 그 응답을 결정적으로 받는다. 외부 시스템이 이벤트로 응답하는 경우(예: 결제 합의가 별도 콜백으로 옴)에만 Signal로 응답을 주입한다.
...
상세는 `[05-06.CDC §1-1](../04_ConsistencyPattern/04-06.CDC.md)`의 Aggregate 단위 토픽 권장 이유와 같다.
...
상세는 `[02-02.Schema Registry](../01_MessageContract/01-02.Schema%20Registry.md)`와 `[02-06.Avro 스키마 진화 패턴](../01_MessageContract/01-06.Avro%20스키마%20진화%20패턴.md)` 참조.
...
1. **Debezium만 먼저**: 폴링 Outbox를 Debezium CDC로 교체해 발행 지연을 ms 단위로 줄임. Temporal은 도입하지 않음. `[05-06.CDC](../04_ConsistencyPattern/04-06.CDC.md)`의 패턴 그대로.
...
- [05-03.Outbox](../04_ConsistencyPattern/04-03.Outbox.md) — outbox 테이블 설계
- [05-06.CDC](../04_ConsistencyPattern/04-06.CDC.md) — Debezium 설정과 EventRouter
- [05-05.Inbox](../04_ConsistencyPattern/04-05.Inbox.md) — 컨슈머 측 멱등성의 일반 패턴
- [04-08.Exactly-once 의미론과 Consumer Idempotency](../04_ConsistencyPattern/04-09.Exactly-once%20의미론과%20Consumer%20Idempotency.md) — at-least-once 위 멱등 처리
