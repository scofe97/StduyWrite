...
> ⚠️ 본 다이어그램의 `KafkaProducerConfig` 단일 클래스는 옛 구조다. 현재는 `KafkaDefaultsEnvironmentPostProcessor / KafkaErrorConfig / KafkaHeaderMapperConfig / KafkaJsonConsumerConfig / KafkaRetryTemplateConfig` 다섯 클래스로 분리됐다. 분리된 5개 클래스의 책임·기동 순서·상호 작용은 [06-01.message-lib config 5개 클래스 종합](../04_BrokerArchitecture/01-12.message-lib%20config%205개%20클래스%20종합.md).
