---
title: 03-04.KafkaErrorConfig DLT 헤더 폭증 사고
...
`02-05.Avro 직렬화 예외처리 전략`이 "왜 즉시 DLQ로 격리하는가"를 다룬다면, 본 문서는 "그 DLQ 격리 코드가 실제로 어떻게 동작하며, 어떤 가정이 깨지면 시스템이 어떻게 부서지는가"를 다룬다. 한쪽이 전략 일반론이라면 다른 쪽은 운영 디테일과 회고다.
...
생성자의 두 번째 인자(`BiFunction`)는 *어떤 토픽-파티션으로 보낼지*를 결정한다. `Topics.DLQ.getValue()`는 `"tps.v305p.dlq"` 단일 토픽이며, 파티션도 항상 0번이다. 단일 DLQ 정책의 장점과 단점은 `02-05 §3-5`에서 다룬다 — 본 문서는 그 선택을 *이미 한 상태*에서 출발한다.
...
`DeadLetterPublishingRecoverer`는 DLT로 메시지를 보낼 때 기본으로 7종의 헤더를 추가한다(`02-05 §3-5` 표 참조). 그중 `kafka_dlt-exception-stacktrace`가 *풀 스택트레이스 문자열을 통째로* 헤더 값에 넣는다. 일반 도메인 예외는 그 자체로 수십 KB가 되고, 복잡한 프레임워크 스택은 100KB를 넘기기도 한다.
...
> **1차 소스**: Fixed / Exponential / WithMaxRetries 곡선 비교, jitter, 외부 시스템 SLA 기준 곡선 결정 절차는 [03-05.Backoff 전략 비교와 선택](04-14.Backoff%20전략%20비교와%20선택.md).
...
`DeserializationException`이 목록에 있는 이유는 미묘하다. `ErrorHandlingDeserializer`는 역직렬화 실패를 raw bytes + 헤더 형태로 listener까지 전달하는데, Spring Kafka는 이 헤더가 있으면 자동으로 `DeserializationException`을 재발생시킨다(상세는 `02-05 §3-4`). 그 시점에 백오프를 태우면 의미 없이 컨슈머가 멈춰 있으므로 즉시 격리해야 한다.
...
```
retry_topic-attempts: "   0"   (48번째 attempt)
retry_topic-attempts: "  '?"
retry_topic-attempts: "  '?"
retry_topic-attempts: "   1"   (49번째)
retry_topic-attempts: "  '?"
... (수십~수백 회 반복)
```
...
다음 학습 단계로는 [../02-05.Avro 직렬화 예외처리 전략](../02_MessageContract/01-05.Avro%20직렬화%20예외처리%20전략.md)으로 돌아가 "재시도 분류" 결정 기준을 다시 확인하고, [../04-08.Exactly-once 의미론과 Consumer Idempotency](04-09.Exactly-once%20의미론과%20Consumer%20Idempotency.md)의 Inbox 패턴이 `@DltHandler` 무게를 어떻게 덜어주는지 점검한다. 5개 config 클래스 전체의 동작 흐름은 [06-01.message-lib config 5개 클래스 종합](../04_BrokerArchitecture/01-12.message-lib%20config%205개%20클래스%20종합.md)에 정리되어 있다.
