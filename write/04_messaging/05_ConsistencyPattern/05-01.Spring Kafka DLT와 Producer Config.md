---
title: 05-01.Spring Kafka DLT와 Producer Config
tags: []
status: draft
related: []
updated: 2026-04-19
...
> 현재 구조는 **두 방식이 공존**한다. 전역 기본값은 `DefaultErrorHandler + DeadLetterPublishingRecoverer` (`KafkaErrorConfig`) — `@RetryableTopic` 을 명시하지 않은 모든 컨슈머에 자동 적용된다. CICD Command/Result 컨슈머 5개(`executor/engine` 의 `Build`/`Deploy`/`Test`/`Example` + `operator/cicd` 의 `DeployResult`)는 `@RetryableTopic + @DltHandler` 로 토픽별 retry-N / -dlt 체인을 별도 운영한다. 본 문서는 전자(전역 기본값)를 다룬다. 후자의 함정과 사고 회고는 [01-12 §부품 카탈로그](05-03.Kafka%20예외%20처리%20통합.md) 및 [01-13 §4](05-04.KafkaErrorConfig%20DLT%20헤더%20폭증%20사고.md) 참조.
...
flowchart LR
    ...
    C -->|"Topics.DLQ 로 발행"| D["tps.v305p.dlq"]
    D -. "현재 미배포" .-> E["DlqConsumer<br/>(외부 모니터링으로 위임)"]
```

> ⚠️ `DlqConsumer` 는 현재 코드베이스에 **존재하지 않는다**. DLQ 끝단 관측은 외부 모니터링(Grafana, Redpanda Console)으로 위임된 상태다. 상세는 [01-11](05-02.DlqConsumer.md).

- 프로듀서 측은 하나의 `KafkaTemplate<String, byte[]>`를 중심으로 동작한다.

```mermaid
flowchart LR
    A["KafkaProducerConfig (옛 단일 클래스)"] --> B["ProducerFactory<String, byte[]>"]
...
> ⚠️ 본 다이어그램의 `KafkaProducerConfig` 단일 클래스는 옛 구조다. 현재는 `KafkaDefaultsEnvironmentPostProcessor / KafkaErrorConfig / KafkaHeaderMapperConfig / KafkaJsonConsumerConfig / KafkaRetryTemplateConfig` 다섯 클래스로 분리됐다. 분리된 5개 클래스의 책임·기동 순서·상호 작용은 [04-01.message-lib config 5개 클래스 종합](../04_BrokerArchitecture/04-01.message-lib%20config%205개%20클래스%20종합.md).



> **`KafkaErrorConfig` 의 5개 결정 (요약)**: ① `tps.v305p.dlq` 단일 토픽 + 파티션 0 고정, ② `ExponentialBackOff(1s, 2.0)` + maxElapsedTime 7s (약 3회), ③ 재시도 불가 예외 3종(`IllegalArgumentException` / `AvroSerializationException` / `DeserializationException`), ④ `createProducerRecord` 오버라이드로 Avro 객체 byte[] 재직렬화, ⑤ `@AutoConfiguration` 자동 등록. 라인 단위 정독은 [01-13 §2](05-04.KafkaErrorConfig%20DLT%20헤더%20폭증%20사고.md), 통합 가이드는 [01-12](05-03.Kafka%20예외%20처리%20통합.md).
...
4. (옛 구조) `DlqConsumer`가 그 DLQ 메시지를 다시 읽어서 로그를 남긴다. **현재 미배포 — 외부 모니터링으로 위임** ([01-11](05-02.DlqConsumer.md)).
...
1. **Consumer 처리 실패**
   ...
   - (옛 설계) 이후 `DlqConsumer`가 수신했지만 **현재 미배포** — 외부 모니터링이 토픽 lag·헤더로 진단한다.
...
- Consumer 측: Kafka DLQ(`tps.v305p.dlq`)
...
> **1차 소스**: 세 패턴의 *측면별 비교표*는 [01-12 §세 가지 재시도 패턴](05-03.Kafka%20예외%20처리%20통합.md) 이 유지보수되는 정본이다. 본 절(과 아래 §세 패턴의 관계 / §무엇을 선택해야 하는가 / §실무 요약)은 *예시 코드 중심* 설명이며, 표는 03-03 을 우선 본다.
    ...
    public CommonErrorHandler kafkaErrorHandler(KafkaTemplate<String, byte[]> kafkaTemplate) {
        DeadLetterPublishingRecoverer recoverer =
                new DeadLetterPublishingRecoverer(
                        kafkaTemplate,
                        (record, ex) -> new TopicPartition("tps.v305p.dlq", 0)
                );
...
빈 타입이 `CommonErrorHandler`인 점에 주목할 만하다. `CommonErrorHandler`는 Spring Kafka가 단건 리스너와 batch 리스너의 에러 처리를 하나로 묶기 위해 둔 상위 인터페이스이고, `DefaultErrorHandler`는 그 표준 구현체다. 즉 위 코드는 "공통 에러 핸들러 슬롯에 기본 구현체를 꽂은" 형태다. 같은 슬롯에 다른 구현(예: batch 전용 핸들러)을 꽂을 수도 있지만, 단건·batch를 모두 커버하는 `DefaultErrorHandler` 하나면 대개 충분하다. 컨테이너는 이 빈을 자동으로 찾아 모든 리스너에 적용한다.
...
아래는 TPS executor/operator의 실제 Command/Result 컨슈머가 쓰는 형태다 (`BuildCommandConsumer` 등 5개가 같은 골격).
...
@KafkaListener(
        topics = "#{T(org.okestro.tps.messaging.topic.Topics).OPERATOR_CMD_BUILD.getValue()}"
        , groupId = "${executor.consumer-group:executor-group}"
)
public void onCommand(@Payload BuildCommandAvro command) {
    process(command);
}

@DltHandler
public void onDlt(ConsumerRecord<String, BuildCommandAvro> record) {
    log.error("[BuildCmd] DLT received: topic={}, key={}, partition={}, offset={}"
            , record.topic(), record.key(), record.partition(), record.offset());
}
```

실코드와 맞춰 둔 세 가지가 일반 예제와 다르다. `attempts="3"`(원본 1회 + 재시도 2회), `topicSuffixingStrategy = SUFFIX_WITH_INDEX_VALUE`(retry 토픽을 `-retry-0`/`-retry-1` 인덱스로 명명), 그리고 `kafkaTemplate = "retryKafkaTemplate"` 명시다. 마지막 속성을 비우면 안 되는 이유는 바로 다음 절에서 다룬다. DLT 핸들러 인자도 `payload` 단건이 아니라 `ConsumerRecord`로 받아 토픽·파티션·오프셋을 로그에 남긴다.
...
### `kafkaTemplate="retryKafkaTemplate"`를 비우면 안 되는 이유

앞 예시에서 `kafkaTemplate="retryKafkaTemplate"`를 일부러 명시했다. 이걸 비우면 빌드는 통과하지만 운영에서 무한 루프가 난다. 실제로 한 번 겪은 사고다.

`@RetryableTopic`은 컨슈머가 받은 *원본 record value를 그대로* retry/DLT 토픽으로 재발행한다. 그런데 이 컨슈머들은 `KafkaAvroDeserializer + specific.avro.reader=true`로 동작하므로 value가 `SpecificRecord`(Avro 생성 객체)다. `kafkaTemplate`을 비우면 Spring Boot가 기본 등록한 `KafkaTemplate<String, byte[]>`(outbox 발행용, `ByteArraySerializer`)가 선택되는데, `SpecificRecord`를 byte[] 직렬화기에 넣으면 `ClassCastException`이 난다. 재발행이 실패하니 같은 메시지가 다시 retry 사이클에 들어오고, 들어올 때마다 또 ClassCastException — 무한 루프다.

그래서 message-lib는 retry/DLT 발행 전용으로 `KafkaAvroSerializer` 기반 `retryKafkaTemplate`을 따로 만들어 둔다 (`message-lib/.../config/KafkaRetryTemplateConfig.java`, `schema.registry.url`이 설정된 환경에서만 활성화). outbox 경로의 byte[] 템플릿은 그대로 두고, `@RetryableTopic`만 이 Avro 템플릿을 명시 주입한다.

문제는 어노테이션 인자 하나를 빠뜨리는 실수가 코드 리뷰에서 잘 안 보인다는 점이다. 그래서 `RetryableTopicConfigurationGuardTest`가 *빌드 단계*에서 막는다. 이 테스트는 패키지를 스캔해 `@RetryableTopic`이 붙은 모든 메서드가 `kafkaTemplate="retryKafkaTemplate"`을 지정했는지 검사하고, 하나라도 비어 있으면 빌드를 깬다. 사고 회고와 DLT 헤더 폭증의 전말은 [05-04.KafkaErrorConfig DLT 헤더 폭증 사고](05-04.KafkaErrorConfig%20DLT%20헤더%20폭증%20사고.md)에 정리되어 있다.

### `@RetryableTopic` 없을 때의 디폴트 — 10회 재시도

`@RetryableTopic`도, 커스텀 `DefaultErrorHandler`도 두지 않으면 Spring Kafka의 디폴트가 그대로 동작한다. 이 디폴트는 **같은 메시지를 10회 재시도한 뒤 그냥 넘어간다** — DLT를 따로 설정하지 않았다면 그 메시지는 유실된다. ING 발표가 짚은 부분으로, "아무 설정 안 해도 10번은 봐주지만 그 뒤엔 조용히 사라진다"는 디폴트가 운영에서 위험한 이유다.

message-lib는 이 디폴트를 쓰지 않는다. `KafkaErrorConfig`가 `DefaultErrorHandler`를 커스텀해 **3회(1s→2s→4s)** 로 줄이고 최종 실패를 `tps.v305p.dlq`로 보낸다. 즉 "디폴트 10회 + 유실"이 아니라 "3회 + DLQ 격리"다. 디폴트와 다르게 가져간 이유는 외부 시스템(Jenkins/ArgoCD) 호출이 지배적인 도메인이라 10회까지 끌 이유가 없고, 유실 대신 격리가 운영 안전에 맞기 때문이다.

### 재시도 메타데이터 조회 — `KafkaMessageHeaderAccessor`

`@RetryableTopic`이 메시지를 retry 토픽으로 옮길 때, Spring Kafka는 현재 시도 횟수·원본 토픽·예외 메시지·풀 스택트레이스를 Kafka 헤더에 자동으로 채운다. 리스너에서 이 값을 읽고 싶을 때 헤더를 직접 파싱하는 대신 `KafkaMessageHeaderAccessor`를 주입받으면 된다.

```java
@RetryableTopic(attempts = "3", kafkaTemplate = "retryKafkaTemplate")
@KafkaListener(topics = "orders")
public void onMessage(OrderEvent event, KafkaMessageHeaderAccessor headers) {
    // headers 로 현재 시도 횟수·원본 토픽·예외 정보를 헤더 파싱 없이 조회
    log.warn("retry 메타: {}", headers);
    orderService.handle(event);
}
```

`KafkaMessageHeaderAccessor`는 retry/DLT 헤더(시도 횟수, 원본 토픽·파티션, 예외 메시지, 풀 스택트레이스)를 타입 안전하게 꺼내 주는 유틸이다. 헤더 키 문자열을 직접 다루지 않아도 되는 게 핵심이다. 시도 횟수를 알면 "마지막 시도에서만 알림을 보낸다"거나 "N회차부터 로그 레벨을 올린다" 같은 분기를 깔끔하게 짤 수 있다. 이 헤더들이 곧 DLT 메시지에 그대로 실려 가는 값이며, 그 헤더가 사이클마다 *덮어쓰기가 아니라 누적*되어 폭증한 사고가 [01-13](05-04.KafkaErrorConfig%20DLT%20헤더%20폭증%20사고.md)이다. 즉 같은 헤더를 여기서는 읽고, 거기서는 그 누적이 문제가 됐다.
