---
title: 02-01.Outbox
tags: [outbox, transactional-outbox, dual-write, polling, cdc, eventual-consistency]
status: final
source:
  - https://microservices.io/patterns/data/transactional-outbox.html
  - https://debezium.io/blog/2019/02/19/reliable-microservices-data-exchange-with-the-outbox-pattern/
  - https://www.confluent.io/blog/microservices-and-outbox-pattern/
related:
...
> Outbox 패턴은 *DB 트랜잭션*과 *메시지 발행*을 같은 묶음에 넣어 dual-write 문제를 푸는 표준 패턴이다. 도메인 상태 변경과 이벤트 기록이 같은 트랜잭션에서 끝나고, 별도 폴러(또는 CDC)가 outbox 테이블을 읽어 메시지를 발행한다. 부분 실패가 구조적으로 불가능해진다.



## 학습 목표

> Outbox가 *dual-write 문제*를 어떻게 트랜잭션 한 묶음으로 해결하는지로 이해한다.

이 장을 다 읽고 다음 다섯 가지에 자신 있게 답할 수 있으면 학습이 완료된다.

1. Dual-write 문제(도메인 변경 commit 후 메시지 발행 실패)의 본질을 설명할 수 있다.
2. Outbox 테이블의 핵심 컬럼(aggregate_id, payload, status, retry_count, created_at)을 말할 수 있다.
3. 폴링 기반과 CDC 기반 Outbox의 트레이드오프를 비교할 수 있다.
4. `FOR UPDATE SKIP LOCKED`로 다중 폴러 인스턴스가 충돌 없이 작업 분배하는 메커니즘을 설명할 수 있다.
5. PENDING → PROCESSING → SENT/DEAD 상태 전이와 retry 정책을 설명할 수 있다.
...
소비자 측 대칭 패턴인 Inbox와의 비교, 그리고 어느 상황에 어떤 패턴이 맞는지는 [03-01.Inbox](03-01.Inbox.md) 참조.
...
다중 프로세서와 병렬 발행을 도입할 때는 **순서 보장 trade-off**를 인식해야 한다. `SKIP LOCKED`로 여러 워커가 동시에 폴링하면, 같은 aggregate의 이벤트가 서로 다른 워커에 분배되어 발행 순서가 깨질 수 있다. 순서가 중요한 도메인에서는 단일 프로세서를 유지하거나, Consumer 측에서 Inbox 패턴으로 순서를 재정렬해야 한다. 각 최적화 단계의 벤치마크 수치와 심화 내용은 [02-02.Outbox 스케일링](02-02.Outbox%20스케일링.md)에서 다룬다.
...
> - **차이점**: 학습 문서의 CDC 기반 Outbox 대신 폴링 채택(인프라 단순화). 변종 비교는 [`08_advanced/01_variants/01-01.Outbox 변종 비교`](../08_advanced/01_variants/01-01.Outbox%20변종%20비교.md).
...
### Q1. Dual-write 문제가 일어나는 정확한 시점은?

도메인 변경을 DB에 commit한 후, 같은 트랜잭션 밖에서 Kafka 발행을 시도할 때다. commit은 성공했지만 Kafka가 일시 단절이라면 *데이터는 바뀌었는데 다른 시스템은 모르는 상태*가 영원히 남는다. 반대로 Kafka에 먼저 보낸 뒤 DB에 commit하려는 경우, Kafka 발행은 성공했지만 DB commit이 실패하면 *존재하지 않는 사건*이 토픽에 떠 있게 된다. Outbox는 이 두 자원의 비원자성을 *DB 한 자원*으로 줄여 해결한다.

### Q2. Outbox 폴러가 PENDING 행을 어떻게 잠그나?

`SELECT ... FOR UPDATE SKIP LOCKED`로 다중 폴러 인스턴스가 동시에 작업을 분배한다. 한 폴러가 잠근 행은 다른 폴러가 SKIP하므로 락 충돌 없이 병렬 처리가 된다. 잠근 행은 PENDING → PROCESSING으로 상태를 바꾸고, Kafka 발행 성공 시 SENT로, 실패 시 retry_count 증가 후 PENDING으로 되돌린다. retry_count가 임계치를 넘으면 DEAD로 표시한다.

### Q3. 폴링 기반과 CDC 기반 Outbox 중 무엇을 골라야 하나?

폴링은 인프라가 단순하다(앱 + DB만 있으면 됨). 500ms 폴링이라면 그만큼 지연이 누적되지만 보통 도메인에는 충분하다. CDC는 *binlog/WAL를 직접 읽어* ms 단위 지연을 달성하지만, Debezium + Kafka Connect 같은 별도 인프라를 운영해야 한다. 처리량이 폴러 단일 인스턴스 한계(수천 MPS)에 닿거나 지연이 ms 단위로 필요할 때 CDC를 검토한다.

### Q4. Outbox row를 SENT 후에도 보관해야 하나?

세 가지 이유로 *일정 기간 보관*한다. 첫째, 디버깅·재처리 시 *어떤 메시지가 언제 발행됐는지* 추적하기 위해서다. 둘째, *동일 message_id 중복 발행 감지*에 쓸 수 있다. 셋째, 폴링 retry와 메시지 순서 검증의 기준이 된다. 다만 영원히 두면 테이블이 커지므로, 보통 *N일 후 자동 archive*(별도 테이블로 이동 또는 삭제) 작업을 둔다.

### Q5. Outbox는 SAGA·CDC와 어떻게 결합되나?

SAGA의 Command/Event 발행은 *반드시 Outbox를 거쳐야* 부분 실패가 없다. Orchestrator가 SagaInstance를 갱신하고 명령 발행을 Outbox에 적재하는 것이 같은 트랜잭션이어야 한다. CDC와는 두 갈래다. (1) CDC가 Outbox 테이블을 직접 읽어 발행(폴러 대체), (2) CDC가 *비즈니스 테이블*을 읽어 발행하면 Outbox가 필요 없는 것처럼 보이지만 *발행 의도와 도메인 의미가 섞이는* 문제가 생긴다. 대부분 (1)이 깔끔하다.



## 관련 문서

- [02-02.Outbox 스케일링](02-02.Outbox%20스케일링.md) — 폴러 다중 인스턴스·shard 전략
- [03-01.Inbox](03-01.Inbox.md) — 수신 측 짝 패턴
- [04-01.CDC](04-01.CDC.md) — CDC 기반 발행 대안
- [03-04.Exactly-once 의미론과 Consumer Idempotency](03-04.Exactly-once%20의미론과%20Consumer%20Idempotency.md) — Outbox+Inbox로 만드는 effectively-once
