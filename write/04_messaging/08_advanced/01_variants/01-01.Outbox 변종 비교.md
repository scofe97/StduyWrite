...
상위 `02-01.Outbox`가 폴링 Relay 기반 구현을 다뤘다면, 여기서는 그 외 선택지를 운영 시점에서 비교한다.
...
상위 `02-01.Outbox`에 자세히 다룬 형태. `@Scheduled(fixedDelay = N)`이 outbox 테이블에서 PENDING 행을 `FOR UPDATE SKIP LOCKED`로 클레임 → Kafka 발행 → 상태 업데이트.
...
본 학습 트리의 정상 흐름에서 `02-01.Outbox`를 마쳤다면, 도입 후 운영 시그널을 수집하기 시작한 시점에 본 문서를 다시 읽으면 된다. 도입 시 검증·메트릭 설계는 [01-04.스키마 거버넌스](01-04.스키마%20거버넌스.md)와 함께 본다.
