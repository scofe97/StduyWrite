...
다음 학습 단계로는 [../02-05.Avro 직렬화 예외처리 전략](../01_MessageContract/01-05.Avro%20직렬화%20예외처리%20전략.md)으로 돌아가 "재시도 분류" 결정 기준을 다시 확인하고, [../04-08.Exactly-once 의미론과 Consumer Idempotency](04-09.Exactly-once%20의미론과%20Consumer%20Idempotency.md)의 Inbox 패턴이 `@DltHandler` 무게를 어떻게 덜어주는지 점검한다. 5개 config 클래스 전체의 동작 흐름은 [06-01.message-lib config 5개 클래스 종합](../03_BrokerArchitecture/03-12.message-lib%20config%205개%20클래스%20종합.md)에 정리되어 있다.
