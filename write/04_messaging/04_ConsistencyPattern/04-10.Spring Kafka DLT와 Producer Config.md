---
title: 03-01.Spring Kafka DLT와 Producer Config
tags: []
status: draft
related: []
updated: 2026-04-19
...
> 현재 구조는 **두 방식이 공존**한다. 전역 기본값은 `DefaultErrorHandler + DeadLetterPublishingRecoverer` (`KafkaErrorConfig`) — `@RetryableTopic` 을 명시하지 않은 모든 컨슈머에 자동 적용된다. CICD Command/Result 컨슈머 5개(`executor/engine` 의 `Build`/`Deploy`/`Test`/`Example` + `operator/cicd` 의 `DeployResult`)는 `@RetryableTopic + @DltHandler` 로 토픽별 retry-N / -dlt 체인을 별도 운영한다. 본 문서는 전자(전역 기본값)를 다룬다. 후자의 함정과 사고 회고는 [03-03 §부품 카탈로그](04-12.Kafka%20예외%20처리%20통합.md) 및 [03-04 §4](04-13.KafkaErrorConfig%20DLT%20헤더%20폭증%20사고.md) 참조.
...
> ⚠️ `DlqConsumer` 는 현재 코드베이스에 **존재하지 않는다**. DLQ 끝단 관측은 외부 모니터링(Grafana, Redpanda Console)으로 위임된 상태다. 상세는 [03-02](04-11.DlqConsumer.md).
...
> ⚠️ 본 다이어그램의 `KafkaProducerConfig` 단일 클래스는 옛 구조다. 현재는 `KafkaDefaultsEnvironmentPostProcessor / KafkaErrorConfig / KafkaHeaderMapperConfig / KafkaJsonConsumerConfig / KafkaRetryTemplateConfig` 다섯 클래스로 분리됐다. 분리된 5개 클래스의 책임·기동 순서·상호 작용은 [06-01.message-lib config 5개 클래스 종합](../03_BrokerArchitecture/03-12.message-lib%20config%205개%20클래스%20종합.md).



> **`KafkaErrorConfig` 의 5개 결정 (요약)**: ① `tps.v305p.dlq` 단일 토픽 + 파티션 0 고정, ② `ExponentialBackOff(1s, 2.0)` + maxElapsedTime 7s (약 3회), ③ 재시도 불가 예외 3종(`IllegalArgumentException` / `AvroSerializationException` / `DeserializationException`), ④ `createProducerRecord` 오버라이드로 Avro 객체 byte[] 재직렬화, ⑤ `@AutoConfiguration` 자동 등록. 라인 단위 정독은 [03-04 §2](04-13.KafkaErrorConfig%20DLT%20헤더%20폭증%20사고.md), 통합 가이드는 [03-03](04-12.Kafka%20예외%20처리%20통합.md).
...
4. (옛 구조) `DlqConsumer`가 그 DLQ 메시지를 다시 읽어서 로그를 남긴다. **현재 미배포 — 외부 모니터링으로 위임** ([03-02](04-11.DlqConsumer.md)).
...
> **1차 소스**: 세 패턴의 *측면별 비교표*는 [03-03 §세 가지 재시도 패턴](04-12.Kafka%20예외%20처리%20통합.md) 이 유지보수되는 정본이다. 본 절(과 아래 §세 패턴의 관계 / §무엇을 선택해야 하는가 / §실무 요약)은 *예시 코드 중심* 설명이며, 표는 03-03 을 우선 본다.
