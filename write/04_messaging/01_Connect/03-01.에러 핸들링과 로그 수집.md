...
> `retry`/`fallback`/`switch` 문법 자체는 [02-redpanda-connect.md §3 "Output Wrapper"](./02-02.Redpanda%20Connect%20문법.md)에서 다루었다. 여기서는 HTTP 응답 코드 분기에 집중한다.
...
> 메트릭 기반 모니터링(Prometheus, Grafana, rpk 메트릭 수집)은 04-operations §4 "모니터링과 트러블슈팅" (별도 문서 미작성)에서 다루었다. 여기서는 **로그** 수집 파이프라인만 다룬다.
...
- [02-redpanda-connect.md §3 — Output Wrapper (retry/fallback/switch)](./02-02.Redpanda%20Connect%20문법.md)
