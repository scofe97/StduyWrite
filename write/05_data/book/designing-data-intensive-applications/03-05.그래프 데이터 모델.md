...
이 노트를 읽고 나면 다음을 할 수 있습니다.

- 속성 그래프와 트리플 스토어를 같은 발상의 다른 표현으로 설명합니다.
- Cypher·SPARQL·Datalog 같은 선언형 그래프 쿼리가 SQL 재귀 CTE보다 왜 간결한지 말합니다.
- GraphQL이 왜 의도적으로 제한적인지 설명합니다.

이 노트는 3장의 그래프 데이터 모델을 다룹니다. 데이터에 다대다 관계가 흔하고 연결이 복잡해지면 데이터를 **그래프** 로 모델링하는 것이 더 자연스러워집니다. 관계형이 단순한 다대다는 다룰 수 있어도 그렇습니다. 그래프는 두 종류의 객체로 이뤄집니다. **정점**(vertex, 노드·엔티티)과 **간선**(edge, 관계·아크)입니다.

전형적 예로 소셜 그래프, 웹 그래프, 도로망이 있습니다. 각각 사람과 교우, 페이지와 링크, 교차로와 도로가 정점·간선이 됩니다. 이 위에서 최단 경로·PageRank 같은 알고리즘이 동작합니다.

이 노트는 두 가지 그래프 모델을 따라갑니다. Neo4j·Memgraph·KùzuDB의 **속성 그래프 모델**과 Datomic·AllegroGraph의 **트리플 스토어 모델**입니다. 이어서 네 가지 그래프 쿼리 언어(Cypher·SPARQL·Datalog·GraphQL)와 SQL의 그래프 쿼리 지원을 봅니다. 실행 예는 미국 아이다호 출신 Lucy와 프랑스 생로 출신 Alain이 런던에 사는 그래프입니다.
...
**속성 그래프**(property graph, labeled property graph) 모델에서 정점과 간선은 각각 다음으로 이뤄집니다.

- **정점** — 고유 식별자, 객체 유형을 기술하는 label(문자열), 나가는 간선 집합, 들어오는 간선 집합, 속성(key-value) 모음
- **간선** — 고유 식별자, 시작 정점(꼬리), 끝 정점(머리), 관계 종류를 기술하는 label, 속성 모음
...
그래프는 데이터 모델링에 큰 유연성을 줍니다. 전통적 관계형 스키마로 표현하기 어려운 것들을 담을 수 있습니다.

- 나라마다 다른 지역 구조 — 프랑스의 département·région 대 미국의 county·state
- 나라 안의 나라 같은 역사적 특이성
- 데이터 입도의 차이 — Lucy의 현 거주지는 도시 단위, 출생지는 주 단위

**그래프는 진화에 좋습니다.** 애플리케이션에 기능을 더할 때 데이터 구조 변화를 쉽게 수용하기 때문입니다. 예를 들어 식품 알레르기를 표현하려면 알레르겐 정점과 사람-알레르겐 간선을 추가합니다. 그다음 알레르겐을 어떤 음식이 어떤 물질을 함유하는지 보이는 정점들과 연결하면, 각 사람이 무엇을 안전하게 먹을 수 있는지 쿼리할 수 있습니다.
...
> **원문 정오**: 원문 예제는 `in_usa` 절에서 `label = 'Location'`(대문자), `in_europe` 절에서 `label = 'location'`(소문자)로 대소문자가 엇갈립니다. `label` 이 `text` 컬럼이므로 실제로는 한쪽이 매칭되지 않습니다. 위 코드는 원문의 `in_usa` 쪽 표기를 그대로 옮겼습니다.
...
**트리플 스토어**(triple store) 모델은 속성 그래프 모델과 대체로 동등하며 같은 발상을 다른 말로 기술합니다. 모든 정보가 단순한 세 부분 문장 **(주어, 술어, 목적어)** 로 저장됩니다. Jim, likes, bananas 라는 트리플이라면 Jim 이 주어, likes 가 술어, bananas 가 목적어입니다.

트리플의 주어는 그래프의 정점에 해당합니다. 목적어는 둘 중 하나입니다.

- **원시 타입 값** — 이 경우 술어·목적어가 정점 속성의 키·값이 됩니다.
- **다른 정점** — 이 경우 술어가 간선, 주어가 꼬리, 목적어가 머리가 됩니다.
...
트리플 스토어 연구·개발의 일부는 **시맨틱 웹(Semantic Web)** 에서 동기를 얻었습니다. 데이터를 사람이 읽는 웹 페이지뿐 아니라 표준화된 기계 판독 형식으로도 발행해, 인터넷 전반의 데이터 교환을 촉진하려던 2000년대 초의 노력입니다. 원래 구상대로 성공하진 못했지만 유산이 남았습니다. JSON-LD, 생의학 온톨로지, 페이스북 Open Graph 프로토콜, Wikidata, Schema.org 가 그것입니다.

**RDF**(Resource Description Framework)는 시맨틱 웹용으로 설계된 데이터 모델이고 Turtle 은 그 인코딩 방법입니다. RDF 는 인터넷 전반 교환을 위해 주어·술어·목적어를 흔히 URI 로 씁니다. 남의 데이터와 합칠 때 within·lives_in 에 다른 의미를 붙여도, 서로 다른 URI 라 충돌하지 않게 하기 위함입니다.
...
**Datalog** 는 SPARQL·Cypher보다 훨씬 오래된 언어로, 1980년대 학술 연구에서 나왔습니다. 소프트웨어 엔지니어에게 덜 알려졌고 주류 데이터베이스 지원도 적습니다. 그럼에도 표현력이 뛰어나고 복잡한 쿼리에 강력해 더 알려질 만합니다. Datomic·LogicBlox·CozoDB 가 씁니다. 그래프가 아닌 관계형 데이터 모델에 기반하지만, 그래프의 재귀 쿼리가 Datalog 의 특별한 강점입니다.
...
Cypher·SPARQL 이 곧장 SELECT 로 들어가는 것과 달리 Datalog 는 한 번에 작은 걸음을 뗍니다. 기본 사실에서 새 가상 테이블을 파생하는 규칙을 정의합니다. 이 파생 테이블은 가상 SQL 뷰와 같습니다. 저장되진 않지만 저장된 사실처럼 쿼리할 수 있습니다.

규칙의 `:-` 앞부분이 가상 테이블 이름·컬럼을 정하고 뒷부분이 어떤 패턴에 맞는 행을 찾는지 정합니다. 규칙은 함수처럼 다른 규칙을 참조할 수 있습니다. 함수가 재귀하듯 규칙 2처럼 자기 자신을 호출할 수도 있어 그래프 순회가 가능해집니다.

이 접근은 다른 언어와 다른 사고를 요구합니다. 그 대신 복잡한 쿼리를 규칙 단위로 쌓아 올리게 합니다.
...
응답은 쿼리 구조를 그대로 비추는 JSON 문서로, 요청된 속성만 정확히 담습니다. 서버는 클라이언트가 어떤 속성을 필요로 하는지 알 필요가 없습니다. 클라이언트가 필요한 것을 직접 요청하기 때문입니다.

같은 사용자가 여러 메시지를 보내면 sender 정보가 메시지마다 반복됩니다. GraphQL 은 응답 크기가 커지더라도 UI 렌더링을 단순하게 만드는 설계 선택을 합니다. 서버 데이터베이스는 데이터를 더 정규화된 형태로 저장하고 쿼리 처리 시 필요한 조인을 수행할 수 있지만, GraphQL 스키마에 명시적으로 선언된 조인만 클라이언트가 요청할 수 있습니다. 응답이 문서 데이터베이스 응답과 비슷해 보이고 이름에 "graph"가 들어가도, GraphQL은 관계형·문서·그래프 어느 유형의 데이터베이스 위에도 구현할 수 있습니다.
...
- [03-04 모델 선택과 스키마 유연성](03-04.%EB%AA%A8%EB%8D%B8%20%EC%84%A0%ED%83%9D%EA%B3%BC%20%EC%8A%A4%ED%82%A4%EB%A7%88%20%EC%9C%A0%EC%97%B0%EC%84%B1.md) § "언제 어느 모델을 쓰는가" — 다대다가 복잡해지면 그래프로 가는 흐름
- [03-06 이벤트 소싱·CQRS·DataFrame](03-06.%EC%9D%B4%EB%B2%A4%ED%8A%B8%20%EC%86%8C%EC%8B%B1%C2%B7CQRS%C2%B7DataFrame.md) — 쓰기 최적화 표현(이벤트 로그)이라는 또 다른 모델로 연결
