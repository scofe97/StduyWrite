...
정렬 키가 수십 개로 늘어나 `switch` 분기가 부담스러워지면 `PathBuilder.get(sortKey)`로 키-경로 매핑을 한 줄로 줄이는 변형이 있다. 다만 문자열 기반 접근이므로 *화이트리스트 검증이 더 중요해진다* — 컴파일러가 더 이상 컬럼명을 검증해 주지 않기 때문이다. PathBuilder 의 *정의·트레이드오프* 자체는 [02-01.PathBuilder — 동적 path 빌더 깊이](02-01.PathBuilder%20%E2%80%94%20%EB%8F%99%EC%A0%81%20path%20%EB%B9%8C%EB%8D%94%20%EA%B9%8A%EC%9D%B4.md) 에서 다루고, 운영 코드의 실무 응용은 [03-03. 실무 변형 모음](03-03.%EC%8B%A4%EB%AC%B4%20%EB%B3%80%ED%98%95%20%EB%AA%A8%EC%9D%8C.md) 이 짚는다.
...
- [01-03. 기본 문법과 조인](01-03.%EA%B8%B0%EB%B3%B8%20%EB%AC%B8%EB%B2%95%EA%B3%BC%20%EC%A1%B0%EC%9D%B8.md) — `where` 가변 인자의 기초
- [01-05. 프로젝션과 DTO 매핑](01-05.%ED%94%84%EB%A1%9C%EC%A0%9D%EC%85%98%EA%B3%BC%20DTO%20%EB%A7%A4%ED%95%91.md) — 검색 결과를 DTO로 받는 방법
- [jpa/03-05. 커스텀 리포지토리 패턴](../jpa/03-05.%EC%BB%A4%EC%8A%A4%ED%85%80%20%EB%A6%AC%ED%8F%AC%EC%A7%80%ED%86%A0%EB%A6%AC%20%ED%8C%A8%ED%84%B4.md) — 검색 메서드를 Spring Data JPA에 통합
