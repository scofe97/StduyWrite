...
## Spring Data가 제공하는 QueryDSL 기능의 한계

> Spring Data JPA가 박스 안에 제공하는 `QuerydslPredicateExecutor`·`QuerydslRepositorySupport` 같은 통합은 강의에서 *알아만 두라* 고 말한다. 본 묶음의 표준 패턴 (`JPAQueryFactory` + 사용자 정의 리포지토리) 으로 굳이 가는 이유를 정리한다.

### QuerydslPredicateExecutor

리포지토리에 다음을 끼우면 `Predicate`를 받는 메서드 묶음이 생긴다.
...
1. **명시적 조인이 불가능하다.** 묵시적 조인까지만 된다. `left join`을 표현할 길이 없어 실무에서 자주 막힌다.
2. **서비스 계층이 QueryDSL에 의존하게 된다.** `Predicate`를 만들려면 Q 클래스를 import해야 하므로, 호출자(서비스)가 QueryDSL 구현 기술에 묶인다.
3. **복합 동적 조건이 표현되지 않는다.** `BooleanBuilder`나 `BooleanExpression` 메서드 분해 패턴을 그대로 옮기기 어렵다.

세 한계 때문에 검색 페이지 수준의 *복합 동적 쿼리* 가 바로 막힌다. 새 프로젝트라면 시작부터 `JPAQueryFactory` + `RepositoryCustom` + `RepositoryImpl` 조합으로 간다.

### QuerydslRepositorySupport

`EntityManager`를 받아 `from()` 헬퍼를 노출하던 옛 패턴이다. 본 묶음의 [jpa/03-05 §QuerydslRepositorySupport 사용 여부](../jpa/03-05.%EC%BB%A4%EC%8A%A4%ED%85%80%20%EB%A6%AC%ED%8F%AC%EC%A7%80%ED%86%A0%EB%A6%AC%20%ED%8C%A8%ED%84%B4.md)에 다룬 세 가지 이유로 *사실상 안 쓴다*.

요약하면 `JPAQueryFactory` API 일관성, `PageableExecutionUtils.getPage`의 등장, EntityManager setter 주입 함정 — 이 셋이 합쳐져 6.x 시대에는 `JPAQueryFactory` 표준 하나로 정리된다.

### 그래서 본 묶음의 표준

- 검색·정렬·페이징은 `JPAQueryFactory` + 사용자 정의 리포지토리 ([jpa/03-05](../jpa/03-05.%EC%BB%A4%EC%8A%A4%ED%85%80%20%EB%A6%AC%ED%8F%AC%EC%A7%80%ED%86%A0%EB%A6%AC%20%ED%8C%A8%ED%84%B4.md))로 해결한다.
- 카운트 최적화는 `PageableExecutionUtils.getPage` ([01-06 §PageableExecutionUtils로 카운트 최적화](01-06.%ED%8E%98%EC%9D%B4%EC%A7%95%EA%B3%BC%20fetch%20join%20%ED%95%A8%EC%A0%95.md))로 해결한다.
- `QuerydslPredicateExecutor`·`QuerydslRepositorySupport`는 *옛 코드를 만났을 때 정체를 안다* 수준이면 충분하다.
...
- [01-01. QueryDSL 입문과 6.12의 위치](01-01.QueryDSL%20%EC%9E%85%EB%AC%B8%EA%B3%BC%206.12%EC%9D%98%20%EC%9C%84%EC%B9%98.md) — 거버넌스 출발점
- [01-02. 프로젝트 셋업 (Gradle 6.12)](01-02.%ED%94%84%EB%A1%9C%EC%A0%9D%ED%8A%B8%20%EC%85%8B%EC%97%85%20(Gradle%206.12).md) — 좌표 변경의 실제 코드
