...
- 7.x 에서 추가 개선이 들어왔으며 자세한 차이는 [03-02. 대안 비교와 6.12→7.x 마이그레이션](03-02.%EB%8C%80%EC%95%88%20%EB%B9%84%EA%B5%90%EC%99%80%206.12-7.x%20%EB%A7%88%EC%9D%B4%EA%B7%B8%EB%A0%88%EC%9D%B4%EC%85%98.md) 에서 다룬다.
...
> 위 `orderBy` 의 `new CaseBuilder().when(...).then(0).otherwise(1)` 은 *조건부 정렬 키* 를 만드는 QueryDSL 의 CASE 표현식 빌더다 — 여기선 "지정 결재(`ordrDsgnYn='Y'`) 를 먼저(0), 나머지는 나중에(1)" 정렬한다. CaseBuilder 의 문법과 NULLS LAST 응용은 [02-03 § NULLS LAST — 형태 2 CASE 기반](02-03.%EC%A0%95%EB%A0%AC%C2%B7%EC%A7%91%EA%B3%84%C2%B7%ED%94%84%EB%A1%9C%EC%A0%9D%EC%85%98%20%EB%B3%B4%EC%B6%A9.md) 이 다룬다.
...
- `ORDER BY ... LIMIT 1` 의 평탄화로 윈도 함수 한 의미를 흉내낸다. 본 패턴이 *왜 윈도 함수의 대체* 가 되는지는 [03-06](03-05.window%20%ED%95%A8%EC%88%98%20%EC%97%86%EB%8A%94%20JPA%20QueryDSL%EC%9D%98%20ROW_NUMBER%20%EB%8C%80%EC%B2%B4.md) 가 한 케이스 통째로 분해한다.
...
- [01-03. 기본 문법과 조인](01-03.%EA%B8%B0%EB%B3%B8%20%EB%AC%B8%EB%B2%95%EA%B3%BC%20%EC%A1%B0%EC%9D%B8.md) — 메인 쿼리의 `JPAQueryFactory` 진입점과 비교
...
- [03-02. 대안 비교와 6.12→7.x 마이그레이션](03-02.%EB%8C%80%EC%95%88%20%EB%B9%84%EA%B5%90%EC%99%80%206.12-7.x%20%EB%A7%88%EC%9D%B4%EA%B7%B8%EB%A0%88%EC%9D%B4%EC%85%98.md) — sub-query LIMIT 의 7.x 개선 차이
- [03-03. 실무 변형 모음](03-03.%EC%8B%A4%EB%AC%B4%20%EB%B3%80%ED%98%95%20%EB%AA%A8%EC%9D%8C.md) § "상관 서브쿼리" — 상관 서브쿼리 응용 패턴
- [03-05. window 함수 없는 JPA QueryDSL의 ROW_NUMBER 대체](03-05.window%20%ED%95%A8%EC%88%98%20%EC%97%86%EB%8A%94%20JPA%20QueryDSL%EC%9D%98%20ROW_NUMBER%20%EB%8C%80%EC%B2%B4.md) — 스칼라 서브쿼리 + ORDER BY + LIMIT 1 의 한 케이스 통째 분해
