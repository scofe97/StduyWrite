---
title: 05_data/03_persistence/querydsl — QueryDSL 학습 MOC
...
# 05_data/03_persistence/querydsl — QueryDSL 학습 MOC
...
> 커스텀 리포지토리 패턴(`RepositoryCustom` + `Impl`)은 Spring Data JPA 주제라 [`jpa/03-05`](../jpa/03-05.%EC%BB%A4%EC%8A%A4%ED%85%80%20%EB%A6%AC%ED%8F%AC%EC%A7%80%ED%86%A0%EB%A6%AC%20%ED%8C%A8%ED%84%B4.md) 로 분리해 두었다.

| # | 문서 | 다루는 핵심 |
|---|------|-----------|
| 01-01 | [입문과 6.12의 위치](01-01.QueryDSL%20%EC%9E%85%EB%AC%B8%EA%B3%BC%206.12%EC%9D%98%20%EC%9C%84%EC%B9%98.md) | 등장 동기, 원조 archived 사실, 6.12 vs 7.x 선택 |
| 01-02 | [프로젝트 셋업 (Gradle 6.12)](01-02.%ED%94%84%EB%A1%9C%EC%A0%9D%ED%8A%B8%20%EC%85%8B%EC%97%85%20(Gradle%206.12).md) | `build.gradle`, annotationProcessor, Q클래스 경로, `JPAQueryFactory` 빈 |
| 01-03 | [기본 문법과 조인](01-03.%EA%B8%B0%EB%B3%B8%20%EB%AC%B8%EB%B2%95%EA%B3%BC%20%EC%A1%B0%EC%9D%B8.md) | `select/from/where/join/fetchJoin`, `Expressions` |
| 01-04 | [동적 쿼리](01-04.%EB%8F%99%EC%A0%81%20%EC%BF%BC%EB%A6%AC.md) | `BooleanBuilder` vs `BooleanExpression`, null 안전성 |
| 01-05 | [프로젝션과 DTO 매핑](01-05.%ED%94%84%EB%A1%9C%EC%A0%9D%EC%85%98%EA%B3%BC%20DTO%20%EB%A7%A4%ED%95%91.md) | `Tuple`, `Projections.*`, `@QueryProjection` 트레이드오프 |
| 01-06 | [페이징과 fetch join 함정](01-06.%ED%8E%98%EC%9D%B4%EC%A7%95%EA%B3%BC%20fetch%20join%20%ED%95%A8%EC%A0%95.md) | `HHH000104`, `distinct`, 카운트 분리, `@BatchSize` |
| 01-07 | [벌크 연산과 SQL 함수](01-07.%EB%B2%8C%ED%81%AC%20%EC%97%B0%EC%82%B0%EA%B3%BC%20SQL%20%ED%95%A8%EC%88%98.md) | `update().execute()`/`delete().execute()`, 영속성 컨텍스트 우회, `Expressions.stringTemplate` |
...
| 03-01 | [테스트와 멀티모듈](03-01.%ED%85%8C%EC%8A%A4%ED%8A%B8%EC%99%80%20%EB%A9%80%ED%8B%B0%EB%AA%A8%EB%93%88.md) | `@DataJpaTest` + `JPAQueryFactory`, Q클래스 가시성 |
| 03-02 | [대안 비교와 6.12→7.x 마이그레이션](03-02.%EB%8C%80%EC%95%88%20%EB%B9%84%EA%B5%90%EC%99%80%206.12-7.x%20%EB%A7%88%EC%9D%B4%EA%B7%B8%EB%A0%88%EC%9D%B4%EC%85%98.md) | MyBatis/JOOQ 비교, OpenFeign 좌표 이전, 7.x 차이 |
| 03-03 | [실무 변형 모음](03-03.%EC%8B%A4%EB%AC%B4%20%EB%B3%80%ED%98%95%20%EB%AA%A8%EC%9D%8C.md) | PathBuilder·Embedded ID·상관 서브쿼리·JPQL limit 한계·동적 검색 추상 베이스·`nullExpression`, **Hooks 4 분할·ThreadLocal userId 캡처·BooleanBuilder.or() 가상값 누적** (02-04 에서 이동) |
| 03-04 | [락과 동시성 제어](03-04.%EB%9D%BD%EA%B3%BC%20%EB%8F%99%EC%8B%9C%EC%84%B1%20%EC%A0%9C%EC%96%B4.md) | `@Version` 낙관, `PESSIMISTIC_WRITE/READ`, `setLockMode` ↔ `@Lock`, 데드락 방어 |
| 03-05 | [window 함수 없는 JPA QueryDSL의 ROW_NUMBER 대체](03-05.window%20%ED%95%A8%EC%88%98%20%EC%97%86%EB%8A%94%20JPA%20QueryDSL%EC%9D%98%20ROW_NUMBER%20%EB%8C%80%EC%B2%B4.md) | 상관 sub-select + `CaseBuilder` 정렬로 `ROW_NUMBER`/`PARTITION BY` 의미 동등 표현, group by 이식성 함정 |
| 03-06 | [스프링 데이터 페이징 통합](03-06.%EC%8A%A4%ED%94%84%EB%A7%81%20%EB%8D%B0%EC%9D%B4%ED%84%B0%20%ED%8E%98%EC%9D%B4%EC%A7%95%20%ED%86%B5%ED%95%A9.md) | `JpaRepository` + `MemberRepositoryCustom` + `Impl` 합치기, 분리 카운트 패턴, `PageableExecutionUtils.getPage` |
