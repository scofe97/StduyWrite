...
- 리포지토리 단위 검증에는 `@DataJpaTest` 가 *5~10배 빠른* 피드백을 준다. 
- 트레이드오프는 *JPAQueryFactory 빈이 자동 로드되지 않는다* 는 점 — 다음 절에서 보완한다.
...
- `@DataJpaTest`가 자동으로 트랜잭션을 걸고 테스트 후 롤백하므로 데이터가 누적되지 않는다. 
- `em.flush()` + `em.clear()` 한 번 끼워 영속성 컨텍스트 캐시가 결과에 영향을 주지 않게 만든다.
...
## annotationProcessor 가 Q클래스를 만드는 메커니즘

> 멀티모듈 절로 들어가기 전에 Q클래스가 *어디서·언제* 생기는지를 한 단락 먼저 박아 둡니다. 이 메커니즘을 알면 뒤이은 4단계 점검 순서가 자연스럽게 따라옵니다.

`@Entity` 가 붙은 클래스를 만나면 컴파일 단계에서 `annotationProcessor "querydsl-apt:...:jakarta"` 가 같은 패키지에 `QMember.java` 같은 **새 .java 파일을 직접 생성** 합니다. 생성 위치는 기본값으로 `build/generated/sources/annotationProcessor/java/main/` 입니다. 이 새 파일은 같은 모듈의 `sourceSets.main.java` 에 자동으로 포함되어 *원본 소스와 함께 컴파일* 되고, 최종적으로 `domain.jar` 안에 `QMember.class` 형태로 들어갑니다. 즉 Q클래스는 *내가 작성하지 않은, 그러나 모듈 산출물에 정상 포함된 .class* 입니다.

Lombok 과의 차이가 헷갈리면 한 줄로 박아 둡니다. Lombok 은 *기존 클래스 안에 메서드를 주입* 하므로 결과가 같은 .class 한 개로 끝나지만, QueryDSL 은 *완전히 새 클래스 파일* 을 만들어 별도 .class 로 떨어집니다. 그래서 Q클래스는 *생성 후 노출* 이라는 단계 하나가 더 필요하고, 멀티모듈에서 함정이 생깁니다 — 같은 모듈 안이면 sourceSet 통합으로 자동 해결되지만, 다른 모듈에서 보려면 *jar 안 .class* 가 의존 모듈의 컴파일 클래스패스에 들어와야 합니다.

`annotationProcessor` 키워드 자체와 `implementation`·`compileOnly` 등 다른 Gradle 의존 키워드의 *분리된 classpath* 의미는 [`../../01_language/java/06_Build/02-01.Gradle 의존성 키워드.md`](../../../01_language/java/06_Build/02-01.Gradle 의존성 키워드.md) 의 §6~7 에 카탈로그로 정리돼 있습니다. 본 챕터는 그 토대 위에서 *QueryDSL 한정* 관점만 짚습니다.
...
> 4개 질문에 *먼저 스스로 답해 보세요.* 자답이 끝나면 아래 §정답 (자답 후 펼치기) 으로 내려갑니다.
...
### 정답 1 — `@DataJpaTest` 만으로 QueryDSL 테스트가 되는가

`@DataJpaTest` 는 JPA 슬라이스만 올리므로 `EntityManager`·`JpaRepository`·`DataSource` 같은 빈은 자동 등록하지만, `JPAQueryFactory` 는 QueryDSL 라이브러리가 제공하는 *애플리케이션 빈* 이라 슬라이스 후보에 들어 있지 않습니다. 따라서 슬라이스 안에서 `@Autowired JPAQueryFactory` 가 비어 NPE 가 떨어집니다. 해결책은 두 가지가 있고 본 챕터 노트에서 모두 다룹니다 — `@TestConfiguration` 로 테스트 전용 빈을 새로 등록하거나, 운영의 `QueryDslConfig` 를 그대로 `@Import` 해 재사용합니다. 후자가 설정이 단일한 경우 중복이 없고, 전자는 테스트 전용 빈을 따로 두고 싶을 때 의도가 명확해집니다. 본 챕터 실습 `Ch08_TestSetupTest` 는 후자(`@Import(QuerydslConfig.class)`) 를 적용해 6개 테스트가 모두 GREEN 으로 통과합니다.

### 정답 2 — 멀티모듈에서 Q클래스 점검 순서

네 곳을 순서대로 점검합니다. ① Q클래스를 만드는 모듈에 `annotationProcessor "querydsl-apt:...:jakarta"` 가 걸려 있는지 확인합니다. ② 해당 모듈이 `api 'querydsl-jpa'` 로 노출했는지 보고 `implementation` 으로 닫혀 있으면 의존 모듈에서 querydsl 타입을 못 봅니다. ③ Gradle 의존 그래프가 의도대로 잡혀 의존 모듈이 Q클래스 모듈보다 *뒤에* 빌드되는지 확인합니다. ④ IDE 가 `build/generated/sources/annotationProcessor/...` 경로를 소스 폴더로 인식하지 못하면 빨간 줄만 보이므로 `./gradlew clean build` 후 IntelliJ Gradle 새로고침, 그래도 안 되면 `File > Invalidate Caches / Restart` 를 적용합니다. 본 프로젝트 `querydsl-practice` 는 단일 모듈이라 네 단계 모두 자동으로 해결되지만, 도메인 모듈을 분리하는 순간 이 점검 순서가 바로 필요해집니다.

### 정답 3 — Testcontainers 가 필요한 이유

QueryDSL 자체는 JPQL 을 생성하므로 표준 CRUD 라면 H2 로도 통과합니다. 그러나 운영 DB 와 결과가 갈리는 영역이 명확하게 존재합니다. PostgreSQL 에서 `sum(int * int)` 는 `bigint` 를 반환해 `Tuple.get(..., Long.class)` 로 받지 않으면 `ClassCastException` 이 발생하지만 H2 는 같은 식을 `int` 로 평탄화합니다. 윈도우 함수·`Expressions.stringTemplate` 으로 호출하는 DB 함수·격리 수준 시나리오도 H2 와 운영 DB 가 다르게 동작합니다. 따라서 핵심 통합 테스트는 Testcontainers 로 운영과 같은 엔진을 띄워 검증해야 안전합니다. 본 챕터 `Ch08_TestSetupTest` 는 그보다 한 단계 가벼운 길을 택해 `Replace.NONE` + `@ActiveProfiles("test")` 조합으로 *이미 떠 있는* Supabase PostgreSQL 에 붙고, `databaseProductNameIsPostgreSQL` 테스트가 그 사실을 직접 단언합니다.

### 정답 4 — fragment 단위 테스트의 이점

fragment 단위 테스트가 유리한 이유는 *실패 원인 격리* 와 *기동 비용 감소* 두 가지입니다. 한 fragment 만 `@Import` 로 끌어오면 메인 리포지토리·다른 fragment 의 영향이 사라져, 검증이 깨졌을 때 원인을 해당 fragment 의 `BooleanExpression` 조립이나 `JPAQueryFactory` 주입으로 좁힐 수 있습니다. 또 통계용 fragment 처럼 영속성과 분리된 메서드는 fragment 단독으로 검증하는 편이 의도와 코드가 일치합니다. 본 프로젝트는 별도 fragment 인터페이스 대신 `MemberRepositoryCustom` + `MemberRepositoryImpl` 패턴을 쓰므로, `Ch08_TestSetupTest#customFragmentMethodAlone` 슬롯이 같은 의도를 변형해 재현합니다 — 커스텀 메서드 하나만 호출해 DTO 프로젝션이 좁은 단위로 동작하는지 확인합니다.
...
- [01-02. 프로젝트 셋업 (Gradle 6.12)](01-02.%ED%94%84%EB%A1%9C%EC%A0%9D%ED%8A%B8%20%EC%85%8B%EC%97%85%20(Gradle%206.12).md) — 단일 모듈 셋업과 비교
- [jpa/03-05. 커스텀 리포지토리 패턴](../jpa/03-05.%EC%BB%A4%EC%8A%A4%ED%85%80%20%EB%A6%AC%ED%8F%AC%EC%A7%80%ED%86%A0%EB%A6%AC%20%ED%8C%A8%ED%84%B4.md) — fragment를 만들 때의 단위
- [10_spring/04_testing/01-01. 테스트 피라미드와 Spring 테스트 종류](../../../10_spring/04_testing/01-01.테스트 피라미드와 Spring 테스트 종류.md) — `@DataJpaTest`·슬라이스가 전체 테스트 계층에서 차지하는 위치 (본 문서의 JPA 슬라이스를 더 넓은 맥락에서 봄)
