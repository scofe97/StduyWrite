...
`EntityManager`를 받아 `from()` 헬퍼를 노출하던 옛 패턴이다. 본 묶음의 [jpa/03-05 §QuerydslRepositorySupport 사용 여부](../jpa/03-05.커스텀%20리포지토리%20패턴.md)에 다룬 세 가지 이유로 *사실상 안 쓴다*.
...
- 검색·정렬·페이징은 `JPAQueryFactory` + 사용자 정의 리포지토리 ([jpa/03-05](../jpa/03-05.커스텀%20리포지토리%20패턴.md))로 해결한다.
- 카운트 최적화는 `PageableExecutionUtils.getPage` ([01-06 §PageableExecutionUtils로 카운트 최적화](01-06.페이징과%20fetch%20join%20함정.md))로 해결한다.
