...
우리 실습이 `Replace.NONE` 인 이유는, 검증 대상이 **PostgreSQL 에서만 정확한 동작** (예: `sum(int*int)` 이 bigint 반환, 윈도우 함수, `Expressions` DB 함수) 이기 때문이다. H2 로 바꿔치기하면 방언이 달라 같은 QueryDSL 코드가 다른 SQL·다른 결과를 내, 통과해도 운영 DB 에서 깨질 수 있다 (§위 "인메모리 DB vs Testcontainers" 의 함정과 같은 맥락).
...
> 본 테스트·멀티모듈 문서가 묶음 내 다른 챕터와 어떻게 연결되는지. 단일 모듈 셋업은 01-02, 커스텀 리포지토리 패턴은 03-01 로 자연스럽게 이어진다.

- [01-02. 프로젝트 셋업 (Gradle 6.12)](01-02.프로젝트%20셋업%20(Gradle%206.12).md) — 단일 모듈 셋업과 비교
- [03-01. 커스텀 리포지토리 패턴](03-01.커스텀%20리포지토리%20패턴.md) — fragment를 만들 때의 단위
