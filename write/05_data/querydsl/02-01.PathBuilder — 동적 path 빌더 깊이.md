...
2. **Q-class import 가 어려운 모듈 경계** — *Q-class 로는 불가능에 가까운 유일한 자리*. 
   ...
   - 다만 PathBuilder 가 *유일한* 해법은 아니다. 모듈 구조를 손볼 수 있다면 "도메인 모듈에서 Q-class 를 빌드해 공유" 하는 정공법이 있고, 그 셋업·함정은 [03-02 § 멀티모듈 해결 패턴](03-02.테스트와%20멀티모듈.md) 이 다룬다. PathBuilder 는 *모듈 구조를 못 건드릴 때* 의 우회로다.
