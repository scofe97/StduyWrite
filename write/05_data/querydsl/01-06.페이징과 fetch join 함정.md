...
`@BatchSize` 의 동작 원리(왜 N+1 이 N/배치+1 로 줄어드는지)와 `@EntityGraph`·서브쿼리 같은 *다른 N+1 해결책과의 비교* 는 [JPA 노트 03-03. 프록시와 N+1](../jpa/03-03.프록시와%20N%2B1.md) 의 "§3-4 @BatchSize"·"§3-6 어떤 방법을 쓸까" 절에서 상세히 다룬다.

### 더 읽기 — 이 챕터 주제를 깊이 다루는 문서

| 주제 | 상세 문서 |
|------|----------|
| N+1·`@BatchSize`·`@EntityGraph`·fetch 전략 비교 | [JPA 03-03. 프록시와 N+1](../jpa/03-03.프록시와%20N%2B1.md) |
| Spring Data `Page`/`Pageable`/`Slice`·정렬 변환 | [JPA 03-04. Auditing, 페이징, Projection](../jpa/03-04.Auditing,%20페이징,%20Projection.md) §3 |
| `JpaRepository` 공통 페이징 인터페이스 | [JPA 03-01. Spring Data JPA 공통 인터페이스](../jpa/03-01.Spring%20Data%20JPA%20공통%20인터페이스.md) |
| 컬렉션 fetch join 데카르트 곱·distinct·Bag/Set | [01-03. 기본 문법과 조인](01-03.기본%20문법과%20조인.md) §fetchJoin |
| 프로젝션으로 1:N 못 받는 이유·두 단계 쿼리 | [01-05. 프로젝션과 DTO 매핑](01-05.프로젝션과%20DTO%20매핑.md) §컬렉션 결과 매핑의 한계 |
...
- [jpa/03-05. 커스텀 리포지토리 패턴](../jpa/03-05.커스텀%20리포지토리%20패턴.md) — 페이지 결과를 Spring Data JPA에 통합
