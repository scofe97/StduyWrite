...
`countQuery::fetchOne`은 `Supplier<Long>`이다. 여기서 *메서드 참조로 넘긴다는 점* 이 핵심이다 — `countQuery.fetchOne()` 처럼 *즉시 호출* 하면 count 가 무조건 실행되지만, `countQuery::fetchOne` 으로 *호출을 미뤄* 넘기면 `getPage` 가 *정말 필요하다고 판단할 때만* 실행한다. count 는 *전체 행을 세는* 연산이라 큰 테이블에서 인덱스를 타도 무거운데, 이 지연 실행 한 줄로 불필요한 count 를 건너뛴다.
