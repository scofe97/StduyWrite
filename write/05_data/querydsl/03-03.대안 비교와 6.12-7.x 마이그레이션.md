...
`EntityManager`를 받아 `from()` 헬퍼를 노출하던 옛 패턴이다. 본 묶음의 [03-01 §QuerydslRepositorySupport 사용 여부](03-01.커스텀%20리포지토리%20패턴.md)에 다룬 세 가지 이유로 *사실상 안 쓴다*.
...
- 검색·정렬·페이징은 `JPAQueryFactory` + 사용자 정의 리포지토리 ([03-01](03-01.커스텀%20리포지토리%20패턴.md))로 해결한다.
