...
`select` 에 컬럼을 *둘 이상* 나열하면 결과 타입이 무엇이 될지 애매하다. 회원 엔티티 하나면 `Member`, 이름 하나면 `String` 이지만, `select(member.address.city, member.count())` 처럼 *서로 다른 타입을 여럿* 고르면 하나의 자바 타입으로 묶을 수 없다. 이때 QueryDSL 이 *임시로 한 행을 담아 주는 그릇* 이 `com.querydsl.core.Tuple` 이다.
