...
> 흔한 오해 — *`@OneToMany(fetch = EAGER)` 로 바꾸면 N+1 이 해결되지 않는다*. EAGER 는 "언제 가져오나(시점)"만 바꿀 뿐, 연관을 *한 SQL 로 JOIN 해 가져오는지* 는 바꾸지 못해 여전히 연관마다 별도 SELECT 가 나간다(오히려 항상 나간다). N+1 은 *쿼리 단위로 fetch join·`@EntityGraph`·`@BatchSize`* 를 줘야 풀린다. EAGER/LAZY 와 N+1 해결책 비교는 [JPA 노트 03-03. 프록시와 N+1](../jpa/03-03.프록시와%20N%2B1.md) 의 "왜 EAGER 가 위험한가"·"어떤 방법을 쓸까" 절이 깊이 다룬다.
...
> 결론: *지금 한 번 쓰고 버릴 조회* 면 `Tuple`, *여러 곳에서 쓰이거나 밖으로 나갈 데이터* 면 DTO. DTO 매핑(`Projections.constructor` / `fields` / `@QueryProjection`)은 [01-05. 프로젝션과 DTO 매핑](01-05.프로젝션과%20DTO%20매핑.md) 이 본격적으로 다룬다.
