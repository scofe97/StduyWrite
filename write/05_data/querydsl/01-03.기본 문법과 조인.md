...
flowchart TB
    subgraph nplusone["❌ N+1 문제 (일반 조인 또는 LAZY)"]
        n1["SELECT 1번<br>members 10건 가져옴"]
        n2["m.team 접근 10번<br>= SELECT 10번 추가"]
        n3["총 11번 SQL"]
        n1 --> n2 --> n3
    end
    subgraph fetchjoin["✅ fetchJoin"]
        f1["SELECT 1번<br>JOIN FETCH team<br>members 10건 + team 같이"]
        f2["m.team 접근<br>이미 영속성 컨텍스트에 채워짐"]
        f3["총 1번 SQL"]
        f1 --> f2 --> f3
    end
...
`fetchJoin()` 의 한 글자 차이가 *SQL 11번 → 1번* 으로 줄인다. 회원과 회원의 팀을 한 쿼리로 가져와 N+1 문제를 막는다.
...
- 일반 `join`만 쓰면 SQL은 inner join을 하지만, JPA는 결과를 매핑할 때 회원 엔티티만 만들고 `Team`은 프록시로 둔다. 이후 
- `member.getTeam().getName()`을 호출하면 추가 쿼리가 나간다(N+1). `fetchJoin`을 붙이면 한 번의 SQL로 회원과 팀을 동시에 채운다.
...
```java
// ✗ 의도와 다르게 동작 — team 만 fetchJoin, address 는 일반 join
List<Member> bad = queryFactory.selectFrom(member)
    .join(member.team, team)
    .join(member.address, address).fetchJoin()  // ← address 만 fetchJoin
    .where(team.id.eq(1L))
    .fetch();
```

위 코드는 `team` 도 *프록시* 로 남아 이후 `member.getTeam().getName()` 호출이 N+1 을 일으킨다. `fetchJoin()` 이 *바로 직전* 의 `join(member.address, address)` 에만 붙기 때문이다.

```java
// ✓ 두 join 모두 fetchJoin
List<Member> good = queryFactory.selectFrom(member)
    .join(member.team, team).fetchJoin()        // ← team fetchJoin
    .join(member.address, address).fetchJoin()  // ← address fetchJoin
    .where(team.id.eq(1L))
    .fetch();
```
