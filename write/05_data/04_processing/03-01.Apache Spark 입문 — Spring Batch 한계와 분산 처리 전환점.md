...
> Spring Batch 의 *Remote Partitioning* ([`./01-05`](01-05.Parallel%C2%B7Partition%20Batch%20%E2%80%94%20TaskExecutor%C2%B7PartitionStep%C2%B7Remote%20Partitioning.md)) 까지 가도 *어떤 잡* 은 안 됩니다. 데이터가 *수십 TB* 이고, *복잡한 집계·조인* 이 잡 본체이고, *수백 대 머신을 통합 자원으로* 다뤄야 하는 자리입니다. 그 자리에 *Apache Spark* 가 답입니다. 본 편은 *비교 편* 입니다. Spark 의 깊이가 아니라, *Spring Batch 에서 Spark 로 넘어가야 하는 분기점* 과 *Spark 의 3가지 핵심 모델 (RDD · DataFrame · Structured Streaming)* 의 의미를 잡습니다.
...
이 예시가 [`../01_foundation/03-01.배치 처리.md`](../01_foundation/03-01.%EB%B0%B0%EC%B9%98%20%EC%B2%98%EB%A6%AC.md) 의 *MapReduce 가 Unix 파이프를 분산 환경으로 옮긴* 사고를 그대로 보여줍니다. `map` 은 분산 변환, `reduceByKey` 는 shuffle + reduce 입니다.
...
자세한 스트림 처리 사고는 [`../01_foundation/03-02.스트림 처리.md`](../01_foundation/03-02.%EC%8A%A4%ED%8A%B8%EB%A6%BC%20%EC%B2%98%EB%A6%AC.md) 와 [`../../04_messaging/06_StreamProcessing/`](../../04_messaging/06_StreamProcessing/) 에서 다룹니다.
...
- *Load* 는 *목적 DB 에 INSERT*. Spring Batch 의 JdbcBatchItemWriter ([`./01-06`](01-06.Bulk%20Insert%C2%B7Update%20%E2%80%94%20JdbcBatchItemWriter%EC%99%80%20JPA%20batch_size%20%ED%95%A8%EC%A0%95.md)) 가 적합.
...
- [`./01-05.Parallel·Partition Batch — TaskExecutor·PartitionStep·Remote Partitioning.md`](01-05.Parallel%C2%B7Partition%20Batch%20%E2%80%94%20TaskExecutor%C2%B7PartitionStep%C2%B7Remote%20Partitioning.md) — Spring Batch 의 *분산 처리 상한선* 인 Remote Partitioning. 본 편 Spark 가 *그 상한선 너머* 의 자리. 둘을 같이 봐야 *어디서 도구를 바꿔야 하는지* 의 분기점이 명확합니다
- [`./02-01.Airflow DAG 모델 — Spring Batch와의 책임 분담 비교.md`](02-01.Airflow%20DAG%20%EB%AA%A8%EB%8D%B8%20%E2%80%94%20Spring%20Batch%EC%99%80%EC%9D%98%20%EC%B1%85%EC%9E%84%20%EB%B6%84%EB%8B%B4%20%EB%B9%84%EA%B5%90.md) — Spark 잡과 Spring Batch 잡을 *DAG 노드로 묶는* 자리. *Extract = Spring Batch, Transform = Spark, Load = Spring Batch* 같은 조합이 자연스럽습니다
- [`../01_foundation/03-01.배치 처리.md`](../01_foundation/03-01.%EB%B0%B0%EC%B9%98%20%EC%B2%98%EB%A6%AC.md) — 이론 측. *MapReduce·Shuffle·Dataflow 엔진* 의 진화가 Spark RDD·DataFrame 의 직접적인 토대. 본 편을 읽기 전에 그 문서를 읽으면 *왜 RDD 가 MapReduce 의 다음 단계인지* 가 자연스럽게 닫힙니다
- [`../01_foundation/03-02.스트림 처리.md`](../01_foundation/03-02.%EC%8A%A4%ED%8A%B8%EB%A6%BC%20%EC%B2%98%EB%A6%AC.md) — Structured Streaming 의 사고 모델. *무한 테이블 위의 같은 SQL* 이라는 통합 모델의 일반 이론
