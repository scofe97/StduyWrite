---
title: 05_data/04_processing — Spring Batch 중심 배치 처리 시리즈
...
# 05_data/04_processing — Spring Batch 중심 배치 처리 시리즈

---

> 유한한 입력을 받아 새 출력을 만드는 *오프라인 처리* 를 Spring Batch 5.x 로 구현하는 방법을 정리합니다. 이론 측 *왜 배치인가* 는 [`../01_foundation/03-01.배치 처리.md`](../01_foundation/03-01.%EB%B0%B0%EC%B9%98%20%EC%B2%98%EB%A6%AC.md) 가 다루고, 여기는 *어떻게 짜는가* 만 담당합니다. Airflow·Spark 는 비교 축으로만 한 편씩 등장합니다.

## 왜 별도 묶음인가

`05_data/01_foundation/03-01` 은 DDIA Ch.11 기준으로 *Unix → MapReduce → Dataflow 엔진* 의 진화와 Shuffle·분산 JOIN 같은 일반 메커니즘을 다룹니다. 그 다음 단계는 *그래서 내 Spring Boot 앱에서 배치를 어떻게 짜는가* 입니다. Spring Batch 가 그 자리를 채웁니다.
...
| # | 문서 | 다루는 핵심 질문 |
|---|------|----------------|
| 01-01 | [Spring Batch 골격 — Job·Step·Chunk·Tasklet](01-01.Spring%20Batch%20%EA%B3%A8%EA%B2%A9%20%E2%80%94%20Job%C2%B7Step%C2%B7Chunk%C2%B7Tasklet.md) | Job 이 Step 을 어떻게 담고, Step 안에서 Chunk 와 Tasklet 이 언제 갈리는가 |
