...
정책상 가장 중요한 것은 "마스킹 후" 가 아니라 *마스킹 전의 원본이 개발자 PC 에 남지 않게 하는 것* 이다. 마스킹은 SQL `UPDATE` 한 줄로 끝나지만, 원본이 한 번 노출되면 회수가 불가능하다. 컬럼별 마스킹 예시·격리 절차는 [`./03-02.DB덤프와 로컬이관 (심화).md`](01-02.DB%20덤프와%20로컬%20이관%20(%EC%8B%AC%ED%99%94).md) §5 참고.
...
- [03-01. DB 덤프와 로컬이관](01-01.DB%20덤프와%20로컬%20이관.md) — `mariadb-dump` 옵션 풀이, docker compose entrypoint, 단계별 적용
- [03-02. DB 덤프와 로컬이관 (심화)](01-02.DB%20덤프와%20로컬%20이관%20(%EC%8B%AC%ED%99%94).md) — DEFINER, initdb.d 함정, 마스킹 정책, 사례 트러블슈팅
