...
related:
  - ./README.md
  - ../03_persistence/querydsl/03-04.락과 동시성 제어.md
  - ../README.md
updated: 2026-05-22
---
...
둘째는 **명시적 잠금** 이다. `SELECT ... FOR UPDATE` 로 행을 잠근 뒤 비즈니스 로직을 거쳐 갱신한다. 비관 락 패턴이며, ORM 측면 적용은 [`querydsl/02-05.락과 동시성 제어.md`](../03_persistence/querydsl/03-04.%EB%9D%BD%EA%B3%BC%20%EB%8F%99%EC%8B%9C%EC%84%B1%20%EC%A0%9C%EC%96%B4.md) 가 자세히 다룬다.
...
대안으로 두 갈래가 있다. 단일 DB 내부의 분산 트랜잭션(CockroachDB, TiDB, Spanner) 은 Coordinator 와 Participant 가 같은 시스템 안에 있어 SPOF 문제가 약하다. 마이크로서비스 간 분산 트랜잭션은 Saga 패턴(Choreography 또는 Orchestration) 으로 빠지는 편이 운영 비용이 낮다. Saga 의 이론과 구현은 [`../`](../) 와 [`../../04_messaging/`](../../04_messaging/) 가 다룬다.
...
가장 자주 보는 패턴은 첫 줄과 셋째 줄이다. 일반 조회·수정은 Read Committed 에 `@Version` 을 결합해 lost update 를 막고, 결제·재고처럼 충돌이 잦은 자리만 비관 락을 쓴다. ORM 적용 코드는 [`querydsl/02-05.락과 동시성 제어.md`](../03_persistence/querydsl/03-04.%EB%9D%BD%EA%B3%BC%20%EB%8F%99%EC%8B%9C%EC%84%B1%20%EC%A0%9C%EC%96%B4.md) 의 패턴을 그대로 가져다 쓸 수 있다.
...
- [`querydsl/02-05.락과 동시성 제어.md`](../03_persistence/querydsl/03-04.%EB%9D%BD%EA%B3%BC%20%EB%8F%99%EC%8B%9C%EC%84%B1%20%EC%A0%9C%EC%96%B4.md) — 격리 수준의 ORM 적용, `@Version`·`@Lock`·`setLockMode`
- [`../README.md`](../README.md) — 복제·합의·분산 트랜잭션의 상위 이론
