---
title: 01-03.202 Accepted + Polling 패턴
tags: [eda, http-202, polling, async-api, idempotency, retry-after, read-model]
status: final
source:
...
> "주문 버튼 눌렀는데... 지금 어디쯤이지?" 비동기 시스템에서 클라이언트가 결과를 아는 방법은 무엇인가? [01-02.EDA 기반 요청응답 통합](./../../../03_architecture/04_edd/05_KafkaApplied/05-02.EDA%20기반%20요청응답%20통합.md)에서 다룬 4가지 브릿지 패턴 중, 가장 범용적이고 인프라 요구사항이 적은 **Event-First(202 + Polling)** 패턴을 깊이 파고든다.

## 학습 목표

- HTTP 202와 200/201의 의미 차이를 RFC 7231 정의로 구분한다.
- 202 응답에 담겨야 할 필수 요소(orderId, status, statusUrl)와 Location/Retry-After 헤더의 역할을 설명한다.
- Polling 라이프사이클(시작·반복·종료 조건)을 설계하고 종료 상태에서 Polling을 중단하는 코드를 작성한다.
- 202 패턴이 멱등성을 구조적으로 돕는 이유와 Idempotency Key 패턴 결합 방법을 설명한다.
- Polling vs SSE vs WebSocket을 인프라 부담·실시간성 기준으로 선택한다.

## 1. 문제: 동기 API로 오래 걸리는 작업을 처리한다면?

### 1.1 일상의 비유 — 카페 진동벨

카페에서 커피를 주문하는 과정을 떠올려보자.

1. 카운터에서 주문하고 번호표를 받는다 → **202 Accepted**
2. 자리에 앉아서 가끔 전광판을 확인한다 → **Polling**
3. 내 번호가 뜨면 카운터에서 수령한다 → **COMPLETED**

만약 진동벨 없이 카운터 앞에서 커피가 나올 때까지 서서 기다린다면? 뒷사람은 주문도 못 하고, 나는 아무것도 할 수 없다. 이것이 바로 동기 API의 문제다.

### 1.2 음식 배달을 동기로 처리한다면?

배달앱에서 주문 버튼을 누르면, 가게 접수 → 조리 → 라이더 배정 → 배달까지 30분~1시간이 걸린다. 이 전체 과정을 동기 HTTP 요청 하나로 처리한다고 상상해보자.
...
서버 스레드 하나가 주문 건당 30분을 점유한다. 동시 주문 100건이면 100개 스레드가 아무 일도 못 하고 대기한다.
...
> 202 응답의 의미·필수 요소·Polling 라이프사이클·멱등성·안티패턴을 정리한다.

### 2.1 HTTP 202 Accepted란?

HTTP 상태 코드 중 202는 특별한 의미를 가진다. 먼저 2xx 계열에서 자주 쓰이는 코드들을 비교해보자.
...
[RFC 7231 Section 6.3.3](https://tools.ietf.org/html/rfc7231#section-6.3.3)에서 202를 이렇게 정의한다.
...
여기서 놓치기 쉬운 포인트가 2가지 있다.

**1) "processing has not been completed" — 아직 안 끝남**

202는 "성공"이 아니다. "접수"다. 배달앱에서 주문 버튼을 누르고 "주문이 접수되었습니다"라는 토스트를 보는 것이지, "배달이 완료되었습니다"가 아니다. 최종 결과는 나중에 별도로 확인해야 한다.

**2) "might or might not eventually be acted upon" — 실패할 수도 있음**

202를 받았다고 해서 처리가 보장되는 것이 아니다. 배달 주문을 넣었지만 가게가 거절할 수 있고, Jenkins 빌드를 요청했지만 큐가 가득 차서 드롭될 수 있다. 그래서 Polling 응답에 FAILED/CANCELLED 같은 실패 상태가 반드시 있어야 한다.

### 왜 200으로 하면 안 되는가?

"그냥 200 OK에 `{ status: 'PROCESSING' }` 넣으면 안 되나요?" 라는 질문이 나올 수 있다.
...
HTTP 상태 코드는 **기계가 읽는 API의 첫 번째 언어**다. 200은 "완료"라는 약속이고, 202는 "접수"라는 약속이다. 약속을 정확히 지켜야 클라이언트(사람이든, 프록시든, 모니터링 시스템이든)가 올바르게 동작한다.

## 2.2 202 응답 설계 — 무엇을 돌려줘야 하는가?

202를 반환할 때 body를 비워두면 클라이언트는 막막하다. "접수됐다는 건 알겠는데, 그래서 어떻게 확인하지?" 202 응답에는 **클라이언트가 다음 행동을 할 수 있는 정보**가 있어야 한다.
...
- **Location 헤더**: RFC 7231에서는 202 응답에 Location 헤더로 "상태를 확인할 수 있는 URL"을 제공하도록 권장한다. body의 statusUrl과 같은 역할이지만, 표준 헤더를 쓰면 HTTP 클라이언트 라이브러리가 자동으로 활용할 수 있다.
- **Retry-After 헤더**: "N초 후에 다시 확인해"라는 서버의 힌트다. 클라이언트가 이 값을 존중하면 불필요한 요청을 줄일 수 있다.

## 2.3 Polling 엔드포인트 설계 — 어떻게 물어보고, 어떻게 대답하는가?

202로 접수 응답을 보냈으면, 클라이언트가 상태를 확인할 **Polling 엔드포인트**가 필요하다. 이 엔드포인트의 설계가 패턴의 품질을 결정한다.

### URL 패턴

Polling 엔드포인트의 URL을 설계하는 방식은 크게 2가지다.
...
배달 주문의 경우, POST 시점에 이미 주문(Order)이 DB에 생성되므로 **방식 A**가 자연스럽다. 반면 01-02 문서의 게시판 예시처럼 글 자체가 비동기로 생성되는 경우, 아직 boardId가 없을 수 있으므로 **방식 B**로 requestId를 추적한다.

### 상태별 응답 설계

Polling 엔드포인트는 **현재 상태에 따라 다른 응답**을 반환해야 한다. 중요한 원칙은, 진행 중일 때와 완료됐을 때의 응답 구조가 달라야 한다는 점이다.
...
Polling 엔드포인트의 HTTP 상태 코드에 대해 두 가지 설계 방식이 있다.
...
실무에서는 **방식 1(항상 200)** 이 더 흔하다. 프론트엔드 입장에서 body의 status 필드만 보면 되니까 더 직관적이고, axios 같은 HTTP 클라이언트에서 202를 별도로 처리하는 코드를 작성할 필요가 없다.

## 2.4 Polling의 라이프사이클

Polling은 무한히 계속되면 안 된다. 명확한 시작 조건, 반복 조건, 종료 조건이 있어야 한다.
...
타임아웃은 반드시 설정해야 한다. 서버 장애로 상태가 영원히 PROCESSING에 머물면, 클라이언트가 무한히 Polling하게 된다.

## 2.5 멱등성 — 202가 자연스럽게 해결하는 문제

동기 API에서 30분 대기 후 타임아웃이 나면, 사용자는 "주문이 된 건지 안 된 건지" 모른다. 새로고침하면 POST가 다시 실행되어 중복 주문이 발생할 수 있다.

202 패턴에서는 이 문제가 구조적으로 완화된다.
...
202 패턴이 멱등성을 돕는 이유는 다음과 같다.

1. **POST가 즉시 완료**되므로 타임아웃이 발생할 여지가 거의 없다 (수십ms).
2. **orderId를 즉시 반환**하므로 클라이언트가 "이미 주문이 접수됐다"는 것을 확실히 안다.
3. 페이지가 **주문 상태 화면으로 전환**되어, 같은 POST를 다시 보낼 UI 자체가 사라진다.

추가로 완전한 멱등성이 필요하면 **Idempotency Key** 패턴을 결합한다.
...
클라이언트가 다음 행동을 할 수 없다. 최소한 requestId와 statusUrl은 반드시 포함해야 한다.
...
DELIVERED/CANCELLED에서 Polling을 중단하는 로직이 없으면, 서버 장애로 상태가 갱신되지 않을 때 클라이언트가 영원히 요청을 보낸다. 최대 Polling 횟수나 시간 제한을 반드시 걸어야 한다.
...
사용자가 "왜 취소됐지?"를 알 수 없으면 같은 주문을 다시 시도할지, 포기할지 판단할 수 없다.
...
202는 "아직 처리중"이라는 약속이다. 이미 다 처리했으면 200이나 201을 반환해야 한다. 상태 코드의 의미를 지키지 않으면 클라이언트가 불필요한 Polling을 시작한다.

## 2.7 01-02 브릿지 패턴 중 위치

[01-02 문서](./../../../03_architecture/04_edd/05_KafkaApplied/05-02.EDA%20기반%20요청응답%20통합.md)에서 소개한 4가지 브릿지 패턴 비교표를 다시 보자.
...
202+Polling의 특징은 **가장 범용적**이고, **인프라 요구사항이 최소**(추가 인프라 없이 HTTP만으로 동작)이며, **클라이언트 주도**라는 점이다.

## 3. 실전 적용: 음식 배달 주문 시스템

> POST 한 번에 끝나는 주문 접수와, 비동기 이벤트로 갱신되는 Read Model을 결합해 Polling으로 노출한다.

### 3.1 시나리오

사용자가 배달앱에서 주문을 넣으면 다음 단계를 거친다.

> 주문 접수 → 가게 확인 → 조리중 → 조리완료 → 라이더 배정 → 픽업 → 배달중 → 배달완료

이 과정에서 데이터는 두 종류로 나뉜다.
...
핵심 설계 원칙은, 즉시 필요한 것은 202 응답에 담고, 계속 갱신되는 것은 Polling으로 조회한다는 것이다.
...
POST는 DB 저장 + Kafka 발행만 하고 즉시 202를 반환한다. 조리/배달은 별도 서비스가 비동기로 처리한다.
...
각 상태 전이마다 `order.progress` 또는 `order.completed` 이벤트가 발행되고, Status Consumer가 Read Model을 갱신한다.
...
배민앱에서 주문 후 보이는 화면을 떠올려보면, 각 상태에서 사용자가 보는 정보가 다르다.
...
DELIVERED나 CANCELLED에 도달하면 더 이상 상태가 변하지 않으므로 **Polling을 중단**한다. 불필요한 서버 요청을 방지하는 중요한 설계 포인트다.

## 4. 다른 사례로 확장 — 같은 패턴, 다른 도메인

> POST → 202 + ID → Polling → 종료 상태 감지 골격은 도메인이 달라도 동일하게 적용된다.

202+Polling 패턴은 도메인이 달라도 구조가 동일하다.

| 요소 | 음식 배달 | 게시판 글쓰기 (01-02) | 이커머스 주문 (01-02) | Jenkins 빌드 |
| --- | --- | --- | --- | --- |
...
핵심 인사이트는 **구조는 동일하고 도메인 용어만 다르다**는 점이다. POST → 202 + ID 반환 → Polling → 종료 상태 감지라는 골격은 어디에나 적용된다.

## 5. 설계 시 고려사항

> 간격·타임아웃·SSE/WebSocket 비교를 짚어 운영에서의 선택 근거를 정리한다.

### 5.1 Polling 간격과 Retry-After

Polling 간격을 결정하는 3가지 전략이 있다.
...
음식 배달의 경우, 상태에 따라 변화 속도가 다르므로 **서버 주도 Retry-After**가 적합하다.
...
CANCELLED이나 FAILED 상태에는 사유를 함께 전달한다.
...
배달 주문 상태는 수 초~수 분 간격으로 변한다. SSE나 WebSocket의 실시간성이 반드시 필요하지 않고, Polling만으로 충분히 좋은 UX를 제공할 수 있다. 연결을 유지하지 않으므로 서버 리소스도 절약된다.

> SSE 방식이 궁금하다면 [01-02 문서의 Section 3 (Server-Sent Events)](./../../../03_architecture/04_edd/05_KafkaApplied/05-02.EDA%20기반%20요청응답%20통합.md)을 참고한다.

## 6. 정리

> 접수 확인은 즉시(202), 진행 상태는 주기적 확인(Polling)이라는 골격을 카페 진동벨 비유로 한 번 더 압축한다.
...
이 패턴은 인프라 요구사항이 최소이면서도 클라이언트-서버 간 비동기 처리를 깔끔하게 연결한다. 카페 진동벨처럼, **접수 확인은 즉시 주고 결과는 나중에 확인**하는 — 우리가 일상에서 이미 쓰고 있는 패턴이다.
...
### Q1. 200 OK + `{status: "PROCESSING"}` 대신 202를 쓰는 이유는?

HTTP 상태 코드는 "완료(200)"와 "접수(202)"를 다른 단어로 약속해 둔 인터페이스다. 200은 캐시되거나 모니터링에서 성공으로 집계될 수 있어서, body에 PROCESSING이 들어 있어도 중간 상태가 그대로 캐싱되거나 "성공"으로 잘못 분류된다. 202는 처음부터 "아직 안 끝남"을 의미하므로 클라이언트·프록시·로드밸런서가 상태 코드만 보고도 비동기 작업임을 판단한다.

### Q2. 202 응답에 최소한 무엇을 담아야 하나?

세 가지가 필수다. 작업을 식별할 `orderId`(또는 requestId), 현재 상태인 `status`, 그리고 클라이언트가 다음에 호출할 `statusUrl`이다. `statusUrl`이 없으면 클라이언트는 "어디로 폴링을 보내야 하는지" 모르고, 단순한 가이드 문서로 약속해 두면 API가 바뀔 때마다 양쪽이 깨진다. RFC 7231은 `Location` 헤더와 `Retry-After`를 함께 권장하므로 표준 헤더로 전달하면 HTTP 클라이언트 라이브러리가 자동 활용한다.

### Q3. Polling은 언제 멈추는가?

종료 조건은 세 가지다. 첫째, 상태가 성공 종료(예: DELIVERED, COMPLETED)에 도달했을 때. 둘째, 실패 종료(FAILED, CANCELLED)에 도달했을 때 — 이때는 사용자가 원인을 알도록 사유를 함께 받는다. 셋째, 미리 정한 최대 횟수·시간을 넘었을 때로, 서버 장애로 상태가 영원히 PROCESSING에 머무는 경우를 대비한 안전 장치다. 종료 조건이 없는 Polling은 사실상 무한 루프와 같다.

### Q4. 202 패턴이 멱등성을 어떻게 자연스럽게 돕는가?

POST가 수십 ms에 끝나므로 동기 API에서 30분 대기 후 타임아웃이 나서 새로고침으로 중복 전송되는 시나리오가 사라진다. 그리고 immediately 반환되는 `orderId`가 클라이언트에게 "이미 접수됨"이라는 영수증 역할을 해서 UI가 상태 페이지로 전환되고, 같은 POST를 다시 보낼 버튼 자체가 화면에서 사라진다. 더 강한 보장을 원하면 `Idempotency-Key` 헤더로 동일 요청을 서버 측에서 묶어 처리한다.

### Q5. SSE/WebSocket이 있는데 굳이 Polling을 쓰는 이유는?

선택은 "실시간성 요구"와 "인프라 비용"의 트레이드오프다. SSE는 단방향 스트림, WebSocket은 양방향이라 실시간성이 강하지만 연결을 유지해야 하므로 서버 리소스와 LB·프록시·CDN 호환성을 따로 챙겨야 한다. 배달 주문처럼 상태 변화가 초~분 단위이고 클라이언트가 가끔 확인하면 충분한 경우, 추가 인프라 없이 HTTP만으로 동작하는 Polling이 가장 저렴하다. 채팅·주가·협업 편집처럼 ms 단위 갱신이 필요할 때만 SSE/WebSocket을 꺼낸다.



## 8. 관련 문서

- [01-02.EDA 기반 요청응답 통합](./../../../03_architecture/04_edd/05_KafkaApplied/05-02.EDA%20기반%20요청응답%20통합.md) — 4가지 브릿지 패턴 중 이 문서가 다루는 Event-First의 상위 분류
- [05-03.Outbox](./../../../04_messaging/05_ConsistencyPattern/01-03.Outbox.md) — 202 응답 이후 이벤트 발행 안정성을 보장하는 패턴
- [07-01.Kafka CQRS](./../../../04_messaging/07_CQRS_EventSourcing/01-01.Kafka%20CQRS.md) — Polling이 조회하는 Read Model을 이벤트로 구성하는 구조
- [05-05.Inbox](./../../../04_messaging/05_ConsistencyPattern/01-05.Inbox.md) — Status Consumer의 멱등 처리를 인프라 수준으로 강제하는 짝 패턴



---

← [01-02.EDA 기반 요청응답 통합](./../../../03_architecture/04_edd/05_KafkaApplied/05-02.EDA%20기반%20요청응답%20통합.md)으로 돌아가기
