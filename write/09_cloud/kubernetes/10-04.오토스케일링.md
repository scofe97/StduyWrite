...
오토스케일링은 트래픽과 메트릭이 정상 분포일 때 잘 동작한다. 그러나 단일 Pod 안에서 메모리 모델이 어긋나면 HPA로 Pod를 더 많이 띄워도 같은 OOMKill이 반복된다. 다음 장 [OOMKilled 사례 분석](./10-05.OOMKilled%20사례%20분석.md)은 이 단원에서 다룬 Requests·Limits 신호가 실제 cgroup 메모리와 어긋난 한 사례를 따라가며, 자원 관리 장과 오토스케일링 장을 운영 관점에서 닫는다.
...
- [오토스케일링 점검](./deepdive/10-04.%EC%98%A4%ED%86%A0%EC%8A%A4%EC%BC%80%EC%9D%BC%EB%A7%81%20점검.md) — 본 장의 점검 편
- [자원 관리](./10-03.%EC%9E%90%EC%9B%90%20%EA%B4%80%EB%A6%AC.md) — 이전 장, Requests/Limits와 QoS
- [OOMKilled 사례 분석](./10-05.OOMKilled%20사례%20분석.md) — 다음 장, JVM heap과 cgroup의 어긋남
