...
|---|---|---|---|
| Heartbleed | 길이를 속여 메모리를 읽음 | 암호화돼도 길이 필드로 판정 | §1 |
| 포트 스캔 (port scan) | 열린 포트를 훑어보는 행위 | 서명이 캡처에 남음 | §2 |
| ARP 스푸핑 | MAC 을 속여 트래픽을 끌어옴 | 중간자 공격의 흔한 입구 | §3 |
| 중간자 (MITM) | 두 끝 사이에 끼어듦 | 통신은 되는데 느리고 이상함 | §3 |
| Endpoints | 종단별 통계 창 | 누가 도는가 · 나가는 연결을 셈 | §4 |
| Protocol Hierarchy | 프로토콜 비중 통계 창 | 무엇이 도는가 · 낯선 캡처를 훑음 | §4 |
...
원문의 `ssl.` 접두사는 4.6 에서도 컴파일되지만 경고가 붙습니다. 로컬 4.6.8 의 `dftest` 가 그대로 알려 줍니다. **`Warning: Deprecated token "ssl.record.content_type".`** 내부적으로는 `tls.record.content_type` 으로 바뀌어 실행됩니다.
...
현행 4.6 에서는 이 길이를 손으로 비교할 필요가 없습니다. 해석기가 판정 필드를 직접 제공합니다.
...
> 원문의 필터와 통계 창은 지금도 그대로 돕니다. 원문이 다루지 않는 두 축, 지금 만나는 Heartbleed 의 의미와 ARP 중독의 구성 방어를 함께 봅니다.

`arp.duplicate-address-frame` 필터는 1.0.0 부터 4.6.8 까지 그대로이고, `Statistics | Endpoints` · `Conversations` 도 원문 그대로 씁니다.
...
- [04-03 열쇠와 실패](./04-03.열쇠와 실패.md) — 같은 `ssl-enum-ciphers` 를 내 서버 조사에 쓰는 자리
