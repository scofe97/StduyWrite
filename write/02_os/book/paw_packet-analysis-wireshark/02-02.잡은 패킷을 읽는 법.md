...
| 디스플레이 필터 | 저장된 것에서 시야만 좁힘 | 캡처 필터와 문법 체계가 다름 · `-Y` 에 넣음 | §3 |
| 이더넷 프레임 | 목적지·출발지 MAC 과 타입 | preamble 은 안 올라옴 · FCS 는 환경에 따라 | §5 |
| 생성 필드 (generated field) | Wireshark 가 얹은 계산 값 | 대괄호 표기 · 선 위의 바이트가 아님 | §5 · §6 |
...
예제 캡처를 직접 뜨는 명령은 [02-05 터미널에서 같은 화면을 읽는 법](./02-05.%ED%84%B0%EB%AF%B8%EB%84%90%EC%97%90%EC%84%9C%20%EA%B0%99%EC%9D%80%20%ED%99%94%EB%A9%B4%EC%9D%84%20%EC%9D%BD%EB%8A%94%20%EB%B2%95.md) §1 로 옮겼습니다.
...
>
> `http || dns | ssl` 은 마지막 구분자가 파이프 하나입니다. 논리 OR 는 `or` 또는 `||` 이며 파이프 하나는 연산자 목록에 없습니다.[^dfilter-ops] `Snmp` 는 대문자로 시작합니다. 공식 문서는 5.0 부터 영문 연산자만 대소문자를 가리지 않게 됐다고 적으며, **필드 이름은 여전히 대소문자를 가린다**고 명시합니다.[^dfilter-ops] 셋 다 `tcp.srcport == 2222`·`http || dns || ssl`·`snmp` 로 고쳐야 동작합니다.

마지막 행의 `ssl` 은 새로 쓰는 필터에서 `tls` 로 씁니다. Wireshark 3.0 릴리스 노트가 "SSL 디섹터의 이름이 TLS 로 바뀌었다"고 적고, 옛 `ssl.*` 필드는 호환을 위해 지원하지만 "앞으로 제거될 수 있다"고 덧붙입니다.[^ssl-tls] 원문이 `ssl` 로 적은 것은 이름이 바뀌기 전인 1.12 판을 기준으로 썼기 때문입니다.

원문이 다루지 않는 축이 하나 있습니다. **필터 칸의 색이 문법 검사 결과를 알려준다는 점입니다.** 초록이면 유효한 필터, 빨강이면 문법 오류,[^wsug-toolbar] 노랑이면 문법은 맞지만 의도와 다르게 동작할 가능성이 있다는 경고입니다. 위 정오 항목의 `tcp.srcport=2222` 나 `Snmp` 를 실제로 쳐 보면 칸이 빨갛게 변해, 문서를 찾지 않고도 잘못된 필터임을 알 수 있습니다.
...
처음 받은 파일은 어디가 문제인지 모르는 채로 엽니다. 그때는 어떤 필드를 걸지부터 막힙니다. 기준선으로 먼저 걸어 볼 필터가 몇 개 있어, 원문의 예제 표를 실무 관점으로 다시 묶어 같은 캡처에 걸었습니다. 대화 필터의 주소와 포트는 lo0 캡처에 맞췄습니다.
...
| `ip.addr == 192.0.2.10 && tcp.port == 80` | 2~7번 여섯 줄 | 80번 포트를 쓴 HOSTA 의 연결 하나입니다 |
...
`http.time` 은 요청 프레임이 아니라 응답 프레임에 붙는 "Time since request" 필드라,[^dfref-http] 느린 요청을 찾으면 응답 줄이 나옵니다. 앞의 셋은 원문의 예제 표에 없지만 같은 문법 체계 안에 있고, 필드 이름은 디스플레이 필터 레퍼런스에서 확인할 수 있습니다.[^dfref]
...
Length 열은 포트 번호와 비슷한 자리에 비슷한 크기의 숫자로 찍혀 헷갈리기 쉽습니다. 공식 문서의 정의는 "각 패킷의 길이"이고,[^wsug-plist] 계산해 보면 무엇을 센 값인지가 드러납니다. 아래는 문서용 주소(RFC 5737)로 만든 캡처를 tshark 4.6.8 로 읽은 두 줄입니다.
...
ARP 요청의 42 는 이더넷 헤더 14바이트에 ARP 28바이트를 더한 값입니다. ARP 는 필드 크기가 고정이라 하드웨어 주소 6바이트, 프로토콜 주소 4바이트일 때 28바이트가 됩니다.[^rfc826] SYN 의 78 은 이더넷 14 에 IP 헤더 20, TCP 헤더 20 을 더하고 TCP 옵션 24바이트를 얹은 값입니다. IP 와 TCP 헤더는 옵션이 없으면 20바이트이고, TCP 옵션은 헤더 끝에 붙어 길이가 운영체제마다 달라집니다.[^rfc791-9293] 그래서 SYN 의 Length 는 고정값이 아닙니다.

두 계산이 모두 이더넷 14바이트에서 출발한다는 점이 요지입니다. Length 는 IP 패킷이나 TCP 세그먼트의 크기가 아니라 링크 계층 프레임 전체를 센 값입니다. lo0 캡처가 이 점을 거꾸로 보여 줍니다. 루프백의 링크 헤더는 이더넷 14바이트가 아니라 4바이트라, 같은 24바이트 옵션을 단 SYN 이 78 이 아니라 68 로 찍혔습니다. ARP 42 와 이더넷 위의 SYN 78 은 en0 캡처에서 확인합니다.
...
`[TCP Retransmission]` 이 찍힌 줄을 보면 대괄호가 무슨 뜻인지부터 걸립니다. 같은 줄의 주소나 길이와 같은 무게로 읽어도 되는지 판단이 서지 않기 때문입니다. 앞 절들에 이미 단서가 있었습니다. §4 의 Protocol 열은 포트로 짐작한 값이었고, §5 의 `Frame` 줄은 선을 지나간 헤더가 아니었습니다. 이 둘을 한 기준으로 모으면 화면의 글자는 세 갈래로 나뉩니다.

그림은 아래 한 줄을 세 번 다시 그려 갈래마다 가진 조각만 남긴 것이고, 번호와 시각 열은 뺐습니다. 문서용 주소(RFC 5737)로 만든 캡처를 tshark 4.6.8 로 읽은 출력입니다.
...
대괄호는 Wireshark 의 표기 규칙입니다. 공식 문서는 캡처 데이터에 없는 정보를 Wireshark 가 스스로 만들어 붙일 때 대괄호로 감싼다고 적고, 그 예로 응답 시간과 TCP 분석, IP 위치 정보, 체크섬 검증을 듭니다.[^wsug-generated] 그래서 `[TCP Retransmission]` 은 선 위에 그런 표시가 있었다는 뜻이 아닙니다. Wireshark 가 앞선 프레임의 시퀀스 번호와 비교해 내린 결론입니다.[^wsug-tcp]

판단은 캡처에 들어온 프레임만 가지고 내립니다. 위 줄의 `[TCP Previous segment not captured]` 는 시퀀스 번호가 기대한 값보다 크다는 뜻인데,[^wsug-tcp] 실제로 보내지 않은 것인지 캡처가 놓친 것인지는 이 판단만으로 가를 수 없습니다. 공식 문서도 자동으로 분석하기 어려운 캡처가 있다고 따로 적어 둡니다.[^wsug-tcp]
...
원문(1.12, 2015)과 현행(4.6, 2026)을 맞대 보면 이 편에서 달라진 것은 §3 에 녹인 두 가지입니다. 하나는 `ssl` 필터의 이름이 `tls` 로 바뀐 것입니다. 다른 하나는 필터 문법 규칙입니다. 필드 이름은 지금도 대소문자를 가리며 영문 연산자만 5.0 부터 대소문자를 가리지 않게 됐습니다. 네 개 창 구성과 메뉴 경로는 원문에 적힌 그대로 씁니다.
...
- [ ] tshark 로 읽을 때 색을 켜는 옵션과, 색을 볼 수 없는 자리에서 걸린 규칙 이름을 글자로 뽑는 명령을 쓸 수 있습니까?
- [ ] `-Y`·`-V`·`-O`·`-x` 가 각각 GUI 의 어느 칸을 대신하는지 말하고, 파일을 읽을 때 `-f` 가 왜 거절되는지 설명할 수 있습니까?
