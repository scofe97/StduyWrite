...
source:
  ...
  - https://www.rfc-editor.org/rfc/rfc9293.txt  # RFC 9293 §3.1·§3.4·§3.8.4·§3.10.4·§3.10.7.4·§4 축자 인용 (2026-09-15 조회)
related:
  - ./README.md
...
여는 쪽은 단순합니다. 세 번의 왕복으로 양쪽이 서로의 시작 시퀀스 번호를 확인하면 끝입니다. 확인 번호는 상대가 보낸 시퀀스 번호에 그 세그먼트가 차지한 번호 수를 더한 값입니다. 데이터가 없는 SYN 도 번호 하나를 차지하므로 handshake 에서는 +1 이 두 번 나타나고, 이 규칙을 알면 Wireshark 화면의 숫자가 읽힙니다.
...
| 4-way 종료 | 닫을 때 오가는 네 번 | 양쪽이 각자 닫음 · half-close 가 가능해서 | §5 |
| TCP 상태 (TCP states) | 연결이 지나는 이름 붙은 단계 | 패킷과 짝지어 읽음 · 전이도로 봄 | §6 |
...
확인 번호 칸의 설명이 이 표에서 가장 쓸모 있습니다. **`tcp.ack` 는 "내가 받은 마지막 바이트"가 아니라 "내가 다음에 받고 싶은 바이트"입니다.** RFC 9293 도 이 칸을 "the value of the next sequence number the sender of the segment is expecting to receive" 로 정의합니다.[^rfc9293-hdr]
...
RFC 9293 이 세그먼트 길이를 "the number of octets occupied by the data in the segment (counting SYN and FIN)" 로 정의하는 것이 이 식의 뒷부분입니다.[^rfc9293-space] **+1 이 되는 것은 데이터 없는 SYN·FIN 이거나, SYN·FIN 없이 데이터가 정확히 1바이트일 때입니다.** 데이터가 29바이트면 +29 입니다. 29바이트를 실은 FIN 이면 +30 이고, 데이터 없는 ACK 면 +0 입니다. 다음 절의 숫자와 §4 의 검산이 이 식대로 이어집니다.
...
handshake 의 세 패킷에는 데이터가 한 바이트도 없습니다. 셋 다 데이터 없이 번호·플래그·윈도우만 싣는 **0바이트 세그먼트**입니다. 그런데 서버의 확인 번호는 클라이언트 ISN + 1 이고, 클라이언트의 확인 번호도 서버 ISN + 1 입니다. 데이터가 없는데 무엇을 하나 센 것인지가 여기서 막힙니다.
...
SYN 도 도중에 사라질 수 있으니 다시 보내야 합니다. 다시 보낸 SYN 과 먼저 보낸 SYN 을 헷갈리지 않고 확인하고 여러 사본 가운데 한 사본만 처리되게 하려면, SYN 자체에 번호가 있어야 합니다. 그래서 SYN 은 데이터가 없어도 번호 하나를 차지하고, 그 번호를 확인하는 값이 ISN + 1 입니다.

헤더 정의도 같은 말을 합니다. 시퀀스 번호 칸의 정의가 "If SYN is set, the sequence number is the initial sequence number (ISN) and the first data octet is ISN+1." 로 이어집니다.[^rfc9293-hdr] 셋째 패킷의 클라이언트 SEQ 가 3613047130 인 까닭입니다. ISN 은 SYN 이 쓰고, 첫 데이터 바이트는 ISN + 1 부터 셉니다.
...
**반대로 ACK 비트는 번호를 차지하지 않습니다.** RFC 9293 용어집은 ACK 를 "A control bit (acknowledge) occupying no sequence space" 로 정의합니다.[^rfc9293-noack] 그래서 ACK 만 실은 셋째 패킷 뒤에도 클라이언트의 다음 번호는 3613047130 그대로이고, §5 에서 서버 FIN 이 싣는 확인 번호도 3613047130 입니다.

그래서 정상 흐름에서 데이터 없는 ACK 는 확인받는 대상이 아닙니다. 사라져도 다시 보내지지 않습니다. 같은 RFC 가 "ACK segments that contain no data are not reliably transmitted by TCP" 라고 적습니다.[^rfc9293-noack] 데이터 없는 ACK 하나하나에 대한 확인이 따로 오지 않는 까닭입니다.
...
handshake 뒤의 패킷 목록에는 데이터가 있는 것과 확인만 하는 것이 섞여 있습니다. 확인만 하는 것은 데이터 없이 번호·플래그·윈도우만 싣는 0바이트 세그먼트입니다. 둘을 눈으로 가르려면 Length 열을 일일이 봐야 하는데, 필터로 가르는 편이 빠릅니다.
...
다섯 번째 항목의 ACK 는 이 패킷만의 특징이 아닙니다. 연결이 성립한 뒤의 세그먼트에는 RST 로 끊는 경우 같은 예외를 빼면 ACK 비트가 켜져 있고, 그 근거는 §5 에서 FIN 과 함께 봅니다.

packet#4 로 §2 의 식을 검산할 수 있습니다. 데이터 세그먼트의 SEQ 는 첫 바이트의 번호이고, 그다음 세그먼트의 SEQ 는 데이터 크기만큼 나아갑니다.[^rfc9293-hdr]
...
세 값은 모두 원문에 있습니다. 29바이트가 번호 29개를 차지했으므로 서버의 다음 세그먼트인 FIN 은 2581725299 에서 시작합니다. 원문의 packet#4 Details 스크린샷에도 `Sequence number: 2581725270` 이 `[TCP Segment Len: 29]`·`[Next sequence number: 2581725299]` 와 함께 찍혀 있어 이 식과 맞습니다.
...
두 번째 항목의 `[FIN,ACK]` 에 ACK 가 붙는 것은 FIN 을 확인하려는 것이 아닙니다. RFC 9293 은 확인 번호 칸에 대해 "Once a connection is established, this is always sent." 라고 적습니다.[^rfc9293-hdr] 연결이 성립한 뒤에는 RST 로 끊는 경우 같은 예외를 빼면 세그먼트가 ACK 비트를 켜고 다음에 받을 번호를 싣습니다.[^rfc9293-hdr] packet#5 의 ACK 도 평소처럼 클라이언트에게서 다음에 받을 번호 3613047130 을 싣고 있을 뿐이고, §4 의 `[PSH,ACK]` 에 ACK 가 붙는 것도 같은 이유입니다.

예제는 서버가 먼저 닫으므로, 일반형에 대면 서버가 먼저 닫는 쪽이고 클라이언트가 FIN 을 받는 쪽입니다. 서버가 `FIN_WAIT-1`·`FIN_WAIT-2`·`TIME_WAIT` 을 지나고, 클라이언트가 `CLOSE_WAIT`·`LAST_ACK` 을 지납니다. 원문 종료 절의 패킷 목록 스크린샷과 전이 표로 #4~#9 를 이어 놓으면 이렇습니다.
...
번호와 방향은 모두 원문 스크린샷의 패킷 목록에 찍혀 있고, #9 의 확인 번호 3613047131 은 원문이 빨간 상자로 짚어 둔 값입니다. 상태는 같은 절의 Sr. No. 1~10 전이 표에서 읽었습니다. 이 표에는 패킷 번호 열이 없고 인쇄본에서 서버 상태 열의 오른쪽이 잘려 `FIN_W`·`TIME_` 처럼 앞글자만 보이므로, 잘린 칸은 다음 행의 시작 상태로 이어 읽었습니다.

#6 은 서버 데이터 29바이트에 대한 클라이언트의 확인입니다. 서버 FIN 인 #5 뒤에 잡혔지만 확인 번호가 2581725299 라서 FIN 까지는 확인하지 않고, 서버는 `FIN_WAIT-1` 에 그대로 있습니다. FIN 을 확인하는 것은 확인 번호가 하나 더 큰 #7 입니다. 원문 표 6행은 이 ACK 를 FIN 앞에 두고 서버를 ESTABLISHED 로 적었으므로, #6 행의 서버 상태는 표가 아니라 캡처 순서에서 읽은 값입니다.

전이 표의 두 행은 스크린샷과 다릅니다. 표 5행은 데이터 세그먼트를 SEQ 3613047130 · ACK 2581725270 으로 적었지만 스크린샷의 #4 는 SEQ 2581725270 · ACK 3613047130 이어서, 표에서 두 값이 서로 바뀌어 있습니다. 표 10행은 서버를 `TIME_WAIT` 에서 CLOSED 로 옮겨 적었지만 CLOSED 로 가는 것은 2×MSL 이 지난 뒤라([03-02 §3](./03-02.TCP가 어긋날 때.md)), #9 행에는 유지로 적었습니다.

여기서 눈여겨볼 것은 packet#7 과 packet#8 사이입니다. **두 패킷을 만들어 보내는 것은 둘 다 클라이언트 커널입니다. 다른 것은 계기입니다.** #7 은 상대 FIN 이 도착한 것이 계기라 커널이 곧바로 보냅니다. RFC 9293 §3.10.7.4 는 FIN 이 도착했을 때의 처리를 "send an acknowledgment for the FIN" 으로 적습니다. ESTABLISHED 였다면 그와 함께 CLOSE-WAIT 로 들어갑니다.[^rfc9293-close]
...
원문이 가리키는 것은 Packet Details 창의 **"SEQ/ACK analysis" 하위 트리**입니다. 그 안에서 확인한 세그먼트로 이어 주는 필드가 `tcp.analysis.acks_frame`("This is an ACK to the segment in frame")입니다. Wireshark 가 붙인 판정을 한꺼번에 보려면 §3 의 `tcp.analysis.flags` 를 씁니다.
...
애플리케이션이 이미 놓아 버린 고아 연결이 `FIN_WAIT_2` 에 머무는 시간은 커널 파라미터 `tcp_fin_timeout` 이 정하고, 기본값은 **60초**입니다. 커널 문서는 이 값을 "어떤 애플리케이션도 더 이상 참조하지 않는 고아 연결이 로컬에서 중단되기 전까지 `FIN_WAIT_2` 상태에 머무는 시간"으로 정의합니다. 고아가 아닌 연결의 `FIN_WAIT_2` 는 받기만 하는 온전한 상태라고도 적습니다.[^ip-sysctl] 커널 문서가 이 값으로 정의하는 상태는 `FIN_WAIT_2` 하나입니다.
