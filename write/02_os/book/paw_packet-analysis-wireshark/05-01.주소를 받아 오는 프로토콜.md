...
세그먼트 경계는 캡처에서도 드러납니다. 주소가 없는 클라이언트가 보내는 첫 메시지는 브로드캐스트(IPv4)나 멀티캐스트(IPv6)로 나가므로, 스위치 환경에서도 같은 세그먼트라면 남의 장비의 DHCP 요청이 내 캡처에 잡힙니다. 유니캐스트를 보려면 TAP 이나 SPAN 이 필요했던 것과 다릅니다. 반대로 말하면 다른 장비가 주소를 못 받고 같은 요청을 되풀이하는 모습도 내 자리에서 관찰할 수 있습니다.
