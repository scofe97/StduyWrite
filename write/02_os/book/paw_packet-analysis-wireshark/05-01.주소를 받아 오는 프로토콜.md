...
캡처를 떴는데 조사하려던 통신이 하나도 없습니다. 애플리케이션 문제로 보이지만, 그 장비가 애초에 주소를 못 받았다면 나갈 패킷 자체가 없었던 것입니다. 요즘 장비는 주소를 손으로 넣지 않고 켜질 때 스스로 받아 오는데, 이것을 주소 **자동 구성**(autoconfiguration)이라 부르고 이 편의 DHCP 가 그 일을 합니다. 그 경우 확인할 것은 애플리케이션이 아니라 캡처의 맨 앞입니다.
...
세그먼트 경계는 캡처에서도 드러납니다. IPv4 클라이언트의 첫 메시지는 브로드캐스트라 스위치가 모든 포트로 복사하므로, 스위치 환경에서도 같은 세그먼트라면 남의 장비의 DHCP 요청이 내 캡처에 잡힙니다. 유니캐스트를 보려면 TAP 이나 SPAN 이 필요했던 것과 다릅니다.

IPv6 는 조건이 하나 붙습니다. 첫 메시지가 서버·릴레이가 가입하는 멀티캐스트 그룹 `ff02::1:2` 로 가는데, 스위치가 MLD 스누핑을 켜 두면 가입한 포트로만 보냅니다. 모든 포트로 보내는 예외는 전체 호스트 주소 `ff02::1` 하나뿐입니다.[^rfc4541] 그래서 스누핑하는 스위치에서는 서버도 릴레이도 아닌 내 자리에 남의 SOLICIT 가 오지 않을 수 있습니다. 반대로 말하면 다른 장비가 주소를 못 받고 같은 요청을 되풀이하는 모습도 내 자리에서 관찰할 수 있습니다.
...
2. 클라이언트는 링크 로컬 주소를 출발지로 씁니다. 링크 로컬 주소는 인터페이스가 스스로 만드는 `fe80::` 로 시작하는 주소라, 아직 받은 주소가 없어도 이 링크 안에서는 이 주소로 말할 수 있습니다.
3. 목적지는 링크 범위 멀티캐스트 주소입니다. 이 문장에서 원문은 값을 적지 않고, 뒤의 주소 표가 그 값 `ff02::1:2` 를 줍니다. RFC 8415 는 이렇게 하는 이유를 "클라이언트가 DHCP 서버의 주소를 설정해 둘 필요가 없도록" 이라고 적습니다.[^rfc8415-udp]
...
여기서 필드 하나를 먼저 짚습니다. 트랜잭션 ID 는 클라이언트가 교환마다 새로 뽑아 메시지에 싣는 번호이고, 서버는 답에 같은 번호를 돌려줍니다. 그래서 같은 번호끼리 묶으면 어느 답이 어느 요청의 것인지가 보입니다. 원문 캡처에서 SOLICIT·ADVERTISE 는 `0x10eafe` 를 나눠 가집니다.

**`REQUEST`** 에서 클라이언트가 서버 하나를 고릅니다. 원문이 짚는 것은 셋입니다. 여전히 멀티캐스트로 보냅니다. **새 트랜잭션 ID**(`0x3ec03e`)를 담습니다. 고른 서버의 식별자를 넣습니다.

**`REPLY`** 에서 서버가 확정합니다. 서버는 자기 정책과 설정에 따라 그 클라이언트를 위한 바인딩, 곧 어느 클라이언트에게 어느 주소를 줬는지 적은 장부 기록을 만들고, 요청받은 IA 와 정보를 기록한 뒤 `dhcpv6.msgtype == 7` 로 보냅니다. 트랜잭션 ID 는 REQUEST 의 것과 같아야 합니다.

앞 두 메시지가 `0x10eafe`, 뒤 두 메시지가 `0x3ec03e` 로 번호가 다르다는 것은 REQUEST 가 SOLICIT 의 이어짐이 아니라 새 요청이라는 뜻입니다. 캡처에서 짝을 찾을 때는 메시지 순서보다 이 번호를 봅니다.
...
IPv4 도 고르는 대상이 주소가 아니라 서버라는 점은 같고, 고른 결과를 알리는 방식은 [§5](#request-한-번의-브로드캐스트가-선택과-거절을-겸합니다)에서 봅니다.
...
새 주소를 받을 때도 두 걸음으로 줄이는 길이 있습니다. 원문이 특히 짚는 rapid commit 옵션입니다. "rapid commit 은 별도의 DHCPv6 메시지가 아니라 SOLICIT 의 옵션의 일부" 라는 점이 중요합니다. 동작 규칙은 셋입니다.
...
위 도식은 SARR 기준입니다. IPv4 의 DORA 에도 같은 발상의 rapid commit 옵션이 있습니다. 원문은 BOOTP/DHCP 절의 주소 할당 대목에서 이렇게 덧붙입니다. "DHCPv6 를 본떠 만들어졌으며, DHCPv4 클라이언트를 빠르게 설정하기 위해 두 메시지 교환을 씁니다."
...
| 옵션 | 이름 | 싣는 쪽 |
|---|---|---|
| 53 | DHCP 메시지 종류 (1 DISCOVER · 2 OFFER · 3 REQUEST · 5 ACK · 6 NAK) | 모두 |
| 55 | 파라미터 요청 목록 | 클라이언트 |
...
**`REQUEST`**(`== 3`) — 클라이언트가 다시 브로드캐스트합니다. 반드시 옵션 54 를 담아 어느 서버를 골랐는지 알려야 합니다. 원문은 이유를 적지 않았지만, 여러 서버가 OFFER 를 보냈을 수 있기 때문입니다(아래 §5). 원문은 이어서 REQUEST 에서 선택된 서버가 그 클라이언트의 바인딩을 저장소에 기록하고 ACK 로 응답한다고 적습니다.
...
§1 에서 남의 DHCP 요청은 옆자리 캡처에 잡힌다고 했습니다. 서버의 답은 사정이 다릅니다. 주소가 없는 클라이언트는 자기 앞으로 온 유니캐스트를 받지 못할 수 있어서, 그런 클라이언트는 DISCOVER·REQUEST 의 `flags` 필드에 BROADCAST 비트를 켜 둡니다.[^rfc2131-bcast] 릴레이 없이(`giaddr` 0) 아직 주소가 없는(`ciaddr` 0) 클라이언트에게 답할 때, 서버는 이 비트를 보고 보내는 방식을 정합니다.
...
DHCPv6 는 이 비트가 없습니다. 클라이언트가 처음부터 링크 로컬 주소를 갖고 있으므로 ADVERTISE·REPLY 는 그 주소로 가는 유니캐스트입니다. 그래서 옆자리 캡처에 요청만 있고 답이 없으면, 서버가 침묵한 것인지 답이 유니캐스트라 내 포트에 오지 않은 것인지부터 가려야 합니다.
...
서버가 둘이면 OFFER 도 둘 옵니다. `yiaddr` 칸은 4바이트 하나라 OFFER 하나에는 주소 하나만 실리므로, §2 의 IPv6 처럼 클라이언트가 고르는 대상은 주소가 아니라 서버입니다.[^rfc2131-choose] 클라이언트는 하나를 골라 옵션 54 에 그 서버의 식별자를 넣고 REQUEST 를 한 번만 브로드캐스트합니다. 고르지 않은 서버에게 따로 거절을 보내지 않습니다.
...
이름이 같아도 TCP 헤더의 ACK 와는 다른 것입니다. DHCP 의 ACK·NAK 은 "받았다"는 신호가 아니라 옵션 53 의 값 5·6 으로 구분되는 메시지 종류이고, ACK 는 "장부에 적었다"는 확정, NAK 은 "들어줄 수 없다"는 거절입니다. UDP 위라 메시지가 사라질 수 있지만[^rfc768] 그 빈자리는 클라이언트의 재전송이 메웁니다.[^rfc2131-rtx]
...
§4 의 "ARP 로 다시 확인"은 전제 하나를 깔고 있습니다. ARP 요청은 MAC 브로드캐스트(`ff:ff:ff:ff:ff:ff`)로 나갑니다. RFC 826 은 주소를 묻는 장비가 요청을 "케이블 위의 모두에게" 브로드캐스트하고, 그 주소의 주인만 브로드캐스트가 아닌 직접 응답을 돌려준다고 적습니다.[^rfc826] 그러니 방금 받은 주소를 ARP 로 물었는데 응답이 오면, 세그먼트 안 누군가가 이미 그 주소를 쓰고 있는 것입니다.

그 누군가가 DHCP 클라이언트일 필요는 없습니다. 원문은 ARP 가 잡는 상대를 "다른 DHCP 클라이언트"로 적습니다. RFC 2131 은 쓰는 쪽을 가리지 않고 "이미 쓰이는 주소"라고만 적습니다.[^rfc2131-arp] 서버 장부에 없는 장비도 걸린다는 뜻입니다. 이 차이가 아래에서 두 확인을 나누는 근거가 됩니다.
