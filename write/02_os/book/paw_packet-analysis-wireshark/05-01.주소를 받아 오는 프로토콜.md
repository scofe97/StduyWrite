...
캡처를 떴는데 조사하려던 통신이 하나도 없습니다. 애플리케이션 문제로 보이지만 그 장비가 애초에 주소를 못 받았다면 나갈 패킷 자체가 없었습니다. 요즘 장비는 주소를 손으로 넣지 않고 켜질 때 스스로 받아 옵니다. 이것을 주소 **자동 구성**(autoconfiguration)이라 부르고 이 편의 DHCP 가 그 일을 합니다. 그 경우 확인할 곳은 캡처의 맨 앞입니다.
