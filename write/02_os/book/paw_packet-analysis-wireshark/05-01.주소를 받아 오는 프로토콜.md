...
다만 서버의 답까지 늘 보이지는 않습니다. IPv4 서버의 OFFER·ACK 는 클라이언트가 BROADCAST 플래그를 켰을 때만 브로드캐스트입니다. 꺼져 있으면 클라이언트 MAC 으로 유니캐스트됩니다.[^rfc2131-srvcast] 거절인 NAK 은 플래그와 상관없이 브로드캐스트입니다. DHCPv6 의 ADVERTISE·REPLY 는 클라이언트의 링크 로컬 주소로 가는 유니캐스트입니다. 옆자리 캡처에 요청만 있고 답이 없다면 서버가 침묵한 것인지, 답이 유니캐스트라 내 포트에 오지 않은 것인지부터 가려야 합니다.
...
REPLY 줄의 "모든 메시지에 대해"에는 예외가 하나 있습니다. RFC 8415 가 REPLY 로 답한다고 꼽는 클라이언트 메시지는 REQUEST·RENEW·REBIND·CONFIRM·RELEASE·DECLINE·INFORMATION-REQUEST, 그리고 rapid commit 이 붙은 SOLICIT 입니다.[^rfc8415-reply] 옵션이 없는 SOLICIT 에는 REPLY 가 아니라 ADVERTISE 가 옵니다.
...
여기의 ACK·NAK 은 TCP 헤더의 ACK 와 이름만 같습니다. TCP 의 ACK 는 전송 계층이 바이트를 받았다고 알리는 헤더 플래그입니다. DHCP 의 ACK·NAK 은 응용 계층의 메시지 종류입니다. DHCP 는 UDP 위에서 돕니다. RFC 768 은 UDP 가 전달을 보장하지 않는다고 적습니다.[^rfc768]

그렇다고 DHCP 의 ACK 가 그 빈자리를 메우는 수신 확인은 아닙니다. 잃어버린 메시지는 클라이언트가 재전송으로 메우며, RFC 2131 은 재전송을 전적으로 클라이언트의 책임으로 둡니다.[^rfc2131-rtx] ACK 는 "장부에 적었다"는 확정이고 NAK 은 "들어줄 수 없다"는 거절입니다. 둘은 옵션 53(DHCP 메시지 종류)의 값 5 와 6 으로 구분됩니다.
