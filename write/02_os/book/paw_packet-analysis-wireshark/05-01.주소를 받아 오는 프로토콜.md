...
source:
  ...
  - https://www.rfc-editor.org/rfc/rfc2132.txt  # RFC 2132 옵션 53 값 1~8 (2026-09-26 조회)
  - https://www.rfc-editor.org/rfc/rfc4861.txt  # RFC 4861 라우터 광고 M·O 플래그 (2026-09-26 조회)
  - https://www.rfc-editor.org/rfc/rfc4862.txt  # RFC 4862 SLAAC (2026-09-26 조회)
  - https://www.rfc-editor.org/rfc/rfc3646.txt  # RFC 3646 DHCPv6 DNS 옵션 23 (2026-09-26 조회)
  - https://www.rfc-editor.org/rfc/rfc4541.txt  # RFC 4541 MLD 스누핑 (2026-09-26 조회)
related:
  - ./README.md
  - ./04-03.열쇠와 실패.md
  - ./05-02.이름과 요청.md
...
> 이 편이 매달린 하나의 생각은 주소를 받는 절차가 IPv4 든 IPv6 든 "찾고 · 제안받고 · 고르고 · 확정받는" **네 걸음**으로 같아서 한쪽을 읽을 줄 알면 다른 쪽도 읽힌다는 것입니다.
...
| 호스트 식별자 | MAC 주소 | DUID (DHCP 고유 식별자) |
| 거절 | `NAK` 메시지 | 상태 코드 NoAddrsAvail |

캡처에서 이 교환을 찾는 일이 실무에서 자주 필요합니다. 주소를 못 받으면 그 뒤의 모든 조사가 무의미합니다. 받았다면 몇 번째 메시지까지 갔는지가 어디서 막혔는지를 말해 줍니다.
...
| DHCP (Dynamic Host Configuration Protocol) | 주소와 네트워크 설정을 자동으로 나눠 주는 프로토콜 | 캡처의 맨 앞 · IPv4 판은 DORA 네 메시지 | §1 · §4 |
| DHCPv6 | IPv6 주소를 받아 오는 프로토콜 | 주소를 모를 때 쓰므로 멀티캐스트로 시작 | §2 |
| SLAAC | 라우터 광고로 주소를 스스로 만듦 | DHCPv6 가 안 보여도 주소가 있는 이유 · M·O 플래그 | §2 |
| SARR | DHCPv6 의 네 메시지 | Solicit·Advertise·Request·Reply | §2 |
| DUID | IPv6 의 호스트 식별자 | IPv4 의 MAC 자리 · 서버 식별자 옵션에 실림 | §2 |
| rapid commit | 네 걸음을 두 걸음으로 줄임 | 왕복을 아낌 · 양쪽이 지원해야 | §3 |
| DORA | DHCPv4 의 네 메시지 | Discover·Offer·Request·Ack | §4 |
| `yiaddr` | 서버가 주는 주소 칸 | OFFER 에 값이면 제안 · ACK 에 같으면 확정 | §4 |
| DHCPNAK | 서버가 REQUEST 를 거절하는 메시지 | 무응답과 다름 · IPv6 는 상태 코드로 | §5 |
| BOOTP | DHCP 의 전신 | 필터 이름이 `bootp` 였던 이유 · 헤더를 물려줌 | §4 · §6 |



## 1. 주소를 못 받으면 아무것도 안 됩니다

> DHCP 는 통신이 아니라 통신의 전제를 만드는 프로토콜입니다. 그래서 캡처의 맨 앞에 오며 여기서 막히면 뒤가 전부 비어 있습니다.

### 아무 트래픽도 안 잡힐 때

캡처를 떴는데 조사하려던 통신이 하나도 없습니다. 애플리케이션 문제로 보이지만 그 장비가 애초에 주소를 못 받았다면 나갈 패킷 자체가 없었습니다. 요즘 장비는 주소를 손으로 넣지 않고 켜질 때 스스로 받아 옵니다. 이것을 주소 **자동 구성**(autoconfiguration)이라 부르고 이 편의 **DHCP**(Dynamic Host Configuration Protocol, 동적 호스트 구성 프로토콜)가 그 일을 합니다. 그 경우 확인할 곳은 캡처의 맨 앞입니다.
...
### 같은 세그먼트라면 남의 DHCP 요청이 내 캡처에 잡힙니다

세그먼트 경계는 캡처에서도 드러납니다. IPv4 클라이언트의 첫 메시지는 브로드캐스트라 스위치가 모든 포트로 복사합니다. 그러니 스위치 환경에서도 같은 세그먼트라면 남의 장비의 DHCP 요청이 내 캡처에 잡힙니다. 유니캐스트를 보려면 TAP 이나 SPAN 이 필요했던 것과 다릅니다.

IPv6 는 스위치 설정에 따라 결과가 달라집니다. 그 조건은 이 절 끝의 「IPv6 는 스위치의 MLD 스누핑에 따라 달라집니다」에서 봅니다.

다만 서버의 답까지 늘 보이지는 않습니다. 서버가 답을 누구에게 어떤 방식으로 보내는지는 네 걸음의 이름을 배운 뒤 [§4](#서버의-답은-옆자리에-보이는가--broadcast-플래그)에서 봅니다.

브로드캐스트와 유니캐스트가 닿는 범위의 차이는 1장의 도식에 그대로 있습니다. 가운데 열이 분석기 자리이고, 브로드캐스트 줄은 그 칸까지 도착합니다. 남의 유니캐스트 줄은 그 칸에 오지 않습니다.

![주소 종류가 정하는 도착 범위](./_assets/01-01.address-reach.svg)

### IPv6 는 스위치의 MLD 스누핑에 따라 달라집니다

IPv6 클라이언트의 첫 메시지는 브로드캐스트로 가지 않습니다. 서버와 릴레이가 가입하는 멀티캐스트 그룹 `ff02::1:2` 로 갑니다. 스위치는 원래 멀티캐스트도 브로드캐스트처럼 모든 포트로 복사합니다. MLD 스누핑은 스위치가 그룹 가입을 알리는 MLD 메시지를 엿보고 가입한 포트로만 멀티캐스트를 보내 대역폭을 아끼는 기능입니다.[^rfc4541-snoop] 모든 포트로 보내는 예외는 전체 호스트 주소 `ff02::1` 하나뿐입니다.[^rfc4541]

스누핑을 켠 스위치에서는 서버도 릴레이도 아닌 내 자리에 남의 SOLICIT 가 오지 않을 수 있습니다. 스누핑이 없는 스위치라면 IPv4 처럼, 다른 장비가 주소를 못 받고 같은 요청을 되풀이하는 모습도 내 자리에서 관찰할 수 있습니다.
...
### IPv6 주소를 얻는 세 경로 — SLAAC · 무상태 DHCPv6 · 상태 유지 DHCPv6

원문은 DHCPv6 를 "DHCPv6 클라이언트에게 IPv6 주소와 그 밖의 설정 정보를 제공하는 응용 계층 프로토콜" 로 소개하고 그 정보가 DHCPv6 옵션에 실려 온다고 설명합니다. 그리고 성격을 둘로 나눕니다. DHCPv6 는 **상태 유지** 주소 자동 설정 프로토콜이면서 동시에 **무상태** 주소 설정 프로토콜입니다.

상태 유지는 서버가 어느 클라이언트에게 어느 주소를 줬는지 장부에 적어 두고 관리하는 방식입니다. 무상태는 주소는 나눠 주지 않고 DNS 서버 목록 같은 설정만 알려 주는 방식입니다. 이 절의 네 걸음은 상태 유지 쪽이고 무상태 쪽은 §3 에서 보는 INFORMATION-REQUEST 교환입니다.

DHCPv6 교환이 캡처에 하나도 없는데 장비가 IPv6 주소를 갖고 있을 수 있습니다. IPv6 에는 DHCPv6 말고도 주소를 얻는 길이 있기 때문입니다. 라우터가 보내는 라우터 광고(Router Advertisement)를 받아 호스트가 스스로 주소를 만드는 SLAAC(Stateless Address Autoconfiguration)입니다.[^rfc4862] 원문은 이 경로를 다루지 않으므로 RFC 로 채웁니다. 어느 경로를 쓸지는 라우터 광고의 M·O 두 플래그가 알려 줍니다.[^rfc4861-mo]

| 경로 | 주소를 정하는 쪽 | 라우터 광고가 알리는 것 | 캡처에 보이는 것 |
|---|---|---|---|
| SLAAC | 호스트 | M·O 둘 다 0 이면 DHCPv6 로 얻을 정보가 없음 | 라우터 광고뿐 |
| 무상태 DHCPv6 | 호스트 (주소 밖의 설정만 DHCPv6) | O=1 이면 DNS 같은 다른 설정을 DHCPv6 로 | 라우터 광고와 INFORMATION-REQUEST·REPLY (§3) |
| 상태 유지 DHCPv6 | DHCPv6 서버 | M=1 이면 주소를 DHCPv6 로 (이때 O 는 무시해도 됨) | 라우터 광고와 SARR (이 절) |
...
# 라우터 광고가 켠 두 플래그
icmpv6.nd.ra.flag.m == 1
icmpv6.nd.ra.flag.o == 1
```

### 주소 없이 서버를 찾는 법 — UDP · 링크 로컬 · 멀티캐스트

주소가 없는 상태에서 어떻게 서버를 찾는지가 이 프로토콜의 첫 문제입니다. 원문의 답은 세 줄입니다. 원문은 이유를 붙이지 않았으므로 각 줄이 그 문제의 어느 조각을 푸는지는 노트가 덧붙였습니다.
...
2. 클라이언트는 링크 로컬 주소를 출발지로 씁니다. 링크 로컬 주소는 인터페이스가 스스로 만들고 `fe80::` 로 시작합니다. 아직 받은 주소가 없어도 이 링크 안에서는 이 주소로 말할 수 있습니다.
3. 목적지는 링크 범위 멀티캐스트 주소입니다. 이 문장에서 원문은 값을 적지 않고, 뒤의 주소 표가 그 값 `ff02::1:2` 를 줍니다. RFC 8415 는 이렇게 하는 이유를 "클라이언트가 DHCP 서버의 주소를 설정해 둘 필요가 없도록" 이라고 밝힙니다.[^rfc8415-udp]

멀티캐스트도 라우터를 넘지 않으므로 서버가 같은 링크에 없으면 위 세 줄만으로는 닿지 않습니다. 원문에 따르면 그때는 클라이언트 링크의 DHCPv6 릴레이 에이전트가 메시지를 중계합니다. 앞 절의 세그먼트 문제와 같은 문제이고 해결책도 같습니다.
...
REPLY 줄의 "모든 메시지에 대해"에는 예외가 하나 있습니다. RFC 8415 가 REPLY 로 답한다고 꼽는 클라이언트 메시지는 REQUEST·RENEW·REBIND·CONFIRM·RELEASE·DECLINE·INFORMATION-REQUEST, 그리고 §3 의 rapid commit 이 붙은 SOLICIT 입니다.[^rfc8415-reply] 옵션이 없는 SOLICIT 에는 REPLY 가 아니라 ADVERTISE 가 옵니다.

표의 RENEW·REBIND 는 주소에 기한이 있어서 생긴 메시지입니다. 서버가 준 주소는 영구 할당이 아니라 수명이 있는 임대(lease)이고 서버는 IA 옵션에 T1·T2 두 시간을 실어 줍니다. T1 이 지나면 클라이언트는 주소를 준 그 서버에 RENEW 를 보내 수명을 늘립니다. T2 가 지나도록 연장하지 못하면 아무 서버에나 REBIND 를 보냅니다.[^rfc8415-t1t2] 캡처에서 RENEW 가 되풀이되다 REBIND 로 바뀌면 처음 서버에게서 연장을 받지 못한 경우입니다.

### 네 걸음이 각각 확정하는 것

원문은 `DHCPv6-Flow-SOLICIT.pcap` 으로 SARR 을 짚습니다. 위 그림이 그 흐름이고, 각 걸음이 담는 것은 이렇습니다.

네 걸음에는 DHCPv6 옵션 번호가 여럿 나옵니다. 번호만 보고 읽지 않도록 이 편에서 쓰는 것을 먼저 모아 둡니다.[^rfc8415-opts] 필터는 모두 `dhcpv6.option.type == 번호` 꼴입니다.

| 코드 | 이름 | 뜻 | 이 편에서 보는 자리 |
|---|---|---|---|
| 1 | Client Identifier | 클라이언트의 DUID | SOLICIT |
| 2 | Server Identifier | 서버의 DUID | ADVERTISE · REQUEST |
| 3 | IA_NA | 비임시 주소 묶음 | SOLICIT · ADVERTISE · REPLY |
| 4 | IA_TA | 임시 주소 묶음 | 임시 주소가 필요할 때 |
| 6 | ORO | 받고 싶은 옵션 목록 | SOLICIT |
| 7 | Preference | 서버의 선호도 값 | ADVERTISE (아래 서버 선택) |
| 9 | Relay Message | 릴레이가 싼 원래 메시지 | RELAY-FORWARD (§5) |
| 13 | Status Code | 처리 결과 코드 | NoAddrsAvail (§5) |
| 14 | Rapid Commit | 두 걸음 교환 요청 | SOLICIT · REPLY (§3) |
| 23 | DNS Recursive Name Server | DNS 서버 목록 | ADVERTISE |
...
**`ADVERTISE`** 에서 서버가 자기를 알립니다. 원문에 따르면 여러 서버가 응답할 수 있고 클라이언트는 선호도에 따라 고릅니다. 서버가 담는 것은 자기 선호에 맞춘 IA_NA 값, 서버 식별자(`== 2`), 그리고 SOLICIT 이 요청한 이름 서버 정보(`== 23`)입니다.
...
여기서 필드 하나를 먼저 짚습니다. 트랜잭션 ID 는 클라이언트가 교환마다 새로 뽑아 메시지에 싣는 번호이고, 서버는 답에 같은 번호를 돌려줍니다. 같은 번호끼리 묶으면 어느 답이 어느 요청의 것인지가 보입니다. 원문 캡처에서 SOLICIT·ADVERTISE 는 `0x10eafe` 를 나눠 가집니다.

**`REQUEST`** 에서 클라이언트가 서버 하나를 고릅니다. 원문이 짚는 것은 셋입니다. 여전히 멀티캐스트로 보냅니다. **새 트랜잭션 ID**(`0x3ec03e`)를 담습니다. 고른 서버의 식별자를 넣습니다.

**`REPLY`** 에서 서버가 확정합니다. 서버는 자기 정책과 설정에 따라 그 클라이언트의 바인딩을 만듭니다. 바인딩은 어느 클라이언트에게 어느 주소를 줬는지 적은 장부 기록입니다. 서버는 요청받은 IA 와 정보를 기록한 뒤 `dhcpv6.msgtype == 7` 로 보냅니다. 트랜잭션 ID 는 REQUEST 의 것과 같아야 합니다.

앞 두 메시지는 `0x10eafe`, 뒤 두 메시지는 `0x3ec03e` 로 번호가 다릅니다. REQUEST 가 SOLICIT 을 이어 가는 메시지가 아니라 새 요청이라는 뜻입니다. 캡처에서 짝을 찾을 때는 메시지 순서보다 이 번호를 봅니다.
...
고르는 기준은 서버가 정합니다. 서버는 ADVERTISE 에 Preference 옵션으로 선호도 값을 실을 수 있습니다. RFC 8415 는 클라이언트가 이 값이 가장 높은 ADVERTISE 를 우선하라(SHOULD)고 규정합니다. 다만 제안한 설정이 더 나은 서버라면 선호도가 낮아도 고를 수 있습니다(MAY).[^rfc8415-pref]
...
IPv4 도 고르는 대상이 서버라는 점은 같습니다. 고른 결과를 알리는 방식은 [§5](#request-한-번의-브로드캐스트가-선택과-거절을-겸합니다)에서 봅니다.
...
### 주소가 필요 없으면 요청 하나에 답 하나로 끝납니다

DNS 서버 목록만 알고 싶은데 주소 할당 절차를 통째로 도는 것은 낭비입니다. 원문은 그런 경우를 두 메시지 교환으로 분류합니다. 원문이 이유를 적지는 않았습니다. 노트가 덧붙이면, 이 교환들은 여러 서버의 제안을 비교해 새 주소를 받아 오는 일이 아니라서 고르는 단계가 없습니다. 설정만 묻거나 이미 가진 주소를 확인·반납하는 요청 하나에 서버가 답 하나를 돌려주면 끝납니다.
...
### 새 주소도 두 걸음으로 — rapid commit

새 주소를 받을 때도 두 걸음으로 줄이는 길이 있습니다. rapid commit 옵션입니다. 원문은 "rapid commit 은 별도의 DHCPv6 메시지가 아니라 SOLICIT 의 옵션의 일부" 라고 설명합니다. 동작 규칙은 셋입니다.
...
위 도식은 SARR 기준입니다. IPv4 의 DORA(§4)에도 같은 발상의 rapid commit 옵션이 있습니다. 원문은 BOOTP/DHCP 절의 주소 할당 대목에서 이렇게 덧붙입니다. "DHCPv6 를 본떠 만들어졌으며, DHCPv4 클라이언트를 빠르게 설정하기 위해 두 메시지 교환을 씁니다."

캡처에서 이것을 읽는 요령이 있습니다. SOLICIT 다음에 ADVERTISE 없이 바로 REPLY 가 오면 **rapid commit** 이 쓰인 경우입니다. 두 메시지만 보여도 정상일 수 있으므로 메시지 수만 세고 판단하면 안 됩니다.



## 4. IPv4 의 DORA — `yiaddr` 로 진행을 읽습니다
...
IPv4 의 DHCP 메시지는 고정 헤더 필드와 그 뒤의 옵션으로 이뤄집니다. 헤더는 DHCP 의 전신인 BOOTP 에서 그대로 물려받았습니다. 두 프로토콜의 관계는 §6 에서 봅니다. 이 편에서 쓰는 헤더 필드는 일곱입니다.[^rfc2131-fields]

| 필드 | 뜻 | 채우는 쪽 | Wireshark 필드 | 이 편에서 보는 자리 |
|---|---|---|---|---|
| `hops` | 거친 릴레이 수 | 클라이언트가 0 · 릴레이가 늘림 | `dhcp.hops` | §5 릴레이 |
| `xid` | 요청과 응답을 짝짓는 트랜잭션 ID | 클라이언트가 고른 난수 | `dhcp.id` | DHCPv6 트랜잭션 ID(§2)와 같은 역할 |
| `flags` | BROADCAST 비트 | 클라이언트 | `dhcp.flags.bc` | 아래 BROADCAST 플래그 |
| `ciaddr` | 클라이언트가 이미 쓰는 주소 | 클라이언트 (BOUND·RENEW·REBINDING 상태에서만) | `dhcp.ip.client` | 아래 BROADCAST 플래그 |
| `yiaddr` | 서버가 클라이언트에게 주는 주소 | 서버 | `dhcp.ip.your` | 이 절 |
| `giaddr` | 릴레이 에이전트 주소 (릴레이가 없으면 0) | 릴레이 | `dhcp.ip.relay` | §5 릴레이 |
| `chaddr` | 클라이언트 하드웨어(MAC) 주소 | 클라이언트 | `dhcp.hw.mac_addr` | 아래 BROADCAST 플래그 · §5 |

### 네 걸음마다 `yiaddr` 가 어떻게 바뀌나

IPv4 에서 진행을 알려 주는 칸은 `yiaddr` 하나입니다. 이름은 "your IP address" 의 줄임이고 서버가 클라이언트에게 주는 주소를 적는 4바이트 칸입니다.[^rfc2131-choose] 아래 네 걸음에서 이 칸이 언제 비고 언제 채워지는지를 봅니다.

원문은 `DHCPv4.pcap` 으로 흐름을 짚고 필터를 `bootp.` 접두사로 적었습니다. Wireshark 3.0 에서 디섹터 이름이 DHCP 로 바뀌었으므로 아래 필드는 새 이름 `dhcp.` 로 옮겼습니다. 옛 이름도 대부분 아직 먹지만 `bootp.dhcp` 하나는 호환에서 빠졌습니다.[^ws30]

네 걸음에는 옵션 번호가 넷 나옵니다. 번호만 보고 읽지 않도록 먼저 모아 둡니다.

| 옵션 | 이름 | 싣는 쪽 |
|---|---|---|
| 53 | DHCP 메시지 종류 (1 DISCOVER · 2 OFFER · 3 REQUEST · 4 DECLINE · 5 ACK · 6 NAK · 7 RELEASE · 8 INFORM) | 모두 |
| 55 | 파라미터 요청 목록 | 클라이언트 |
| 54 | DHCP 서버 식별자 | 서버가 자기를 밝힐 때 · 클라이언트가 고른 서버를 가리킬 때 |
| 50 | 요청 IP 주소 | 클라이언트가 원하는 주소를 밝힐 때 (REQUEST 등) |
...
DISCOVER 는 IP 헤더의 출발지 주소도 `0.0.0.0` 입니다. 원문에는 없는 내용으로, RFC 2131 은 클라이언트가 주소를 얻기 전에 브로드캐스트하는 DHCP 메시지라면 출발지 주소를 0 으로 둔다고 정합니다.[^rfc2131-src] 뒤의 REQUEST 도 같은 처지라, 캡처에서 `ip.src == 0.0.0.0` 로 두 메시지를 함께 골라낼 수 있습니다.
...
**`REQUEST`**(`== 3`) — 클라이언트가 다시 브로드캐스트합니다. 반드시 옵션 54 를 담아 어느 서버를 골랐는지 알려야 합니다. 원문은 이유를 적지 않았지만 여러 서버가 OFFER 를 보냈을 수 있기 때문입니다(아래 §5). 원문은 이어서 REQUEST 에서 선택된 서버가 그 클라이언트의 바인딩을 저장소에 기록하고 ACK 로 응답한다고 설명합니다.
...
원문이 마지막에 붙이는 절차 하나가 앞 편들과 이어집니다. 클라이언트는 받은 설정을 검증하고 ARP 로 그 IPv4 주소를 다시 확인합니다. 다른 DHCP 클라이언트가 이미 쓰고 있으면 서버에 `DECLINE` 을 보내고 설정 과정을 다시 시작합니다. 곧 캡처에 `DECLINE` 이 보인다면 주소 충돌이 있었습니다.

### 서버의 답은 옆자리에 보이는가 — BROADCAST 플래그

§1 에서 남의 DHCP 요청은 옆자리 캡처에 잡힌다고 했습니다. 서버의 답은 사정이 다릅니다. 주소가 없는 클라이언트는 자기 앞으로 온 유니캐스트를 받지 못할 수 있습니다. 그런 클라이언트는 DISCOVER·REQUEST 의 `flags` 필드에 BROADCAST 비트를 켜 둡니다.[^rfc2131-bcast] 릴레이 없이(`giaddr` 0) 아직 주소가 없는(`ciaddr` 0) 클라이언트에게 답할 때, 서버는 이 비트를 보고 보내는 방식을 정합니다.

- 비트가 켜져 있으면 OFFER·ACK 를 브로드캐스트합니다. 옆자리 캡처에도 잡힙니다.
- 꺼져 있으면 `yiaddr` 와 클라이언트 MAC(`chaddr`)으로 유니캐스트합니다. 옆자리에는 오지 않습니다.[^rfc2131-srvcast]
- 거절인 NAK(§5)은 이 경우 비트와 상관없이 브로드캐스트입니다.

DHCPv6 는 이 비트가 없습니다. 클라이언트가 처음부터 링크 로컬 주소를 갖고 있으므로 ADVERTISE·REPLY 는 그 주소로 가는 유니캐스트입니다. 옆자리 캡처에 요청만 있고 답이 없으면 서버가 침묵한 것인지 답이 유니캐스트라 내 포트에 오지 않은 것인지부터 가려야 합니다.
...
DORA 의 네 이름을 알아도 "왜 네 걸음이어야 하나"는 따로 물어야 풀립니다. 이 절은 원문이 짧게 넘어간 다섯 자리를 RFC 원문으로 채웁니다. OFFER 가 예약이 아니라는 것, REQUEST 한 번이 선택과 거절을 겸하는 방식, 거절 메시지, 두 장치가 나눠 하는 확인, 세그먼트 밖 서버로 가는 중계입니다.
...
서버가 같은 주소를 두 클라이언트에게 차례로 제안하는 일이 생길 수 있습니다. 둘 다 그 주소로 REQUEST 를 보내면 먼저 도착한 쪽이 장부에 적히고 ACK 를 받습니다. 늦은 쪽은 이미 할당된 주소를 요청한 셈이라 NAK 을 받습니다.
...
서버가 둘이면 OFFER 도 둘 옵니다. `yiaddr` 칸은 4바이트 하나라 OFFER 하나에는 주소 하나만 실립니다. 그러니 §2 의 IPv6 처럼 클라이언트가 고르는 대상은 주소가 아니라 서버입니다.[^rfc2131-choose] 클라이언트는 하나를 골라 옵션 54 에 그 서버의 식별자를 넣고 REQUEST 를 한 번만 브로드캐스트합니다. 고르지 않은 서버에게 따로 거절을 보내지 않습니다.

RFC 2131 은 선택되지 않은 서버들이 이 REQUEST 를 "클라이언트가 자기 제안을 거절했다는 통지로 쓴다"고 밝힙니다.[^rfc2131-select] 같은 메시지가 선택된 서버에는 요청이고 나머지 서버에는 거절 통지입니다. 브로드캐스트라서 가능한 일입니다.
...
주소를 줄 수 없을 때 서버가 알리는 방법은 단계마다 다릅니다. 원문의 DHCPv4 메시지 목록은 ACK(5)에서 끝나지만 DHCPv4 에는 거절 메시지 **DHCPNAK**(메시지 종류 6, `dhcp.option.dhcp == 6`)이 있습니다.[^rfc2132-type] RFC 2131 은 선택된 서버가 REQUEST 를 들어줄 수 없을 때, 예를 들어 요청된 주소가 이미 할당됐을 때 NAK 로 응답하라고 권고합니다.

NAK 을 받은 클라이언트는 설정 과정을 처음부터 다시 시작합니다.[^rfc2131-nak]

이름이 같아도 TCP 헤더의 ACK 와는 다른 것입니다. DHCP 의 ACK·NAK 은 "받았다"는 신호가 아닙니다. 옵션 53 의 값 5·6 으로 구분되는 메시지 종류이고 ACK 는 "장부에 적었다"는 확정, NAK 은 "들어줄 수 없다"는 거절입니다. UDP 위라 메시지가 사라질 수 있지만[^rfc768] 그 빈자리는 클라이언트의 재전송이 메웁니다.[^rfc2131-rtx]
...
DHCPv6 에는 거절 전용 메시지가 없습니다. 대신 서버가 REPLY 의 IA 옵션 안에 상태 코드 NoAddrsAvail(값 2, `dhcpv6.status_code == 2`)을 실어 줄 주소가 없다고 알립니다. IPv4 와 다른 점은 이 표시가 ADVERTISE 에도 실린다는 것입니다. RFC 8415 는 뒤따를 REQUEST 에도 주소를 줄 수 없는 서버가 ADVERTISE 의 IA 에 NoAddrsAvail 을 담아야 한다(MUST)고 규정합니다.[^rfc8415-status] IPv6 에서는 제안 단계의 거절도 캡처에 남습니다.

### 주소 확인은 서버의 ICMP 와 클라이언트의 ARP 로 나뉩니다

§4 의 "ARP 로 다시 확인"은 전제 하나를 깔고 있습니다. ARP 요청은 MAC 브로드캐스트(`ff:ff:ff:ff:ff:ff`)로 나갑니다. RFC 826 은 주소를 묻는 장비가 요청을 "케이블 위의 모두에게" 브로드캐스트하고, 그 주소의 주인만 브로드캐스트가 아닌 직접 응답을 돌려준다고 적습니다.[^rfc826] 그러니 방금 받은 주소를 ARP 로 물었는데 응답이 오면 세그먼트 안 누군가가 이미 그 주소를 쓰고 있습니다.

그 누군가가 DHCP 클라이언트일 필요는 없습니다. 원문은 ARP 가 잡는 상대를 "다른 DHCP 클라이언트"로 적습니다. RFC 2131 은 쓰는 쪽을 가리지 않고 "이미 쓰이는 주소"라고만 합니다.[^rfc2131-arp] 그러면 서버 장부에 없는 장비도 걸립니다. 이 차이가 아래에서 두 확인을 나누는 근거가 됩니다.

확인은 두 번 있고 두 장치가 나눠 합니다. RFC 2131 은 서버가 새 주소를 할당할 때 OFFER 전에 그 주소가 쓰이지 않는지 확인하라(SHOULD)고 정하고 예로 ICMP Echo Request 를 듭니다. 관리자가 이 확인을 끌 수 있게 구현하라는 조항도 함께 둡니다.[^rfc2131-probe] 클라이언트는 ACK 를 받은 뒤 ARP 로 마지막 확인을 합니다.
...
5. 릴레이는 `giaddr` 로 응답을 내보낼 인터페이스를 정하고 클라이언트에게 내려 줍니다. 이때 §4 의 BROADCAST 플래그를 봅니다. 켜져 있으면 `255.255.255.255` 로, 꺼져 있으면 `yiaddr` 와 클라이언트 MAC(`chaddr`)으로 유니캐스트합니다.[^rfc1542-reply]

![브로드캐스트가 라우터에서 멈추고 릴레이가 유니캐스트로 대신 전달한다](./_assets/05-01.relay-v4.svg)

`giaddr` 하나가 두 가지 일을 합니다. 서버에게는 어느 세그먼트의 주소를 줄지 알려 주고 릴레이에게는 응답을 어느 인터페이스로 내보낼지 알려 줍니다. `hops` 는 릴레이가 여러 단 이어질 때를 대비한 계수기입니다.

RFC 1542 는 `hops` 가 16 을 넘은 요청을 반드시 버리라고 정합니다. 동시에 관리자가 이 임계값을 더 낮출 수 있게 하고 그 기본값을 4 로 두라고 권합니다.[^rfc1542-giaddr] 16 은 넘을 수 없는 상한이고 4 는 권고 기본값이라 실제 기본값은 릴레이 구현마다 다를 수 있습니다.
...
`RELAY-FORWARD` 에도 `giaddr` 에 해당하는 칸이 있습니다. link-address(`dhcpv6.linkaddr`)는 서버가 클라이언트가 있는 링크를 알아보는 데 쓰는 주소입니다.[^rfc8415-linkaddr]

캡처 자리에 따라 보이는 것이 다릅니다. 클라이언트 세그먼트에서 잡으면 릴레이가 없을 때와 같은 교환이 보입니다. 서버 쪽에서 잡으면 `giaddr` 이 채워진 유니캐스트가 릴레이와 서버 사이를 오갑니다.



## 6. 필터가 bootp 였던 이유 — DHCP 는 BOOTP 형식을 물려받았습니다

> DHCP 는 프로토콜로서는 BOOTP 를 대체했지만 BOOTP 의 메시지 형식과 포트를 그대로 물려받은 확장입니다. 그래서 필터 이름이 오래도록 `bootp` 였습니다.

![BOOTP 위에 얹힌 DHCP](./_assets/05-01.bootp-dhcp.svg)

Wireshark 3.0 전에는 DHCP 를 찾으려고 `dhcp` 를 쳐도 안 나왔고 `bootp` 를 쳐야 했습니다. 이유는 프로토콜의 역사에 있습니다. 원문의 설명이 관계를 정합니다. "DHCP 는 BOOTP 메커니즘의 확장입니다. 달리 말해 DHCP 는 BOOTP 를 자기 전송 프로토콜로 씁니다. 이 동작 덕분에 기존 BOOTP 클라이언트는 초기화 소프트웨어를 바꾸지 않고도 DHCP 서버와 상호 운용됩니다."
...
> **노트의 읽기**: 원문 표의 DHCP 칸은 연도를 1993, RFC 를 2131 로 나란히 적었지만 둘은 다른 문서입니다. 1993년 10월에 나온 첫 DHCP 명세는 RFC 1531 이고, 표가 가리키는 RFC 2131 은 1997년 3월에 나온 현행 명세입니다.[^rfc2131-date] 표는 원문대로 두고, DHCP 가 1993년에 처음 나와 1997년 RFC 2131 로 정리됐다고 읽으면 됩니다.

포트가 같다는 점이 상호 운용의 근거입니다. 같은 포트에서 같은 메시지 형식을 쓰되 DHCP 가 옵션을 더 얹은 구조라 서버 하나가 양쪽 클라이언트를 모두 받을 수 있습니다. Wireshark 가 오래도록 DHCP 를 `bootp` 디섹터 안에서 읽은 것도 같은 형식이기 때문입니다.



## 심화 학습

> 원문이 참조한 DHCPv6 명세는 새 RFC 로 바뀌었고 메시지 흐름·포트·필터는 그대로입니다. 원문이 다루지 않는 SLAAC 경로는 §2 「IPv6 주소를 얻는 세 경로」로 옮겼습니다.
...
| `ADVERTISE`·`OFFER` 까지만 | 제안이 클라이언트에 닿지 않았거나, 닿았는데 고르지 못했습니다 | 클라이언트 쪽 캡처에도 제안이 있는가(BROADCAST 플래그 · 릴레이 · 방화벽) · 클라이언트 로그 · IPv6 는 ADVERTISE 안의 `NoAddrsAvail` |
| `REQUEST` 뒤 응답 없음 | 서버가 확정하지 못했습니다 | 옵션 54 가 지목한 서버의 상태 · [OFFER 는 예약이 아닙니다](#offer-는-예약이-아닙니다--ack-가-확정하는-이유) |
| `NAK`, 또는 `NoAddrsAvail` 상태 코드 | 서버가 거절했습니다 | IPv4 `NAK` 은 REQUEST 거절이라 요청한 주소가 무효인가 · 다른 클라이언트가 먼저 확정했는가, IPv6 `NoAddrsAvail` 은 주소 풀 소진 · [거절 절](#거절은-dhcpnak-로-옵니다--응답이-없는-것과-다릅니다) |
| `DECLINE` 이 보임 | 주소 충돌입니다 | ARP 로 그 주소를 쓰는 장비 찾기 |
| `SOLICIT` → `REPLY` 두 개만 | rapid commit 입니다 | REPLY 에 `NoAddrsAvail` 이 없으면 정상입니다 |
...
A: 고르지 못한 다른 서버들에게도 "나는 저 서버를 골랐다"를 알려 그들이 자기 제안을 거두게 하기 위해서입니다. 그래서 옵션 54 로 고른 서버를 반드시 지목해야 합니다. 서버가 둘인 흐름은 [§5](#request-한-번의-브로드캐스트가-선택과-거절을-겸합니다)에 있습니다.

Q: 원문은 `bootp.` 필드를 쓰는데 지금 화면에는 `dhcp.` 로 나옵니다. 어느 쪽을 써야 합니까?
A: Wireshark 3.0 부터 디섹터 이름이 DHCP 로 바뀌었으니 `dhcp` 로 씁니다. 옛 `bootp` 이름도 아직 먹습니다. 2026-09-23 tshark 4.6.8 에서 `bootp` 와 `dhcp` 가 같은 캡처에서 같은 프레임을 골랐습니다. 다만 릴리스 노트가 앞으로 제거될 수 있다고 적었고 `bootp.dhcp` 는 이미 빠졌으므로 새로 쓰는 필터는 `dhcp.option.dhcp` 처럼 적습니다.



## 핵심 개념 체크리스트

목표 번호 순서대로 하나 이상씩 짝지었습니다.

- [ ] (목표 1) SARR 과 DORA 의 네 걸음을 짝짓고, 주소를 모를 때 두 쪽이 서버를 찾는 목적지가 어떻게 다른지 말할 수 있습니까?
- [ ] (목표 2) DISCOVER 만 반복되는 캡처, NAK 이 온 캡처, DECLINE 이 보이는 캡처가 각각 무엇을 뜻하는지 가를 수 있습니까?
- [ ] (목표 2) OFFER 를 받고도 ACK 를 기다려야 하는 이유를 서버 장부로 설명할 수 있습니까?
- [ ] (목표 3) OFFER·REQUEST·ACK 에서 `yiaddr` 와 옵션 50 이 각각 무엇을 담는지 보고 할당이 어디까지 갔는지 판단할 수 있습니까?
- [ ] (목표 4) INFORMATION-REQUEST 와 rapid commit 이 각각 언제 두 메시지로 끝나는지, 그 캡처를 실패한 교환과 어떻게 구분하는지 말할 수 있습니까?
- [ ] (목표 5) 서버가 둘일 때 클라이언트가 고르는 것이 주소가 아니라 서버이고, REQUEST 한 번이 선택과 거절을 겸하는 이유를 설명할 수 있습니까?
- [ ] (목표 5) 서버가 다른 세그먼트에 있을 때 릴레이가 `giaddr` 로 서버와 자기 자신에게 무엇을 알리는지 말할 수 있습니까?
...
[^rfc1542-giaddr]: RFC 1542 §4.1.1 — "If this field is zero, the relay agent MUST fill this field with the IP address of the interface on which the request was received." · "The value of the 'hops' field MUST be incremented." · "The relay agent MUST silently discard BOOTREQUEST messages whose 'hops' field exceeds the value 16.  A configuration option SHOULD be provided to set this threshold to a smaller value if desired by the network manager.  The default setting for a configurable threshold SHOULD be 4." <https://www.rfc-editor.org/rfc/rfc1542.txt> (2026-09-23 조회)
...
[^rfc8415-reply]: RFC 8415 §7.3 — "A server sends a Reply message containing assigned leases and configuration parameters in response to a Solicit, Request, Renew, or Rebind message received from a client. … in response to an Information-request message. … in response to a Confirm message … to acknowledge receipt of a Release or Decline message." Solicit 에 대한 Reply 는 rapid commit 일 때입니다(§18.3.1). <https://www.rfc-editor.org/rfc/rfc8415.txt> (2026-09-23 조회)

[^rfc4541]: RFC 4541 §3 — "MLD is mandated for addresses with scope 2 (link-scope) or greater. The only exception is the address FF02::1 which is the all hosts link-scope address for which MLD messages are never sent.  Packets with the all hosts link-scope address should be forwarded on all ports." <https://www.rfc-editor.org/rfc/rfc4541.txt> (2026-09-23 조회)

[^rfc2131-date]: RFC 1531 머리글 — "Request for Comments: 1531 … October 1993" · RFC 2131 머리글 — "Request for Comments: 2131 … Obsoletes: 1541 … March 1997". RFC 1541 이 RFC 1531 을 폐기했으므로 1531 → 1541 → 2131 순서입니다. <https://www.rfc-editor.org/rfc/rfc1531.txt> · <https://www.rfc-editor.org/rfc/rfc2131.txt> (2026-09-23 조회)

[^rfc2131-srvcast]: RFC 2131 §4.1 — "If 'giaddr' is zero and 'ciaddr' is zero, and the broadcast bit is set, then the server broadcasts DHCPOFFER and DHCPACK messages to 0xffffffff. If the broadcast bit is not set and 'giaddr' is zero and 'ciaddr' is zero, then the server unicasts DHCPOFFER and DHCPACK messages to the client's hardware address and 'yiaddr' address.  In all cases, when 'giaddr' is zero, the server broadcasts any DHCPNAK messages to 0xffffffff." <https://www.rfc-editor.org/rfc/rfc2131.txt> (2026-09-23 조회)

[^rfc2131-rtx]: RFC 2131 §4.1 — "DHCP clients are responsible for all message retransmission.  The client MUST adopt a retransmission strategy that incorporates a randomized exponential backoff algorithm to determine the delay between retransmissions." <https://www.rfc-editor.org/rfc/rfc2131.txt> (2026-09-23 조회)
...
[^rfc4541-snoop]: RFC 4541 §1 — "an IGMP snooping switch provides the benefit of conserving bandwidth on those segments of the network where no node has expressed interest in receiving packets addressed to the group address. This is in contrast to normal switch behavior where multicast traffic is typically forwarded on all interfaces." 문서 제목이 "IGMP and MLD Snooping Switches Considerations" 로 MLD 도 같은 방식입니다. <https://www.rfc-editor.org/rfc/rfc4541.txt> (2026-09-26 조회)

[^rfc4862]: RFC 4862 Abstract — "The autoconfiguration process includes generating a link-local address, generating global addresses via stateless address autoconfiguration, and the Duplicate Address Detection procedure to verify the uniqueness of the addresses on a link." <https://www.rfc-editor.org/rfc/rfc4862.txt> (2026-09-26 조회)

[^rfc4861-mo]: RFC 4861 §4.2 — M: "Managed address configuration" flag — "When set, it indicates that addresses are available via Dynamic Host Configuration Protocol" · "If the M flag is set, the O flag is redundant and can be ignored" · O: "Other configuration" flag — "When set, it indicates that other configuration information is available via DHCPv6." · "If neither M nor O flags are set, this indicates that no information is available via DHCPv6." <https://www.rfc-editor.org/rfc/rfc4861.txt> (2026-09-26 조회)

[^rfc8415-opts]: RFC 8415 §21 — "OPTION_CLIENTID (1)" · "OPTION_SERVERID (2)" · "OPTION_IA_NA (3)" · "OPTION_IA_TA (4)" · "OPTION_ORO (6)" · "OPTION_PREFERENCE (7)" · "OPTION_RELAY_MSG (9)" · "OPTION_STATUS_CODE (13)" · "OPTION_RAPID_COMMIT (14)". RFC 3646 §3 — "OPTION_DNS_SERVERS (23)". <https://www.rfc-editor.org/rfc/rfc8415.txt> · <https://www.rfc-editor.org/rfc/rfc3646.txt> (2026-09-26 조회)

[^rfc8415-t1t2]: RFC 8415 §4.2 — T1: "The time interval after which the client is expected to contact the server that did the assignment to extend (renew) the lifetimes of the addresses assigned" · T2: "The time interval after which the client is expected to contact any available server to extend (rebind) the lifetimes of the addresses assigned". <https://www.rfc-editor.org/rfc/rfc8415.txt> (2026-09-26 조회)

[^rfc2131-fields]: RFC 2131 §2 Table 1 — hops "Client sets to zero, optionally used by relay agents when booting via a relay agent." · xid "Transaction ID, a random number chosen by the client, used by the client and server to associate messages and responses" · ciaddr "Client IP address; only filled in if client is in BOUND, RENEW or REBINDING state and can respond to ARP requests." · yiaddr "'your' (client) IP address." · giaddr "Relay agent IP address, used in booting via a relay agent." · chaddr "Client hardware address." <https://www.rfc-editor.org/rfc/rfc2131.txt> (2026-09-26 조회)

[^rfc8415-linkaddr]: RFC 8415 §9.1 — link-address "An address that may be used by the server to identify the link on which the client is located." <https://www.rfc-editor.org/rfc/rfc8415.txt> (2026-09-26 조회)
