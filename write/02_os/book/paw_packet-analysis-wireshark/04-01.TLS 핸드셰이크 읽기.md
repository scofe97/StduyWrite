...
| DHE · ECDHE | 매번 새 키를 만드는 임시 디피-헬먼 | 이름 첫 조각 · 원리는 04-03 §1 | §3 · §4 |
| 상호 인증 (mutual TLS) | 클라이언트도 인증서를 내미는 구성 | 메시지 셋이 더 붙음 | §4 |
| ChangeCipherSpec | TLS 1.2 보호 상태 전환 | 송신 방향별 경계 · 다음 Finished부터 | §5 |

아래는 여섯 절의 읽는 순서입니다. §2에서 전체 흐름을 잡고 §3~§5에서 각 메시지를 읽은 뒤, §6에서 실패 단서와 캡처 판독 절차를 확인합니다. Alert는 마지막 자리에 고정되지 않고 문제가 감지된 시점에 올 수 있습니다.
...
셋은 캡처에 남은 Alert 번호로 먼저 갈립니다. 버전이 안 맞으면 `protocol_version(70)`, 제안한 cipher suite 로 협상하지 못하면 보통 `handshake_failure(40)` 가 옵니다. 인증서 체인을 신뢰하지 못하면 `unknown_ca(48)` 가 대표 단서입니다. 읽는 법은 [§6](#6-alert--실패를-알리는-프로토콜)에서 봅니다. 그 번호를 해석하려면 먼저 캡처가 어느 버전인지 알아야 합니다. 버전마다 메시지 순서와 암호화가 시작되는 자리가 다르기 때문입니다.

### 책의 버전과 캡처의 버전

TLS(Transport Layer Security)는 SSL(Secure Sockets Layer)을 계승한 프로토콜입니다. TLS는 통신의 기밀성과 무결성을 보호하고 상대의 신원을 확인합니다. 보통 HTTPS에서는 서버를 인증하고 상호 인증을 구성하면 서버도 클라이언트 인증서를 확인합니다.
...
책은 2015년 자료이므로 TLS 1.3을 초안으로 소개하지만 아래 표는 확정된 표준을 기준으로 읽습니다.
...
아래 개념도는 TLS 1.2에서 서버 인증서를 쓰는 전체 핸드셰이크를 그립니다. 이 범위의 기본 메시지는 실선, 키 교환 방식과 클라이언트 인증 조건에 따라 추가되는 메시지는 점선입니다. 책의 `two-way-handshake.pcap`은 상호 인증 예제 이름이며, 이 그림은 패킷 번호를 옮긴 실측 기록이 아닙니다.
...
| 4 | NewSessionTicket · 재개용 티켓 발급 | `tls.handshake.type == 4` |
...
표의 `NewSessionTicket` 은 책이 열거한 열 가지 밖이지만 실제 캡처에 자주 나옵니다. 랩의 TLS 1.3 캡처에서는 핸드셰이크가 끝난 뒤 프레임 두 개에 종류 4 가 이어서 찍혔습니다. 핸드셰이크가 끝난 뒤에 오는 메시지라 순서 그림에는 넣지 않았습니다.
...
그림의 네 단계는 클라이언트 제안, 서버 선택과 인증, 클라이언트 키 재료와 전환, 서버 전환과 검증이며, 단계가 바뀔 때마다 보내는 쪽이 바뀝니다. 이 편의 TLS 교환은 앞 편에서 본 TCP 연결 성립 뒤에 시작합니다. 한 TCP 패킷에 TLS 메시지가 여럿 담기거나 한 메시지가 여러 패킷에 걸칠 수 있으므로 메시지 수를 패킷 수로 세지 않습니다.
...
그림은 ClientHello 가 내민 세 필드가 ServerHello 에서 각각 어떻게 줄어드는지, 고를 cipher suite 가 없으면 어디로 빠지는지를 보여 줍니다.
...
Wireshark 상세 창에서는 그 한 줄이 ServerHello 아래 두 필드로 보입니다. 아래 출력은 실측이 아닌 형태 예시입니다.
...
필드 값만 보고 단정하지 않을 자리가 넷 있습니다. TLS 1.3도 호환용 Version 필드에는 `0x0303`을 쓰므로 실제 선택 버전은 ServerHello의 `supported_versions` 확장까지 확인합니다. cipher suite 목록의 첫 항목이 반드시 암호학적으로 가장 강하다는 뜻은 아닙니다. 캡처에서 Random 의 앞 4바이트를 신뢰할 수 있는 시계로 간주하지 않고 세션 티켓까지 포함한 모든 재개 가능성을 Session ID 하나로 판단하지도 않습니다.
...
확장은 고유 번호로 식별합니다. 책에서 다룬 네 확장과 SNI를 IANA 번호로 정리한 표입니다.[^iana-tls-ext]
...
이 편에서 다루는 평문 ClientHello에 SNI가 포함되면 캡처에서도 그 이름을 읽을 수 있습니다. 앞서 잡힌 ClientHello는 암호화가 시작된 뒤에도 캡처에 그대로 남습니다. SNI가 없는 연결이나 ClientHello를 보호하는 ECH(Encrypted ClientHello)는 이 예제의 범위 밖입니다.

### 도청되는 선 위에서 같은 키를 갖는 법 — 키 교환(Key Exchange)과 인증(Authentication)

서버가 고른 cipher suite 를 읽기 전에, 그 이름이 무슨 문제를 나눠 푸는지부터 봅니다. 문제는 하나입니다. 본문을 감싸는 데 쓰는 **대칭키**(symmetric key)는 암호화와 복호화에 같은 값을 쓰므로 양쪽에 같은 값이 있어야 합니다. 그런데 클라이언트와 서버는 방금 처음 만났고, 그 사이의 선은 누구나 엿볼 수 있습니다. 키를 그냥 실어 보내면 엿보는 쪽도 같이 갖게 됩니다.
...
그림에서 선을 건너는 화살표는 둘뿐입니다.

![공개값만 건너가고 비밀값은 남습니다](./_assets/04-01.key-exchange-idea.svg)

TLS 가 쓰는 것은 같은 구조를 타원곡선 위의 점 연산으로 하는 쪽입니다. 왜 한쪽 방향만 쉬운 계산이 이것을 세우는지, 작은 수로 직접 따라가 보는 예제와 곡선 쪽이 왜 짧은 키로 같은 방어력을 얻는지는 [04-03 §1](./04-03.열쇠와 실패.md) 이 맡습니다.
...
키 교환이 만든 그 공유 비밀이 TLS 1.2 에서 `pre_master_secret` 입니다. 그 뒤로 이름은 이렇게 이어집니다.

- **`pre_master_secret`**: 키 교환이 만든 공유 비밀. 정적 RSA 에서는 클라이언트가 만들어 서버 공개키로 암호화해 보내고, DHE·ECDHE 에서는 양쪽이 각자 계산합니다.
- **`master_secret`**: `pre_master_secret` 과 양쪽 Hello 의 Random 을 PRF 에 넣어 만드는 값. PRF 는 비밀·시드·레이블을 받아 원하는 길이의 출력을 만드는 함수입니다.[^rfc5246]
...
TLS 1.2의 Hello는 조건을 고르는 메시지입니다. 평문으로 전달되지만 평문이라는 말이 난수 생성 등 암호학적 처리가 전혀 없다는 뜻은 아닙니다. 서버가 고른 suite 이름을 어떻게 읽는지는 바로 다음 소절에서 봅니다.
...
| 조각 | 하는 일 | 실제로 도는 자리 | 언제 | 종류 |
|---|---|---|---|---|
| `ECDHE` | 세션 키 재료를 합의 | ServerKeyExchange · ClientKeyExchange | 핸드셰이크 | 비대칭 |
| `RSA` | 서버가 개인키 소유를 증명 | ServerKeyExchange의 서명 | 핸드셰이크 | 비대칭 |
| `AES_256`·`GCM` | 본문을 감싸고 무결성까지 보호 | ChangeCipherSpec 뒤로 계속 | 그 뒤 데이터 | 대칭 |
| `SHA384` | PRF의 해시 | master secret·레코드 키 파생, Finished 계산 | 양쪽 | 해시 |

`WITH` 가 이 경계를 가릅니다. 앞의 두 조각은 양쪽이 같은 비밀 값을 갖게 되는 방법이고, 뒤의 두 조각은 그 값으로 만든 키를 써서 데이터를 감싸고 검사하는 방법입니다. 이름 하나에 서로 다른 시점의 일이 붙어 있는 셈입니다.

키 교환 방식이 갈려도 도착점은 같습니다. 달라지는 것은 그 비밀 값이 생기는 경로뿐입니다.

| 구분 | 정적 RSA | ECDHE |
|---|---|---|
| 비밀 값을 만드는 쪽 | 클라이언트가 혼자 만듭니다 | 양쪽이 각자 계산합니다 |
| 선을 지나가는 것 | 그 값을 서버 공개키로 잠근 암호문 | 서로의 공개값 |
| 장기 개인키의 역할 | 그 암호문을 푸는 데 씁니다 | 공개값에 서명하는 데만 씁니다 |
| 도착점 | 양쪽이 같은 `pre_master_secret` | 양쪽이 같은 `pre_master_secret` |

아래 네 역할은 `TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384` 하나를 기준으로 읽습니다. 다른 suite 는 같은 이름 조각이 다른 자리에 앉을 수 있으므로 다음 소절의 OpenSSL 출력에서 `Kx` 와 `Au` 열로 확인한 뒤에 적용합니다.

`ECDHE` 는 양쪽이 공유 비밀을 합의하는 방법입니다. 타원곡선 위에서 매번 새 키를 만들어 나누는 **임시**(ephemeral) 디피-헬먼이고, `DHE` 는 같은 일을 정수 나머지 위에서 합니다. 끝의 `E` 가 그 임시성을 가리킵니다. 왜 매번 새로 만들어야 하고 그것이 사후 복호화 가능 여부를 어떻게 가르는지는 [04-03 §1](./04-03.열쇠와 실패.md) 이 맡습니다.

`RSA` 는 이 suite 에서 서버가 개인키 소유를 증명하는 방법입니다. 서버는 `ServerKeyExchange` 에 실어 보내는 임시 공개값에 자기 개인키로 서명하고, 클라이언트는 인증서에 담긴 공개키로 그 서명을 검증합니다. 반면 정적 RSA suite 에서는 같은 `RSA` 가 키 교환까지 맡아, 클라이언트가 만든 비밀을 서버 공개키로 잠그는 데 쓰입니다.

`AES_256` 과 `GCM` 은 레코드 키로 본문을 감싸는 방법입니다. 이 suite 의 레코드 무결성은 GCM 이 만드는 인증 태그로 확인합니다.

`SHA384` 는 PRF(Pseudorandom Function, 의사난수 함수)의 해시입니다. `master_secret` 과 레코드 키를 파생할 때, 그리고 Finished 를 계산할 때 이 해시를 씁니다.[^rfc5289][^rfc5246]

인증서 발급자가 그 인증서에 서명할 때 쓴 알고리즘은 suite 이름의 `RSA` 와 별개일 수 있습니다.

이 네 조각 분해는 TLS 1.2 의 suite 이름에만 적용합니다. TLS 1.3 의 suite 이름에는 키 교환과 인증 방식이 들어 있지 않습니다.[^rfc8446]

핸드셰이크 캡처와 서버 개인키로 정적 RSA(`TLS_RSA_WITH_...`)의 과거 통신을 복호화할 수 있는 조건과, ECDHE·DHE 에 세션 키 로그가 필요한 이유는 [04-03](./04-03.열쇠와 실패.md)가 다룹니다.

### 같은 cipher suite 를 OpenSSL 은 다르게 적습니다

명령줄에서 만나는 이름은 지금까지 읽은 IANA 이름과 다릅니다. OpenSSL 은 `TLS_` 접두사와 `WITH` 를 빼고 남은 조각을 하이픈으로 잇습니다. 키 교환이 RSA 인 suite 에서는 그 키 교환 조각까지 생략하므로 이름이 한 조각 더 짧아집니다.

```bash
# 두 표기의 대응을 직접 확인합니다. 왼쪽이 IANA, 오른쪽이 OpenSSL 이름입니다.
openssl ciphers -stdname 'ECDHE-RSA-AES256-GCM-SHA384:AES256-SHA256'
```

```bash
# 출력 — 왼쪽이 IANA 이름, 가운데가 OpenSSL 이름입니다
...
첫 줄은 조각이 그대로 대응합니다. `TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384` 에서 접두사와 `WITH` 를 빼고 남은 네 조각을 하이픈으로 이으면 `ECDHE-RSA-AES256-GCM-SHA384` 가 됩니다.

둘째 줄은 조각 하나가 사라집니다. OpenSSL 이름 `AES256-SHA256` 에는 키 교환 조각이 없습니다. 키 교환은 합니다. 키 교환이 RSA 일 때 OpenSSL 이 그 조각을 적지 않을 뿐입니다. IANA 이름 `TLS_RSA_WITH_AES_256_CBC_SHA256` 의 첫 `RSA` 가 그 사라진 조각입니다.

생략된 조각은 이름 대신 출력의 열이 알려 줍니다. 둘째 줄의 `Kx=RSA` 가 키 교환 방식을 따로 적어 주기 때문입니다. 이름보다 열을 보고 판정하는 편이 안전한 이유입니다.

| 열 | 뜻 |
|---|---|
| `Kx` | 키 교환 방식. `ECDH` · `RSA` 등 |
| `Au` | 인증 방식. 서버가 무엇으로 소유를 증명하는가 |
| `Enc` | 레코드를 감싸는 대칭 암호와 키 길이 |
| `Mac` | 무결성 확인 방식 |

두 줄을 이 열로 비교하면 `RSA` 가 앉는 자리가 다릅니다. 첫 줄은 `Kx=ECDH Au=RSA` 라서 `RSA` 가 인증에만 쓰입니다. 둘째 줄은 `Kx=RSA Au=RSA` 라서 키 교환과 인증 양쪽에 쓰입니다. suite 이름에 `RSA` 가 보인다고 해서 늘 서명을 뜻하지는 않습니다.

`Mac` 열은 무결성을 무엇이 맡는지 가릅니다. 첫 줄의 `Mac=AEAD` 는 별도의 MAC 을 붙이지 않고 암호 자체가 무결성까지 맡는다는 표시이며, GCM 과 ChaCha20-Poly1305 가 이쪽입니다. 그래서 이름 끝의 `SHA384` 는 레코드마다 붙는 MAC 이 아니라 PRF 의 해시입니다. 둘째 줄의 `Mac=SHA256` 은 반대로 레코드마다 MAC 을 따로 붙이는 방식입니다.[^rfc5289]
...
두 조건이 가르는 메시지를 표에 모았습니다. 위 넷은 키 교환 방식이 정하는 축이고, 맨 아래 줄은 서버를 인증하는 위 세 행에 더해지는 축입니다.

정적 DH 와 `DH_anon` 행은 과거 캡처를 읽으려고 남긴 것입니다. 둘의 차이는 아래 「Server Key Exchange」 소절에서 봅니다.
...
그림은 Certificate 메시지로 오는 인증서와 클라이언트가 이미 가진 루트를 가르는 경계, 그리고 서명을 확인하며 루트까지 올라가는 방향을 보여 줍니다.
...
서버는 자기 인증서를 먼저 보내고, 이어 신뢰 경로 구성에 필요한 중간 CA 인증서를 보냅니다. 루트 인증서는 보통 생략합니다. 서버가 루트를 함께 보내더라도 그것만으로 신뢰가 생기지는 않습니다. 클라이언트가 이미 신뢰하는 기준점까지 서명 체인이 이어져야 합니다.[^rfc5246]
...
그림은 정적 RSA 와 인증된 DHE·ECDHE 의 같은 구간을 나란히 놓아 ServerKeyExchange 자리가 비는지 채워지는지 보여 줍니다.
...
그림은 공격자가 할 수 있는 두 시도가 같은 두 검사를 지나며 어디서 처음 갈리는지 보여 줍니다.
...
표에서 번호를 봐도 막상 캡처를 열면 눈에 안 띈다는 말이 나옵니다. Alert 가 별도의 화려한 표시 없이 레코드 하나로 지나가기 때문입니다.

Alert 는 레코드 종류 21 이고 길이는 2바이트입니다. 수준 1바이트와 번호 1바이트가 전부입니다. 실패는 대개 ChangeCipherSpec 보다 앞에서 나므로 이 두 바이트는 평문으로 읽힙니다.

성공한 연결과 나란히 놓으면 차이가 분명합니다. 랩의 `c8-alerts.pcap` 에서 뜬 값입니다.

```bash
# 성공 — ClientHello 뒤에 서버의 핸드셰이크가 이어집니다
#   tls.handshake.type   1  →  2, 11, 12, 14
#
# 실패 — ClientHello 뒤에 레코드 종류 21 하나로 끝납니다
#   cipher 불일치   frame 7   content_type 21   level 2   desc 40   length 2
#   버전 불일치     frame 17  content_type 21   level 2   desc 70   length 2
```

`desc 40` 은 `handshake_failure`, `desc 70` 은 `protocol_version` 입니다. 종류 2 가 아예 오지 않았다는 사실 자체가 협상이 서지 못했다는 단서입니다.
...
접속이 실패했을 때 "복호화부터 해야 하나"를 먼저 떠올리기 쉽지만 협상이 서지 못한 연결에는 복호화할 대상이 없습니다. 실패는 보호가 시작되기 전에 나므로 필요한 단서가 모두 평문입니다.

```bash
# 1. Alert 번호를 읽습니다         desc 40 협상 · 70 버전 · 48 신뢰 경로
#         ↓
# 2. ClientHello 의 제안을 봅니다   버전 · cipher suite 목록 · 그룹 · 서명 알고리즘
#         ↓
# 3. 서버가 받아들이는 조건을 훑습니다   묶음을 하나씩 제안해 붙여 봅니다(04-03 §3)
```

세 걸음 모두 키 없이 진행됩니다. 복호화 재료를 찾는 일은 연결이 성립한 뒤 내용이 안 보일 때의 문제이고, 그쪽은 [04-03](./04-03.%EC%97%B4%EC%87%A0%EC%99%80%20%EC%8B%A4%ED%8C%A8.md) 이 맡습니다.

### 판독 연습 — 한 연결에서 네 가지 근거를 찾습니다

TLS 1.2 전체 핸드셰이크를 처음부터 담은 캡처를 준비합니다. 아래는 학습자가 자기 캡처에 적용하는 절차이며, 특정 파일을 실행해 얻은 패킷 번호나 관찰 결과는 아닙니다. 책의 `two-way-handshake.pcap`이 있다면 상호 인증 메시지까지 같은 방법으로 확인할 수 있습니다.
...
랩에서 TLS 1.3 으로 한 번 붙여 이 표를 실측으로 확인했습니다. 키 없이 `c10-tls13.pcap` 을 열면 핸드셰이크 종류가 `1` 과 `2` 두 개뿐이고, 인증서 이름으로 검색하면 0건입니다. 같은 캡처에 키 로그를 물리면 프레임 7 하나에서 `2, 8, 11, 15, 20` 이 한꺼번에 드러나고 인증서도 1건 잡힙니다. 메시지는 처음부터 와 있었고 가려져 있었을 뿐입니다.

그렇다고 1.3 이 전부를 가리지는 않습니다. 협상에 필요한 값은 여전히 평문으로 지나갑니다.

| 1.3 에서 | 키 없이 보이는가 | 실측값 |
|---|:---:|---|
| SNI | 보입니다 | `a.test` |
| 고른 cipher suite | 보입니다 | `0x1302` |
| `supported_versions` | 보입니다 | TLS 1.3 |
| `key_share` 그룹 | 보입니다 | `29` (x25519) |
| 인증서 | 안 보입니다 | 키 로그가 있어야 종류 11 이 드러납니다 |
| 인증 쪽 선택 | 안 보입니다 | suite 이름에 인증 칸이 없습니다 |

경계가 앞당겨진 폭은 두 버전을 나란히 놓으면 보입니다.

| 버전 | 평문으로 읽히는 끝 |
|---|---|
| TLS 1.2 | ServerHelloDone 까지. CCS 다음 Finished 부터 보호 |
| TLS 1.3 | ServerHello 까지. 그다음 EncryptedExtensions 부터 보호 |

그래서 1.3 캡처에는 서버 개인키로 여는 길이 없습니다. 개인키는 서명에만 쓰이고 키 합의에 관여하지 않으므로 키 로그가 유일한 경로입니다. 이 판정의 근거와 재료 확보 방법은 [04-03 §2](./04-03.%EC%97%B4%EC%87%A0%EC%99%80%20%EC%8B%A4%ED%8C%A8.md) 가 다룹니다.

캡처 없이 서버 설정을 먼저 확인하는 방법도 있습니다. 다음 편이 다루는 `nmap` 스크립트가 그것이며, 서버가 어떤 버전과 cipher suite 를 지원하는지 한 번에 나열해 줍니다.

위 비교표의 항목을 하나씩 움직여 보려면 [Security Ronin 시각화](https://tls-handshake.securityronin.com/)의 what-if 목록이 쓸모 있습니다. 그중 이 편이 다룬 자리와 맞물리는 것만 골랐습니다.
...
- **TLS 1.2 다운그레이드**: 전방 비밀성이 없는 정적 RSA 로 내려갔을 때의 차이를 [04-03 §1](./04-03.열쇠와 실패.md) 과 함께 읽습니다.
...
- [04-02 인증서와 두 서명](./04-02.%EC%9D%B8%EC%A6%9D%EC%84%9C%EC%99%80%20%EB%91%90%20%EC%84%9C%EB%AA%85.md) — 이 편의 뒤. 두 서명이 각각 보증하는 것과 키의 수명
- [04-03 열쇠와 실패](./04-03.열쇠와 실패.md) — 이 편의 뒤. 키 교환 방식·복호화·핸드셰이크 실패 디버깅
- [03-02 TCP가 어긋날 때](./03-02.TCP가 어긋날 때.md) — TLS 아래의 TCP. 핸드셰이크는 TCP 연결 위에서 시작합니다
- [04-04 실습](./04-04.%EC%8B%A4%EC%8A%B5%20%E2%80%94%20TLS%20%ED%95%B8%EB%93%9C%EC%85%B0%EC%9D%B4%ED%81%AC%EB%A5%BC%20%EC%A7%81%EC%A0%91%20%EC%9E%A1%EA%B8%B0.md) — 이 편의 메시지와 Alert 를 컨테이너에서 직접 잡습니다. `s_server`·`s_client`·`curl` 옵션 풀이도 그쪽에 있습니다
