...
표의 `NewSessionTicket` 은 책이 열거한 열 가지 밖이지만 실제 캡처에 자주 나옵니다. 랩의 TLS 1.3 캡처에서는 핸드셰이크가 끝난 뒤 프레임 두 개에 종류 4 가 이어서 찍혔습니다. 핸드셰이크 도중이 아니라 끝난 뒤에 오는 메시지라 순서 그림에는 넣지 않았습니다.
...
둘째 줄은 조각 하나가 사라집니다. OpenSSL 이름 `AES256-SHA256` 에는 키 교환 조각이 없습니다. 키 교환을 하지 않는 suite 여서가 아니라, 키 교환이 RSA 일 때 OpenSSL 이 키 교환 조각을 적지 않기 때문입니다. IANA 이름 `TLS_RSA_WITH_AES_256_CBC_SHA256` 의 첫 `RSA` 가 그 사라진 조각입니다.

생략된 조각은 이름 대신 출력의 열이 알려 줍니다. 둘째 줄의 `Kx=RSA` 가 키 교환 방식을 따로 적어 주기 때문입니다. 이름만 보고 판정하지 않고 열을 보는 편이 안전한 이유입니다.
...
아래 네 역할은 `TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384` 하나를 기준으로 읽습니다. 다른 suite 는 같은 이름 조각이 다른 자리에 앉을 수 있으므로, 앞의 `Kx` 와 `Au` 열로 확인한 뒤에 적용합니다.
...
랩에서 TLS 1.3 으로 한 번 붙여 이 표를 실측으로 확인했습니다. 키 없이 `c10-tls13.pcap` 을 열면 핸드셰이크 종류가 `1` 과 `2` 두 개뿐이고, 인증서 이름으로 검색하면 0건입니다. 같은 캡처에 키 로그를 물리면 프레임 7 하나에서 `2, 8, 11, 15, 20` 이 한꺼번에 드러나고 인증서도 1건 잡힙니다. 숨어 있던 것이지 오지 않은 것이 아닙니다.
