...
# 응답이 한계를 넘으면 잘린 응답(TC=1)을 받고 TCP 로 다시 묻는다. 한계는 OPT 가 알린 크기까지, OPT 가 없으면 512. 서버가 더 작게 자를 수도 있다.
...
d = D(W, H, "PACKET ANALYSIS WITH WIRESHARK · 05-02 §1",
      ...
      "존 전송은 처음부터 TCP 로 간다. 보통 질의는 UDP 로 묻고, 응답이 한계를 넘으면 서버가 잘라서 TC 비트를 세우고 클라이언트가 같은 질의를 TCP 로 다시 묻는다. 한계는 질의의 OPT 가 알린 크기까지이고 서버가 그보다 작게 자를 수도 있으며, OPT 가 없으면 512바이트다.",
...
step(UX, CY - RH / 2, UW, "UDP 53 질의", "OPT 가 있으면 크기 알림")
