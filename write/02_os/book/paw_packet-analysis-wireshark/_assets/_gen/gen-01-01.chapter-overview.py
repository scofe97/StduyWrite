# 01-01 학습 목표 뒤 전체 지도 — 절 다섯을 물음 넷의 읽는 순서로 잇는다.
#           §4(서 있는 위치의 한계)와 §5(도구 기능의 한계)는 본문이 적은 넷째 물음 "어디까지가 경계인가" 하나에
#           함께 답하므로 한 칸에 묶는다 — 본문 "네 물음의 순서로 이은 지도" 가 칸 수를 넷으로 고정한다.
...
d = D(W, H, "PACKET ANALYSIS WITH WIRESHARK · 01-01",
      ...
      "1장 노트의 절 다섯을 물음 넷의 읽는 순서로 이은 지도. 분석기가 서는 자리에서 출발해 Wireshark 의 구성과 캡처 전 관문을 지나, 그 자리에서 보이지 않는 것과 이 도구가 못 하는 일로 닫는다.",
...
cards = [
    ...
    ("§4 · §5", "보이는 범위와 못 하는 일", "주소 종류 · 암호화 · 편집 · 재생 · 모바일"),
]
