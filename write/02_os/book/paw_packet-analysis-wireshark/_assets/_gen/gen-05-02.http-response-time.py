...
d = D(W, H, "PACKET ANALYSIS WITH WIRESHARK · 05-02 §3",
      "가장 느린 응답을 찾는 순서",
      "원문이 제시하는 세 단계. 재조립 설정을 먼저 끄는 이유는 한 응답이 몇 개의 세그먼트로 쪼개져 왔는지를 세기 위해서다. 원문은 그 개수를 알면 TCP 윈도우 크기 같은 파라미터를 미세 조정하는 데 도움이 된다고 적는다. 마지막 칸은 원문 절차가 아니라 노트가 덧붙인 다음 볼 곳이다.",
      "첫 칸의 설정이 목록을 바꿉니다 — 끄면 첫 조각 뒤 조각마다 Continuation 한 줄입니다")
...
oval(CX, Y[4], 440, 40, "노트 · 조각 수는 창 크기 열을 볼 계기", OK)
