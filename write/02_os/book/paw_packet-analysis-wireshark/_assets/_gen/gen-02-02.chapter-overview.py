# 02-02 학습 목표 뒤 전체 지도 — 절 여섯을 화면을 읽어 내려가는 순서로 잇는다.
...
d = D(W, H, "PACKET ANALYSIS WITH WIRESHARK · 02-02",
      "잡은 패킷을 읽는 법 — 읽는 순서",
      "2장 분석 축 노트의 절 여섯을 화면을 읽어 내려가는 순서로 이은 지도. 네 개 창의 관계에서 출발해 색과 필터로 범위를 좁히고, 목록 한 줄과 그 안의 계층으로 내려간 뒤, 화면의 글자가 사실인지 추측인지 판단인지를 가른다.",
      "세 번째 칸이 이 편의 중심입니다 — 필터는 지우지 않고 시야만 좁힙니다")

CW, CH, GAP, X0 = 400, 80, 24, 24
ROW, Y0 = 112, 112
cards = [
    ("§1", "네 개 창이 보는 같은 바이트",  "목록 · 계층 · 바이트의 겹침"),
    ("§2", "색으로 먼저 보이는 이상",     "컬러링 규칙이 먼저 잡는 줄"),
    ("§3", "디스플레이 필터",             "지우지 않고 시야만 좁히기"),
    ("§4", "Packet List 의 일곱 열",      "한 줄이 답하는 것과 시간 기준"),
    ("§5", "Packet Details 의 계층",      "frame 부터 응용 계층까지"),
    ("§6", "화면의 어디까지가 사실인가",   "사실 · 추측 · 판단 가르기"),
]
