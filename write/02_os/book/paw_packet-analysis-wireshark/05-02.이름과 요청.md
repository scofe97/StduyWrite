...
HTTP 쪽에서 읽을 것은 트랜잭션의 응답 시간과 상태 코드입니다. HTTP 트랜잭션은 요청 하나와 그에 대한 응답 하나의 짝입니다. TCP 연결 하나에 이 짝이 여럿 차례로 실립니다. 원문이 제시하는 절차의 첫 단계가 TCP 재조립을 *끄는* 것이라는 점이 눈에 띕니다. 한 응답이 몇 개의 TCP 세그먼트로 쪼개져 왔는지를 세기 위해서입니다. 원문은 그 개수를 TCP 윈도우 크기 같은 파라미터를 조정하는 근거로 씁니다.
...
응답이 왔는데 답에 주소가 없으면 헤더의 응답 코드(RCODE)를 봅니다. `dns.flags.rcode == 3` 은 Name Error, 곧 질의한 이름이 없다는 뜻입니다.[^rfc1035-rcode] 마지막 판단부터가 연결입니다. SYN 에 RST 가 돌아오는지, 아무 응답 없이 SYN 만 다시 나가는지는 [03-02](./03-02.TCP%EA%B0%80%20%EC%96%B4%EA%B8%8B%EB%82%A0%20%EB%95%8C.md) §1 「RST — 연결이 거부되거나 끊길 때」에서 가릅니다.
...
```bash
dig +noedns TXT google.com @8.8.8.8   # OPT 없이 — 한계 512
dig TXT google.com @8.8.8.8           # 기본값 — OPT 에 4096
```
...
```bash
dns.flags.truncated == 1    # TC 가 켜진 잘린 응답
tcp.port == 53              # TCP 로 넘어간 질의
```
...
```bash
dns.resp.type == 5      # 응답 섹션의 CNAME 줄
dns.resp.type == 28     # 응답 섹션의 AAAA 줄
```
...
3장 원문의 진단 예제는 켠 쪽에서 셉니다. `slow_download.pcap` 에서 `tcp.segment.count == 2584` 로 5MB 응답이 2,584개 조각이었음을 확인하고 창 크기를 보러 갑니다. 그 절차는 [03-02](./03-02.TCP%EA%B0%80%20%EC%96%B4%EA%B8%8B%EB%82%A0%20%EB%95%8C.md) §4 「원문의 진단 예제」에 있습니다. 두 장이 같은 지표를 재조립 끔과 켬의 양쪽에서 다루는 셈입니다.

조각 수는 그 자체로 답이 아니라 다음에 볼 곳을 정합니다. 수신 창은 받는 쪽이 지금 더 받을 수 있다고 알리는 바이트 수이고, 보내는 쪽은 데이터를 그 창에 맞는 세그먼트로 꾸립니다.[^rfc9293-window] 창이 조각 하나에 실을 수 있는 최대 크기(MSS)보다 작게 묶이면 조각이 창 크기로 잘게 쪼개져 수가 늘어납니다. 창만큼 보낼 때마다 확인을 기다려야 하니 왕복도 늘어납니다. 그래서 조각이 많다 싶으면 3장 절차대로 창 크기 열을 봅니다.

다만 3장 예제의 창 100 과 조각 2,584 를 곧바로 인과로 잇지는 않습니다. 5,242,880바이트를 100바이트씩 나르면 조각이 52,428개 필요하므로 산술이 맞지 않고, 원문도 100 에서 2,584 를 끌어내지 않습니다. 03-02 §4 의 노트의 읽기가 그 어긋남을 적어 두었습니다.
...
> 필터 이름은 그대로이고 HTTP 상태 코드에 하나가 더해졌습니다. 원문이 다루지 않는 HTTP 버전과 암호화된 DNS 도 함께 봅니다.
...
2. **원문은 성능을 잴 때 TCP 재조립을 끕니다.** 끄면 응답 첫 조각에 HTTP 줄이 붙고 나머지가 Continuation 줄로 늘어서 몇 조각인지 목록에서 셉니다. 켜 두면 같은 개수가 마지막 조각의 상세 창(`tcp.segment.count`)으로 옮겨 갑니다. 조각이 많으면 창 크기를 보러 갑니다.
...
A: 아닙니다. 응답이 한계를 넘어 잘렸으면(TC=1) 클라이언트가 TCP 로 다시 묻습니다. 존 전송(AXFR)은 처음부터 TCP 입니다. 한계는 질의의 OPT 에 적은 크기이며, OPT 가 없으면 512바이트입니다.
...
[^rfc1035-rcode]: RFC 1035 §4.1.1 RCODE — "3 Name Error - Meaningful only for responses from an authoritative name server, this code signifies that the domain name referenced in the query does not exist." <https://www.rfc-editor.org/rfc/rfc1035.txt> (2026-09-21 조회)

[^rfc1034-zone]: RFC 1034 §4.2 — "The database is divided up into sections called zones, which are distributed among the name servers." §4.3.5 — "Because accuracy is essential, TCP or some other reliable protocol must be used for AXFR requests." <https://www.rfc-editor.org/rfc/rfc1034.txt> (2026-09-21 조회)
