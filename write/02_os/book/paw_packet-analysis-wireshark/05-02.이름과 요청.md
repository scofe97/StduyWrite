...
HTTP 쪽에서 읽을 것은 트랜잭션의 응답 시간과 상태 코드입니다. HTTP 트랜잭션은 요청 하나와 그에 대한 응답 하나의 짝입니다. TCP 연결 하나에 이 짝이 여럿 차례로 실립니다. 원문이 제시하는 절차의 첫 단계가 TCP 재조립을 *끄는* 것이라는 점이 눈에 띕니다. 한 응답이 몇 개의 TCP 세그먼트로 쪼개져 왔는지를 세기 위해서입니다. 원문은 그 개수를 알면 TCP 윈도우 크기 같은 파라미터를 미세 조정하는 데 도움이 된다고 적습니다. 개수가 곧 조정값을 정해 주지는 않으며, 어디를 볼지는 §3 에서 가립니다.
...
|---|---|---|---|
| 이름 해석 (name resolution) | 이름을 주소로 바꾸는 단계 | 여기서 막히면 접속 자체가 안 됨 | §1 |
| DNS over TCP | 잘리면 TCP 로 다시 묻기 | EDNS(0) 는 UDP 한계만 늘림 · 존 전송은 늘 TCP | §1 |
| 리소스 레코드 (resource record) | DNS 가 돌려주는 답의 단위 | 타입마다 담는 것이 다름 | §2 |
...
위의 세 판단이 이름 해석입니다. `dns` 질의가 아예 없으면 리졸버가 TTL 안에 캐시해 둔 답을 썼을 수 있습니다. RFC 1034 가 정한 리졸버 동작의 첫 단계가 가진 정보에 답이 있으면 그것을 돌려주는 것이라, 이때는 질의가 선에 나가지 않습니다.[^rfc1034-cache] 질의를 TLS 로 감싸는 DoT·DoH 여도 `dns` 필터에 안 잡히며, 이 갈래는 심화 학습에서 봅니다.

응답이 왔는데 답에 주소가 없으면 헤더의 응답 코드(RCODE)를 봅니다. `dns.flags.rcode == 3` 은 Name Error, 곧 질의한 이름이 없다는 뜻입니다.[^rfc1035-rcode]
...
여러 답이 올 수 있다는 마지막 조건이 화면을 읽을 때 자주 걸립니다. 응답 섹션에 줄이 여럿이면 그것은 오류가 아니라 **CNAME 체인**이거나 여러 주소를 가진 이름입니다.
...
RFC 6891 이 정의한 EDNS(0) 가 바꾼 것은 이 512 라는 한계입니다. 클라이언트가 질의의 추가 섹션에 OPT 의사 레코드(타입 41)를 붙이고, 그 CLASS 자리에 자기가 받아 재조립할 수 있는 UDP 페이로드 크기를 적습니다.[^rfc6891] 서버는 그 크기까지는 자르지 않고 UDP 로 답할 수 있습니다.[^rfc7766] 그래서 512바이트를 넘는 응답도 UDP 한 번에 오고 TCP 로 넘어가는 일이 줄어듭니다. 알린 크기마저 넘는 응답은 여전히 잘리고 TCP 로 넘어갑니다.
...
3장 원문의 진단 예제는 재조립이 켜진 쪽에서 셉니다. 다만 3장이 직접 켜는 것은 `Protocols | HTTP` 의 재조립 옵션이고, 5장이 끄는 `Protocols | TCP` 의 Allow subdissector 설정은 기본값인 켬 그대로입니다. 2026-09-23 tshark 4.6.8 의 기본 설정에서 `tcp.desegment_tcp_streams`·`http.desegment_headers`·`http.desegment_body` 셋이 모두 켬이었습니다.

`slow_download.pcap` 에서 `tcp.segment.count == 2584` 로 5MB 응답이 2,584개 조각이었음을 확인하고 창 크기를 보러 갑니다. 그 절차는 [03-02](./03-02.TCP%EA%B0%80%20%EC%96%B4%EA%B8%8B%EB%82%A0%20%EB%95%8C.md) §4 「원문의 진단 예제」에 있습니다. 5장은 TCP 설정을 꺼서 목록에서 세고, 3장은 기본값대로 두고 필드로 세는 셈입니다.
...
다만 조각 수는 응답이 클수록 당연히 늘어나므로 개수 하나만으로 창이 작다고 결론 내리지는 않습니다. 개수는 창 크기 열을 볼 계기이고, 판단은 3장 절차대로 창 크기 열에서 합니다.

다만 3장 예제의 창 100 과 조각 2,584 를 곧바로 인과로 잇지는 않습니다. 5,242,880바이트를 100바이트씩 나르면 조각이 52,429개 필요하므로 산술이 맞지 않습니다. 게다가 원문이 열로 올린 것은 `tcp.window_size_value`, 곧 창 배율을 곱하기 전의 값이라 100 이 실제 바이트 수라는 보장도 없습니다. 원문도 100 에서 2,584 를 끌어내지 않습니다. 03-02 §4 의 노트의 읽기가 그 어긋남을 적어 두었습니다.
...
마지막 CONNECT 가 실무에서 자주 걸립니다. 프록시를 거쳐 HTTPS 를 쓰면 첫 요청이 `CONNECT` 로 나가고, 그 뒤로는 HTTP 요청·응답 줄이 더 나오지 않습니다. 프록시 환경의 캡처에서 HTTP 가 갑자기 끊긴 것처럼 보이면 이 메서드를 찾아봅니다. 왜 끊겨 보이는지는 바로 아래에서 봅니다.
...
터널은 바이트를 바꾸지도 감싸지도 않습니다. 그래서 그 안으로 4장의 TLS 핸드셰이크가 그대로 지나갑니다. ClientHello 는 평문이고 SNI 도 읽힙니다([04-01](./04-01.TLS%20%ED%95%B8%EB%93%9C%EC%85%B0%EC%9D%B4%ED%81%AC%20%EC%9D%BD%EA%B8%B0.md) §3). 내용이 안 보이기 시작하는 자리는 터널이 열린 곳이 아니라 TLS 가 보호를 시작하는 곳입니다(04-01 §5).
...
A: 먼저 어느 구간이 느린지 가릅니다. 연결 수립 왕복은 SYN 과 SYN/ACK 사이에서 따로 보고, `http.time` 은 요청에서 응답까지라 서버 처리가 들어 있습니다. 응답이 여러 조각으로 오래 걸렸다면 조각 수와 창 크기를 봅니다. 창이 작게 묶이면 조각 수와 왕복이 늘어나기 때문입니다. 다만 3장 예제의 창 100 과 조각 2,584 는 산술로 이어지지 않으니 인과로 외우지 않습니다.
...
- [ ] DNS 가 TCP 로 넘어가는 두 경우와, EDNS(0) 가 그중 무엇을 바꾸는지 설명할 수 있습니까?
- [ ] 캡처에서 이름 해석 실패와 연결 실패를 무엇이 있고 없는지로 가를 수 있습니까?
- [ ] `http.time` 이 재는 구간이 연결 수립 왕복과 어떻게 다른지, 연결 하나에 트랜잭션이 여럿이면 SYN 이 몇 번인지 말할 수 있습니까?
- [ ] 재조립을 켤 때와 끌 때 조각 수와 `http.time` 이 어디에 찍히는지 말할 수 있습니까?
- [ ] 성공했지만 느린 요청만 뽑는 필터를 쓸 수 있습니까?
- [ ] 프록시가 목적지를 아는 것이 CONNECT 덕인지 SNI 덕인지, CONNECT 뒤가 안 보이는 이유가 무엇인지 설명할 수 있습니까?
- [ ] 평문 DNS 가 UDP·TCP 어느 쪽이든 읽히는 이유와 DoT·DoH 가 바꾸는 축을 말할 수 있습니까?
...
- [03-02 TCP가 어긋날 때](./03-02.TCP%EA%B0%80%20%EC%96%B4%EA%B8%8B%EB%82%A0%20%EB%95%8C.md) — 3장. 재조립을 켠 채 조각 수를 세는 진단 예제와, 창 100 과 조각 2,584 를 인과로 잇지 않는 이유
