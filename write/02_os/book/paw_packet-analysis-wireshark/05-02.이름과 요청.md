...
| RCODE 0 인데 답이 비어 있음 | 그 타입의 레코드가 없습니다(NODATA) | 질의 타입 |
...
| 연결 왕복은 짧은데 `http.time` 이 긺 | 요청 뒤 응답까지가 느립니다 (서버 처리 · 응답 전송) | `http.response.code == 200 && http.time > 1` 로 추린 뒤 조각 수 · 창 크기 열 |
| `http` 에 0줄인데 웹은 됨 | HTTPS · HTTP/2 · HTTP/3 입니다 | 복호화 · `http2` · `quic` |
