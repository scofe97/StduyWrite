...
`http.time` 도 붙는 프레임을 따라 옮깁니다. 끄면 첫 조각에서 재어 32.352ms, 켜면 마지막 조각에서 재어 32.929ms 였습니다. 두 값의 차이는 첫 조각과 마지막 조각 사이, 곧 응답 본문이 전송되는 동안의 시간입니다. 재조립은 2장에서 헤더가 끊겨 보일 때 켜는 설정으로 먼저 나왔습니다([02-03](./02-03.%EB%B6%84%EC%84%9D%EC%9D%84%20%EB%8F%95%EB%8A%94%20%EA%B8%B0%EB%8A%A5%EB%93%A4.md) §2). 여기서는 목록에서 조각을 세려고 끕니다.
...
3장은 `Protocols | HTTP` 의 재조립 옵션을 켭니다. 5장이 끄는 `Protocols | TCP` 설정은 기본값이 켬입니다. 2026-09-23 tshark 4.6.8 기본 설정에서 `tcp.desegment_tcp_streams`·`http.desegment_headers`·`http.desegment_body` 는 모두 켬이었습니다.

조각 수는 다음에 어디를 볼지 알려 줍니다. 수신 창은 받는 쪽이 지금 더 받을 수 있다고 알리는 바이트 수입니다. 보내는 쪽은 데이터를 그 창에 맞는 세그먼트로 꾸립니다.[^rfc9293-window] 창이 조각 하나에 실을 수 있는 최대 크기(MSS)보다 작게 묶이면 조각이 창 크기로 잘게 쪼개져 수가 늘어납니다. 창만큼 보낼 때마다 확인을 기다려야 하니 왕복도 늘어납니다.
