...
related:
  ...
  - ./01-01a.%EC%8B%A4%EC%8A%B5%20%E2%80%94%20%EC%BA%A1%EC%B2%98%20%ED%99%98%EA%B2%BD%EA%B3%BC%20%EC%B2%AB%20%ED%94%84%EB%A0%88%EC%9E%84.md
...
learning:
  ...
  next_lesson: "1장 Phase 4 자답. Phase 3 실습(2026-09-14)에서 계층 위치와 캡슐화는 화면으로 정착됐으므로, 첫 문항은 남은 두 축입니다 — 스위치가 MAC 테이블로 포트를 가르는 것, 캡처 전 다섯 관문. 실습에서 확인된 멀티캐스트 예외를 함정 축 문항의 재료로 씁니다"
updated: 2026-09-14
---
...
| 다음 레슨 후보 | **1장 Phase 4 자답입니다.** 2026-09-14 Phase 3 실습에서 세 가지를 맥북에서 직접 돌렸습니다 — `dumpcap -D` 로 인터페이스 목록(26 대 26 으로 `ifconfig` 와 같게 나와, "권한 있는 것만 나온다" 가 ChmodBPF 로 권한이 다 열린 환경에서는 드러나지 않는다는 조건을 확인), `tshark -i en0 -c 1 -V` 로 프레임 맨 위의 이더넷 헤더, `dumpcap -w` 뒤 `tshark -r` 로 오프라인 분업. **실습 20장 안에 같은 네트워크의 다른 기기 이름이 mDNS 멀티캐스트로 들어와, §3 첫째 관문(허락받았는지 확인)이 왜 첫 자리인지가 실측으로 드러났습니다.** 자답 첫 문항은 남은 두 축입니다. 그다음이 2장이며, 3장의 Java 소켓 재현과 6장의 monitor 모드 캡처, 7장의 SYN 홍수·ICMP 홍수는 맥북에서 직접 캡처를 떠 IO Graph 와 Conversations 를 열어 볼 수 있습니다 |
