...
learning:
  ...
  scope: durable
  level: 입문            # 1장 Phase 1 에서 전제 결손 3건 — 러너 규칙대로 선행 개념 쪽으로 내렸습니다
  last_verified:            # Phase 4 자답 여전히 미실시. 2026-09-14 세션은 Phase 1 이었으므로 채우지 않습니다
  blocked_count: 3          # Phase 1 기준 — 분석기의 계층 위치·스위치의 MAC 기반 포트 결정·캡처 전 다섯 관문
  next_lesson: "1장 Phase 3 실습. 맥북에서 `dumpcap -D` 와 `tshark -i en0 -c 1 -V` 를 직접 돌려, 2026-09-14 에 산문으로만 세운 계층 위치를 화면으로 확인합니다. 그 다음이 1장 Phase 4 자답이며, 다섯 관문과 계층 위치를 첫 문항으로 냅니다"
updated: 2026-09-14
---
...
| 항목 | 값 |
|------|-----|
| 난이도 레벨 | 입문 — **읽기는 7개 장 전부 끝났으나 정착은 그만큼 가지 않았습니다.** 2026-09-14 에 1장을 4-Phase 로 열었을 때, 계획은 Phase 4 자답이었는데 회상할 것이 서 있지 않아 Phase 1 로 내려가 개념을 새로 세웠습니다. 노트가 있다는 것과 그 내용이 머리에 있다는 것이 다르다는 게 이 회차의 소득입니다. 러너의 난이도 규칙(3문항 이상 막힘 또는 전제 오류 시 선행 개념으로)에 따라 기본에서 입문으로 내렸습니다 |
| 막힌 지점 | **2026-09-14 1장 Phase 1 에서 전제 결손 세 건이 드러났습니다.** 첫째, 패킷 분석기의 자리를 IP 계층으로 잡았습니다 — 링크 계층 프레임과 IP 패킷을 "다음에 오는 것"으로 보는 순차 감각이 원인이라, 프레임이 IP 패킷을 담는 포함 관계를 먼저 세운 뒤에야 "MAC 이 보이면 안 벗겨진 것"으로 스스로 뒤집었습니다. 둘째, 같은 LAN 에서 프레임을 어느 포트로 보낼지 정하는 장비를 게이트웨이로 지목했습니다 — NAT·포트포워딩은 알고 있으나 스위치의 MAC 테이블이 빈자리였습니다. 셋째, 캡처 전 다섯 관문을 회상하지 못했습니다. 반면 promiscuous 모드는 정의만 듣고 "안 오는 트래픽은 어차피 안 보인다"를 스스로 끌어냈고, `dumpcap` 이 실제 캡처를 맡는 것도 맞혔습니다 — **개념을 주면 그 자리에서 반례를 만들어 검증하는 쪽은 강하고, 계층 위치·절차 순서 같은 정적 지식의 회상이 약합니다.** 실습 공백도 그대로입니다 — 원문이 인용하는 예제 캡처 파일(`normal-connection.pcap`·`RST-01.pcap`·`close_wait.pcap`·`slow_download.pcap`·`two-way-handshake.pcap`·`DHCPv6-Flow-SOLICIT.pcap`·`802.11.pcap` 등)이 PDF 에 경로 없이 이름만 나와 있어, 화면 대조 없이 원문 서술과 페이지 이미지로만 작성했습니다 |
| 다음 레슨 후보 | **1장 Phase 3 실습이 먼저입니다.** 2026-09-14 회차에서 계층 위치를 산문으로만 세웠고 학습자가 "와이어샤크를 아직 안 봐서 진짜인지 모르겠다"고 정확히 짚었으므로, 맥북에서 `dumpcap -D` 와 `tshark -i en0 -c 1 -V` 를 직접 돌려 프레임 맨 위의 `Ethernet II, Src/Dst` 를 눈으로 확인하는 일이 다음 칸입니다. 그다음이 1장 Phase 4 자답이며, 막힌 세 축(계층 위치·스위치의 MAC 기반 포트 결정·다섯 관문)을 첫 문항으로 냅니다. 2장 이후는 그 뒤입니다 — 3장의 Java 소켓 재현과 6장의 monitor 모드 캡처, 7장의 SYN 홍수·ICMP 홍수는 맥북에서 직접 캡처를 떠 IO Graph 와 Conversations 를 열어 볼 수 있습니다 |
