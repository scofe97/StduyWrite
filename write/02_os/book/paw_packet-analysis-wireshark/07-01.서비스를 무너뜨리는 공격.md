...
|---|---|---|---|
| SYN 홍수 (SYN flood) | 절반만 연 연결로 자리를 채움 | 서버가 규격을 지켜서 무너짐 | §2 |
| 반열림 연결 (half-open) | ACK 가 안 오는 연결 | ACK 가 SYN 의 절반으로 나오는 이유 | §2 |
| SYN 쿠키 (SYN cookie) | 자리를 안 잡고 여는 방식 | 커널 쪽 완화책 · 실제로 듣는 항목 | §4 |
| 반사 (reflection) | 제3자를 거쳐 보냄 | 피해자 캡처에 공격자가 없음 | §5 |
| 증폭 (amplification) | 작은 요청이 큰 응답을 부름 | DrDoS · 출발지 주소를 속임 | §5 |
| Conversations | 대화 단위 통계 창 | 한 방향만 흐르는 대화 · 평균 프레임 크기 | §3 |
...
첫 행의 기본값이 원문의 첫 줄을 바꿔 놓습니다. `tcp_syncookies` 는 현행 커널 기본값이 이미 1 이라, 원문의 "SYNcookie 를 켜라"는 지시가 지금은 대부분 아무것도 바꾸지 않습니다. 그리고 커널 문서가 이것을 최후 수단으로 못박습니다. **"syncookies 는 폴백 설비입니다. 부하가 높은 서버가 정상적인 연결 속도를 견디도록 돕는 데 써서는 안 됩니다."**

이유도 같은 문서의 한 문장에 적혀 있습니다. **"syncookies 는 TCP 프로토콜을 심각하게 위반하고 TCP 확장을 쓸 수 없게 하며 일부 서비스(예: SMTP 릴레이)를 심각하게 저하시킬 수 있습니다."**[^sysctl]

로그에 SYN 홍수 경고가 뜨는데 실제로는 공격이 아니라면 그것은 방어가 아니라 설정 오류의 신호입니다.
...
> 원문의 명령과 필터는 대부분 지금도 그대로 돕니다. 원문이 다루지 않는 SYN 쿠키의 대가와 위조 출발지 필터링을 함께 봅니다.

원문의 `tcp.flags.syn==1` 등 네 필터는 지금도 그대로 컴파일됩니다. `Statistics | IO Graph` · `Statistics | Conversations` 는 Statistics 메뉴 아래에, `Tools | Firewall ACL Rules` 는 Tools 메뉴 아래에 그대로 있습니다.
