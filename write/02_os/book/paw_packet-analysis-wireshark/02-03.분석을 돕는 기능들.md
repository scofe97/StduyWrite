...
Protocol 열에 TCP 가 찍히는 것이 왜 문제인지는 잃는 것을 세어 보면 드러납니다. 아래는 문서용 주소(RFC 5737)로 만든 캡처에서 18080 포트로 보낸 HTTP 요청 한 프레임입니다. 원문 시절처럼 포트로만 판별하는 상황을 재현하려고 tshark 4.6.8 의 HTTP 휴리스틱을 끄고 읽었습니다.

```bash
tshark -r http-18080.pcap --disable-heuristic http_tcp
```
...
포트 번호를 얼마나 믿을지는 [02-01 패킷을 잡는 법](./02-01.패킷을 잡는 법.md) §3 의 포트 구간이 다룹니다. 이런 짐작이 화면에서 어떻게 사실과 섞이는지는 [02-02 잡은 패킷을 읽는 법](./02-02.잡은 패킷을 읽는 법.md) §6 이 다룹니다.

> **지금은 다릅니다 (4.6)**: 현행 판은 포트 표에 더해 패킷 내용으로 짐작하는 휴리스틱 디섹터를 씁니다.[^wsug-dissection] 
>
> 2026-09-14 빈 설정의 tshark 4.6.8 에서는 `HTTP over TCP` 와 `SSL/TLS over TCP` 휴리스틱이 기본으로 켜져 있었습니다. 18080 의 HTTP 요청도, 4433 의 TLS ClientHello 도 스스로 알아봤습니다. 공식 문서가 알아보지 못하는 예로 드는 800 포트의 HTTP 도 같은 설정에서는 HTTP 로 읽혔고, HTTP 기본 포트 표에는 8080 도 들어 있었습니다. 반면 연결 중간부터 잡혀 요청 줄 없이 본문 조각만 있는 18080 의 프레임은 TCP 로 남았고, Decode As 를 걸어도 HTTP 로 풀리지 않았습니다. 요즘 TCP 로만 보이는 경우는 휴리스틱이 없는 프로토콜이 표에 없는 포트에서 돌 때와, 짐작할 머리가 캡처에 없을 때입니다.
