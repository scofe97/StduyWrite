...
related:
  - 15-01.BPF — 개요·BCC vs bpftrace·BCC.md
  - 13-02.perf — 이벤트 소스.md
  - 00-00.책 개요와 학습 로드맵.md
updated: 2026-06-06
---


# BPF — bpftrace 도구·원라이너
---
...
> **갱신 (2026-09)**: 4.9 는 원서 시점의 하한입니다. 지금 upstream bpftrace 의 지원 정책은 **최소 커널 6.1**(안정 커널과 최근 LTS 넷)이고, 더 오래된 커널에서는 그 커널을 지원하던 구버전 bpftrace 를 씁니다. 또 0.25 부터 tracepoint 의 `args` 를 쓰려면 BTF 가 필요합니다.

**도구** 는 bpftrace 저장소(30개 넘음)와 저자의 bpf-perf-tools-book 저장소(추가 다수)에 있습니다.

> bpftrace의 핵심은 *추적의 awk* 라는 점입니다 — 간결한 문법으로 프로브·필터·액션을 한 줄에 쓰고, 종료 시 맵을 자동 출력합니다. 위 한 줄이 보이듯, kretprobe·필터·맵·hist를 조합해 "수신 크기를 프로세스별 히스토그램으로"를 한 줄에 표현합니다 — BCC 도구의 평균 길이가 228줄인 것과 대비됩니다(15-01).
