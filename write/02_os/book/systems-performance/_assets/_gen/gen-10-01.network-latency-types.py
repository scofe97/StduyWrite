...
for k in ("start", "syn", "est", "byte", "close"):
    # 눈금선은 막대 위까지만 긋는다 — 막대 행까지 내리면 오른쪽 주석 글자를 관통한다
    d.line(T[k], AXIS + 8, T[k], BAR_Y - 8, RULE, 1.0, "3 6")
...
d.t(T["start"] + 128, y + 21, "ICMP echo · 전파 + 홉 처리 · 라우터 우선순위가 다를 수 있음",
    13, MUTED, KR, "start")

y2 = y + BAR_STRIDE + 16
d.t(X0 - 12, y2, "스케일(localhost ping = 1초): 10GbE 4초 · 1GbE 12초 · Wi-Fi 1분 · SF→뉴욕 13분 · SF→호주 1시간",
    13, SOFT, KR, "start")

d.legend(y2 + 24, [("서버 think time 을 품는 지연", ACC), ("네트워크만 재는 왕복", INFO), ("구간 지연", MUTED)])
