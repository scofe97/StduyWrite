...
d.t(373, 302, "있음", 11, ACC, KR, "middle")
d.t(631, 302, "없습니다 — 그것이 마지막이었음", 11, BAD, KR, "middle")

LEFT = [("뒤 세그먼트가 도착함", "받는 쪽이 구멍을 봄", OK),
        ("중복 ACK 를 즉시 보냄", "도착이 방아쇠", OK),
        ("셋이 모이면 재전송", "타이머 만료를 안 기다림", ACC)]
RIGHT = [("아무것도 도착하지 않음", "받는 쪽은 구멍을 못 봄", SOFT),
         ("중복 ACK 가 안 생김", "만들 계기가 없음", BAD),
...
d.t(44, 642, "빠른 재전송은 타임아웃을 대체하는 것이 아니라 흔한 경우를 앞당기는 것 · 마지막 세그먼트가 사라지면 왼쪽이 통째로 사라짐",
    11, MUTED, KR, "start")
