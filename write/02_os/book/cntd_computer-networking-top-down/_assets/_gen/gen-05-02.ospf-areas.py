...
AREAS = [(40, 170, "영역 1", "링크 상태는 여기서만 돔"),
         (370, 500, "영역 2", "자기 영역의 지도만 암"),
         (700, 830, "영역 3", "밖은 경계 라우터에게 맡김")]
AW, AY, AH = 260, 112, 96
...
d.t(500, KY + 50, "모든 영역 경계 라우터를 반드시 포함", 11, MUTED, KR)

# 영역을 넘는 트래픽은 백본 안을 지난다
d.arrow([(190, KY + 74), (810, KY + 74)], INFO, "info", 1.8)
d.t(500, KY + 90, "영역 1 에서 영역 3 으로 가는 트래픽은 여기를 지남", 11, INFO, KR)

d.t(30, 434, "라우터는 링크 상태가 바뀔 때마다, 그리고 바뀌지 않아도 최소 30분에 한 번 광고를 다시 뿌림", 11, MUTED, KR, "start")
d.t(30, 454, "RFC 2328 — 주기적 재광고가 링크 상태 알고리즘에 견고함을 더함", 11, MUTED, KR, "start")
d.t(30, 474, "OSPF 메시지는 IP 위에 직접 · 상위 프로토콜 번호 89", 11, SOFT, KR, "start")
