...
band(Y1, "L7", "HTTP/2 프레이밍 층", "스트림마다 따로 이어 붙임")
band(Y2, "L4", "TCP 수신 버퍼", "스트림 경계를 모름", focal=True)
band(Y3, "L3", "네트워크", "세그먼트 2 가 도중에 사라졌음")
...
d.t(24, 464, "HTTP/2 는 프레임을 섞어 앞 객체가 뒤 객체를 막는 일을 없앴음",
     13, MUTED, KR, "start")
d.t(24, 486, "그런데 TCP 는 바이트 스트림 하나라, 하나가 빠지면 뒤에 쌓인 것을 스트림과 무관하게 붙듦",
     13, MUTED, KR, "start")
d.t(24, 508, "같은 형태가 04-02 의 입력 큐에도 있음 · 앞의 것이 못 나가면 뒤도 못 나감",
     13, SOFT, KR, "start")

d.legend(H - 44, [("빠진 것과 그것이 막은 것", BAD), ("올라간 세그먼트", OK), ("기다리는 스트림", WARN), ("막히는 층", ACC)])
