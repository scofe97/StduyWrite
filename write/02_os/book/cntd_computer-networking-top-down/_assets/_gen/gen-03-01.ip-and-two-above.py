...
d.t(48, 494, "호스트 사이까지만 · 보장하지 않는 것이 셋", 11, MUTED, KR, "start")
for i, (lab, note) in enumerate([("배달", "세그먼트가 도착한다는 보장이 없음"),
                                 ("순서", "보낸 순서대로 온다는 보장이 없음"),
                                 ("무결성", "안의 데이터가 그대로라는 보장이 없음")]):
    x = 48 + i * 306
    d.chip(x + 34, 534, lab, BAD)
    d.t(x + 74, 538, note, 11, SOFT, KR, "start")

d.t(24, 604, "UDP 의 '더하지 않는 것' 칸을 아래 셋과 겹쳐 보면 같은 말 · UDP 는 IP 의 결함을 고치지 않고 포트와 체크섬만 얹음",
    11, ACC, KR, "start")
d.t(24, 626, "TCP 의 혼잡 제어만 성격이 다름 · 부른 앱을 위한 서비스라기보다 인터넷 전체를 위한 것이라, 앱이 원해서 얻는 기능이 아님",
    11, MUTED, KR, "start")
