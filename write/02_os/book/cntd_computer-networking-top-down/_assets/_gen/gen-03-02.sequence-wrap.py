...
TOP = [
    (170, 80, 180, "세그먼트 A", "seq 1000", "보냈는데 큐에 붙들림", WARN),
    (470, 330, 280, "그 사이 4 GiB 를 나름", "2³² 바이트", "번호가 0 으로 돌아옴", MUTED),
...
d.t(330, 324, "망이 붙들고 있다가 뒤늦게 뱉어 냄", 11, WARN, KR)
d.arrow([(750, AXY), (750, 346)], ACC, "acc", 1.4)

BOT = [
    (430, 230, "A 가 먼저 도착함", "그 자리에 옛 내용을 넣음", "옛 내용이 들어감", BAD),
    (700, 230, "B 가 나중 도착함", "이미 1000 을 받았다고 봄", "진짜 새것이 버려짐", BAD),
]
...
d.t(44, 512, "아무도 오류를 못 느낌", 12, INK, KR, "start", 600)
d.t(44, 534, "체크섬은 A 자신에 대해 계산된 것이라 통과하고 ACK 도 정상으로 나가니 재전송이 일어나지 않음 · 종단 간 검사도 이것은 못 잡음",
    11, MUTED, KR, "start")
d.t(44, 556, "비트가 뒤집힌 것이 아니라 멀쩡한 세그먼트가 엉뚱한 자리에 들어앉은 것이기 때문 · 그래서 번호 밖의 축인 타임스탬프가 필요함",
    11, OK, KR, "start")
