...
T1B = unit(332, 140, 188, 60, "tier-1 ISP", "아무에게도 안 냄")
RGA = unit(88, 264, 172, 60, "지역 ISP", "성·국가 단위도 있음")
RGB = unit(316, 264, 172, 60, "지역 ISP", "여럿이 경쟁함")
...
d.t(CX0 + CW / 2, 214, "IXP 안에도 작은 데이터센터를 둠", 11, MUTED, KR)
...
d.t(CX0 - 96, 322, "상위 계층을 건너뜀", 11, ACC, KR)
d.path(f"M {CX0 + CW / 2} 140 L {CX0 + CW / 2} 128 L {T1B[0] + T1B[2] + 6} 128 L {T1B[0] + T1B[2] + 6} 158",
       MUTED, 1.3, m="ar", dash="4 4")
d.t(CX0 - 40, 122, "닿지 않는 곳은 tier-1 에 값을 냄", 11, MUTED, KR, "end")

d.t(ZL, 520, "2020년 기준 Amazon·Google·IBM·Microsoft 는 tier-1 을 거치지 않고 인터넷의 76% 에 닿음",
     11, MUTED, KR, "start")
