...
# ── 위: 두 노드 루프는 막힙니다
BY = 210
d.t(24, BY - 78, "두 노드가 맞물린 루프 — 막힙니다", 11, OK, KR, "start", 600)
...
d.t(675, BY + 34, "z 는 실제로 y 를 거칩니다", 11, MUTED, KR)
d.t(24, BY + 68, "z 가 x 로 갈 때 y 를 거치므로, z 는 y 에게 \"x 까지 무한대\" 라고 알립니다. y 는 z 를 거칠 생각을 하지 않습니다.", 11, MUTED, KR, "start")
d.t(24, BY + 90, "그래서 c(y,x) 가 올라도 y 는 직통 값을 그대로 쓰고, z 는 곧장 자기 직통으로 옮겨 갑니다. 2 씩 오르는 왕복이 생기지 않습니다.", 11, MUTED, KR, "start")

# ── 아래: 세 노드 고리는 뚫립니다
BY2 = 520
d.t(24, BY2 - 78, "세 노드가 고리를 이루면 — 뚫립니다", 11, BAD, KR, "start", 600)
...
d.t(520, BY2 + 54, "고리가 닫혀 있습니다", 11, SOFT, KR)
...
d.t(365, BY2 + 34, "y 에게는 유한한 값을 그대로 광고합니다", 11, OK, KR)
d.t(24, BY2 + 96, "y 가 z 를 거친다는 사실 자체가 z 가 y 에게 유한한 값을 줬다는 뜻입니다. 그 광고를 막는 쪽이 없습니다 — z 가 입을 막은 상대는 w 이지 y 가 아니기 때문입니다.", 11, MUTED, KR, "start")
d.t(24, BY2 + 118, "w → z 와 y → w 도 같은 이유로 열려 있어, 잘못된 값이 고리를 거꾸로 한 바퀴 돌아 제자리로 옵니다.", 11, MUTED, KR, "start")
