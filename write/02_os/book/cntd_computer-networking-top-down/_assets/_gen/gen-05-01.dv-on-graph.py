...
cut(180, "라운드 0 — 직통 비용만 암",
    {'y': "[2,0,1]", 'x': "[0,2,7]", 'z': "[7,1,0]"}, set(), [],
    "x 는 z 까지 직통 7 로 알고 있음 · y 를 거치면 더 싸다는 것을 아직 모름")

cut(400, "라운드 1 — 서로 건네고, 받은 값으로 다시 품",
    {'y': "[2,0,1]", 'x': "[0,2,3]", 'z': "[3,1,0]"}, {'x', 'z'},
    [('y','x',-14), ('x','y',14), ('x','z',-14), ('z','x',14)],
    "x — y 의 [2,0,1] 로 min{2+1, 7+0} = 3 · z 도 같은 이유로 3 · y 만 그대로")

cut(620, "라운드 2 — 바뀐 x·z 만 전달",
    {'y': "[2,0,1]", 'x': "[0,2,3]", 'z': "[3,1,0]"}, set(),
    [('x','y',-14), ('x','z',-14), ('z','x',14)],
    "y 는 보낼 것 없음 · 받은 쪽도 값 불변 → 여기서 정지 (조용한 상태)")

d.tone(660, 700, 300, 30, OK, 6, "12", 1.2)
d.t(810, 720, "조용한 상태(quiescent)", 11, OK, KR)

d.t(24, 752, "최소를 낸 이웃이 곧 다음 홉 — x→z 의 다음 홉은 z 가 아니라 y · 식을 푸는 순간 답과 전달 표가 함께 나옴", 11, INFO, KR, "start")
