...
d.t(SX1 + 92, SPINE_Y + 28, "처리량이 내려감", 11, SOFT, KR)

BONES = [
    (1, "시나리오 1", "큐잉 지연이 커짐", "도착률이 링크 용량에 가까워질수록", INFO),
    (-1, "시나리오 2", "재전송을 해야 함", "버려진 패킷을 메우려고", WARN),
    (1, "시나리오 2", "불필요한 재전송이 낭비함", "이미 도착한 패킷의 사본을 나름", WARN),
    (-1, "시나리오 3", "상류의 작업이 버려짐", "중간에서 버려지면 거기까지의 전송 용량이 전부", OK),
]
BX = [190, 320, 450, 580]
...
d.t(24, 508, "시나리오 2 — 제공 부하 R/2 일 때 처리량 R/3 · 0.5R 중 0.333R 만 원래 데이터, 0.166R 은 재전송",
     11, MUTED, KR, "start")
d.t(24, 530, "패킷마다 평균 두 번 전달 → 처리량 R/4 로 수렴 · 링크는 쉬지 않되 쓸모 있는 일만 줄어듦",
     11, MUTED, KR, "start")
