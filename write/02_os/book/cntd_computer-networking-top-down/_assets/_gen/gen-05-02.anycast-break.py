...
d = D(W, H, "SECTION 5.4.4 · WHY CDNS AVOID ANYCAST",
      ...
      "도쿄와 LA 인스턴스가 같은 주소 203.0.113.9 를 광고한다. 경로가 도쿄일 때 맺은 TCP 연결의 상태는 도쿄에만 있다. BGP 가 LA 쪽 경로로 갈아타면 같은 연결의 다음 세그먼트가 상태 없는 LA 에 도착하고, LA 는 맞는 소켓이 없어 리셋을 돌려준다.",
...
# 경로가 바뀌는 순간 — 두 칸 사이
d.t(500, 318, "BGP 가 경로를 다시 고름 · 이제 LA 쪽 광고가 이김", 12, WARN, KR, "middle", 600)
...
d.t(24, 596, "주소는 끝까지 203.0.113.9 하나인데 그 주소를 받는 기계가 도쿄에서 LA 로 바뀌었음", 13, INK, KR, "start", 600)
d.t(24, 620, "연결 상태는 도쿄 메모리에만 → LA 는 이 세그먼트를 모름 · 사용자에게는 연결 끊김", 13, MUTED, KR, "start")

d.legend(640, [("연결 상태가 있는 곳", OK), ("BGP 가 새로 고른 경로", WARN), ("상태가 없는 곳", BAD), ("리셋 · 노트의 읽기", ACC)])
d.t(960, 680, "KUROSE-ROSS 9E 5.4.4 / RFC 5737", 8, SOFT, MONO, "end")
