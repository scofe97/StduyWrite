...
#   ACK 를 다시 보냄", 타임아웃 시 "보냈지만 확인되지 않은 패킷을 전부 다시 보냄".
#   SR 수신자는 "순서에서 벗어난 패킷은 빠진 것이 채워질 때까지 버퍼에 둠".
...
d.t(44, 450, "값은 재전송 양이 아니라 수신자가 드는 무게", 12, INK, KR, "start", 600)
d.t(44, 474, "GBN 의 수신자가 들고 있는 것은 '다음에 기대하는 번호' 하나뿐 · SR 의 수신자는 버퍼와 받음 표시와 창 로직을 함께 듦",
    11, MUTED, KR, "start")
d.t(44, 494, "손실이 잦을 때 SR · 수신자가 단순할 때 GBN · TCP 는 누적 ACK + SACK 으로 그 중간",
    11, MUTED, KR, "start")
