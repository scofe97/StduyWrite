...
d.t(x2 + BARW / 2, 270, "한 국면 일찍 시작함", 11, MUTED, KR)
...
d.t(AXL + 2 * (BARW + GAP) + 68, 270, "국면 하나가 통째로 사라짐", 11, SOFT, KR, "start")

# ── 스트림 다중화
d.line(24, 320, 976, 320, RULE, 0.8)
d.t(24, 346, "하나의 QUIC 연결 안에서 스트림으로 나름", 13, INK, KR, "start", 600)
...
d.t(24, 480, "HTTP/3 이 QUIC API 에 지정하는 것은 연결 ID·스트림 ID 둘뿐 · 암호화·프레임 분할·다중화는 그 아래에서", 11, MUTED, KR, "start")
d.t(24, 502, "그래서 HTTP/3 코드는 QUIC API 까지만 만지고 UDP 소켓 API 는 만지지 않음", 11, ACC, KR, "start")
