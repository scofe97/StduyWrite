...
#   GBN 은 n 부터 N 까지 전부 다시 보냄. TCP 는 많아야 한 개, 즉 n 만 다시 보냄.
...
COLS = ["1~5 를 보냄", "다섯 다 도착함", "ACK 3 만 유실됨", "그래서 다시 보내는 것은"]
...
ROWS = [
    ("순수 GBN 이라면", "누적 ACK · 타이머 하나", WARN,
     [("보냄", "창이 5 ", MUTED), ("받는 쪽은 다 받았음", "ACK 1~5 를 보냄", MUTED),
      ("송신자는 3 에서 막힘", "SendBase 가 3 에 멈춤", WARN),
      ("3 · 4 · 5 를 전부", "이미 도착한 둘까지 다시", BAD)]),
    ("TCP — 뒤 ACK 가 늦으면", "타임아웃을 일으킨 것만", INFO,
     [("같음", "", SOFT), ("같음", "", SOFT),
      ("같음", "SendBase 가 3 에 멈춤", WARN),
      ("3 하나만", "많아야 한 개", ACC)]),
    ("TCP — 뒤 ACK 가 제때 오면", "누적 ACK 가 앞을 덮음", OK,
     [("같음", "", SOFT), ("같음", "", SOFT),
      ("ACK 4 가 도착함", "4까지 다 받았다는 뜻", OK),
      ("하나도 안 보냄", "SendBase 가 그냥 넘어감", OK)]),
]
...
d.t(44, 552, "그래서 어느 이름도 안 맞음", 12, INK, KR, "start", 600)
d.t(44, 574, "누적 확인 + 순서 밖 개별 확인 없음 = GBN 쪽 · 재전송이 「많아야 한 개」 = SR 쪽",
    11, MUTED, KR, "start")
d.t(44, 594, "SACK 을 켜면 순서 밖 세그먼트를 골라 확인 → SR 쪽으로 더 감 · 결론이 혼종인 이유",
    11, ACC, KR, "start")
