...
DOWN = [("Configuration", "설정 값을 묻고 정함"),
        ("Modify-State", "흐름 표 항목을 더하고 지우고 고침"),
        ("Read-State", "흐름 표와 포트의 통계·계수기를 모음"),
        ("Send-Packet", "지정한 포트로 패킷 하나를 내보냄")]
...
UP = [("Flow-Removed", "표 항목이 사라졌음을 알림"),
      ...
      ("Packet-in", "일치하는 항목이 없던 패킷을 올려보냄")]
...
d.t(30, 542, "Packet-in 이 4장의 이야기와 이어짐 · 흐름 표에 일치하는 항목이 없는 패킷을 컨트롤러로 보내는 동작이,", 11, MUTED, KR, "start")
d.t(30, 562, "일치 실패를 폐기가 아니라 질문으로 바꿈", 11, MUTED, KR, "start")
