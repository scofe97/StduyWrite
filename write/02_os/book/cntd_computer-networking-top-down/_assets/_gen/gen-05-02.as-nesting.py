...
d.t(36, 144, "인터넷 — AS 사이는 모두가 BGP 하나로 맞춤", 12, INK, KR, "start", 600)
...
for x, y, w, name in AS:
    ...
    d.t(x + w / 2, y + 52, "안의 알고리즘은 이 AS 가 고름", 12, MUTED, KR)
...
CARDS = [
    (20, "규모", "계산은 경계 안쪽에서만",
     ["밖의 목적지는 BGP 가 나르는 접두어로만 암",
      "내부 라우팅 정보는 자기 AS 안에서만 오감",
      "AS 가 너무 커지면 둘로 쪼갬"]),
    (516, "관리 자율성", "경계 안쪽은 그 AS 의 몫",
     ["AS 안은 한 관리 주체가 자기 뜻대로 운영",
      "안에서 쓸 알고리즘은 그 AS 가 고름",
      "밖에 알리는 것은 내부 구조가 아니라 접두어"]),
]
