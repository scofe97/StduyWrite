...
d.msg("클라이언트", "서버", "seq=43  ack=80  (데이터 없음)", 382, MUTED, dash="5 4",
      sub="서버의 메아리 'C' 를 받았다는 확인 — 80 을 기다림")

d.t(20, 442, "세 번째 세그먼트는 데이터 없이도 순서 번호를 가짐 — 헤더에 그 필드가 있는 한 어떤 번호든 필요",
     11, MUTED, KR, "start")
d.t(20, 464, "번호는 바이트를 셈 — 'C' 한 글자 뒤 순서 번호 42 → 43",
     11, MUTED, KR, "start")
d.t(20, 486, "데이터가 양방향 한 번씩 → 확인도 둘 · 하나는 메아리에 얹혀, 하나는 홀로",
     11, MUTED, KR, "start")
