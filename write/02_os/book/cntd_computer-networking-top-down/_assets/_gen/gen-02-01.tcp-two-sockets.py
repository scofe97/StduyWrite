...
step(C1, "서버 — 환영 소켓", "socket + bind", "12000 을 붙임")
step(C1, "클라이언트 프로세스", "socket(SOCK_STREAM)", "포트는 OS 가 붙임")
step(C2, "클라이언트 프로세스", "connect(host, 12000)", "문을 두드림")
step(C2, "서버 — 환영 소켓", "listen(1) · accept()", "두드림을 듣음")
step(C2, "서버 — 연결 소켓", "accept()", "가 만든 이 클라이언트 전용 소켓", ACC, True)
step(C3, "클라이언트 프로세스", "send · recv", "주소를 안 붙임")
step(C3, "서버 — 연결 소켓", "recv · send", "바이트 스트림")
step(C4, "클라이언트 프로세스", "close()", "연결을 끊음")
step(C4, "서버 — 연결 소켓", "close()", "이 소켓만 닫음")
step(C4, "서버 — 환영 소켓", "still listening", "다음 클라이언트를 받음", INFO, True)
...
d.path(f"M {C2} {CY['클라이언트 프로세스'] + BH / 2 + 4} L {C2} {CY['서버 — 환영 소켓'] - BH / 2 - 10}",
       MUTED, 1.3, m="ar")
d.t(C2 - 12, CY["클라이언트 프로세스"] + BH / 2 + 26, "3-way 핸드셰이크는 코드에 안 보", 11, SOFT, KR, "end")

d.path(f"M {C2} {CY['서버 — 환영 소켓'] + BH / 2 + 4} L {C2} {CY['서버 — 연결 소켓'] - BH / 2 - 10}",
       ACC, 1.6, m="acc")
d.t(C2 - 12, CY["서버 — 환영 소켓"] + BH / 2 + 26, "새 소켓을 만듦", 11, ACC, KR, "end")
...
d.path(f"M {C3 + 26} {CY['서버 — 연결 소켓'] - BH / 2 - 4} L {C3 + 26} {CY['클라이언트 프로세스'] + BH / 2 + 10}",
       MUTED, 1.2, m="ar", dash="5 4")
d.t(12, 506, "데이터는 클라이언트 소켓과 연결 소켓 사이에서만 흐름 · 환영 소켓을 거치지 않음", 11, MUTED, KR, "start")
d.t(12, 528, "둘을 한 소켓으로 보면 두 번째 클라이언트의 접속을 설명할 수 없음", 11, MUTED, KR, "start")
