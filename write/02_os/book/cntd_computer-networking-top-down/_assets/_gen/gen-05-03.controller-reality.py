...
d = D(W, H, "COMPUTER NETWORKING TOP-DOWN · 05-03 §2",
      ...
      "논리적으로 중앙인 SDN 컨트롤러가 실제로는 여러 서버에 흩어져 상태를 복제하고, 스위치에게는 하나의 서비스로 보이는 배치. 서버끼리는 합의로 이야기하고, 스위치와는 남향 인터페이스로 이야기한다.",
...
node(528, 150, 320, 168, "SERVER", "제어 서버 2·3", "같은 것을 나눠 듦", badge="x2")
chip(552, 244, 272, "같은 컨트롤러 소프트웨어", "복제본")
chip(552, 282, 272, "NIB 사본", "복제본")

node(290, 424, 340, 100, "SWITCH", "패킷 스위치", "흐름 표에 일치와 동작을 실음", badge="x4")

# 서버끼리 — 상태 복제와 합의
d.path("M 392 236 L 528 236", ACC, 1.4, dash="5 4")
d.t(460, 214, "상태 복제 · 합의", 12, ACC, KR, "middle", 600)
d.t(460, 338, "여기서 순서와 일관성을 맞춤", 12, MUTED, KR)
...
d.t(12, 584, "스위치는 서버 대수를 모름 — 컨트롤러 하나와 이야기한다고 간주",
    13, MUTED, KR, "start")
d.t(12, 606, "서버를 나누는 이유 = 장애 대비·성능 · 쓰기를 한 서버가 맡는 것은 여러 구현 중 하나",
    13, MUTED, KR, "start")
