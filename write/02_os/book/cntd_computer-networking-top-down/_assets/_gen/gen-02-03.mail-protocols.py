...
d = D(W, H, "COMPUTER NETWORKING TOP-DOWN · 02-03 §3",
      ...
      "SMTP 로는 편지함을 열어 볼 수 없음 — 방향이 반대이기 때문입니다")
...
d.t(XS[1] + 60, NY + NH + 92, "여기 남아 30분마다 다시 시도함", 11, SOFT, KR)

d.t(20, 384, "중간 메일 서버를 거치지 않음 · Alice 의 서버가 홍콩에 있고 Bob 의 서버가 세인트루이스에 있어도 둘 사이의 TCP 연결이 직접 맺힘",
     11, MUTED, KR, "start")
d.t(20, 406, "Alice 의 에이전트가 Bob 의 서버로 곧장 보내지 않는 이유는, 도달할 수 없을 때 에이전트에게 아무 대책이 없기 때문",
     11, MUTED, KR, "start")
