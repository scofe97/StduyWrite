...
ROW_B = [("§5", "UDP 소켓", "편지마다 주소를 붙임"),
         ("§6", "TCP 소켓", "서버의 문이 둘"),
         ("§7", "QUIC API", "악수가 하나로 합쳐짐")]
bx = []
...
d.t(96, 402, "애플리케이션 층과 트랜스포트 층 사이의 인터페이스", 12, INK, KR, "start")
d.t(44, 424, "위쪽은 전부 통제함 · 아래쪽에 남는 것은 프로토콜 하나를 고르는 일과 파라미터 몇 개뿐",
    11, MUTED, KR, "start")
...
d.t(24, 556, "그리고 아무도 주지 않는 것 — 처리량 보장과 타이밍 보장은 오늘날 어느 트랜스포트 프로토콜에도 없음",
    11, MUTED, KR, "start")
