...
d.t(690, 412, "조각마다 다른 출처 가능", 11, ACC, KR)

d.t(30, 456, "원문이 꼽는 SDN 의 네 가지 특징", 11, INK, KR, "start", 600)
for i, txt in enumerate(["흐름 기반 전달 — 목적지 IP 하나가 아니라 여러 헤더 필드로 일치",
                         "데이터·제어 평면 분리 — 스위치는 표를 실행, 서버가 표를 결정",
                         "제어 기능이 스위치 밖 — 원격 서버의 소프트웨어",
                         "프로그래밍 가능한 제어 평면 — 제어 앱이 API 로 데이터 평면을 지정"]):
