...
LX, LW = 64, 840
LAYERS = [
    (112, 64, "애플리케이션 층", "프로세스 = 집. 메시지를 만들고 읽는 코드가 전부 여기 있음", INK, "전부 통제함"),
    (256, 64, "트랜스포트 층", "TCP · UDP. 고를 수 있는 것은 프로토콜 하나와 파라미터 몇 개", INK, "거의 통제하지 못함"),
    (332, 56, "네트워크 층", "데이터그램을 목적지 호스트까지 나름", MUTED, "통제 없음"),
    (400, 56, "링크 · 물리 층", "한 칸씩 실어 나름", MUTED, "통제 없음"),
]
...
d.t(LX + 72, 212, "집의 문 — 애플리케이션과 네트워크 사이의 API ", 12, INK, KR, "start")
d.t(LX + 20, 234, "밀어낸 쪽은 문 반대편에 메시지를 날라 줄 운송 기반 시설이 있다고 가정함", 11, MUTED, KR, "start")

# 주소 두 조각
d.line(24, 484, 976, 484, RULE, 0.8)
d.t(24, 510, "그 문을 지목하는 주소는 두 조각", 13, INK, KR, "start", 600)
for i, (head, body) in enumerate([("호스트", "IP 주소로 지목함"),
                                  ("받는 소켓", "목적지 포트 번호로 지목함")]):
    x = 24 + i * 300
...
d.t(632, 566, "전체 목록은 IANA 가 관리함", 11, MUTED, KR, "start")
