...
def node(cx, cy, label, sub, c=MUTED, focal=False, w=NW):
    ...
    d.t(cx, cy - 2, label, 11, c if focal else INK, MONO, "middle", 600)
...
for x, y, lab in L1:
    ...
    d.path(f"M {500 + (x-500)*0.18} {ROOT[1] + NH/2 + 2} L {x} {y - NH/2 - 8}", MUTED, 1.2, m="ar")

node(220, 348, "…00010", "21비트", OK, True)
d.path(f"M 292 {250 + NH/2 + 2} L 232 {348 - NH/2 - 8}", MUTED, 1.2, m="ar")
node(600, 348, "…00011", "21비트", OK, True)
d.path(f"M 692 {250 + NH/2 + 2} L 610 {348 - NH/2 - 8}", MUTED, 1.2, m="ar")
node(824, 348, "…00011000", "24비트", ACC, True, 148)
d.path(f"M 712 {250 + NH/2 + 2} L 800 {348 - NH/2 - 8}", ACC, 1.4, m="acc")

LEAF = [(220, 446, "인터페이스 0", OK), (600, 446, "인터페이스 2", OK), (824, 446, "인터페이스 1", ACC)]
for x, y, lab, c in LEAF:
    d.box(x - NW / 2, y - 20, NW, 40, PAPER, c, 1.2, 6)
    d.t(x, y + 4, lab, 11, c, KR)
    d.path(f"M {x} {348 + NH/2 + 2} L {x} {y - 26}", c, 1.2, m="acc" if c is ACC else "ok")

d.box(24, 486, 420, 62, PAPER2, RULE, 0.9, 6)
d.t(40, 510, "…00011000 10101010 은 21비트와 24비트 둘 다에 맞습니다", 11, SOFT, KR, "start")
d.t(40, 532, "긴 쪽인 24비트가 이겨 인터페이스 1 로 갑니다", 11, ACC, KR, "start")

d.t(468, 510, "어느 항목에도 안 맞으면 길이 0 인 기본 항목이 이깁니다 — 인터페이스 3.", 11, MUTED, KR, "start")
d.t(468, 532, "TCAM 의 '신경 안 씀' 비트가 이 접두 표를 작게 유지하는 열쇠입니다.", 11, MUTED, KR, "start")

d.legend(H - 32, [("가장 긴 접두", ACC), ("더 짧은 접두", OK), ("공통 부분", INFO)])
