...
# 클라이언트 포트는 OS 가 붙임이고, 목적지 주소는 sendto 가 붙이고, 회신 주소는 recvfrom 이 알려 준다.
...
d.t(742, 268, "포트를 개발자가 명시함", 11, SOFT, KR, "end")
...
d.msg("UDPServer.py", "UDPClient.py", "sendto(MSG, clientAddress)", 476, MUTED, dash="5 4",
      sub="회신 주소는 recvfrom 이 알려 준 것")

d.selfmsg("UDPClient.py", "close()", 528, sub="소켓을 닫고 프로세스가 끝남")

d.t(12, 586, "출발지 주소도 패킷에 붙음 — 다만 애플리케이션 코드가 아니라 하위 운영체제의 몫",
     11, MUTED, KR, "start")
