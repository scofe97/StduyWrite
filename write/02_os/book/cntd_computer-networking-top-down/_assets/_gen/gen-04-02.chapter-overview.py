...
# 박자마다 문제를 하나 세우고 그 해결과 남는 문제로 다음 박자에 넘긴다. 입력 줄 박자는 본문에서 세 절로 갈린다.
...
d = D(W, H, "CHAPTER MAP · 04-02",
      ...
      lead="왜 서는가 → 없앨 수 있나 → 얼마나 둘까 → 누가 먼저 → 누가 정하나")

CARDS = [("§1–3", "입력 줄",  "빠르게 해도 남는다",  INFO, ["· N 배면 안 선다", "· 그래도 HOL 블로킹", "· 58% 에서 발산"]),
         ("§4",   "출력 줄",   "빠르게 해도 안 준다", INFO, ["· 출력 링크는 하나", "· 꼬리 버리기", "· AQM 선제 표시"]),
         ("§5", "얼마나 둘까", "버퍼는 소금이다",    ACC,  ["· RTT × C", "· 흐름 많으면 ÷√N", "· 버퍼블로트"]),
         ("§6", "누가 먼저", "규율 넷",              INFO, ["· FIFO · 우선순위", "· 라운드 로빈 · WFQ", "· 일 보존"]),
         ("§7", "누가 정하나", "기술에서 정책으로",  INFO, ["· 클래스는 ISP 가", "· 망 중립성", "· FCC 2015 → 2024"])]
...
d.t(W / 2, 350, "이 편의 한 줄 — 버퍼는 소금 · 알맞으면 약, 많으면 독", 12, MUTED)
