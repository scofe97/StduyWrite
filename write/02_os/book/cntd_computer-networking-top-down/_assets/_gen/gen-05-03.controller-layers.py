...
LAYERS = [
    ...
    ("북향 인터페이스", "앱이 상태와 흐름 표를 읽고 씀 · 상태 변화 알림 구독", ACC, 208, 56),
...
d.t(BX + BW + 14, 338, "본체", 11, SOFT, KR, "start")

# 캡션을 범례 위에 둔다 — 범례선(y)보다 아래로 내려가면 스와치와 겹친다
d.t(30, 546, "논리적으로는 하나, 실제로는 여러 서버에 분산 → 사건의 순서·일관성·합의 같은", 11, MUTED, KR, "start")
d.t(30, 566, "분산 시스템의 문제가 그대로 따라옴 · ONOS 와 Orion 이 공들인 자리가 바로 여기", 11, MUTED, KR, "start")
