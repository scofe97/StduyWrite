...
for zx, (t1, t2, limit, c) in zip(ZX, TITLES):
    ...
    d.t(zx + ZW - 16, ZY - 8, t2, 11, SOFT, KR, "end")
    for i in range(3):
        y = ZY + 42 + i * 54
        d.box(zx + 18, y, NW, NH, PAPER2, RULE, 1.0, 5)
        d.t(zx + 18 + NW / 2, y + 22, f"in {i+1}", 11, SOFT, MONO)
        d.box(zx + ZW - 18 - NW, y, NW, NH, PAPER2, RULE, 1.0, 5)
        d.t(zx + ZW - 18 - NW / 2, y + 22, f"out {i+1}", 11, SOFT, MONO)
    d.tone(zx + 16, ZH + ZY - 56, ZW - 32, 40, c, 5, "14", 1.2)
    d.t(zx + ZW / 2, ZH + ZY - 30, limit, 11, c, MONO)

# 메모리 — 공유 메모리 한 덩어리를 거친다
mx = ZX[0] + ZW / 2
d.tone(mx - 44, ZY + 84, 88, 68, WARN, 5, "14", 1.4)
d.t(mx, ZY + 114, "메모리", 11, WARN, KR)
d.t(mx, ZY + 132, "쓰기+읽기", 11, SOFT, KR)
for i in range(3):
    y = ZY + 42 + i * 54 + NH / 2
    d.path(f"M {ZX[0]+18+NW+4} {y} L {mx-48} {ZY+118}", MUTED, 1.0, m="ar")
    d.path(f"M {mx+48} {ZY+118} L {ZX[0]+ZW-18-NW-6} {y}", MUTED, 1.0, m="ar")

# 버스 — 세로 한 줄
bx = ZX[1] + ZW / 2
d.path(f"M {bx} {ZY+40} L {bx} {ZY+206}", WARN, 2.4)
d.t(bx + 8, ZY + 30, "공유 버스", 11, WARN, KR, "start")
for i in range(3):
    y = ZY + 42 + i * 54 + NH / 2
...
d.t(cx0 + 48, cy0 + 142, "동시에 둘이 건넙니다", 11, ACC, KR)
