...
d = D(W, H, "COMPUTER NETWORKING TOP-DOWN · 05-03 §1",
      ...
      "같은 부하 분산을 전통 방식과 SDN 방식으로 했을 때 요청이 밟는 정거장. 전통은 경로에 상자 하나가 더 끼고, SDN 은 어차피 지나는 스위치가 흐름 표로 그 일을 한다.",
...
lane("전통 — 경로에 상자 하나가 더 낌 · 거치는 자리 다섯", 152,
     {0: ("클라이언트", "요청 하나"), 1: ("스위치", "목적지만 보고 나름"),
      2: ("로드밸런서", "여기서 고름"), 3: ("스위치", "목적지만 보고 나름"),
      4: ("서버", "고른 백엔드")}, focal=2)

d.line(12, 268, W - 12, 268, RULE, 0.9)

lane("SDN — 지나던 스위치가 가름 · 거치는 자리 셋", 316,
     {0: ("클라이언트", "요청 하나"), 1: ("스위치", "흐름 표가 가름"),
      4: ("서버", "표가 고른 백엔드")})

d.t(12, 424, "두 방식 다 스위치를 지남 · 갈리는 것은 그 스위치가 무엇을 보느냐",
    13, MUTED, KR, "start")
d.t(12, 446, "목적지 주소만 보는 장비는 선택 불가 → 고르는 상자가 경로에 따로 필요",
    13, MUTED, KR, "start")
d.t(12, 468, "헤더 여러 필드로 일치를 걸고 동작으로 헤더를 고칠 수 있으면 그 일이 지나가는 자리에서 끝남",
    13, MUTED, KR, "start")
