...
# dd-lint 사선 경고 다섯은 형태에 내재한 것으로 판정한다 — 이 그림의 논지 자체가 45도(가산 증가)와
# 원점 방향(승산 감소)이라, 직교 엘보로 바꾸면 그림이 말하려는 것이 사라진다. 공평선도 45도 파선이다.
...
d.t(px(0.26), py(0.12), "차이가 매번 절반씩 줄어듦", 11, SOFT, KR, "start")

d.t(20, 512, "더하기는 45도 고정 · 나누기는 원점 쪽으로 당김 → 차이가 절반씩 줄어 공평선 위에서 오르내림",
     11, MUTED, KR, "start")
d.t(20, 534, "현실에서는 깨집니다 — 왕복 시간이 다르거나, UDP 가 섞이거나, 한 앱이 병렬 연결을 여럿 열면 몫이 크게 갈림",
     11, MUTED, KR, "start")

d.legend(H - 44, [("승산 감소 — 원점 방향", ACC), ("가산 증가 — 45도", MUTED)])
