...
# 제어 평면 zone
d.o.append(f'<rect x="24" y="100" width="{W-72}" height="72" rx="8" fill="{INFO}08" '
           f'stroke="{INFO}" stroke-width="1" stroke-dasharray="4 4"/>')
d.t(36, 94, "제어 평면 — 밀리초에서 초 · 소프트웨어", 12, INFO, KR, "start")
d.box(FAB_X - 210, 112, 420, 50, PAPER2, INFO, 1.2, 6)
d.t(FAB_X, 134, "라우팅 프로세서", 12, INFO, KR, "middle", 600)
d.t(FAB_X, 152, "라우팅 프로토콜 실행 · 포워딩 테이블 계산 · SDN 이면 컨트롤러와 통신", 12, SOFT, KR)

# 데이터 평면 zone
d.o.append(f'<rect x="24" y="212" width="{W-72}" height="310" rx="8" fill="{INK}05" '
           f'stroke="{RULE}" stroke-width="1" stroke-dasharray="4 4"/>')
d.t(36, 206, "데이터 평면 — 나노초 · 하드웨어", 12, SOFT, KR, "start")
...
d.t(FAB_X, 366, "스위칭 패브릭", 12, INK, KR, "middle", 600)
d.t(FAB_X, 388, "라우터 안의 망", 12, SOFT, KR)
d.t(FAB_X, 412, "메모리 · 버스 · 크로스바", 12, SOFT, MONO)

d.path(f"M {FAB_X} 162 L {FAB_X} {ROWS[0] - 8}", INFO, 1.3, m="info", dash="5 4")
d.t(FAB_X + 12, 200, "포워딩 테이블을 라인 카드로 복사", 12, INFO, KR, "start")

d.tone(IN_X - PW / 2, 548, 300, 56, ACC, 6, "14", 1.2)
d.t(IN_X - PW / 2 + 16, 572, "64바이트 / 100 Gbps = 5.12 ns", 12, ACC, MONO, "start", 600)
d.t(IN_X - PW / 2 + 16, 592, "다음 데이터그램이 오기까지의 시간", 12, SOFT, KR, "start")

d.t(400, 572, "포트가 N 개 묶인 라인 카드라면 처리 파이프라인이 N 배 빨라야 합니다.", 12, MUTED, KR, "start")
d.t(400, 594, "소프트웨어로는 불가능해서 이 셋이 거의 언제나 하드웨어입니다.", 12, MUTED, KR, "start")
