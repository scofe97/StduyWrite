...
d.t((PX0 + PX1) / 2, PY1 + 26, "시간 — 청크 하나가 몇 초", 11, SOFT, KR)
...
d.t(PX0, PY1 + 68, "대역폭이 3 Mbps 아래로 떨어지자 다음 청크부터 1 Mbps 판으로, 더 떨어지자 300 kbps 판으로 내려감",
     11, MUTED, KR, "start")
d.t(PX0, PY1 + 90, "회복되면 다시 올라감 · 판을 갈아타는 판단이 청크마다 새로 이뤄짐",
     11, MUTED, KR, "start")

d.t(20, 528, "서버가 준비하는 것은 판마다의 파일과 매니페스트뿐 · 무엇을 언제 가져갈지는 클라이언트가 측정한 대역폭과 버퍼 잔량으로 정함",
     11, MUTED, KR, "start")
