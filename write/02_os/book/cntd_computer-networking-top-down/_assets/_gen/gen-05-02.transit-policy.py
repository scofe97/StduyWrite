...
d = D(W, H, "SECTION 5.4.5 · TRANSIT POLICY",
      ...
      "백본 A·B·C 와 액세스 W·X·Y 로 된 같은 망에서 두 장면을 본다. 장면 1 에서 X 는 X-C-Y 를 B 에게 광고하지 않아 B 의 Y 행 트래픽이 X 를 지나지 않는다. "
      "장면 2 에서 B 는 A 에게서 배운 W 행 경로를 고객 X 에게만 알리고 C 에게는 알리지 않아, C 가 B 를 거쳐 W 로 보내지 않는다.",
...
panel(24, 1, "X 는 남의 통과 트래픽을 막습니다",
      ...
      labels=[labels1],
      result="B 의 표에 B-X-C-Y 가 없어 Y 행은 B-C-Y 로 갑니다")
...
panel(528, 2, "B 는 A 와 C 사이를 공짜로 나르지 않습니다",
      ...
      labels=[labels2],
      result="C 의 표에 B-A-W 가 없어 W 행을 B 로 보내지 않습니다")

# ── 경험칙 (focal) ──────────────────────────────────────────
d.tone(24, 460, 952, 72, ACC, r=8, op="12", sw=1.4)
d.t(44, 488, "경험칙: 내 백본을 지나는 트래픽은 출발지나 목적지 중 하나가 내 고객이어야 합니다", 14, INK, KR, "start", 600)
d.t(44, 516, "둘 다 아니면 무임승차라 광고하지 않습니다. 광고되지 않은 길은 짧아도 후보조차 되지 않습니다.", 13, MUTED, KR, "start")
