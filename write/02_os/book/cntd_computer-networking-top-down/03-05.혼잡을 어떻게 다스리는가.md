...
느린 시작을 빼고 손실이 중복 ACK 로만 온다고 하면 남는 동작은 둘입니다. **왕복마다 1 MSS 더하기(가산 증가)** 와 **손실 때 절반으로 나누기(승산 감소)**, 그래서 **AIMD** 입니다. Additive-Increase, Multiplicative-Decrease 의 줄임입니다. 이 규칙이 선형으로 오르다 반으로 떨어지는 **톱니**를 만들고, 앞 절의 대역폭 탐침이 그림으로 드러난 것입니다. 시뮬레이터에 [손실률 1% 를 걸면](https://tcp.overshoot.ai/?preset=cold-iw10&cc=cubic&loss=1&autoplay=1) 손실마다 꺾이는 톱니가 나옵니다. 다만 그 도구의 손실 기반은 Reno 가 아니라 CUBIC 이라 오르막이 직선이 아닙니다.
