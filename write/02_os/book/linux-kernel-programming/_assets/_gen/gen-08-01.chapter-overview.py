...
for i in range(4):
    ...
    d.arrow([(x + 4, CY + CH / 2), (x + STRIDE - CW - 6, CY + CH / 2)],
            ACC if i == 3 else MUTED, "acc" if i == 3 else "ar", 1.4)

d.t(X0, 336, "§1~§3 이 엔진의 동작이고 §4 가 그 대가입니다. §5·§6 은 그 엔진을 실제로 부르는 법입니다.", 13, MUTED, KR, "start")
d.t(X0, 360, "GFP 플래그를 잘못 고르면 머신이 멈추거나 커널이 죽습니다 — 그래서 마지막 카드가 논점입니다.", 13, SOFT, KR, "start")
