...
# 본문이 요구한 형태: "badness score 0~1000" + "init·커널 스레드·adj -1000 은 후보에서 빠진다".
...
TIERS = [
    ...
    ("높은 badness score", "가용 메모리를 가장 많이 쓴 쪽", BAD),
    ("보통 프로세스", "점수 순서대로 후보가 됩니다", WARN),
    ("init (PID 1)", "is_global_init 으로 제외됩니다", OK),
    ("커널 스레드", "PF_KTHREAD 로 제외됩니다", OK),
    ("oom_score_adj = -1000", "절대 victim 이 되지 않습니다", OK),
]

d = DK(W, H, "LINUX KERNEL PROGRAMMING · 09-03 §5",
       "누가 먼저 죽는지는 점수가 정합니다",
       "커널은 프로세스마다 badness score 를 0~1000 으로 매긴다. 0 은 가용 메모리를 전혀 안 쓰는 쪽, 1000 은 전부 쓰는 쪽이다. oom_score_adj 는 이 점수를 계산하는 안에서 더해지고, 아래 세 층은 아예 후보에서 빠진다.",
...
d.t(24, BOT + 40, "oom_score_adj 는 badness 안에서 더해집니다 — 파일 값에 또 더하지 않습니다. 조회와 설정은 choom(1) 유틸로 합니다(root 필요).", 13, MUTED, KR, "start")
