...
# 본문의 수치를 그대로 옮겼다 — 132KB 요청 · order 6 의 256KB 할당 · 124KB 낭비(약 50%).
...
d = DK(W, H, "LINUX KERNEL PROGRAMMING · 08-01 §4",
       "2의 거듭제곱이 아니면 버려집니다",
       "132KB 를 요청하면 페이지 할당자는 그보다 적게 줄 수 없어 order 6 의 256KB 청크를 준다. 소비자는 앞 132KB 만 쓰고 나머지 124KB, 곧 거의 절반을 버린다. alloc_pages_exact() 는 평소처럼 받은 뒤 초과분을 즉시 free 해 이 낭비를 되돌린다.",
       "막대 길이가 곧 KB 입니다 — 위아래 두 막대의 차이가 버려지는 양입니다")

ROWS = [
    ("요청한 크기", 0, 132, "132 KB", INFO, "실제 필요한 양"),
    ("받은 크기", 0, 256, "256 KB — order 6", WARN, "더 적게는 못 줍니다"),
