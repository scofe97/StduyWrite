...
|---|---|---|---|
| 동적 라우팅 (dynamic routing) | 라우터끼리 경로를 주고받아 테이블을 스스로 채웁니다 | 손으로 넣던 한 줄을 프로토콜이 넣습니다 · 대신 "왜 없지" 가 생깁니다 | §1 |
| 추적 사다리 | 이웃·광고·수신·설치 네 칸으로 고장 위치를 좁힙니다 | 어느 칸에서 끊겼는지가 곧 원인 부위입니다 | §1 |
| OSPF (Open Shortest Path First) | hello 로 이웃을 찾고 링크 상태를 동기화하는 IGP 입니다 | 인접은 **조건부** · area·타이머·인증이 하나만 어긋나도 무시됩니다 | §2 |
| 인접 (adjacency) | hello 조건이 맞아야 성립하는 이웃 관계입니다 | hello 가 오가는 것과 인접이 맺히는 것은 다른 일입니다 | §2 |
| area | OSPF 가 링크 상태를 공유하는 구역 단위입니다 | 같은 링크 양 끝이 달라도 hello 는 오갑니다 · 대칭이어야 합니다 | §2 |
| DR 선출 (designated router) | 세그먼트 대표를 뽑는 절차입니다 | **선점이 없어** 설정을 고쳐도 역할이 남습니다 · `clear` 로 재선거 | §2 |
| BGP (Border Gateway Protocol) | AS 사이에서 프리픽스를 광고하는 경로 벡터 프로토콜입니다 | TCP 179 위에서 돕니다 · 세션과 광고가 별개 단계 | §3 |
| Established | BGP 세션이 수립된 상태입니다 | **성공 지표가 아닙니다** — 전화가 연결된 것이지 할 말을 한 게 아닙니다 | §3 |
| PfxRcd / PfxSnt | 받은·보낸 프리픽스 개수입니다 | `Up` 인데 `PfxRcd 0` 이면 광고 단계에서 끊긴 것 | §3 |
| advertised-routes | 내가 상대에게 실제로 내보내는 목록입니다 | 사다리 두 번째 칸을 직접 확인하는 명령 | §3 |
| RFC 8212 | 정책이 없으면 eBGP 광고·수신을 거부하는 기본 동작입니다 | 이 랩은 껐습니다 · 실무에선 **정책 필터가 또 하나의 원인** | §3 |
