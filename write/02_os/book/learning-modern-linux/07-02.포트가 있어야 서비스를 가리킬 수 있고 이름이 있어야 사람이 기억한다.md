...
> 이 구간을 읽고 나면 아래 열두 가지를 스스로 해낼 수 있습니다.
...
9. `ECE` 와 `CWR` 이 무엇을 알리는 칸인지 말하고, 첫 SYN 에 둘이 함께 붙는 이유를 설명할 수 있습니다.
10. 소켓이 왜 새 층이 아닌지 근거를 대며 말할 수 있습니다.
11. 한 이름에 A 레코드가 여럿일 때 무슨 일이 일어나는지와 그 대가를 말할 수 있습니다.
12. CA 가 도메인 소유를 확인하는 두 방법을 구분하고 각각 무엇을 증명하는지 말할 수 있습니다.
...
### 캡처에 SYN 말고 ECE 와 CWR 이 같이 찍힐 때

여기도 노트의 읽기입니다. 저자가 꼽은 플래그는 SYN 과 ACK 둘인데, 실제로 패킷을 떠 보면 첫 SYN 에 `ECE` 와 `CWR` 이 함께 붙어 나가고 돌아오는 SYN-ACK 에는 `ECE` 만 붙는 것을 봅니다. 표에 없는 그 둘이 무엇인지 짚어 둡니다.

```bash
# 핸드셰이크의 첫 두 패킷만 골라 플래그를 본다
sudo tshark -i en0 -Y "tcp.flags.syn==1" -T fields -e tcp.flags.str
```

시작은 혼잡 제어의 오래된 약점입니다. 기존 방식에서 「길이 막혔다」는 신호는 **패킷이 사라지는 것**입니다. 보내는 쪽은 응답이 안 오는 것을 보고서야 알아차리므로, 알아차리려면 먼저 손실을 치러야 합니다.

ECN 은 그 순서를 바꿉니다. RFC 3168 이 적은 대로 큐 관리 장치가 **패킷을 버리는 대신** IP 헤더에 혼잡 경험(CE) 표시를 넣을 수 있고, 그러면 받는 쪽이 패킷을 온전히 받으면서도 혼잡을 압니다. 손실 뒤의 재전송으로 생기는 지연을 피하는 것이 그 값입니다.[^ecn]

표시는 IP 헤더에 들어가는데 그것을 알려 줄 자리는 TCP 헤더에 있습니다. 그래서 플래그 둘이 붙었습니다. **`ECE`** 는 받는 쪽이 「CE 표시가 붙은 패킷을 받았다」고 보내는 쪽에 알리는 칸이고, **`CWR`** 은 보내는 쪽이 「혼잡 윈도를 줄였다」고 되돌려 주는 칸입니다.[^ecnflags]

첫 SYN 에 둘이 같이 붙는 것은 협상입니다. RFC 는 `ECE` 와 `CWR` 이 다 선 SYN 을 **ECN-setup SYN 패킷**이라 부르고, `ECE` 만 서고 `CWR` 은 안 선 SYN-ACK 을 **ECN-setup SYN-ACK 패킷**이라 부릅니다.[^ecnsetup] 캡처에서 본 `SYN, ECE, CWR` → `SYN, ACK, ECE` 가 정확히 그 한 쌍입니다. 「나 ECN 할 줄 안다」와 「나도 한다」입니다.

같은 캡처에서 하나가 더 보였습니다. 협상이 안 먹히면 macOS 가 두 플래그를 **빼고 다시 SYN 을 보냅니다.** 중간 장비가 ECN 이 붙은 SYN 을 버리는 경우가 있어서 그렇습니다. 층을 하나 늘린 대가가 여기서도 나타납니다.
...
여기도 노트의 읽기입니다. 이 노트가 포트를 다루고 소켓을 다루는 순서로 이어지다 보니 「주소 위에 포트, 포트 위에 소켓」처럼 층이 하나 더 있는 것으로 읽히기 쉽습니다. 그렇지 않습니다.

**소켓은 새 층이 아니라 API 추상입니다.** 앞 편이 세운 네 층에 다섯째가 붙는 것이 아닙니다. 선 위를 지나가는 것은 여전히 이더넷 프레임 안의 IP 패킷 안의 TCP 세그먼트뿐이고, 소켓에 해당하는 비트는 어디에도 없습니다.

그러면 무엇인가 하면, 커널이 **이미 있는 값들을 하나로 묶어 프로그램에 내주는 손잡이**입니다. 묶는 값은 프로토콜과 양쪽의 주소·포트, 그러니까 흔히 5-tuple 이라 부르는 다섯입니다. 저자가 소켓의 정체성이라 한 그 튜플이 이것이고, `ss` 출력의 `Local Address:Port` 와 `Peer Address:Port` 열이 그 다섯을 그대로 보입니다.

커널은 그 다섯을 파일 디스크립터 하나에 붙여 돌려줍니다. 그래서 프로그램은 다섯을 매번 들고 다니는 대신 정수 하나를 들고 `read` 와 `write` 를 부릅니다. 소켓이 하는 일이 그것뿐입니다.

가르는 방법이 하나 있습니다. **패킷을 떠서 소켓을 찾아보십시오.** 없습니다. IP 헤더도 있고 TCP 헤더도 있는데 소켓 헤더는 없습니다. 층은 선 위에 흔적을 남기지만 API 추상은 기계 안에만 있습니다.
...
여기는 노트의 읽기입니다. 저자는 이 출력을 그냥 지나가지만 눈에 띄는 것이 하나 있습니다. **주소가 넷입니다.** 이름 하나에 A 레코드가 여럿 있을 수 있고, 그때 어느 것을 쓸지는 묻는 쪽이 고릅니다.

```bash
# 같은 질문을 두 번 던져 보면 순서가 바뀌는 것을 볼 수 있다
dig +short example.com
dig +short example.com
```

이 노트를 쓰며 재 보니 `example.com` 은 `172.66.147.243` 과 `104.20.23.154` 둘을 답했고, 다시 물었을 때 **순서가 뒤바뀌어** 돌아왔습니다. 클라이언트는 보통 앞의 것부터 쓰므로, 순서를 섞어 주는 것만으로 요청이 여러 서버에 갈립니다.

이것이 가장 간단한 부하 분산입니다. 앞에 장비를 두지 않고 존 파일에 A 를 몇 줄 더 적는 것으로 끝납니다. 대가는 분명합니다. 서버 하나가 죽어도 DNS 는 그것을 모르고 계속 그 주소를 답합니다. 죽은 것을 빼려면 레코드를 고쳐야 하는데, 그러면 아래 §7 의 TTL 이야기가 그대로 걸립니다.
...
### 도메인을 가졌다는 것을 어떻게 증명하는가

여기서부터는 노트의 읽기입니다. 저자는 TXT 레코드가 「오늘날에는 보안 관련 DNS 확장 문맥에서 기계가 읽는 데이터를 담는 경우가 많다」고만 적고 넘어갑니다. 그 대표적인 쓰임을 열어 둡니다.

인증서를 받으려면 CA 에게 **그 도메인이 내 것임을 보여야** 합니다. 그런데 CA 는 나를 모릅니다. 그래서 증명의 형태가 이렇게 됩니다 — **그 도메인을 조종할 수 있는 사람만 할 수 있는 일을 시키고, 그것이 됐는지 본다.**

방법이 둘입니다. **HTTP-01** 은 무작위 토큰을 주고 `http://<도메인>/.well-known/acme-challenge/<토큰>` 에 올리게 한 뒤 가져가 봅니다. 그 경로에 파일을 둘 수 있다는 것은 그 도메인이 가리키는 서버를 쥐고 있다는 뜻입니다.[^http01]

**DNS-01** 은 `_acme-challenge.<도메인>` 에 토큰을 TXT 로 넣게 한 뒤 조회합니다. 존 파일을 고칠 수 있다는 것은 그 이름 자체를 쥐고 있다는 뜻입니다.[^dns01] 서버가 없어도 되고 와일드카드 인증서에도 쓸 수 있습니다.

실물이 지금도 조회됩니다.

```bash
# ACME 챌린지 토큰이 실제로 TXT 로 올라가 있다
...
같은 수법을 CA 말고도 여럿이 씁니다.

```bash
# 큰 도메인의 TXT 에는 여러 회사의 소유 확인 토큰이 겹겹이 쌓여 있다
...
위 두 블록은 이 노트를 쓰며 실제로 조회한 응답입니다. 토큰은 계속 바뀌므로 값 자체보다 **이름의 모양**을 보십시오. `_acme-challenge` 라는 밑줄 이름과 `<회사>-verification=` 이라는 형식이 반복됩니다.

이 절의 논지가 [셋째 노트](./07-03.%EC%A3%BC%EA%B3%A0%EB%B0%9B%EB%8A%94%20%EC%9D%BC%EC%9D%80%20%EA%B2%B0%EA%B5%AD%20%EB%AA%87%20%EA%B0%9C%EC%9D%98%20%EB%AA%85%EB%A0%B9%EC%9C%BC%EB%A1%9C%20%EC%A4%84%EC%96%B4%EB%93%A0%EB%8B%A4.md) §4 의 SSH 호스트 키와 짝입니다. 거기서는 **처음 만난 서버를 어떻게 믿는가**가 문제였고 답이 「첫 접속에 받아 적고 다음부터 대조」였습니다. 여기서는 **처음 만난 도메인 주인을 어떻게 믿는가**가 문제이고 답이 「그 사람만 할 수 있는 일을 시켜 본다」입니다. 

둘 다 미리 아는 사이가 아닐 때 신뢰를 세우는 방법입니다. 갈리는 것은 **확인할 통로가 있느냐**입니다. DNS 는 누구나 조회할 수 있는 공개 통로라 CA 가 대신 확인해 줍니다. SSH 에는 그런 통로가 없어 사람이 지문을 들고 대조해야 합니다.
...
10. 캡처의 첫 SYN 에 `ECE` 와 `CWR` 이 함께 붙어 있습니다. 무엇을 하는 중입니까?
11. 소켓은 TCP 위에 놓인 다섯째 층입니까? 아니라면 무엇이며, 어떻게 확인합니까?
12. `dig +short example.com` 이 IP 를 둘 답했습니다. 클라이언트는 무엇을 하며 그 대가는 무엇입니까?
13. CA 는 내가 그 도메인의 주인인지 어떻게 확인합니까? 두 방법을 대 보십시오.
...
### 정답 10 — SYN 에 붙은 ECE 와 CWR

**ECN 을 쓸지 협상하는 중입니다.** RFC 3168 은 `ECE` 와 `CWR` 이 다 선 SYN 을 「ECN-setup SYN 패킷」, `ECE` 만 선 SYN-ACK 을 「ECN-setup SYN-ACK 패킷」이라 부르고, 연결이 ECN 을 쓰려면 이 한 쌍이 오가야 한다고 적습니다.

두 칸의 평소 역할은 따로 있습니다. `ECE` 는 받는 쪽이 「혼잡 표시가 붙은 패킷을 받았다」고 알리는 칸이고, `CWR` 은 보내는 쪽이 「혼잡 윈도를 줄였다」고 답하는 칸입니다.

**값은 손실 없이 신호가 전달된다는 것입니다.** 기존 혼잡 제어는 패킷이 사라지는 것이 신호라 알아차리려면 먼저 손실을 치러야 했는데, ECN 은 라우터가 버리는 대신 표시만 남깁니다.

### 정답 11 — 소켓은 층인가

**아닙니다. API 추상입니다.** 커널이 프로토콜과 양쪽 주소·포트, 곧 5-tuple 을 묶어 파일 디스크립터 하나로 내주는 손잡이입니다.

확인하는 방법이 간단합니다. **패킷을 떠 보면 소켓 헤더가 없습니다.** IP 헤더도 TCP 헤더도 선 위에 흔적을 남기지만 소켓에 해당하는 비트는 어디에도 없습니다. 층은 선 위에 남고 API 추상은 기계 안에만 있습니다.

### 정답 12 — A 레코드가 여럿일 때

**클라이언트가 고릅니다.** 보통 앞의 것부터 쓰는데 네임서버가 답할 때마다 순서를 섞어 주므로, 그것만으로 요청이 여러 서버에 갈립니다. 앞에 장비를 두지 않고 존 파일에 A 를 몇 줄 더 적는 것이 가장 간단한 부하 분산입니다.

**대가는 DNS 가 서버의 생사를 모른다는 것입니다.** 하나가 죽어도 계속 그 주소를 답합니다. 빼려면 레코드를 고쳐야 하는데 그러면 남은 TTL 만큼 옛 답이 계속 돌아옵니다.

### 정답 13 — CA 의 도메인 소유 확인

**그 도메인을 조종할 수 있는 사람만 할 수 있는 일을 시키고 됐는지 봅니다.**

**HTTP-01** 은 무작위 토큰을 `/.well-known/acme-challenge/<토큰>` 에 올리게 한 뒤 가져가 봅니다. 그 경로에 파일을 둘 수 있다는 것이 그 도메인이 가리키는 서버를 쥐고 있다는 증거입니다.

**DNS-01** 은 `_acme-challenge.<도메인>` 에 토큰을 TXT 로 넣게 한 뒤 조회합니다. 존 파일을 고칠 수 있다는 것이 이름 자체를 쥐고 있다는 증거이고, 서버가 없어도 되며 와일드카드 인증서에도 씁니다.
...
- [ ] `ECE` 와 `CWR` 이 각각 누가 누구에게 무엇을 알리는 칸인지 아는가?
- [ ] ECN 이 기존 혼잡 제어보다 무엇을 아끼는지 말할 수 있는가?
- [ ] 소켓이 층이 아니라는 것을 패킷으로 확인하는 방법을 아는가?
- [ ] A 레코드가 여럿일 때의 값과 대가를 짝지을 수 있는가?
- [ ] HTTP-01 과 DNS-01 이 각각 무엇을 증거로 삼는지 구분할 수 있는가?
...
- [RFC 3168 — The Addition of Explicit Congestion Notification (ECN) to IP](https://www.rfc-editor.org/rfc/rfc3168.txt) — ECE·CWR 플래그와 ECN 협상의 1차 자료
- [RFC 8555 — Automatic Certificate Management Environment (ACME)](https://www.rfc-editor.org/rfc/rfc8555.txt) — HTTP-01·DNS-01 도전의 1차 자료
...
[^ecn]: RFC 3168 §4 — "AQM can set a Congestion Experienced (CE) codepoint in the packet header instead of dropping the packet, when such a field is provided in the IP header and understood by the transport protocol. The use of the CE codepoint with ECN allows the receiver(s) to receive the packet, avoiding the potential for excessive delays due to retransmissions after packet losses." <https://www.rfc-editor.org/rfc/rfc3168.txt> (2026-09-07 조회)

[^ecnflags]: RFC 3168 §5 — "an ECN-Echo (ECE) flag in the TCP header so that the data receiver can inform the data sender when a CE packet has been received; and a Congestion Window Reduced (CWR) flag in the TCP header so that the data sender can inform the data receiver that the congestion window has been reduced." <https://www.rfc-editor.org/rfc/rfc3168.txt> (2026-09-07 조회)

[^ecnsetup]: RFC 3168 §6.1.1 — "We call a SYN packet with the ECE and CWR flags set an \"ECN-setup SYN packet\", and we call a SYN packet with at least one of the ECE and CWR flags not set a \"non-ECN-setup SYN packet\" ... we call a SYN-ACK packet with only the ECE flag set but the CWR flag not set an \"ECN-setup SYN-ACK packet\"" 이어서 "Before a TCP connection can use ECN, Host A sends an ECN-setup SYN packet, and Host B sends an ECN-setup SYN-ACK packet." 라고 적습니다. <https://www.rfc-editor.org/rfc/rfc3168.txt> (2026-09-07 조회)

[^http01]: RFC 8555 §8.3 HTTP Challenge — "With HTTP validation, the client in an ACME transaction proves its control over a domain name by proving that it can provision HTTP resources on a server accessible under that domain name. The ACME server challenges the client to provision a file at a specific path, with a specific string as its content." 경로는 "fixed prefix \"/.well-known/acme-challenge/\", followed by the \"token\"" 입니다. <https://www.rfc-editor.org/rfc/rfc8555.txt> (2026-09-07 조회)

[^dns01]: RFC 8555 §8.4 DNS Challenge — "When the identifier being validated is a domain name, the client can prove control of that domain by provisioning a TXT resource record containing a designated value for a specific validation domain name." 이름은 "prepending the label \"_acme-challenge\" to the domain name being validated" 로 만듭니다. <https://www.rfc-editor.org/rfc/rfc8555.txt> (2026-09-07 조회)
