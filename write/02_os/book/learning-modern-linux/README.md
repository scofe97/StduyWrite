...
learning:
  ...
  blocked_count: 0
  next_lesson: "09-01 Advanced Topics — IPC(시그널·named pipe·유닉스 도메인 소켓)·가상 머신·Kerberos 와 PAM"
updated: 2026-09-05
---
...
|------|-----|
| 난이도 레벨 | 기본 |
| 진행률 | 8 / 11 (본문 9장 + 부록 2) · 3장과 7장은 길어 노트 셋으로, 6장과 8장은 둘로 나눔 |
| 최근 검증 | 2026-09-05 · 1~8장 형식 센서 전수(도식 81장 error 0) + 적대적 내용 검증 전수. 8장에서는 지적 15건 중 13건을 반영 — 도식·본문의 원서 절단분 채움 3건, 원문이 채우지 않은 표 칸 2건, 지어낸 눈금값 1건을 되돌리고, 조판 결손을 정오로 승격한 `-rm` 1건은 철회했습니다 |
| 막힌 지점 | 없음 |
| 다음 레슨 후보 | 9장 Advanced Topics. 저자가 선택 사항이라 밝힌 구간이고, 4장이 Kerberos·PAM 을 여기로 미뤄 뒀습니다 |
| 발견한 원문 정오 | 32건 — 1장 1(CPU 모델명) · 2장 4(단위 목록 문구·eBPF 최소 버전·`mallocopt` 표기·시스템 콜 명령어) · 3장 4(`set` 으로 변수 만들기·`unset $VAR`·Ctrl+Z 서술·날짜 예제 입력값) · 4장 1(`/proc/$pid` 로 실제 UID 조회) · 5장 5(`mount -a`·`/proc/self/net/arp`·`devfs` 이름·`var` 행 복붙·AUFS 시제) · 6장 5(`initd`·Ruby 칸 Rails·`ENTRYPOINT` vs `CMD`·`prune -all`·`ARGS`) · 7장 5(멀티캐스트 `/24`·인프라 TLD 예·SRV 를 TTL 로 읽음·URI 순서·`rsync` 목적지 오타) · 8장 7(RFC 5424 `VER` 을 생략 가능이라 적음 · `2&>` 리다이렉션 · 부하 평균이 정규화됐다는 서술 · `sys` 를 대기 시간이라 부름 · `top` 의 `m` 을 정렬 키라 적음 · Prometheus UI 를 `localhost:9000` 이라 적음 · Grafana 데이터소스를 `9100` 이라 적음) |
