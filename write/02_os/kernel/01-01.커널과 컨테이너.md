...
- 그래서 Pod의 `securityContext`나 `seccompProfile`로 시스템 콜 화이트리스트를 강제하는 것이 표준 패턴이다. 보안 정책 자체의 운영은 [10-02 RBAC과 보안](../../08_cloud/kubernetes/05-09.RBAC%EA%B3%BC%20%EB%B3%B4%EC%95%88.md)에서 다룬다.
...
- CPU throttling은 스케줄러 영역, OOMKilled는 메모리 관리 영역, "Pod가 안 뜬다"가 ImagePullBackOff면 VFS·블록 레이어, "Pod끼리는 되는데                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 Service가 안 된다"는 네트워크 스택. CPU·메모리 운영 디테일은 [10-03 자원 관리](../../08_cloud/kubernetes/05-10.%EC%9E%90%EC%9B%90%20%EA%B4%80%EB%A6%AC.md)와 짝을 이뤄 보면 좋다.
...
- [Pod 네트워크와 Linux 기반](../../08_cloud/kubernetes/02-02.Pod%20%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC%EC%99%80%20Linux%20%EA%B8%B0%EB%B0%98.md) — K8s 측에서 본 같은 메커니즘
- [자원 관리](../../08_cloud/kubernetes/05-10.%EC%9E%90%EC%9B%90%20%EA%B4%80%EB%A6%AC.md) — Requests/Limits, QoS, allocatable 계산
- [RBAC과 보안](../../08_cloud/kubernetes/05-09.RBAC%EA%B3%BC%20%EB%B3%B4%EC%95%88.md) — Pod Security Standards, seccomp, capabilities 운영
