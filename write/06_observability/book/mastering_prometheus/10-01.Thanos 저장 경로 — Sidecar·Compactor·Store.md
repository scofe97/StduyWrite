...
> Query 가 목록에 있는 것이 헷갈릴 수 있습니다. Query 의 본업은 여러 StoreAPI 에 부채꼴로 요청을 뿌리는 **소비자** 쪽입니다. 그러면서 자기도 StoreAPI 를 열어 두기 때문에 목록에 함께 듭니다 — Query 를 다른 Query 아래에 계층으로 쌓을 때 쓰는 성질입니다. 즉 Query 는 **소비자이면서 제공자**이고, 나머지 넷은 제공자 쪽입니다.
...
Object Storage 버킷에 뭔가 보이기까지 **최대 2시간** 걸립니다. Sidecar 가 자기 주기로 올리는 것이 아니라, Prometheus 가 압축되지 않은 새 블록을 써 내야 그것을 집어 올리기 때문입니다. 그 블록이 2시간마다 만들어지므로([03-03 §1](./03-03.TSDB%20저장%20구조.md)) 업로드도 그 박자를 따릅니다.

### 3. Thanos Compactor

Compactor 는 오브젝트 스토리지에 놓인 TSDB 블록을 **압축하고 다운샘플링** 합니다. Prometheus 쪽 로컬 압축을 껐으니 누군가는 압축해야 하고 그 일을 이 부품이 맡습니다.

압축 방식 자체는 Prometheus 와 같습니다. 작은 블록 여럿을 가져와 인덱스와 샘플을 합쳐 더 큰 블록 하나를 만듭니다. 합쳐진 인덱스는 각 블록이 따로 인덱스를 갖고 있을 때보다 작은데, **대부분의 시계열이 연속된 여러 블록에 걸쳐 존재한다** 는 전제에 기대기 때문입니다. 거의 언제나 참입니다.

Prometheus 와 다른 점은 하나뿐입니다. Compactor 는 압축하려면 블록을 오브젝트 스토리지에서 **먼저 내려받아야** 합니다.
...
>
> 프로젝트가 이 셋에 붙여 둔 라벨이 그 성격을 말해 줍니다. `--compact.enable-vertical-compaction` 은 `--help` 에 노출되지 않는 **hidden 플래그**이고, `--deduplication.func` 과 `--deduplication.replica-label` 은 **Experimental** 로 표기됩니다.
...
![Store 파티셔닝 두 방식 — 시간 기반은 min-time·max-time 으로 기간을 나누고, external label 기반은 keep·hashmod 로 블록을 나눈다](./_assets/10-01-store-partitioning.svg)

**시간 기반.** Store 인스턴스마다 특정 시간 구간만 맡깁니다. 오브젝트 스토리지에 90일을 둔다면 Store 셋을 띄워 각각 0~29일 전, 30~59일 전, 60~90일 전을 맡깁니다. 얻는 것이 둘입니다.

- 인스턴스별 데이터셋이 작아져 **자원 부담이 낮아집니다.**
- 여러 Store 가 독립적인 데이터셋 위에서 동시에 일해 **쿼리가 더 병렬화됩니다.**

`--min-time` 과 `--max-time` 으로 정하며 절대값(`2021-10-06T20:06:32Z`) 도 상대값(`-30d`) 도 됩니다.
...
아래 다섯 항목을 소스로 확정했습니다. Go 코드가 이어지므로 결론을 먼저 둡니다.

| 확인한 것 | 결론 |
|---|---|
| 다운샘플링 40시간·10일 | 데이터의 **나이**가 아니라 블록의 **시간 폭** 기준 |
| `--retention.resolution-raw` 를 짧게 주면 | 조용히 무시가 아니라 **기동 실패** |
| StoreAPI 의 RPC 개수 | 정확히 넷 (`RemoteWrite` 는 별도 서비스) |
| Sidecar 가 압축 블록을 거르는 방식 | `Compaction.Level > 1` 이면 건너뜀 |
| Compactor `--wait-interval` | 기본 5분, 단 `--wait` 를 줘야 동작 |
...
![다운샘플링 판정은 데이터 나이가 아니라 블록의 시간 폭 — 2시간 블록은 아무리 오래돼도 대상이 아니고, 수평 압축이 span 을 40시간 너머로 키운 뒤에야 다운샘플링된다](./_assets/10-01-downsample-trigger.svg)
...
> Sidecar 를 붙이기 전에 Prometheus 의 로컬 압축을 끕니다. 빠뜨리면 블록이 한 장도 올라가지 않는데 에러도 나지 않아 원인을 찾기 어렵습니다.
...
**Spring 서비스 관점.** 여기까지는 애플리케이션이 전혀 관여하지 않습니다. Micrometer 는 `/actuator/prometheus` 를 낼 뿐이고 Prometheus 가 긁은 블록을 Sidecar 가 통째로 올립니다.

다만 애플리케이션이 만드는 **라벨 카디널리티가 곧 오브젝트 스토리지 비용** 이 됩니다. 여기에 함정이 하나 있습니다.

- 다운샘플링은 **샘플 수만** 줄입니다. **인덱스는 거의 그대로** 남습니다.
- 그래서 카디널리티가 큰 메트릭은 **다운샘플링으로 구제되지 않습니다.**
- 줄이려면 `metric_relabel_configs` 로 **스크레이프 단계에서** 잘라야 합니다([7장](./07-01.최적화와%20디버깅.md)).
...
- [ ] StoreAPI 의 네 RPC 와 그것을 구현하는 다섯 컴포넌트를 말하고, Query 가 소비자이자 제공자인 이유를 설명할 수 있는가?
...
A. 먼저 Prometheus 의 로컬 압축이 꺼져 있는지 봅니다. Thanos 는 `meta.json` 의 `compaction.level` 이 1보다 큰 블록을 무시하므로, Prometheus 가 작은 블록들을 큰 블록으로 합쳐 버리면 올릴 것이 없습니다. `--storage.tsdb.min-block-duration` 과 `--storage.tsdb.max-block-duration` 을 둘 다 `2h` 로 맞춥니다.

Prometheus Operator 를 쓰면 Sidecar 를 켤 때 자동으로 처리됩니다. 그다음, Prometheus 가 압축되지 않은 새 블록을 써 내기까지 두 시간 넘게 걸릴 수 있다는 점도 감안합니다.
