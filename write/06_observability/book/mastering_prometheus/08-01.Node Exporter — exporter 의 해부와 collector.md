...
> exporter 는 메트릭 정의·collector·레지스트리 세 부품으로 이뤄집니다. Node Exporter 가 다른 exporter 와 다른 점은 이 collector 를 수십 개 거느린다는 것이고 각 collector 가 자기 고루틴에서 도는 덕에 개수가 스크레이프 속도를 좌우하지 않습니다. 
>
> `/proc` 에 있으면 Node Exporter 가 가져올 수 있습니다. loadavg 는 리눅스 커널 소스조차 "실없는 숫자"라 부르니 PSI 기반 pressure collector 로 갈아타는 편이 낫고 단명 프로세스는 textfile collector 로 거둡니다. 스크레이프가 느려지면 `node_scrape_collector_duration_seconds` 로 범인 collector 를 한 번에 짚습니다.
...
세 부품이 두 시점에 어떻게 관여하는지 한 장으로 보면 이렇습니다.

위아래 두 줄기에 **같은 collector 가 두 번 등장한다**는 점이 요점입니다. 위는 프로세스가 뜰 때 한 번 도는 줄기라 `Describe()` 로 형상만 알리고 끝납니다. 아래는 스크레이프마다 반복되는 줄기라 그때 비로소 `Collect()` 가 값을 채웁니다.

![exporter 는 메트릭 정의·collector·레지스트리 세 부품으로 이뤄지고, 등록 시점에 Describe 가 형상만 알리고 스크레이프 시점에 Collect 가 값을 채운다](./_assets/08-01-exporter-anatomy.svg)
...
> 책의 표는 `stalled` 둘만 싣지만 exporter 가 실제로 내는 것은 다섯입니다. `node_pressure_cpu_waiting_seconds_total`, `node_pressure_io_waiting_seconds_total`, `node_pressure_io_stalled_seconds_total`, `node_pressure_memory_waiting_seconds_total`, `node_pressure_memory_stalled_seconds_total`. 이름에서 보듯 `cpu_stalled` 만 없습니다.[^ne-pressure]
