...
아래 지도는 SD 후보가 스크레이프와 두 relabeling 단계를 거쳐 TSDB 시계열이 되는 전체 흐름을 이어 보여 줍니다. 아래쪽 보조 영역은 Operator 설정의 두 경로와 HTTP SD·진단 경계를 같은 파이프라인에 대입할 수 있게 정리합니다.

![서비스 디스커버리와 relabeling 전체 지도](./_assets/04-01.chapter-overview.svg)
...
1 단계와 2 단계에서 앱은 아직 아무 요청도 받지 않았습니다. Prometheus 는 쿠버네티스 API 나 Eureka 레지스트리에 물어봤을 뿐입니다. `__meta_` 는 그 답에서 만들어집니다.

앱에 처음 연락이 가는 것은 3 단계 스크레이프입니다. 이때 앱이 내놓는 것은 메트릭 본문뿐입니다. `/actuator/prometheus` 응답 어디에도 `__meta_` 는 없고 앱은 그런 것이 있는 줄도 모릅니다.
...
- 범위도 갈립니다. `metric_relabel_configs` 는 **그 잡이 긁어온 메트릭에만** 걸리므로, 같은 이름의 메트릭이 다른 잡에 있어도 건드리지 않습니다. 
- 대상이 그 메트릭을 내지 않는 잡에 규칙을 걸면 효과가 0 입니다. 규칙을 적었다는 사실만으로 카디널리티가 줄었다고 보지 말고 적용 전후를 세어 봐야 합니다.

여기에 예외가 하나 있습니다. **메트릭 relabeling 은 `up` 처럼 Prometheus 가 자동 생성하는 시계열에는 걸리지 않습니다.**[^prom-metric-relabel] 넓은 `drop` 규칙을 걸어도 `up` 은 살아남으므로 `up == 0` 알림이 조용히 사라지는 사고는 일어나지 않습니다. 뒤집어 말하면 `up` 계열을 이 규칙으로 줄이려는 시도는 통하지 않습니다.



## 3. 규칙 하나는 어떻게 읽습니까

> 규칙은 부품 몇 개로 이루어지고, 여러 개일 때는 목록이 아니라 순서 있는 파이프라인입니다. 자주 쓰는 것 중 후보 수를 바꾸는 액션은 `keep`·`drop` 둘입니다.

![relabel 규칙이 후보 500 개를 120 개, 40 개로 줄이고 마지막 replace 규칙은 수를 바꾸지 않는다](_assets/04-01.relabel-filter-funnel.svg)

| 필드 | 뜻 | 기본값 |
|---|---|---|
| `source_labels` | 어떤 라벨의 값을 볼 것인가 | 없음 — 없는 라벨은 빈 문자열(`""`)로 읽힙니다 |
| `regex` | 그 값이 이 패턴에 맞는가 | `(.*)` — 무엇이든 매칭 |
| `action` | 맞으면 무엇을 할 것인가 | `replace` |
| `target_label` | `replace` 일 때 어느 라벨에 쓸 것인가 | 없음 — `replace` 에는 필수입니다 |
| `replacement` | `target_label` 에 실제로 쓸 값 | `$1` — `regex` 의 첫 캡처 그룹입니다[^prom-relabel-fields] |

`source_labels` 가 여럿이면 값들이 `separator`(기본 `;`)로 이어져 하나의 문자열이 되고 `regex` 는 그 이어붙인 문자열에 걸립니다.

`replacement` 의 기본값이 `$1` 이라는 점이 앞의 예제들을 설명해 줍니다. `regex` 를 생략하면 기본값 `(.*)` 가 값 전체를 첫 그룹으로 잡고, `replacement` 기본값 `$1` 이 그것을 그대로 씁니다. `source_labels` 하나에 `target_label` 만 적은 규칙이 "값을 그대로 옮겨 담는" 동작이 되는 이유입니다. 고정 문자열을 넣고 싶으면 `replacement` 에 직접 적습니다.

`regex` 는 **양쪽 끝이 앵커링**됩니다. `regex: "true"` 는 값이 정확히 `true` 일 때만 걸리며, 부분 일치를 원하면 `.*<regex>.*` 로 풀어야 합니다.[^prom-relabel-fields]

현행 문서는 액션을 **11개** 정의합니다.[^prom-relabel-actions] 실무에서 손이 가는 것은 위쪽 다섯이고, 나머지는 쓰임이 좁습니다.
...
| `keepequal` | `source_labels` 가 `target_label` 과 다르면 버림 | 줄어듦 | 두 라벨이 같은 것만 남길 때 |
| `dropequal` | `source_labels` 가 `target_label` 과 같으면 버림 | 줄어듦 | 위의 반대 |
| `hashmod` | 해시를 `modulus` 로 나눈 나머지를 씀 | 안 바뀜 | 샤딩할 때 |
| `labelmap` | 라벨 **이름**을 정규식으로 골라 새 이름에 복사 | 안 바뀜 | 접두사를 통째로 옮길 때 |
| `lowercase` | 읽은 값을 소문자로 | 안 바뀜 | 값 표기를 통일할 때 |
| `uppercase` | 읽은 값을 대문자로 | 안 바뀜 | 위와 같음 |

후보 수를 줄이는 것은 `keep`·`drop`·`keepequal`·`dropequal` 넷입니다. 앞의 둘은 **정규식**으로 판정하고 뒤의 둘은 **다른 라벨과 같은지**로 판정한다는 차이가 있습니다.

`keep` 과 `drop` 은 서로 반대이므로 하나로 같은 일을 표현할 수 있습니다. 정규식이 단순해지는 쪽을 고르는 것이 관례입니다. `replace` 는 **값**을 옮기고 `labeldrop` 은 **라벨 자체**를 없앤다는 차이도 짚어 둘 만합니다. `labeldrop`·`labelkeep`·`labelmap` 의 `regex` 는 라벨 값이 아니라 **이름**에 걸리므로 `source_labels` 를 쓰지 않습니다.

`labeldrop` 과 `labelkeep` 에는 문서가 경고를 답니다. 라벨을 지운 뒤에도 각 시계열이 **여전히 유일하게 식별되는지** 확인해야 합니다. 구분하던 라벨을 지우면 서로 다른 시계열이 같은 이름으로 겹쳐 충돌합니다.[^prom-relabel-actions]
...
지킬 요구사항은 HTTP **200**, `Content-Type: application/json`, **UTF-8** 셋이고,[^prom-http-sd] 응답은 static config 와 같은 모양의 JSON 입니다.
...
응답 형식 말고 **호출 방식**에도 계약이 둘 붙습니다.[^prom-http-sd-refresh]

- **매번 전체 목록을 돌려줘야 합니다.** 증분 갱신은 지원하지 않습니다. 앞선 응답과의 차이만 보내는 구현은 성립하지 않습니다.
- **호출은 `refresh_interval` 마다 오고 기본값은 1분입니다.** 요청에 `X-Prometheus-Refresh-Interval-Seconds` 헤더가 실려 오므로 서버 쪽에서 주기를 읽을 수 있습니다. 목록을 만드는 비용이 크면 이 주기를 염두에 두고 캐시를 답니다.

Prometheus 는 자기 호스트명을 보내지 않고, 서버는 그 요청이 재시작 직후 첫 호출인지도 알 수 없습니다. 요청만 보고 상태를 추론하는 설계는 피합니다.
...
이 캐시에는 **시간 기반 만료가 없습니다.** 인벤토리 API 가 사흘간 죽어 있으면 사흘 전 목록을 그대로 쓰며, 스스로 포기하지 않으니 증상도 없습니다. 옛 목록의 서버들이 여전히 살아 있다면 스크레이프는 전부 성공하고 `up` 은 전부 1 이며 대시보드는 초록색입니다. 무너진 것은 **목록의 신선도**뿐이라 어느 화면에도 드러나지 않습니다.

지워지는 경로는 시간이 아니라 **재시작**입니다. 문서가 목록은 재시작을 넘겨 보존되지 않는다고 못 박습니다.[^prom-http-sd-cache] 그래서 인벤토리가 죽어 있는 동안 Prometheus 를 재시작하면 옛 목록마저 사라져 타깃이 통째로 비고, 조용하던 문제가 그제야 요란해집니다. 갱신 실패를 방치하면 장애가 재시작 시점까지 미뤄질 뿐입니다.
...
같은 시계열을 저장소에서 꺼내면 라벨이 열네 개로 늘어 있습니다. 앱이 붙인 여섯 개가 그대로 있습니다. 거기에 `job`·`namespace`·`pod`·`container`·`service`·`endpoint`·`instance` 일곱 개가 더해졌고, 메트릭 이름이 `__name__` 라벨로 들어갔습니다. 값 `91` 은 바뀌지 않습니다 — relabeling 은 라벨만 만지고 샘플 값에는 손대지 않습니다.

그래서 최종 라벨 개수는 메트릭마다 다릅니다. 같은 타깃에서 와도 `jvm_threads_live_threads` 처럼 앱이 구분 축을 두지 않은 메트릭은 대상 라벨 일곱 개와 `__name__` 만 갖고, `jvm_memory_used_bytes` 는 `area`·`id` 가 더해져 아홉 개가 됩니다. 공통으로 깔리는 것은 대상 라벨 쪽입니다. 메트릭 고유 라벨은 그 위에서 시계열을 가릅니다.
...
빈 리스트를 그대로 반환하면 Jackson 이 `[]` 로 직렬화하므로, **아무것도 하지 않는 것이 정답**입니다. 반대로 DB 조회 자체가 실패했다면 그때는 예외를 던지는 쪽이 맞습니다 — 0 대임을 확인한 것이 아니라 "모르는" 상태라, 옛 목록을 유지하는 편이 안전합니다.
...
- [ ] `replacement` 의 기본값이 `$1` 이라는 사실과, 그것이 "값을 그대로 옮기는" 동작을 만든다는 점을 아는가?
- [ ] `regex` 가 양쪽 끝 앵커링이라는 사실과 부분 일치를 만드는 법을 아는가?
- [ ] 메트릭 relabeling 이 `up` 같은 자동 생성 시계열에는 걸리지 않는다는 사실을 아는가?
...
- [ ] `http_sd` 목록 캐시가 재시작을 넘겨 보존되지 않는다는 사실과 그 의미를 아는가?
- [ ] `http_sd` 가 증분 갱신을 지원하지 않고 기본 호출 주기가 1분이라는 사실을 아는가?
...
메트릭 엔드포인트로는 못 싣습니다. `__meta_` 는 앱이 아니라 SD 프로바이더가 만듭니다. 그 시점에는 앱에 요청조차 가지 않습니다.

값을 실으려면 **프로바이더가 들여다보는 곳**에 심어야 합니다.

- **Eureka** — `eureka.instance.metadata-map` 에 넣으면 `__meta_eureka_app_instance_metadata_<이름>` 으로 들어옵니다.
- **쿠버네티스** — 파드 annotation 이 `__meta_kubernetes_pod_annotation_*` 으로 들어옵니다.

앱은 레지스트리에 등록할 뿐이고 Prometheus 는 레지스트리에게 물어봅니다.
