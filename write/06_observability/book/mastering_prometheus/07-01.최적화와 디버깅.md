...
앞 장들과 달리 이 장은 순서대로 읽는 파이프라인이 아니라 **증상별로 갈라 들어가는 진단표**에 가깝습니다. 첫 줄기만 세 단계(찾기 → 떨구기 → 막기)로 이어지고 나머지 셋은 서로 독립입니다. 마지막 줄기의 붉은 띠가 이 장에서 가장 값비싼 실수입니다. `GOMEMLIMIT` 없이 `GOGC` 기본값으로 두면 GC 목표가 계속 달아나 커널이 프로세스를 정리합니다.
...
두 가지가 중요합니다. 첫째, **모든 리밋은 메트릭 relabeling 이 끝난 뒤 평가**됩니다. 원치 않는 시계열이나 라벨을 먼저 떨구면 리밋 계산에 잡히지 않습니다. 둘째, 리밋을 하나라도 어기면 **스크레이프 전체가 실패로 처리되고 어떤 샘플도 수집되지 않습니다**.[^prom-scrape-limits] 그래서 리밋을 켤 때는 실패한 스크레이프에 대한 알림을 반드시 함께 걸어야 합니다. 그러지 않으면 조용히 메트릭이 사라집니다.
...
> 책의 서술과 공식 문서·소스가 어긋나는 여섯 지점입니다. 라벨 길이의 단위, 규약을 어기는 예시, 제거 예정인 지터 플래그, 조용히 깎이는 허용치를 각각 원본으로 확정합니다.

**라벨 길이 리밋은 글자 수가 아니라 바이트 수입니다.** 본문은 `label_value_length_limit` 와 `label_name_length_limit` 을 "공백을 포함한 **문자 수**" 기준이라고 적습니다. 공식 문서는 다르게 적습니다.
...
이 차이가 한글 환경에서 사고로 이어집니다. 한글은 UTF-8 에서 글자당 3 바이트이므로 `label_value_length_limit: 50` 은 한글 라벨 값 기준으로 **16 글자 남짓**입니다. 한글이나 이모지가 라벨 값에 들어가면 예상보다 훨씬 일찍 리밋에 걸립니다. 문서는 네 리밋 모두 기본값이 `0`(제한 없음)이라는 사실도 함께 적습니다.[^prom-scrape-limits]

**본문이 든 레코딩 룰 예시는 본문이 방금 말한 규약을 어깁니다.** 규약은 "카운터(`_total`)에 `rate`·`irate` 를 적용하면 접미사를 떼라" 인데, 바로 뒤에 나오는 kube-prometheus 의 실제 룰 이름은 `_total` 을 그대로 답니다.
...
확인해 보면 **업스트림 kube-prometheus 가 정말 그렇게 이름 짓습니다**. 규약을 어긴 쪽은 저자가 아니라 kube-prometheus 이고, 공식 문서 역시 저자와 같은 규약을 적습니다 — "`metric` … should be unchanged other than **stripping `_total` off counters** when using `rate()` or `irate()`." 널리 쓰이는 룰이라 이제 와 고치기 어려울 뿐이니, 규약은 규약대로 새 룰에 적용하면 됩니다.

같은 문서가 본문이 빠뜨린 규약 하나를 더 줍니다. **다른 연산이 있으면 `_sum` 은 생략**합니다.[^prom-rule-naming]

**본문이 권하는 지터 플래그는 업스트림에서 실험적이고 제거 예정입니다.** `--scrape.adjust-timestamps` 와 `--scrape.timestamp-tolerance` 는 Prometheus 소스에서 둘 다 `Hidden()` 으로 감춰져 있습니다. 플래그 설명에는 이렇게 적혀 있습니다.

> Experimental. **This flag will be removed in a future release.**

기본값이 `2ms` 라는 본문의 서술은 소스와 일치합니다. 상수 이름은 `ScrapeTimestampTolerance` 이고 값은 `2 * time.Millisecond` 입니다. 튜닝하기 전에 이 플래그가 언젠가 사라진다는 사실을 알아 두는 편이 낫습니다.
