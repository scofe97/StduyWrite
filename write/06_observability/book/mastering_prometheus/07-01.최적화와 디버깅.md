...
네 표의 신뢰도를 한눈에 정리하면 이렇습니다. 진단은 ①로 시작하고 ③④는 카디널리티 판단에서 빼 둡니다.

![/tsdb-status 네 표의 카디널리티 진단 신뢰도 — ① 라벨 이름별 값 개수가 가장 중요, ③ 메모리 라벨과 ④ 라벨-값 쌍은 진단에 쓰지 말 것](./_assets/07-01-tsdb-status-trust.svg)
...
이 차이가 한글 환경에서 사고로 이어집니다. 한글은 UTF-8 에서 글자당 3 바이트이므로 `label_value_length_limit: 50` 은 한글 라벨 값 기준으로 **16 글자 남짓**입니다. 한글이나 이모지가 라벨 값에 들어가면 예상보다 훨씬 일찍 리밋에 걸립니다. 문서는 네 리밋 모두 기본값이 `0`(제한 없음)이라는 사실도 함께 적습니다. ([Configuration — scrape_config](https://prometheus.io/docs/prometheus/latest/configuration/configuration/#scrape_config))
