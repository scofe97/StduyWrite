...
아래는 이 절 전체를 증상에서 도구로 가는 진단 흐름으로 이은 지도입니다. 각 증상 옆의 절 번호가 본문에서 그 손잡이를 다루는 곳입니다.

앞 장들과 달리 이 장은 순서대로 읽는 파이프라인이 아니라 **증상별로 갈라 들어가는 진단표**에 가깝습니다. 첫 줄기만 세 단계(찾기 → 떨구기 → 막기)로 이어지고 나머지 셋은 서로 독립입니다.

지도에서 색이 붙은 곳은 두 군데뿐이고 둘 다 경고입니다. 나머지 상자는 전부 같은 회색이라 색을 해석할 일이 없습니다. 앰버 띠는 리밋을 하나만 어겨도 그 스크레이프가 통째로 실패한다는 뜻이고, 붉은 띠는 이 장에서 가장 값비싼 실수입니다. `GOMEMLIMIT` 없이 `GOGC` 기본값으로 두면 GC 목표가 계속 달아나 커널이 프로세스를 정리합니다.

![최적화와 디버깅 전체 지도](./_assets/07-01.chapter-overview.svg)



## 1. 카디널리티 범인 찾기

> 시작점은 `/tsdb-status` 입니다. 네 표 중 **라벨 이름별 값 개수**만 믿고, 메모리 표와 라벨-값 쌍 표는 카디널리티 진단에서 빼 둡니다.
...
네 표의 신뢰도를 한눈에 정리하면 이렇습니다. 진단은 첫째 표로 시작하고 셋째·넷째 표는 카디널리티 판단에서 빼 둡니다.

![/tsdb-status 네 표의 카디널리티 진단 신뢰도 — 첫째 표인 라벨 이름별 값 개수가 가장 중요하고, 셋째 표인 메모리 라벨과 넷째 표인 라벨-값 쌍은 진단에 쓰지 말 것](./_assets/07-01-tsdb-status-trust.svg)



## 2. 범인을 떨구기

> 라벨을 손으로 뒤지지 않고 PromQL 로 좁힌 뒤 `metric_relabel_configs` 의 `drop` 으로 버립니다. `labeldrop` 은 시계열 고유성을 깨뜨릴 수 있어 조심합니다.
...
> 사후에 떨구는 대신 애초에 수집되지 않게 막습니다. 네 리밋은 **메트릭 relabeling 이 끝난 뒤** 평가되고, 하나라도 어기면 **스크레이프 전체가 실패**합니다.
...
1. 첫째, **모든 리밋은 메트릭 relabeling 이 끝난 뒤 평가**됩니다. 원치 않는 시계열이나 라벨을 먼저 떨구면 리밋 계산에 잡히지 않습니다. 
2. 둘째, 리밋을 하나라도 어기면 **스크레이프 전체가 실패로 처리되고 어떤 샘플도 수집되지 않습니다**.[^prom-scrape-limits] 그래서 리밋을 켤 때는 실패한 스크레이프에 대한 알림을 반드시 함께 걸어야 합니다. 그러지 않으면 조용히 메트릭이 사라집니다.

네 리밋 모두 **기본값이 `0`(제한 없음)** 이므로 적지 않으면 아무 방어도 없습니다.[^prom-scrape-limits]

### 길이 리밋의 단위는 글자가 아니라 바이트입니다

책은 두 길이 리밋을 "공백을 포함한 문자 수" 기준이라고 적습니다. 공식 문서는 다르게 적습니다.

> Limit on the length (**in bytes**) of each individual label value… Note that label values are UTF-8 encoded, and **characters can take up to 4 bytes**.

이 차이가 한글 환경에서 사고로 이어집니다. 한글은 UTF-8 에서 글자당 3 바이트입니다. `label_value_length_limit: 50` 은 영문 라벨 값이라면 50 글자지만 한글 라벨 값에는 **16 글자 남짓**입니다. 한글이나 이모지가 라벨 값에 들어가면 예상보다 훨씬 일찍 리밋에 걸립니다.[^prom-scrape-limits]

### 타깃 쪽에도 리밋이 둘 더 있습니다

위 넷이 *스크레이프 내용*을 제한한다면, 문서는 *타깃 개수*를 제한하는 항목도 함께 정의합니다. 본문이 다루지 않는 쪽이라 여기 붙여 둡니다.

| 리밋 | 무엇을 제한하는가 | 걸렸을 때 |
|------|------------------|----------|
| `target_limit` | 스크레이프 잡 하나가 가질 수 있는 **고유 타깃 수** | 긁지 않고 타깃을 실패로 표시합니다 |
| `keep_dropped_targets` | relabeling 으로 **버려진 타깃** 중 메모리에 남길 수 | 초과분을 들고 있지 않습니다 |

둘의 평가 시점이 앞의 넷과 다릅니다. `target_limit` 은 **타깃 relabeling** 이 끝난 뒤에 걸리므로(메트릭 relabeling 이 아닙니다) [4장](04-01.서비스%20디스커버리와%20relabeling.md)의 `keep`·`drop` 으로 후보를 줄이면 이 셈에서 빠집니다. `target_limit` 은 문서가 실험적 기능으로 표시합니다.[^prom-target-limits]



## 4. 레코딩 룰 — 미리 계산해 두기

> 비싼 쿼리를 미리 계산해 새 시계열로 남깁니다. 이름은 `level:metric:operations` 규약을 따르고, 룰이 늘수록 rule manager 가 RAM 을 먹습니다.
...
이 예시는 방금 말한 규약을 어깁니다. 카운터에 `irate` 를 걸었으면 `_total` 을 떼야 하는데 그대로 답니다.

| | 이름 |
|---|---|
| 실제 kube-prometheus 룰 | `...container_cpu_usage_seconds_total:sum_irate` |
| 규약대로라면 | `...container_cpu_usage_seconds:sum_irate` |

확인해 보면 업스트림 kube-prometheus 가 정말 그렇게 이름 짓습니다. 규약을 어긴 쪽은 이 책의 저자가 아니라 kube-prometheus 이고, 공식 문서도 저자와 같은 규약을 적습니다. 이미 널리 쓰이는 룰이라 이제 와 고치기 어려울 뿐입니다. 규약은 규약대로 새 룰에 적용하면 됩니다.

같은 공식 문서가 규약 하나를 더 줍니다. **다른 연산이 있으면 `_sum` 은 생략**합니다.[^prom-rule-naming]
...
## 5. 스크레이프 지터 — 몇 밀리초가 기가바이트로

> 저자가 본 **과도하게 큰 블록의 가장 흔한 원인**입니다. 간격이 흔들리면 델타의 델타가 0 이 아니게 되어 샘플마다 타임스탬프를 저장해야 합니다.
...
기본값 2ms 는 소스와 일치합니다. 상수 이름은 `ScrapeTimestampTolerance` 이고 값은 `2 * time.Millisecond` 입니다. 다만 튜닝에 들어가기 전에 알아 둘 것이 있습니다. 두 플래그는 업스트림 소스에서 `Hidden()` 으로 감춰져 있고 설명에 이렇게 적혀 있습니다.

> Experimental. **This flag will be removed in a future release.**

언젠가 사라질 손잡이라는 뜻입니다. 지금 쓸 수는 있지만 여기에 운영을 의존하지는 않는 편이 낫습니다.

Prometheus 메인테이너 Julien Pivotto 가 만든 [`oy-scrape-jitter`](https://github.com/o11ydev/oy-toolkit/releases) 가 이 일을 자동으로 해 줍니다.

```bash
# 튜닝 전 — 스크레이프 정렬 상태를 먼저 잽니다
...
정렬된 타깃이 2 개뿐이고 22 개가 어긋나 있으며 최대 어긋남은 30ms 입니다. `--scrape.timestamp-tolerance` 는 **스크레이프 주기의 1% 를 넘을 수 없습니다.** 기본 주기가 30초이니 최대 300ms 까지입니다. 저자의 최대 어긋남이 30ms 였으므로 거기서 시작합니다.

이 상한은 소스에서도 확인됩니다. 스크레이프 루프가 허용치를 이렇게 자릅니다.
...
`min` 이라 조용히 깎인다는 점이 중요합니다. 30초 주기에서 플래그에 300ms 보다 큰 값을 넣어도 오류가 나지 않고 그냥 300ms 가 됩니다. 설정한 값이 그대로 적용됐다고 착각하기 쉬운 자리입니다.
...
`max_ms` 가 30 에서 34 로 **올랐는데도** 이것이 성공입니다. 정렬된 타깃이 2 개에서 23 개로 늘었고 어긋난 타깃은 1 개만 남았습니다. `max_ms` 는 전체의 평균이 아니라 **정렬 실패의 최대치**입니다. 남은 그 하나의 어긋남이 34ms 라서 허용치 30ms 로는 여전히 못 잡을 뿐입니다.
...
## 6. pprof 로 Go 런타임 들여다보기

> "메모리를 만드는 것이 코드의 어디인가" 를 묻는 도구입니다. heap 엔드포인트는 **힙만** 세므로 전체 사용량과 일치하지 않습니다.
...
```bash
# 힙 프로파일을 받아 대화형 모드로 들어갑니다
go tool pprof http://localhost:9090/debug/pprof/heap
```

대화형 프롬프트에서 `top` 을 치면 메모리를 많이 쓰는 순서로 나옵니다.
...
```bash
# pprof 데이터 일곱 종류를 한 번에 받아 파일로 묶습니다
...
> 로그는 사후 대응이고 플래그가 예방입니다. 쿼리 로그에는 **끝난** 쿼리만 남고, 돌고 있는 쿼리는 `queries.active` 가 따로 관리합니다.
...
기본값 20 은 큰 환경에서는 꽤 낮습니다. 이 수를 넘기면 Prometheus 는 **새 요청을 거절하기 시작합니다** — 큐에 쌓아 두고 기다리게 하는 것이 아니라 그 자리에서 되돌려 보냅니다. `--query.max-samples` 가 세는 것은 **반환되는 샘플이 아니라 쿼리를 처리하려고 적재해야 하는 샘플**입니다. 30초 주기로 수집한 메트릭에 5분 `rate()` 를 거는 순간 쿼리는 데이터 포인트를 하나만 돌려주지만 메모리에는 샘플 10개가 올라옵니다.

CLI 문서는 여기서 한 줄 더 나갑니다. 적재량을 막으면 "**so this also limits the number of samples a query can return**" 이라고 적습니다. 반환량을 직접 세지는 않지만 적재량이 상한이니 반환량도 자연히 그 아래로 묶인다는 뜻입니다.



## 8. 가비지 컬렉션 조이기

> 손잡이는 `GOGC` 와 `GOMEMLIMIT` 둘뿐입니다. 앞의 것은 움직이는 상대값, 뒤의 것은 고정된 소프트 리밋이라 함께 씁니다.
...
저자는 이 설명이 **의도적인 단순화**라고 못 박습니다. GC 가 재는 크기에는 라이브 힙만이 아니라 고루틴 스택의 메모리와 전역 포인터도 들어가고, 퍼센트는 그 전체에 걸립니다. 다만 Prometheus 의 메모리 사용량에서 이 둘이 차지하는 몫은 크지 않아 실무에서는 "라이브 힙 기준" 으로 이해해도 무방합니다.
...
이 인과를 흐름으로 보면 이렇습니다. `GOMEMLIMIT` 이 없으면 목표가 계속 두 배로 달아나 GC 가 손을 못 씁니다.

```mermaid
flowchart TB
    A["GOGC=100<br/>GOMEMLIMIT 없음"] --> B["라이브 힙이<br/>시스템 메모리의 50% 초과"]
    B --> C["다음 GC 목표 = 라이브 × 2<br/>→ 시스템 전체 메모리 초과"]
    C --> D["마크 단계가 영영 오지 않음<br/>(목표에 닿을 수 없으니까)"]
    D --> E["커널 OOM 킬러가<br/>프로세스를 정리"]
    F["GOMEMLIMIT ≈ 가용 90%<br/>설정"] -.->|"소프트 리밋에 다가가면<br/>GC 가 공격적으로"| C
    style A fill:#f5edda,color:#3a2f10
    style B fill:#dceaf7,color:#10243a
    style C fill:#5b9bd5,color:#e8e4df
    style D fill:#0d0d0d,color:#c9c4be,stroke:#8a8580
    style E fill:#3a1414,color:#f0a0a0
    style F fill:#66bb6a,color:#0d0d0d
```
...
> 이 장의 진단·리밋·룰 이름·GC 가 Spring 환경에서 어디로 떨어지는지입니다. 카디널리티 출처·`ServiceMonitor` 리밋·룰 이름·JVM 대비 넷을 짚습니다.
...
**GC 두 손잡이는 JVM 을 아는 사람에게 오히려 낯섭니다.** `GOGC` 는 힙 크기의 **증가 비율**로 GC 시점을 정하는 값이라 JVM 의 고정 힙 상한과 성격이 다릅니다. 가장 헷갈리는 지점은 `GOMEMLIMIT` 이 **소프트 리밋**이라는 사실입니다. JVM 상한과 대비하면 이렇습니다.

| | JVM `-Xmx` | Go `GOMEMLIMIT` |
|---|---|---|
| 성격 | 하드 상한 | 소프트 리밋 |
| 넘으면 | `OutOfMemoryError` 예외 | 예외 없음, GC 를 더 공격적으로 |
| 그래도 못 줄이면 | — | 커널 **OOM 킬러**가 프로세스 정리 |

컨테이너 메모리 한계를 `-XX:MaxRAMPercentage` 로 맞추듯, `GOMEMLIMIT` 을 가용 메모리의 90% 로 두는 이유가 여기 있습니다. 상한에 닿기 전에 GC 가 먼저 일하게 만들려는 조치입니다.
...
1. Prometheus 에서 카디널리티 문제를 어떻게 찾습니까?
2. "메모리를 많이 쓰는 라벨" 표로 카디널리티를 진단하면 안 되는 이유는 무엇입니까?
3. 스크레이프 리밋은 언제 평가됩니까?
4. 레코딩 룰 이름 `level:metric:operations` 를 설명해 보십시오.
5. 레코딩 룰을 많이 만들면 무엇이 나빠집니까?
6. 스크레이프 지터가 왜 디스크를 먹습니까?
7. `--query.max-samples` 는 무엇을 세는 값입니까?
8. `GOGC` 와 `GOMEMLIMIT` 의 차이는 무엇입니까?
...
- 본문이 안내하는 링크 — [Scrape Jitter (o11y.tools)](https://o11y.tools/oy-scrape-jitter/) · [runtime/pprof](https://pkg.go.dev/runtime/pprof) · [Profiling Go with pprof (Julia Evans)](https://jvns.ca/blog/2017/09/24/profiling-go-with-pprof/) · [Analysing Prometheus memory usage (Robust Perception)](https://www.robustperception.io/analysing-prometheus-memory-usage/) · [Optimising startup time of Prometheus 2.6.0 with pprof (Robust Perception)](https://www.robustperception.io/optimising-startup-time-of-prometheus-2-6-0-with-pprof/) · [A Guide to the Go Garbage Collector](https://go.dev/doc/gc-guide) · [GOMEMLIMIT is a game changer (Weaviate)](https://weaviate.io/blog/gomemlimit-a-game-changer-for-high-memory-applications) · [pprof (google/pprof)](https://github.com/google/pprof)
...
- 관련 노트: [README (책 MOC)](README.md) · [03-03. TSDB 저장 구조](03-03.TSDB%20저장%20구조.md) · [06-01. 샤딩·페더레이션·고가용성](06-01.샤딩·페더레이션·고가용성.md) · [04-01. 서비스 디스커버리와 relabeling](04-01.서비스%20디스커버리와%20relabeling.md)
...
### 정답 5 — rule manager 가 TSDB 를 계속 두드린다
...
| 문서 | 이 절과 어떻게 이어지는가 |
|------|------------------------|
| [06-01. 샤딩·페더레이션·고가용성](06-01.샤딩·페더레이션·고가용성.md) | **앞 절입니다.** 거기서 "카디널리티가 범인" 이라 말하고 7장으로 미룬 식별·완화가 이 절입니다 |
| [03-03. TSDB 저장 구조](03-03.TSDB%20저장%20구조.md) | 스크레이프 지터가 왜 디스크를 먹는지 — 델타의 델타와 컴팩션이 거기 있습니다 |
| [04-01. 서비스 디스커버리와 relabeling](04-01.서비스%20디스커버리와%20relabeling.md) | `metric_relabel_configs` 의 `drop`·`labeldrop` 문법과 타깃 relabeling 의 경계 |
| [03-04. PromQL 기초](03-04.PromQL%20기초.md) | 카디널리티 범인을 좁히는 `sort_desc(count by (...))` 의 집계 문법 |
| [00-01. 용어집](00-01.용어집.md) | 카디널리티·레코딩 룰·지터 등 이 절 용어의 정의 |


[^prom-scrape-limits]: Prometheus — Configuration, `scrape_config`. 네 리밋 모두 `default = 0` 이고 길이는 `(in bytes)` 기준(`characters can take up to 4 bytes`). 실패 범위는 `post metric-relabeling, the entire scrape will be treated as failed` (2026-08-06 조회).
<https://prometheus.io/docs/prometheus/latest/configuration/configuration/#scrape_config>

[^prom-target-limits]: Prometheus — Configuration, `scrape_config`. `target_limit` 은 `after target relabeling` 평가되고 `mark the targets as failed without scraping them` 이며 `This is an experimental feature` (2026-08-06 조회).
<https://prometheus.io/docs/prometheus/latest/configuration/configuration/#scrape_config>

[^prom-rule-naming]: Prometheus — Recording rules, Naming. 카운터 규약의 원문은 `should be unchanged other than stripping _total off counters when using rate() or irate()` 이며, 같은 절이 다른 연산이 있을 때 `_sum` 을 생략하도록 적습니다 (2026-08-06 조회).
