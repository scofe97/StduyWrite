...
flowchart LR
    ...
    C -->|"일치"| D["스크레이프함"]
    C -->|"불일치"| E["버림"]
    style A fill:#dceaf7,color:#10243a
    style B fill:#5b9bd5,color:#e8e4df
    style C fill:#f5edda,color:#3a2f10
    style D fill:#66bb6a,color:#0d0d0d
    style E fill:#0d0d0d,color:#8a8580,stroke:#8a8580
```
...
**세 번째 relabeling 이 여기서 등장합니다.** 4장이 표준 relabeling 과 메트릭 relabeling 을 갈랐다면, 이 장은 **알림 relabeling** 을 더합니다. 알림을 Alertmanager 로 보내기 직전에 도는 단계입니다.

공식 설정 문서에서도 `alert_relabel_configs` 는 `scrape_configs` 가 아니라 최상위 `alerting` 블록 아래 있습니다. 셋은 도는 시점이 다르므로 문제를 찾을 자리도 다릅니다.
...
**`honor_labels` 가 실제로 하는 일은 "덮어쓰기 방지" 보다 구체적입니다.** 공식 문서는 이 설정이 두 종류의 라벨 사이 **충돌**을 어떻게 풀지 정한다고 적습니다. 하나는 스크레이프한 데이터에 이미 있는 라벨이고, 다른 하나는 Prometheus 가 서버 쪽에서 붙이려는 라벨(`job`·`instance`, 수동 설정한 타깃 라벨, 서비스 디스커버리가 만든 라벨)입니다.
...
문서는 페더레이션과 Pushgateway 스크레이프를 이 설정을 켜야 하는 대표 사례로 꼽습니다. 어느 쪽이든 라벨을 지우지 않고 **충돌만** 푼다는 점이 요점입니다. 같은 문서가 전역 `external_labels` 는 이 설정의 영향을 받지 않는다고 덧붙이므로, 앞 절에서 본 `prometheus_replica` 는 `honor_labels` 값과 무관하게 페더레이션 결과에 남습니다.

**Operator 문서는 `__tmp_hash` 를 특별한 이름으로 취급합니다.** 본문은 "`__tmp_hash` 라는 이름에 특별한 것은 없다" 고 적는데, 순정 Prometheus 설정에서는 맞는 말입니다. 그러나 Operator 환경에서는 이 이름이 약속된 이름입니다.
...
**샤드를 줄여도 데이터는 옮겨 가지 않습니다.** 본문은 뒤처리로 `shards: 0` 을 권하지만 그 뜻을 짚지 않습니다. Operator 문서가 명시하는 재배치 규칙은 이렇습니다.

- **줄일 때** — 남은 인스턴스로 데이터가 재배치되지 않아 **수동으로 옮겨야** 합니다.
- **늘릴 때** — 역시 재배치는 없고, 기존 데이터는 원래 인스턴스에서 계속 조회됩니다.[^promop-shards]

그래서 같은 문서가 전역 조회 수단을 함께 안내합니다. Thanos sidecar 와 querier 를 쓰거나, remote write 로 중앙에 보내라는 것입니다. 본문 §4 의 HA 해법 표가 고른 답과 같은 방향입니다.

실습 뒤처리라면 무해합니다. 그러나 운영 중인 클러스터에서 샤드 수를 바꾸는 일은 데이터 이동 계획을 동반해야 합니다.

**`shards` 의 실험적 표시는 지금 문서에 없습니다.** 본문은 "집필 시점에 실험적" 이라 적었습니다. 그러나 현재 Operator API 레퍼런스의 `shards` 항목에는 그 표시가 붙어 있지 않고, `spec.replicas × spec.shards` 가 생성되는 파드 총수라는 설명만 있습니다. 본문의 2×2=4 계산과 일치합니다.

다만 "실험적 표시가 사라졌다" 는 관찰이지 안정성 보증은 아닙니다. ([Prometheus Operator API](https://github.com/prometheus-operator/prometheus-operator/blob/main/Documentation/api-reference/api.md))

**블록 최대 31일은 3장 노트와 이어집니다.** [03-03. TSDB 저장 구조](03-03.TSDB%20저장%20구조.md)의 심화에 적어 둔 대로 컴팩션된 블록의 상한은 *보존 기간의 10%* 와 *31일* 중 작은 쪽입니다. 이 장이 말하는 "최대 31일" 은 그 상한의 절대 한계에 해당합니다. 보존 기간이 짧으면 31일에 닿기 전에 10% 규칙이 먼저 걸립니다.
...
[^promop-shards]: Prometheus Operator — [API Reference, `PrometheusSpec.shards`](https://prometheus-operator.dev/docs/api-reference/api/). 같은 항목이 본문의 네 주장을 차례로 규정합니다(2026-08-06 원문 대조).
    ...
    - 파드 총수 — `spec.replicas multiplied by spec.shards is the total number of Pods being created`
    - 재배치 없음 — `scaling down shards will not reshard data onto the remaining instances, it must be manually moved`
    - 전역 조회 — `To query globally, use either Thanos sidecar + querier ... Remote-write to send metrics to a central location`
    - 기본 대상 라벨 — `The __address__ target's metadata label for PodMonitor, ServiceMonitor and ScrapeConfig resources` 와 `The __param_target__ label for Probe resources`
