...
해시 알고리즘은 Prometheus 소스가 `crypto/md5` 로 확정합니다. 정확히는 MD5 해시의 **마지막 8바이트**에 모듈로를 취합니다 ([`model/relabel/relabel.go`](https://github.com/prometheus/prometheus/blob/main/model/relabel/relabel.go)).
...
- `true` — 스크레이프한 값을 남기고 서버 쪽 라벨을 버립니다.
- `false` — 스크레이프한 쪽 라벨을 `exported_instance`·`exported_job` 처럼 **`exported_` 접두사를 붙여 개명한 뒤** 서버 쪽 라벨을 답니다.

문서는 페더레이션과 Pushgateway 스크레이프를 이 설정을 켜야 하는 대표 사례로 꼽습니다. ([Configuration — honor_labels](https://prometheus.io/docs/prometheus/latest/configuration/configuration/#scrape_config))
...
|------|------------------------|
| `__tmp_hash` | 자기만의 샤딩 구현을 정의하려면 타깃 탐색 중 relabeling 으로 이 라벨을 설정하라고 API 레퍼런스가 안내합니다 |
...
- **늘릴 때** — 역시 재배치는 없고, 기존 데이터는 원래 인스턴스에서 계속 조회됩니다.
...
| [03-01. 데이터 모델과 TSDB](03-01.데이터%20모델과%20TSDB.md) | 블록 최대 31일·독립 DB 라는 이 절의 장기 보존 한계가 거기의 컴팩션·보존과 이어집니다 |
