...
> 저자의 권고를 상류가 그대로 따랐습니다. **query pushdown 은 v0.34.0 에서 아예 제거됐습니다** — 릴리스 노트가 든 이유가 "쿼리 경로를 단순화하려고" 입니다. 실험 기능이었고 얻는 것에 비해 복잡도가 컸다는 판정입니다. 지금 판에서는 이 손잡이를 만질 수 없으니 쿼리 샤딩만 남습니다.
...
Receive 로 들어온 메트릭에는 테넌트를 밝히는 라벨이 자동으로 붙습니다. 이름은 `--receive.tenant-label-name` 이 정하고 기본값이 `tenant_id` 입니다. 복제본 번호는 `--receive.replica-header`(기본 `THANOS-REPLICA`) 로 받은 값이 라벨이 됩니다.

> 책은 여기에 더해 `receive` 라벨이 자동으로 붙어 `up{receive="true"}` 로 거를 수 있다고 적습니다. **공식 문서에서 근거를 찾지 못했습니다.** Receive 가 자동으로 붙이는 것은 위 둘이고, 그 밖의 라벨은 `--label` 로 직접 선언하는 외부 라벨입니다. "Receive 로 들어온 것만 골라 보고 싶다" 면 `--label` 에 원하는 표시를 직접 넣는 편이 확실합니다.
...
> **여기까지는 v0.32.5 기준이고, 그 뒤 RF=2 가 특례로 갈라졌습니다.** 현재 `writeQuorum()` 에는 분기가 하나 붙어 있습니다.
>
> ```go
...
> ```
>
> RF=2 에서 `floor(2/2)+1 = 2` 는 곧 **모든 쓰기가 항상 성공해야 한다**는 뜻이라 복제의 의미가 없어집니다. 그래서 쿼럼을 1 로 낮췄습니다. 결과적으로 지금은 RF=2 에서 소스가 **1** 이고, 공교롭게도 책의 식과 *일치* 합니다.
>
> 그러니 위 표의 RF=2 행은 v0.32.5 에서만 유효하고, "짝수에서는 언제나 다르다" 는 현재 판에서 **RF 4 이상의 짝수** 에 한정됩니다. 반증 자체가 버전에 매여 있었던 셈입니다.

![복제 계수 1~6 에서 소스의 floor(RF/2)+1 과 책의 (RF+1)/2 를 나란히 비교 — 홀수는 일치하고 짝수는 소스가 노드를 하나 더 요구한다](./_assets/10-02-write-quorum.svg)

실무에서 복제 계수는 홀수로 잡는 것이 관례이므로 이 오차가 실제로 문제를 일으킬 일은 드뭅니다. 그래도 4 이상의 짝수를 쓴다면 책의 식보다 노드 하나가 더 필요하다는 것을 알아야 합니다. 복제 계수 4 는 3개 성공을 요구하지 2개가 아닙니다.
...
차이의 원인은 **개수 정보가 소실** 되는 데 있습니다. 20 이라는 값만 받아서는 그것이 세 개의 평균인지 하나의 값인지 알 수 없습니다. `sum` 은 이 문제가 없습니다. 합의 합은 언제나 참된 합이기 때문입니다. `min`·`max` 도 최솟값의 최솟값이 참된 최솟값이라 마찬가지입니다.

`count` 는 분해되기는 하되 **위아래 연산자가 다릅니다.** 아래에서 `count` 를 내고 위에서 그것을 `sum` 해야 합니다 — 위에서도 `count` 를 쓰면 "노드가 몇 개인가" 를 세는 꼴이 되어 틀립니다.
...
> Query 는 SRV 레코드로 붙여 두면 Sidecar 가 늘고 줄어도 설정을 고치지 않아도 됩니다. Query Frontend 는 Query 복제본을 함께 늘려야 분할의 효과가 납니다.
...
복제 계수는 홀수로 둡니다. 4 이상의 짝수는 쿼럼이 하나 더 필요해 내구성 대비 가용성이 나빠집니다(RF=2 는 현재 판에서 특례로 쿼럼 1 입니다). 그리고 **Receive 에는 중복 제거가 없으므로**, HA Prometheus 두 대가 같은 데이터를 보내면 그대로 두 벌 쌓입니다. Compactor 의 수직 압축을 켜거나, 애초에 Sidecar 로 올리는 편을 택합니다.
