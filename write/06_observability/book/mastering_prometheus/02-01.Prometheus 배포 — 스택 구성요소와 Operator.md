...
방향만 미리 한 조각 남겨 둡니다. **병목은 기계가 아니라 사람입니다.** 알림 50건을 보내는 일 자체는 기계에 부담이 아니지만, 그것을 읽어야 하는 당직자에게는 부담입니다.

Alertmanager 의 네 기능은 그 사람 쪽 병목을 줄이는 장치입니다. 그중 **그룹핑이 기본이고 억제는 특수**합니다. 억제는 원인과 결과의 계층을 미리 알아야 설정할 수 있기 때문입니다. 그 비용을 치를 값어치는 아래 [§심화 학습 — Alertmanager 를 왜 분리했습니까](#심화-학습) 에서 따로 봅니다.
...
```text
┌─ alertname=LabAlwaysFiring ───────┐   ← 덩어리(상자) 1개
...
**그런데 파일에 쓰는 것도 결국 push 아닌가요.** 배치 입장에서 보면 둘 다 "어딘가에 값을 쓰는" 일이고, 마지막 구간은 둘 다 Prometheus 가 긁어 갑니다.
...
세 축을 한 장에 겹쳐 두면 이렇습니다. 같은 조작 하나 — **컨테이너 재시작** — 에 두 경로가 정반대로 반응하는 것이 축을 가장 선명하게 가릅니다.
...
> ⚠️ **재시작 실험에는 측정 함정이 있습니다.** 재시작 직후 곧바로 조회하면 컨테이너가 아직 뜨는 중이라 빈 응답이 옵니다. 이것을 "값이 사라졌다" 로 읽으면 오판입니다. `RestartCount`·`StartedAt` 으로 재시작이 실제로 일어났는지 확인하고, **HTTP 200 이 돌아온 뒤에** 조회해야 합니다. 실측에서 한 번 이 함정에 빠졌다가 `StartedAt` 대조로 잡았습니다.
...
**첫째, `path` 를 빼먹으면 안 됩니다.** `path` 를 생략하면 Prometheus 는 기본값 `/metrics` 를 긁습니다. Spring Boot 의 메트릭은 `/actuator/prometheus` 에 있으므로 스크레이프가 404 로 실패합니다. 위 그림의 셋째 층이 이것이고, `prometheus.yml` 을 직접 쓸 때의 `metrics_path` 도 같은 성질입니다.
