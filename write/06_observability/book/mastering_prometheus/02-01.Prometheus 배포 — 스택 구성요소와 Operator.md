...
여기서 "넷"이 §1 에 이어 두 번째로 나오는데, **두 넷은 다른 층입니다.** §1 의 넷은 따로 뜨는 프로세스이고, 여기 넷은 그중 Prometheus **한 프로세스 안**의 부품입니다.

```text
[스택 — 프로세스 넷]          [Prometheus 내부 — 부품 넷]
├─ Prometheus  ──────────────▶ ├─ TSDB            (중심)
├─ Alertmanager  (별개 프로세스) ├─ 스크레이프 매니저  → 넣습니다
├─ exporter      (별개 프로세스) ├─ 룰 매니저        → 읽습니다
└─ Grafana       (별개 프로세스) └─ 웹 UI·REST API  → 꺼냅니다
```
