...
remote read 를 쓰면 로컬 Prometheus 에 저장된 메트릭과 **나란히 질의되는** 외부 저장 시스템을 붙일 수 있습니다. 다른 Prometheus 인스턴스일 수도, Mimir 나 InfluxDB 같은 외부 메트릭 저장소일 수도, 심지어 메트릭이 아닌 데이터 소스를 위한 커스텀 어댑터일 수도 있습니다. remote read 요청을 받아 SQL 데이터베이스에 질의하고 결과를 Prometheus 메트릭 형식으로 돌려주는 어댑터를 짜는 것도 가능합니다.

유연하니 재미난 일을 벌일 수 있습니다. 그런데 실무에서 대부분의 사람은 외부 저장소를 Prometheus 에 붙여 *Prometheus 에서* 질의하지 않습니다. 반대로 Prometheus 를 다른 저장 시스템에 붙여 *그 저장 시스템을* 질의합니다. 이유의 일부는 설정한 remote read 엔드포인트로 쿼리가 언제 날아갈지 통제하기 어렵다는 데 있습니다. 붙여 둔 시스템에 문제가 생기면 내 쿼리가, 특히 **알림 쿼리가** 덩달아 실패하기 시작합니다.
...
### 5. remote write 튜닝 — ① write relabel configs
...
설정된 remote write 목적지마다 Prometheus 는 메모리에 새 큐를 하나 만듭니다. 각 큐는 **동적인 수의 샤드** 로 나뉩니다. 큐당 샤드 수는 목적지로 보내야 할 데이터의 유입 속도를 근거로 Prometheus 가 백그라운드에서 끊임없이 다시 계산합니다. 우리가 통제할 수 있는 것은 목적지별 샤드 수의 최솟값과 최댓값뿐입니다.
...
`capacity` 는 샤드 하나가 담을 수 있는 샘플 수이고 기본 **2500** 입니다. 초당 2,500 샘플 이상을 수집하고 있다면(이 책을 읽고 있다면 아마 그럴 겁니다) 초기 샤드 **1개** 로는 어림도 없습니다.
...
`capacity` 와 함께 각 샤드는 `max_samples_per_send` 의 지배를 받습니다. 샤드에서 목적지로 배치를 언제 보낼지 정하는 값이고 기본 **500** 입니다. 샤드에 500 샘플이 차면 전송이 촉발됩니다. 유입 속도가 샤드를 그만큼 빨리 채우지 못하면 `batch_send_deadline` 이 작동해 정해진 주기마다 배치를 보냅니다. 기본 **5초** 입니다. 볼륨이 낮은 Prometheus 가 아니라면 이 데드라인에 걸리지 않는 것이 이상적입니다.
...
처리량이 큰 시스템이라면 `max_samples_per_send` 와 `capacity` 를 함께 올려야 합니다. `max_samples_per_send` 를 올릴 때는 **capacity 가 언제나 그 세 배 이상** 이도록 capacity 도 함께 올립니다. 다만 두 값을 올리면 메모리에 들고 있는 샘플이 늘어 Prometheus 의 메모리 사용량도 함께 오릅니다. 그래서 메모리 부족을 피하려면 `max_shards` 를 오히려 낮추는 것이 권장됩니다. 여유가 넉넉하다면 그대로 둬도 좋습니다.
...
초당 30,000 샘플이고 현재 기본 `capacity: 10000` 이라면 `min_shards: 3` 에서 출발합니다. 책의 옛 기본값 2500 으로 계산하면 12가 나오니 결과가 네 배 어긋납니다.
