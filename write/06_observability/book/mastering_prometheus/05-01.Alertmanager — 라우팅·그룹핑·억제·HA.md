...
모든 리시버가 공통으로 갖는 설정은 `send_resolved` 와 `http_config` 둘입니다. `http_config` 는 손댈 일이 거의 없습니다. `send_resolved` 는 기본적으로 켜져 있어 알림이 발화할 때와 해소될 때 모두 알립니다.[^am-send-resolved] 리눅스 OOM 킬러가 동작했다는 알림처럼 "일이 일어났다" 만 알면 되고 "더는 안 일어난다" 는 알 필요 없는 경우 `send_resolved: false` 로 무의미한 해소 알림을 막습니다.
...
`equal` 절이 억제의 **겨냥 범위**를 결정합니다. Alertmanager 는 알림 사이의 인과를 이해하지 못하고 라벨 값만 비교하므로, 무엇을 `equal` 에 넣느냐가 곧 "어떤 상황을 억제할 것인가" 입니다. 위 예제처럼 `alertname` 을 넣으면 **이름이 같고 severity 만 다른** 알림만 걸립니다. 그래서 이 규칙은 다단계 임계값을 겨냥할 뿐, 이 절 도입부의 "데이터센터가 죽었으니 그 안 서버 알림은 필요 없다" 는 잡지 못합니다. `DatacenterDown` 과 `ServerUnreachable` 은 이름이 다르기 때문입니다. 후자를 잡으려면 `alertname` 을 빼고 `datacenter` 같은 공통 라벨로 묶어야 합니다.

kube-prometheus 가 배포하는 기본 억제 규칙이 정확히 전자입니다(`equal: [namespace, alertname]`). 실측해 보면 같은 네임스페이스에 critical 과 warning 이 함께 발화 중이어도 `alertname` 이 서로 다르면 하나도 억제되지 않습니다. 기본값을 깔았다고 해서 상하위 장애가 자동으로 정리되지는 않는다는 뜻입니다.

Alertmanager 에는 알림이 자기 자신을 억제하지 못하게 하는 안전장치가 있습니다. 한 알림이 억제 규칙의 target 쪽과 source 쪽에 **모두** 걸리면 억제가 적용되지 않습니다.[^am-inhibit] 애초에 그런 일이 생기지 않도록 매처를 고르는 편이 낫습니다.
...
`check-config` 는 하이픈으로 이어진 **한 단어**입니다.[^am-check-config] `check config` 처럼 띄우면 `expected command but got "check"` 로 떨어집니다. `config routes` 는 반대로 두 단어이므로 둘을 섞기 쉽습니다.

통과하면 파싱된 항목을 세어 보여 줍니다. **내가 쓴 개수와 세어진 개수가 다르면 그 자체가 오타 신호**입니다. 억제 규칙을 셋 썼는데 `0 inhibit rules` 가 나오면 들여쓰기가 어긋나 블록이 통째로 무시된 것입니다.
...
커스텀 템플릿을 지정하지 않으면 Alertmanager 는 내장 기본 템플릿만 씁니다. 가장 많이 쓰이는 내장 템플릿은 `__subject` 이며 Slack 메시지, PagerDuty 인시던트 제목, Discord 메시지 제목의 기본값입니다. 한 줄에 너무 많은 일을 해서 읽기 어렵기로 유명한데, 하는 일은 넷입니다.

1. `define` 으로 이름 붙은 템플릿을 열고 대응하는 `{{ end }}` 로 닫습니다.
2. 발화 중인 알림 개수를 대괄호로 감싸 `[FIRING:4]` 처럼 그립니다.
3. 그룹 라벨 값들을 공백으로 이어 붙여 `[FIRING:4] production api us-east-1` 같은 모양을 만듭니다.
4. 마지막으로 그룹의 모든 알림이 공유하지만 `group_by` 에 명시되지 않은 공통 라벨을 덧붙입니다.
...
> Alertmanager 는 가십으로 "이미 보냈는지" 만 공유하고 "어떤 알림을 받았는지" 는 공유하지 않습니다. 그래서 Prometheus 는 모든 인스턴스로 알림을 보내야 하고, 앞에 로드밸런서를 두면 안 됩니다.

Prometheus 자신과 달리 Alertmanager 는 고가용성 기능을 내장하고 있어 클러스터로 뜹니다. 

- 가십 프로토콜로 정보를 공유하며 인스턴스를 띄울 때 반복 가능한 `--cluster.peer` 플래그로 피어를 지정합니다. 
- 클러스터링을 끄고 싶으면 `--cluster.listen-address` 를 빈 문자열로 두면 되지만 운영 환경에는 권장하지 않습니다.

**가십(gossip)** 은 소문이 퍼지는 방식을 그대로 옮긴 통신 방법입니다. 중앙 조정자가 없습니다. 각 노드가 아무나 몇 명을 골라 자기가 아는 것을 알려 주면, 그걸 들은 노드가 또 다른 몇 명에게 전달합니다. 몇 다리만 건너면 전체가 알게 됩니다.

전체에게 직접 방송하는 방식보다는 느립니다. 대신 노드 몇이 죽거나 네트워크가 잠깐 끊겨도 다른 경로로 정보가 돌아가므로 튼튼하고, 한 노드가 보내는 메시지 수도 적습니다. 방금 A 가 안 사실을 C 는 아직 모를 수 있어 즉시 일관성은 없지만 시간이 지나면 결국 같아집니다.

여기서 성질 하나를 기억해 둘 필요가 있습니다. 가십은 **내가 아는 것을 퍼뜨리는** 방식이지 **네가 뭘 아는지 물어보는** 방식이 아닙니다. 상대가 먼저 말해 주지 않은 것은 알 길이 없습니다. 뒤에 나올 로드밸런서 문제가 정확히 이 성질에서 나옵니다.
...
주의할 점은 인스턴스들이 **어떤 알림을 받았는지는 가십하지 않는다**는 사실입니다. 그래서 Prometheus 는 클러스터의 **모든** Alertmanager 인스턴스로 알림을 보내도록 설정해야 하고 앞에 로드밸런서를 두어 일부 인스턴스가 알림을 못 받게 만들면 안 됩니다.[^am-ha-no-lb]

![가십이 공유하는 것과 로드밸런서가 깨뜨리는 것](./_assets/05-01.ha-gossip-vs-lb.svg)

그림의 두 배치를 견주면 로드밸런서가 무엇을 깨뜨리는지 드러납니다. 위쪽은 세 대가 **모두 같은 알림을 들고** 있으므로 어느 하나가 죽어도 나머지가 대신 보냅니다. 가십은 그중 누가 이미 보냈는지만 알려 주어 사람에게 한 통만 가게 합니다. 아래쪽은 로드밸런서가 알림을 한 대에게만 넘기므로 나머지 둘은 그 알림의 존재조차 모릅니다.

**이 구멍을 가십이 메워 주지 않는 이유가 핵심입니다.** 가십은 "보냈는지" 를 나눌 뿐 "받았는지" 는 나누지 않습니다. B 와 C 는 A 가 무엇을 들고 있는지 물어볼 방법이 없으므로, A 가 죽는 순간 그 알림은 아무도 보내지 못한 채 사라집니다. 고가용성을 노리고 세 대를 띄웠는데 실제로는 한 대짜리와 다를 바 없어지고, 게다가 그 사실이 장애가 나기 전까지 드러나지 않습니다.

정리하면 두 층이 각각 다른 문제를 맡습니다. **알림을 모두에게 중복해서 보내는 것**이 가용성을 만들고, **가십이 발송 사실을 공유하는 것**이 그 중복을 사람 눈앞에서 걷어 냅니다. 로드밸런서는 앞 단계를 없애면서 뒤 단계로는 복구할 수 없는 손실을 만듭니다.

클러스터 크기는 저자 경험상 둘이나 셋을 넘긴 적이 없습니다. etcd 나 Vault, 데이터베이스와 달리 Alertmanager 는 **합의 알고리듬을 쓰지 않으므로** 정족수를 맞출 필요가 없습니다. 합의는 "과반이 동의해야 결정한다" 이므로 셋 중 둘은 살아 있어야 하지만, 가십은 각자 알아서 판단하고 정보만 나누므로 한 대만 살아 있어도 그 한 대가 알림을 보냅니다. 하나 이상만 살아 있으면 계속 동작하니 `n-1`, `n-2` 같은 단순한 계산이면 됩니다. 그래서 개수보다 **가용 영역(AZ) 분산**이 중요합니다. 둘이면 서로 다른 두 AZ 에, 셋이면 서로 다른 세 AZ 에 둡니다.
