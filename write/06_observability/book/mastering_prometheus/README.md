...
| [03-01 데이터 모델 — 메트릭·시계열·샘플](./03-01.데이터%20모델.md) | 3장 앞부분 — 메트릭·시계열·샘플 3층 · TSM 과의 차이(무엇이 시계열을 정하는가) · `HELP`/`TYPE` 은 주석이 아니라 메타데이터 · 메트릭 타입 4종 · 분위수와 꼬리 지연 · 히스토그램 대 서머리(합산 가능 여부) |
