...
| [03-01 데이터 모델과 TSDB](./03-01.데이터%20모델과%20TSDB.md) | 3장 앞부분 — 메트릭·시계열·샘플 3층 · 메트릭 타입 4종 · WAL→head chunk→영속 블록 경로 · 체크포인트와 재생 · 블록 불변성과 툼스톤 · 델타-오브-델타·XOR 압축 · 역색인(포스팅·시리즈) · 컴팩션과 보존 |
| [03-02 PromQL 기초](./03-02.PromQL%20기초.md) | 3장 뒷부분 — 순간 vs 범위 벡터(대괄호가 경계) · `/query` vs `/query_range` · offset·서브쿼리 · staleness 와 lookback delta · `by`/`without` · `group_left` 와 `_info` 조인 · `and`/`or`/`unless` · `rate`·`irate`·`increase`·`changes`·`delta` |
| [04-01 서비스 디스커버리와 relabeling](./04-01.서비스%20디스커버리와%20relabeling.md) | 4장 전체 — `<type>_sd_configs` 규칙 · Operator 의 두 경로(`additionalScrapeConfigs` vs 자동 생성 `kubernetes_sd_configs`) · 표준 relabeling(스크레이프 전) vs 메트릭 relabeling(후) · `__meta_` 는 relabeling 중에만 · `jobLabel` 이 `job` 을 덮어써 만든 2,234 시계열 · `/service-discovery` 디버깅 · `Discoverer` 인터페이스와 Linode 이벤트 폴링 · 모라토리엄 해제(2019)와 `http_sd`(2021) · `instance` 와 `__address__` 분리 |
