...
아래는 이 절 전체를 하나의 축으로 이은 지도입니다. 맨 위의 구분 하나가 셀렉터·API·함수 세 층에서 그대로 되풀이되고, 그 위에 결합 도구 셋이 얹힙니다. 각 상자 옆의 절 번호가 본문에서 그것을 다루는 곳입니다.

![PromQL 기초 전체 지도](./_assets/03-04.chapter-overview.svg)
...
다만 자동으로 살아남는다는 성질에는 대가가 따릅니다. 새로 붙은 라벨이 파드 이름처럼 **값이 계속 바뀌는 것**이라면, 질의를 한 글자도 안 고쳤는데 결과 시계열이 몇 벌에서 수백 벌로 불어납니다. 터지는 곳은 셋입니다.

- **대시보드** — 범례가 수백 줄로 늘어납니다.
- **알림** — 발화 개수가 시계열 수만큼 늘어납니다.
- **recording rule** — 그 카디널리티가 TSDB 에 그대로 쌓입니다.

그래서 기본값은 자리마다 갈립니다.

| 자리 | 기본값 | 이유 |
|---|---|---|
| 탐색적 질의 | `without` | 라벨이 남을수록 파고들 여지가 큽니다 |
| recording rule · 알림 룰 · 상시 대시보드 | `by` | 결과 개수가 곧 비용이라 명시적으로 닫습니다 |
...
```bash
# node-exporter + 노드의 kubelet_version, os_image 조인 
