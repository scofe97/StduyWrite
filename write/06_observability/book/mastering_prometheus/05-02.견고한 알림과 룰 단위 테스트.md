...
status: draft
source: "Mastering Prometheus (Packt, ISBN 9781805125662) — Chapter 5: Effective Alerting with Prometheus (Making robust alerts · Unit-testing alerting rules 절)"
related:
  - README.md
...
아래는 이 절 전체를 알림 룰 하나가 견고해지는 과정으로 이은 지도입니다. 네 단계 중 앞의 셋은 오탐과 소음을 줄이는 손잡이이고, 각 단계 옆의 절 번호가 본문에서 그 손잡이를 다루는 곳입니다.

![견고한 알림 전체 지도](./_assets/05-02.chapter-overview.svg)



## 1. 논리·집합 연산자로 오탐 줄이기

> 조건 하나가 아니라 여러 조건을 함께 봅니다. `and`·`or`·`unless` 로 "메모리도 높고 페이지 폴트도 높을 때" 만 울리게 좁히면 오탐이 줄어듭니다.

견고한 알림을 만드는 능력은 Prometheus 가 Nagios 같은 전통적 체크 기반 모니터링과 갈리는 지점입니다. 여러 조건을 함께 볼 수 있기 때문입니다. 

- 메모리 사용률이 높다는 사실만으로 알리는 대신, 메모리 사용률이 높으면서 **메이저 페이지 폴트 비율도 높을 때** 알리는 편이 메모리 압박을 훨씬 잘 가리킵니다. 
- 핵심은 오탐을 줄여 **실제로 눈에 보이는 영향이 있을 때만** 울리게 만드는 데 있습니다. 증상에 알릴 것이냐 원인에 알릴 것이냐는 Rob Ewaschuk 의 *My Philosophy on Alerting* 이 깊이 다룹니다.
...
> **`_info` 메트릭 패턴** — 값이 항상 `1` 인 게이지에 `_info` 접미사를 붙이고 정보성 라벨을 잔뜩 매다는 관례입니다. 
>
> 라벨 값이 바뀔 때 모든 시계열이 갈라지는 대신 이 메트릭 하나만 바뀌므로 churn 이 줄어듭니다. 2장에서 kube-prometheus 와 함께 배포한 kube-state-metrics 가 클러스터 노드 정보를 담은 `kube_node_info` 를 내놓습니다.



## 2. `for` 기간이 알림의 품질을 가른다

> 알림을 조용하게 만드는 가장 단순한 손잡이입니다. warning 은 15분보다 짧지 않게, critical 은 15분보다 길지 않게 두되, `for` 는 룰 평가 주기보다 길어야 의미가 있습니다.
...
여기에 놓치기 쉬운 제약이 하나 붙습니다. `for` 기간은 룰의 **평가 주기보다 길어야** 의미가 있습니다.[^prom-for] 알림 룰을 5분마다 평가하도록 설정해 두고 `for: 3m` 을 걸면 그 3분은 아무 일도 하지 않습니다. `for` 가 평가 주기보다 짧으면 조건이 참이 된 뒤 **다음 평가에서 곧바로** 발화하므로, `10s` 든 `25s` 든 결과가 같아집니다.

### 알림이 지나는 세 상태

`for` 가 벌리는 것은 두 상태 사이의 간격입니다. Prometheus UI 의 Alerts 화면이 이 셋을 색으로 구분합니다.

| 상태 | 뜻 | UI |
|---|---|---|
| inactive | 조건이 거짓입니다 | 초록 |
| **pending** | 조건은 참인데 `for` 를 아직 못 채웠습니다[^prom-for] | 노랑 |
| firing | `for` 를 채워 Alertmanager 로 넘어갑니다 | 빨강 |

`for` 를 걸지 않으면 pending 을 건너뛰고 첫 평가에서 곧바로 firing 이 됩니다.[^prom-for] 앞 절에서 "스크레이프 실패가 `for` 를 지운다" 고 할 때 사라지는 것이 바로 이 pending 구간입니다.

`inactive` 는 공식 문서의 용어가 아니라 UI 와 API 가 쓰는 표현입니다. 문서는 조건을 만족한 알림을 active 로 부르고 그중 아직 발화하지 않은 것을 pending 이라 적습니다.

### 반대편 손잡이 — `keep_firing_for`

`for` 가 **발화 전** 대기라면, `keep_firing_for` 는 조건이 거짓이 된 **뒤에도** 얼마나 더 firing 을 유지할지 정합니다.[^prom-keep-firing] 조건이 참과 거짓을 오가는 알림에서 해소와 발화가 번갈아 쏟아지는 것을 막습니다.

```yaml
- alert: EtcdMembersDown
  expr: ...
  for: 20m                # 발화까지 20분 기다림
  keep_firing_for: 15m    # 해소된 뒤에도 15분 더 유지
```

- kube-prometheus 가 배포하는 기본 룰 상당수가 이 절을 함께 답니다. 
- 이 책은 다루지 않지만 `for` 만으로는 flapping 을 막지 못한다는 점에서 짝으로 알아 둘 값어치가 있습니다.



## 3. 스크레이프 실패가 `for` 를 지운다

> 긴 `for` 가 새로 만드는 함정입니다. 결측에 stale 마커가 찍혀 5분 lookback 이 막히고, 빈 결과가 곧 거짓이 되어 pending 이 풀립니다. `last_over_time` 이 그 초기화를 막습니다.
...
> 알림도 코드이므로 테스트합니다. `promtool test rules` 로 시계열을 지어 넣고 몇 분에 무엇이 어떤 라벨로 발화해야 하는지 단언하면, 룰을 손본 뒤에도 동작이 유지되는지 확인됩니다.
...
> 원문의 계산이 한 칸씩 밀려 있는 지점을 공식 문서와 실측으로 바로잡습니다. 뿌리는 하나입니다 — `input_series` 의 첫 샘플이 0분에 놓인다는 사실입니다.
...
| 발화와 해소가 번갈아 쏟아진다 | `keep_firing_for` | 조건이 거짓이 된 뒤에도 firing 을 유지해 flapping 을 덮습니다 |
...
> 이 절의 `and`·`unless`·`for`·`promtool` 이 Spring 환경에서 어디로 떨어지는지입니다. 오류율 조이기·배포 직후 오탐·평가 주기·CI 테스트 넷을 짚습니다.
...
- [ ] inactive·pending·firing 셋을 구분하고, `for` 가 어느 구간을 벌리는지 말할 수 있는가?
- [ ] `keep_firing_for` 가 `for` 의 반대편에서 무엇을 막는지 아는가?
...
| [00-01. 용어집](00-01.용어집.md) | 이 책 전반의 용어 정의. 알림 상태 셋과 확장 표기법은 이 절 §2·§4 가 직접 다룹니다 |
...
[^prom-keep-firing]: Prometheus — Alerting rules. `keep this alert firing for the specified duration after the firing condition was last met` 이며, 목적으로 `flapping alerts, false resolutions due to lack of data` 를 듭니다.
<https://prometheus.io/docs/prometheus/latest/configuration/alerting_rules/#defining-alerting-rules>
