...
- `axn` 은 `a+0xn` 의 축약이며 `a` 를 **`n+1` 번** 반복합니다.[^prom-expanding]
...
`for` 기간을 늘립니다. 대부분의 경우 임계값을 바꾸는 것보다 효과가 큽니다. 다만 `for` 는 룰 평가 주기보다 길어야 의미가 있습니다.[^prom-for] 5분마다 평가하는 룰에 `for: 3m` 을 걸면 아무 일도 하지 않습니다.
