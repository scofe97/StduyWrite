...
스크레이프 실패
  → stale 마커가 찍힘
  → lookback 이 막혀 시계열이 조회되지 않음
  → 비교 연산의 결과가 비어 있음
  → Prometheus 가 "조건 거짓" 으로 판정
  → pending 해제 · 쌓아 온 시간 소멸
```
...
**`last_over_time` 이 기본값이어야 하는 이유를 한 번 더 짚습니다.** 본문의 예제 룰은 정작 `max_over_time` 을 씁니다. 발화 시점을 14분으로 만들어 설명하기 좋기 때문인데, 실전에서 스크레이프 실패로부터 `for` 를 지키는 것이 목적이라면 `last_over_time` 이 지연을 만들지 않습니다. `max_over_time` 때문에 발화가 **4분** 늦어지는 구간이 바로 그 대가입니다(위 실측 표).
...
- [ ] 비교 연산자가 값을 돌려주지 않고 시계열을 걸러낸다는 사실과, 그것이 참·거짓 판정과 어떻게 이어지는지 아는가?
