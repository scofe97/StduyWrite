# 비즈니스 로직 활용과 Webhook/API

---

> **"Jenkins를 단순 CI/CD 도구가 아닌 비즈니스 자동화 엔진으로 사용할 수 있는가?**
>
> 많은 조직에서 Jenkins는 이미 수년간 운영되어 온 인프라 자산이다. UI, 스케줄링, 로깅, 알림, 플러그인 생태계가 이미 갖춰져 있으니 "이 위에 비즈니스 로직도 올리면 되지 않겠는가?"라는 유혹이 생긴다. 

## 1. Jenkins를 비즈니스 로직에 사용하는 사례

**데이터 파이프라인 실행 (ETL Job 스케줄링)**

데이터 엔지니어링 팀이 매일 새벽 3시에 데이터 웨어하우스로 데이터를 적재하는 ETL 작업을 Jenkins 파이프라인으로 실행하는 경우가 있다. 

- Jenkins의 `cron` 트리거(`triggers { cron('0 3 * * *') }`)로 스케줄을 설정하고, 각 스테이지에서 Extract, Transform, Load 단계를 순차 실행한다. 
- 이 패턴이 채택되는 이유는 별도의 Airflow 클러스터를 구축하고 운영할 인력이 부족한 소규모 팀에서 이미 존재하는 Jenkins를 활용하는 것이 현실적이기 때문이다.
...
> Jenkins의 아키텍처는 "코드 변경 → 빌드 → 테스트 → 배포"라는 CI/CD 워크플로우에 최적화되어 있다. 
>
> 비즈니스 로직을 올리는 것은 가능하지만, 규모가 커질수록 Jenkins의 설계 한계에 부딪히게 된다. 
