...
**ISR**은 리더와 충분히 동기화된 팔로워들의 집합이다. "충분히"의 기준은 `replica.lag.time.max.ms` 설정으로 결정된다. 팔로워가 이 시간 내에 리더의 최신 메시지를 복제하면 ISR에 포함되고, 그렇지 않으면 제외된다.

ISR의 목적은 **쓰기 성능과 데이터 안전성의 균형**이다. 모든 복제본을 기다리면 느린 노드 하나가 전체 쓰기를 지연시킨다. ISR만 기다리면 "현재 동기화된" 노드만 고려하므로 빠른 쓰기와 안전성을 동시에 달성할 수 있다.
...
`min.insync.replicas`를 RF보다 1 작게 설정하는 것이 일반적이다. RF=3이면 `min.insync.replicas=2`로, 1개 노드 장애를 허용하면서 최소 2개 노드에 데이터가 있음을 보장한다.
...
여기까지의 디폴트 선택(파티션 수, RF, acks, ISR)은 단일 노드 실험부터 매우 큰 분산 클러스터까지 같은 코드 형태로 확장된다는 점이 분산 커밋 로그 추상의 본질이다. 감각을 잡기 위한 사례로 ING(글로벌 은행)의 운영 통계가 자주 인용된다.
...
- [04-02.리더 선출](04-02.%EB%A6%AC%EB%8D%94%20%EC%84%A0%EC%B6%9C.md) — Quorum 합의와 ZK/KRaft 비교
- [04-03.Consumer Group](04-03.Consumer%20Group.md) — 파티션-Consumer 매핑이 만드는 병렬성
- [04-04.리밸런스 프로토콜](04-04.%EB%A6%AC%EB%B0%B8%EB%9F%B0%EC%8A%A4%20%ED%94%84%EB%A1%9C%ED%86%A0%EC%BD%9C.md) — Consumer 가입/탈퇴 시 파티션 재분배
- [04-05.Redpanda 아키텍처](04-05.Redpanda%20%EC%95%84%ED%82%A4%ED%85%8D%EC%B2%98.md) — 같은 추상의 다른 구현
- [04-08.Exactly-once 의미론과 Consumer Idempotency](04-08.Exactly-once%20%EC%9D%98%EB%AF%B8%EB%A1%A0%EA%B3%BC%20Consumer%20Idempotency.md) — Producer 멱등성과 트랜잭션
