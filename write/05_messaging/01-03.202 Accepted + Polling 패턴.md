...
> [01-02.EDA 기반 요청응답 통합](./01-02.EDA%20기반%20요청응답%20통합.md)에서 다룬 4가지 브릿지 패턴 중,
...
배달 주문의 경우, POST 시점에 이미 주문(Order)이 DB에 생성되므로 **방식 A**가 자연스럽습니다. 반면 01-02 문서의 게시판 예시처럼 글 자체가 비동기로 생성되는 경우, 아직 boardId가 없을 수 있으므로 **방식 B**로 requestId를 추적합니다.
...
[01-02 문서](./01-02.EDA%20기반%20요청응답%20통합.md)에서 소개한 4가지 브릿지 패턴 비교표를 다시 보면:
...
> SSE 방식이 궁금하다면 [01-02 문서의 Section 3 (Server-Sent Events)](./01-02.EDA%20기반%20요청응답%20통합.md)을 참고하세요.
