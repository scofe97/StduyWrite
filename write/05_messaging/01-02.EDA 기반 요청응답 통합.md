...
>
> - 외부 클라이언트는 REST API를 기대(HTTP 요청 -> 응답)
> - 내부 서비스는 이벤트 기반 비동기 통신(즉시 응답 불가)
>
> 문제: 이 2개의 패러다임을 어떻게 연결할 것인가? 이를 이해하기 위해서는 2 방식의 차이점을 먼저 알아야합니다.

# EDA vs Request/Response -  6가지 구조적 차이

---

이커머스 시스템을 예시로 들어봅시다. StoreFront가 주문을 생성하면 주문/재고/알림 서비스가 후속작업을 처리해야 합니다.
...
Request-Response에서는 StoreFront가  각 서비스를 명시적으로 호출합니다. 새로운 후속 서비스가 추가되면 StoreFront코드를 수정하여 호출을 추가해야 합니다.
...
EDA 방식에서는 이벤트를 발행할 뿐, 누가 소비하는지는 모릅니다. 새 서비스가 추가되면 해당 서비스가 토픽을 구독하기만 하면됩니다.
...
Request-Response는 동기 호출 체인에서 트랜잭션을 사용하면 강한 일관성을 보장할 수 있습니다. (모든 단계 성공/실패)

EDA는 최종 일관성(eventual consistency)을 전제로 설계합니다. 이벤트가 발행된 시점과 모든 소비자가 처리를 완료한 시점 사이에 간격이 존재합니다.
...
- T0 ~ T3 사이의 불일치 구간은 보통 밀리초이며, 대부분의 비즈니스 로직에서는 허용되지만 은행과 같은 곳은 강한 일관성이 필수입니다.
- 클라이언트는 202(Accepted)을 받은 후 최종 결과(실패/성공)을 어떻게 알 수 있을까요? Rest에서는 응답코드가 에러마다 들어오지만, EDA는 별도의 수신이 필요합니다.

## 4. Historical State(상태 이력)

Request-Response 호출은 현재 상태를 반환하고, 이전 상태를 덮어 씁니다. "어제 재고가 몇 개 였는가?"에 답하려면 별도 감사 로그를 구현해야 합니다.

EDA에서는 이벤트 로그가 자연스럽게 상태 이력을 보존합니다. 토픽에 쌓인 이벤트를 처음부터 재생하면 임의 시점의 상태를 복원할 수 있습니다.

## 5. Architecture Flexibility(아키텍쳐 유연성)

Request-Response는 새로운 서비스를 추가하면 기존 호출자를 수정해야 합니다. 서비스 간 의존 관계가 복잡해지며 변경의 영향 범위를 예측하기 어려워집니다.

EDA는 새로운 서비스에 기존 토픽을 구독하기만 하면 됩니다. Uber의 ride.requested 이벤트를 처음에는 matching 서비스만 소비했지만, 나중에 ETA 예측, surge pricing, 텔레메트리 서비스가 추가되어 이벤트를 독립적으로 소비했습니다.

## 6. Data Access & Reuse(데이터 접근과 재사용)

Request-Response는 데이터가 각 서비스의 DB에 격리되어 있습니다. 

EDA는 이벤트 토픽이 조직 전체의 데이터 허브 역할을 합니다. 분석 팀은 토픽을 구독하여 실시간 데이터를 수집하고, ML팀은 동일 토픽으로 모델 학습 파이프라인을 구축할 수 있습니다.

# EDA vs Request/Response 방식 연결하는 브릿지 패턴
...
## 1. ReplyingKafkaTemplate(동기식 요청 응답)
...
만약 CQRS를 쓰지 않는 전통적인 구조에서는 단순하고 POST 직후 GET하면 바로 데이터가 보입니다. 하지만 EDA를 도입하면서 비동기 쓰기를 같은 DB에서 읽으려하면 한계가 드러납니다. 

- 읽기 최적화 문제: CQRS는 이벤트 소비 시점에 미리 JOIN하여 View를 만들어 두므로, 조회가 단일 테이블 SELECT로 끝난다.
- 독립 확장 불가: 1000 TPS(쓰기), 100,000TPS(읽기)가 같은 DB에 몰리게된다.

## 3. Server-Sent Events(SSE)

클라이언트가 HTTP 연결을 열면 서버가 Kafka 이벤트를 실시간으로 스트리밍 합니다.
...
## 4. Event-First(202 + Polling) + Outbox

API 경계에서는 Event First로 서비스간 완전 분리를 담당하고, Processor 내부에서는 Outbox패턴으로 DB저장과 이벤트 발행의 원자성을 보장해야 합니다. 이 패턴은 "처음부터 비동기로 설계하되, 클라이언트와 데이터 정합성을 어떻게 보장할까"에 초점을 맞춥니다.
...
이 구조는 핵심적인 2가지 문제를 해결합니다.

1. API 경계: 프론트엔드는 202 응답만 받고, requestID로 Polling하여 결과를 확인합니다. API Gateway의 Processor가 완전히 분리되어 독립 배포/확장이 가능해집니다.
2. Processor 내부: Board 저장, 요청 상태 갱신, OutBox 저장을 같은 트랜잭션으로 묶어 Dual-Write 문제를 차단합니다.

### Dual-Write 문제와 Outbox 패턴

Processor가 Board를 DB에 저장한 뒤 Kafka에 board-created를 직접 발행하면, 두 시스템의 트랜잭션이 같지 않아 서로의 원자성이 지켜지지 않습니다. Outbox패턴으로 이를 해소해야합니다.

### Read-Your-Own-Writes(비동기 쓰기 후 즉시 조회 문제)

비동기 쓰기(Post -> Kafka 발행 -> 202 Accepted)를 사용하면 Consumer가 DB에 적재하기 전에 클라이언트가 조회하여 데이터가 없을 수 있습니다. 이를 이를 해결하는 패턴에 대해서 소개합니다.

### 202 + Retry-After Polling 

202 + Retry-After는 CQRS나 특정 아키텍쳐에 종속되지 않은 비동기 처리 전반의 범용 패턴입니다. 서버가 "아직 처리중이니 N초 후에 다시 확인 해주세요"라고 알려주는 표준입니다.
