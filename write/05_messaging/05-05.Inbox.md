...
Outbox 패턴 자체의 상세는 [05-03.Outbox](05-03.Outbox.md) 참조.
...
>
> - **상태**: 명시적 Inbox 테이블 없음. consumer는 결과 이벤트의 `aggregateId`(`pipelineId`) + 상태 검증으로 단순 멱등성 처리(중복 결과 도착 시 상태가 이미 진행됐으면 무시).
> - **적용 시 설계**: `TB_TRB_IB_001` 테이블 신설(eventId UNIQUE) → `BuildResultConsumer.@KafkaListener`에서 수신 즉시 `INSERT`(UNIQUE 위배 시 skip) → 같은 TX에서 `PipelineExcnService.advanceStage()` 호출 → 외부 알림 같은 후속 처리는 별도 폴러로 분리.
> - **이득/비용**: 외부 알림(메일·Slack)이 추가될 때 안전. 단일 DB 트랜잭션으로 끝나는 현재 흐름엔 과투자.
