...
> *토픽 파이프라인은 메시지가 하나의 토픽에서 시작하여 여러 중간 토픽을 거치며, 단계적으로 처리되는 아키텍쳐 패턴입니다.*

각 단계(State)는 독립적인 Consumer Group으로 구성되며, 입력 토픽에서 메시지를 소비하고 처리 결과를 출력 토픽에 발행합니다.

## 왜 필요한가?

단일 토픽에서 모든 처리를 수행하면 3가지 한계에 부딪힙니다.
...
- 각 서비스는 독립적인 Consumer Group입니다. 각 중간 토픽은 버퍼 역할을 하며, 다음 단계가 자신의 속도로 처리할 수 있게 해줍니다.
...
> *SendTo는 @KafkaListener의 메서드 반환값을 자동으로 다른 토픽에 전송하는 선언적 메커니즘입니다.*

## 내부 동작 원리

1. MessagingMessageConverter: @KafkaListener 메서드가 값을 변환하면, Spring Kafka는 MessagingMessageConverter를 사용하며 반환값을 Kafka ProducerRecord로 변환
2. ReplyTemplate: 변환된 레코드는 KafkaTemplate(ReplyTemplate이라고도 불림)을 통해 전송된다.
3. 헤더 전파: 원본 메시지의 KafkaHeader.REPLY_TOPIC 헤더가 있으면 해당 토픽으로 전송됩니다. 없으면 @SendTo 어노테이션에 지정된 토픽으로 전송
4. null 반환 = 전송 안함
...
- application.yml에서 환경별로 다른 토픽명을 설정할 수 있다.
...
> *@KafkaListener 메서드에서 예외가 발생하면 @SendTo는 실행되지 않습니다. 대신 ErrorHandler가 호출됩니다.*
...
## @SendTo vs Kafka.Template.send() 선택기준
...
메시지가 A→B→C 순서로 토픽을 거치며 단계적으로 처리됩니다.
...
- 하나의 Consumer에서 검증 → 보강 → 변환을 모두 수행하면 코드가 비대해지고, 한 단계의 장애가 전체를 중단시킬 수 있습니다.
...
- 장점: 각 단계가 단일 책임을 가져 코드가 명확해지고, 독립적으로 스케일링/배포 가능
- 비용: 체인이 길어질수록, 각 단계마다 직렬화/역직렬화가 발생하고, 브로커를 거치므로 네트워크 홉이 추가됩니다.

## Fan-Out(1:N 브로드캐스트)

하나의 소스 토픽에서 발생한 이벤트를 N개의 다운스트림 토픽으로 분배합니다. 동일한 이벤트를 서로 다른 관점에서 처리해야할 때 사용합니다.
...
- 장점: 소스 서비스와 다운스트림 서비스가 완전히 분리됩니다.
- 비용: 여러 토픽에 전송하므로 부분 실패 가능성이 있습니다. 원자적 전송이 필요하면 카프카 트랜잭션이 필요합니다.
...
public void fanOutTransactional(OrderCompletedEvent event) {
    ...
    // 하나라도 실패하면 전체 롤백 — 트랜잭션 설정은 08-transaction-patterns.md 참조
}
```

## 패턴 3. Fan-IN(N:1 집계/합류)

여러 소스 토픽의 메시지가 하나의 집계 지점으로 수렴합니다. 병렬 처리 결과를 모아 최종결정을 내릴 때 사용합니다.
...
메시지의 내용을 검사하여 조건에 따라 다른 토픽으로 라우팅합니다. Content-Based Router 패턴이라고도 하며, 하나의 입력 토픽에서 메시지 타입이나 속성에 따라 분기됩니다.
...
- 비용: 라우터가 단일 장애점이 된다. 모든 메시지가 라우터를 거쳐야하므로 라우터의 처리량이 전체 시스템의 병목이 될 수 있다.

## 패턴 5. Diamond(Fan-Out, Fan-In 결합)

메시지를 여러 브랜치로 분산(Fan-out)하고, 각 결과를 다시 합류시키는 패턴입니다.
...
# ReplyingKafka Template(Request-Reply 패턴)

------

> *ReplyingKafkaTemplate은 Kafka 위에서 동기식 Request-Reply 패턴을 구현합니다. 일반적으로 Kafka는 비동기 메시징에 사용되지만, 때로는 요청을 보내고 응답을 기다려야할 때가 있습니다.*

## 왜 HTTP가 아닌 Kafka로 Request-Reply를 하는가?

Kafka 인프라가 있는 환경에서 추가적인 서비스 디스커버리나 로드 밸런서 없이 RPC를 수행할 수 있습니다. 다만 지연 시간이 HTTP보다 길고, 처리량이 낮으므로 고빈도 호출에는 적합하지 않습니다.

이 기능이 필요한 이유는 “가끔 동기 응답이 필요한” 경계 지점이 존재하기 때문입니다.
...
1. 호출자가 ReplyingKafkaTemplate으로 요청을 전송합니다. 이때 KafkaHeaders.CORRELATION_ID와 KafkaHeaders.REPLY_TOPIC 헤더가 자동으로 추가됩니다.
2. 응답 서비스가 요청을 처리하고, @SendTo로 응답을 전송합니다.
3. 호출자의 Reply container가 응답 토픽을 구독하고 있다가, CORRELATION_ID로 요청/응답을 매칭한다.
