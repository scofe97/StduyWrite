---
title: 04-07.Kafka·Redpanda SASL 인증
...
related:
  ...
  - 04-06.Redpanda Console 인증.md
  - 04-10.Kafka 공통 정책 스타터 패턴.md
updated: 2026-05-16
---
...
리소스 패턴 타입(`literal` vs `prefixed`)도 운영 비용에 직결된다. 토픽 이름을 도메인 prefix(`tps.executor.`, `tps.operator.`)로 끊는 우리 명명 규칙([03-01.토픽 디자인](./03-01.%ED%86%A0%ED%94%BD%20%EB%94%94%EC%9E%90%EC%9D%B8.md))과 prefix ACL을 같이 쓰면, 새 토픽이 생겨도 ACL을 다시 부여하지 않아도 된다. 반면 `literal`로만 쓰면 토픽이 추가될 때마다 ACL PR이 따라간다. 우리 환경에서는 prefix를 기본으로 두고, 민감한 토픽 한두 개만 literal로 좁히는 방식이 운영 부담이 가장 낮다.
...
Kafka 문서를 그대로 적용하다 막히는 지점이 몇 군데 있다. 그 차이가 04-05([Redpanda 아키텍처](./04-05.Redpanda%20%EC%95%84%ED%82%A4%ED%85%8D%EC%B2%98.md))에서 본 단일 바이너리·KRaft 대체 구조에서 나온다.
...
- [04-05.Redpanda 아키텍처](04-05.Redpanda%20%EC%95%84%ED%82%A4%ED%85%8D%EC%B2%98.md) — 단일 바이너리 위에서의 SASL 위치
- [04-06.Redpanda Console 인증](04-06.Redpanda%20Console%20%EC%9D%B8%EC%A6%9D.md) — UI 게이트와의 짝 패턴
- [04-10.Kafka 공통 정책 스타터 패턴](04-10.Kafka%20%EA%B3%B5%ED%86%B5%20%EC%A0%95%EC%B1%85%20%EC%8A%A4%ED%83%80%ED%84%B0%20%ED%8C%A8%ED%84%B4.md) — SASL/TLS 디폴트를 starter로 묶기



## 15. 다음 단계 / 참고 문서

- [04-06.Redpanda Console 인증](./04-06.Redpanda%20Console%20%EC%9D%B8%EC%A6%9D.md) — UI 게이트 4가지 옵션. 이 문서와 한 쌍으로 읽는다.
- [04-05.Redpanda 아키텍처](./04-05.Redpanda%20%EC%95%84%ED%82%A4%ED%85%8D%EC%B2%98.md) — 단일 바이너리·KRaft 대체 구조가 SASL 구성에 어떻게 떨어지는지의 배경.
- [09_advanced/09-04.스키마 거버넌스](./09_advanced/09-04.%EC%8A%A4%ED%82%A4%EB%A7%88%20%EA%B1%B0%EB%B2%84%EB%84%8C%EC%8A%A4.md) — Schema Registry 측 ACL 보강 논의.
