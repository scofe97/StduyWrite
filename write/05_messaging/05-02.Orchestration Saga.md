---
title: 05-02.Orchestration Saga
tags: [saga, orchestration, distributed-transaction, state-machine, compensating-transaction]
status: final
source:
  - https://microservices.io/patterns/data/saga.html
  - https://learn.microsoft.com/en-us/azure/architecture/reference-architectures/saga/saga
  - https://camunda.com/best-practices/orchestrating-the-saga/
related:
  - 05-01.Choreography Saga.md
  - 05-03.Outbox.md
  - 05-05.Inbox.md
  - 03-01.토픽 디자인.md
...
> 오케스트레이션 SAGA는 *명시적 Orchestrator*가 단계 진행과 보상을 지시하는 분산 트랜잭션 패턴이다. 코레오그래피와 같은 목적(2PC 회피, 부분 실패 보상)을 가지지만, *상태 머신과 가시성*을 코드로 박아 단계 변경·분기·재시도가 쉬워진다. 단점은 Orchestrator가 단일 책임 지점이 되어 그 자체를 *고가용성으로 운영*해야 한다는 것이다.



## 학습 목표

> 오케스트레이션 SAGA의 *명시적 상태 머신*이 만드는 가시성 이득과 운영 비용을 이해한다.

이 장을 다 읽고 다음 다섯 가지에 자신 있게 답할 수 있으면 학습이 완료된다.

1. Orchestrator가 어떤 책임(단계 진행·보상 트리거·타임아웃)을 지는지 설명할 수 있다.
2. Command/Success Event/Failure Event/Compensation Event 4분류로 메시지를 가르는 이유를 설명할 수 있다.
3. SagaInstance 상태 테이블이 가시성과 retry 정책을 어떻게 만드는지 설명할 수 있다.
4. Orchestrator의 고가용성 운영 전략(여러 인스턴스 + 분산 락 또는 파티션 키)을 설명할 수 있다.
5. 코레오그래피로 만들 수 없는 시나리오(승인 단계, 조건부 분기)를 오케스트레이션이 어떻게 푸는지 설명할 수 있다.
...
### Q1. Orchestrator의 가장 큰 책임은?

*상태 진행과 분기를 코드 한 곳에서 결정*하는 것이다. 각 단계의 명령을 발행하고, 결과 이벤트를 받아 다음 단계를 결정하며, 실패 시 보상 흐름을 트리거하고, 단계 타임아웃을 감시한다. 결과적으로 "이 saga가 지금 어디에 있는가"라는 질문에 *단일 DB 쿼리*로 답할 수 있다.

### Q2. 메시지를 Command/Success Event/Failure Event/Compensation Event로 가르는 이유는?

리스너 코드와 토픽 설계가 즉시 명료해진다. Command는 Orchestrator → Service 단방향이라 워커별 command 토픽으로 분리한다. Success/Failure는 같은 reply 토픽에 묶지만 헤더 `event_type`으로 분기한다. Compensation은 *되감기 의미*라 별도 compensate-reply 토픽으로 빼서 운영 알람·메트릭에서 따로 추적한다. 의미가 다른 메시지는 토픽 분리가 곧 운영 분리가 된다.

### Q3. Orchestrator 고가용성은 어떻게 만드나?

여러 인스턴스를 띄우되 *같은 saga ID는 한 인스턴스만 처리*하게 만든다. 두 가지 방식이다. (1) saga ID를 파티션 키로 사용하면 Consumer Group에서 자동으로 한 컨슈머에만 할당된다. (2) 분산 락(Redis·DB row lock)으로 처리 중인 saga ID를 잠근다. 첫 방식이 인프라가 더 단순하고 성능이 좋다.

### Q4. SagaInstance 상태 테이블이 단순 로그 이상인 이유는?

retry·resume의 *진입점*이 된다. 단계 5에서 외부 API가 실패해 timeout이 났다면, SagaInstance를 보고 "단계 5에 머물러 있다"는 사실로부터 *그 단계 명령만 재발행*할 수 있다. 단순 로그라면 "다시 단계 1부터" 같은 위험한 재시도밖에 못 한다. 또 운영자가 *수동으로 단계를 건너뛰거나 되돌리는* UI를 SagaInstance 위에 얹을 수 있다.

### Q5. 코레오그래피로 못 푸는 시나리오 중 가장 흔한 것은?

*인간 승인 단계*다. "결제 후 관리자 승인 대기"가 끼면 코레오그래피로는 *대기 상태를 어디에 둘지*가 모호하다. 오케스트레이션은 SagaInstance가 `AWAITING_APPROVAL`로 떠 있다가 승인 API 호출이 다음 단계를 트리거한다. 조건부 분기(예: 금액 100만 원 미만은 자동 승인)도 Orchestrator 코드 안에서 깔끔하게 if문 하나로 표현된다.



## 관련 문서

- [05-01.Choreography Saga](05-01.Choreography%20Saga.md) — 코레오그래피 대안과 트레이드오프
- [05-03.Outbox](05-03.Outbox.md) — Command·Event 발행의 안전 보장
- [05-05.Inbox](05-05.Inbox.md) — Reply 이벤트 수신 측 멱등성
- [03-01.토픽 디자인](03-01.%ED%86%A0%ED%94%BD%20%EB%94%94%EC%9E%90%EC%9D%B8.md) — Saga 메시지 4분류와 토픽 설계
- [04-08.Exactly-once 의미론과 Consumer Idempotency](04-08.Exactly-once%20%EC%9D%98%EB%AF%B8%EB%A1%A0%EA%B3%BC%20Consumer%20Idempotency.md) — Saga에서 효과적 EOS
...
>
> - **모듈/위치**: `operator/cicd/src/main/java/org/okestro/tps/operator/cicd/pipeline/application/pipeline/service/PipelineExcnService.java`, `jobs/application/test/event/TestJobExecutionTransactionHandler.java`, `jobs/application/build/event/BuildJobExecutionPublishService.java`
> - **요점**: `PipelineExcnService`가 파이프라인 상태(BUILD_PENDING → BUILD_DONE → TEST_PENDING → ...)를 DB에 저장하면서 단계별 명령 토픽으로 executor에 작업을 위임한다. 각 결과 컨슈머(`BuildResultConsumer` 등)가 다음 단계를 트리거. 보상 흐름은 `*TransactionHandler`에서 처리.
> - **차이점**: 학습 문서의 generic Saga Orchestrator 대신 도메인별 서비스가 직접 상태 머신을 구현. 외부 워크플로우 엔진 미사용.
> - **상세**: 외부 엔진(Temporal, Cadence 등)과의 비교는 [`09_advanced/09-02.Saga 엔진 비교`](09_advanced/09-02.Saga%20엔진%20비교.md).
