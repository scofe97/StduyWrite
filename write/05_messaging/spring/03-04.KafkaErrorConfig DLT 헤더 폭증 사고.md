...
> **1차 소스**: Fixed / Exponential / WithMaxRetries 곡선 비교, jitter, 외부 시스템 SLA 기준 곡선 결정 절차는 [03-05.Backoff 전략 비교와 선택](03-05.Backoff%20전략%20비교와%20선택.md).
...
다음 학습 단계로는 [../02-05.Avro 직렬화 예외처리 전략](../02-05.Avro%20직렬화%20예외처리%20전략.md)으로 돌아가 "재시도 분류" 결정 기준을 다시 확인하고, [../04-08.Exactly-once 의미론과 Consumer Idempotency](../04-08.Exactly-once%20의미론과%20Consumer%20Idempotency.md)의 Inbox 패턴이 `@DltHandler` 무게를 어떻게 덜어주는지 점검한다. 5개 config 클래스 전체의 동작 흐름은 [06-01.message-lib config 5개 클래스 종합](06-01.message-lib%20config%205개%20클래스%20종합.md)에 정리되어 있다.
