...
- 라우팅 비유 한 줄: 택배를 보낼 때 동네 한 블록은 걸어가고, 시내는 자전거, 타 도시는 트럭을 쓰는 것과 같습니다. 
- 단, 이 비유는 "거리(난이도)에 맞춰 수단을 고른다"까지 유효하고, 모델은 거리뿐 아니라 *작업 종류*(코딩이냐 분류냐)에 따라서도 갈린다는 점은 택배 비유로 표현되지 않습니다.
...
- SDK를 쓴다면 `client.messages.parse()`가 권장됩니다. 위 예시처럼 `create` + `output_config`로도 되지만, `parse()`는 응답을 스키마에 맞춰 검증까지 자동으로 해 줍니다.
...
# 두 종류로 갈립니다: 출력 전(pre-output) 거부는 content 가 빈 배열 + 과금 없음,
# 스트리밍 중(mid-stream) 거부는 이미 나온 부분이 과금되므로 버립니다.
...
```python
# 폴백 옵트인 — beta 헤더 + fallbacks 파라미터. 현재 유일한 대상은 claude-opus-4-8.
...
정책 거부 시 API가 같은 요청을 폴백 모델로 다시 돌려 답을 반환합니다. 최종 응답도 `stop_reason`이 `"refusal"`이면 폴백 모델까지 전부 거부한 것입니다. (Amazon Bedrock·Vertex 등 서버사이드 폴백 미지원 환경에서는 SDK의 클라이언트사이드 폴백 미들웨어를 씁니다.)
