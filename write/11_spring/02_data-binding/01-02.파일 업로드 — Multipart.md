...
related:
  ...
  - ../03_network/webflux/02-01.Multipart와 파일 업·다운로드.md
updated: 2026-05-23
---
...
> Spring MVC 의 `MultipartFile` 은 서블릿 스택에서만 동작하는 동기 API 이고, WebFlux 환경에서는 같은 멀티파트 요청을 `Mono<FilePart>` / `Flux<Part>` 로 비동기 스트리밍 방식으로 받습니다. WebFlux 측 구현 세부는 [`../03_network/webflux/02-01.Multipart와 파일 업·다운로드.md`](../03_network/webflux/02-01.Multipart와 파일 업·다운로드.md) 에 별도로 정리돼 있습니다.
...
- [`../03_network/webflux/02-01.Multipart와 파일 업·다운로드.md`](../03_network/webflux/02-01.Multipart와%20파일%20업·다운로드.md) — WebFlux 에서 `Mono<FilePart>` / `Flux<Part>` 로 같은 멀티파트를 다루는 방식
