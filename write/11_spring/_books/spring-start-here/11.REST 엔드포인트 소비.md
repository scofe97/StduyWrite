...
전체 구도를 그림으로 보면, 개발자가 짜는 것은 인터페이스(계약) 하나뿐이고 나머지는 OpenFeign이 대신합니다. OpenFeign이 그 인터페이스를 구현한 클라이언트를 만들어 컨텍스트에 bean으로 등록하면, 컨트롤러는 다른 bean처럼 프록시를 주입받아 호출합니다.

![OpenFeign 인터페이스에서 구현 주입까지](_assets/11-fig03.openfeign-interface-to-impl.svg)
...
앞의 부채 계산 시나리오를 이 방식으로 다시 그리면, 타임라인의 단계가 독립 작업이 됩니다. total debt를 계산하는 작업이 내부 부채·외부 부채를 구하는 두 작업에 각각 subscribe하고, 그 두 작업은 서로 의존하지 않으므로 여러 스레드가 동시에 처리합니다.

![리액티브 producer/subscriber 의존 선언](_assets/11-fig04.reactive-producer-subscriber.svg)
