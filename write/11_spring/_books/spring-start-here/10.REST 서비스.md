...
![HTTP 응답 3요소와 ResponseEntity 제어](_assets/10-fig04.http-response-three-parts.svg)
...
두 방식을 나란히 두면 차이가 분명합니다. 첫 방식은 정상 흐름과 예외 처리가 한 메서드에 섞여 같은 예외를 여러 엔드포인트가 처리하면 중복되고, 둘째 방식은 컨트롤러를 정상 흐름만 남기고 예외 처리를 어드바이스 한곳으로 뽑아냅니다.

![예외 처리 두 방식 대비](_assets/10-fig03.exception-handling-two-ways.svg)
