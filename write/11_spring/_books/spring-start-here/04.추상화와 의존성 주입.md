...
한 인터페이스에 구현체가 둘 이상이면, 타입만으로 주입할 때 `NoUniqueBeanDefinitionException`이 납니다. Spring이 어느 구현을 넣을지 못 고르기 때문입니다. 3장과 같은 두 방법으로 해결합니다.

![같은 인터페이스 다중 구현체의 주입 모호함과 두 해소법](_assets/04-fig02.multiple-implementations-disambiguation.svg)

예외 메시지는 어느 타입이 모호한지와 후보 bean 이름을 함께 알려 줍니다.

```
// 구현체가 둘인 채 타입으로만 주입하면 (책 p.19)
