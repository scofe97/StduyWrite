...
WAR 배포의 복잡함을 피하려고 톰캣을 라이브러리로 끌어와 `main()` 안에서 직접 띄우는 방식이 현대 스프링 부트의 표준입니다. 톰캣을 라이브러리로 품는 원리, `SpringApplication.run()` 의 실체, Fat JAR와 실행 가능 JAR의 차이는 분량이 커서 [02-02.내장 톰캣과 SpringApplication — JAR로 WAS를 품다](02-02.내장%20톰캣과%20SpringApplication%20—%20JAR로%20WAS를%20품다.md) 로 분리했습니다. 본 문서(서블릿 표준)를 읽은 뒤 바로 이어 보면 "서블릿을 실행하는 WAS를 스프링 부트가 어떻게 품는가"가 한 흐름으로 연결됩니다.
...
## 7. 다음에 읽을 것
- [내장 톰캣과 SpringApplication — JAR로 WAS를 품다](02-02.내장%20톰캣과%20SpringApplication%20—%20JAR로%20WAS를%20품다.md) — 본문의 서블릿 표준을 실행하는 WAS를 스프링 부트가 어떻게 품고 `java -jar` 로 띄우는지.
- [Spring MVC — FrontController에서 DispatcherServlet까지](03-01.Spring%20MVC%20—%20FrontController에서%20DispatcherServlet까지.md) — 본문 3장 `@WebServlet`을 한 단계 추상화한 `DispatcherServlet`과 내부 핸들러 매핑/어댑터 구조.
