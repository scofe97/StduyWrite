...
> Boot 자체(auto-config/Actuator/Properties), MVC(DispatcherServlet, FrontController), AOP, 예외처리 영역은 아직 본 폴더에 정식 문서로 없다. 노션 import raw 는 [`_notion_import/`](_notion_import/) 에 있으며, 재작성이 끝난 묶음부터 위 표에 행을 추가한다.
...
2. **`04_webflux/`** — WebClient·Reactive. RestTemplate 경험이 있다면 01-01 부터 진입.
3. **`05_testing/`** — 단위·통합·E2E 전 범위. Spring Boot 3.x 기준.
4. **도메인별** — 본인 관심 영역. 메시징이면 [`04_messaging/spring/`](../04_messaging/spring/), 데이터·ORM 이면 [`05_data/querydsl/`](../05_data/querydsl/), 보안이면 [`10_security/`](../10_security/).
