...
> Netty 위에 Reactive Streams 를 얹은 네트워크 엔진이라는 한 줄 정의에서 출발합니다.
...
> Reactor Netty 가 애플리케이션 코드의 어느 층 아래에 서는지, 인바운드·아웃바운드가 왜 같은 엔진을 쓰는지 봅니다.
...
> 한 스레드가 여러 연결을 처리할 수 있는 이유 — 블로킹과 논블로킹 소켓의 `read` 차이를 짚습니다.
...
> 순수 소켓이라면 직접 다뤘을 부품들 — ByteBuf·Channel·프로토콜 지원 — 을 Netty 가 어떻게 감싸는지 봅니다.
