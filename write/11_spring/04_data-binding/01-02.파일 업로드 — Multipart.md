...
related:
  ...
  - ../04_webflux/02-01.Multipart와 파일 업·다운로드.md
updated: 2026-05-23
---
...
> Spring MVC 의 `MultipartFile` 은 서블릿 스택에서만 동작하는 동기 API 이고, WebFlux 환경에서는 같은 멀티파트 요청을 `Mono<FilePart>` / `Flux<Part>` 로 비동기 스트리밍 방식으로 받습니다. WebFlux 측 구현 세부는 [`../04_webflux/02-01.Multipart와 파일 업·다운로드.md`](../04_webflux/02-01.Multipart와 파일 업·다운로드.md) 에 별도로 정리돼 있습니다.
...
- [`../04_webflux/02-01.Multipart와 파일 업·다운로드.md`](../04_webflux/02-01.Multipart와%20파일%20업·다운로드.md) — WebFlux 에서 `Mono<FilePart>` / `Flux<Part>` 로 같은 멀티파트를 다루는 방식
- [`../03_mvc/01-01.Spring MVC — FrontController에서 DispatcherServlet까지.md`](../03_mvc/01-01.Spring%20MVC%20—%20FrontController에서%20DispatcherServlet까지.md) — `DispatcherServlet` 이 `MultipartResolver` 를 어느 시점에 호출하는지 전체 흐름
