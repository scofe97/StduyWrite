...
트랜잭션은 도메인 결합도가 큰 주제입니다. `@Transactional` 한 줄은 영속성 계층(JPA·JDBC)과 떼어 설명하기 어렵고, 그래서 본체 문서는 [`05_data/03_persistence/jpa/`](../../05_data/03_persistence/jpa/) 에 자리잡는 편이 자연스럽습니다. 같은 내용을 `11_spring/` 에 다시 쓰면 두 곳이 어긋나는 순간 어느 쪽이 옳은지 알 수 없게 됩니다.
