...
related:
  - ../README.md
  - ../03_webflux/02-03.테스트 (MockWebServer와 WebTestClient).md
...
Spring Boot 3.x 기준으로 단위 테스트에서 외부 시스템 E2E까지를 한 줄기로 엮은 학습 시리즈입니다. 한 챕터씩 따로 읽어도 닫혀 있도록 썼지만, 전체를 순서대로 따라가면 테스트 피라미드의 어느 층이 어떤 결함을 잡는지를 비용·속도·신뢰도 트레이드오프와 함께 손에 익힐 수 있습니다.



## 이 묶음의 위치

`03_webflux/02-03.테스트` 가 WebClient 한 어댑터의 단위 테스트에 집중한다면, 이 묶음은 **단위 테스트의 모범 작성법부터 시작해 슬라이스·통합·E2E·아키텍처 가드까지 전 계층의 사용법과 함정을 다룹니다**. WebClient 챕터를 먼저 읽었다면 이 묶음의 1장(기초)을 빠르게 건너뛸 수 있고, JPA 위주로 들어가는 사람은 `05_data/querydsl/02-02.테스트와 멀티모듈` 과 본 묶음의 02-01을 함께 봅니다.



## 학습 곡선

읽는 순서 자체가 비용·환경 의존도가 낮은 것에서 높은 것 순입니다. 단위 테스트는 컨텍스트를 띄우지 않아 1초 안에 끝나고, 슬라이스 테스트는 Spring 컨텍스트 일부만 띄워 수 초 내 완료됩니다. 통합 테스트부터는 Testcontainers·EmbeddedKafka 같은 인프라가 함께 뜨면서 수십 초가 들고, 외부 시스템 E2E는 WireMock으로 격리하지 않으면 환경 가용성에 결과가 묶입니다. 이 순서를 의식하면 빌드 시간이 폭주하기 전에 어느 층에 테스트를 늘릴지 판단이 섭니다.
...
1. **왜 이 단계가 필요한가** — 다른 단계로는 잡지 못하는 결함이 무엇인지를 한 문단으로 설명합니다.
2. **핵심 API와 어노테이션** — 표나 짧은 코드 예로 결정적 차이를 보입니다.
3. **함정과 회피** — Ryuk 비활성으로 인한 컨테이너 누수, schema-registry timeout 30초, ThreadLocal 누수 같은 실제 사고 사례를 정리합니다.
4. **TPS 사례** — operator·message-lib·executor 모듈에서 짧게 인용합니다. 사내 도메인이 본질이 아닌 인용은 일반화하고, 흐름이 본질인 인용은 그대로 둡니다. 종합 챕터(02-05)는 사례 분석에 한 챕터를 통째로 씁니다.



## 관련 문서

- [03_webflux/02-03.테스트 (MockWebServer와 WebTestClient)](../03_webflux/02-03.테스트%20(MockWebServer와%20WebTestClient).md) — WebClient 어댑터 단위 테스트, 본 묶음 1장과 보완 관계
