...
- 개발자 로컬에서는 Node 18이 설치되어 있지만 Jenkins Agent에는 Node 16이 설치된 상황이 빈번하다. "내 로컬에서는 되는데요"의 근본 원인이 바로 이것이다.
- `node:18.17.0-alpine`이라고 명시하면 언제 어디서 실행하든 동일한 빌드 환경이 보장된다.
- 파이프라인이 시작되면 Jenkins는 해당 이미지로 컨테이너를 생성하고, Jenkins 워크스페이스를 볼륨 마운트한 뒤, 컨테이너 내부에서 스텝을 실행한다.
- 빌드가 끝나면 컨테이너는 삭제된다.

Docker agent 실행 흐름은 다음과 같다.
...
- 프로덕션에서는 빌드 번호와 Git SHA를 조합한 복합 태그가 가장 실용적이다. 빌드 순서도 파악할 수 있고, 어떤 커밋의 이미지인지도 확인할 수 있다.
