...
---
> `05-08` 문서는 순수 승인 API와 운영 조회 API만 다룬다. 이 문서는 `input` 스텝의 상태 해석, 승인자 제어, readiness 판단, TPS 운영 패턴처럼 API 바깥의 운영 개념을 정리한다.

## 1. 이 문서의 범위

> 이 문서는 승인과 운영 관리 API를 "어떻게 해석하고 운영에 연결할 것인가"를 설명한다.

| 주제 | 이전 관점 | 현재 관점 |
|------|------|------|
| 승인 대기 해석 | 단순히 멈춘 빌드처럼 보이기 쉬웠음 | `PAUSED_PENDING_INPUT` 이라는 별도 운영 상태로 봐야 함 |
| 승인 권한 | Jenkinsfile `input` 이면 누구나 승인 가능하다고 보기 쉬웠음 | `submitter`, `submitterParameter` 로 더 엄격히 제어 가능 |
| liveness 체크 | `/api/json` 만 보면 충분해 보였음 | `/login`, `/api/json`, `/computer/api/json` 을 목적별로 나눠 봐야 함 |
| readiness 판단 | Jenkins에 `/health` 하나가 있을 것처럼 기대하기 쉬웠음 | 코어는 단일 readiness 값을 주지 않으므로 정책을 직접 정의해야 함 |
| 프로젝트 연결 | 승인 API와 운영 모니터링이 스펙 문서에 섞이기 쉬웠음 | TPS 감지/노출/운영 판단은 별도 개념 문서에서 봐야 함 |



## 2. `PAUSED_PENDING_INPUT` 상태 해석
...
## 5. liveness와 readiness를 분리해서 보기
...
> TPS 관점에서 이 문서와 연결되는 운영 패턴은 승인 대기 감지와 노드 모니터링 두 갈래다. `05-08`이 "무슨 API를 치는가"라면, `05-08a`는 "그 API를 운영에서 어떤 의미로 읽는가"에 가깝다.
