...
---
> 이 문서는 Jenkins 파이프라인(Job) CRUD API 자체를 설명하는 스펙 문서다.
>
> - 조회, 생성, 수정, 삭제, Jenkinsfile 검증 엔드포인트를 다룬다.
> - TPS 패턴과 현대화 이론은 `05-03a`에서 별도로 다룬다.

## 1. 이 문서의 범위

> 이 문서는 파이프라인(Job)을 다루는 아래 API만 설명한다.
...
인증 헤더와 crumb/cookie 흐름은 별도 문서에서 다룬다:
...
`{pipelineStruct}`는 Jenkins URL 경로 구조를 그대로 따른다.

- 단일 Job: `/job/{name}`
- 폴더 중첩 Job: `/job/{folder}/job/{subfolder}/job/{name}`

`{parent}`는 새 아이템을 생성할 상위 경로다.
...
예시에서 Job 경로처럼 환경마다 달라지는 값은 먼저 환경 변수로 빼두는 편이 좋다. 이후 문서들도 동적인 경로값, 이름값, 상위 폴더값이 나오면 같은 방식으로 `export`해서 재사용하는 것을 기본 원칙으로 본다.
...
이후 POST 예시에서는 기본값이 `Jenkins-Crumb`라고 가정해도 되지만, 가장 안전한 방식은 헤더 이름도 같이 변수로 쓰는 것이다:
...
- XML 응답은 파일로 저장한 뒤 확인한다.
- HTML 응답은 원문 그대로 확인한다.
...
이 응답은 XML이므로 `jq`가 아니라 원문이나 XML 포매터로 확인한다.
...
폴더와 파이프라인을 생성하는 API다. 생성 타입은 요청 본문의 XML 구조로 결정된다.
...
이 요청은 보통 본문이 거의 없거나 리다이렉트 중심이므로 `jq`보다 상태 코드와 헤더를 우선 확인한다.

루트 폴더를 만들 때는 예를 들어 다음처럼 잡는다:
...
이 요청도 성공 여부는 본문보다 상태 코드와 헤더가 더 중요하다.

바로 테스트할 수 있도록 최소 원본 파일도 같이 둔다:

- `05-03.sample-pipeline-config.xml`

이 파일은 Declarative Pipeline 최소 예제다. 현재 디렉토리에서 바로 사용할 수 있고, 필요하면 `<description>`과 `<script>`만 수정해서 재사용하면 된다.

현재 샘플은 `<script>` 안에 한 줄 Declarative Pipeline을 `CDATA`로 감싼 형태다. Jenkins가 줄바꿈을 공백처럼 처리하는 환경에서도 깨지지 않게 하려는 목적이다.

XML 파일 업로드는 `-d @file`가 아니라 `--data-binary @file`를 사용한다. `-d @file`는 줄바꿈 원문을 보존하지 않아 Jenkinsfile 스크립트가 한 줄 공백 문자열처럼 바뀔 수 있다.

이미 예전 샘플로 파이프라인을 만들었다면 다음 중 하나로 반영한다:

- 기존 파이프라인을 삭제하고 다시 생성한다.
- 아래 `POST /{pipelineStruct}/config.xml` 예시로 설정 XML을 덮어쓴다.

파일이 없다면 아래 명령으로 바로 생성할 수 있다.

macOS/Linux:

```bash
cat > 05-03.sample-pipeline-config.xml <<'EOF'
...
'@ | Set-Content -Encoding utf8 05-03.sample-pipeline-config.xml
```
...
Job 설정 전체를 교체하는 수정 API다. 부분 수정은 지원하지 않으며, 전체 config.xml을 다시 전송해야 한다.
...
현재 문서의 샘플 XML로 바로 덮어쓰려면 다음처럼 실행한다:
...
조회한 `config.xml`을 수정해서 다시 반영하려면 다음처럼 실행한다:
...
조회 응답은 XML이므로 `jq` 대상이 아니고, 수정 응답은 상태 코드 확인이 우선이다.

`Started by user Jenkins Admin ... Expected a symbol ... Missing required section "agent"` 같은 오류가 계속 나오면 먼저 전송 옵션을 확인한다. 이 경우는 Jenkins가 아니라 `curl -d @config.xml` 때문에 스크립트 줄바꿈이 깨진 사례일 가능성이 크다.
...
삭제도 JSON 본문을 기대하지 않으므로 `jq` 대신 상태 코드와 `Location` 헤더를 본다.
...
Jenkins에는 Jenkinsfile을 실행 없이 검증하는 API가 두 개 있다. 이 둘은 검증 관점이 다르므로, 목적에 따라 골라야 한다.
...
- 혼합 환경 → 파이프라인 타입을 먼저 구분해서 검증 경로를 나눈다

**3-4-1. `POST /{pipelineStruct}/descriptorByName/.../checkScriptCompile`**

> Groovy/CPS 컴파일 관점에서 검증한다. 스크립트가 Jenkins Pipeline Groovy로서 컴파일 가능한지를 본다.
>
> - 기존 Job 경로 아래에서 호출하므로 대상 Job이 먼저 존재해야 한다.
> - `workflow-cps` 플러그인 내부 descriptor 경로에 의존하는 구조다.
...
성공이면 `status == "success"`, 실패면 `line`/`column`/`message`에 오류 위치와 내용이 담긴다.
...
> Declarative Pipeline 모델·문법 규칙 관점에서 검증한다. 공식 문서에서 Jenkinsfile validation endpoint로 안내하는 경로다.
>
> - 루트 엔드포인트이므로 기존 Job이 없어도 호출할 수 있다.
> - `pipeline { ... }` 구조, `stages`/`stage`/`steps` 배치, Declarative에서 허용되지 않는 위치의 구문 같은 모델 규칙을 검사한다.
> - Groovy 문법만 맞으면 통과하는 `checkScriptCompile`과 달리, Declarative 모델 위반은 실패로 잡는다.
...
응답은 텍스트 형식이다. 성공이면 다음처럼 보인다:

```text
Jenkinsfile successfully validated.
```

실패하면 오류 내용이 텍스트로 반환된다. 예를 들어 `agent` 섹션이 없으면 다음처럼 보인다:
...
> **주의**: HTTP 상태 코드 `200`은 요청 처리 성공을 뜻할 뿐이다. Jenkinsfile 자체가 유효한지는 응답 본문 텍스트로 확인해야 한다.
...
- `05-02. 젠킨스 인증 API 스펙 (ID-Password + Crumb).md`
- `05-02a. 젠킨스 인증 모델과 TPS 패턴 (2.222+).md`
- `05-03a. 젠킨스 파이프라인 CRUD 모델과 TPS 패턴 (2.222+).md`
