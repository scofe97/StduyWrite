...
# 젠킨스 큐 내부 흐름과 실행 순서
---
> 이 문서는 Jenkins 내부에서 빌드 요청이 큐를 거쳐 executor에 도달하기까지의 데이터 흐름을 설명한다.
>
> - `05-04`가 REST API 요청/응답 형식을 다루고, `05-04a`가 TPS 운영 패턴을 다룬다면, 이 문서는 Jenkins 소스 코드 수준의 큐 메커니즘을 다룬다.
> - Queue ID 할당, 상태 전이(WaitingItem → BuildableItem → LeftItem), `maintain()` 루프, executor 배정, 실행 순서 보장 여부를 다룬다.
> - `cancelItem` API 호출 시 큐 내부에서 일어나는 취소 처리 흐름도 다룬다.
...
- 추가로, 같은 Job을 짧은 간격으로 트리거하면 `05-04` 섹션 7에서 설명한 큐 병합(Queue coalescence)이 발생할 수 있다. 이 경우 4번 호출해도 실제 queue item은 1~2개만 생길 수 있다.
...
Quiet Period가 짧거나 Executor 여유가 즉시 있는 환경에서는 큐 대기 시간이 매우 짧아 취소 타이밍을 놓칠 수 있다. `cancelItem` 호출 직전에 `executable` 필드가 `null`인지 확인하고 분기하는 것이 안전하다.

`cancelItem`과 `stop` 중 어느 것을 써야 하는지 판단하는 방법과 실제 API 호출 방법은 `05-04` 섹션 5를 참조한다.
...
- `05-04. 젠킨스 빌드 실행·큐 API 스펙.md` — REST API 요청/응답 형식, cancelItem·stop API
- `05-04a. 젠킨스 빌드 실행·큐 모델과 TPS 패턴 (2.222+).md` — Pre-trigger Guard, Queue 운영 판단
- `05-04b. 젠킨스 큐-빌드 전환 흐름과 실행기 환경.md` — VM/K8s 실행기 환경, 데이터 추적
- `05-05a. 젠킨스 빌드 상태 추적 모델과 TPS 패턴.md` — 큐 상태와 빌드 상태의 매핑
