...
### 3-5. Run 생성과 Executor 점유는 다른 순간이다

> `buildNumber`가 생긴 순간 = Executor가 busy가 된 순간이라고 보면 안 된다. 이 둘 사이에는 실무에서 무시하기 어려운 과도 구간이 있다.

앞의 다이어그램에서 ④ 구간은 개념상 하나의 단계처럼 보이지만, 실제로는 세 개의 하위 순간으로 나뉜다.

```text
④-a. Run 객체 생성 → buildNumber 확정
④-b. Queue 이탈 → LeftItem 전환, queue 자료구조에서 제거
④-c. 해당 Run이 실제 Executor 슬롯에 붙음 → currentExecutable 채워짐, busy 반영
```

이 두 순간 사이의 짧은 구간에서는 다음 상태가 공존한다:

- `buildNumber`는 이미 존재한다
- queue에서는 이미 빠져나왔을 수 있다
- `/computer/api/json`의 `busyExecutors`나 `currentExecutable`에는 아직 반영이 안 됐을 수 있다
- `run.getExecutor()`가 null일 수 있다

**`run.getExecutor()`가 null인 근본 원인은 Pipeline의 2단계 실행 구조다.** Pipeline 잡은 `onStarted` 시점에 Controller 위의 flyweight task로 시작되며, 아직 어떤 Agent의 executor 슬롯도 점유하지 않는다. 이는 VM/K8s 에이전트 유형과 무관하다. Freestyle 잡은 executor 스레드 내에서 `onStarted`가 호출되므로 이 문제가 없다. K8s 동적 에이전트 환경에서는 Pod 프로비저닝이 추가되어 이 과도 구간이 더 길어진다.

Jenkins 코어의 관련 소스코드([`Run.getExecutor()`](https://github.com/jenkinsci/jenkins/blob/master/core/src/main/java/hudson/model/Run.java))는 모든 Computer의 executor를 순회하면서 `currentExecutable == this`인 슬롯을 찾는다. flyweight task 시점에는 어떤 executor에도 해당 Run이 올라가지 않았으므로 null이 반환된다:
...
API 필드 체인 전체와 상태 폴링 패턴은 `05-04b. 젠킨스 Queue 적재 이후 실행 흐름과 데이터 추적.md`를 참조한다. `WaitingItem → BuildableItem → LeftItem` 내부 상태 전이와 `maintain()` 루프 동작은 `05-04c. 젠킨스 큐 내부 흐름과 실행 순서.md`를 참조한다.
