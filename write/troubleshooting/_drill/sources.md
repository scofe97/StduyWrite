...
사례집 25편은 남이 만든 해설을 제가 재구성한 것입니다. `troubleshooting/` 의 문항은 증상만 보고 직접 푼 것입니다. 전자를 통째로 옮기면 안 푼 76건이 "푼 사례" 폴더에 섞여 들어가고, 그러면 그 폴더가 무엇을 모은 곳인지가 흐려집니다.
...
- 사례의 신선도는 **계층마다 다르게** 봅니다. 연도 하나로 자르면 안 낡는 것을 버리고 낡은 것을 들이게 됩니다.

| 계층 | 기준 연도 | 왜 |
|------|----------|-----|
| A 리눅스 호스트 | 무관 | 커널 기전은 안 낡습니다 |
| E JVM·런타임 | 무관 | 컬렉터 세대만 대조에 밝힙니다 |
| B 쿠버네티스 | 2020년 이후 | v1.18 이후로 API 와 기본값이 안정됐습니다 |
| C 서비스 메시 | 2022년 이후 | 그 전은 아키텍처가 다릅니다 |
| D 클라우드 | 2021년 이후 | 제공자 기본값과 할당량이 자주 바뀝니다 |

  연도보다 먼저 볼 것이 둘 있습니다.

  첫째는 **그 도구를 지금도 쓰는가** 입니다. Linkerd 1.x, kube-dns, Docker 런타임처럼 세대가 바뀐 것은 연도와 무관하게 거릅니다.

  둘째는 **기전이 제품 버그인가 구조인가** 입니다. 특정 버전 버그는 고쳐지면 끝이지만 구조적 한계는 안 낡습니다. 기준 연도보다 오래된 것을 쓸 때는 1차 자료로 지금도 유효한지 확인하고 그 사실을 대조에 적습니다.
...
| 날짜 | 문항 | 계층 | 점수 | 재출제 |
|------|------|------|------|--------|
| 2026-09-07 | 실패율 0.3%가 사라지지 않는 서버 | A | 2 | 2026-09-10 완료 |
| 2026-09-07 | Pod 로는 되고 이름으로는 안 되는 호출 | B | 3 | |
| 2026-09-07 | 트래픽이 늘 때만 섞여 나오는 503 | C | 3 | |
| 2026-09-08 | 아무것도 안 바꿨는데 늘어난 커넥션 리셋 | A | 3 | |
| 2026-09-08 | 한 하위 서비스에서 시작된 전면 오류 | B | 3 | |
| 2026-09-08 | 수거량에 비해 지나치게 긴 멈춤 | E | 4 | |
| 2026-09-09 | 사라진 Pod 로 계속 가는 요청 | C | 스킵 | |
| 2026-09-09 | 아무도 안 죽었는데 다 죽은 클러스터 | B | 4 | |
| 2026-09-09 | 절반만 뜨고 멈춘 배포 | D | 4 | |
| 2026-09-10 | 초당 한 번 규칙적으로 늘어나는 드롭 | A | 3 | |
| 2026-09-12 | 배포할 때마다 잠깐 나오는 502 | B | 4 | |
| 2026-09-12 | CPU 는 노는데 응답이 끊긴 서비스 | E | 4 | |
| 2026-09-12 | 정확히 1초씩 늦는 요청 | B | 3 | |
| 2026-09-14 | 몇몇 사용자만 들어오지 못하는 사이트 | A | 2 | 2026-09-17 |
| 2026-09-14 | 업그레이드 2분 뒤 멈춘 클러스터 | B | 2 | 2026-09-19 |
...
**확인 방법**. 2026-09-07 부터 다섯 문항 연속으로 "어디를 보면 갈리는지"에서 멈췄습니다. 자료를 늘리는 방식이 듣지 않아 09-09 부터 답하는 순서를 바꿔 첫 확인을 가설보다 먼저 적게 했고, 그 회차에 처음으로 그 칸이 채워졌습니다. 09-10·09-12 에는 이름까지 나왔습니다 — `tcpdump` 와 `exec` 가 되물음 없이 나왔습니다.

다만 갈래가 하나 드러났습니다. **진단 도구는 나오고 관측 도구는 안 나옵니다.** `tcpdump`·`exec` 처럼 무언가를 하는 도구는 떠올리는데, `ethtool -S`·`jcmd Thread.print`·JFR 처럼 상태를 읽기만 하는 도구는 계속 비어 있습니다. 다음 출제에서는 관측 도구를 요구하는 증상에 가중치를 둡니다.

**한도의 출처**. 09-08 두 문항과 09-09 세 문항 모두 이 축이었습니다. 다섯 문항 연속이라 09-10 회차에 축을 시간·순서·관측 도구로 옮겼습니다. 세 문항이 각각 다른 축으로 나와 재발하지 않았습니다. 09-08 두 문항은 한도를 사람이 정한 설정값으로 가정했습니다. A 는 커널이 총 메모리에서 유도한 값이었고, B 는 사람이 정했으되 피크가 아니라 과거 사용량을 근거로 삼은 값이었습니다. 상한을 만나면 그 숫자가 어디서 왔는지 묻는 문항에 가중치를 둡니다.
...
| 사이트 | 방식 | 다루는 범위 |
|--------|------|------------|
| [k8s.af](https://k8s.af/) | 공개된 실패 사례·포스트모템 큐레이션. 회사·근본원인 태그로 색인 | Kubernetes 를 축으로 하되 근본원인은 커널·네트워크·런타임까지 |
