...
| 계층 | 기준 연도 | 왜 |
|------|----------|-----|
| 전 계층 하한 | **2022년 이후** | 사용자 요청(2026-09-21). 기전이 안 낡는 계층이라도 도구·기본값·판본이 그사이 바뀝니다 |
| A 리눅스 호스트 | 2022년 이후 | 커널 기전 자체는 안 낡지만 관측 도구와 기본값이 바뀝니다 |
| E JVM·런타임 | 2022년 이후 | 컨테이너 인식과 컬렉터 세대가 최근에도 움직입니다 |
| B 쿠버네티스 | 2022년 이후 | 하한이 올라가 기존 2020년 기준을 대체합니다 |
| C 서비스 메시 | 2022년 이후 | 그 전은 아키텍처가 다릅니다 |
| D 클라우드 | 2022년 이후 | 제공자 기본값과 할당량이 자주 바뀝니다 |

2022년보다 오래된 사례는 **그 기전이 지금도 유효한지 1차 자료로 확인한 뒤** 씁니다. 확인한 사실은 대조 절에 적습니다.

2026-09-14 B 문항은 2023년이고 E 문항은 2026년이라 이 하한 안입니다. 2026-09-07 회차의 옛 사례들은 이 기준 이전에 뽑은 것입니다.
...
| 2026-09-14 | 힙은 평평한데 몇 시간마다 죽는 컨테이너 | E | 3 | |
| 2026-09-21 | 긴 로그만 간헐적으로 사라지는 수집기 | A | 무효 | |
| 2026-09-21 | 배포하지 않은 아침에 끊긴 리전 다섯 곳 | A | 3 | |

`무효` 는 스킵과 다릅니다. 스킵은 푸는 쪽이 문항을 물리는 것이고, 무효는 **출제 쪽 결함으로 문항이 못 쓰게 된 것**입니다. 09-21 A 첫 문항은 제가 출제하면서 원인을 대화로 흘려 무효가 됐습니다. 파일은 지우지 않았습니다. 대조 절에 RFC 2991 과 RFC 1213 축자 인용이 들어 있어 근거로는 계속 씁니다.

2026-09-14 A 의 재출제는 아직 열려 있습니다. 그날 막힌 지식은 흐름을 가르는 값이고, 무효가 된 문항이 그 자리를 메우려던 것이었습니다. 같은 날 다시 낸 문항은 계층만 같고 요구하는 지식이 달라 재출제로 세지 않습니다.
...
**09-16·09-21 갱신.** B 에서는 `calicoctl node status`, E 에서는 `jcmd VM.native_memory` 와 `pmap` 이 모두 설명 뒤에 나왔습니다. 아홉 회차째 같은 자리라, 이제는 도구 이름을 묻기보다 **"무엇을 알고 싶은가"를 먼저 적게 하는 순서**를 유지합니다. 두 회차 모두 그 순서로 물었을 때 방향은 스스로 나왔습니다.
...
대조를 쓰다가 찾은 사고 중 아직 문항으로 내지 않은 것입니다. 같은 기전의 다른 증상이라 재출제 소재로 씁니다.

| 계층 | 사고 | 날짜 | 왜 후보인가 |
|------|------|------|------------|
| A | [Heroku · Summary of June 10 Outage](https://www.heroku.com/blog/summary-of-june-10-outage/) | 사건 2025-06-10 | 2026-09-21 A 와 방아쇠가 같습니다. 무인 OS 업그레이드가 네트워크 서비스를 재시작시켰고, 첫 부팅 때만 경로를 까는 스크립트 탓에 재시작 뒤 경로가 비었습니다. 증상이 Datadog 과 달라 같은 지식을 다른 증상으로 낼 수 있습니다 |
