...
이 추상화와 구현의 분리가 이 장의 핵심입니다. 앱은 JDK의 추상화만 바라보고 코드를 짜므로, 드라이버만 갈아 끼우면 같은 코드로 다른 DB에 붙습니다.

![JDBC 추상화(JDK)와 벤더 드라이버 구현](_assets/12-fig03.jdbc-abstraction-vs-driver.svg)
...
![purchase 테이블 구조](_assets/12-fig04.purchase-table-structure.svg)
