...
Spring이 `parrot()` 호출을 가로채 판단하는 흐름은 다음과 같습니다. 이미 Context에 bean이 있으면 메서드 본문을 건너뛰고 기존 것을 돌려주기 때문에, 메서드를 몇 번 부르든 인스턴스는 하나로 유지됩니다.

```mermaid
flowchart TB
    call["person()이 parrot() 호출"] --> check{"parrot bean이<br>이미 Context에 있나?"}
    check -->|"① 있음"| reuse["메서드 본문 실행 안 함<br>기존 인스턴스 반환"]
    check -->|"② 없음"| create["메서드 본문 실행<br>새 인스턴스 생성 후 Context 등록"]
    reuse --> one["결과: 전체에서 인스턴스 1개"]
    create --> one
    style call fill:#5b9bd5,color:#fff
    style check fill:#d4a853,color:#000
    style reuse fill:#66bb6a,color:#000
    style create fill:#66bb6a,color:#000
    style one fill:#dceaf7,color:#000
```
...
A를 만들려면 B가 필요한데 B를 만들려면 다시 A가 필요한 교착 상태입니다. Spring은 A도 B도 만들지 못하고 예외로 실패합니다. Person 생성자가 Parrot을 요구하고 Parrot 생성자가 다시 Person을 요구하면, Spring은 어느 쪽도 먼저 만들 시작점을 잡지 못합니다.

![순환 의존 교착 — 서로를 기다리다 아무도 못 만듦](_assets/03-fig04.circular-dependency-deadlock.svg)
