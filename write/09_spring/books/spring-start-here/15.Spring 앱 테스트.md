...
흐름은 이렇습니다. 개발자가 코드를 저장소에 올리면 CI 도구가 그 변경으로 테스트를 돌리고, 하나라도 실패하면 개발자에게 되돌려 알립니다.

```mermaid
flowchart LR
    D["개발자"] -->|"코드 push"| G["GIT 저장소"]
    G --> J["Jenkins가<br>테스트 실행"]
    J --> R{"테스트<br>통과?"}
    R -->|"통과"| OK["빌드 성공"]
    R -->|"실패"| N["개발자에게 알림"]
    N -.->|"고치고 다시 push"| D
    ...
    style G fill:#0d0d0d,color:#e8e4df,stroke:#8a8580
    style J fill:#d4a853,color:#000
    style R fill:#d4a853,color:#000
...
두 시나리오를 나란히 두면 무엇을 검증하는지 분명합니다. 정상 흐름은 네 단계가 모두 성공하지만, 예외 흐름은 받는 계좌 조회가 실패하는 순간 예외가 던져져 그 뒤 계산·변경 단계가 아예 실행되지 않습니다 — 테스트는 이 "실행되지 않음"까지 검증해야 합니다.

![정상 흐름과 예외 흐름 비교](_assets/15-fig03.happy-vs-exception-flow.svg)
