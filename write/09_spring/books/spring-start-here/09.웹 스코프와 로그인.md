...
request와 session의 차이는 인스턴스 해시로 보면 분명합니다. request는 같은 사용자의 두 요청도 서로 다른 인스턴스라 해시가 다르고, session은 같은 세션이면 두 요청이 같은 인스턴스를 공유해 해시가 같습니다.

![request와 session 인스턴스 생성 차이](_assets/09-fig02.request-vs-session-instances.svg)
...
두 흐름(로그인 검증과 `/main` 접근 제어)이 session bean을 사이에 두고 어떻게 맞물리는지 그림으로 보면 이렇습니다. 로그인은 자격이 맞으면 username을 세션에 저장하고 메인으로 보내며, 틀리면 로그인 페이지로 되돌립니다. `/main`은 반대로 세션에 username이 *있는지*를 확인해, 없으면(=미로그인) 로그인 페이지로 되돌립니다.

```mermaid
flowchart TD
    A["로그인 페이지"] -->|"자격 증명 제출"| B{"자격이 유효한가?"}
    B -->|"아니오"| A
    B -->|"예"| C["session bean에<br>username 저장"]
    C --> D["redirect: /main"]
    D --> E{"session에<br>username 있나?"}
    E -->|"없음 = 미로그인"| A
    E -->|"있음"| F["메인 페이지 표시"]
    ...
    style D fill:#0d0d0d,color:#e8e4df,stroke:#8a8580
    style E fill:#d4a853,color:#000
    style F fill:#66bb6a,color:#000
```

> **핵심은 session bean의 username이 "로그인 상태"의 단일 근거라는 점입니다.** 로그인은 이 값을 *채우고*, `/main`은 이 값이 *있는지*로 접근을 판단하며, 로그아웃은 이 값을 *비웁니다*. 상태 하나로 세 동작이 맞물립니다.
