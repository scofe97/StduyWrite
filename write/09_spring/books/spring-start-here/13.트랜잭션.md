...
![송금 중 한 단계 실패로 데이터 정합성이 깨지는 흐름](_assets/13-fig03.transfer-consistency-broken.svg)
...
컨트롤러가 서비스를 부르는 순간 트랜잭션이 열리고, 그 안에서 두 계좌 조회(SELECT)·금액 계산·양쪽 변경(UPDATE)이 차례로 일어난 뒤, 예외 없이 끝나면 그 시점에 commit됩니다. 트랜잭션 스코프가 서비스 메서드 전체를 감싸는 모습은 이렇습니다.
...
sequenceDiagram
    ...
    C->>S: transferMoney() 호출
    Note over S: 트랜잭션 시작 (메서드 직전)
    S->>R: 보내는 계좌 조회
    R->>DB: SELECT
    S->>R: 받는 계좌 조회
    R->>DB: SELECT
    ...
    S->>R: 받는 계좌 변경
    R->>DB: UPDATE
    Note over S: 예외 없이 끝 → commit<br/>런타임 예외 → rollback
    S-->>C: 반환
```
...
- 심화 카테고리: [트랜잭션 격리 수준 — Spring 관점](../../08_transaction/01-01.트랜잭션%20격리%20수준%20—%20Spring%20관점.md) · [@Transactional 테스트 가드](../../08_transaction/01-02.@Transactional%20테스트%20가드.md)
