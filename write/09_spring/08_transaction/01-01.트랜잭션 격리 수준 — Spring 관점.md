...
related:
  - README.md
  - ../../05_data/02_relational/sql-mysql/04-02.동시성제어와 락.md
  - ../../05_data/01_foundation/01-04.트랜잭션과 격리 수준.md
...
격리 수준이 무엇을 막는지의 정의, 그리고 Dirty Read·Non-Repeatable Read·Phantom Read 세 이상현상의 의미는 DB 이론 영역이라 본 문서에서 다시 적지 않습니다. 정본은 [`05_data/02_relational/sql-mysql/04-02 동시성제어와 락`](../../05_data/02_relational/sql-mysql/04-02.%EB%8F%99%EC%8B%9C%EC%84%B1%EC%A0%9C%EC%96%B4%EC%99%80%20%EB%9D%BD.md) 과 [`05_data/01_foundation/01-04 트랜잭션과 격리 수준`](../../05_data/01_foundation/01-04.%ED%8A%B8%EB%9E%9C%EC%9E%AD%EC%85%98%EA%B3%BC%20%EA%B2%A9%EB%A6%AC%20%EC%88%98%EC%A4%80.md) 에 있습니다.
...
이것은 DB 격리 수준이 올라간 게 아니라 *조회가 DB 까지 가지 않은* 것입니다. 1차 캐시를 비우고(`em.clear()`) 다시 조회하거나 JPQL 로 강제 조회하면 격리 수준의 실제 동작이 드러납니다. 격리 수준을 검증하는 테스트를 짤 때 이 차이를 모르면 "격리가 되는 줄 알았는데 캐시였다" 는 잘못된 결론에 이릅니다. 락과 영속성 컨텍스트의 관계는 [`05_data/03_persistence/jpa/04-02 낙관적 비관적 락`](../../05_data/03_persistence/jpa/04-02.%EB%82%99%EA%B4%80%EC%A0%81%20%EB%B9%84%EA%B4%80%EC%A0%81%20%EB%9D%BD.md) 에서 더 다룹니다.
...
전파 자체의 규칙(REQUIRED·REQUIRES_NEW·NESTED, 물리 트랜잭션과 논리 트랜잭션)은 [`05_data/03_persistence/jpa/04-01 스프링 트랜잭션 §7`](../../05_data/03_persistence/jpa/04-01.%EC%8A%A4%ED%94%84%EB%A7%81%20%ED%8A%B8%EB%9E%9C%EC%9E%AD%EC%85%98.md) 에 정리돼 있으니, 여기서는 "격리는 물리 트랜잭션 단위" 라는 결론만 들고 갑니다.
