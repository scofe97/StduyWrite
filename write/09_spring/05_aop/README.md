...
related:
  - ../README.md
  - ../01_core/
  - ../01_core/03-02.예외 처리 — 서블릿에서 @ControllerAdvice까지.md
updated: 2026-05-24
---
...
위 항목 중 막히는 부분이 있으면 [`spring/`](../README.md) 의 다른 묶음(`01_core/`)을 먼저 보고 돌아오는 편이 빠릅니다.
...
- [Spring MVC — FrontController 에서 DispatcherServlet 까지](../01_core/03-01.Spring%20MVC%20—%20FrontController에서%20DispatcherServlet까지.md) — Interceptor 가 어디서 끼어드는지 비교
- [예외 처리 — 서블릿에서 @ControllerAdvice 까지](../01_core/03-02.예외%20처리%20—%20서블릿에서%20@ControllerAdvice까지.md) — `@ControllerAdvice` 가 AOP 와 어떻게 연결되는지 확인
...
트랜잭션 AOP 는 별도 편으로 두지 않습니다. `@Transactional` 이 `Advisor` 로 등록돼 프록시 안에서 해석되는 *메커니즘* 은 [`01-01 §4·§5`](01-01.횡단%20관심사와%20AOP%20—%20프록시로%20풀어내기.md) 가 일반론으로 다루고, 트랜잭션 *본체* (전파·격리·롤백)는 [`05_data/03_persistence/jpa/04-01`](../../05_data/03_persistence/jpa/04-01.%EC%8A%A4%ED%94%84%EB%A7%81%20%ED%8A%B8%EB%9E%9C%EC%9E%AD%EC%85%98.md) 에 정리돼 있어, 둘을 합치면 별도 편 없이 트랜잭션 AOP 가 설명됩니다.
