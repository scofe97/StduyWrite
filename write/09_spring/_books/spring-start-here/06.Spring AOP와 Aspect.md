...
![aspect weaving 프록시 동작](_assets/06-fig01.aspect-weaving-proxy.svg)
...
![execution pointcut 식 토큰 해부](_assets/06-fig02.execution-pointcut-anatomy.svg)
...
이 조작이 실제로 어떻게 관측되는지는 로그로 드러납니다. `newArgs`로 다른 comment("Some other text!")를 넘기고 반환값은 `"FAILED"`로 바꾸면, 실행 로그는 이렇게 어긋납니다.

```
INFO: Method publishComment with parameters [Comment{text='Demo comment', ...}] will execute
INFO: Publishing comment:Some other text!          ← 메서드는 바뀐 파라미터를 받음
INFO: Method executed and returned SUCCESS          ← 메서드가 실제 반환한 값
INFO: FAILED                                        ← main()이 받은 값 (aspect가 바꿈)
```

caller(`main()`)는 파라미터로 "Demo comment"를 넘겼는데 메서드는 "Some other text!"를 찍고, 메서드는 SUCCESS를 반환했는데 caller는 FAILED를 받습니다. 즉 파라미터도 반환값도 aspect가 중간에서 갈아끼운 셈입니다. 그만큼 강력하므로, 이런 조작은 caller 입장에서 완전히 투명하다(보이지 않는다)는 위험을 늘 염두에 둬야 합니다.
...
`@Around` 외 advice는 `ProceedingJoinPoint`를 받지 않고, 위임 시점을 스스로 정하지 않습니다 — 애너테이션 성격대로 자동 결정됩니다. 이 차이가 곧 선택 기준입니다. `@Around`만 `proceed()` 호출 여부를 직접 쥐고 있어, **위임을 아예 거부**할 수 있습니다. 인가(authorization) aspect가 조건 미충족 시 `proceed()`를 부르지 않으면 실제 메서드는 실행되지 않고, caller는 그 사실을 알지 못한 채 aspect가 대신 정한 결과만 받습니다. 반대로 단순히 전·후에 로직만 끼우면 되는 경우엔 위임 여부를 통제할 필요가 없으므로, `@Before`·`@AfterReturning` 같은 더 단순한 advice로 구현을 가볍게 유지하는 편이 낫습니다.
...
한 메서드를 여러 aspect가 가로채면 실행 순서가 생깁니다. 예로 `SecurityAspect`(조건 미충족 시 위임 안 함)와 `LoggingAspect`가 같은 메서드에 걸리면, 순서에 따라 결과가 달라집니다. aspect들은 서로 위임하며 **실행 체인**을 이룹니다 — 앞 aspect가 `proceed()`로 다음 aspect에 위임하고, 마지막이 실제 메서드에 위임한 뒤 역순으로 반환이 돌아옵니다.

순서를 정하지 않으면(원본 기준 `LoggingAspect`가 먼저) 다음처럼 흐릅니다.
...
`@Order`로 순서를 뒤집으면(`SecurityAspect`가 먼저) 체인의 진입 aspect가 바뀝니다.
...
> **순서가 중요합니다.** `SecurityAspect`가 먼저 실행되고 위임을 거부하면 `LoggingAspect`가 실행되지 않습니다. 거부된 호출까지 로깅하려면 `LoggingAspect`가 *먼저* 실행돼야 합니다. 두 흐름의 차이가 바로 이 지점 — 진입 aspect가 위임을 끊으면 그 뒤 aspect는 아예 실행 기회를 얻지 못합니다.
...
- 연관 노트: [Bean 스코프와 생애주기](./05.Bean%20스코프와%20생애주기.md) · `10_spring/05_aop` 카테고리
