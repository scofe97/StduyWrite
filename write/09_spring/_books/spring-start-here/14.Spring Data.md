...
전체 구도는 이렇습니다. 앱은 인터페이스로 계약을 확장하기만 하고, Spring Data가 그 인터페이스를 구현한 bean을 컨텍스트에 등록합니다. 그러면 `TransferService` 같은 다른 컴포넌트가 5장에서 배운 대로 인터페이스 타입으로 DI 받아 씁니다.

![repository가 계약을 확장하고 DI로 주입되는 구조](_assets/14-fig03.repository-extends-and-di.svg)
