...
스타터가 없던 시절엔 개발자가 웹에 필요한 의존성을 *하나하나 직접 나열*하고, 그것들이 서로 호환되는 버전인지까지 손으로 맞춰야 했습니다. 스타터는 이 구도를 뒤집습니다 — "Tomcat, Spring Context, AOP를 이 버전으로 달라"가 아니라 **"웹 능력을 달라"** 한 줄로 요청하면, Spring Boot가 그 능력에 필요한 의존성을 호환 버전으로 묶어 내려 줍니다. 개발자는 정확히 어떤 의존성이 필요한지, 어느 버전이 호환되는지 몰라도 됩니다. `pom.xml`엔 `spring-boot-starter-web` 하나뿐이지만 External Libraries 폴더를 열면 Tomcat·Context·AOP JAR가 다 들어와 있는 이유입니다.
...
위 시퀀스가 시간 순서라면, 아래 그림은 같은 흐름을 **컴포넌트 배치**로 보여 줍니다. 다섯 컴포넌트 중 초록으로 강조된 Controller만 우리가 직접 짜고, 나머지는 Spring Boot가 자동 구성합니다.

![Spring MVC 컴포넌트 아키텍처](_assets/07-fig02.mvc-architecture-components.svg)
