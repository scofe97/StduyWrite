...
!`클라이언트가 직접 구현체를 선택하는 구조 — DIP·OCP 위반` <!-- 링크 끊김(2026-08): assets/01-4-before.png -->
...
!`정적 클래스 의존 — 코드만 봐도 무엇을 의존하는지 보인다` <!-- 링크 끊김(2026-08): assets/01-4-static-dep.png -->

!`동적 객체 의존 — 어떤 구현이 들어올지는 실행 시점에 결정된다` <!-- 링크 끊김(2026-08): assets/01-4-dynamic-dep.png -->
...
!`요청마다 새 객체를 만드는 경우 — 메모리 낭비` <!-- 링크 끊김(2026-08): assets/03-2-no-singleton.png -->

!`싱글톤 — 모든 요청이 한 객체를 공유` <!-- 링크 끊김(2026-08): assets/03-2-singleton.png -->
...
!`실행 결과 — 스레드 A 가 받은 값도 B 의 입력으로 오염됐다` <!-- 링크 끊김(2026-08): assets/03-2-race-condition.png -->
...
!`노션 학습 노트의 생명주기 흐름 — 위 표와 같은 7단계` <!-- 링크 끊김(2026-08): assets/03-4-lifecycle.png -->
...
!`요청마다 새 프로토타입 빈이 생성되지만 소멸 콜백은 호출되지 않는다` <!-- 링크 끊김(2026-08): assets/03-4-prototype-each.png -->
...
!`의존성 주입을 프록시 객체로 변경하면 호출 시점에 실제 request 빈을 찾는다` <!-- 링크 끊김(2026-08): assets/03-4-scope-proxy.png -->
