...
rows = [
    ("1", "연결 기초", [("Computer Networking", "1단계 · 전송·응용 계층", ["DNS·HTTP·TLS", "TCP/UDP·혼잡 제어"])]),
    ("1–2", "Linux 경로", [("Systems Performance", "1–2단계 · 네트워크 장", ["socket·큐·포트", "인터페이스·병목"])]),
    ("3", "패킷 관측", [("Packet Analysis with Wireshark", "3단계 · 캡처와 판독", ["TCP 상태·재전송", "TLS ClientHello"])]),
    ("3–4", "DNS", [("Learning CoreDNS", "3–4단계 · DNS 진단", ["Corefile·plugin", "resolver·Kubernetes DNS"])]),
    ("6", "서비스 메시", [("Istio in Action", "6단계 · 운영 경계", ["sidecar·traffic", "mTLS·Zero Trust"])]),
]

W, TOP, ROW_H = 1000, 160, 124
H = TOP + len(rows) * ROW_H + 68
d = D(W, H, "WRITE · NETWORK BOOK FLOW", "네트워크 책 읽기 흐름", "연결 기초에서 패킷 관측, DNS, 서비스 메시로 올라가며 필요한 책을 배치했습니다.", "위에서 아래로 진행하며 단계 표의 진단 과제와 함께 읽습니다")
