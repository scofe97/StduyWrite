...
stages = [
    ...
    ("5 · 동시성", "메모리 모델이 무엇을 보장하는가",
     [("자바 메모리 모델과 하드웨어 효율", "심층 자바 가상 머신 12장", "필수"),
      ...
      ("가상 스레드 · mount·unmount", "심층 자바 가상 머신 12장", "추천"),
      ("pinning — 캐리어를 붙드는 자리", "심층 자바 가상 머신 12장", "추천")],
    ...
    ("6 · 성능 측정과 튜닝", "추측하지 않고 재는 법",
     [("무엇을 측정할까 — 벤치마크와 지표", "Java Performance 2장", "필수"),
      ...
      ("상태 읽기 — jstat · jstack · jcmd", "심층 자바 가상 머신 4장", "필수")],
    ...
    ("7 · 장애 진단", "증상에서 원인으로 좁히는 절차",
     [("조사 기법의 지형과 네 시나리오", "Troubleshooting Java 1장", "추천"),
      ...
      ("긴 STW 의 구성 — 세이프포인트 · 로그", "심층 자바 가상 머신 3장", "필수")],
     [("스레드 덤프 획득과 데드락 추적", "Troubleshooting Java 8장", "필수"),
      ...
      ("pinning 교착 · 커넥션 풀 고갈", "책 없음 — 채울 자리", "추천"),
...
NOTES = {
    ...
    6: "정독 노트가 이 로드맵의 자료다. 책은 셋이라 아래 줄이 장 번호만 가리킨다.",
}
...
d = D(W, H, "WRITE · JVM ROADMAP",
      "JVM 학습 로드맵",
      "코드가 JVM 안에서 어디에 놓이는지에서 시작해 클래스 로딩·실행 엔진·가비지 컬렉션·동시성을 지나 "
      "성능 측정과 장애 진단으로 간다. 척추에 단계 일곱을 걸고 개념을 좌우로 뻗었다. 노드의 주인공은 개념이고 아래 줄은 "
