...
# 절단선은 4단계 뒤에 긋는다 — 1~4 는 노드 한 대로도 생기는 일이고,
#   5 부터는 여러 노드로 넓힐 때 생기거나(복제·합의) 커지는(격리·스트림) 문제다.
#
...
stages = [
    ("1 · 데이터 시스템의 축", "무엇을 재고 무엇을 지킬 것인가",
     ...
     [("응답 시간과 처리량", "DDIA 2장", "필수"),
      ...
      ("DEK · KEK · envelope", "Real-World Crypto 8장", "추천"),
      ("crypto-shredding", "NIST SP 800-88", "추천"),
      ("삭제권과 보존 기간", "DDIA 14장", "추천")]),
    ...
    ("5 · 복제와 샤딩", "같은 데이터를 여러 곳에 두면",
     ...
     [("키 범위 샤딩", "DDIA 7장 · PoDS 20장", "필수"),
      ...
      ("rendezvous 해싱 · HRW", "Thaler·Ravishankar 1998", "추천"),
...
CUT_AFTER = 3          # 4단계 뒤에 "노드 한 대" ↔ "여러 노드" 절단선
NOTES = {
    3: "1~4단계는 노드 한 대로도 생기고, 5단계부터는 여러 노드로 넓힐 때 생기거나 커지는 문제다.",
    7: "자료를 소장 책과 DDIA 2판 정독 노트로 한정했다. 05_data 의 자체 노트는 순서의 근거로 쓰지 않는다.",
}
...
for i, (title, sub, left, right) in enumerate(stages):
    ...
    if i == CUT_AFTER:
        ...
        d.t(SX, y + 17, "1~4단계는 노드 한 대 · 5단계부터 여러 노드로 넓힌다", 13, WARN, KR)
        y += 56
