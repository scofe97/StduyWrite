...
stages = [
    ("1 · 연결", "응용과 전송 — 연결이 무엇인가",
     [("socket · bind · listen · accept", "TCP/IP Illustrated 12·13장", "필수"),
      ...
      ("혼잡 제어 · CUBIC · Vegas", "TCP/IP Illustrated 16장", "필수"),
      ("스트림 멀티플렉싱 · HOL 블로킹", "yamux spec · RFC 9000", "추천"),
      ("UDP · 단편화", "TCP/IP Illustrated 10장", "추천"),
      ("EPIPE · SIGPIPE · RST", "TCP/IP Illustrated 13장", "추천"),
      ("TCP keepalive", "TCP/IP Illustrated 17장", "선택")],
     [("DNS 질의 · 이름 해석", "TCP/IP Illustrated 11장", "필수"),
      ("DNS 레코드 · 위임 · TTL", "Learning CoreDNS 2장", "필수"),
      ("HTTP/1.1 · HTTP/2", "HTTP/2 in Action 2·4장", "필수"),
      ("QUIC · HTTP/3", "HTTP/2 in Action 9장", "추천"),
      ("HTTP 성능 축", "HPBN 11·12장", "선택"),
      ("TLS 핸드셰이크 · SNI", "HPBN 4장 · Real-World Crypto 9장", "필수"),
      ("ClientHello · 확장 협상", "Real-World Crypto 9장", "필수"),
      ("key_share · HelloRetryRequest", "Real-World Crypto 5·9장", "추천"),
      ("ECDHE · X25519 · forward secrecy", "Real-World Crypto 5장", "필수"),
      ("AEAD · AES-GCM · ChaCha20", "Real-World Crypto 4장", "필수"),
      ("ECH · HPKE", "RFC 9180 · tls-esni", "추천"),
      ("인증서 체인 · SAN 검증", "HPBN 4장", "필수"),
      ("reverse proxy · half-close · 배압", "", "추천"),
      ("listen 큐 · accept 큐 · 포트 고갈", "TCP/IP Illustrated 13장", "추천"),
      ("OS CA bundle · truststore", "Container Security 11장", "선택")]),
    ...
    ("2 · Linux 경로", "커널 안에서 패킷이 지나는 길",
     [("interface · MAC · ARP · NDP", "TCP/IP Illustrated 3·4장", "필수"),
      ...
      ("conntrack · 상태 테이블 포화", "Networking and K8s 2장 · 랩 09편", "필수"),
      ("VLAN · 802.1Q · STP", "TCP/IP Illustrated 3장", "추천")],
     [("MTU · MSS · PMTUD", "TCP/IP Illustrated 10장 · 랩 12·13편", "필수"),
    ...
    ("3 · 관측", "정말 그 길로 갔는지 눈으로 본다",
     [("캡처 위치 · 디스플레이 필터", "Packet Analysis 2장", "필수"),
      ("tcpdump · tshark", "Packet Analysis 2장", "필수"),
    ...
    ("4 · Kubernetes", "같은 커널 경로 위에 얹힌 이름과 정책",
     [("Pod IP · Pod CIDR · Node CIDR", "Networking and K8s 4장", "필수"),
      ...
      ("readiness · stale Endpoint", "Kubernetes in Action 11장", "필수")],
     [("클러스터 DNS · Service FQDN", "Learning CoreDNS 6장", "필수"),
      ("IPAM · Pod IP 고갈", "Cilium 4장", "필수"),
      ("service discovery · east-west", "", "추천"),
      ("Ingress", "Networking and K8s 5장", "필수"),
      ("Gateway API · HTTPRoute", "K8s in Action 13장 · Cilium 7장", "추천"),
      ("L4 로드밸런싱 · health check", "System Design on AWS 5장", "추천"),
      ("인증서 만료와 TLS 실패 구분", "", "추천"),
      ("externalTrafficPolicy · 소스 IP", "Kubernetes in Action 11장", "추천")]),
    ...
    ("6 · 데이터패스와 정책", "같은 일을 다른 데이터패스로",
     ...
     [("eBPF 네트워킹", "Learning eBPF 8장", "추천"),
      ...
      ("Beyla · Caretta — 자동 계측", "Beyla · Caretta 문서", "선택"),
      ("투명 암호화 · WireGuard", "Cilium 14장", "선택"),
      ("egress 게이트웨이 · 클러스터 access", "Cilium 10·11장", "선택"),
      ("eBPF host routing · netkit", "Cilium 8장", "선택")]),
    ...
    ("7 · 운영 경계", "클러스터가 한 종류가 아닐 때",
     [("dual-stack · ipFamilyPolicy", "", "추천"),
      ...
      ("retry · circuit breaking", "Istio in Action 6장", "필수")],
     [("Envoy · VirtualService · DR", "Istio in Action 3·4·5장", "추천"),
    ...
    ("8 · 오버레이와 신뢰", "분산 네트워크가 되풀이하는 문제들",
     [("bootstrap · trust anchor", "I2P Network Database", "추천"),
      ("peer discovery · DHT · gossip", "Patterns of Distributed Sys 28장", "추천"),
      ("signed descriptor · 공개키 신원", "Real-World Crypto 7장", "추천"),
      ("key rotation · replay 방지", "Real-World Crypto 3·8·9장", "추천"),
      ("Sybil · eclipse · poisoning", "I2P Network Database", "추천"),
      ("Sybil 저항 — 자원 증명", "", "선택")],
     [("traffic correlation", "Tor 설계 논문", "필수"),
    ...
    ("9 · 터널과 경로", "고른 노드로 실제 길을 내는 일",
     [("피어 발견 대 터널 구성", "I2P Tunnel Routing", "필수"),
      ("멀티홉 — 홉 수의 대가", "I2P Tunnel Routing", "필수"),
      ("홉별 계층 암호화", "I2P Garlic Routing", "추천"),
      ("path selection · 경로 다양성", "I2P Peer Selection", "필수"),
      ("inbound 와 outbound 의 분리", "I2P Tunnel Routing", "추천"),
      ("소스 라우팅", "I2P Tunnel Routing", "추천")],
     [("종단 성공 확률 — 곱으로 쌓인다", "", "필수"),
      ("기하분포와 평균 시도 횟수", "", "추천"),
      ("재시도 · heartbeat · 감지 시간", "Patterns of Distributed Sys 7장", "필수"),
      ("backoff · jitter · retry budget", "", "필수"),
      ("터널 풀 — 미리 열어 두기", "I2P Tunnel Routing", "추천"),
      ("예비 터널 — 준비 비용과 전환 시간", "I2P Tunnel Routing", "추천"),
      ("터널 수명과 교체", "I2P Tunnel Routing", "추천")]),
]

CUT_AFTER = 2          # 3단계 뒤에 노드 한 대 ↔ 클러스터 절단선
NOTES = {
    2: "Kubernetes 네트워크 장애의 상당수가 4단계가 아니라 2단계에서 풀린다. conntrack 과 MTU 가 먼저다.",
    4: "클라우드 축은 Networking and K8s 6장과 System Design on AWS 9장이 맡는다. AWS 밖은 공식 문서로 메운다.",
