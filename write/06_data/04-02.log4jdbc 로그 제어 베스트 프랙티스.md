...
>
> - 04-01 §4 "네 수도꼭지" 를 각 레이어별로 깊이 파고, 비용 모델·외과성·디버깅 손실을 수치로 표시한다.
> - logback EvaluatorFilter 의 패턴 4가지(키워드·thread·MDC·복합)와 샘플링 패턴 3가지(DuplicateMessageFilter·BurstFilter·counter)를 카탈로그로 정리한다.
...
> 본 절은 *번호 없는* 도입 섹션이다. 본문 §1부터 본격 번호가 시작된다. log4jdbc 자체는 죄가 없고 *제어 안 된 환경* 이 폭주를 만든다는 인식이 본 문서의 출발점이다.

폭주는 *어느 한 가지 설정 실수* 가 아니라 *세 가지 미세한 미스매치가 겹쳤을 때* 일어난다. 04-01 §6 회고에서 operator k8s 만 폭주한 이유가 (a) DriverSpy 사용 + (b) root level INFO + (c) EvaluatorFilter 키워드 매칭의 *조합* 이었던 것과 같다. 본 문서는 이 세 가지를 각각 *어디서 제어 가능한가* 로 분해한다.
...
jdbc-url 은 `mariadb` 인데 underlying driver 로 `com.mysql.cj.jdbc.Driver` 가 박혀 있다. 04-01 §5 "흔한 실수" 가 executor 에 그대로 살아 있다. 본 문서 §4 properties 위생에서 이 미스매치를 정정하는 한 줄을 박는다.



## 1. 본 글의 좌표 — 04-01 §4를 깊이 들여다보는 deep-dive

> 이 글은 04-01 §4 "네 수도꼭지" 의 한 칸 한 칸을 한 단계 깊이 파고드는 deep-dive 이며, Spring `@ConditionalOnExpression` 과 같은 *조건식 기반 제어* 발상이지만 *컴파일된 Java 식을 매 이벤트마다 평가* 한다는 점이 비용 차이를 만든다. 본 절은 사전 지식 앵커링이며, §2부터 본격 결정 매트릭스가 시작된다.
...
logback EvaluatorFilter 의 발상은 Spring `@ConditionalOnExpression` 또는 Drools 같은 룰 엔진과 같다 — "어떤 조건이 참일 때만 동작" 이라는 분기 결정을 코드 본문이 아니라 *외부 식* 으로 둔다. 단 비교 대상과 결정적으로 다른 점 한 가지가 있다.
...
매 이벤트 평가 비용을 받는다는 trade-off 가 EvaluatorFilter 의 가장 큰 특성이며, 본 문서 §6 에서 정량화한다.



## 2. 4개 차단 레이어 결정 매트릭스

> wrap 로그를 잠그는 결정 위치 네 곳(driver / properties / logger level / EvaluatorFilter) 을 *외과성·평가 비용·디버깅 손실* 세 축으로 비교한다. 한 환경에 한 레이어만 쓰는 게 아니라 *조합* 으로 쓰는 것이 일반적이며, §3~§6에서 각 레이어를 깊이 본다.

네 레이어를 한 그림으로 정리하면 다음과 같다.
...
> `spring.datasource.driver-class-name` 을 순정 driver 로 바꾸고 jdbc-url 접두사 `jdbc:log4jdbc:` 를 빼면 wrap 채널 자체가 생성되지 않는다. 평가 비용 0·외과성 0·디버깅 손실 100 — 폭주를 막는 가장 단단한 차단이지만 *디버깅 능력 전부* 를 잃는 비싼 거래다.
...
executor 의 현재 상태(`application-local.yml`) 에서는 DriverSpy 가 박혀 있고 *로컬 디버깅에 필요* 하므로 (1) 은 부적합. §9 처방에서 (2)+(4) 조합을 권장한다.



## 4. (2) properties 위생 — auto.load 끄고 underlying 명시

> `log4jdbc.log4j2.properties` 의 6개 핵심 설정으로 wrap 라이브러리 *부팅 잡음과 출력 형식* 을 조정한다. `auto.load.popular.drivers=false` + `drivers=<순정>` 의무 명시가 핵심이며, executor 환경의 mysql/mariadb 미스매치가 본 절의 정정 대상이다.

log4jdbc-log4j2 공식 properties 15개 중 운영에서 실제로 만지는 6개만 추려서 본다.
...
Janino 가 식에 자동 주입하는 변수 9개와, 자주 헷갈리는 *주입되지 않는* 변수의 우회 방법을 한 표로 박는다.
...
EvaluatorFilter 의 `OnMatch` / `OnMismatch` 두 속성이 식 결과를 어떻게 처리할지 결정한다.
...
식 복잡도별 평가 비용을 마이크로벤치 기준 (대략적인 값) 으로 박으면 다음과 같다.
...
폴러 워크로드의 분당 로그 수가 수천 수준이면 누적 CPU 영향은 측정 한계 이하. 분당 수십만 건이 흐르는 환경에서만 식 단순화를 고민한다.



## 7. EvaluatorFilter 패턴 카탈로그 4가지

> 키워드 매칭·thread name·MDC 키·복합 조건 네 가지 패턴을 표현력·외과성·코드 침투도로 비교한다. 04-01 §6 회차 1(키워드)→회차 3(thread name) 갈이가 이 카탈로그의 (a)→(b) 전이와 일치한다.
...
**단점:** 스레드 풀 이름 규약에 의존. `@Scheduled` 의 기본 풀 이름은 `scheduling-1` 같이 불명확할 수 있어 `TaskScheduler` 빈을 명시 설정해 `outbox-poller-` 같은 prefix 를 박아야 한다.
...
> 사용자 환경(`tps-gitlab2/executor/engine/src/main/resources/`) 의 실측 진단 결과 3가지 미스매치가 발견됐다. (a) properties 의 mysql driver 명시(실제는 mariadb), (b) EvaluatorFilter 가 키워드 매칭 회차 1 수준에 머무름, (c) 임시 디버깅용 토글 부재. 본 절은 *PR 한 번으로 적용 가능한* 정정 처방을 박제한다.
...
executor 의 polling 스레드 풀 이름을 먼저 식별해야 한다. `executor.recover.{submitted,running,sonarqube}` 세 폴러가 `@Scheduled` 또는 명시 `TaskScheduler` 로 등록되는데, *기본 풀 이름이 명시되지 않으면* `scheduling-1`, `task-scheduler-1` 같이 식별이 어렵다.
...
같은 폴러 사이클 한 번이 두 식에서 어떻게 다르게 보이는지 박제한다.
...
이 세 가지를 *한 PR* 에 묶지 말고 단계별로 적용한다.
...
## 10. OTel JDBC 마이그레이션 단계

> wrap 라이브러리의 세 가치를 OpenTelemetry JDBC auto-instrumentation 이 *trace 데이터* 로 대체한다. 본 절은 wrap → wrap+OTel 병행 → OTel only 의 3단계 마이그레이션 경로와, 각 단계에서 wrap 을 *언제 끌 수 있는가* 의 결정 기준을 정리한다.
...
### 10.1 1단계 — wrap only (현재 상태)

executor·operator 의 현재. `DriverSpy` + logback 차단으로 운영 위생을 유지. 본 문서 §3~§9 가 이 단계의 운영 매뉴얼.

### 10.2 2단계 — wrap + OTel 병행 (검증 기간)

OTel agent 를 추가하되 wrap 라이브러리는 그대로 둔다. 검증 체크리스트:
...
검증 기간은 보통 2~4주. trace 완전성과 collector 안정성을 모두 확인한 뒤에만 3단계.

### 10.3 3단계 — OTel only

wrap 라이브러리 제거. 단 *완전 제거* 와 *비활성화* 두 갈래.
...
신규 서비스라면 처음부터 OTel only. 기존 서비스는 *비활성화* 로 시작해 *6개월 안정 운영* 후 완전 제거가 권장 흐름.

### 10.4 wrap 을 끄는 시점 결정 기준
...
네 조건이 모두 yes 일 때만 wrap 비활성화. 한 조건이라도 no 면 2단계 유지.
...
### 한 줄 정의
log4jdbc 로그 제어란 wrap 채널이 만든 분당 수천 줄 폭주를 4개 위치(driver·properties·logger level·EvaluatorFilter) 중 환경 우선순위에 맞는 조합으로 외과적으로 잠그는 결정이며, 외과성과 평가 비용은 반비례한다는 trade-off 가 핵심이다.

### 핵심 포인트 3가지

1. **4개 레이어의 외과성·평가 비용 trade-off.** Driver/logger level 차단은 평가 비용 0 이지만 디버깅 능력 전부를 잃고, EvaluatorFilter 는 외과적이지만 매 이벤트 Janino 식 평가 비용 (단순 식 0.3μs, 복합 식 1.0~1.5μs) 을 받는다. polling 워크로드에 EvaluatorFilter 가 일반적으로 가치 있는 거래인 이유는 평가 비용보다 디버깅 손실 0 의 가치가 크기 때문이다.
...
3. **wrap 라이브러리는 OTel 졸업의 임시방편이다.** OTel JDBC auto-instrumentation 이 SQL 본문·바인딩·실행 시간을 모두 trace 로 대체하므로 새 서비스는 처음부터 OTel only 로 가고, 기존 서비스는 wrap → wrap+OTel 병행(2~4주) → OTel only 3단계로 마이그레이션. wrap 끄는 시점은 trace 완전성·sampling rate·bind parameter 가시성·dashboard 이관 4조건 모두 충족 시.

### 자주 묻는 질문

**Q. log4jdbc 의 두 delegator(Slf4jSpyLogDelegator vs Log4j2SpyLogDelegator) 중 어느 쪽을 쓰는 게 일반적인가?**
A. Spring Boot logback 기본 환경에서는 `Slf4jSpyLogDelegator` 가 일반적이다. 단일 채널 `log4jdbc.log4j2` 라 카테고리별 OFF 가 불가능한 단점이 있지만, EvaluatorFilter 의 `logger.startsWith("log4jdbc.")` prefix 매칭으로 거의 메워진다. `Log4j2SpyLogDelegator` 는 5개 카테고리(`jdbc.sqlonly`/`sqltiming`/`audit`/`resultset`/`connection`) 로 분리되지만 SLF4J→log4j2 어댑터 의존성 약 1~2MB 추가가 필요하고, 그 비용이 카테고리 분리의 이득보다 큰 경우가 많다.
...
**Q. OTel 졸업의 결정적인 전환점은 무엇인가?**
A. *바인딩 파라미터 가시성* 이 분기점이다. OTel JDBC auto-instrumentation 의 기본 설정은 `db.statement` 에 평문 SQL 만 넣고 바인딩 값은 빼는데(보안 default), wrap 라이브러리는 바인딩 치환된 평문을 한 줄에 같이 찍어 디버깅이 쉽다. OTel agent 의 `otel.instrumentation.jdbc.statement-parameter-names.enabled=true` 또는 동등 옵션으로 바인딩 노출을 켜고, 그게 *팀의 보안 정책상 허용되는지* 확인한 시점이 졸업 전환점. 허용되지 않으면 wrap+OTel 병행을 유지하며 wrap 은 차단한 채 디버깅용 dev 환경에만 풀어두는 변형이 일반적이다.
