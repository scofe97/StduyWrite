...
정렬 키가 수십 개로 늘어나 `switch` 분기가 부담스러워지면 `PathBuilder.get(sortKey)`로 키-경로 매핑을 한 줄로 줄이는 변형이 있다. 다만 문자열 기반 접근이므로 *화이트리스트 검증이 더 중요해진다* — 컴파일러가 더 이상 컬럼명을 검증해 주지 않기 때문이다. 그 패턴과 트레이드오프는 [02-04 실무 변형 모음](./02-04.실무%20변형%20모음.md)에서 다룬다.
