...
- **CHECKSUM**: 운영과 로컬의 row count 와 `CHECKSUM TABLE tbl_name` 을 비교한다. 두 값이 일치하면 데이터는 같다고 본다.
- **샘플 비교**: 핵심 테이블 N 개에서 같은 키 범위를 SELECT 해 row 단위로 diff. CHECKSUM 보다 직관적이지만 시간이 든다.
...
```text
운영/dev DB 에서 원본 dump 생성
  ↓ (격리된 위치에서)
mariadb-dump 결과를 일회용 컨테이너에 import
  ↓
일회용 컨테이너에서 마스킹 SQL 적용
  ↓
mariadb-dump 로 마스킹된 결과만 재추출
  ↓
원본 dump 파일 즉시 삭제 (`shred -u`)
  ↓
마스킹된 dump 를 사내 NAS / S3 / Nexus raw repo 로 배포
```
