...
CAP 정리는 거친 분류라 운영에서는 더 정밀한 모델(예: PACELC) 을 쓰지만, 첫 결정에는 충분하다. 자세한 분산 합의·일관성 모델은 [`../04_distributed/`](../04_distributed/) 가 다룬다.
...
```
ACID 트랜잭션이 핵심?           → PostgreSQL (RDBMS)
관계 탐색이 도메인의 본질?      → Neo4j (Graph)
세션·캐시·실시간 카운터?         → Redis (Key-Value)
시계열·로그·대용량 쓰기?         → Cassandra (Wide-Column)
유연한 문서, 자주 바뀌는 스키마? → MongoDB (Document)
그 외 일반 OLTP/OLAP            → PostgreSQL
```
...
- [`../04_distributed/README.md`](../04_distributed/README.md) — CAP·합의·복제의 분산 이론
