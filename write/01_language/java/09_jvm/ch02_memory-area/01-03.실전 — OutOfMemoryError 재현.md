...
```java
// _code/ch02-memory-area/method-area/ — JDK 21 호환 변형
...
각 모듈은 `_code/ch02-memory-area/` 안에서 독립 Gradle 서브모듈로 빌드된다. 다음 명령으로 한 종류씩 실행해 OOM 메시지를 직접 확인할 수 있다.
...
각 모듈을 실행해 얻은 *실제 출력*은 본 저장소의 `_code/ch02-memory-area/results/` (자동 생성 디렉토리)에 남는다. 실행 결과가 책의 결과와 다를 때마다 *어느 JDK 버전 변경이 차이를 만들었는지*를 노트에 별도로 정리한다.
...
- [`../02-01.GC 알고리즘과 튜닝.md`](../02-01.GC%20알고리즘과%20튜닝.md) — 루트 평탄본 2부 요약, 힙 OOM의 GC 측면 처방
- [`../_code/ch02-memory-area/`](../_code/ch02-memory-area/) — 7개 OOM 재현 모듈, 한 모듈에 한 OOM
