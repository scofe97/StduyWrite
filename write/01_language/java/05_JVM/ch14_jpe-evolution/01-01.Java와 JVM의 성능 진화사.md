...
### Generational GC와 단편화

OpenJDK의 generational GC들은 "most objects die young", 곧 새로 할당된 객체 대부분이 단명한다는 원칙에 기댄다. heap collection은 live 객체를 식별하고, garbage가 차지한 공간을 회수하며, 경우에 따라 compaction으로 단편화를 줄인다. 단편화는 두 갈래다. internal 단편화는 할당된 블록이 필요보다 커서 블록 안에 낭비가 생기는 경우이고, external 단편화는 할당·해제가 반복돼 free 메모리가 비연속 블록으로 쪼개지는 경우다. external 단편화는 비효율과 할당 실패를 부른다. compaction은 객체를 옮겨 free 메모리를 단일 연속 블록으로 모으는 기법인데, CPU 비용이 크고 STW로 하면 긴 pause를 낳는다.

OpenJDK GC가 쓰는 알고리즘 부류를 정리하면 이렇다.
...
CMS는 concurrent로 pause를 줄이지만 compaction을 하지 않아 단편화가 쌓이고, 결국 compaction을 포함한 Serial Old로 full GC fallback이 일어날 수 있다. G1은 그 단편화를 풀려고 incremental compaction을 들였다. Shenandoah와 ZGC는 thread-local handshake로 STW를 최소화하며 marking·relocation·compaction을 concurrent로 수행한다. 다만 JDK 17 기준으로는 전통적 의미의 generational이 아니라 heap을 region으로 나눠 서로 다른 시점에 수집하는 방식이며, generational화 작업이 진행 중이다.

### Young Collection과 약한 세대 가설

generational heap에서 대부분의 할당은 young generation의 eden space에서 일어난다. eden이 거의 차면 할당 스레드가 allocation failure를 만나고 GC가 개입한다. 첫 young collection에서 eden은 scavenging을 거쳐 live 객체가 to survivor space로 옮겨진다. survivor는 전이 공간으로, 객체가 from과 to 사이를 복사·aging하며 왕복하다 tenuring threshold를 넘으면 old generation으로 승격된다. 장수가 입증된 객체만 승격하는 셈이고, 저자는 이를 멘토 Charlie Hunt를 빌려 "Teenage Wasteland"라 부른다.

약한 세대 가설은 두 특성으로 요약된다. 하나는 most objects die young으로, 장수 객체만 승격하면 transient나 중기 객체를 올리지 않아 장수 데이터셋이 작게 유지되고 premature promotion·단편화·evacuation failure가 억제된다. 다른 하나는 generation 유지 비용이다. young collector가 old보다 자주, 따로 돌며 live data를 옮기기 때문에, 도달 가능한 객체를 모두 마킹하려면 bookkeeping 오버헤드가 든다. 이를 write barrier로 세대 간 참조를 추적해 해결한다. young collection에는 대부분 "scavenge" 알고리즘을 쓰는데, Serial GC는 단일 GC 스레드를, Parallel·G1·CMS는 다수 GC worker 스레드를 동원한다.

### Old-Generation 회수

old 회수 알고리즘은 처리량·응답성 또는 둘의 혼합을 노린다. Serial GC는 단일 스레드 mark-sweep-compacting(MSC)을, Parallel GC는 다중 스레드 MSC를 쓴다. CMS는 대부분 concurrent marking을 하고 마킹 후 compaction 없이 in-place로 해제하다가, 단편화가 생기면 serial MSC로 fallback한다. Java 7u4에서 도입된 G1은 첫 incremental collector로, old를 단일 monolithic 회수·compaction 대신 region 단위로 점진 회수한다.

여러 young collection 뒤 old가 차오르면 full heap marking cycle이 필요하다. 트리거는 세 가지다. promotion failure, 일반 크기 객체의 승격이 old나 전체 heap을 marking 임계 너머로 밀어 올릴 때, 그리고 large object 할당(G1의 humongous allocation)이 heap occupancy를 정해진 임계 너머로 밀 때다. Shenandoah(JDK 12)와 ZGC(JDK 11)는 ultra-low-pause 컬렉터로 JDK 17에선 single-generational이며, thread-local handshake와 graceful degradation을 쓴다. graceful degradation은 앱 스레드가 GC를 돕거나 반대로 back off하게 만드는 기법이다.

### GC 스레드 구성

parallel GC threads는 start-up 시 Java 프로세스가 가진 코어 수의 일부로 계산되며 `-XX:ParallelGCThreads=<n>`로 조정한다. concurrent phase를 쓰는 GC용으로는 `-XX:ConcGCThreads=<n>`가 있다. 기본적으로 parallel 스레드 수는 코어 기반으로 자동 계산되고, concurrent 스레드는 보통 parallel의 1/4이다.

parallel 스레드를 늘리면 GC 처리량이 오르고 pause가 줄어 앱 처리량이 높아질 수 있지만, 자원을 과도하게 점유하지 않도록 주의해야 한다. concurrent 스레드를 늘리면 GC cycle이 빨라지는 대신 CPU 사용이 늘고 앱 스레드와 CPU를 두고 경쟁한다. 반대로 줄이면 CPU 사용은 낮아지나 pause가 길어질 수 있다. concurrent collector가 할당 속도를 못 따라가는 상황을 GC가 "losing the race"라 부르는데, 이때 graceful degradation이 일어나 STW 모드로 fallback하거나 앱의 할당 속도를 throttling한다. 그래서 parallel과 concurrent 스레드 수, 그리고 앱 성능 사이의 균형을 성능 테스트와 모니터링으로 찾아야 한다.
...
- [`../ch01_java-tech/01-01.JDK 구조와 바이트코드`](../ch01_java-tech/01-01.JDK%20구조와%20바이트코드.md) — JDK/JRE/JVM 구성과 bytecode의 플랫폼 독립성
- [`../ch01_java-tech/01-03.컴파일과 최적화`](../ch01_java-tech/01-03.컴파일과%20최적화.md) — javac 프런트엔드 컴파일과 JIT·Graal·AOT 백엔드 최적화
- [`../ch03_gc/01-01.GC 운영 — 로그와 튜닝`](../ch03_gc/01-01.GC%20운영%20—%20로그와%20튜닝.md) — GC 로그·jstat·튜닝 실무
- [`../ch03_gc/02-04.클래식 가비지 컬렉터`](../ch03_gc/02-04.클래식%20가비지%20컬렉터.md) — Serial·Parallel·CMS 컬렉터 상세
