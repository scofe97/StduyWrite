...
> 할당과 승격을 스레드별 전용 버퍼로 처리해 lock 경쟁을 없앤다. TLAB는 할당을, PLAB는 GC의 객체 승격을 가속하며, 둘 다 rate 기반으로 자동 resize된다.
...
Spring 앱에서 TLAB의 효과는 부팅 직후 가장 두드러진다. component scan이 수많은 `@Component`·`@Bean`을 인스턴스화하고 CGLIB·JDK dynamic proxy가 프록시 객체를 대량 생성하는데, 이 짧은 수명 객체들이 여러 스레드에서 동시에 할당된다. TLAB fast-path가 lock 경쟁을 없애 주기 때문에 이 구간의 할당 비용이 낮게 유지되고, 그만큼 부팅 지연이 줄어든다. 부팅 워밍업의 할당 패턴이 궁금하면 `-Xlog:gc+tlab=debug`로 그 구간의 TLAB resize 빈도를 보면 된다 — resize가 잦다면 스레드별 할당량이 초기 추정과 어긋났다는 신호다.



## 3. NUMA-aware GC — 가까운 메모리에서 할당하기

> eden을 NUMA 노드별로 나눠 스레드가 자기 노드의 메모리에서 할당하게 한다. 노드 간 hop을 없애 메모리 접근 latency를 줄이는 게 목적이다.
...
pause-time 예측 가능성은 G1의 핵심이다. GC 작업을 작은 chunk로 쪼개 점진 처리하고(incremental collection), CMS의 concurrent marking을 SATB(snapshot-at-the-beginning) marking으로 강화한다. 예측이 가능한 이유는 G1이 과거 실행 history를 근거로 다음 작업의 시간을 추정하기 때문이다. 추정하는 값은 다음 다섯 가지다:

1. single region copy time. region 하나를 evacuate하는 데 걸리는 시간으로, collection set 크기를 정하는 기본 단위가 된다.
2. concurrent marking time. IHOP(`-XX:InitiatingHeapOccupancyPercent`)를 동적으로 조정해, old 점유율이 어느 선에 닿으면 marking을 시작할지 정한다.
3. mixed GC time. young과 old region을 함께 수집하는 mixed collection의 소요를 추정해, 한 번에 얼마나 많은 old region을 넣을지 가늠한다.
4. young GC time. `-XX:MaxGCPauseTimeMillis` 목표에 맞춰 young 크기를 키우거나 줄인다 — young이 크면 회수량은 늘지만 pause도 길어진다.
5. 회수 가능 공간. 각 region이 비워 줄 free space를 추정해, garbage가 많은 region부터 collection set에 넣는다.

설계상 멀티스레딩을 지원해 young/old collection을 병렬화하고 marking phase에 concurrent 스레드를 쓴다.
...
이 절이 실무와 닿는 지점은 단순하다. Spring Boot 앱은 GC를 따로 지정하지 않으면 JVM 기본 GC를 그대로 상속하는데, JDK 9부터 그 기본이 G1이다. 즉 대부분의 Spring Boot 서비스가 별도 설정 없이 이 region 기반 컬렉터 위에서 돈다. 그래서 큰 응답 페이로드나 대용량 캐시 엔트리가 humongous object 임계(region의 50%)를 넘기면, 의도치 않게 old에 직접 할당돼 mixed collection 부담을 키울 수 있다 — region 크기(`-XX:G1HeapRegionSize`)와 큰 객체의 관계를 함께 봐야 하는 이유다.



## 5. G1 튜닝 — 응답성과 throughput

> `MaxGCPauseMillis`는 soft goal이라 못 맞출 때가 있다. 응답성·throughput·marking 시점을 각각 다른 손잡이로 잡는데, 손잡이마다 반대급부가 있다.
...
한 marking 사이클의 pause 흐름은 아래 도식처럼 흐른다. young GC가 반복되다 IHOP를 넘으면 marking 단계(initial-mark → concurrent mark → remark → cleanup)가 끼어들고, 그 뒤 mixed GC가 7번 돌며 old region을 점진 회수한다. 마지막 mixed의 pause가 가장 길어 SLO를 넘기 쉬운데, 바로 그 지점이 튜닝의 출발점이다.

![G1 pause histogram — 한 marking 사이클의 pause 흐름](./_assets/05-01-g1-pause-histogram.svg)
...
throughput SLO(예: GC overhead ≤ 5%)를 못 맞출 때는 다른 손잡이를 쓴다. 핵심은 "비싼 region을 수집에서 빼는" 두 가지다:

1. `-XX:G1MixedGCLiveThresholdPercent=<p>`로 live data 비율이 cutoff를 넘는 region을 mixed collection set에서 제외한다. live가 많은 region은 evacuate 비용이 큰데 회수량은 적어, 빼는 편이 throughput에 이득이다.
2. `-XX:G1HeapWastePercent=<p>`로 heap의 일정 비율을 미수집(heap waste)으로 허용한다. marking이 region을 회수량 순으로 정렬했을 때, 그 배열 끝의 비싼 region을 아예 건드리지 않는 효과다.

JDK 11~17은 여기에 abortable mixed collection, unused memory 반환(`G1PeriodicGCInterval`), NUMA-awareness(JDK 14), adaptive IHOP를 더해 수동 튜닝의 필요 자체를 줄였다.
...
> TLAB/PLAB의 할당·승격 가속, region이 G1의 작업 단위라는 점, soft goal과 튜닝 손잡이 — 이 셋을 자기 말로 설명할 수 있으면 이 장은 끝이다.
...
- [`./05-02.ZGC 심화와 워크로드별 GC 선택`](./05-02.ZGC%20심화와%20워크로드별%20GC%20선택.md) — 같은 장 후반부: ZGC·미래 트렌드·OLAP/OLTP/HTAP
- [`../ch01_java-tech/01-01.Java와 JVM의 성능 진화사`](../ch01_java-tech/03-01.Java와%20JVM의%20성능%20진화사.md) — generational GC·NUMA-aware GC·G1 도입 연대기
- `../ch05_efficient-concurrency/06-02.동기화와 NUMA, JMH 벤치마킹` <!-- 링크 끊김(2026-08): ../ch05_efficient-concurrency/06-02.동기화와 NUMA, JMH 벤치마킹.md --> — NUMA traffic 패턴과 memory controller
- [`../ch02_automatic-memory-management/02-06.클래식 가비지 컬렉터`](./02-06.클래식%20가비지%20컬렉터.md) — Serial·Parallel·CMS 컬렉터 기초
