...
---
> 본 노트는 GC *알고리즘*이 아니라 GC *운영*을 다룬다. 정독 노트 02-03~02-10이 "어떤 알고리즘이 어떤 트레이드오프를 갖는가"를 정리했다면, 본 노트는 **운영 시점에 GC가 무엇을 말하는지를 읽는 법**(GC 로그)과 **그 결과로 무엇을 돌릴 수 있는지**(튜닝 파라미터)를 다룬다. 파일 번호상 앞에 있지만, 읽기 순서는 GC 본문 뒤가 자연스럽다.
>
> 본 절을 한 줄로 압축하면 — **GC 튜닝의 첫 단계는 알고리즘을 아는 일이고, 두 번째 단계는 GC 로그가 그것을 어떻게 보여 주는지를 읽는 일**이다.

## 0. 전제 — 로그를 읽기 전에 아는 개념

> 이 노트는 운영에 집중하므로, GC 개념은 한 줄씩만 짚고 상세는 옆 칸 정독 노트에 맡긴다.
...
| Promotion Failure | 승격하려는데 Old가 꽉 차 실패 → Full GC 유발 | [02-10](./02-11.%EC%8B%A4%EC%A0%84%20%E2%80%94%20%EB%A9%94%EB%AA%A8%EB%A6%AC%20%ED%95%A0%EB%8B%B9%EA%B3%BC%20%ED%9A%8C%EC%88%98%20%EC%A0%84%EB%9E%B5.md) |
| GC 종류 (G1·ZGC·Shenandoah 등) | 위 일을 어떤 알고리즘으로 하느냐의 차이 | [02-06](./02-06.%ED%81%B4%EB%9E%98%EC%8B%9D%20%EA%B0%80%EB%B9%84%EC%A7%80%20%EC%BB%AC%EB%A0%89%ED%84%B0.md)·[02-07](./02-08.%EC%A0%80%EC%A7%80%EC%97%B0%20%EA%B0%80%EB%B9%84%EC%A7%80%20%EC%BB%AC%EB%A0%89%ED%84%B0.md)·[02-09](./02-10.GC%20%EC%84%A0%ED%83%9D%ED%95%98%EA%B8%B0.md) |

> STW(Stop-The-World)는 1절 로그 예시 직후에 정의한다 — 로그의 마지막 숫자가 바로 STW 시간이라 그 자리에서 보는 게 자연스럽다.



## 1. GC 로그 분석

> `-Xlog:gc*`로 남긴 로그에서 일시 정지 시간·빈도·힙 회수량을 읽어 GC 문제를 진단한다.
...
> 각 컬럼의 의미(`S0/S1`=Survivor, `E`=Eden, `O`=Old 점유율, `YGC/FGC`=Young/Full GC 횟수 등)는 [02-02.Java 성능 — JMH와 측정 방법론](./02-02.Java%20%EC%84%B1%EB%8A%A5%20%E2%80%94%20JMH%EC%99%80%20%EC%B8%A1%EC%A0%95%20%EB%B0%A9%EB%B2%95%EB%A1%A0.md)이 자세히 다룬다. 본 노트는 이 출력을 *진단에 어떻게 쓰는지*에 집중한다.



## 2. GC 튜닝 파라미터

> 튜닝은 측정 → 분석 → 조정 순서를 지키는 것이 원칙이며, 힙 크기·일시 정지 목표·세대 비율이 주요 조정 대상이다.

**GC 튜닝의 기본 원칙은 측정 → 분석 → 조정의 순서를 지키는 것이다.** 

먼저 GC 로그와 프로파일링으로 문제를 정확히 파악한 뒤 파라미터를 조정한다. 이 노트 전체를 한 장으로 묶으면, 1절의 로그 관측에서 시작해 지표를 읽고 증상을 진단해 처방을 적용한 뒤 다시 로그로 재측정하는 *닫힌 사이클*이 된다.
...
- 다만 `-Xms=-Xmx` 고정이 정석인 것은 *메모리가 어차피 이 JVM 전용*(전용 서버·고정 limit 컨테이너)일 때다. *여러 JVM이 한 호스트 메모리를 나눠 쓰거나 클라우드에서 점유 자체가 비용*이면, 큰 힙을 상시 점유하는 게 낭비가 되므로 이야기가 달라진다 — 이때는 유휴 힙을 OS에 *반납(uncommit)* 하는 기능(G1의 주기적 동시 GC, ZGC·Shenandoah의 uncommit)으로 "지연 vs 메모리 절약"을 절충한다. 그 동작과 옵션은 [05-01](./05-01.TLAB%C2%B7PLAB%C2%B7NUMA-aware%20GC%EC%99%80%20G1%20%EC%8B%AC%ED%99%94.md)·[05-02](./05-02.ZGC%20%EC%8B%AC%ED%99%94%EC%99%80%20%EC%9B%8C%ED%81%AC%EB%A1%9C%EB%93%9C%EB%B3%84%20GC%20%EC%84%A0%ED%83%9D.md)가 다룬다.

> **Spring 부트 앱도 같다.** 이 튜닝 옵션은 결국 `java -jar app.jar`로 뜨는 JVM에 거는 것이라, Spring 부트 실행 프로세스에 그대로 적용된다 — 보통 컨테이너로 배포하므로 `-XX:MaxRAMPercentage`로 Pod 메모리 한도를 인식시키고, 트래픽이 정상 상태에서 힙을 다 쓰는 서버 특성상 `-Xms`와 `-Xmx`를 같게 잡는 게 정석이다. 다만 *어느 객체가 GC를 압박하는가*는 순수 JVM 관점만으로는 안 보인다 — Spring 빈·세션·캐시·커넥션 풀이 흔한 출처라, 힙 덤프 분석은 [02-02.Java 성능](./02-02.Java%20%EC%84%B1%EB%8A%A5%20%E2%80%94%20JMH%EC%99%80%20%EC%B8%A1%EC%A0%95%20%EB%B0%A9%EB%B2%95%EB%A1%A0.md)의 도구와 함께 본다.
...
- [02-08.저지연 가비지 컬렉터.md](./02-08.%EC%A0%80%EC%A7%80%EC%97%B0%20%EA%B0%80%EB%B9%84%EC%A7%80%20%EC%BB%AC%EB%A0%89%ED%84%B0.md) — Shenandoah/ZGC의 저지연 설계
- [02-10.GC 선택하기.md](./02-10.GC%20%EC%84%A0%ED%83%9D%ED%95%98%EA%B8%B0.md) — 워크로드별 GC 선택
- [02-12.마치며.md](./02-12.마치며.md) — 3장 전체 요약과 GC 구현체 비교표
