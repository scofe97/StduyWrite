...
ZGC는 여섯 가지 트리거를 가진다. **Timer**는 `-XX:ZCollectionInterval=<sec>`로 정한 간격마다 GC를 검사하며 기본값 0은 비활성이다. **Warm-up**은 ramp-up 단계에서 heap occupancy가 soft_max_capacity의 10%·20%·30%에 닿으면 트리거해 GC를 미리 준비시킨다(`-XX:SoftMaxHeapSize`로 조정). **High Allocation Rate**는 할당률과 free 메모리로 OOM까지의 시간을 추정해 그것이 GC cycle 예상 시간보다 짧으면 트리거하며, `UseDynamicNumberOfGCThreads` 설정에 따라 worker 수를 동적 계산하거나(Dynamic) 고정한다(Static, headroom 포함). **Allocation Stall**은 공간 부족으로 할당이 막히고 stall이 관측되면 반응적으로 트리거하고 SoftReference를 비운다. **High Usage**는 free 메모리가 5% 이하로 떨어지면 예방적으로 트리거하는데, 할당률이 낮아 allocation rate rule이 발동하지 않을 때 유용하다. **Proactive**는 heuristic으로 heap에 여유가 있어도 이득이 되면 GC를 미리 돌린다(`-XX:+ZProactive`, 기본 활성). 각 트리거는 `GC(N) Garbage Collection (Timer)`처럼 로그에 이유를 남긴다.
...
- [`./01-01.TLAB·PLAB·NUMA-aware GC와 G1 심화`](./01-01.TLAB·PLAB·NUMA-aware%20GC와%20G1%20심화.md) — 같은 장 전반부: TLAB/PLAB·NUMA·G1
- [`../ch14_jpe-evolution/01-01.Java와 JVM의 성능 진화사`](../ch14_jpe-evolution/01-01.Java와%20JVM의%20성능%20진화사.md) — ZGC·Shenandoah·STW 도입 연대기
- [`../ch02_automatic-memory-management/02-07.저지연 가비지 컬렉터`](../ch02_automatic-memory-management/02-07.저지연%20가비지%20컬렉터.md) — 《밑바닥》 쪽 저지연 GC(ZGC·Shenandoah)
- [`../ch02_automatic-memory-management/02-08.GC 선택하기`](../ch02_automatic-memory-management/02-08.GC%20선택하기.md) — 《밑바닥》 쪽 GC 선택 기준
