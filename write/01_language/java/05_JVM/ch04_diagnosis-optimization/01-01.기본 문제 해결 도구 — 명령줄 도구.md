...
`java.lang.Thread.State`가 `BLOCKED`이고 `waiting to lock`과 `locked`이 두 스레드 사이에서 *교차*하면 전형적인 데드락입니다. `Found one Java-level deadlock` 한 줄이 결정적 증거입니다 — 이 줄이 나오면 코드에서 두 락의 획득 순서를 통일하는 쪽으로 수정 방향을 잡습니다. JConsole·VisualVM의 데드락 탐지 버튼도 내부적으로 이 같은 스레드 덤프 분석을 GUI로 감싼 것입니다(상세는 [01-02 §2](#))。
...
- [01-02. 시각화 문제 해결 도구](01-02.시각화%20문제%20해결%20도구.md) § "JConsole" — `jstack`이 콘솔로 뜨는 데드락·스레드 상태를 GUI로 보는 짝
