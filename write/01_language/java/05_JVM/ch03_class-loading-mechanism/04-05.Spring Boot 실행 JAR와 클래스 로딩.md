...
`layers.idx`가 실제 Dockerfile에서 어떻게 쓰이는지 짚으면, 이 색인이 왜 JAR 안에 들어 있는지가 분명해집니다. fat jar를 `COPY app.jar` 한 줄로 이미지에 넣으면 **JAR 전체가 한 레이어**가 됩니다. 그러면 내 코드 몇 KB만 고쳐도 무거운 의존성까지 같은 레이어에 묶여 통째로 다시 빌드됩니다. Docker 레이어 캐시는 한 레이어가 바뀌면 그 위 레이어까지 전부 무효화하므로(캐시 미스가 위로 전파), 한 덩어리 JAR는 캐시 효율이 나쁩니다.
