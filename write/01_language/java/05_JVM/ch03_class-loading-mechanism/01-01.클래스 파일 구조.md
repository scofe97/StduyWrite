...
`max_stack`과 `max_locals`가 런타임이 아니라 *컴파일 타임에 확정*되는 이유가 여기 있습니다. 컴파일러가 메서드의 모든 실행 경로를 정적 분석해 스택 깊이와 지역 변수 수를 미리 계산하기 때문입니다. JVM은 이 값을 보고 스택 프레임을 *정확히 필요한 크기만큼* 미리 할당합니다.

`exception_table`은 자바의 `try-catch`가 별도 분기 명령어 없이 동작하는 원리입니다. 바이트코드 오프셋 범위(`start_pc`~`end_pc`)와 핸들러 위치(`handler_pc`)를 테이블에 등록해두면, JVM이 예외 발생 시 이 테이블을 순서대로 검색해 핸들러로 점프합니다. 바이트코드 명령어 집합 자체는 예외를 모릅니다(→ [01-02 바이트코드 명령어](01-02.바이트코드%20명령어.md) §예외 처리).
