...
# 가상 머신 실행 서브시스템
---
> JVM이 메서드를 어떻게 호출하고 실행하는지 이해한다. 이 글은 `ch03_class-loading-mechanism/`의 두 번째 개관으로, 앞 글이 `.class`라는 입력 형식을 잡았다면 여기서는 그 바이트코드가 스택 프레임 위에서 호출·디스패치·실행되는 흐름을 잡는다.
>
> 본 요약을 한 줄로 압축하면 — **자바 메서드 호출은 스택 프레임 push/pop과 다섯 invoke 명령어의 분기로 정의되며, `invokedynamic`이 그 결정 시점을 *컴파일 타임에서 첫 호출 시점으로* 늦춰 람다·동적 언어 지원의 기반이 됐다**. `02-NN`이 클래스를 언제·누가 로딩하는가를 다룬다면, `03-NN`은 로딩된 코드가 어떻게 실행되는가를 다룬다.
...
- [`./00-개관.JDK 구조와 바이트코드.md`](./00-개관.JDK%20구조와%20바이트코드.md) — 본 요약의 전제, 클래스 파일과 바이트코드 명령어
- [`../ch04_compilation-optimization/02-01.JIT 컴파일러 — 인터프리터와 계층형 컴파일.md`](../ch04_compilation-optimization/02-01.JIT%20컴파일러%20—%20인터프리터와%20계층형%20컴파일.md) — 본 요약 다음 단계, JIT가 어떻게 인보크 비용을 줄이는지
