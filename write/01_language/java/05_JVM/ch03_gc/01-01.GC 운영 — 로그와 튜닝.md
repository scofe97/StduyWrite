...
---
> 본 노트는 GC *알고리즘*이 아니라 GC *운영*을 다룬다. 정독 노트 01-02~01-07이 "어떤 알고리즘이 어떤 트레이드오프를 갖는가"를 정리했다면, 본 노트는 **운영 시점에 GC가 무엇을 말하는지를 읽는 법**(GC 로그)과 **그 결과로 무엇을 돌릴 수 있는지**(튜닝 파라미터)를 다룬다. 
>
...
| 용어 | 한 줄 정의 | 자세히 |
|------|----------|--------|
| Young / Old 세대 | 갓 만든 객체는 Young, 오래 살아남으면 Old로 | [02-01](./02-01.대상이%20죽었는가.md) |
| Minor GC / Full GC | Young만 청소 = Minor(짧음), Old까지 = Full(무거움) | [02-02](./02-02.가비지%20컬렉션%20알고리즘.md) |
| 승격 (Promotion) | Young에서 살아남은 객체를 Old로 옮기는 것 | [02-02](./02-02.가비지%20컬렉션%20알고리즘.md) |
| Promotion Failure | 승격하려는데 Old가 꽉 차 실패 → Full GC 유발 | [02-07](./02-07.실전%20—%20메모리%20할당과%20회수%20전략.md) |
| GC 종류 (G1·ZGC·Shenandoah 등) | 위 일을 어떤 알고리즘으로 하느냐의 차이 | [02-04](./02-04.클래식%20가비지%20컬렉터.md)·[02-05](./02-05.저지연%20가비지%20컬렉터.md)·[02-06](./02-06.GC%20선택하기.md) |
...
> 각 컬럼의 의미(`S0/S1`=Survivor, `E`=Eden, `O`=Old 점유율, `YGC/FGC`=Young/Full GC 횟수 등)는 [01-02.Java 성능 — JMH와 측정 방법론](./01-02.Java%20성능%20—%20JMH와%20측정%20방법론.md) §4-2가 자세히 다룬다. 본 노트는 이 출력을 *진단에 어떻게 쓰는지*에 집중한다.
...
![GC 운영 사이클 — 관측·진단·튜닝·재측정](_assets/01-01-gc-ops-cycle.svg)
...
> **Spring 부트 앱도 같다.** 이 튜닝 옵션은 결국 `java -jar app.jar`로 뜨는 JVM에 거는 것이라, Spring 부트 실행 프로세스에 그대로 적용된다 — 보통 컨테이너로 배포하므로 `-XX:MaxRAMPercentage`로 Pod 메모리 한도를 인식시키고, 트래픽이 정상 상태에서 힙을 다 쓰는 서버 특성상 `-Xms`와 `-Xmx`를 같게 잡는 게 정석이다. 다만 *어느 객체가 GC를 압박하는가*는 순수 JVM 관점만으로는 안 보인다 — Spring 빈·세션·캐시·커넥션 풀이 흔한 출처라, 힙 덤프 분석은 [01-02.Java 성능](./01-02.Java%20성능%20—%20JMH와%20측정%20방법론.md) §4의 도구와 함께 본다.
