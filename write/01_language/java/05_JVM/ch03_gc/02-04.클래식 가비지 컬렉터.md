...
```mermaid
graph TB
    subgraph Young["신세대 컬렉터"]
        Serial["Serial<br/>단일 스레드 STW<br/>마크-카피"]
        ParNew["ParNew<br/>다중 스레드 STW<br/>마크-카피"]
        PScav["Parallel Scavenge<br/>다중 스레드 STW<br/>마크-카피"]
    end

    subgraph Old["구세대 컬렉터"]
        SOld["Serial Old<br/>단일 스레드 STW<br/>마크-컴팩트"]
        POld["Parallel Old<br/>다중 스레드 STW<br/>마크-컴팩트"]
        CMS["CMS<br/>동시 마크-스윕<br/>(JDK 14 제거)"]
    end
    ...
    PScav --- POld
    PScav -.fallback.- SOld
    ParNew -.fallback.- SOld

    G1["G1 (단독)<br/>신세대 + 구세대<br/>Region 기반 마크-컴팩트"]

    style Serial fill:#ADD8E6,stroke:#333,color:#000
    style ParNew fill:#ADD8E6,stroke:#333,color:#000
    style PScav fill:#ADD8E6,stroke:#333,color:#000
    style SOld fill:#FFE4B5,stroke:#333,color:#000
    style POld fill:#FFE4B5,stroke:#333,color:#000
    style CMS fill:#FFE4B5,stroke:#333,color:#000
    style G1 fill:#90EE90,stroke:#333,color:#000
```
...
```mermaid
graph LR
    s1["① 초기 마크<br/>(STW)"] --> s2["② 동시 마크<br/>(concurrent)"]
    s2 --> s3["③ 재마크<br/>(STW, SATB)"]
    s3 --> s4["④ 선별 회수<br/>(STW, 마크-컴팩트)"]
...
- [02-03.핫스팟 알고리즘 상세 구현](./02-03.핫스팟%20알고리즘%20상세%20구현.md) — 본 컬렉터들이 공유하는 다섯 부품의 정의
- [02-05.저지연 가비지 컬렉터](./02-05.저지연%20가비지%20컬렉터.md) — G1의 영역 모델을 더 끌어내려 *동시 이동*까지 푼 ZGC·Shenandoah
- [02-06.GC 선택하기](./02-06.GC%20선택하기.md) — 본 노트의 6+1종을 워크로드별로 *어느 컬렉터를 고를지*의 의사결정 트리
