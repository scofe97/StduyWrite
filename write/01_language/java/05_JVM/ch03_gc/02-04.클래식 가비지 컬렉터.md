...
![클래식 GC 컬렉터 분류 좌표](_assets/02-04-collector-map.svg)
...
![Serial과 Serial Old 흐름](_assets/02-04-serial-flow.svg)
...
![ParNew와 CMS의 신세대 짝 흐름](_assets/02-04-parnew-latency.svg)
...
![Parallel Scavenge와 Parallel Old 흐름](_assets/02-04-parallel-throughput.svg)
...
> **Concurrent Mode Failure는 언제 나는가.** 두 가지 경로가 있다. 하나는 위 단편화로 Old 영역에 *연속된 빈 공간*이 부족해 객체를 못 넣는 경우다. 다른 하나는 *타이밍 경합* — 애플리케이션이 객체를 Old로 승격시키는 속도가 CMS의 *동시 회수 속도*를 앞질러, 동시 스윕이 끝나기 전에 Old가 먼저 가득 차는 경우다. 어느 쪽이든 CMS는 동시 회수를 *포기*하고 Serial Old(단일 스레드 마크-컴팩트)로 폴백해 *전체 STW Full GC*를 돈다. 동시성으로 줄여 둔 일시 정지가 이 순간 한꺼번에 터지므로, GC 로그에 `concurrent mode failure`가 찍히면 *운영 사고 신호*로 본다.

CMS는 JDK 9 deprecated, **JDK 14에서 제거**됐다. JDK 21에서 `-XX:+UseConcMarkSweepGC` 옵션은 *에러로 종료*하거나 *경고 후 G1 대체*된다.

![CMS 동시 마크-스윕 흐름](_assets/02-04-cms-flow.svg)
...
![G1 Region 기반 회수 흐름](_assets/02-04-g1-region.svg)
...
CMS의 실패가 ZGC·Shenandoah로 가는 길을 열었다. 다음 노트(02-05)는 *저지연 가비지 컬렉터* — 일시 정지를 *10ms 이하*로 끌어내린 ZGC와 Shenandoah — 를 다룬다.
