...
# 저지연 가비지 컬렉터
---
> §3.5의 클래식 컬렉터들은 *수십~수백 밀리초*의 일시 정지를 받아들였다. §3.6의 두 컬렉터, **Shenandoah**와 **ZGC**는 그 한계를 *10ms 이하*로 끌어내린다. 둘은 다른 회사(Red Hat / Oracle)가 다른 길로 풀어낸 같은 문제 — *동시 마킹*뿐 아니라 *동시 정리·동시 이동*까지 가능하게 만들기 — 의 답이다. 본 노트는 두 컬렉터의 핵심 아이디어와 차이를 정리한다. 본 절을 한 줄로 압축하면 — **저지연 GC의 본질은 "객체를 옮기는 동안에도 애플리케이션이 그 객체에 안전하게 접근하게 하는" 트릭**이며, Shenandoah는 *Brooks 포인터*로, ZGC는 *colored 포인터*로 같은 문제를 다르게 풀었다.
...
저지연 GC의 야심은 *컴팩트조차 동시에* 돌리는 것이다. 그러려면 *애플리케이션이 객체에 접근하는 동안* GC가 *그 객체를 옮길 수 있어야* 한다. 정상적으로는 데이터 정합성이 깨진다. 두 컬렉터는 그 정합성을 *읽기 장벽* (Load Barrier)으로 보장한다.
...
```mermaid
graph LR
    read["객체 필드 읽기"] --> rb["Read Barrier<br/>forwarding pointer 확인"]
    rb -->|"이동 안 됨<br/>(fwd→self)"| self["원본 위치에서 읽기"]
    rb -->|"이동 중/완료<br/>(fwd→new)"| moved["새 위치에서 읽기"]

    style read fill:#E0E0E0,stroke:#333,color:#000
    style rb fill:#FFE4B5,stroke:#333,color:#000
    style self fill:#90EE90,stroke:#333,color:#000
    style moved fill:#ADD8E6,stroke:#333,color:#000
```
...
```mermaid
graph LR
    load["참조 읽기"] --> lb["Load Barrier<br/>컬러 비트 검사"]
    lb -->|"최신 (fast path)"| ok["그대로 사용"]
    lb -->|"낡음 (stale)"| remap["remap → 최신 주소로 갱신"]
    remap --> ok

    style load fill:#E0E0E0,stroke:#333,color:#000
    style lb fill:#FFE4B5,stroke:#333,color:#000
    style ok fill:#90EE90,stroke:#333,color:#000
    style remap fill:#FFB6C1,stroke:#333,color:#000
```
...
| Barrier 종류 | Read Barrier (필드 읽기마다) | Load Barrier (참조 읽기마다) |
...
다음 노트(01-06)는 *어느 컬렉터를 어떤 워크로드에 쓸지* — JDK 버전별 디폴트와 의사결정 트리 — 를 다룬다.
...
- [02-04.클래식 가비지 컬렉터](./02-04.클래식%20가비지%20컬렉터.md) — Shenandoah·ZGC가 *극복하려 한* 클래식 컬렉터들의 일시 정지 한계
- [02-06.GC 선택하기](./02-06.GC%20선택하기.md) — 저지연 GC가 적합한 워크로드와 그렇지 않은 워크로드의 의사결정
- [02-07.실전 — 메모리 할당과 회수 전략](./02-07.실전%20—%20메모리%20할당과%20회수%20전략.md) — 동시 이동이 가능한 GC에서 *할당 위치*가 갖는 의미
