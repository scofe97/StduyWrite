...
```mermaid
graph TB
    new["new 객체"] --> q1{"PretenureSizeThreshold<br/>초과하는 큰 객체?"}
    q1 -->|"예 (규칙 2)"| old["구세대 직행"]
    q1 -->|"아니오 (규칙 1)"| eden["Eden 할당"]
    eden --> q2{"Minor GC 생존 후<br/>age 임계값 도달?"}
    q2 -->|"또는 동적 age (규칙 3·4)"| old
    q2 -->|"아니오"| surv["Survivor 잔류 (age++)"]
    surv --> q2
    ...
    style surv fill:#FFD0A0,stroke:#333,color:#000
...
```mermaid
graph TB
    alloc["객체 할당 요청"] --> q1{"큰 객체인가?"}
    q1 -->|"예"| cas["공유 Eden에서 CAS 할당"]
    q1 -->|"아니오"| q2{"TLAB에 공간 있나?"}
    q2 -->|"있음 (fast path)"| bump["TLAB 안에서<br/>bump-the-pointer (동기화 없음)"]
    q2 -->|"없음"| refill["새 TLAB 요청 (1회 동기화)"]
    refill --> bump

    style alloc fill:#E0E0E0,stroke:#333,color:#000
    style q1 fill:#FFE4B5,stroke:#333,color:#000
    style q2 fill:#FFE4B5,stroke:#333,color:#000
    style bump fill:#90EE90,stroke:#333,color:#000
    style refill fill:#FFD0A0,stroke:#333,color:#000
    style cas fill:#FFB6C1,stroke:#333,color:#000
```
