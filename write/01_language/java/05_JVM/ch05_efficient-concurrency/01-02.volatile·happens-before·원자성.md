...
두 스레드가 동시에 `count`(값 5)를 읽으면 둘 다 5를 보고, 각자 6을 써서 결과가 7이 아니라 6이 됩니다. 

- 이런 복합 연산의 원자성은 `synchronized`, `java.util.concurrent.atomic`의 CAS 기반 클래스(`AtomicInteger` 등), 또는 락으로 풀어야 합니다. 
- 세 도구의 자세한 비교는 [JMM 심화 노트](./05-01.Java%20Memory%20Model%20심화.md)에서 다룹니다.
...
- `initialized`가 `volatile`이 아니면 (1)과 (2)가 재정렬돼, 사용 스레드가 `initialized == true`를 보는 시점에 `config`가 아직 준비되지 않았을 수 있습니다. 
- `volatile`이 (2) 앞의 쓰기를 (2) 뒤로 넘기지 못하게 막아 이 문제를 없앱니다. 메모리 펜스의 동작은 [JMM 심화 노트](./05-01.Java%20Memory%20Model%20심화.md)에서 더 깊게 봅니다.
