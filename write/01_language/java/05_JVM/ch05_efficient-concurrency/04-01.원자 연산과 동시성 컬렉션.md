...
- [`./03-02.메모리 가시성과 동기화.md`](./03-02.메모리%20가시성과%20동기화.md) — `synchronized`와 대비되는 *lock-free* 모델의 가시성 모델
- [`./03-03.생산자-소비자 패턴.md`](./03-03.생산자-소비자%20패턴.md) — `BlockingQueue`가 본 노트의 동시성 컬렉션 위에서 어떻게 추상화되는지
- [`./04-03.ThreadLocal과 동시성 라이브러리.md`](./04-03.ThreadLocal과%20동시성%20라이브러리.md) — *공유를 없애는* 다른 접근법
