...
status: draft
related:
  - ./06-01.성능 엔지니어링과 하드웨어·메모리 모델.md
...
# 동기화와 NUMA, JMH 벤치마킹
---
> relaxed 메모리 모델이 연 순서 이탈을 barrier·fence·volatile이 다시 묶고, JMM이 atomicity와 happens-before로 그 보장을 정의한다. NUMA는 메모리의 물리적 거리를 traffic 패턴으로 드러내며, 이 모든 저수준 동작은 JMH(JEP 230)로 JIT·loop 왜곡을 통제해야 신뢰성 있게 측정된다.
...
> 프로세서마다 가까운 메모리와 먼 메모리가 갈리는 구조다. memory controller가 근접성 기반으로 할당해 cross-node traffic을 줄이고, local traffic을 늘리는 것이 NUMA 효율의 핵심이다.
...
NUMA에는 세 가지 traffic 패턴이 있다:

1. local traffic. 프로세서가 자기 local 메모리 뱅크에서 데이터를 가져오는 경우로, interconnect를 거치지 않아 가장 빠르다.
2. cross-traffic. 다른 프로세서의 메모리 뱅크에 접근하는 경우로, cross-node 이동이라 interconnect latency가 따른다.
3. interleaved memory. 연속된 주소를 여러 뱅크에 흩어 부하를 균형 잡는데, 분산되는 만큼 cross-node traffic이 늘 수 있다.
...
> 성능 분석은 두 방향에서 접근한다. bottom-up은 하드웨어·JVM 같은 저수준에서 위로, top-down은 고수준 목표에서 병목으로 내려간다. 둘 다 monitoring → profiling → analysis → tuning 순환으로 수렴한다.
...
1. bottom-up은 저수준 컴포넌트에서 시작해 위로 올라간다. 하드웨어 nuance(코어·cache 아키텍처가 GC·스레드 스케줄링에 미치는 영향)에서 JVM의 GC, OS 스레드 관리, 컨테이너 환경(Kubernetes resource allocation, container 메모리 limit 대비 heap tuning), Netty 같은 프레임워크를 차례로 본다. 어디서 비용이 나는지 모를 때 바닥부터 훑는 방식이다.
2. top-down은 고수준 목표에서 하향한다. 잘 아는 "known-knowns"(workflow·access pattern·tunable)에서 출발해 잘 모르는 "known-unknowns"(병목·예측 불가 동작)를 드러내고, 이를 다시 "known-knowns"로 바꾼다. SoW(Statement of Work) 아래에서 특히 유효하며, OLTP DB 시스템에서는 call chain traversal time(함수 호출·반환의 누적 시간)과 tail latency(99th percentile 이상의 worst-case)를 줄이는 것이 목표가 된다.
...
> CAS는 많은 동기화 primitive의 근간 연산이다. JMH로 CAS overhead를 재고 perfasm으로 assembly를 들여다보면, Arm LSE가 LL/SC 대비 contention을 어떻게 줄이는지 하드웨어 레벨에서 확인할 수 있다.
...
> 순서 보장 도구(barrier·fence·volatile), NUMA의 traffic 패턴, JMH가 JIT·loop 왜곡을 통제하는 방법 — 이 셋을 자기 말로 설명할 수 있으면 이 장은 끝이다.
...
- [`./06-01.성능 엔지니어링과 하드웨어·메모리 모델`](./06-01.성능%20엔지니어링과%20하드웨어·메모리%20모델.md) — 같은 장 전반부: 메트릭·STW·메모리 모델·SMT
- [`../ch01_java-tech/01-01.Java와 JVM의 성능 진화사`](../ch01_java-tech/03-01.Java와%20JVM의%20성능%20진화사.md) — NUMA-aware GC·Parallel GC 도입
- [`../ch02_automatic-memory-management/02-02.Java 성능 — JMH와 측정 방법론`](../ch02_automatic-memory-management/02-02.Java%20성능%20—%20JMH와%20측정%20방법론.md) — 《밑바닥》 쪽 JMH·측정 방법론
- [`./03-02.원자 연산과 동시성 컬렉션`](./03-02.원자%20연산과%20동시성%20컬렉션.md) — 《자바 동시성》 쪽 CAS·atomic·lock-free 컬렉션 기초
- [`./01-02.volatile·happens-before·원자성`](./01-02.volatile·happens-before·원자성.md) — happens-before·메모리 펜스·DCL·safe publication
