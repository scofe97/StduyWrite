...
---
> Java 동기화의 근간은 모든 객체에 내재된 monitor lock으로, 경쟁이 없으면 thin lock, contention이 감지되면 OS mutex를 쓰는 heavyweight lock으로 inflate된다. JDK 11~17의 contended locking 개선(JEP 143)과 Project Loom의 virtual thread(carrier 위 continuation)가 그 위에서 멀티스레드 성능과 확장성을 끌어올린다.
...
> 모든 객체에 내재된 monitor lock이 배타 접근을 보장한다. 경쟁이 없으면 가벼운 thin lock으로, contention이 감지되면 OS mutex를 쓰는 heavyweight lock으로 inflate된다.
...
> 락은 API와 VM 두 레벨에서 발전했다. API는 `ReentrantLock`·`StampedLock`으로 `synchronized`보다 유연해졌고, VM은 biased locking·lock elision·coarsening·adaptive spinning으로 락 비용을 줄였다.
...
HotSpot VM 레벨에서는 biased locking·lock elision·lock coarsening·adaptive spinning 네 최적화로 락 비용을 줄인다. 이 다섯 기법(스핀·제거·굵게·경량·편향)의 동작 원리·Mark Word·락 팽창 흐름은 《밑바닥》 정독본 [`02-03.락 최적화`](./02-03.락%20최적화%20—%20스핀·제거·굵게·경량·편향.md)가 SSOT다. 여기서는 그 상세를 반복하지 않고 Beckwith 책 고유의 두 포인트만 덧붙인다. 첫째, lock elision은 escape analysis로 한 스레드를 벗어나지 않는 lock을 제거할 때 값을 register에 보관하는 scalar replacement와 함께 작동한다. 둘째, biased locking은 bias revocation 지연이 스레드 풀 환경에서 복잡성을 더해 **JDK 15에서 deprecated**됐다 — 이 폐기가 다음 절 JEP 143 contended locking 개선의 배경이다.
...
> spin-wait는 곧 풀릴 lock을 busy-wait로 기다리는 기법이다. `Thread.onSpinWait()`가 PAUSE 명령을 intrinsic으로 불러, spin 중의 power 소비와 pipeline flush 페널티를 줄인다.
...
> virtual thread는 JVM이 관리하는 경량 thread로, carrier thread 위에 스케줄된다. blocked되면 continuation으로 상태를 저장했다 재개하므로, 실제 작업할 때만 자원을 쓴다.
...
Spring 진영에서는 Spring Boot 3.2부터 `spring.threads.virtual.enabled=true` 한 줄로 이 모델을 받아들였다. 이 플래그를 켜면 Tomcat·Jetty의 요청 처리와 `@Async`가 carrier에 묶인 platform thread 대신 virtual thread 위에서 돈다. 그래서 thread-per-request 모델이라도 blocking I/O(DB·외부 API 호출)가 carrier를 점유하지 않아, 적은 OS thread로 훨씬 많은 동시 요청을 받는다. 다만 JEP 444가 경고한 carrier pinning은 Spring에서도 유효하다 — `synchronized` 블록 안에서 blocking이 일어나면 virtual thread가 carrier에 고정돼 이점이 사라지므로, 경쟁 구간은 `ReentrantLock`으로 바꾸는 편이 안전하다.



## 8. 면접 대비 요약

> thin lock ↔ heavyweight lock의 inflate, JEP 143으로 측정된 37% 개선, virtual thread가 carrier 위에서 continuation으로 도는 원리 — 이 셋을 자기 말로 풀 수 있으면 이 장은 끝이다.
...
- [`../ch02_automatic-memory-management/05-03.문자열 런타임 최적화`](../ch02_automatic-memory-management/05-03.문자열%20런타임%20최적화.md) — 같은 책 7장 전반부: string pool·dedup·indy-fication·compact string
- [`../ch01_java-tech/01-01.Java와 JVM의 성능 진화사`](../ch01_java-tech/03-01.Java와%20JVM의%20성능%20진화사.md) — contended locking·virtual thread 도입 연대기
- [`./06-02.동기화와 NUMA, JMH 벤치마킹`](./06-02.동기화와%20NUMA,%20JMH%20벤치마킹.md) — JMH·atomic·happens-before·CAS
- [`../ch02_automatic-memory-management/02-07.저지연 가비지 컬렉터`](../ch02_automatic-memory-management/02-07.저지연%20가비지%20컬렉터.md) — 동시성과 맞물리는 저지연 GC
- [`./02-03.락 최적화 — 스핀·제거·굵게·경량·편향`](./02-03.락%20최적화%20—%20스핀·제거·굵게·경량·편향.md) — §3 락 최적화 5기법(편향·경량·제거·거칠게·적응적 스핀)의 《밑바닥》 정독 SSOT
- [`./05-02.Virtual Threads 기초`](./05-02.Virtual%20Threads%20기초.md) — §7 virtual thread·continuation의 《자바 동시성》 쪽 기초(carrier·mount/unmount)
- [`./05-02.Virtual Threads 기초`](./05-02.Virtual%20Threads%20기초.md) — §7 carrier pinning의 발생 조건·감지·해결(§11 Pinning)
- [`./03-02.생산자-소비자 패턴`](./03-02.생산자-소비자%20패턴.md) — §3·§6 ReentrantLock·Condition·StampedLock의 실무 사용처
