...
HotSpot VM 레벨에서는 네 가지 최적화가 있다:

1. Biased locking(Java 6). CAS로 thread ID를 객체 header에 embed해, 같은 thread가 반복해 lock을 잡을 때 atomic 연산을 없앴다. 그러나 다른 thread가 끼어들 때의 bias revocation 지연이 복잡성을 더해 JDK 15에서 deprecated됐다.
2. Lock elision. escape analysis로 lock 객체가 method·thread를 벗어나지 않음이 확인되면 lock 자체를 제거하고 값을 register에 보관한다(scalar replacement). 경쟁이 원천적으로 불가능한 lock을 지우는 것이다.
3. Lock coarsening. 같은 객체에 대한 다중 synchronized를 단일 coarse lock으로 합친다. 예컨대 loop 안에서 매 반복 잡고 풀던 lock을 loop 밖으로 끌어내, 획득·반환 횟수를 줄인다.
4. Adaptive spinning(Java 7). locked monitor를 만난 thread가 즉시 wait(context switch)하는 대신 잠깐 spin(busy-wait)해, 곧 풀릴 lock이면 비싼 park/unpark 없이 바로 잡게 한다. 멀티프로세서에서 효과가 크다.
