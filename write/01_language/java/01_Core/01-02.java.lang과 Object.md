---
title: java.lang과 Object
tags: []
status: draft
source:
  - https://coding-bird.tistory.com/24
...
related:
  - ../05_JVM/ch03_class-loading-mechanism/02-04.클래스%20로더와%20부모%20위임%20모델.md
updated: 2026-06-22
---
...
- 이 설계 덕분에 `print()` 메서드는 새 클래스가 추가되어도 수정할 필요가 없다. `toString()`만 오버라이딩하면 기존 코드가 새 동작을 자동으로 인식한다.
- 이것이 OCP(개방-폐쇄 원칙)가 `Object`에서 자연스럽게 실현되는 방식이다.
...
- 메서드 오버로딩(overloading)은 컴파일타임에 매개변수 타입으로 어떤 메서드를 호출할지 결정한다.
- 매개변수 타입이 `Object`인 메서드와 `String`인 메서드가 있을 때, `Object` 변수에 `String`을 담아 전달하면 `Object` 버전이 호출된다. 오버로딩과 오버라이딩을 혼동하지 않도록 주의해야 한다.

### 3-3. 참조 동일성과 타입 판정

`==`, `equals()`, `instanceof`, 캐스팅은 객체를 다루지만 서로 다른 질문을 던진다. 두 참조가 같은 객체를 가리키는지, 두 객체를 논리적으로 같다고 볼지, 현재 객체를 특정 타입으로 다룰 수 있는지를 구분해야 한다.

| 표현 | 판정하는 것 | 결과 | `null` 처리 |
|------|-------------|------|-------------|
| `a == b` | 두 참조가 같은 객체나 배열을 가리키는가 | `boolean` | 두 값이 모두 `null`이면 `true` |
| `a.equals(b)` | `a` 타입이 정의한 논리적 동등성을 만족하는가 | `boolean` | `a`가 `null`이면 `NullPointerException` |
| `a instanceof T` | `a`를 `T`로 안전하게 캐스팅할 수 있는가 | `boolean` | 항상 `false` |
| `(T) a` | `a`를 `T` 참조로 다루겠다고 런타임에 검사하는가 | `T` 타입 참조 | `null` 참조로 성공 |
| `a.getClass() == T.class` | `a`의 정확한 런타임 클래스가 `T`인가 | `boolean` | `NullPointerException` |

- `Object.equals()`의 기본 구현은 `==`처럼 참조 동일성을 비교한다. 하지만 `String`, `Integer`, `record`처럼 `equals()`를 재정의한 타입은 필드 값으로 논리적 동등성을 판정한다. 따라서 `equals()`를 "주소 비교"로 정의하면 재정의 후의 동작을 설명할 수 없다. 
- 호출자가 `null`일 수 있다면 `a.equals(b)` 대신 `Objects.equals(a, b)`를 쓴다.

다음 예제는 네 판정의 차이를 한 번에 보여 준다.
...
Object first = new Dog();
Object alias = first;
Object anotherDog = new Dog();

System.out.println(first == alias);           // true: 같은 객체
System.out.println(first == anotherDog);      // false: 서로 다른 객체
System.out.println(first.equals(anotherDog)); // false: Object 기본 구현

System.out.println(first instanceof Animal);  // true: Dog는 Animal의 하위 타입
System.out.println(first instanceof Dog);     // true
System.out.println(first instanceof Cat);     // false

Dog dog = (Dog) first; // 성공
Cat cat = (Cat) first; // ClassCastException
```

- `instanceof`와 참조 캐스팅은 같은 타입 호환성을 검사한다. 차이는 실패 처리다. `a instanceof T`는 `(T) a`가 `ClassCastException` 없이 성공할 수 있을 때 `true`를 반환하고, 호환되지 않으면 `false`를 반환한다. 캐스팅은 객체를 새로 만들거나 실제 타입을 바꾸지 않고, 이미 있는 객체를 목표 타입의 참조로 다룰 수 있는지만 검사한다.

Java 16 이후에는 패턴 매칭 `instanceof`로 검사와 바인딩을 하나로 합칠 수 있다.

```java
if (first instanceof Dog dog) {
    // dog은 이 블록 안에서 Dog 타입으로 안전하게 사용할 수 있다.
}
```

`instanceof`는 하위 타입까지 허용하지만 `getClass()` 비교는 정확히 같은 클래스만 허용한다. 상속 가능한 클래스의 `equals()`에 `instanceof`를 쓰면 부모와 자식 사이의 비교를 허용하므로, 자식이 새 상태를 추가하면 대칭성이나 추이성을 깨뜨릴 수 있다. 값 객체는 `final`로 닫거나 `record`로 만들면 이 문제를 피할 수 있다.

끝으로 JVM은 타입을 이진 이름만으로 판정하지 않고 그 클래스를 정의한 클래스 로더도 함께 본다. 이름과 바이트코드가 같아도 정의 로더가 다르면 `instanceof`는 `false`, 캐스팅은 `ClassCastException`으로 끝난다. 이 경계는 [클래스 로더와 부모 위임 모델](../../book/Inside%20the%20Java%20Virtual%20Machine%20JVM%20Advanced%20Features%20and%20Best%20Practices/ch03_class-loading-mechanism/02-04.%ED%81%B4%EB%9E%98%EC%8A%A4%20%EB%A1%9C%EB%8D%94%EC%99%80%20%EB%B6%80%EB%AA%A8%20%EC%9C%84%EC%9E%84%20%EB%AA%A8%EB%8D%B8.md)에서 심화한다.
...
> 객체 동등성의 구체적인 적용과 JVM 타입 동일성을 이어서 읽는다.

- [불변 객체와 Record](02-02.불변%20객체와%20Record.md) — 값 객체의 `equals()`와 `hashCode()` 구현 방법
- [String 클래스](02-03.String%20클래스.md) — 문자열의 `==`와 `equals()` 결과가 갈리는 실제 사례
- [클래스 로더와 부모 위임 모델](../../book/Inside%20the%20Java%20Virtual%20Machine%20JVM%20Advanced%20Features%20and%20Best%20Practices/ch03_class-loading-mechanism/02-04.%ED%81%B4%EB%9E%98%EC%8A%A4%20%EB%A1%9C%EB%8D%94%EC%99%80%20%EB%B6%80%EB%AA%A8%20%EC%9C%84%EC%9E%84%20%EB%AA%A8%EB%8D%B8.md) — 이름이 같아도 로더가 다르면 타입이 갈리는 이유
