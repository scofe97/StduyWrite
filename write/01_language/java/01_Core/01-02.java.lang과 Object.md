...
끝으로 JVM은 타입을 이진 이름만으로 판정하지 않고 그 클래스를 정의한 클래스 로더도 함께 본다. 이름과 바이트코드가 같아도 정의 로더가 다르면 `instanceof`는 `false`, 캐스팅은 `ClassCastException`으로 끝난다. 이 경계는 [클래스 로더와 부모 위임 모델](../../book/05_JVM/ch03_class-loading-mechanism/02-04.클래스%20로더와%20부모%20위임%20모델.md)에서 심화한다.
...
- [클래스 로더와 부모 위임 모델](../../book/05_JVM/ch03_class-loading-mechanism/02-04.클래스%20로더와%20부모%20위임%20모델.md) — 이름이 같아도 로더가 다르면 타입이 갈리는 이유
