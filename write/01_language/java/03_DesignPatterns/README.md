...
> **클린 코드 원칙은 언어중립 카테고리로 이관됐다** — 네이밍·함수·주석 같은 줄 단위 미시 규칙은 Java에 종속되지 않으므로 [`../_writing/01-01.클린 코드 원칙`](../../_writing/01-01.클린%20코드%20원칙.md)에 둔다. 이 폴더는 패턴(01-)과 *Java 코드 스타일*(02-01·02-03)에 집중한다.
...
처음 보는 사람은 SOLID → 생성 패턴 → 구조 패턴 → 행동 패턴 → ([클린 코드](../../_writing/01-01.클린%20코드%20원칙.md), 언어중립 카테고리) → 일급객체 사상 순서로 읽으면 인지 부담이 적다. 패턴 카탈로그를 먼저 익혀 *언어*를 확보한 다음, 클린 코드와 일급객체 사상으로 *문장*을 쓰는 흐름이다.

02-01 다음에 02-03을 읽어 패턴 도입의 *함정 지도*를 같이 잡으면 좋다. 함수형 패턴이 트랜잭션·프록시·자원 수명과 만났을 때 어디서 깨지는가를 다른 카테고리 문서와 연결해 보여준다.

설계 토론이나 면접 직전 점검이 목적이라면 역순도 가능하다. 02-01 §6 적용 우선순위 표와 [클린 코드 §12](../../_writing/01-01.클린%20코드%20원칙.md) 패턴 도입 신호만 훑어도 즉시 활용 가능한 판단 기준이 잡힌다.
