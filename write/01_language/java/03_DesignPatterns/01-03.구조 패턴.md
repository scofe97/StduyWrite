...
한 줄로 요약하면 "복잡한 하위 시스템을 단순한 인터페이스 뒤로 숨기는 패턴"이다. 퍼사드는 서브시스템의 복잡성을 숨기는 것이지, 서브시스템 기능을 제한하는 것이 아니다. 필요하다면 클라이언트가 서브시스템에 직접 접근하는 경로도 열어둘 수 있다. 컴퓨터 부팅 과정이 전형적인 예시다. 사용자는 전원 버튼 하나(`start()`)만 누르면 되고, 내부에서 CPU, Memory, HardDrive가 협력하는 복잡한 과정은 몰라도 된다.
...
한 줄로 보면 "호환되지 않는 인터페이스를 가진 라이브러리나 API를 함께 작동하도록 연결하는 패턴"이다. 어댑터는 기존 코드를 수정하지 않고 서로 다른 인터페이스를 연결한다. 레거시 시스템 통합, 외부 라이브러리 래핑에 자주 쓰인다.
