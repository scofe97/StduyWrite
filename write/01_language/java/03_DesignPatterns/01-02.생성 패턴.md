...
한 줄 정의로 압축하면 "생성 로직을 별도의 클래스로 분리해 복잡성을 캡슐화하는 패턴"이다. 클라이언트 코드가 `new ConcreteClass()`를 직접 호출하면 그 구현체에 강하게 묶인다. 팩토리 메서드는 이 생성 결정권을 서브클래스로 위임해 DIP를 실현한다. 피자 가게 비유가 직관적이다. 본사(`PizzaStore`)는 주문 프로세스를 정의하고, 각 지점(`NYPizzaStore`, `ChicagoPizzaStore`)이 어떤 피자를 만들지 결정한다.
...
빌더의 본질은 "복잡한 객체를 단계별로 구축할 수 있게 해 코드 가독성을 높이는 것"이다. 불변 객체는 생성자에 모든 필드를 넘겨야 하는데, 선택적 필드가 많아지면 *텔레스코핑 생성자(telescoping constructor)* 문제가 생긴다. 빌더는 각 단계를 메서드로 표현해 가독성과 불변성을 동시에 달성한다.
...
가장 흔한 적용 예가 로거다. 애플리케이션 전체가 같은 로거 인스턴스를 공유해야 로그 출력이 일관되게 모이고, 인스턴스를 반복 생성할 비용도 사라진다. 멀티스레드 환경에서의 초기화 경쟁 조건을 피하는 것이 핵심 구현 과제다. Java에서는 *enum 방식*이 리플렉션 공격과 역직렬화에 모두 안전한 가장 견고한 구현으로 권장된다.
