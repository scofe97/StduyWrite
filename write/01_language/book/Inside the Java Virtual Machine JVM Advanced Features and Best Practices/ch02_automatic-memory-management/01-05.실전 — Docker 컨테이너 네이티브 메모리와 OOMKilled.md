...
status: draft
source:
  - Philip Park, "JVM heap은 멀쩡한데 왜 메모리가 터질까 — Docker 환경 네이티브 메모리 삽질기" (Medium, @philip_park, 2026-06 조회)
  - https://medium.com/@philip_park/jvm-heap은-멀쩡한데-왜-메모리가-터질까-docker-환경-네이티브-메모리-삽질기-part-1-4d48d788e1cc   # Part 1 — 힙 밖 네이티브·cgroup RSS·NMT 사각지대
  - https://medium.com/@philip_park/jvm-heap은-멀쩡한데-왜-메모리가-터질까-docker-환경-네이티브-메모리-삽질기-part-2-c4d782935a79   # Part 2 — Inflater 누수·jemalloc/async-profiler·완화 실측
...
> "힙만 한도 안이면 안전하다", "OOM이면 자바 예외가 뜬다" — 컨테이너 메모리에서 반복되는 오해를 바로잡는다.
...
> 힙 밖 네이티브 누수 진단·NMT의 한계·자원 누수 예방을 묻는 실전 질문으로 이해를 점검한다.
...
- [`08-02.Native Memory Tracking — NMT와 shared library 한계`](../../jpf_java-performance/08-02.Native%20Memory%20Tracking%20—%20NMT와%20shared%20library%20한계.md) — NMT 영역별 해석·`MALLOC_ARENA_MAX` 원리·Inflater/direct buffer 이론
...
이 노트의 사례·명령어·실측 수치는 아래 Medium 2부작을 흡수한 것이다.

- [Philip Park — Docker 환경 네이티브 메모리 삽질기 Part 1](https://medium.com/@philip_park/jvm-heap은-멀쩡한데-왜-메모리가-터질까-docker-환경-네이티브-메모리-삽질기-part-1-4d48d788e1cc) — 힙 밖 네이티브·cgroup RSS·OOM Killer·NMT 사각지대(9GB vs 14GB)
- [Philip Park — Docker 환경 네이티브 메모리 삽질기 Part 2](https://medium.com/@philip_park/jvm-heap은-멀쩡한데-왜-메모리가-터질까-docker-환경-네이티브-메모리-삽질기-part-2-c4d782935a79) — 암묵적 Inflater 누수·jemalloc/jeprof + async-profiler·try-with-resources·완화 실측표
