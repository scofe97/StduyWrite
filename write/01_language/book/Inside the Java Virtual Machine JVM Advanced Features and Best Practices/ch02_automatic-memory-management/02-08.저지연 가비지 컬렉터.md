...
운영에서 자주 만지는 한 가지가 *메모리 반환*이다. ZGC는 쓰지 않는 힙을 OS에 돌려줘 JVM 풋프린트를 줄인다.

- `-XX:+ZUncommit` — 미사용 힙을 OS에 반환하는 기능. **기본 활성화**다.
- `-XX:ZUncommitDelay=<초>` — 얼마나 오래 미사용이어야 반환 대상이 되는지. **기본 300초(5분)**. commit/uncommit은 비싼 연산이라, 값을 낮추면 반환은 빨라지지만 곧 다시 commit해야 할 위험이 커진다.
- `-XX:+UseLargePages` — 큰 페이지를 써 TLB 미스를 줄여 성능을 높인다(큰 힙에서 특히).

> 컨테이너처럼 *메모리를 다른 프로세스와 나눠 쓰는* 환경에서 ZUncommit이 실질적 이득을 준다. 다만 ZGC의 워크로드별 튜닝·`SoftMaxHeapSize` 같은 상세는 [05-02 ZGC 심화](./05-02.ZGC%20%EC%8B%AC%ED%99%94%EC%99%80%20%EC%9B%8C%ED%81%AC%EB%A1%9C%EB%93%9C%EB%B3%84%20GC%20%EC%84%A0%ED%83%9D.md)가 다룬다. 위 값은 [Oracle GC Tuning Guide](https://docs.oracle.com/en/java/javase/21/gctuning/z-garbage-collector.html)·[OpenJDK ZGC Wiki](https://wiki.openjdk.org/spaces/zgc/pages/34668579/Main)로 확인했다.
