...
source:
  ...
  - https://openjdk.org/jeps/192   # JEP 192: String Deduplication in G1
status: draft
related:
...
> 불변이고 곳곳에서 중복되는 문자열을 pool에 한 번만 저장한다. `intern()`으로 같은 값을 공유하면 메모리를 아끼고, 참조가 같아져 `==` 비교까지 빨라진다.
...
> G1이 GC 도중 동일한 `char[]`를 가진 String들을 찾아 backing array를 하나로 합친다. 메모리를 줄이지만 dedup 연산이 CPU를 쓰므로 trade-off가 있고, 기본 비활성이라 명시적으로 켜야 한다.

dedup이 왜 필요한지는 메모리 측정이 말해 준다. JEP 192(String Deduplication in G1)의 동기에 따르면, 대규모 Java 애플리케이션의 live heap 중 평균 25%를 `String` 객체가 차지하고, 그 `String` 중 평균 13.5%가 서로 `equals`가 참인 중복이다. 중복이 생기는 흔한 경로는 `new String("MangKyu")`다. string pool에 들어가는 literal과 달리 `new String(...)`은 같은 내용이라도 매번 별도 인스턴스와 별도 backing array를 만들므로, `string1 == string2`는 거짓이고 `equals`만 참이다. 이렇게 내용은 같고 배열만 따로인 String이 heap에 쌓이는 것이 dedup이 노리는 낭비다(한 측정에서는 평균 문자열 길이 45자, String 객체 하나가 최소 24바이트를 차지한다고 본다).
...
그래서 무턱대고 켤 기능은 아니다. dedup 후보는 age threshold(기본 3) 이상 살아남은 long-lived String이라, 짧게 쓰이고 버려지는 문자열은 대상이 되기 전에 회수된다. 오늘날 많은 문자열은 클라이언트 요청이나 DB 조회로 그때그때 만들어졌다가 곧 사라지는 단명 객체라, 캐싱 없이 도는 서비스에서는 dedup 대상이 거의 없어 효과가 미미하고 marking·dedup 비용으로 pause time만 늘 수 있다. 반대로 같은 내용의 문자열을 오래 들고 있는 캐시·사전·설정처럼 long-lived 중복이 많은 워크로드에서는 메모리 절감이 크다. 결국 `-Xlog:stringdedup*`로 실제 dedup 비율을 측정해 자기 서비스에 효과가 있는지 확인한 뒤 켜는 것이 맞다.
...
1. empty·null 문자열이 개별 최적화돼 불필요한 연산을 우회한다. concat 전략을 런타임이 고르므로, 자주 나오는 특수 케이스에 전용 경로를 둘 수 있다.
2. `StringBuilder`의 내부 버퍼 크기를 미리 계산할 필요가 없어진다. JVM이 결과 문자열의 최적 크기를 동적으로 정하므로, 연결할 문자열 길이가 예측 불가일 때 특히 유리하다.
3. 코드가 미래 JVM 개선에 대비된다. `invokedynamic`의 유연성 덕에 JVM이 새 concat 최적화를 들일 때마다 bytecode나 소스 수정 없이 자동으로 그 혜택을 받는다.
...
> JDK 9는 String의 backing array를 `char[]`에서 `byte[]`로 바꿔, Latin-1 문자를 2바이트가 아닌 1바이트로 저장한다. Java 6의 compressed strings와 달리 decompression이 없어 연산까지 그대로 혜택을 본다.
...
> string pool/dedup의 중복 제거, indy-fication이 concat 결정을 런타임으로 미룬다는 점, compact string의 `byte[]` 전환 — 네 최적화의 핵심을 자기 말로 설명할 수 있으면 이 장은 끝이다.
...
- `../ch05_efficient-concurrency/06-03.락과 동시성 — 동기화부터 Virtual Threads까지` <!-- 링크 끊김(2026-08): ../ch05_efficient-concurrency/06-03.락과 동시성 — 동기화부터 Virtual Threads까지.md --> — 같은 책 7장 후반부: monitor lock·contended locking·virtual thread
- [`../ch01_java-tech/01-01.Java와 JVM의 성능 진화사`](../ch01_java-tech/03-01.Java와%20JVM의%20성능%20진화사.md) — invokedynamic·String Deduplication·JIT 도입
- [`./05-01.TLAB·PLAB·NUMA-aware GC와 G1 심화`](./05-01.TLAB·PLAB·NUMA-aware%20GC와%20G1%20심화.md) — G1 dedup이 동작하는 evacuation·marking phase
- [`../ch03_class-loading-mechanism/01-02.바이트코드 명령어`](../ch03_class-loading-mechanism/01-02.바이트코드%20명령어.md) — invokedynamic 등 bytecode 명령
