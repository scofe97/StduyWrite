...
related:
  ...
  - ../ch02_automatic-memory-management/01-01.런타임 데이터 영역.md
...
---
> 성능은 기능 요구사항과 나란히 서는 핵심 품질 속성(ilities)으로, SUT가 UoW를 실행하는 효율을 footprint·responsiveness·throughput·availability 네 메트릭으로 잰다. 그 측정은 하드웨어 cache 계층과 메모리 모델(순차 일관성 vs relaxed) 위에서만 제대로 해석되며, STW pause는 그 효율을 정량으로(Acw·Wda·Wdb) 깎는다.
...
> STW pause는 모든 스레드를 멈춰 연속 작업을 끊는다. 그 결과 응답시간이 늘고 throughput이 떨어지는데, 이를 Acw·Wda·Wdb로 정량화할 수 있다. 평균이 아니라 worst-case를 봐야 한다.
...
> 성능은 언어·프로세서·메모리 모델의 조화다. cache 계층을 이해해 정렬·prefetching으로 miss를 줄이고 false sharing을 피하는 것이 하드웨어를 의식한 프로그래밍의 출발점이다.
...
성능은 프로그래밍 언어·프로세서·메모리 모델의 조화다. 메모리 모델은 스레드가 메모리 연산을 어떻게 인식하는지를 규율하며, 일관성과 성능 사이에서 균형을 잡는다. Java의 메모리 모델은 일관성을 위해 특정 연산 순서를 강조해 성능을 일부 희생하는 반면, C++11 이후는 더 fine-grained한 메모리 순서 제어를 준다.

하드웨어를 의식한 프로그래밍은 cache 계층을 이해하는 데서 출발한다. 핵심 손잡이는 네 가지다:

1. cache line 정렬. 자료구조를 cache line 경계에 맞춰 배치해 한 번의 적재로 더 많은 유효 데이터를 가져와 miss를 줄인다.
2. true sharing 관리. 여러 코어가 실제로 함께 쓰는 데이터를 같은 line에 모아 접근을 효율화한다.
3. false sharing 회피. 무관한 데이터가 우연히 같은 line에 놓이면 한쪽 갱신이 line 전체를 invalidate한다 — padding으로 분리해 막는다.
4. HW prefetching 활용. 곧 쓸 데이터를 미리 당겨와, spatial·temporal locality가 좋은 접근 패턴에서 대기를 줄인다.
...
> SMT는 한 코어에 여러 하드웨어 스레드를 태워 throughput을 높이되 contention을 부른다. cache 계층과 out-of-order 실행이 CPU를 메모리 대기에서 풀어 주고, 아키텍처별 메모리 일관성(TSO vs relaxed)이 동기화 부담을 가른다.
...
> 네 메트릭(footprint·responsiveness·throughput·availability), STW의 정량 영향(Acw·Wda·Wdb), 메모리 모델과 성능의 trade-off — 이 셋을 자기 말로 풀 수 있으면 이 장은 끝이다.
...
- [`./05-02.동기화와 NUMA, JMH 벤치마킹`](./05-02.동기화와%20NUMA,%20JMH%20벤치마킹.md) — 같은 장 후반부: barrier·fence·volatile·NUMA·JMH
- [`../ch01_java-tech/01-01.Java와 JVM의 성능 진화사`](../ch01_java-tech/03-01.Java와%20JVM의%20성능%20진화사.md) — GC·STW·JIT 기초
- [`../ch02_automatic-memory-management/01-01.런타임 데이터 영역`](../ch02_automatic-memory-management/01-01.런타임%20데이터%20영역.md) — Java heap·native memory 영역
- [`../ch02_automatic-memory-management/04-01.최적화 사례 분석`](../ch02_automatic-memory-management/04-01.최적화%20사례%20분석.md) — 실전 성능 최적화 사례
