...
- 실무로 옮기면 케이스 B는 *부모가 소유한 공유 계약을 자식도 따로 정의했을 때* 생기는 일반적인 타입 분리를 재현합니다. Tomcat의 Servlet·JSP·EL·WebSocket API는 부모 우선 예외라 웹앱 저장소에서 임의로 대체되지 않습니다. 외부 WAR에서 컨테이너 제공 API를 패키징하지 않는 규칙은 [패키지·JAR·클래스패스 §6](../../../java/06_Build/01-01.패키지·JAR·클래스패스.md)에서 배포 형태별로 구분합니다.
