...
### 3.1 layers.idx와 Docker 레이어 캐시 — COPY 4분할

> fat JAR·shaded JAR·slim(thin) JAR가 각각 무엇이고 의존성을 어떻게 담는지는 빌드 관점의 [웹 애플리케이션 빌드 — WAR와 실행 JAR §2.1](../../../java/06_Build/01-02.웹%20애플리케이션%20빌드%20—%20WAR와%20실행%20JAR.md)이 SSOT입니다. 이 절은 그 fat JAR를 컨테이너 레이어로 쪼개는 각도만 다룹니다.

`layers.idx`가 실제 Dockerfile에서 어떻게 쓰이는지 짚으면, 이 색인이 왜 JAR 안에 들어 있는지가 분명해집니다. 

- fat jar를 `COPY app.jar` 한 줄로 이미지에 넣으면 **JAR 전체가 한 레이어**가 됩니다. 그러면 내 코드 몇 KB만 고쳐도 무거운 의존성까지 같은 레이어에 묶여 통째로 다시 빌드됩니다. 
- Docker 레이어 캐시는 한 레이어가 바뀌면 그 위 레이어까지 전부 무효화하므로(캐시 미스가 위로 전파), 한 덩어리 JAR는 캐시 효율이 나쁩니다.

`layers.idx`는 JAR 내용물을 **변경 빈도별로 어느 레이어에 넣을지** 미리 분류해 둔 색인입니다. 빌드 단계에서 `java -Djarmode=layertools -jar app.jar extract`로 네 디렉터리를 뽑아, 잘 안 바뀌는 것부터 순서대로 COPY합니다.

```dockerfile
# layertools로 추출한 결과를 안 바뀌는 것 → 자주 바뀌는 것 순서로 COPY
COPY --from=build dependencies/          ./   # ① 릴리스 의존성 (거의 안 바뀜) ← CACHED 유지
COPY --from=build spring-boot-loader/    ./   # ② 로더 (안 바뀜)
COPY --from=build snapshot-dependencies/ ./   # ③ SNAPSHOT 의존성
COPY --from=build application/           ./   # ④ 내 코드 (자주 바뀜) ← 여기만 재빌드
ENTRYPOINT ["java", "org.springframework.boot.loader.launch.JarLauncher"]
```

네 디렉터리는 앞서 본 JAR 내부를 변경 빈도로 재분류한 것입니다 — `dependencies/`는 `BOOT-INF/lib`의 정식 릴리스 의존성(거의 안 바뀜), `snapshot-dependencies/`는 SNAPSHOT 버전만 따로, `spring-boot-loader/`는 로더 클래스, `application/`은 `BOOT-INF/classes`의 내 코드(가장 자주 바뀜)입니다. 그 결과 내 코드만 고치면 맨 뒤 `application/` 레이어만 다시 만들어지고, 무거운 `dependencies/` 레이어는 캐시로 살아남습니다.

> 여기서 `ENTRYPOINT`가 `java -jar app.jar`가 아니라 `JarLauncher`를 직접 부르는 이유는, `extract`로 JAR를 **미리 펼쳐** 이미지에 `app.jar` 파일 자체가 없기 때문입니다. `java -jar`는 열 JAR 파일이 있어야 하지만, 펼쳐진 디렉터리 구조에서는 §4~5에서 볼 `JarLauncher`를 진입점으로 직접 실행합니다. 실행 주체(`JarLauncher`)는 `java -jar`일 때와 같고, 대상만 "압축된 JAR" → "펼쳐진 디렉터리"로 바뀝니다.
...
> 실습: `META-INF/services` 를 손으로 작성하고 `ServiceLoader` 로 발견하는 과정, 그리고 등록을 지웠을 때 구현체가 안 보이는 것까지 직접 확인하려면 `~/jvm-practice/spi-demo` 를 봅니다. `--args="manual"` 로 실행하면 `getResources()` 로 등록 파일을 직접 읽어 `ServiceLoader` 의 내부를 재현합니다. 클래스 로더 관점의 같은 주제는 [04-01 §6 TCCL](./04-01.톰캣의%20클래스%20로더%20아키텍처.md)에서 다룹니다.
...
> 각 항목을 스스로 설명할 수 있으면 `java -jar` 실행 흐름과 등록 파일 기반 발견 흐름을 섞지 않고 말할 수 있습니다. §9 핵심 포인트와 겹치지 않는 관측 포인트만 추렸습니다.
...
- [웹 애플리케이션 빌드 — WAR와 실행 JAR](../../../java/06_Build/01-02.웹%20애플리케이션%20빌드%20—%20WAR와%20실행%20JAR.md) — 이 글이 다루는 `BOOT-INF` 구조를 빌드 도구가 어떻게 만드는지(빌드 관점의 짝 문서)
- [Kubernetes in Action 02-02. Kiada 애플리케이션 빌드와 배포](../../../../08_cloud/book/kubernetes-in-action/02-02.Kiada%20%EC%95%A0%ED%94%8C%EB%A6%AC%EC%BC%80%EC%9D%B4%EC%85%98%20%EB%B9%8C%EB%93%9C%EC%99%80%20%EB%B0%B0%ED%8F%AC.md) § "Layered JAR" — 이 글의 §3.1 레이어 캐시를 Docker 이미지 빌드 실습 맥락에서 다룬 편
