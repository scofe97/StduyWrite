...
source:
  ...
  - https://www.itworld.co.kr/article/3568004/%EB%A6%AC%ED%94%8C%EB%A0%89%EC%85%98%EB%B3%B4%EB%8B%A4-%EB%82%AB%EB%8B%A4-%EC%9E%90%EB%B0%94%EC%9D%98-%EB%A9%94%EC%86%8C%EB%93%9C-%ED%95%B8%EB%93%A4%EA%B3%BC-%EB%B3%80%EC%88%98-%ED%95%B8%EB%93%A4.html
related:
...
`MethodHandle`은 메서드를 가리키는 *핸들*로, `invoke*` 바이트코드 명령을 코드 수준에서 흉내 냅니다. 이름 때문에 `MethodHandler`처럼 기억하기 쉽지만, 공식 클래스명은 `java.lang.invoke.MethodHandle`입니다. 리플렉션의 `Method`와 비슷해 보이지만 결이 다릅니다.

먼저 "핸들이 메서드의 메모리 주소를 들고 있나?"부터 정리해야 합니다. 자바 코드에서 보는 `MethodHandle` 객체 자체는 평범한 객체이므로 힙(heap)에 놓입니다. 하지만 그 객체가 실제 네이티브 코드 주소를 그대로 필드에 노출하는 것은 아닙니다. HotSpot 기준으로 `MethodHandle`은 내부적으로 JVM 메타데이터, 메서드 타입, 호출 어댑터 같은 정보를 가리키고, 최종적으로 인터프리터·JIT 컴파일러가 그 정보를 이용해 실제 호출 경로를 만듭니다.

따라서 학습 단계에서는 이렇게 나누면 안전합니다. *메서드의 정의 정보*는 클래스 로딩 후 메타스페이스 쪽 JVM 메타데이터로 관리되고, *`MethodHandle` 객체*는 힙에 있으며, *JIT가 만든 네이티브 기계어*는 코드 캐시(code cache)에 놓입니다. `MethodHandle`은 이 셋 중 "메서드 주소 변수"가 아니라, 힙에 있는 호출 설명서이자 JVM 내부 링크를 품은 객체라고 이해하는 편이 정확합니다.
...
처음 보면 위 코드가 낯선 이유는 "메서드를 호출한다"는 익숙한 문법 대신, 호출에 필요한 부품을 직접 조립하기 때문입니다. 읽는 순서는 아래처럼 잡으면 됩니다.

- `MethodType.methodType(void.class, String.class)`는 "반환값은 `void`, 인자는 `String` 하나"라는 메서드 모양을 만듭니다. 메서드 이름은 아직 없습니다.
- `lookup()`은 "내가 어떤 권한으로 메서드를 찾을 수 있는가"를 담은 조회 객체를 만듭니다. 일반 메서드 호출에도 접근 제어가 있듯, 핸들을 찾을 때도 `public`·`private`·패키지 접근 권한을 검사합니다.
- `findVirtual(receiver.getClass(), "println", mt)`는 수신자의 실제 클래스에서 `println(String):void` 가상 메서드를 찾습니다. 여기서 만들어진 핸들은 아직 "수신자 + 문자열"을 받아야 호출할 수 있는 형태입니다.
- `bindTo(receiver)`는 첫 번째 인자인 수신자를 미리 고정합니다. 그래서 결과 핸들은 문자열 하나만 받는 `println` 호출처럼 보입니다.
- `invokeExact("icyfenix")`는 그 핸들을 실제로 실행합니다. `invokeExact`는 핸들의 타입과 호출부 타입이 정확히 맞아야 하므로, 예제에서는 문자열 하나를 넘겨야 합니다.

`obj`가 `System.out`이든 `ClassA`든, `findVirtual`로 얻은 핸들이 수신자의 실제 타입에 맞는 `println`을 호출합니다. 예제의 `bindTo(receiver)`는 완전히 새 메서드를 만드는 것이 아니라, *기존 핸들을 변형해* 인자 하나가 줄어든 핸들을 돌려줍니다. 핵심 대비는 다음입니다.
...
### 참고 — MethodHandle은 리플렉션 대체재인가

실무 관점에서는 `MethodHandle`을 "리플렉션보다 새롭고 무조건 빠른 API"로만 외우면 위험합니다. 일반 메서드 호출로 해결되는 코드는 그대로 일반 호출을 쓰는 것이 가장 좋고, 프레임워크처럼 런타임에 클래스·메서드·필드를 찾아야 할 때 `MethodHandle`과 `VarHandle`을 먼저 검토합니다. 그래도 생성자 탐색, 어노테이션 조사, 클래스 구조 분석처럼 메타데이터 자체가 필요하면 여전히 리플렉션이 맞습니다.

`lookup()`도 단순한 검색 함수가 아닙니다. `MethodHandles.Lookup`은 "어떤 클래스의 권한으로 멤버를 찾는가"를 들고 있으며, 핸들을 만들 때 그 권한으로 접근 검사를 합니다. 그래서 `public` 메서드는 일반 `lookup()`으로 찾을 수 있지만, 다른 클래스의 `private` 필드나 메서드는 무작정 열리지 않습니다. 이런 경우에는 `MethodHandles.privateLookupIn(targetClass, lookup())`처럼 대상 클래스에 대한 private 조회 권한을 명시적으로 만들어야 합니다.

필드 쪽에는 `MethodHandle`의 짝처럼 `VarHandle`이 있습니다. `MethodHandle`이 메서드·생성자·필드 접근 같은 동작을 호출 형태로 다룬다면, `VarHandle`은 필드나 배열 요소 같은 *변수 위치*를 읽고 쓰는 표준 핸들입니다. 과거에는 private 필드 접근을 위해 `Field#setAccessible(true)` 같은 리플렉션 우회를 자주 썼지만, 현대 자바에서는 접근 권한을 명시한 `Lookup`과 `VarHandle` 조합이 더 표준적인 선택지입니다.
...
class Person {
    ...
    static {
        try {
            NAME = MethodHandles.privateLookupIn(Person.class, MethodHandles.lookup())
...
위 예제에서 `NAME`은 `Person.name`이라는 필드 위치를 가리키는 핸들입니다. `NAME.get(this)`는 "이 인스턴스의 `name` 값을 읽어라"라는 뜻이고, 정적 필드로 캐시해 두면 같은 클래스의 여러 인스턴스에 재사용할 수 있습니다. 핸들을 매번 찾는 비용이 사라지고 JVM이 호출 형태를 더 잘 최적화할 수 있으므로, 반복 호출 경로에서는 이런 캐싱이 중요합니다.

다만 성능 결론은 단순하지 않습니다. 핸들을 매번 조회해서 한두 번만 호출하면 리플렉션보다 항상 빠르다고 말하기 어렵고, 정적 상수처럼 재사용될 때 JIT 최적화의 이점을 얻기 쉽습니다. 또 `MethodHandle`·`VarHandle`은 인스턴스를 새로 만드는 범용 API가 아니므로, "클래스 이름만 문자열로 알고 객체를 만들고 구조를 훑는" 작업은 리플렉션 영역으로 남습니다.
...
`invokedynamic`은 등장인물이 셋(`invokedynamic` 명령·부트스트랩 메서드·`CallSite`)이라 역할이 섞이기 쉽습니다. 면접에서 헷갈리는 두 지점을 콕 집어 둡니다.

**① 부트스트랩 메서드 vs CallSite — *결정자*와 *그릇*은 다르다.** "`CallSite`가 호출 대상을 정한다"고 뭉뚱그리기 쉬운데, *정하는* 것은 부트스트랩 메서드이고 `CallSite`는 그 *결과를 담는 그릇*입니다.

| | 역할 | 언제 | 비유 |
|---|------|------|------|
| 부트스트랩 메서드 | "이 호출 지점에서 *무엇을 부를지*" 정하는 로직(언어 구현자가 작성) | 첫 호출 때 **한 번** | 처음 한 번 하는 *설정 작업* |
| `CallSite` | 그 결정 결과인 실제 호출 대상(`MethodHandle` target)을 *담는 객체* | 이후 호출이 여기로 직행 | 설정 결과를 저장한 *콘센트* |

흐름으로 끊으면 — `invokedynamic` 첫 실행 → ① 부트스트랩 메서드 1회 호출 → ② 그것이 `CallSite`를 반환(안에 target `MethodHandle`이 담김) → ③ 그 `MethodHandle`이 가리키는 메서드 실행 → 이후 같은 호출 지점은 부트스트랩을 다시 부르지 않고 `CallSite`의 target으로 직행합니다(§위 CallSite 생명주기). 그래서 *결정은 한 번, 호출은 매번 빠르게*가 됩니다.

**② `invokedynamic`이 앞 네 명령과 다른 핵심은 "대상을 *누가* 정하느냐"다.** `invokevirtual`·`invokeinterface`는 "수신자의 실제 타입을 따라간다"는 결정 규칙이 *JVM에 고정*돼 있어 자바든 코틀린이든 같은 규칙으로 풀립니다. 반면 `invokedynamic`은 그 결정 로직을 *언어(컴파일러)가 부트스트랩 메서드로 짜 넣고*, JVM은 첫 호출 때 그것을 한 번 불러 결과를 `CallSite`에 묶을 뿐입니다. 그래서 `invokedynamic`은 *언어가 JVM에 호출 의미를 주입하는 통로*입니다 — JVM이 모르는 동적 언어도 자기 호출 규칙을 심을 수 있고, 자바 람다도 이 틀을 빌려 씁니다.

**③ `MethodHandle`은 "호출 리모컨", 리플렉션 `Method`는 "신상명세서".** §1에서 본 대비를 한 줄로 굳히면 — 리플렉션 `Method`는 메서드의 *메타데이터 전체*(이름·반환·파라미터·예외·수식어)를 담는 *자바 언어 수준*의 무거운 객체이고, `MethodHandle`은 호출 *동작*만 담는 *바이트코드 수준*의 가벼운 핸들입니다. 결정적 차이는 `MethodHandle`이 *언어 무관*이라는 점입니다 — 리플렉션은 자바 전용이지만 `MethodHandle`은 JVM 위 어떤 언어든 쓸 수 있어, 동적 언어 지원의 토대가 됩니다. `CallSite`가 품는 target이 바로 이 `MethodHandle`이라, 세 개념(`invokedynamic`→`CallSite`→`MethodHandle`)이 한 줄로 꿰입니다.
