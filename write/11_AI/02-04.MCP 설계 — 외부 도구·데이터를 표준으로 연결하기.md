...
- Tools와 Resources의 차이가 시험 포인트입니다. Tools는 *행동*을 일으키고(부수효과 가능), Resources는 *데이터*를 읽기만 합니다(읽기 중심). 
- 이슈를 *만드는* 것은 Tool이고, 이슈 목록을 *읽는* 것은 Resource입니다.
...
- **Host**는 LLM 애플리케이션 자체입니다(예: Claude Code, IDE). Host 안에는 **Client**가 있고, 각 Client가 하나의 **Server**(외부 기능 제공자)에 연결합니다. 
- 하나의 Host는 여러 Server에 동시에 붙을 수 있습니다 — Jira 서버, DB 서버, 브라우저 서버를 한 Host가 함께 씁니다.
...
flowchart LR
    ...
    c1 -->|"Streamable HTTP"| s1["Server: mcp-atlassian<br>Tools: 이슈 생성<br>Resources: 이슈 조회"]
    c2 -->|"stdio"| s2["Server: context7<br>Resources: 라이브러리 문서"]
    c3 -->|"Streamable HTTP"| s3["Server: claude-in-chrome<br>Tools: 브라우저 조작"]

    note["1 Host = 여러 Server 동시 연결<br>Client는 Server마다 1개씩"]
    note -.-> host

    style user fill:#dceaf7,color:#0d0d0d
    style model fill:#e6f7e7,color:#0d0d0d
    style c1 fill:#f5edda,color:#0d0d0d
    style c2 fill:#f5edda,color:#0d0d0d
    style c3 fill:#f5edda,color:#0d0d0d
    style s1 fill:#ddd4f0,color:#0d0d0d
    style s2 fill:#ddd4f0,color:#0d0d0d
    style s3 fill:#ddd4f0,color:#0d0d0d
    style note fill:#fdf3df,color:#0d0d0d
```
...
MCP 서버에 붙으려면 OAuth 토큰 같은 자격증명이 필요합니다. 이 비밀을 에이전트 정의에 박으면 두 가지 문제가 생깁니다. 

1. 첫째, 정의를 재사용·버전 관리할 때 비밀이 함께 따라다닙니다. 
2. 둘째, 정의를 공유하면 비밀이 새어 나갑니다.
