...
2. 이 경로가 실제로 열려 있고 Cloud 정보가 읽히면 `K8S_CLOUD`로 판정
3. 경로가 `404`이거나 Cloud 정보를 읽지 못하면 `/computer/api/json`의 `totalExecutors` 조회
...
현재 실습 인스턴스에서는 Kubernetes plugin은 활성화되어 있지만 `/cloud/api/json`은 `404 Not Found`를 반환한다. 따라서 이 환경에서는 사실상 `/computer/api/json` fallback이 실질적인 capacity 판단 경로가 된다.
