#!/bin/bash
# =============================================================================
# E2E 테스트: GitLab MR 병합 → Workflow → Jenkins 배포
# =============================================================================
#
...
#   4. GitLab MR 병합 webhook 시뮬레이션 (→ Redpanda Connect)
#   5. 실행 결과 확인
# =============================================================================
...
GITLAB_WEBHOOK_URL="$CONNECT_URL/gitlab/webhook/gitlab"
...
# =============================================================================
# 3. 워크플로우 등록 (MR 병합 → deploy 파이프라인 실행)
# =============================================================================
...
# =============================================================================
# 4. GitLab MR 병합 시뮬레이션 (Redpanda Connect webhook)
# =============================================================================
step 4 "GitLab MR 병합 webhook 시뮬레이션"

WEBHOOK_CODE=$(curl -sf -o /dev/null -w "%{http_code}" \
    -X POST "$GITLAB_WEBHOOK_URL" \
    -H 'Content-Type: application/json' \
    -d '{
