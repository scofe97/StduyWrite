...
| `info` | string[] | | **창 내부에 상시 표시하는 설명 — 딱 한 문장.** 배열은 한 칸이고 그 안에도 문장 하나만 씁니다(빌더가 줄 수와 문장 수를 함께 셉니다). 이 노드가 *무엇을 하는가* 를 입문자가 읽고 이해할 수 있는 완전한 문장으로 씁니다(제1원칙 2·3항). 둘째 문장부터는 `detail` 로 보냅니다 — 창에 문장이 쌓이면 캔버스가 무거워지고 패널과 같은 말을 두 번 하게 됩니다. 예: `["라우팅을 정하기 전에 nat 테이블을 먼저 확인합니다."]` |
