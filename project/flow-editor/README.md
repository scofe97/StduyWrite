...
| 경로 | 역할 |
|---|---|
| `schema/flow-schema.md` | DSL 명세 정본 (v0.2) |
| `schema/flow.schema.json` | JSON Schema (외부 도구용 — 빌더는 자체 검증 내장) |
| `renderer/template.html` | HTML 골격. 마커 3곳에 빌드 시 인라인됨 |
| `renderer/flow-style.css` | 스타일 (팔레트·패널·인스펙터·메커니즘 UI) |
| `renderer/flow-renderer.js` | 렌더러 로직 (레이아웃·애니메이션·패널·인스펙터) |
| `examples/src/*.json` | 흐름도 원본 데이터 |
...
- **클릭 탐색** = 재생도 순번도 없음. 순서는 좌→우 배치가 알려준다. 노드/엣지/그룹 클릭 → 우측 패널(520px): 개요·여기서 일어나는 일·패킷 변형·내부 동작·연결(점프 버튼). 캔버스 엣지에는 텍스트 없음 — 서술은 전부 패널
