...
- **레인** = `meta.lanes` + 노드 `lane`으로 **행위 주체**를 교차축에 고정한다. 왕복이 있는 흐름(TLS 두 당사자, 리눅스 유저↔커널)에서 "누가 하는가"와 "되돌아옴"이 드러난다. 없으면 종전 배치 그대로
- **클릭 탐색** = 재생도 순번도 없음. 순서는 좌→우(또는 위→아래) 배치가 알려준다. 노드/엣지/그룹 클릭 → 우측 패널(520px). 캔버스 엣지에는 텍스트 없음 — 서술은 전부 패널
- **패널은 펼쳐서 열린다** = 왜 그런가·여기서 일어나는 일·패킷 변형·내부 동작·연결이 개수와 함께 처음부터 다 보인다. 제1원칙이 이해시키는 것이므로, 읽을 내용을 한 번 더 클릭해야 나오게 두면 그냥 안 읽는다. 섹션은 접을 수 있고(긴 것을 치워둘 때), **접은 섹션만 기억**해 노드를 옮겨 다녀도 그 선택이 유지된다
- **`info`와 `detail`의 역할이 다르다** = `info`는 "무엇을 하는가"(노드 창·패널 첫 줄 공용), `detail`은 **"왜 그런가"**. 같은 말을 두 번 쓰지 않는다
